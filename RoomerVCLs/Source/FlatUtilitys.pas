...
procedure CalcButtonLayout (Canvas: TCanvas; const Client: TRect; const Offs: TPoint; Layout: TButtonLayout; Spacing, Margin: Integer; FGlyph: TBitmap; FNumGlyphs: Integer; const Caption: string; var TextBounds: TRect; var GlyphPos: TPoint);
...
procedure CalcButtonLayout (Canvas: TCanvas; const Client: TRect; const Offs: TPoint; Layout: TButtonLayout;
...
begin
  ...
  with GlyphPos do
  begin
    Inc(X, Client.Left + Offs.X);
    Inc(Y, Client.Top + Offs.Y);
  end;
  OffsetRect(TextBounds, TextPos.X + Client.Left + Offs.X, TextPos.Y + Client.Top + Offs.X);
end;

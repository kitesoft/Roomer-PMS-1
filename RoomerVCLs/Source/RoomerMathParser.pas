...
type
  ...
  TRoomerMathParser = class(TComponent)
  private
...
constructor TRoomerMathParser.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TRoomerMathParser.Destroy;
begin
  fResultStack.Free;
  fPrefixStack.Free;
  Variables.Free;
...
function TRoomerMathParser.GetOperation(Index: Integer): TOperationInfo;
begin
  Result := OperationsInfo[Index];
end;

function TRoomerMathParser.GetOperationCount: Integer;
begin
  Result := Length(OperationsInfo);
end;

function TRoomerMathParser.GetOperationIndex(OperationName: String): Integer;
var i: Integer;
begin
  Result := -1;
...
function TRoomerMathParser.GetOperationInfo(OperationName: string): TOperationInfo;
var i: Integer;
begin
...
function TRoomerMathParser.RenameOperation(CurrentName, NewName: String): Boolean;
var i: Integer;
begin
  Result := false;
...
function TRoomerMathParser.AddCustomOperation(Name: String; Arguments: Integer; Priority: Byte): Boolean;
var l: Integer;
begin
  Result := false;
  if Name = '' then Exit;
...
function TRoomerMathParser.ValidVariableName(Name: string): Boolean;
var
 i: Integer;
begin
  Result:= false;
...
procedure TRoomerMathParser.SetExpression(const Value: String);
...
procedure TRoomerMathParser.StrToInfixStack(aExpression: String; aStack: TStack);
var
...
function TRoomerMathParser.ValidateInfixStack(aStack: TStack): Boolean;
var
...
procedure TRoomerMathParser.InfixToPreFix(Infix: TStack; Prefix: TStack);
var
  TmpStack: TStack;
  i: Integer;
  Current: TStackInfo;
...
function TRoomerMathParser.Parse: Boolean;
...
function TRoomerMathParser.GetParserResult: Extended;
begin
...
function TRoomerMathParser.GetLastError: Integer;
begin
  Result := fLastError;
end;

function TRoomerMathParser.GetLastErrorString: String;
begin

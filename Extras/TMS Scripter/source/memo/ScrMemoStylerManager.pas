...
uses
  Classes, Controls, Graphics, SysUtils, ScrMemo, INIFiles, Windows
  {$IFDEF DELPHIXE3_LVL}
  , System.UITypes
  {$ENDIF}
  ;
...
type
  ...
  TScrMemoStylerManager = class;

  TAdvMemoStylersCollectionItem = class(TCollectionItem)
  private
    FName: string;
    FFileName: string;
    FStyler: TScrCustomMemoStyler;
    FStylerIndex: integer;
    procedure SetName(const Value: string);
    procedure SetStyler(const Value: TScrCustomMemoStyler);
  protected
  ...
  published
    ...
    property Styler: TScrCustomMemoStyler read FStyler write SetStyler;
  end;
  ...
  private
    ...
    FOwner: TPersistent;
    {$ENDIF}
    FAdvMemoStylers: TScrMemoStylerManager;
  ...
  public
    constructor Create(Styler: TScrMemoStylerManager);
  ...
  {$ENDIF}
  TScrMemoStylerManager = class(TComponent)
  private
  ...
  public
    ...
    destructor Destroy; override;
    function GetStylerByFileName(FileName: string): TScrCustomMemoStyler;
    function GetStylerByName(strName: string): TScrCustomMemoStyler;
    function GetStyler(Idx: integer): TScrCustomMemoStyler;
    ...
    function GetStylerStyleIndexByInfo(styler: TScrCustomMemoStyler; Info: string): integer;
    procedure SaveAllStylersToFile(FileName: string);
    procedure LoadAllStylersFromFile(FileName: string);
    procedure SaveStylerToFile(Styler: TScrCustomMemoStyler; FileName: string);
    procedure LoadStylerFromFile(Styler: TScrCustomMemoStyler; FileName: string);
...
procedure TAdvMemoStylersCollectionItem.SetStyler(const Value: TScrCustomMemoStyler);
begin
  if Styler <> Value then
    FStyler := Value;
end;

constructor TAdvMemoStylersCollection.Create(Styler: TScrMemoStylerManager);
begin
...
begin
  ...
  TScrMemoStylerManager(Self.Owner).ItemsCount := TScrMemoStylerManager(Self.Owner).ItemsCount + 1;
end;
...
constructor TScrMemoStylerManager.Create(AOwner: TComponent);
begin
...
destructor TScrMemoStylerManager.Destroy;
begin
  FItems.Free;
  inherited;
end;

procedure TScrMemoStylerManager.LoadAllStylersFromFile(FileName: string);
var
  i: Integer;
begin
...
procedure TScrMemoStylerManager.LoadStylerFromFile(Styler: TScrCustomMemoStyler; FileName: string);
var
  I,CNT: Integer;
  init: TMemIniFile;
...
procedure TScrMemoStylerManager.SaveAllStylersToFile(FileName: string);
var
  I: Integer;
begin
...
procedure TScrMemoStylerManager.SaveStylerToFile(Styler: TScrCustomMemoStyler; FileName: string);
var
  I, J: Integer;
  init: TMemIniFile;
...
procedure TScrMemoStylerManager.SetItems(Value: TAdvMemoStylersCollection);
begin
  FItems.Assign(Value);
end;


function TScrMemoStylerManager.GetStylerByFileName(FileName: string): TScrCustomMemoStyler;
var
  i: integer;
  Ext: string;
...
function TScrMemoStylerManager.GetStyler(Idx: integer): TScrCustomMemoStyler;
var
  i: integer;
begin
  Result := nil;
...
function TScrMemoStylerManager.GetStylerStyleIndexByInfo(styler: TScrCustomMemoStyler; Info: string): integer;
var
  i,j: integer;
begin
  Result := -1;
...
function TScrMemoStylerManager.GetStylerItem(Idx: integer): TCollectionItem;
var
  i: integer;
begin
  Result := nil;
...
function TScrMemoStylerManager.GetStylerItemByFileName(FileName: string): TcollectionItem;
var
  i: integer;
begin
  Result := nil;
...
function TScrMemoStylerManager.GetStylerItemByName(strName: string): TCollectionItem;
var
  i: Integer;
begin
  Result := nil;
...
function TScrMemoStylerManager.GetStylerByName(strName: string): TScrCustomMemoStyler;
var
  i: Integer;
begin
  Result := nil;
...
function TScrMemoStylerManager.GetFilter(Idx: integer): string;
var
  tmpFilter: string;
  i: integer;
begin
...
function TScrMemoStylerManager.GetStylerNames(var List: TStrings): integer;
var
  i: integer;
begin
  for i := 0 to pred(FItems.Count) do
...
procedure TScrMemoStylerManager.Notification(comp: TComponent;

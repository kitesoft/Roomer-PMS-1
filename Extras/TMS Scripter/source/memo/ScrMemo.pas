...
uses
  Classes, SysUtils, ScrRTFEng, ScrStyleIf, Windows, Messages, Graphics, Forms,
  Dialogs, StdCtrls, ExtCtrls, ComCtrls, Printers, ActnList, Controls, Menus,
  ScrFindDialogForm, ScrReplaceDialogForm, Types
  {$IFDEF DELPHIXE3_LVL}
  , System.UITypes
  {$ENDIF}
  ;
...
const
  ...
  //          : New : Exposed Options in TScrMemoFindDialog, TScrMemoFindReplaceDialog
  ...
  //          : New : Events OnShow, OnClose added to TScrMemoFindDialog, TScrMemoFindReplaceDialog
  ...
  // 3.0.1.4  : Fixed : Issue with TScrMemoFindReplaceDialog with use of TAdvReplaceDialog
  ...
  // 3.4.0.1  : Fixed : Issue with TScrMemoFindReplaceDialog Replace function
...
//------------------------------------------------------------------------------
  ...
  TScrCustomMemo = class;
  TScrCustomMemoStyler = class;
...
//        TScrMemoStrings
//------------------------------------------------------------------------------
  TScrMemoStrings = class(TStringList)
  private
    Memo: TScrCustomMemo;
    FLockCount: Integer;
    FDeleting: Boolean;
...
//------------------------------------------------------------------------------
  ...
  public
    constructor Create(AOwner: TScrCustomMemo);
    procedure Assign(Source: TPersistent); override;
  published
...
//------------------------------------------------------------------------------
  ...
  public
    ...
    property Memo: TScrCustomMemo read FMemo write FMemo;
  ...
  TScrACorrect = class(TPersistent)
  private
...
//------------------------------------------------------------------------------
  ...
  //se TScrMemoSource.SetMemo, TScrCustomMemo.GetMemoState and TScrCustomMemo.ResetMemoState
  ...
  TScrMemoChecker = class(TComponent)
  private
    FMemo: TScrCustomMemo;
  ...
  {$ENDIF}
  TScrMemoCapitalChecker = class(TScrMemoChecker)
  public
    constructor Create(AOwner: TComponent); override;
...
//------------------------------------------------------------------------------
  ...
  [ComponentPlatformsAttribute(pidWin32 or pidWin64)]
  {$ENDIF}
  TScrMemoSource = class(TComponent)
  private
    FSyntaxStyler: TScrCustomMemoStyler;
    FLines: TScrMemoStrings;
  ...
  protected
    procedure SetSyntaxStyler(const Value: TScrCustomMemoStyler);
    procedure SetLines(const Value: TScrMemoStrings);
    procedure SetMemo(const Value: TScrCustomMemo);
  ...
  published
    property SyntaxStyler: TScrCustomMemoStyler read FSyntaxStyler write SetSyntaxStyler;
    property Lines: TScrMemoStrings read FLines write SetLines;
...
//------------------------------------------------------------------------------
//        TScrCustomMemo - declaration
//------------------------------------------------------------------------------
  TScrCustomMemo = class(TCustomControl, ITMSStyle, ITMSTones)
  private
    FOwner: TComponent;
    FDragMode: TDragMode;
    FDragPos: TCellPos;
    FVersion: string;
    FUseStyler: boolean;
    FInternalStyles: TScrCustomMemoStyler;
    ...
    procedure SetMemoSource(const Value: TScrMemoSource);
    function GetModified: boolean;
    procedure SetModified(const Value: boolean);
    ...
    procedure SetBorderColor(const Value: TColor);
    procedure SetLines(ALines: TScrMemoStrings);
    ...
    procedure SetMemoStyler(Value: TScrCustomMemoStyler);
    ...
    function GetInternalStyles: TScrCustomMemoStyler;
    function GetInternalLines: TScrMemoStrings;
    procedure SetInternalLines(const Value: TScrMemoStrings);
    ...
    procedure SetMemoChecker(const AMemoChecker: TScrMemoChecker);
  ...
  protected
    ...
    property InternalStyles: TScrCustomMemoStyler read GetInternalStyles;
    property InternalLines: TScrMemoStrings read GetInternalLines write SetInternalLines;
    ...
    property AutoCorrect: TScrACorrect read FAutoCorrect write FAutoCorrect;
    ...
    property MemoSource: TScrMemoSource read FMemoSource write SetMemoSource;
    ...
    property MemoChecker: TScrMemoChecker read FMemoChecker write SetMemoChecker;
  ...
  public
    ...
    property SyntaxStyles: TScrCustomMemoStyler read FInternalStyles write SetMemoStyler;
...
//------------------------------------------------------------------------------
  ...
  TScrCustomMemoStyler = class(TComponent)
  private
  ...
  //--------------------------------------------------------------
  //         TScrMemo (SYNTAX MEMO) - declaration
  //--------------------------------------------------------------
  {$IFDEF DELPHIXE2_LVL}
  [ComponentPlatformsAttribute(pidWin32 or pidWin64)]
  {$ENDIF}
  TScrMemo = class(TScrCustomMemo)
  private
  ...
  [ComponentPlatformsAttribute(pidWin32 or pidWin64)]
  {$ENDIF}
  TScrMemoFindDialog = class(TComponent)
  private
    FDisplayMessage: boolean;
    FNotFoundMessage: string;
    FFindText: string;
    FAdvMemo: TScrCustomMemo;
  ...
  published
    ...
    property AdvMemo: TScrCustomMemo read FAdvMemo write FAdvMemo;
  ...
  [ComponentPlatformsAttribute(pidWin32 or pidWin64)]
  {$ENDIF}
  TScrMemoFindReplaceDialog = class(TComponent)
  private
    FDisplayMessage: boolean;
    FNotFoundMessage: string;
    FFindText: string;
  ...
  TScrMemoAction = class(TAction)
  private
    FControl: TScrCustomMemo;
    procedure SetControl(Value: TScrCustomMemo);
  protected
    function GetControl(Target: TObject): TScrCustomMemo; virtual;
    procedure Notification(AComponent: TComponent; Operation: TOperation); override;
  public
    destructor Destroy; override;
    function HandlesTarget(Target: TObject): Boolean; override;
    procedure UpdateTarget(Target: TObject); override;
    property Control: TScrCustomMemo read FControl write SetControl;
  end;

  TScrMemoCut = class(TScrMemoAction)
  public
    procedure ExecuteTarget(Target: TObject); override;
  end;

  TScrMemoCopy = class(TScrMemoAction)
  public
    procedure ExecuteTarget(Target: TObject); override;
  end;

  TScrMemoPaste = class(TScrMemoAction)
  public
    procedure UpdateTarget(Target: TObject); override;
    procedure ExecuteTarget(Target: TObject); override;
  end;

  TScrMemoSelectAll = class(TScrMemoAction)
  public
    procedure ExecuteTarget(Target: TObject); override;
    procedure UpdateTarget(Target: TObject); override;
  end;

  TScrMemoUndo = class(TScrMemoAction)
  public
    procedure ExecuteTarget(Target: TObject); override;
    procedure UpdateTarget(Target: TObject); override;
  end;

  TScrMemoRedo = class(TScrMemoAction)
  public
    procedure ExecuteTarget(Target: TObject); override;
    procedure UpdateTarget(Target: TObject); override;
  end;

  TScrMemoDelete = class(TScrMemoAction)
  public
    procedure ExecuteTarget(Target: TObject); override;
...
uses
  ClipBrd, ComObj, ShellAPI, Math, IniFiles, ImgList, Consts, ScrMdd, ActiveX, StrUtils
...
type
  ...
  public
    ...
    constructor Create(AMemo: TScrCustomMemo);
...
function TScrCustomMemo.GetMarkers(Markers: TAdvMarkers): boolean;
begin
  Result := FMarkerList.Markers.GetMarkers(Markers);
end;

function TScrCustomMemo.MarkerAtLine(LineNo: Integer): Boolean;
begin
  Result := FMarkerList.Markers.MarkerAtLine(LineNo);
end;

procedure TScrCustomMemo.GoToMarker(Marker: integer);
var
  LineNo: Integer;
begin
...
procedure TScrCustomMemo.GoToMarkerName(MarkerText: string);
var
  LineNo,i: Integer;
...
procedure TScrCustomMemo.SaveMemoSettingsToFile(FileName: string); //saves the settings of the memo to a file
const
...
procedure TScrCustomMemo.LoadMemoSettingsFromFile(FileName: string); //saves the settings of the memo to a file
const
  INISection = 'Memo Settings';
var
  ini: TIniFile;
...
procedure TScrCustomMemo.SetCursor(ACurX, ACurY: integer);
begin
  ClearSelection;
  FCurX := 0;
  CurY := ACurY;
...
procedure TScrCustomMemo.Clear;
begin
  CurX := 0;
  CurY := 0;
  FLeftCol := 0;
...
procedure TScrCustomMemo.AddMarker(LineIndex, ImageIndex: integer);
begin
...
procedure TScrCustomMemo.AddMarker(LineIndex, ImageIndex: integer; MarkerText: string);
begin
...
procedure TScrCustomMemo.RemoveMarker(LineIndex: integer);
begin
...
function TScrCustomMemo.WordIsURL(s: string): Boolean;
begin
  s := LowerCase(s);
...
procedure TScrCustomMemo.ClearAllMarkers;
begin
...
procedure TScrCustomMemo.SelectAll;
begin
  FSelStartY := 0;
  FSelStartX := 0;
...
procedure TScrCustomMemo.DoCopyToClipboard;
var
  Allow: boolean;
begin
  Allow := true;
...
procedure TScrCustomMemo.DoCustomContextMenuClick(MenuItem: TMenuItem);
begin
  if Assigned(OnCustomContextMenuClick) then
    OnCustomContextMenuClick(Self, MenuItem);
end;

procedure TScrCustomMemo.DoCustomizeContextMenu(MX, MY: integer; ContextMenu: TPopupMenu);
begin
  if Assigned(OnCustomizeContextMenu) then
    OnCustomizeContextMenu(Self, MX,MY, ContextMenu);
end;

procedure TScrCustomMemo.DoCutToClipboard;
var
  Allow: boolean;
begin
  Allow := true;
...
procedure TScrCustomMemo.DoPasteFromClipboard;
var
  Allow: boolean;
begin
  Allow := true;
...
procedure TScrCustomMemo.CopyToClipBoard;
var
...
procedure TScrCustomMemo.PasteFromClipBoard;
var
...
procedure TScrCustomMemo.DeleteSelection;
begin
...
function TScrCustomMemo.DeleteSelectionInt(bRepaint: boolean): boolean;
var
...
//        LineBreak
//--------------------------------------------------------------
function TScrCustomMemo.LineBreak: string;
begin
  {$IFDEF DELPHI2007_LVL}
...
function TScrCustomMemo.IsEmpty: boolean;
begin
...
procedure TScrCustomMemo.CutToClipBoard;
begin
  if FReadOnly or not EditCanModify then
    Exit;
...
procedure TScrCustomMemo.TextFromPos(X, Y: Integer; var TextPos: integer);
var
  i, l: Integer;
begin
  l := 0;
...
procedure TScrCustomMemo.PosFromText(TextPos: integer; var X, Y: integer);
var
  i, j, l: integer;
begin
  X := 0;
...
procedure TScrCustomMemo.InsertText(AValue: string);
begin
  SetSelText(Avalue);
end;

procedure TScrCustomMemo.InsertTextAtXY(AValue: string; X, Y: Integer);
var
  ss: integer;
begin
  if InternalLines.Count = 0 then
...
procedure TScrCustomMemo.BlockIndent(FromLine, ToLine, Indent: Integer; AllowUndo: boolean = true);
var
  i,j: Integer;
  s:string;
  TempLines : TScrMemoStrings;
  bi: TIndentUndo;
begin
...
procedure TScrCustomMemo.DeleteTextAtXY(X, Y, NumChar: Integer);
var
  s: string;
begin
  s := InternalLines[y];
...
function TScrCustomMemo.GetSelStart: Integer;
var
  i: integer;
  xSelStartX, xSelStartY, xSelEndX, xSelEndY: integer;
begin
...
procedure TScrCustomMemo.SetSelStart(const Value: integer);
var
  len: integer;
begin
  len := GetSelLength;
...
procedure TScrCustomMemo.SetSelectionMode(const Value: TSelectionMode);
begin
...
procedure TScrCustomMemo.SetSelLength(const Value: integer);
begin
{$IFDEF TMSDEBUG}
  outputdebugstring(PChar(IntToStr(FSelStartX) + ':' + IntToStr(FSelStartY)));
{$ENDIF}
...
function TScrCustomMemo.GetSelText: string;
...
procedure TScrCustomMemo.SetSelText(const AValue: string);
var
  i, k, j, vp: integer;
  xSelStartX, xSelStartY, xSelEndX, xSelEndY: integer;
  Buff, S, NL: string;
...
begin
  ...
  begin
    ...
    begin
      ...
      TScrMemoStrings(InternalLines).DoInsert(k + 1, Copy(S, xSelStartX + 1, Length(S)));
      ...
      begin
        ...
        begin
          TScrMemoStrings(InternalLines).DoInsert(xSelStartY + k, tz.Strings[k]);
        end
        else
        begin
...
function TScrCustomMemo.GetSelLength: integer;
begin
...
procedure TScrCustomMemo.SelectionChanged;
begin
...
procedure TScrCustomMemo.FontChangedProc(Sender: TObject);
var
  OldFontName: string;
  tm: TTextMetric;
begin
...
procedure TScrCustomMemo.StatusChanged;
begin
...
procedure TScrCustomMemo.ClearSelection;
var
  Changed: boolean;
begin
...
procedure TScrCustomMemo.ExpandSelection;
//var
//  rct: TRect;
begin
  {
...
function TScrCustomMemo.GetMarkerCount: integer;
begin
  Result := FMarkerList.Markers.MarkerCount;
end;

function TScrCustomMemo.TrimRightWW(LineNo: Integer; Undo: boolean): string;
var
  s,olds,dels: string;
...
procedure TScrCustomMemo.SetMaxLength;
var
  i, len, mx: integer;
begin
  mx := 0;
...
procedure TScrCustomMemo.DoScroll(Sender: TScrollBar; ByValue: integer);
var
  eRect: TRect;
  Old: integer;
begin
...
procedure TScrCustomMemo.DoScrollPage(Sender: TScrollBar; ByValue: integer);
begin
  case Sender.Kind of
...
procedure TScrCustomMemo.SetLines(ALines: TScrMemoStrings);
var
  I: Integer;
begin
  if ALines <> nil then
...
procedure TScrCustomMemo.SetCaseSensitive(Value: boolean);
begin
...
procedure TScrCustomMemo.SetCurX(Value: integer);
var
  len: integer;
  WasVisible: boolean;
  lf: boolean;
...
procedure TScrCustomMemo.DoActiveLineChange(LnNo: integer);
begin
  if Assigned(FOnActiveLineChange) then
    FOnActiveLineChange(LnNo);
end;

procedure TScrCustomMemo.DoColumnChange(ColNo: integer);
begin
  if Assigned(FOnColumnChange) then
    FOnColumnChange(ColNo);
end;

procedure TScrCustomMemo.DoMarkerAdded(LnNo: integer; MarkerText: string);
begin
  if Assigned(FOnMarkerAdded) then
    FOnMarkerAdded(LnNo, MarkerText);
end;

procedure TScrCustomMemo.DoMarkerRemoved(LnNo: integer);
begin
...
procedure TScrCustomMemo.SetCurY(Value: integer);
var
  Old: integer;
  WasVisible: boolean;
  lf: boolean;
...
procedure TScrCustomMemo.MoveCursor(dX, dY: integer; Shift: TShiftState);
var
  Selecting: Boolean;
  S: string;
  ll: integer;
...
procedure TScrCustomMemo.MovePage(dP: integer; Shift: TShiftState);
var
  eRect: TRect;
...
procedure TScrCustomMemo.GoHome(Shift: TShiftState);
var
  Selecting: boolean;
  S: string;
...
procedure TScrCustomMemo.GoEnd(Shift: TShiftState);
var
  Selecting: boolean;
begin
  if ssCtrl in Shift then
...
procedure TScrCustomMemo.InsertChar(C: Char);
var
  S, S1: string;
  NewPlace: integer;
  CurX0, CurY0: integer;
...
procedure TScrCustomMemo.InsertTemplate(AText: string);
var
...
procedure TScrCustomMemo.DeleteChar(OldX, OldY: integer);
var
  S: string;
  C: char;
...
procedure TScrCustomMemo.DeleteLine;
var
  OldX, OldY: integer;
  s: string;
  i: Integer;
begin
  ...
  if InternalLines.Count = 1 then
    TScrMemoStrings(InternalLines)[0] := ''
  else if CurY = InternalLines.Count - 1 then
  begin
    CurY := CurY - 1;
    InternalLines.Delete(CurY + 1);
...
procedure TScrCustomMemo.BackSpace;
var
  iIndent: integer;
  strOrigLine: string;
  OldX, OldY: integer;
...
procedure TScrCustomMemo.BackWord;
var
  OldX, OldY: Integer;
  s: string;
  ch: char;
...
function TScrCustomMemo.HasMarkers: boolean;
begin
...
function TScrCustomMemo.IndentCurrLine: string;
var
  Len, Count,i: integer;
  CurS: string;
begin
...
procedure TScrCustomMemo.InitVCLStyle(init: boolean);
{$IFDEF DELPHIXE2_LVL}
var
...
procedure TScrCustomMemo.TabLine(AShift: TShiftState);
var
...
procedure TScrCustomMemo.DoSave;
begin
...
procedure TScrCustomMemo.DoEscape;
begin
...
procedure TScrCustomMemo.NewLine;
var
  S, sIndent: string;
...
begin
 ...
 { end        // kh:
  ...
  if not CodeFolding.Enabled then
    TScrMemoStrings(InternalLines).DoInsert(CurY + 1, sIndent + S)
  else
  begin
...
function TScrCustomMemo.AddString(S: string): integer;
begin
  if InternalLines.Count = 0 then
    TScrMemoStrings(InternalLines).DoAdd('');
...
procedure TScrCustomMemo.InsertString(Index: integer; S: string);
begin
  CurY := Index;
  CurX := 0;
  ClearSelection;
...
procedure TScrCustomMemo.SearchForStylers;
var
  I: Integer;
begin
  ...
  begin
    ...
    begin
      if Components[I].ClassParent = TScrCustomMemoStyler then
      begin
...
procedure TScrCustomMemo.DoCommand(cmd: TCommand; const AShift: TShiftState);
var
...
procedure TScrCustomMemo.DoContextMenuClick(Sender: TObject);
begin
...
procedure TScrCustomMemo.KeyDown(var Key: word; Shift: TShiftState);
var
  token: string;
  sy: integer;
begin
...
procedure TScrCustomMemo.KeyPress(var Key: char);
var
  wrd, newword: string;
  ps, pe, d: Integer;
  r: TRect;
...
procedure TScrCustomMemo.SelClickUpdate(X, Y: Integer; Down: Boolean; Shift: TShiftState; Button: TMouseButton);
var
  newPos: TCellPos;
  yold: Integer;
  lc, OldV: Boolean;
...
procedure TScrCustomMemo.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: integer);
var
  CharPos: TFullPos;
  R: TRect;
...
procedure TScrCustomMemo.DropText(X, Y: integer; s: string);
begin
...
procedure TScrCustomMemo.UpdateDragCaret(const DCPos: TPoint; const DCState: TDragState);
...
procedure TScrCustomMemo.DragOver(Source: TObject; X, Y: Integer; State: TDragState; var Accept: Boolean);
var
  pt: TPoint;
  r: TRect;
...
procedure TScrCustomMemo.DragDrop(Source: TObject; X, Y: Integer);
var
  s: string;
...
procedure TScrCustomMemo.MouseMove(Shift: TShiftState; X, Y: integer);
var
  newPos: TCellPos;
...
procedure TScrCustomMemo.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: integer);
begin
  inherited;
...
procedure TScrCustomMemo.ReSize;
begin
  inherited;
  ResizeEditor;
end;
...
//--------------------------------------------------------------
(*
procedure TScrCustomMemo.WMSize(var Msg: TWMSize);
begin
  try
    ResizeEditor;
  except
...
procedure TScrCustomMemo.WMGetDlgCode(var Msg: TWMGetDlgCode);
begin
  if FWantTab then
  begin
...
procedure TScrCustomMemo.WMEraseBkgnd(var Msg: TWmEraseBkgnd);
begin
  Msg.Result := 1;
end;

procedure TScrCustomMemo.WMSysChar(var Msg: TWMSysChar);
begin
...
procedure TScrCustomMemo.WMWindowPosChanged(var Message: TWMMove);
begin
  inherited;
  if FHintForm.Visible then
    FHintForm.Hide;
...
{$IFDEF DELPHIXE2_LVL}
procedure TScrCustomMemo.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(true);
end;
{$ENDIF}

procedure TScrCustomMemo.CMWantSpecialKey(var Msg: TCMWantSpecialKey);
begin
...
procedure TScrCustomMemo.WMSetCursor(var Msg: TWMSetCursor);
var
  P: TPoint;
begin
  Msg.Result := 1;
...
procedure TScrCustomMemo.WMSetFocus(var Message: TWMSetFocus);
begin
  inherited;
  if FCellSize.H = 0 then
    SetFont(FFont);
...
procedure TScrCustomMemo.WMKillFocus(var Message: TWMKillFocus);
begin
  Timer.Enabled := True;
...
procedure TScrCustomMemo.WMDropFiles(var Message: TMessage);
var
  Files: TStringList;
...
procedure TScrCustomMemo.ShowCaret(State: boolean);
var
  rct: TRect;
begin
  if not State then
...
function TScrCustomMemo.CellRect(ACol, ARow: integer): TRect;
var
  rct: TRect;
begin
  rct := EditorRect;
...
function TScrCustomMemo.LineRect(ARow: integer): TRect;
var
  rct: TRect;
begin
  rct := EditorRect;
...
function TScrCustomMemo.ColRect(ACol: integer): TRect;
var
  rct: TRect;
begin
  rct := EditorRect;
...
function TScrCustomMemo.LineRangeRect(FromLine, ToLine: integer): TRect;
var
  rct1, rct2: TRect;
begin
...
function TScrCustomMemo.ColRangeRect(FromCol, ToCol: integer): TRect;
var
  rct1, rct2: TRect;
begin
...
//        MOUSE TO CURSOR
//--------------------------------------------------------------
procedure TScrCustomMemo.MouseToCursor(X,Y: integer; var CursorX, CursorY: integer);
var
  cp: TCellPos;
begin
  cp := CellFromPos(X,Y);
...
function TScrCustomMemo.CellFromPos(X, Y: integer): TCellPos;
var
  rct: TRect;
begin
  rct := EditorRect;
...
function TScrCustomMemo.CharFromPos(X, Y: integer): TFullPos;
var
  rct: TRect;
  I: Integer;
...
procedure TScrCustomMemo.CheckCodeInsightChar(Key: Char);
begin
...
//        SET BORDER COLOR
//--------------------------------------------------------------
procedure TScrCustomMemo.SetBorderColor(const Value: TColor);
begin
  FBorderColor := Value;
  Invalidate;
end;
...
procedure TScrCustomMemo.SetColor(Index: integer; Value: TColor);
var
  Changed: boolean;
begin
  Changed := False;
...
procedure TScrCustomMemo.SetFont(Value: TFont);
var
  wW, wi: integer;
  OldFontName: string;
begin
...
procedure TScrCustomMemo.SetHiddenCaret(Value: boolean);
begin
...
function TScrCustomMemo.EditorRect: TRect;
var
  l, t, r, b: integer;
begin
  l := 2;
...
function TScrCustomMemo.CodeFoldingRect: TRect;
begin
  Result := Rect(0, 0, 0, 0);
  if FCodeFolding.Enabled then
  begin
...
procedure TScrCustomMemo.DrawMargin;
var
  eRect: TRect;
  i: integer;
begin
...
procedure TScrCustomMemo.DrawGutter;
begin
...
procedure TScrCustomMemo.FreshLineBitmap;
var
  eRect: TRect;
begin
  eRect := EditorRect;
...
function TScrCustomMemo.GetUpStyle(stopat: integer): TStyle;
var
...
function TScrCustomMemo.GetBreakPoint(Index: Integer): Boolean;
var
  Tlp: TlineProp;
begin
...
function TScrCustomMemo.GetModifiedState(Index: Integer): boolean;
begin
  Result := GetModifiedStateInt(Index) <> lmUnmodified;
end;

function TScrCustomMemo.GetModifiedStateInt(Index: Integer): TLineModifiedState;
var
  Tlp: TlineProp;
begin
  Tlp := InternalLines.GetLineProp(Index);
...
function TScrCustomMemo.GetWrapped(Index: Integer): Boolean;
var
  Tlp: TlineProp;
begin
  Tlp := InternalLines.GetLineProp(Index);
...
function TScrCustomMemo.GetWrappedLine(index: integer): string;
var
  sl: TStringList;
begin
...
function TScrCustomMemo.GetWrappedLineIndex(Index: integer): integer;
var
  i: integer;
  LineNoR: integer;
begin
...
function TScrCustomMemo.GetWrappedText: string;
begin
  BeginUpdate;
  UndoWrap;
...
function TScrCustomMemo.GetBookmark(Index: Integer): Boolean;
var
  Tlp: TlineProp;
begin
  Tlp := InternalLines.GetLineProp(Index);
...
function TScrCustomMemo.GetBookmarkCount: integer;
var
  i: integer;
  Tlp: TlineProp;
begin
...
function TScrCustomMemo.GetBookMarkGlyph: TBitmap;
begin
  Result := FBookmarkBmp;
end;

function TScrCustomMemo.GetExecutable(Index: Integer): Boolean;
var
  Tlp: TlineProp;
begin
  Tlp := InternalLines.GetLineProp(index);
...
procedure TScrCustomMemo.SetBreakPoint(Index: Integer;
  const Value: Boolean);
var
  Tlp: TlineProp;
begin
...
procedure TScrCustomMemo.SetModifiedState(Index: Integer; const Value: boolean);
begin
  SetModifiedStateInt(Index,lmModified);
end;

procedure TScrCustomMemo.SetModifiedStateInt(Index: Integer;
...
procedure TScrCustomMemo.SetOleDropTarget(const Value: TOleDropTarget);
begin
...
procedure TScrCustomMemo.SetBookmark(Index: Integer;
  const Value: Boolean);
var
  Tlp: TlineProp;
begin
...
procedure TScrCustomMemo.SetBookMarkGlyph(const Value: TBitmap);
begin
  FBookmarkBmp.Assign(Value);
end;

procedure TScrCustomMemo.SetWrapped(Index: Integer;
  const Value: Boolean);
var
  Tlp: TlineProp;
begin
...
function TScrCustomMemo.GetLineStyle(Index: Integer; var LineStyle: TStyle): Boolean;
var
  Tlp: TlineProp;
begin
  Tlp := InternalLines.GetLineProp(index);
...
procedure TScrCustomMemo.ClearLineStyles;
var
  i: Integer;
begin
...
procedure TScrCustomMemo.ClearLineStylesFromTo(FromLine, ToLine: integer);
var
  Tlp: TlineProp;
  i: integer;
begin
...
procedure TScrCustomMemo.SetLineStyle(Index: Integer; const LineStyle: TStyle);
var
  Tlp: TlineProp;
begin
  if Index >= InternalLines.Count then
...
procedure TScrCustomMemo.SetExecutable(Index: Integer;
  const Value: Boolean);
var
  Tlp: TlineProp;
begin
...
procedure TScrCustomMemo.SetHideSelection(const Value: boolean);
begin
...
procedure TScrCustomMemo.SetHighlightCaseSensitive(const Value: boolean);
begin
...
procedure TScrCustomMemo.SetHighLightText(const Value: string);
begin
...
(*
procedure TScrCustomMemo.SWAPColors;
var
  c: TColor;
begin
...
procedure TScrCustomMemo.ClearBreakpoints;
var
  i: Integer;
begin
  for i := 0 to InternalLines.FlinesProp.Count - 1 do
...
procedure TScrCustomMemo.ClearModified;
var
  i: Integer;
begin
  for i := 0 to InternalLines.FlinesProp.Count - 1 do
...
procedure TScrCustomMemo.ClearExecutableLines;
var
  i: Integer;
begin
  for i := 0 to InternalLines.FlinesProp.Count - 1 do
...
procedure TScrCustomMemo.DrawLine(ACanvas: TCanvas; LineNo: Integer; var style: TStyle; DM: TDrawMode; PR: TRect; VisLineNo: Integer = -1);
var
...
procedure TScrCustomMemo.Paint;
var
  eRect, pRect: TRect;
  i, ll: integer;
  LineStyle: TStyle;
...
begin
  if TScrMemoStrings(InternalLines).FLockCount > 0 then
    Exit;
...
function TScrCustomMemo.GetVisible(Index: integer): integer;
var
  Coord: TFullPos;
  Cell: TCellPos;
  eRect: TRect;
...
procedure TScrCustomMemo.MakeVisible;
var
  Modified: boolean;
begin
  Modified := False;
...
procedure TScrCustomMemo.ResizeEditor;
var
  otl ,lib: Integer;
  tp, x, y: Integer;
  oldss,oldsl: Integer;
...
procedure TScrCustomMemo.ResizeScrollBars(DoRepaint: Boolean);
var
  eRect, sbRect: TRect;
...
procedure TScrCustomMemo.ResizeGutter;
var
  eRect: TRect;
begin
  eRect := EditorRect;
...
procedure TScrCustomMemo.Notification(AComponent: TComponent;
...
//        CREATE PARAMS
//--------------------------------------------------------------
procedure TScrCustomMemo.CreateParams(var Params: TCreateParams);
begin
  inherited;
  Params.Style := Params.Style or WS_CLIPCHILDREN;
end;

procedure TScrCustomMemo.CreateWnd;
begin
  inherited;
...
procedure TScrCustomMemo.Undo;
begin
  InternalUndoList.Undo;
end;

procedure TScrCustomMemo.Redo;
begin
  InternalUndoList.Redo;
  Refresh;
end;

procedure TScrCustomMemo.AddUndoStep(AUndoStep: TUndo);
begin
...
procedure TScrCustomMemo.SetUILanguage(const AUILanguage: TUILanguage);
begin
  FUILanguage.Assign(AUILanguage);
end;

procedure TScrCustomMemo.SetUndoLimit(Value: integer);
begin
...
procedure TScrCustomMemo.UndoChange;
begin
...
function TScrCustomMemo.CanUndo: boolean;
begin
...
function TScrCustomMemo.CanRedo: boolean;
begin
...
procedure TScrCustomMemo.ClearUndoList;
begin
...
procedure TScrCustomMemo.SetScrollBars(Value: TScrollStyle);
begin
...
constructor TScrCustomMemo.Create(AOwner: TComponent);
var
  i: Integer;
begin
  inherited;
  ...
  FBookmarkBmp.LoadFromResourceName(HInstance, 'SCR_BOOK');
  ...
  AddBmpRes('SCR_CUT',FContextMenuImages);
  AddBmpRes('SCR_COPY',FContextMenuImages);
  AddBmpRes('SCR_PASTE',FContextMenuImages);
  AddBmpRes('SCR_UNDO',FContextMenuImages);
  AddBmpRes('SCR_REDO',FContextMenuImages);
...
procedure TScrCustomMemo.AddBmpRes(Resname: string; ImageList: TImageList);
var
  bmp: TBitmap;
begin
  bmp := TBitmap.Create;
...
procedure TScrCustomMemo.CreateAutoCompleteForm;
begin
...
procedure TScrCustomMemo.SetAutoThemeAdapt(const value: boolean);
begin
...
procedure TScrCustomMemo.OnCodeFoldingChange(Sender: TObject; ChangeMsg: Integer);
begin
  if (ChangeMsg = 2) then
  begin
    ShowCaret(true);
...
procedure TScrCustomMemo.SetCodeFolding(Value: TCodeFolding);
begin
  FCodeFolding.Assign(Value);
end;

procedure TScrCustomMemo.DoClipboardAction(ClipboardAction: TMemoClipboardAction; var Allow: Boolean);
begin
  if Assigned(OnClipboardAction) then
    OnClipboardAction(Self, ClipboardAction, Allow);
end;

procedure TScrCustomMemo.DoLineBkColor(LineNo: Integer; var BkColor: TColor);
begin
  if Assigned(OnLineBkColor) then
    OnLineBkColor(Self, LineNo, BkColor);
end;

function TScrCustomMemo.IsCommentedNode(LineIndex: Integer): Boolean;
var
  RgnIndex: Integer;
  Coments: String;
begin
...
function TScrCustomMemo.HasCompletionListActivationChar(token: string): boolean;
var
  i: integer;
begin
  Result := false;
...
function TScrCustomMemo.IsCompletionListActivationChar(Ch: Char): boolean;
var
  i: integer;
begin
  Result := false;
...
function TScrCustomMemo.IsCompletionListEndChar(Ch: Char): boolean;
begin
  Result := false;
end;

function TScrCustomMemo.IsCompletionListSearchChar(Ch: Char): boolean;
begin
  {$IFDEF DELPHI_UNICODE}
  {$IFDEF DELPHIXE4_LVL}
...
function TScrCustomMemo.GetNodeComments(LineIndex: Integer; var RgnIndex: Integer; var Coments: String): Boolean;
var
  S, PartS, sp: String;
...
procedure TScrCustomMemo.AutoCodeFold;
var
...
function TScrCustomMemo.VisIndexToLineIndex(Index: Integer): Integer;
var
  i, ln, P, C: Integer;
  Tlp: TLineProp;
begin
...
function TScrCustomMemo.LineIndexToVisIndex(Index: Integer): Integer;
var
  i, ln, P, C: Integer;
  Tlp: TLineProp;
begin
...
function TScrCustomMemo.GetExpandNode(Index: Integer): Boolean;
var
  Tlp: TlineProp;
begin
  Tlp := InternalLines.GetLineProp(Index);
...
procedure TScrCustomMemo.SetExpandNode(Index: Integer; const Value: Boolean);
var
  Tlp: TlineProp;
  OldCurX: Integer;
begin
...
function TScrCustomMemo.LastChildOfParent(ParentIndex: Integer): Integer;
var
  i, P, C: Integer;
  Tlp, TlpP: TLineProp;
begin
...
procedure TScrCustomMemo.ExpandParents(ChildIndex: Integer);
var
  Tlp, TlpC: TlineProp;
  i, P, C: Integer;
...
function TScrCustomMemo.AddCodeFolding(StartLineIndex, EndLineIndex: Integer): Boolean;
var
...
procedure TScrCustomMemo.RemoveCodeFoldingFromChild(EndLineIndex: Integer);
var
  i, P, C: Integer;
  Tlp, TlpC: TLineProp;
  DelP: Boolean;
...
procedure TScrCustomMemo.RemoveCodeFolding(StartLineIndex: Integer);
var
  i, P, C: Integer;
  Tlp, TlpP: TLineProp;
begin
...
procedure TScrCustomMemo.RemoveAllCodeFolding;
var
  i: Integer;
  Tlp: TLineProp;
begin
...
function TScrCustomMemo.IsNode(LineIndex: Integer): Boolean;
var
  Tlp: TlineProp;
begin
  Result := False;
...
function TScrCustomMemo.IsParameterHintActivationChar(Ch: Char): boolean;
begin
  result := false;
end;

procedure TScrCustomMemo.ExpandAllNodes;
var
  I: Integer;
begin
  if not FCodeFolding.Enabled then
...
procedure TScrCustomMemo.CollapseAllNodes;
var
  I: Integer;
begin
  if not FCodeFolding.Enabled then
...
procedure TScrCustomMemo.ToggleNode(LineIndex: Integer);
//var
  //Tlp: TlineProp;
begin
...
function TScrCustomMemo.GetCursorEx: TCursor;
begin
  Result := inherited Cursor;
end;

procedure TScrCustomMemo.SetCursorEx(const Value: TCursor);
begin
  inherited Cursor := Value;
  FoldCursor := Value;
end;

function TScrCustomMemo.WordAtXY(X, Y: Integer): string;
var
  i: integer;
  s: string;
  fe, fb: integer;
...
function TScrCustomMemo.FullWordAtXY(X, Y: Integer): string;
var
  i: integer;
  s: string;
  fe, fb: integer;
...
function TScrCustomMemo.NextToken(const s: string; StartFrom: integer; var Offset: integer): string;
var
  i,ls: integer;
  bnd: boolean;
begin
...
function TScrCustomMemo.TokenAtXY(X, Y: Integer): string;
var
  i: integer;
  s: string;
  fe, fb: integer;
...
function TScrCustomMemo.WordAtCursor: string;
var
  p: integer;
begin
...
procedure TScrCustomMemo.SelectWordAtCursor;
var
  p,ss: integer;
  s: string;
begin
...
function TScrCustomMemo.FullWordAtCursor: string;
var
  s: string;
  i: integer;
  fb, fe: integer;
...
function TScrCustomMemo.TokenAtCursor: string;
var
  s: string;
  i: integer;
  fb, fe: integer;
...
function TScrCustomMemo.WordAtCursorPos(var Pos: Integer): string;
var
  s: string;
  i: integer;
  fb, fe: integer;
...
function TScrCustomMemo.WordTillCursor: string;
var
  s: string;
  i: integer;
  fb, fe: integer;
...
function TScrCustomMemo.IsWordBoundary(ch: char): boolean;
begin
  {$IFNDEF DELPHI_UNICODE}
  Result := (ch in [#32,#39,'"',',',';','.']);
  {$ENDIF}
...
function TScrCustomMemo.IsTokenBoundary(ch: char): boolean;
begin
  {$IFNDEF DELPHI_UNICODE}
  Result := ch in [#32, '(', ')', '[', ']', ',', '.', ':', ';', '"', '''', '='];
  {$ENDIF}
...
procedure TScrCustomMemo.LineRefresh;
var
  r: TRect;
begin
  r := ClientRect;
...
procedure TScrCustomMemo.ModificationsSaved;
var
  Index: integer;
  tlp: TlineProp;
begin
...
procedure TScrCustomMemo.ModificationsLoaded;
var
  Index: integer;
  tlp: TlineProp;
begin
...
procedure TScrCustomMemo.LinesChanged(Sender: TObject);
var
  r: TRect;
//  i: integer;
begin
...
destructor TScrCustomMemo.Destroy;
begin
  FPrintOptions.Free;
  html.Free;
  FLines.OnChange := nil;
...
function TScrCustomMemo.EditCanModify: Boolean;
begin
  Result := True;
end;
...
function TScrCustomMemo.NumberOfPages(ACanvas: TCanvas; PageWidth, PageHeight: Integer): Integer;
var
  cRect: TRect;
  th, lpp: Integer;
begin
...
procedure TScrCustomMemo.PrintToCanvas(ACanvas: TCanvas; PageWidth, PageHeight, PageNr: Integer);
var
  hstl: TStyle;
  i: Integer;
  pRect: TRect;
...
procedure TScrCustomMemo.PrintPages(FromPage, ToPage : integer);
var
  lp1, np : integer;
begin
...
procedure TScrCustomMemo.PrintSelection;
var
  tmpmemo: TScrCustomMemo;
begin
  tmpmemo := TScrCustomMemo.Create(self);
  tmpmemo.Parent := self;
...
procedure TScrCustomMemo.Print;
var
  i, np: Integer;
...
function TScrCustomMemo.SaveToHTML(FileName: string;
...
function TScrCustomMemo.SaveToHTMLStream(AStream: TMemoryStream;
  Fixedfonts: Boolean = True): Boolean;
begin
  Result := False;
  OutputHTML(FixedFonts);
...
function TScrCustomMemo.SaveToRTFStream(AStream: TMemorystream; Fixedfonts: Boolean = True): boolean;
begin
  Result := False;
  FRTFEngine := TRTFEngine.Create;
...
function TScrCustomMemo.SaveToRTF(FileName: string; Fixedfonts: Boolean = True): Boolean;
begin
  Result := False;
  FRTFEngine := TRTFEngine.Create;
  OutputRTF(FixedFonts);
...
procedure TScrCustomMemo.GetLines;
var
  i: Integer;
  hstl: TStyle;
  pRect: TRect;
...
procedure TScrCustomMemo.SetUseStyler(const Value: boolean);
begin
  FUseStyler := Value;
  Invalidate;
end;

procedure TScrCustomMemo.SetVersion(const Value: string);
begin
  //
end;

procedure TScrCustomMemo.SetTrimTrailingSpaces(const Value: boolean);
begin
  FTrimTrailingSpaces := Value;
  Invalidate;
end;

procedure TScrCustomMemo.SetShowRightMargin(const Value: boolean);
begin
  FShowRightMargin := Value;
  Invalidate;
end;

procedure TScrCustomMemo.SetSmartTabs(const Value: boolean);
begin
  FSmartTabs := Value;
  Invalidate;
end;

procedure TScrCustomMemo.SetAcceptFiles(const Value: Boolean);
begin
  FAcceptFiles := Value;
...
procedure TScrCustomMemo.SetEnhancedHomeKey(const Value: boolean);
begin
  FEnhancedHomeKey := Value;
  Invalidate;
end;

procedure TScrCustomMemo.CopyHTMLToClipboard;
var
  Data: THandle;
  DataPtr: Pointer;
  CF_HTML: word;
...
procedure TScrCustomMemo.DoFind;
begin
  if Assigned(FonFind) then
    FOnFind(self);
end;

procedure TScrCustomMemo.DoReplace;
begin
  if Assigned(FonReplace) then
    FOnReplace(self);
end;

procedure TScrCustomMemo.OutputHTML(Fixedfonts: Boolean; fromline: integer = -1; toline: integer = -1; fragment: boolean = false);
var
  i: Integer;
  hstl: TStyle;
  pRect: TRect;
...
procedure TScrCustomMemo.OutputRTF(FixedFonts: Boolean; fromline: integer = -1; toline: integer = -1);
var
  i: Integer;
  hstl: TStyle;
  pRect: TRect;
...
procedure TScrCustomMemo.ThemeAdapt;
var
...
procedure TScrCustomMemo.DrawParts(Part: string; var Drawstyle: TStyle; lineno: integer);
var
  bc, c: Tcolor;
  sl: Tfontstyles;
...
procedure TScrCustomMemo.DrawHTML(Part: string; var Drawstyle: Tstyle; lineno: integer);
var
  bc, c: Tcolor;
  sl: Tfontstyles;
...
procedure TScrCustomMemo.DrawRTF(Part: string; var Drawstyle: Tstyle; lineno: integer);
var
  bc, c, OldHClr: Tcolor;
  sl: Tfontstyles;
  AFont: TFont;
...
procedure TScrCustomMemo.WMLButtonDblClk(var Message: TWMLButtonDblClk);
var
  s: string;
  i: integer;
  fb, fe: integer;
...
procedure TScrCustomMemo.WMPrepareShowHint(var Msg: TMessage);
begin
  PrepareShowHint;
end;

procedure TScrCustomMemo.BeginUpdate;
begin
  FLetRefresh := False;
end;

procedure TScrCustomMemo.EndUpdate;
begin
  FLetRefresh := True;
  SetMaxLength;
  ResizeScrollBars(False);
...
procedure TScrCustomMemo.WMContextMenu(var Message: TWMContextMenu);
var
  pt: TPoint;
  MCurX,MCurY: integer;
  mnu: TMenuItem;
...
constructor TAdvGutter.Create(AOwner: TScrCustomMemo);
begin
  inherited Create;
  Memo := AOwner;
  FGutterWidth := 45;
...
procedure TScrCustomMemo.SetComponentStyle(AStyle: TTMSStyle);
begin
  SetStyle(TAdvMemoStyle(AStyle));
end;

procedure TScrCustomMemo.SetColorTones(ATones: TColorTones);
begin
...
procedure TScrCustomMemo.SetStyle(AStyle: TAdvMemoStyle);
begin
...
constructor TAdvActiveLineSettings.Create(AOwner: TScrCustomMemo);
begin
  inherited Create;
  Memo := AOwner;
...
destructor TScrMemoStrings.Destroy;
begin
  ClearLinesProp;
  FlinesProp.Free;
  FListLengths.Free;
...
function TScrMemoStrings.DoAdd(const S: string): integer;
begin
...
procedure TScrMemoStrings.DoInsert(Index: integer; const S: string);
var
  s1: string;
  i: integer;
begin
...
procedure TScrMemoStrings.Delete(Index: integer);
var
...
procedure TScrMemoStrings.Insert(Index: integer; const S: string);
var
  vp: integer;
  su: string;
begin
...
procedure TScrMemoStrings.SaveToFile(const FileName: string);
var
  i: integer;
  tr: string;
begin
...
{$IFDEF TMS_UNICODE}
procedure TScrMemoStrings.SaveToFile(const FileName: string; Encoding : TEncoding );
begin
...
procedure TScrMemoStrings.LoadFromFile(const FileName: string);
var
  i, len, mx: integer;
  s: string;
  Stream: TFileStream;
...
procedure TScrMemoStrings.SaveToStream(Stream: TStream);
begin
...
procedure TScrMemoStrings.LoadFromStream(Stream: TStream);
var
  i, len, mx: integer;
  s: string;
...
function TScrMemoStrings.GetObject(Index: Integer): TObject;
begin
  Result := nil;
...
procedure TScrMemoStrings.PutObject(Index: Integer; AObject: TObject);
var
  P: TObject;
begin
  if (Index >= 0) and (Index < Count) then
...
procedure TScrMemoStrings.RestoreObject(Index: Integer; AObject: TObject);
begin
  inherited PutObject(Index, AObject);
end;

function TScrMemoStrings.Get(Index: Integer): string;
begin
  Result := inherited Get(Index);
end;

procedure TScrMemoStrings.Put(Index: Integer; const S: string);
var
  sz: string;
begin
  if not Assigned(Memo) then
...
procedure TScrMemoStrings.SetUpdateState(Updating: boolean);
begin
  if Updating then
    Inc(FLockCount)
  else
...
function TScrMemoStrings.CreateProp(Index: integer): TLineProp;
var
  Tlp: TLineProp;
begin
...
procedure TScrMemoStrings.AddLineProp(p: TObject);
begin
  FLinesProp.Add(Pointer(p));
end;

procedure TScrMemoStrings.ClearStrings;
begin
  FLinesProp.Clear;
  FListLengths.Clear;
  inherited Clear;
...
procedure TScrMemoStrings.Clear;
begin
  if Assigned(Memo) then
  begin
    with Memo do
...
constructor TScrMemoStrings.Create;
begin
  inherited;
...
procedure TScrMemoStrings.ClearLinesProp;
var
  P: TLineProp;
  i: Integer;
begin
...
function TScrMemoStrings.GetLineProp(Index: Integer): TLineProp;
var
  P: Tobject;
begin
...
procedure TScrMemoStrings.SetLineProp(Index: Integer;
...
procedure TScrMemoStrings.SetTextEx(const Value: string);
{$IFNDEF DELPHI_UNICODE}
var
  ms: TMemoryStream;
{$ENDIF}
...
procedure TScrCustomMemo.ScrollBarScroll(Sender: TObject;
...
//        TScrMemo - CREATE
//--------------------------------------------------------------
constructor TScrMemo.Create(AOwner: TComponent);
begin
  inherited;
...
procedure TScrMemo.AdvSyntaxMemoChange(Sender: TObject);
begin
  Invalidate;
end;

procedure TScrMemo.AdvSyntaxMemoGutterClick(Sender: TObject;
  LineNo: integer);
var
  ss: integer;
begin
...
procedure TScrCustomMemo.Format;
begin
  if Assigned(SyntaxStyles) and (SyntaxStyles.HasFormatting) then
  begin
...
function TScrCustomMemo.FormatLineNumber(var Position: integer; Line: integer): string;
var
  strCurLine: string;
  cw: integer;
begin
...
procedure TScrMemo.AdvSyntaxMemoGutterDraw(Sender: TObject;
...
procedure TScrCustomMemoStyler.SetStyles(const Value: TElementStyles);
begin
...
constructor TScrCustomMemoStyler.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TScrCustomMemoStyler.Destroy;
begin
  FlistAuto.free;
  FNumberStyle.Free;
  FCommentStyle.Free;
...
procedure TScrCustomMemoStyler.SetStyle(const Index: integer;
...
procedure TScrMemo.LoadStyle;
begin
end;



procedure TScrCustomMemoStyler.Update;
begin
  if Assigned(FAllStyles.FOwner) then
    FAllStyles.FOwner.loadstyle;
end;

procedure TScrCustomMemoStyler.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
...
function TScrCustomMemoStyler.PosCommentRight(s: string; var cl,ci,ct: integer): integer;
var
  res,i: integer;
begin
  res := 0;
...
function TScrCustomMemoStyler.HasCommentStyles: boolean;
var
  i: integer;
begin
  Result := false;
...
function TScrCustomMemoStyler.HasFormatting: boolean;
begin
  Result := false;
end;

function TScrCustomMemoStyler.HasMultiComment: boolean;
var
  i: integer;
begin
...
function TScrCustomMemoStyler.PosCommentLeft(s: string; var cl,ci,ct: integer): integer;
var
  res, mres, i: integer;
begin
  res := 0;
...
procedure TScrCustomMemoStyler.DrawKeyword(Canvas: TCanvas;
  AKeyword: string; var ARect: TRect);
begin

end;

function TScrCustomMemoStyler.ForceLineBreakChars: string;
begin
  Result := '';
end;

function TScrCustomMemoStyler.Format(s: string): string;
begin
  Result := s;
end;

procedure TScrCustomMemoStyler.Loaded;
begin
  inherited;
...
function TScrMemo.IsWordBoundary(ch: char): boolean;
begin
...
procedure TScrMemo.RefreshMemo;
begin
  curx := curx;
  cury := cury;
  SetMaxLength;
...
procedure TScrCustomMemo.RepaintGutter;
var
  r: TRect;
begin
...
procedure TScrCustomMemo.SetActiveLine(const Value: integer);
var
  r: TRect;
begin
  FActiveLine := Value;
...
procedure TScrCustomMemo.SetMemoChecker(const AMemoChecker: TScrMemoChecker);
begin
...
procedure TScrCustomMemo.SetAutoCompletionListImages(IL: TImageList);
begin
...
procedure TScrCustomMemo.UpdateGutter;
begin
...
procedure TScrCustomMemo.SetBorderStyle(const Value: TBorderStyle);
begin
  FBorderStyle := Value;
...
procedure TScrCustomMemo.SetCtl3D(const Value: boolean);
begin
  FCtl3D := Value;
  UpdateGutter;
  Invalidate;
...
procedure TScrCustomMemo.SetLeftCol(const Value: integer);
begin
  if FLeftCol >= 0 then
  begin
...
procedure TScrCustomMemo.SetMemoStyler(Value: TScrCustomMemoStyler);
var
  i, j: Integer;
begin
  ...
  begin
    ...
    FInternalStyles.FAllStyles.FOwner := TScrMemo(Self);
...
procedure TScrCustomMemo.SetWordWrap(const Value: TWordWrapStyle);
begin
...
procedure TScrCustomMemo.SetTopLine(const Value: integer);
var
...
procedure TScrCustomMemo.WndProc(var Message: TMessage);
begin
...
function TScrCustomMemo.DoMouseWheelDown(Shift: TShiftState; MousePos: TPoint): boolean;
var
...
function TScrCustomMemo.DoMouseWheelUp(Shift: TShiftState; MousePos: TPoint): boolean;
var
  LinesCount,tr: Integer;
begin
...
procedure TScrCustomMemo.DoOleDropFile(X, Y: integer; FileName: string);
var
  TempLines : TScrMemoStrings;
  Allow: boolean;
begin
...
procedure TScrCustomMemo.DoOleDropText(X, Y: integer; var Text: string);
var
  Allow: boolean;
begin
  Allow := true;
...
procedure TScrCustomMemo.DoOleTextDragged(Text: string; CopyText: Boolean);
begin
  if Assigned(FOnOleTextDragged) then
    FOnOleTextDragged(Self, Text, CopyText);
end;

procedure TScrCustomMemo.SetUrlAware(const Value: boolean);
begin
...
procedure TScrCustomMemo.TestforURLClick(s: string);
var
  i, x, locx: integer;
  urls: TStringList;
begin
...
procedure TScrCustomMemo.SetUrlStyle(const Value: TCharStyle);
begin
  FUrlStyle := Value;
  inherited;
end;

procedure TScrCustomMemo.ExtractURL(s: string; var urls: TStringList);
var
  s1: string;
  txt, i: integer;
  lit: char;
...
function TScrCustomMemo.TestforURLMove(s: string; locx: integer): boolean;
var
  i, x: integer;
  urls: TStringList;
begin
...
procedure TScrCustomMemo.ScrollHChange(Sender: TObject);
var
  delta: integer;
begin
...
procedure TScrCustomMemo.ScrollToBottom;
begin
...
procedure TScrCustomMemo.ScrollToTop;
begin
  TopLine := 0;
end;

procedure TScrCustomMemo.ScrollVChange(Sender: TObject);
var
  delta: integer;
begin
...
function TScrCustomMemo.FindTextCount(SearchStr: string; Options: TFindOptions): integer;
begin
  Result := FindTextInt(SearchStr, Options, true);
end;

function TScrCustomMemo.FindTextCount(SearchStr: string; Options: TFindOptionsEx): integer;
begin
  Result := FindTextInt(SearchStr, Options, true);
end;


function TScrCustomMemo.FindText(SearchStr: string; Options: TFindOptions): integer;
begin
  Result := FindTextInt(SearchStr, Options, false);
end;

function TScrCustomMemo.FindText(SearchStr: string; Options: TFindOptionsEx): integer;
begin
  Result := FindTextInt(SearchStr, Options, false);
end;


function TScrCustomMemo.FindTextInt(SearchStr: string; Options: TFindOptions; Count: boolean): integer;
var
  i, j, cit: integer;
  s: string;
...
function TScrCustomMemo.FindTextInt(SearchStr: string; Options: TFindOptionsEx; Count: boolean): integer;
var
...
function TScrCustomMemo.FindTextInMemo(SearchStr: string; Options: TFindOptions): Integer;
begin
  Result := FindText(SearchStr, Options);
end;

function TScrCustomMemo.FindTextInMemo(SearchStr: string; Options: TFindOptionsEx): Integer;
begin
  Result := FindText(SearchStr, Options);
end;


function TScrCustomMemo.FindAndReplace(SearchStr, NewStr: string; Options: TFindOptions): integer;
var
...
function TScrCustomMemo.FindAndReplace(SearchStr, NewStr: string; Options: TFindOptionsEx): integer;
var
  rz,oldrz,count: integer;
...
function TScrCustomMemo.FindTextPos(SearchStr: string; Options: TFindOptions): integer;
var
  i, j, cit: integer;
  s: string;
...
function TScrMemoStrings.GetRealCount: integer;
begin
...
function TScrMemoStrings.GetTextEx: string;
begin
  Result := inherited Text;
end;

function TScrMemoStrings.AddObject(const S: string; AObject: TObject): Integer;
var
  P: TObject;
begin
...
function TScrMemoStrings.Add(const S: string): integer;
var
  sz,sc: string;
  p: integer;
begin
...
constructor TScrMemoFindDialog.Create(AOwner: TComponent);
begin
  inherited;
  FFindDialogEx := nil;
...
destructor TScrMemoFindDialog.Destroy;
begin
  FFindDialog.Free;
  inherited;
end;

function TScrMemoFindDialog.GetFindOptions: TFindOptions;
begin
  Result := FFindDialog.Options;
end;

function TScrMemoFindDialog.GetFindText: string;
begin
  Result := FFindDialog.FindText;
end;

procedure TScrMemoFindDialog.Marker(Sender: TObject);
begin
  FAdvMemo.SetBookmark(FAdvMemo.CurY, not FAdvMemo.Bookmark[FAdvMemo.CurY]);
end;

procedure TScrMemoFindDialog.CloseDialog;
begin
  FFindDialog.CloseDialog;
end;

procedure TScrMemoFindDialog.Execute;
{$IFDEF DELPHI2006_LVL}
var
  p: TPoint;
{$ENDIF}
...
procedure TScrMemoFindDialog.DoShow(Sender: TObject);
begin
  if Assigned(OnShow) then
    OnShow(Self);
end;

procedure TScrMemoFindDialog.Close(Sender: TObject);
begin
  if Assigned(FAdvMemo) then
...
procedure TScrMemoFindDialog.FindChange(Sender: TObject; var AText: string);
begin
...
procedure TScrMemoFindDialog.FindPrevious(Sender: TObject);
begin
  FindDir(True);
end;

procedure TScrMemoFindDialog.Find(Sender: TObject);
begin
  FindDir(False);
end;

procedure TScrMemoFindDialog.FindDir(prev: boolean);
var
  rz: Integer;
  cnt: boolean;
...
begin
  ...
  begin
    ...
    if (ScrFindDialogForm.fdoDown in FFindDialogEx.Options) then
    begin
      if not prev then
        Options := Options + [freDown];
    end;

    if (ScrFindDialogForm.fdoCaseSensitive in FFindDialogEx.Options) then
      Options := Options + [freMatchCase];

    if (ScrFindDialogForm.fdoWholeWordOnly in FFindDialogEx.Options) then
      Options := Options + [freWholeWord];

    if (ScrFindDialogForm.fdoWrapAtEndOfFile in FFindDialogEx.Options) then
      wrap := true;

    if (ScrFindDialogForm.fdoExpression in FFindDialogEx.Options) then
...
procedure TScrMemoFindDialog.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
...
procedure TScrMemoFindDialog.SetFindOptions(const Value: TFindOptions);
begin
...
constructor TScrMemoFindReplaceDialog.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TScrMemoFindReplaceDialog.Destroy;
begin
  FReplaceDialog.Free;
  inherited;
end;

function TScrMemoFindReplaceDialog.GetReplaceText: string;
begin
  Result := FReplaceDialog.ReplaceText;
end;

function TScrMemoFindReplaceDialog.GetFindText: string;
begin
  Result := FReplaceDialog.FindText;
end;

function TScrMemoFindReplaceDialog.GetFindOptions: TFindOptions;
begin
  Result := FReplaceDialog.Options;
end;

procedure TScrMemoFindReplaceDialog.CloseDialog;
begin
  FReplaceDialog.CloseDialog;
end;

procedure TScrMemoFindReplaceDialog.Execute;
{$IFDEF DELPHI2006_LVL}
var
  p: TPoint;
{$ENDIF}
...
procedure TScrMemoFindReplaceDialog.Close(Sender: TObject);
begin
  if Assigned(FAdvMemo) then
    FAdvMemo.Searching := False;
...
procedure TScrMemoFindReplaceDialog.DoReplaceDone;
begin
  if Assigned(OnReplaceDone) then
    OnReplaceDone(Self);
end;

procedure TScrMemoFindReplaceDialog.DoShow(Sender: TObject);
begin
  if Assigned(OnShow) then
    OnShow(Self);
end;

procedure TScrMemoFindReplaceDialog.FindChange(Sender: TObject; var AText: string);
begin
  if AutoHighLight and (Length(AText) > 1) then
    FAdvMemo.HighlightText := AText
  else
...
procedure TScrMemoFindReplaceDialog.Find(Sender: TObject);
var
  rz: integer;
  AText: string;
  Options: TFindOptionsEx;
...
begin
  ...
  begin
    ...
    if (ScrReplaceDialogForm.fdoDown in FReplaceDialogEx.Options) then
      Options := Options + [freDown];

    if (ScrReplaceDialogForm.fdoCaseSensitive in FReplaceDialogEx.Options) then
      Options := Options + [freMatchCase];

    if (ScrReplaceDialogForm.fdoWholeWordOnly in FReplaceDialogEx.Options) then
      Options := Options + [freWholeWord];

    if (ScrReplaceDialogForm.fdoWrapAtEndOfFile in FReplaceDialogEx.Options) then
      wrap := true;

    if (ScrReplaceDialogForm.fdoExpression in FReplaceDialogEx.Options) then
      Options := Options + [freExpression];
  end
  else
  begin
...
procedure TScrMemoFindReplaceDialog.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
  if (Operation = opRemove) and (AComponent = FAdvMemo) then
    FAdvMemo := nil;
...
procedure TScrMemoFindReplaceDialog.ReplaceAll(Sender: TObject);
var
...
procedure TScrMemoFindReplaceDialog.Replace(Sender: TObject);
var
  rz, oldrz: integer;
  AReplaceText, AFindText: string;
  insel,wrap: boolean;
...
procedure TScrMemoFindReplaceDialog.SetFindOptions(const Value: TFindOptions);
begin
...
procedure TScrMemoStrings.AddStrings(Strings: TStrings);
var
  i: integer;
begin
  if Assigned(Memo) then
...
procedure TScrMemoStrings.Assign(Source: TPersistent);
var
  i: Integer;
  cf: boolean;
begin
  ...
  begin
    ...
    begin
      ...
      Memo.Clear;
      Memo.FBackupTopLine := -1;
      Memo.SetLines(TScrMemoStrings(Source));
      FListLengths.Clear;
      for i := 1 to TScrMemoStrings(Source).Count do
        FListLengths.Add(Length(TScrMemoStrings(Source)[i - 1]));
      Memo.SetMaxLength;
...
procedure TScrMemoStrings.AssignEx(Source: TPersistent);
var
  i: integer;
begin
  if Source is TStrings then
  begin
    ...
    begin
      try
        ...
        Memo.InternalLines.AddStrings(TScrMemoStrings(Source));
...
procedure TScrCustomMemoStyler.SetlistAuto(const Value: TStringList);
begin
  FListAuto.Assign(Value);
end;

procedure TScrCustomMemoStyler.SetRegionDefinitions(Value: TRegionDefinitions);
begin
  FRegionDefinitions.Assign(Value);
end;

procedure TScrCustomMemo.HideForm;
begin
  if Assigned(FormAutoCompletion) then
  begin
...
procedure TScrCustomMemo.ShowForm(ShowAlways: Boolean);
var
  s, method: string;
  i: Integer;
  ab, ae, vp, dw: Integer;
...
procedure TScrCustomMemo.hideauto(sender: Tobject);
begin
...
procedure TScrCustomMemo.UpdateCompletionList(token: string);
var
  i, j, vp, tp: Integer;
  method: string;
  dw: Integer;
...
procedure TScrCustomMemo.ListMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  FinalizeCodeCompletion(true);
end;

procedure TScrCustomMemo.ListKeyDown(Sender: TObject; var Key: Word;
...
procedure TScrCustomMemo.ListKeyPress(sender: TObject; var Key: char);
var
  s: string;
begin
  case Ord(key) of
...
procedure TScrCustomMemo.FinalizeCodeCompletion(AutoParenthesis: boolean);
var
  sEntry: string;
  flg: Boolean;
  ae, ab, vp: integer;
...
procedure TScrCustomMemo.KillEventAutoCompletion;
begin
  if Assigned(FormAutoCompletion) then
...
procedure TScrCustomMemo.SetEventAutoCompletion;
begin
...
procedure TScrCustomMemo.FormClose(Sender: TObject;
...
procedure TScrCustomMemo.PrepareShowHint;
var
  p: TPoint;
  bp, np: string;
...
procedure TScrCustomMemo.FormHintClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := cafree;
end;

procedure TScrCustomMemo.TimerHint(Sender: TObject);
begin
  FHintForm.Hide;
  Timer.Enabled := False;
end;
...
function TScrCustomMemo.ApplyCS(s: string): string;
begin
...
procedure TScrCustomMemo.Assign(Source: TPersistent);
begin
  if (Source is TScrCustomMemo) then
  begin
    AcceptFiles := (Source as TScrCustomMemo).AcceptFiles;
    ActiveLineSettings.Assign((Source as TScrCustomMemo).ActiveLineSettings);
    Align := (Source as TScrCustomMemo).Align;
    {$IFDEF DELPHI2006_LVL}
    AlignWithMargins := (Source as TScrCustomMemo).AlignWithMargins;
    {$ENDIF}
    Anchors := (Source as TScrCustomMemo).Anchors;
    AutoCompletion.Assign((Source as TScrCustomMemo).AutoCompletion);
    AutoCompletionListImages := (Source as TScrCustomMemo).AutoCompletionListImages;
    AutoCorrect.Assign((Source as TScrCustomMemo).AutoCorrect);
    AutoExpand := (Source as TScrCustomMemo).AutoExpand;
    AutoHintParameterDelay := (Source as TScrCustomMemo).AutoHintParameterDelay;
    AutoHintParameterPosition := (Source as TScrCustomMemo).AutoHintParameterPosition;
    AutoHintParameters := (Source as TScrCustomMemo).AutoHintParameters;
    AutoIndent := (Source as TScrCustomMemo).AutoIndent;
    AutoThemeAdapt := (Source as TScrCustomMemo).AutoThemeAdapt;
    BandColor := (Source as TScrCustomMemo).BandColor;
    BkColor := (Source as TScrCustomMemo).BkColor;
    BlockColor := (Source as TScrCustomMemo).BlockColor;
    BlockLineColor := (Source as TScrCustomMemo).BlockLineColor;
    BlockShow := (Source as TScrCustomMemo).BlockShow;
    BorderColor := (Source as TScrCustomMemo).BorderColor;
    BorderStyle := (Source as TScrCustomMemo).BorderStyle;
    BreakPointColor := (Source as TScrCustomMemo).BreakpointColor;
    BreakPointTextColor := (Source as TScrCustomMemo).BreakPointTextColor;
    CaseSensitive := (Source as TScrCustomMemo).CaseSensitive;
    CharCase := (Source as TScrCustomMemo).CharCase;
    ClipboardFormats := (Source as TScrCustomMemo).ClipboardFormats;
    CodeFolding.Assign((Source as TScrCustomMemo).CodeFolding);
    Ctl3D := (Source as TScrCustomMemo).Ctl3D;
    Cursor := (Source as TScrCustomMemo).Cursor;
    DelErase := (Source as TScrCustomMemo).DelErase;
    DragMode := (Source as TScrCustomMemo).DragMode;
    Enabled := (Source as TScrCustomMemo).Enabled;
    EnhancedHomeKey := (Source as TScrCustomMemo).EnhancedHomeKey;
    Font.Assign((Source as TScrCustomMemo).Font);

    Gutter.Assign((Source as TScrCustomMemo).Gutter);
    Hint := (Source as TScrCustomMemo).Hint;
    Lines.Assign((Source as TScrCustomMemo).Lines);
    MarkerList.Assign((Source as TScrCustomMemo).MarkerList);
    PrintOptions.Assign((Source as TScrCustomMemo).PrintOptions);
    ReadOnly := (Source as TScrCustomMemo).ReadOnly;
    RightMargin := (Source as TScrCustomMemo).RightMargin;
    RightMarginColor := (Source as TScrCustomMemo).RightMarginColor;
    ScrollBars := (Source as TScrCustomMemo).ScrollBars;
    ScrollHint := (Source as TScrCustomMemo).ScrollHint;
    ScrollMode := (Source as TScrCustomMemo).ScrollMode;
    SelBkColor := (Source as TScrCustomMemo).SelBkColor;
    ShowHint := (Source as TScrCustomMemo).ShowHint;
    ShowRightMargin := (Source as TScrCustomMemo).ShowRightMargin;
    SmartTabs := (Source as TScrCustomMemo).SmartTabs;
    TabStop := (Source as TScrCustomMemo).TabStop;
    Tag := (Source as TScrCustomMemo).Tag;
    TrimTrailingSpaces := (Source as TScrCustomMemo).TrimTrailingSpaces;
    UndoLimit := (Source as TScrCustomMemo).UndoLimit;
    UndoLineByLine := (Source as TScrCustomMemo).UndoLineByLine;
    URLAware := (Source as TScrCustomMemo).URLAware;
    URLStyle.Assign((Source as TScrCustomMemo).URLStyle);
    UseStyler := (Source as TScrCustomMemo).UseStyler;
    Visible := (Source as TScrCustomMemo).Visible;
    WantTab := (Source as TScrCustomMemo).WantTab;
    WordWrap := (Source as TScrCustomMemo).WordWrap;
  end;
end;

function TScrCustomMemo.NearestStart(s: string; fromX: integer; var res: string; var ParNum: Integer): integer;
var
  found: boolean;
  space: Integer;
begin
...
function TScrCustomMemo.SearchParameter: Boolean;
var
...
procedure TScrCustomMemo.FormHintMouseDown(Sender: TObject;
...
procedure TScrCustomMemo.SetAutoHintParameters(
...
procedure TScrCustomMemo.HideHint;
begin
  if FHintForm.Visible then
    FHintForm.Visible := False;
end;
...
procedure TScrCustomMemo.SetPrintOptions(const Value: TPrintOptions);
begin
  FPrintOptions.Assign(Value);
end;

procedure TScrCustomMemo.KeyUp(var Key: word; Shift: TShiftState);
begin
{$IFDEF BLINK}
  SetCaretBlinkTime(FCaretTime);
  FletgetCaretTime := True;
...
{$B+}
procedure TScrCustomMemo.CursorChanged;
var
...
procedure TScrCustomMemo.ClearErrors;
var
  i: Integer;
  Tlp: TLineProp;
begin
...
procedure TScrCustomMemo.ClearLineErrors(LineNo: Integer);
var
  Tlp: TLineProp;
begin
  Tlp := InternalLines.GetLineProp(LineNo);
...
procedure TScrCustomMemo.ClearWordError(LineNo, LinePos: Integer);
var
  Tlp: TLineProp;
  i: Integer;
begin
...
procedure TScrCustomMemo.SetError(LineNo, ErrPos, ErrLen: Integer);
var
  Tlp: TlineProp;
  r: TRect;
begin
...
procedure TScrCustomMemo.SetBlockMatchStart(LineNo, BlockStart, BlockLen: Integer);
var
  r: TRect;
begin
...
procedure TScrCustomMemo.SetBandColor(const Value: TColor);
begin
...
procedure TScrCustomMemo.SetBlockMatchEnd(LineNo, BlockStart, BlockLen: Integer);
var
  r: TRect;
begin
...
procedure TScrCustomMemo.CMHintShow(var Msg: TMessage);
var
  CanShow: Boolean;
  hi: PHintInfo;
  newPos: TCellPos;
...
procedure TScrCustomMemo.SetAutoHintParameterDelay(const Value: Integer);
begin
...
procedure TScrCustomMemo.AutoCompleteTimer(Sender: TObject);
var
  sWord: string;
...
(*
function TScrCustomMemo.SortAutoCompletList(List: TStringList; Index1, Index2: Integer): Integer;
var
...
function TScrCustomMemo.GetBookmarkInfo(Index: integer; var AValue: integer): boolean;
var
  Tlp: TlineProp;
begin
  Result := false;
...
function TScrCustomMemo.GetBookmarkIndex(Index: Integer): Integer;
var
  Tlp: TlineProp;
begin
  Result := -1;
...
function TScrCustomMemo.GetBookmarks(Index: Integer): Integer;
begin
...
procedure TScrCustomMemo.SetBookmarkIndex(Index: Integer; const Value: Integer);
var
  Tlp: TlineProp;
begin
  if Index >= InternalLines.Count then
...
procedure TScrCustomMemo.SetBookmarks(Index: Integer;
  const Value: Integer);
begin
  if (Index < 10) and (Index >= 0) then
  begin
...
procedure TScrCustomMemo.ClearUndoRedo;
begin
  ClearUndoList;
end;

procedure TScrCustomMemo.ClearBookmarks;
var
  i: Integer;
begin
  for i := 1 to 10 do
...
procedure TScrCustomMemo.GotoBookmark(Index: Integer);
begin
  if (Index < 10) and (Index >= 0) then
  begin
    if FBookmarkList.Items[Index] <> -1 then
...
function TScrCustomMemo.HasBookmarks: Boolean;
var
  i: Integer;
begin
  Result := False;
...
procedure TScrCustomMemo.SetMemoSource(const Value: TScrMemoSource);
var
  i: integer;
  s: string;
begin
...
function TScrCustomMemo.GetInternalStyles: TScrCustomMemoStyler;
begin
  if FMemoSource <> nil then
...
function TScrCustomMemo.GetInternalLines: TScrMemoStrings;
begin
  if FMemoSource <> nil then
...
procedure TScrCustomMemo.SetInternalLines(const Value: TScrMemoStrings);
begin
  if FMemoSource <> nil then
...
procedure TScrCustomMemo.GetMemoState(var value: TAdvMemoSavePos);
begin
  value.CurX := FCurX;
  value.CurY := FCurY;
...
procedure TScrCustomMemo.ResetMemoState(value: TAdvMemoSavePos);
begin
  FCurX := value.CurX;
  FCurY := value.CurY;
...
function TScrCustomMemo.InternalUndoList: TAdvUndoList;
begin
  if FMemoSource <> nil then
...
function TScrCustomMemo.CanCut: boolean;
begin
  Result := (GetSelLength > 0) and not ReadOnly;
end;

function TScrCustomMemo.CanCopy: boolean;
begin
  Result := GetSelLength > 0;
end;

function TScrCustomMemo.CanPaste: boolean;
begin
...
procedure TScrACorrect.Assign(Source: TPersistent);
begin
  if (Source is TScrACorrect) then
  begin
    Active  := (Source as TScrACorrect).Active;
    OldValue.Assign((Source as TScrACorrect).OldValue);
    NewValue.Assign((Source as TScrACorrect).NewValue);
  end;
end;

constructor TScrACorrect.Create;
begin
  inherited Create;
...
destructor TScrACorrect.Destroy;
begin
  FOldValue.Free;
  FNewValue.Free;
  inherited;
...
procedure TScrACorrect.SetDoAutoCorrect(const Value: boolean);
begin
  FDoAutoCorrect := Value;
end;

procedure TScrACorrect.SetNewValue(const Value: TStringList);
begin
  FNewValue.Assign(Value);
end;

procedure TScrACorrect.SetOldValue(const Value: TStringList);
begin
...
constructor TAutoCompletion.Create(AOwner: TScrCustomMemo);
begin
  inherited Create;
  FFont := TFont.Create;
  FOwner := AOwner;
...
function TScrCustomMemo.IsDelimiter(value: Char): boolean;
var
  i: Integer;
begin
  result := False;
...
procedure TScrCustomMemo.SetRightMargin(const Value: Integer);
begin
...
procedure TScrCustomMemo.SetRightMarginColor(const Value: TColor);
begin
...
procedure TScrCustomMemo.SetBreakpointColor(const Value: TColor);
begin
...
procedure TScrCustomMemo.SetBreakpointTextColor(const Value: TColor);
begin
...
begin
  ...
  begin
    FBmpVar.LoadFromResourceName(HInstance, 'SCR_VAR');
    FBmpProp.LoadFromResourceName(HInstance, 'SCR_PROP');
    FBmpEvent.LoadFromResourceName(HInstance, 'SCR_EVENT');
    FBmpProc.LoadFromResourceName(HInstance, 'SCR_PROC');
    FBmpMethod.LoadFromResourceName(HInstance, 'SCR_METHOD');
  end;
end;
...
function TScrCustomMemo.GetModified: boolean;
begin
  Result := (InternalUndoList.Count > 0) and (InternalUndoList.Pos < InternalUndoList.Count);
end;

procedure TScrCustomMemo.SetModified(const Value: boolean);
begin
  if not Value then
  begin
    ClearUndoList;
...
function TScrCustomMemo.GetVersionNr: Integer;
begin
  Result := MakeLong(MakeWord(BLD_VER, REL_VER), MakeWord(MIN_VER, MAJ_VER));
end;

function TScrCustomMemo.GetVersion: string;
var
  vn: Integer;
begin
  vn := GetVersionNr;
...
function TScrCustomMemo.GetVersionString: string;
var
  vn: Integer;
begin
  vn := GetVersionNr;
...
procedure TScrCustomMemo.DoWrap;
var
  i: Integer;
  os, s, nw, ns: string;
  brk, ovf: boolean;
...
procedure TScrCustomMemo.UpdateWrap;
var
  sbvis: boolean;
{$IFDEF TMSDEBUG}
  t: DWORD;
...
procedure TScrCustomMemo.UndoWrap;
var
  i, offs: Integer;
  pl: TList;
  p: TLineProp;
...
procedure TScrCustomMemo.Change;
begin
...
procedure TScrCustomMemo.DoGutterClick(LineNo: integer);
begin
  if Assigned(FOnGutterClick) then
    FOnGutterClick(Self, LineNo);
end;

procedure TScrCustomMemo.DoInsertAutoCompletionEntry(var AEntry: string);
begin
  if Assigned(FOnInsertAutoCompletionEntry) then
    FOnInsertAutoCompletionEntry(Self, AEntry);
end;

procedure TScrCustomMemo.DoIsURL(AToken: string; var IsURL: boolean);
begin
  if Assigned(OnIsURL) then
    OnIsURL(Self, AToken, IsURL);
end;

procedure TScrCustomMemo.DoGetAutoCompletionList(AToken: string; AList: TStringList);
begin
  if Assigned(FOnGetAutoCompletionList) then
    FOnGetAutoCompletionList(Self, AToken, AList);
end;

procedure TScrCustomMemo.DoGetAutoCompletionListIndex(AToken: string;
...
procedure TScrCustomMemo.DoGetParameterHint(AToken: string;
...
procedure TScrCustomMemo.DoGetParameterHintInfo(var AToken: string;
...
constructor TScrMemoSource.Create(AOwner: TComponent);
begin
  inherited;
  FLines := TScrMemoStrings.Create;
  FUndoList := TAdvUndoList.Create;
...
destructor TScrMemoSource.Destroy;
begin
  FLines.Free;
  FUndoList.Free;
  FBookmarkList.Free;
...
function TScrMemoSource.GetModified: boolean;
begin
  if FMemo <> nil then
...
function TScrMemoSource.GetReadOnly: boolean;
begin
  if FMemo <> nil then
...
function TScrMemoSource.GetUndoList: TAdvUndoList;
begin
  result := FUndoList;
end;

procedure TScrMemoSource.Loaded;
begin
  inherited;
end;

procedure TScrMemoSource.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
...
procedure TScrMemoSource.SetLines(const Value: TScrMemoStrings);
begin
  FLines.Clear;
  FLines.AddStrings(Value);
end;

procedure TScrMemoSource.SetMemo(const Value: TScrCustomMemo);
var
  i: Integer;
  ...
  //FMemo.ClearUndoList;
      ...
      begin
        if Assigned(FSyntaxStyler) then
          FSyntaxStyler.FAllStyles.FOwner := TScrMemo(FMemo);
...
procedure TScrMemoSource.SetModified(const Value: boolean);
begin
  if (FMemo <> nil) then
...
procedure TScrMemoSource.SetReadOnly(const Value: boolean);
begin
  if FMemo <> nil then
...
procedure TScrMemoSource.SetSyntaxStyler(const Value: TScrCustomMemoStyler);
begin
...
procedure TScrMemoSource.SetUndoList(const Value: TAdvUndoList);
{$IFNDEF DELPHI7}
var
  i: Integer;
{$ENDIF}
...
destructor TScrMemoAction.Destroy;
begin
...
function TScrMemoAction.GetControl(Target: TObject): TScrCustomMemo;
begin
...
called before ExecuteTarget and UpdateTarget, however, we're being safe. }
  Result := Target as TScrCustomMemo;
end;

function TScrMemoAction.HandlesTarget(Target: TObject): Boolean;
begin
  Result := ((Control <> nil) and (Target = Control) or
    (Control = nil) and (Target is TScrCustomMemo)) and TCustomEdit(Target).Focused;
end;

procedure TScrMemoAction.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
...
procedure TScrMemoAction.UpdateTarget(Target: TObject);
begin
  if (Self is TScrMemoCut) or (Self is TScrMemoCopy) then
    Enabled := GetControl(Target).SelLength > 0;
end;

procedure TScrMemoAction.SetControl(Value: TScrCustomMemo);
begin
...
procedure TScrMemoCopy.ExecuteTarget(Target: TObject);
begin
...
procedure TScrMemoCut.ExecuteTarget(Target: TObject);
begin
...
procedure TScrMemoPaste.ExecuteTarget(Target: TObject);
begin
  GetControl(Target).PasteFromClipboard;
end;

procedure TScrMemoPaste.UpdateTarget(Target: TObject);
begin
...
procedure TScrMemoSelectAll.ExecuteTarget(Target: TObject);
begin
  GetControl(Target).SelectAll;
end;

procedure TScrMemoSelectAll.UpdateTarget(Target: TObject);
begin
...
procedure TScrMemoUndo.ExecuteTarget(Target: TObject);
begin
  GetControl(Target).Undo;
end;

procedure TScrMemoUndo.UpdateTarget(Target: TObject);
begin
...
procedure TScrMemoRedo.ExecuteTarget(Target: TObject);
begin
  GetControl(Target).Redo;
end;

procedure TScrMemoRedo.UpdateTarget(Target: TObject);
begin
...
procedure TScrMemoDelete.ExecuteTarget(Target: TObject);
begin
  GetControl(Target).DeleteSelection;
end;

procedure TScrMemoDelete.UpdateTarget(Target: TObject);
begin
  Enabled := GetControl(Target).SelLength > 0;
end;
...
begin
  FMarkers.Clear;
  TScrCustomMemo(FOwner).FGutter.Invalidate;
end;
...
begin
  inherited;
  if (Operation in [opInsert]) and (Comp.ClassParent = TScrCustomMemoStyler) then
    FStylerList.Add(Pointer(Comp));
  if (Operation in [opRemove]) and (Comp.ClassParent = TScrCustomMemoStyler) then
  begin
...
procedure TScrMemoChecker.CheckLine(LineNo: Integer);
var
  s:string;
begin
  if Assigned(FMemo) then
...
procedure TScrMemoChecker.CorrectLine(LineNo: Integer);
begin

end;

procedure TScrMemoChecker.CorrectWord(LineNo,LinePos: Integer; var s: string);
begin
end;

function TScrMemoChecker.CurrentWordPos(X,Y: integer): TPoint;
var
  r: TRect;
begin
  r := Memo.CellRect(X,Y);
...
procedure TScrMemoChecker.CheckWord(LineNo,LinePos: Integer; s: string);
begin
end;

procedure TScrMemoChecker.Notification(AComponent: TComponent;
...
function TScrMemoChecker.ReplaceOnce(const S, OldPattern, NewPattern: string): string;
var
...
procedure TScrMemoChecker.AddUndo(LineNo: integer; UndoLine: string);
var
  cu: TCorrectUndo;
begin
...
procedure TScrMemoChecker.CheckAllLines;
var
  i: Integer;
begin
  for i := 1 to Memo.Lines.COunt do
...
procedure TScrMemoChecker.CorrectAllLines;
var
  i: Integer;
begin
  for i := 1 to Memo.Lines.COunt do
...
constructor TScrMemoCapitalChecker.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
procedure TScrMemoCapitalChecker.CorrectLine(LineNo: Integer);
var
  Prev,PrevPrev: char;
  i: Integer;
  s: string;
...
constructor TAdvMemoDropTarget.Create(AMemo: TScrCustomMemo);
begin
  inherited Create;
  FAdvMemo := AMemo;
end;

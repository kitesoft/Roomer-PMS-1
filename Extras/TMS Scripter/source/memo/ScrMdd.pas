...
uses
  Windows, Messages, ActiveX, Classes, ShlObj;

type
  TAMDropTarget = class (TInterfacedObject, IDropTarget)
  ...
  public
    constructor Create;
    function WantsFiles: boolean; virtual;
    function WantsText: boolean; virtual;
    ...
    procedure DragMouseEnter(pt: TPoint); virtual; abstract;
  end;

  TAMDropSource = class (TInterfacedObject, IDropSource)
  private
    fNoAccept:boolean;
  public
...
constructor TAMDropTarget.Create;
begin
  inherited Create;
end;

function TAMDropTarget.DragEnter(const DataObj: IDataObject; grfKeyState: Longint; pt: TPoint; var dwEffect: Longint): HResult;
begin
  with TEnumFormats.Create(DataObj) do
  try
    FOk := (HasText and WantsText) or (HasFile and WantsFiles);
  finally
    Free
  end;
...
function TAMDropTarget.DragOver(grfKeyState: Longint; pt: TPoint; var dwEffect: Longint): HResult;
var
  Allow:boolean;
begin
  if FOk then
...
function TAMDropTarget.DragLeave: HResult;
begin
  Result := NOERROR
end;

function TAMDropTarget.Drop(const DataObj: IDataObject; grfKeyState: Longint; pt: TPoint; var dwEffect: Longint): HResult;
begin
  if FOk then
  begin
    with TEnumFormats.Create(DataObj) do
...
procedure TAMDropTarget.DropText(pt:tpoint;s:string);
begin
end;

function TAMDropTarget.WantsFiles: boolean;
begin
  Result := true;
end;

function TAMDropTarget.WantsText: boolean;
begin
  Result := true;
end;

procedure TAMDropTarget.DropRTF(pt:TPoint;s:string);
begin
end;

procedure TAMDropTarget.DropFiles(pt:TPoint;files:TStrings);
begin
end;

procedure TAMDropTarget.DragMouseMove(pt: TPoint; var Allow: boolean);
begin
  Allow := true;
end;
...
function TAMDropSource.QueryContinueDrag(fEscapePressed: BOOL; grfKeyState: Longint): HResult;
begin
  Result := S_OK;
...
function TAMDropSource.GiveFeedback(dwEffect: Longint): HResult;
begin

...
uses
  Windows, Messages, SysUtils, Classes, Controls, StdCtrls, Graphics, ScrMemo,
  Forms, ScrCodeHint, ImgList, Clipbrd, Dialogs, ScrCodeListDD, ActiveX
  {$IFDEF DELPHIXE3_LVL}
  , System.UITypes
  {$ENDIF};
...
const
  ...
  // 2.0.0.1 : Fixed issue with hints for TScrCodeList on secondary forms
  ...
  // 3.0.0.0 : Build with TScrMemo v3.0
...
type
  ...
  public
    constructor Create(AOwner: TScrCodeList);
    ...
    property CodeList: TScrCodeList read FOwner;
  end;
  ...
  {$ENDIF}
  TScrCodeList = class(TCustomListBox)
  private
    FHintBlock: integer;
    ...
    InternalStyles: TScrCustomMemoStyler;
    ...
    procedure SetInternalStyles(const Value: TScrCustomMemoStyler);
  ...
  published
    property SyntaxStyles: TScrCustomMemoStyler read InternalStyles write SetInternalStyles;
  ...
  private
    FCodeList: TScrCodeList;
  public
    constructor Create(ACodeList: TScrCodeList);
...
//------------------------------
constructor TCodeBlocks.Create(AOwner: TScrCodeList);
begin
...
constructor TScrCodeList.Create(AOwner: TComponent);
begin
...
procedure TScrCodeList.CreateWnd;
begin
  inherited;
  FClipChainHandle := SetClipboardViewer(Self.Handle);
end;

destructor TScrCodeList.Destroy;
begin
  FCodeBlocks.Free;
  inherited;
end;

procedure TScrCodeList.AddBlock(Source: String);
begin
  CodeBlocks.Add(Source);
end;

procedure TScrCodeList.CMMouseEnter(var Msg: TMessage);
begin
  inherited;
...
procedure TScrCodeList.CMMouseLeave(var Msg: TMessage);
begin
  inherited;
...
procedure TScrCodeList.CNDrawItem(var Message: TWMDrawItem);
var
...
procedure TScrCodeList.DrawItem(Index: Integer; Rect: TRect; State: TOwnerDrawState);
begin

end;

procedure TScrCodeList.DrawItemEx(ACanvas: TCanvas; Index: Integer; ARect: TRect; State: TOwnerDrawState);
var
  allb: integer;
...
procedure TScrCodeList.DrawCustomLine(ACanvas: TCanvas; LineNo, SubLineNo: Integer; var style: TStyle;DM: TDrawMode; PR: TRect);
var
...
procedure TScrCodeList.ExtractURL(s: string; var urls: TStringList);
begin
...
procedure TScrCodeList.KeyDown(var KeyCode: word; Shift: TShiftState);
begin
  inherited;
...
procedure TScrCodeList.KeyUp(var KeyCode: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TScrCodeList.MouseDown(Button: TMouseButton; Shift: TShiftState; X,Y: Integer);
var
  a: integer;
  P: TPoint;
begin
...
procedure TScrCodeList.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  a: integer;
  P: TPoint;
  s: string;
...
procedure TScrCodeList.MouseUp(Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer);
begin
  inherited;
  FMouseDown := false;
...
function TScrCodeList.GetBlock(index: integer): String;
begin
...
procedure TScrCodeList.SetBlock(index: integer; Value: String);
begin
  if (index >= FBlockCount) or (index < 0) then
    raise exception.Create('Invalid Block Index '+IntToStr(Index));
...
procedure TScrCodeList.Notification(AComponent: TComponent; AOperation: TOperation);
begin
...
procedure TScrCodeList.DrawBorders;
var
  DC: HDC;
  OldPen: HPen;
  ARect: TRect;
...
procedure TScrCodeList.WMNCPaint(var Message: TMessage);
begin
  inherited;
...
procedure TScrCodeList.WndProc(var Message: TMessage);
begin
  inherited;
...
procedure TScrCodeList.WMChangeCBChain(var Msg: TMessage);
begin
  FClipChainHandle := Msg.LParam;
end;

procedure TScrCodeList.WMDestroy(var Msg: TWMDestroy);
begin
...
procedure TScrCodeList.WMPaint(var Message: TWMPaint);
var
  DC, MemDC: HDC;
...
procedure TScrCodeList.WMDrawClipboard(var Msg: TMessage);
var
  newIndex: Integer;
  allow: boolean;
  clipstr: string;
...
procedure TScrCodeList.WMLButtonDblClk(var Message: TWMLButtonDblClk);
var
  P: TPoint;
  a: integer;
begin
...
procedure TScrCodeList.SetCodeBlockCaptionColor(const Value: TColor);
begin
...
procedure TScrCodeList.SetCodeBlockCaptionSelectColor(const Value: TColor);
begin
...
procedure TScrCodeList.SetCodeBlockCaptionTextColor(const Value: TColor);
begin
...
procedure TScrCodeList.SetCodeBlockCaptionSelectTextColor(const Value: TColor);
begin
  if (FCodeBlockCaptionTextColor <> Value) then
  begin
    FCodeBlockCaptionTextColor := Value;
...
procedure TScrCodeList.SetCodeBlockColor(const Value: TColor);
begin
...
procedure TScrCodeList.SetCodeBlockColorTo(const Value: TColor);
begin
...
procedure TScrCodeList.SetCodeBlockSelectColor(const Value: TColor);
begin
...
procedure TScrCodeList.SetCodeBlockSelectColorTo(const Value: TColor);
begin
...
procedure TScrCodeList.SetCodeBorderColor(const Value: TColor);
begin
  FCodeBorderColor := Value;
  Invalidate;
end;

procedure TScrCodeList.SetCodeBorderSelectColor(const Value: TColor);
begin
  FCodeBorderSelectColor := Value;
  Invalidate;
end;

procedure TScrCodeList.SetInternalStyles(const Value: TScrCustomMemoStyler);
begin
  InternalStyles := Value;
  SetActiveStyler(Value);
end;

procedure TScrCodeList.SetItemSize(Index, Size: integer);
begin
 SendMessage(Handle,LB_SETITEMHEIGHT,Index, Size);
end;

procedure TScrCodeList.SetItemStyle(const Value: TItemStyle);
begin
  FItemStyle := Value;
  case FItemStyle of
...
procedure TScrCodeList.SetOleDropSource(const Value: Boolean);
begin
  FOleDropSource := Value;
end;

procedure TScrCodeList.SetOleDropTarget(const Value: Boolean);
begin
...
procedure TScrCodeList.SetUseStyler(const Value: boolean);
begin
...
procedure TScrCodeList.ReMeasure;
var
  i, Height: integer;
begin
  if ItemStyle = isFixedHeight then
...
procedure TScrCodeList.Resize;
begin
  inherited;
  Invalidate;
end;

procedure TScrCodeList.DeleteBlock;
var
  Allow: Boolean;
begin
  // Attempt Code Delete...
...
function TScrCodeList.GetVersionNr: Integer;
begin
  Result := MakeLong(MakeWord(BLD_VER, REL_VER), MakeWord(MIN_VER, MAJ_VER));
end;

function TScrCodeList.GetVersion: string;
var
  vn: Integer;
begin
  vn := GetVersionNr;
...
procedure TScrCodeList.SetVersion(const Value: string);
begin

end;

procedure TScrCodeList.SetCodeBlocks(const Value: TCodeBlocks);
begin
...
constructor TCodeListDropTarget.Create(ACodeList: TScrCodeList);
begin
  inherited Create;
  FCodeList := ACodeList;
end;

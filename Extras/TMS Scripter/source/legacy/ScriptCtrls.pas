...
interface
uses
  Windows, SysUtils, Classes, Graphics, ComCtrls, ExtCtrls, ImgList, atScript, ScrMemo;
...
type
  ...
  ///    The component works attached to a TScrMemo. It plugs into OnChange event of the memo and whenever the script source
  ...
  private
    ...
    procedure SetAdvMemo(const Value: TScrMemo);
    procedure TimerEvent(Sender: TObject);
    procedure CompilationFinished(Sender: TObject);
    procedure AssignChangeEvent(AMemo: TScrMemo);
    procedure UnassignChangeEvent(AMemo: TScrMemo);
  ...
  public
    ...
    ///  </summary>
    ///  <param name="ASource">
    ///  Contains the script source code to be displayed. If do not use this parameter, the content of memo will be used.
    /// </param>
    ///  <remarks>
  ...
  published
    ...
    ///  <summary>
    ///    Points to the TScrMemo component that will be in sync with the source explorer
    ///  </summary>
    ///  <remarks>
    ///    You can optionally set AdvMemo property to associate a memo with the source explorer. When you do that, source explorer
    ///    plugs an event handler to the TScrMemo.OnChange event, so that when the memo changes, the source explorer is automatically
    ...
    ///  </remarks>
    property AdvMemo: TScrMemo read FAdvMemo write SetAdvMemo;
...
procedure TSourceExplorer.SetAdvMemo(const Value: TScrMemo);
begin
  if FAdvMemo <> nil then
    UnassignChangeEvent(FAdvMemo);
  FAdvMemo := Value;
...
procedure TSourceExplorer.AssignChangeEvent(AMemo: TScrMemo);
begin
...
procedure TSourceExplorer.UnassignChangeEvent(AMemo: TScrMemo);
begin
  if (AMemo <> nil) and not (csDesigning in ComponentState) then
  begin

...
uses
  ...
  Dialogs, ScrMemo, Scrmps, StdCtrls, atScript, atPascal, ComCtrls,
  ToolWin, ImgList, ExtCtrls, Menus, ActnList, Scrmbs, atBasic,
  FormScript, fWatchViewer, fWatchPropertiesLegacy
  {$IFDEF DELPHI6_LVL}
  , Variants
  {$ENDIF}
  ;
...
type
  ...
  TDebugForm = class(TForm)
    ...
    AdvPascalMemoStyler1: TScrPascalMemoStyler;
    AdvBasicMemoStyler1: TScrBasicMemoStyler;
  ...
  private
    ...
    procedure PrepareScriptToRun(UpdateSource: boolean);
...
begin
  Scripter.CurrentScript.Breaks.ToggleBreak(line + 1);
...
begin
   ...
   begin
      ...
      begin
        ...
        PrepareScriptToRun(True);
   end else
     PrepareScriptToRun(False);
   FDebugAction := daNone;
end;

procedure TDebugForm.PrepareScriptToRun(UpdateSource: boolean);
...
begin
  ...
  with Scripter do
  begin
    if not Running and UpdateSource then
    begin
...
begin
   ...
   with AdvMemo1, Lines do
     for c:=0 to Count-1 do
        BreakPoint[c] := false;
   for c := 0 to Scripter.CurrentScript.Breaks.Count - 1 do
   begin
     line := Scripter.CurrentScript.Breaks[c].Line - 1;
     if line < AdvMemo1.Lines.Count then
       AdvMemo1.BreakPoint[line] := True;
   end;
end;

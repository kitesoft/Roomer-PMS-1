...
interface                                                
uses
  Types, Windows, SysUtils, Messages, Classes, Graphics, Controls, Forms, ComCtrls,
  atScript, PropertyList, PropertyInspEditors, TypInfo, ScrMemo, atScripter, ImgList, CommCtrl, Menus,
  Vcl.ScripterInit,
  {$IFDEF DELPHI6_LVL}
  Variants,
  {$ENDIF}
...
type
  ...
  private
    ...
    procedure FillEventValues(Comp: TComponent; Events: TStrings);
    procedure LoadEventValues(Comp: TComponent; Events: TStrings);
  protected
    procedure DoComponentBitmap(Comp: TComponent; ABitmap: TBitmap; var Handled: boolean); override;
    procedure DoCopyComponent(Writer: TWriter; Comp: TComponent); override;
    procedure DoPasteComponent(Stream: TStream; Comp: TComponent); override;
    function GetIDEComponentEditor(AComp: TComponent): TIDEComponentEditor; override;
    procedure NotifyChange(AOperationName: string); override;
  ...
  public
    ...
    property CurrentEngine: TIDEEngine read IDEEngine;
  ...
  private
    ...
    procedure SetEditForm(const Value: TForm);
    procedure SetEngine(const Value: TIDEEngine);
    procedure FocusForm;
    ...
    procedure NotifyControlChanged;
  protected
    procedure Notification(AComponent: TComponent; Operation: TOperation); override;
  ...
  public
    ...
    ///  <summary>
    ///    Contains the TIDEScriptForm form that will be designed.
    ///  </summary>
  ...
  public
    property ToolClass: TComponentClass read FClass;
  end;
  ...
  private
    ...
    procedure SetButtonHints(Value: TStrings);
  ...
  public
    ...
    ///  <summary>
    ///    Translations for Component hints with ClassName=Text format.
    ///  </summary>
  ...
  protected
    ...
    procedure DoGetAutoCompletionListIndex(AToken: string; AList: TStringList; var DefaultIndex: integer); override;
    procedure DoGetParameterHint(AToken: string; var AParameterHint: string; var Handled: boolean); override;
    procedure DoGetParameterHintInfo(var AToken: string; var AParIndex: integer; var Found: boolean; var Handled: boolean); override;
    procedure DoInsertAutoCompletionEntry(var AEntry: string); override;
    function IsCompletionListActivationChar(Ch: Char): boolean; override;
    function IsParameterHintActivationChar(Ch: Char): boolean; override;
    function IsCompletionListEndChar(Ch: Char): boolean; override;
    procedure Change; override;
  ...
  public
    ...
    property Engine: TIDEEngine read FEngine write SetEngine;
  published
    property AutoHintParameters default hpManual;
    property AutoHintParameterDelay default 0;
  end;
  ...
  private
    ...
    procedure SetEngine(const Value: TIDEEngine);
  protected
    procedure DoCompileError(var msg: string; row,col: integer; var AShow: boolean; AScript: TatScript); override;
  ...
  public
    ...
    ///  </summary>
    procedure GetPageNames(AStrings: TStrings; AEngine: TIDEEngine = nil);
  ...
  public
    ...
    ///  <summary>
    ///    ActionName property holds the action in designer (undo, insert component, moving, etc.) which lead to the
    ///    current designer state holded by the undo item.
    ///  </summary>
  ...
  ///  </summary>
  TDesignerUndoStack = class(TCollection)
  private
    FCurrent: integer;
    FFile: TIDEProjectFile;
    FUndoLevel: integer;
    ...
    property UndoLevel: integer read FUndoLevel write FUndoLevel;
  public
    ///  <summary>
    ///    Creates the object.
    ///  </summary>
    constructor Create(AFile: TIDEProjectFile);
  end;
  ...
  private
    ...
    procedure SetForm(const Value: TIDEScriptForm);
    ...
    procedure LoadFormFromStream(AStream: TStream; ADFMFormat: TDFMFormat = dfmText);
    procedure SaveFormToStream(AStream: TStream; ADFMFormat: TDFMFormat = dfmText);
    procedure ClearUndoStack;
  protected
  ...
  public
    ...
    ///  </summary>
    property Form: TIDEScriptForm read FForm;
  ...
  public
    ...
    ///  </summary>
    function FindByForm(AForm: TIDEScriptForm): TIDEProjectFile;
  ...
  ///  </remarks>
  TIDENotificationType = (ntComponentSelected, ntActiveFileChanged, ntInspectorChanged, ntDesignerChanged);
  ...
  private
    ...
    FFileExtForm: string;
    FPascalStyler: TScrCustomMemoStyler;
    FBasicStyler: TScrCustomMemoStyler;
    ...
    FCodeInsightContext: TCodeInsightContext;
    FUndoLevel: integer;
    FComponentsFilter: TStrings;
    FProjectExt: string;
    ...
    procedure NotifyInspectorChange(AOperationName: string);
    ...
    procedure FormNameChanged(AForm: TIDEScriptForm; AOldName: string);
    ...
    procedure GetAutoCompletionListIndex(AToken: string; AList: TStringList; var DefaultIndex: integer);
    procedure GetParameterHint(var AToken: string; var AHint: string);
    procedure GetParameterHintInfo(var AToken: string; var AParIndex: integer; var Found: boolean);
    procedure DoInsertAutoCompletionEntry(var AEntry: string);
    ...
    function FindDesignerInForm(AForm: TComponent): TIDEFormDesigner;
    ...
    procedure AdjustCompletionListToMemo(AList: TStrings);
    function UndoStack: TDesignerUndoStack;
    procedure PushUndoStack(AActionName: string);
    function ComponentVisible(RegisterComp: TIDERegisteredComp): boolean;
    procedure SetComponentsFilter(const Value: TStrings);
  protected
    function CreateScriptForm: TIDEScriptForm; virtual;
    procedure PrepareSaveDialog(ADlg: TOpenDialog; AFile: TIDEProjectFile); virtual;
    procedure PrepareOpenDialog(ADlg: TOpenDialog); virtual;
    procedure PrepareOpenProjectDialog(ADlg: TOpenDialog); virtual;
    procedure PrepareSaveProjectDialog(ADlg: TOpenDialog); virtual;
    ...
    function DoLoadFile(IDEFileType: TIDEFileType; AFileName: string; var AContent: string;
      AFile: TIDEProjectFile): boolean; virtual;
    function DoSaveFile(IDEFileType: TIDEFileType; AFileName: string; AContent: string;
      AFile: TIDEProjectFile): boolean; virtual;
    procedure DoComponentPlaced(AComponent: TComponent); virtual;
    procedure DoDesignerSelectionChanged; virtual;
  ...
  public
    ...
    ///  <summary>
    ///    Performs an Undo operation in the form designer. If there is nothing to undo, or if
    ///    there is no form being designed, nothing happens.
    ///  </summary>
    procedure UndoDesigner;

    ///  <summary>
    ///    Indicates if an undo operation can be performed on form designer. If no form is being displayed
    ///    it always return false
    ///  </summary>
    function CanUndoDesigner: boolean;

    ///  <summary>
    ///    Performs an Undo operation in the form designer. If there is nothing to redo, or if
    ///    there is no form being designed, nothing happens.
    ///  </summary>
    procedure RedoDesigner;

    ///  <summary>
    ///    Indicates if a redo operation can be performed on form designer. If no form is being displayed
    ///    it always return false
    ///  </summary>
    function CanRedoDesigner: boolean;

    ///  <summary>
    ///    Retrieves the name of next undo action for the form designer
    ///    If it is an empty string, then there is no undo action to perform.
    ///  </summary>
    function NextUndoDesignerAction: string;

    ///  <summary>
    ///    Retrieves the name of next redo action for the form designer
    ///    If it is an empty string, then there is no redo action to perform.
    ///  </summary>
    function NextRedoDesignerAction: string;

    ///  <summary>
    ///    For internal use only. Do not rely on this method.
    ///  </summary>
    procedure NotifyDesignerChange(AOperationName: string);
    ...
    ///  </summary>
    ///  <remarks>
    ///    Use ComponentsFilter property if you want to specify a subset of components to be displayed in the component palette
    ///    Only components added to this list will be displayed. This list must contain the class name of components, like the following.
    ///    TButton
    ///    TStringGrid
    /// 
    ///    if this list is empty, all components will be displayed
    ///  </remarks>
    property ComponentsFilter: TStrings read FComponentsFilter write SetComponentsFilter;
  ...
  published
    ...
    /// <summary>
    ///   Specified the default extension for TMS Scripter project files.
    ///   Default value is ".ssproj". This value will be used in open/file
    ///   dialogs used to load/save project fiels.
    /// </summary>
    property ProjectExt: string read FProjectExt write FProjectExt;
  end;

  TCompEvents = class(TComponent)
  private
    FEvents: TStrings;
...
uses
  ...
  UITypes,
  {$ENDIF}
  atPascal, atBasic, Scrmps, SCrmbs, MenuInspIDEEditor, MenuInspEditor,
  {$IFDEF DELPHI2009_LVL}
  Character,
  {$ENDIF}
...
function ScriptFormFromProp(P: TProperty): IScriptBasedForm;
begin
  result := nil;
  if Assigned(P.Instance) and (P.Instance is TComponent) then
  begin
    if Supports(P.Instance, IScriptBasedForm, Result) then
      Exit
    else
    if Assigned(TComponent(P.Instance).Owner) and
      Supports(TComponent(P.Instance).Owner, IScriptBasedForm, Result) then
      Exit;
  end;
end;

function GetEventPropertyValue(P: TProperty): string;
var
  SForm: IScriptBasedForm;
  CompName: string;
begin
  SForm := ScriptFormFromProp(P);
  if (SForm <> nil) and (P.Instance is TComponent) then
  begin
    if P.Instance = SForm.GetForm then
      CompName := 'Self'
    else
      CompName := TComponent(P.Instance).Name;
    result := SForm.GetSaveEvents.Values[Format('%s.%s', [CompName, P.Name])];
  end;
end;

procedure SetEventPropertyValue(P: TProperty; const Value: string);
var
  SForm: IScriptBasedForm;
  CompName: string;
begin
  ...
  begin
    if P.Instance = SForm.GetForm then
      ...
      CompName := TComponent(P.Instance).Name;
    SForm.GetSaveEvents.Values[Format('%s.%s', [CompName, P.Name])] := Value;
  end;
end;
...
begin
  if FDesignControl <> nil then
    FDesignControl.NotifyControlChanged;
end;
...
procedure TIDEFormDesignControl.SetEditForm(const Value: TForm);
begin
...
procedure TIDEFormDesignControl.NotifyControlChanged;
begin
  if FEngine <> nil then
    Engine.NotifyDesignerChange('Form Move/Resize');
end;
...
begin
  ...
  begin
    Engine.RegisteredComps.GetPageNames(Tabs, Engine);
    UpdatePanel;
...
begin
  ...
  begin
    for c := 0 to Engine.RegisteredComps.Count - 1 do
      if (SameText(CurrentTabName, Engine.RegisteredComps[c].Page)) and
        (Engine.ComponentVisible(Engine.RegisteredComps[c])) then
      begin
...
procedure TIDEPaletteToolbar.SetButtonHints(Value: TStrings);
begin
  FButtonHints.Assign(Value);
end;
...
procedure TIDEFormDesigner.NotifyChange(AOperationName: string);
begin
  inherited;
  if IDEEngine <> nil then
    IDEEngine.NotifyDesignerChange(AOperationName);
end;

procedure TIDEFormDesigner.DoComponentBitmap(Comp: TComponent; ABitmap: TBitmap;
  var Handled: boolean);
var
  RegComp: TIDERegisteredComp;
begin
  inherited DoComponentBitmap(Comp, ABitmap, Handled);
  if not Handled and (IDEEngine <> nil) and (Comp <> nil) then
  begin
    RegComp := IDEEngine.RegisteredComps.FindByClass(Comp.ClassType);
    if RegComp <> nil then
      IDEEngine.DoGetComponentImage(RegComp, ABitmap, Handled);
  end;
end;
...
procedure TIDEFormDesigner.DoCopyComponent(Writer: TWriter; Comp: TComponent);
var
  E: TCompEvents;
begin
  E := TCompEvents.Create(nil);
  try
    FillEventValues(Comp, E.Events);
    Writer.WriteSignature;
    E.Name := 'Events__';
    Writer.WriteComponent(E);
  finally
    E.Free;
  end;
end;

procedure TIDEFormDesigner.DoPasteComponent(Stream: TStream; Comp: TComponent);
var
  Temp: TComponent;
  E: TCompEvents;
  BinStream: TStream;
...
begin
  ...
  try
    ...
    Reader := TReader.Create(BinStream, 2048);
    try
      ...
      try
        ...
        Temp := Reader.ReadComponent(nil);
        try
          if not (Temp is TCompEvents) then
            raise EReadError.Create('Invalid clipboard format. Events not found.');
          E := TCompEvents(Temp);
          LoadEventValues(Comp, E.Events);
        finally
          Temp.Free;
        end;
      finally
...
procedure TIDEFormDesigner.FillEventValues(Comp: TComponent; Events: TStrings);
var
  List: TPropertyList;
  I: integer;
begin
 List := TPropertyList.Create(nil);
  try
    List.Instance := Comp;
    List.Update;
    for I := 0 to List.Count - 1 do
      if List.Properties[I].TypeKind = tkMethod then
        Events.Values[List.Properties[I].Name] := GetEventPropertyValue(List.Properties[I]);
  finally
    List.Free;
  end;
end;

function TIDEFormDesigner.GetIDEComponentEditor(
  AComp: TComponent): TIDEComponentEditor;
begin
  result := inherited GetIDEComponentEditor(AComp);
end;
...
procedure TIDEFormDesigner.LoadEventValues(Comp: TComponent; Events: TStrings);
var
  List: TPropertyList;
  I: integer;
  Prop: TProperty;
begin
 List := TPropertyList.Create(nil);
  try
    ...
    begin
      Prop := List.FindProperty(Events.Names[I]);
      if (Prop <> nil) and (Prop.TypeKind = tkMethod) then
        SetEventPropertyValue(Prop, Events.Values[Prop.Name]);
    end;
  finally
    List.Free;
  end;
...
begin
  ...
  if Assigned(FEngine) then
    FEngine.NotifyInspectorChange(Format('%s Property', [GetName(TheIndex)]));
end;
...
begin
  ...
  begin
    Result := GetEventPropertyValue(P);
  end else
...
var
  ...
  S: string;
begin
  {If property is an event, then save the events in the SaveEvents property of TIDEScriptForm
   for posterior loading purposes}
  if P.TypeKind = tkMethod then
  begin
    SetEventPropertyValue(P, Value);
  end else
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        for c := 0 to SForm.GetSaveEvents.Count - 1 do
        begin
          S := Uppercase(SForm.GetSaveEvents[c]);
          if Copy(S, 1, Length(CompName)) = CompName then
            SForm.GetSaveEvents[c] := Value + '.' + Copy(SForm.GetSaveEvents[c], Length(CompName) + 1, MaxInt);
        end;
      end;
      ...
      if (P.Instance = SForm.GetForm) and (Engine <> nil) then
        Engine.FormNameChanged(SForm.GetForm, OldName);
    end;
  end else
    inherited SetPropertyValue(P, Value);
end;
...
procedure TIDERegisteredComps.GetPageNames(AStrings: TStrings; AEngine: TIDEEngine = nil);
var
  c: integer;
begin
  AStrings.Clear;
  for c := 0 to Count - 1 do
    if (AStrings.IndexOf(Items[c].Page) = -1) and
      ((AEngine = nil) or AEngine.ComponentVisible(Items[c])) then
...
begin
  inherited;
  RegisterClass(TIDEScriptForm);
  RegisterClass(TCompEvents);

  FComponentsFilter := TStringList.Create;
  FUndoLevel := 10;
  FPascalStyler := TScrPascalMemoStyler.Create(nil);
  FBasicStyler := TScrBasicMemoStyler.Create(nil);
  FAutoStyler := true;
  ...
  FCodeInsightContext := TCodeInsightContext.Create;
...
begin
  ...
  begin
    ...
    begin
      result := FormIntf.GetSaveProps.Values[Prop.FullName];
    end
    else
    begin
      Prop.SetEmulatedOff;
...
begin
  ...
  begin
    ...
    begin
      ...
      if FormIntf.GetForm is TWinControl then
        SendMessage(TWinControl(FormIntf.GetForm).Handle, CM_RECREATEWND, 0, 0);
      Prop.SetEmulatedOn;
    end;
  end;
end;
...
begin
  ...
  RegisterCustomProperty(FormClass, 'Left'        , typeinfo(integer)         , true, GetScriptFormProp, SetScriptFormProp);
  RegisterCustomProperty(FormClass, 'Top'         , typeinfo(integer)         , true, GetScriptFormProp, SetScriptFormProp);
  RegisterCustomProperty(FormClass, 'WindowState' , typeinfo(TWindowState)    , true, GetScriptFormProp, SetScriptFormProp);
  RegisterCustomProperty(FormClass, 'Position'    , typeinfo(TPosition)       , true, GetScriptFormProp, SetScriptFormProp);
  RegisterCustomProperty(FormClass, 'Align'       , typeinfo(TAlign)          , true, GetScriptFormProp, SetScriptFormProp);
  RegisterCustomProperty(FormClass, 'AutoSize'    , typeinfo(boolean)         , true, GetScriptFormProp, SetScriptFormProp);
  RegisterCustomProperty(FormClass, 'Enabled'     , typeinfo(boolean)         , true, GetScriptFormProp, SetScriptFormProp);
  RegisterCustomProperty(FormClass, 'FormStyle'   , typeinfo(TFormStyle)      , true, GetScriptFormProp, SetScriptFormProp);
  RegisterCustomProperty(FormClass, 'KeyPreview'  , typeinfo(boolean)         , true, GetScriptFormProp, SetScriptFormProp);
  RegisterCustomProperty(FormClass, 'Visible'     , typeinfo(boolean)         , true, GetScriptFormProp, SetScriptFormProp);
//  RegisterCustomProperty(FormClass, 'BorderIcons' , typeinfo(TBorderIcons)    , true, GetScriptFormProp, SetScriptFormProp);
  RegisterCustomProperty(FormClass, 'BorderStyle' , typeinfo(TFormBorderStyle), true, GetScriptFormProp, SetScriptFormProp);
  RegisterCustomProperty(FormClass, 'ActiveControl', TWinControl.ClassInfo, true, GetScriptFormProp, SetScriptFormProp);
end;
...
begin
  ...
  DoNotification(ntDesignerChanged);
end;
...
begin
        ...
        case FInspector.InstanceCount of
          ...
          1:
            begin
              ...
              if Assigned(FComponentCombo) and (FInspector.Instance <> FComponentCombo.Instance) then
              begin
                FComponentCombo.Instance := FInspector.Instance;
              end;
            end;
          2:
            begin
  ...
  {notify the component combo}
  if Assigned(FComponentCombo) and Assigned(FInspector) and Assigned(Designer) then
  begin
    FComponentCombo.SynchroLock;
    try
      FComponentCombo.Root := Designer.ParentForm;
      case FInspector.InstanceCount of
        0: FComponentCombo.Instance := Designer.ParentForm;
        1: FComponentCombo.Instance := Inspector.Instance;
      else
        FComponentCombo.Instance := nil;
      end;
    finally
      FComponentCombo.SynchroUnlock;
    end;
  end;
end;
...
procedure TIDEEngine.SetComponentsFilter(const Value: TStrings);
begin
  FComponentsFilter.Assign(Value);
end;
...
begin
  ...
  begin
    ...
    if AComponent = FMemo then
      FMemo := nil;
    if AComponent is TIDEScriptForm then
    begin
      if FFiles <> nil then
      begin
        AFile := FFiles.FindByForm(TIDEScriptForm(AComponent));
        if AFile <> nil then
          AFile.SetForm(nil);
      end;
    end;
...
procedure TIDEEngine.NotifyDesignerChange(AOperationName: string);
begin
  DesignerChange;
  PushUndoStack(AOperationName);
end;

procedure TIDEEngine.NotifyInspectorChange(AOperationName: string);
begin
  InspectorChange;
  PushUndoStack(AOperationName);
end;
...
begin
  if (Memo <> nil) and (FUpdatingMemo = 0) then
  begin
    if ActiveFile <> nil then
    begin
      ActiveFile.FModified := true;
...
function TIDEEngine.NextRedoDesignerAction: string;
begin
  if (UndoStack <> nil) and (ActiveFile <> nil) and (ActiveFile.ViewMode = vmForm) then
    result := UndoStack.NextRedoAction
  else
    result := '';
end;

function TIDEEngine.NextUndoDesignerAction: string;
begin
  if (UndoStack <> nil) and (ActiveFile <> nil) and (ActiveFile.ViewMode = vmForm) then
    result := UndoStack.NextUndoAction
  else
    result := '';
end;
...
function TIDEEngine.CanRedoDesigner: boolean;
begin
  result := NextRedoDesignerAction <> '';
end;

function TIDEEngine.CanUndoDesigner: boolean;
begin
  result := NextUndoDesignerAction <> '';
end;
...
//    if not (csDestroying in ActiveFile.ComponentState) then
  ...
  begin
    ...
    begin
      if (FDesignControl <> nil) then
      begin
        FDesignControl.EditForm := FActiveFile.Form as TForm;
        DesignerSelectionChange;
      end;
    end else
    begin
...
procedure TIDEEngine.FormNameChanged(AForm: TIDEScriptForm; AOldName: string);
var
  AFile: TIDEProjectFile;
  S: string;
begin
  ...
  begin
    ...
    if (MainUnit <> nil) and (MainUnit <> AFile) then
    begin
...
begin
  ...
  begin
    ...
    if not DoSaveFile(iftScript, AFile.GetFullFileName, AFileContent, AFile) then
    ...
    begin
      ...
      if not DoSaveFile(iftForm, AFile.FullFormFileName, AFileContent, AFile) then
...
begin
  ...
  {Load file content. Provide flexibility for loading file from any place (blobs, etc.)
   using OnLoadFile event}
  BeginUpdateSourceCode;
  try
    AFileContent := '';
    if DoLoadFile(iftScript, AFile.GetFullFileName, AFileContent, AFile) then
...
//    AFile.Form.DesignScript := AFile.Script;
    ...
    try
      AFileContent := '';
      if DoLoadFile(iftForm, AFile.FullFormFileName, AFileContent, AFile) then
...
begin
  ...
  begin
    ...
    if FDesignControl.EditForm <> nil then
      FDesignControl.EditForm.UpdateControlState;
...
function TIDEEngine.FindDesignerInForm(AForm: TComponent): TIDEFormDesigner;
var
  c: integer;
begin
  result := nil;
...
procedure TIDEEngine.RedoDesigner;
begin
  if (UndoStack <> nil) and (ActiveFile <> nil) and (ActiveFile.ViewMode = vmForm) then
  begin
    UndoStack.Redo;
...
begin
  ...
  ADlg.Filter := Format('TMS Scripter %s unit (*%s)|*%s', [AFile.LanguageName, AFile.DefaultFileExt, AFile.DefaultFileExt]);
...
begin
  ...
  ADlg.DefaultExt := ProjectExt;
  ADlg.Filter := Format('TMS Scripter project (*%s)|*%s', [ProjectExt, ProjectExt]);
  ADlg.Filter := ADlg.Filter + '|Any file (*.*)|*.*';
...
begin
  ...
  ADlg.Filter :=
    Format('TMS Scripter unit (*%s;*%s)|*%s;*%s', [FileExtPascalUnit, FileExtBasicUnit, FileExtPascalUnit, FileExtBasicUnit]) + '|' +
    Format('TMS Scripter Pascal unit (*%s)|*%s', [FileExtPascalUnit, FileExtPascalUnit]) + '|' +
    Format('TMS Scripter Basic unit (*%s)|*%s', [FileExtBasicUnit, FileExtBasicUnit]);
  ADlg.Filter := ADlg.Filter + '|Any file (*.*)|*.*';
end;
...
procedure TIDEEngine.GetParameterHint(var AToken, AHint: string);
var
  tp: integer;
begin
  if (ActiveScript = nil) then
  ...
  with TScripterCodeInsight.Create(ActiveScript) do
  try
    Context := FCodeInsightContext;
    Memo.TextFromPos(Memo.CurX, Memo.CurY, tp);
    AHint := GetParameterHint(AToken, tp + 1);
  finally
    Free;
  end;
end;

procedure TIDEEngine.GetParameterHintInfo(var AToken: string;
  var AParIndex: integer; var Found: boolean);
var
  tp: integer;
begin
  {Retrieve parameter hint token (qualified name of the function to get parameter hint)}
  if (Memo = nil) or (Scripter = nil) or (Scripter.Running) or (ActiveScript = nil) then
    Exit;

  {actually there is no need for now to update script source code and check for compiled.
   But we will keep it here just in case in future we will need it.
   This will now affect performance because when we will actually get the parameter hint,
   the script must be updated anyway}
  if ActiveFile.FMustUpdateScript then
    UpdateScriptSource(true);
  CheckCompiled;
  ...
  try
    Context := FCodeInsightContext;
    Found := GetParameterHintInfo(Memo.Lines.Text, tp + 1, AToken, AParIndex);
  finally
    Free;
  end;
end;
...
begin
  ...
  try
    ...
    try
      ...
      if not DoSaveFile(iftProject, GetFullProjectFileName, AFileContent, nil) then
...
procedure TIDEEngine.PushUndoStack(AActionName: string);
begin
  if UndoStack <> nil then
    UndoStack.PushState(AActionName);
end;
...
begin
  result := AddPathDelim(BasePath) + FProjectName + ProjectExt;
end;
...
procedure TIDEEngine.GetAutoCompletionListIndex(AToken: string; AList: TStringList;
...
begin
  ...
  try
    Context := FCodeInsightContext;
    sl := CreateCodeCompletionList(Memo.Lines.Text, tp + 1, DefaultIndex);
    try
      AdjustCompletionListToMemo(sl);
      AList.AddStrings(sl);
    finally
      sl.Free;
    end;
...
begin
  ...
  DoComponentPlaced(AComponent);
end;

function TIDEEngine.ComponentVisible(RegisterComp: TIDERegisteredComp): boolean;
begin
  result := false;
  if (RegisterComp <> nil) and (RegisterComp.CompClass <> nil) then
    if (ComponentsFilter.Count = 0) or (ComponentsFilter.IndexOf(RegisterComp.CompClass.ClassName) >= 0) then
      result := true;
end;
...
procedure TIDEEngine.AdjustCompletionListToMemo(AList: TStrings);
var
  e: integer;
  sl: TStrings;
  s: string;
...
procedure TIDEEngine.UndoDesigner;
begin
  if (UndoStack <> nil) and (ActiveFile <> nil) and (ActiveFile.ViewMode = vmForm) then
  begin
    UndoStack.Undo;
...
function TIDEEngine.UndoStack: TDesignerUndoStack;
begin
  if ActiveFile <> nil then
    result := ActiveFile.FUndoStack
  else
    result := nil;
end;
...
function TIDEEngine.DoLoadFile(IDEFileType: TIDEFileType; AFileName: string;
  var AContent: string; AFile: TIDEProjectFile): boolean;
begin
  result := false;
  if Assigned(FOnLoadFile) then
    FOnLoadFile(Self, IDEFileType, AFileName, AContent, AFile, result);
end;

function TIDEEngine.DoSaveFile(IDEFileType: TIDEFileType; AFileName,
  AContent: string; AFile: TIDEProjectFile): boolean;
begin
  result := false;
  if Assigned(FOnSaveFile) then
    FOnSaveFile(Self, IDEFileType, AFileName, AContent, AFile, result);
end;
...
function TIDEEngine.CreateScriptForm: TIDEScriptForm;
begin
  if Scripter <> nil then
    result := Scripter.ScriptFormClass.Create(nil)
  else
    result := TIDEScriptForm.Create(nil)
end;
...
procedure TIDEEngine.DoComponentPlaced(AComponent: TComponent);
begin
  if Assigned(FOnComponentPlaced) then
    FOnComponentPlaced(Self, AComponent);
end;

procedure TIDEEngine.DoDesignerSelectionChanged;
begin
...
procedure TIDEEngine.DoInsertAutoCompletionEntry(var AEntry: string);
var
  P: integer;
begin
  {Remove the "var ", "property " and also the parameters}
  P := Pos(' ', AEntry);
  if P > 0 then
    Delete(AEntry, 1, P);
  P := Pos('(', AEntry);
  if P > 0 then
    Delete(AEntry, P, MaxInt);
  P := Pos('[', AEntry);
  if P > 0 then
    Delete(AEntry, P, MaxInt);

  FCodeInsightContext.LastEntries.Add(FCodeInsightContext.LastUsedToken, AEntry);
  FCodeInsightContext.LastUsedToken := '';
end;
...
begin
  if (ActiveFile <> nil) and (ActiveFile.Form <> nil) and
    Supports(ActiveFile.Form, IScriptBasedForm, FormIntf) then
  begin
    c := 0;
    TheSaveEvents := FormIntf.GetSaveEvents;
    while c < TheSaveEvents.Count do
    begin
      p := Pos('.', TheSaveEvents.Names[c]);
      if (p > 1) and
        not SameText(Copy(TheSaveEvents.Names[c], 1, p - 1), 'Self')
        and (ActiveFile.Form.FindComponent(Copy(TheSaveEvents.Names[c], 1, p - 1)) = nil) then
        TheSaveEvents.Delete(c)
      else
        inc(c);
    end;
  end;
...
procedure TIDEMemo.DoInsertAutoCompletionEntry(var AEntry: string);
begin
  inherited DoInsertAutoCompletionEntry(AEntry);
  if FEngine <> nil then
    FEngine.DoInsertAutoCompletionEntry(AEntry);
end;

function TIDEMemo.IsCompletionListActivationChar(Ch: Char): boolean;
begin
  result := Ch = '.';
end;

function TIDEMemo.IsCompletionListEndChar(Ch: Char): boolean;
begin
  {$IFDEF DELPHI2009_LVL}
  result := CharInSet(Ch, [' ', '(', ')', '<', '>', '+', '-', '/', '*', '=', ':', '[', ']', '{', '}', ',', '.', ';']);
  {$ELSE}
...
function TIDEMemo.IsParameterHintActivationChar(Ch: Char): boolean;
begin
  result := (Ch = '(') or (Ch = '[');
end;

procedure TIDEMemo.DoGetAutoCompletionListIndex(AToken: string; AList: TStringList;
  var DefaultIndex: integer);
begin
  if FEngine <> nil then
    FEngine.GetAutoCompletionListIndex(AToken, AList, DefaultIndex);
  inherited DoGetAutoCompletionListIndex(AToken, AList, DefaultIndex);
end;

procedure TIDEMemo.DoGetParameterHint(AToken: string;
  var AParameterHint: string; var Handled: boolean);
begin
  if FEngine <> nil then
    FEngine.GetParameterHint(AToken, AParameterHint);
  Handled := true;
  inherited DoGetParameterHint(AToken, AParameterHint, Handled);
end;

procedure TIDEMemo.DoGetParameterHintInfo(var AToken: string;
  var AParIndex: integer; var Found, Handled: boolean);
begin
  if FEngine <> nil then
    FEngine.GetParameterHintInfo(AToken, AParIndex, Found);
  Handled := true;
  inherited DoGetParameterHintInfo(AToken, AParIndex, Found, Handled);
end;
...
begin
  ...
  AutoHintParameters := hpManual;
  AutoHintParameterDelay := 0;
end;
...
function TIDEProjectFiles.FindByForm(AForm: TIDEScriptForm): TIDEProjectFile;
var
  c: integer;
begin
  result := nil;
...
begin
  ...
  FUndoStack := TDesignerUndoStack.Create(Self);
  if Engine <> nil then
    FUndoStack.UndoLevel := Engine.UndoLevel;
end;
...
begin
  ...
  result := APath + UnitName + DefaultFileExt;
end;
...
procedure TIDEProjectFile.SaveFormToStream(AStream: TStream; ADFMFormat: TDFMFormat = dfmText);
var
  ADesigner: TIDEFormDesigner;
  DestroyDesigner: boolean;
begin
  ...
  begin
    ...
    try
      ADesigner.SaveToStream(AStream, ADFMFormat);
    finally
      if DestroyDesigner then
        ADesigner.Free;
    end;
...
procedure TIDEProjectFile.LoadFormFromStream(AStream: TStream;
  ADFMFormat: TDFMFormat = dfmText);
var
  ADesigner: TIDEFormDesigner;
  DestroyDesigner: boolean;
begin
  ...
  begin
    ...
    try
      ADesigner.LoadFromStream(AStream, ADFMFormat);
    finally
      if DestroyDesigner then
        ADesigner.Free;
    end;
    ...
    // Notify changes
    if Engine.ActiveFile = Self then
    begin
      if Engine.DesignControl <> nil then
        Engine.DesignControl.UpdateFrame;
      Engine.DesignerChange;
      Engine.InspectorChange;
    end;
  end;
end;

procedure TIDEProjectFile.SetForm(const Value: TIDEScriptForm);
begin
  FForm := Value;
...
procedure TIDEProjectFile.ClearUndoStack;
begin
...
begin
  ...
  inherited SingleDebugHook(AMachine);
end;
...
procedure TIDEScripter.DoCompileError(var msg: string; row, col: integer;
...
begin
  ...
  begin
    ...
    try
      FEngine.RegisteredComps.GetPageNames(ACatList, FEngine);
      ...
      begin
        if SameText(FEngine.RegisteredComps[c].Page, ACaption) and
          (not Filtered or (Filter = '') or (Pos(Uppercase(Filter),
            Uppercase(FEngine.RegisteredComps[c].CompClass.ClassName)) > 0)) then
        begin
          if (FEngine.ComponentVisible(FEngine.RegisteredComps[c])) then
          begin
...
procedure TIDEPaletteButtons.Populate;
  ...
  begin
    FEngine.RegisteredComps.GetPageNames(FCatList, FEngine);
...
procedure TDesignerUndoStack.ClearUndoStack;
begin
  Clear;
  FCurrent := -1;
  PushState('');
...
constructor TDesignerUndoStack.Create(AFile: TIDEProjectFile);
begin
  inherited Create(TDesignerUndoItem);
  FUndoLevel := 10;
  FFile := AFile;
  FCurrent := -1;
end;

function TDesignerUndoStack.NextRedoAction: string;
begin
  result := '';
  if (FCurrent >= -1) and (FCurrent < Count - 1) then
    result := TDesignerUndoItem(Items[FCurrent + 1]).ActionName;
end;

function TDesignerUndoStack.NextUndoAction: string;
begin
  result := '';
  if (FCurrent > 0) and (FCurrent < Count) then
    result := TDesignerUndoItem(Items[FCurrent]).ActionName;
end;

procedure TDesignerUndoStack.PushState(const AActionName: string);
begin
  ...
  with TDesignerUndoItem(Add) do
  begin
    ActionName := AActionName;
    if (FFile <> nil) then
      FFile.SaveFormToStream(FState, dfmBinary);
    inc(FCurrent);
  end;
  if Count > UndoLevel + 1 then
  begin
    Items[0].Free;
    dec(FCurrent);
  end;
...
procedure TDesignerUndoStack.Redo;
begin
  if (FCurrent >= -1) and (FCurrent < Count - 1) then
  begin
    With TDesignerUndoItem(Items[FCurrent + 1]) do
    begin
    FState.Position := 0;
    if (FFile <> nil) then
      FFile.LoadFormFromStream(FState, dfmBinary);
    end;
    inc(FCurrent);
  end;
end;

procedure TDesignerUndoStack.Undo;
begin
  if (FCurrent > 0) and (FCurrent <= Count) then
  begin
    With TDesignerUndoItem(Items[FCurrent - 1]) do
...
constructor TDesignerUndoItem.Create(Collection: TCollection);
begin
  inherited;
  FState := TMemoryStream.Create;
end;

destructor TDesignerUndoItem.Destroy;
begin
  FState.Free;
  inherited;
end;
...
constructor TCompEvents.Create(AOwner: TComponent);
begin
  inherited;
  FEvents := TStringList.Create;
end;

destructor TCompEvents.Destroy;
begin
  FEvents.Free;
  inherited;
end;

procedure TCompEvents.SetEvents(const Value: TStrings);
begin

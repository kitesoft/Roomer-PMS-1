...
begin
  ...
  try
    ...
    if (FFormDesigner is TIDEFormDesigner) then
      FEngine := TIDEFormDesigner(FFormDesigner).CurrentEngine;
    Show;
  except
    Free;
    raise;
  ...
  begin
    ...
    try
      ...
      for i:=0 to AFields.Count-1 do
        if AFields[i].Owner = FFieldsOwner then
...
begin
  ...
  if (EditedDataset <> nil) and (SameText(EditedDataset.ClassName, 'TClientDataset')) then
  begin
...
begin
  lsAggregates.Visible := Assigned(FEditedDataset) and SameText(FEditedDataset.ClassName, 'TClientDataset') and (lsAggregates.Items.Count > 0);
...
begin
  result := Clipboard.HasFormat(CF_TEXT) and (Pos('object ', Clipboard.AsText) = 1);
end;
...
var
  i: integer;
begin
  if (FEngine <> nil) and (FEngine.Inspector <> nil) then
  begin
    FEngine.Inspector.Instance := nil;
    for i := 0 to CurrentListBox.Items.Count - 1 do
      if CurrentListBox.Selected[i] then
        FEngine.Inspector.AddInstance(TComponent(CurrentListBox.Items.Objects[i]));
  end else
...
procedure TfmDatasetFieldsEditor.FormCreate(Sender: TObject);
begin
  FSelectionChanged := false;
end;

procedure TfmDatasetFieldsEditor.InitiateAction;
begin
  inherited;

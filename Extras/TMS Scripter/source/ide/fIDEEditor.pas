...
uses
  ...
  ExtCtrls, ComCtrls, IDEMain, Menus, atScripter, atScript, atPascal, ScrMemo, fObjectInspector,
  fWatches,
  {$IFDEF DELPHI9_LVL}
  fToolPalette,
  {$ENDIF}
...
type
  TIDEEditorForm = class(TForm)
    ...
    procedure acViewLibraryBrowserExecute(Sender: TObject);
  ...
  private
    ...
    {$IFDEF LIBBROWSER}
    FLibraryBrowser: TfmLibraryBrowser;
    {$ENDIF}
    {$IFDEF DELPHI9_LVL}
    FForceOldPalette: boolean;
    {$ENDIF}
    ...
    procedure IDEEngineRunningChanged(Sender: TObject;
      ARunning: Boolean);
    {$IFDEF DELPHI9_LVL}
    procedure SetForceOldPalette(const Value: boolean);
    {$ENDIF}
  ...
  public
    ...
    property Engine: TIDEEngine read FEngine;

    {$IFDEF DELPHI9_LVL}
    property ForceOldPalette: boolean read FForceOldPalette write SetForceOldPalette;
    {$ENDIF}
    ...
    {$IFDEF LIBBROWSER}
    property LibraryBrowser: TfmLibraryBrowser read FLibraryBrowser;
    {$ENDIF}
  end;
...
begin
  ...
  {$IFDEF LIBBROWSER}
  FLibraryBrowser.Engine := FEngine;
  {$ENDIF}
...
destructor TIDEEditorForm.Destroy;
begin
  inherited;
end;
...
{$IFDEF DELPHI9_LVL}
procedure TIDEEditorForm.SetForceOldPalette(const Value: boolean);
begin
  FForceOldPalette := Value;
  if FForceOldPalette then
  begin
    FPaletteForm.OnClose := nil;
    FPaletteForm.Visible := false;
    IDEPaletteToolbar2.Visible := true;
...
begin
  ...
  {$ELSE}
  acViewLibraryBrowser.Visible := false;
  {$ENDIF}
  ...
  IDEMemo1.AutoCompletion.Height := 160;
  IDEMemo1.AutoCompletion.ShowImages := true;
  IDEMemo1.AutoCompletion.MaxWidth := 400;
end;
...
procedure TIDEEditorForm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TIDEEditorForm.Notification(AComponent: TComponent;
...
procedure TIDEEditorForm.acViewLibraryBrowserExecute(Sender: TObject);
begin
  {$IFDEF LIBBROWSER}
  LibraryBrowser.Show;
  LibraryBrowser.RefreshTree;
  {$ENDIF}
end;

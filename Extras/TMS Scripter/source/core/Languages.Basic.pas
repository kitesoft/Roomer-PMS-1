...
begin
   ...
   begin
      ...
      Parser.Comments.Add('REM ',#10).PriorDelims := ' :'#13#10;
      Parser.Comments.Add('#', #10).PriorDelims := ' :'#13#10;
      {$ENDIF}
...
begin
  ...
  while not StackDelimitterFound(stPendingOperators) and
     (BasicOperatorLevel[TInstruction(StackViewAsInt(stPendingOperators) shr 20)] <= BasicOperatorLevel[op]) do
  begin
    opInStack := TInstruction(StackViewAsInt(stPendingOperators) shr 20);
    opNumber := StackPopAsInt(stPendingOperators) and $FFFFF;
    with AppendInstruction(opInStack)^ do
...
function TatBasicRefactor.BuildMethodHint(meth: TatMethod): string;
var
  c: integer;
  ArgDef: TatDataDef;
  PName: string;
...
function TatBasicRefactor.BuildRoutineHint(routine: TatRoutineInfo): string;
var
  c: integer;
...
function TatBasicRefactor.IsStatementEndChar(ch: Char): boolean;
begin
  result := (ch = chr(13)) or (ch = chr(10)) or (ch = ':');
end;

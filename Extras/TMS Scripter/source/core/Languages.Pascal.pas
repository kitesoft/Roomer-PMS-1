...
type
  ...
  protected
    function IsStatementEndChar(ch: Char): boolean; override;
    function BuildMethodHint(meth: TatMethod): string; override;
    function BuildRoutineHint(routine: TatRoutineInfo): string; override;
  public
    function BuildPropertyDeclaration(prop: TatProperty): string; override;
    function BuildMethodHeader(meth: TatMethod): string; override;
...
type
  TPascalDirectiveType = (pdtNone, pdtForm, pdtClass);
const
...
begin
  ...
  begin
     ...
     Case ADirectiveType of
       ...
       pdtClass:
         begin
           AClassName := Trim(DefineID);
           if (ClassName = '') then
             CompileError('Invalid CLASS directive syntax. Correct syntax is "$CLASS CLASSNAME"',
               Parser.ScanningInputPos);
           CompileScriptClass(AClassName);
         end;
     end;
  end;
end;
...
begin
  ...
  for c:=0 to Node.Nodes.Count-1 do
  begin
    AUnitName := StackPopAsString(stIdentifierList);
    try
...
begin
  ...
  while not StackDelimitterFound(stPendingOperators) and
     (PascalOperatorLevel[TInstruction(StackViewAsInt(stPendingOperators) shr 20)] <= PascalOperatorLevel[op]) do
  begin
    opInStack := TInstruction(StackViewAsInt(stPendingOperators) shr 20);
    opNumber := StackPopAsInt(stPendingOperators) and $FFFFF;
    with AppendInstruction(opInStack)^ do
...
begin
  with AppendInstruction(inPushInteger)^ do
  begin
    vInt64   := HexToInt(Node.InputToken); {HexToInt é mais eficiente que StrToInt}
    vDebugInfo := Parser.ScanningInputPos;
  end;
end;
...
begin
   ...
   begin
      ...
      with AppendInstruction(inPushInteger)^ do
      begin
         vInt64   := StrToInt64(Node.InputToken);
         vDebugInfo := Parser.ScanningInputPos;
      end;
end;
...
begin
   StackPush(stContext, ObjectToVar(FPushVarList));
   FPushVarList := TPushVarList.Create;
   FPushVarListObjects.Add(FPushVarList);
   if Node.Nodes.Count>0 then
   begin
      { salva na pilha de contexto a situação atual do controle de objetos }
      StackPush( stContext, ObjectToVar(FCurrentClass) );
      StackPush( stContext, FIsClassReference);
...
begin
   ...
   begin
      ...
      FByRefArgMask := StackPopAsInt(stContext);
      { restaura o controle de objetos anterior à chamada da função }
      FIsClassReference := StackPopAsBool(stContext);
      FCurrentClass:=TatClass(VarToObject(StackPop(stContext)));
   end;
   FLastPushVarList := FPushVarList;
   FPushVarList := TPushVarList(VarToObject(StackPop(stContext)));
end;
...
begin
   ...
   begin
      ...
      StackPush( stContext, ObjectToVar(FCurrentClass) );
      StackPush( stContext, FIsClassReference);
      StackPush( stContext, ObjectToVar(FLastPushVarList) );
   end;
end;
...
begin
   ...
   begin
      { restaura o controle de objetos anterior à chamada da função }
      FLastPushVarList := TPushVarList(VarToObject(StackPop(stContext)));
      FIsClassReference := StackPopAsBool(stContext);
      FCurrentClass := TatClass(VarToObject(StackPop(stContext)));
   end;
end;
...
begin
  ...
  if not Assigned(FCurrentClass) then
    ...
    if FCurrentClass = UndefinedClass then
      CompileError(
  ...
  StackPush( stTempVariables, ObjectToVar(variable) );
end;
...
var
  ...
  ElementName: string;
  index, argCount, idxCount: integer;
  variable: TatVariableInfo;
  subroutine: TatRoutineInfo;
  method_obj: TatMethod;
  property_obj, def_property_obj: TatProperty;
  newinst: pSimplifiedCode;

   function KnownWithProperty: boolean;
   var
     st: pStackElement;
   begin
      result := false;
      { if element has not arguments, then it might be a property }
      if not hasArgList then
         if not Assigned(FCurrentClass) then
            if not hasUpnode then
            begin
               { search for property in the stack context of "with" clauses
                 search is done from last used object to first }
               st := GetStack(stTempVariables);
               while Assigned(st) do
               begin
                 variable := TatVariableInfo(VarToObject(st^.Element));
                 if variable.DeclaredClass = UndefinedClass then
   ...
   begin
      ...
      if not hasArgList then
         if not Assigned(FCurrentClass) then
            ...
            if FCurrentClass = UndefinedClass then // undefined variable instance
            begin
   ...
   function KnownWithMethod: boolean;
   var
     st: pStackElement;
   begin
      Result := false;
   ...
   var
     atClass: TatClass;
     WithInstruction: pSimplifiedCode;
   begin
     ...
     begin
       ...
       case ElementType of
         ...
         etProperty:
           ...
           begin
             ...
             if idxCount <= property_obj.IndexCount then
               atClass := nil;
           end;
         etMethod:
       ...
       begin
         ...
         // At this point we should have two things in the stack: (Memo.Lines[5])
         // Parameter (5) and Object reference (Memo)
         // We must not call "Lines" with the parameter 5. Instead, we need to call only Memo.Lines
         // and then call "Strings[5]" passing the parameter 5. So we must save the stack for index
         // in order to use it later when calling the default property getter
         // So when "Lines" is called, Object Reference is consumed from stack, and then we have
         // the remaining 5 to call Strings
         // Careful: If we are in a With statement, then we must ensure to insert this instruction
         // before the with statement
         if LastInstruction^.OpCode = inWithObject then
         begin
           WithInstruction := RemoveLastInstruction;
           AppendInstruction(inSaveStackValue)^.vInteger := def_property_obj.IndexCount;
           AppendInstruction(WithInstruction);
         end else
           AppendInstruction(inSaveStackValue)^.vInteger := def_property_obj.IndexCount;
         idxCount := idxCount - def_property_obj.IndexCount;
       end;
     end
     else
       result := False;
  ...
  procedure GenerateVariableCode(AVariable: TatVariableInfo; AElementName: string;
    AIdxCount: integer; AHasSubnode: boolean; AIndex: integer);
  begin
     ...
     begin
        ...
        varIsByRef := (operation=doRead) and not AHasSubNode and (AIdxCount = 0) and
        ...
        begin
           ...
           begin
              if AVariable.Global then OpCode := inPushGlobalVarByRef;
              vInteger   := AVariable.VarIndex;
           ...
           if (AIdxCount > 0){ and not hasDefaultProperty} then
              with AppendInstruction(inPushIdxVar)^ do
              begin
                 if AVariable.Global then OpCode := inPushGlobalIdxVar;
                 vInteger   := AVariable.VarIndex;
                 vByte      := AIdxCount;
                 vString    := AElementName;
                 vDebugInfo := Parser.ScanningInputPos;
              ...
              with AppendInstruction(inPushVar)^ do
              begin
                 if AVariable.Global then OpCode := inPushGlobalVar;
                 vInteger   := AVariable.VarIndex;
                 vString    := AElementName;
                 vDebugInfo := Parser.ScanningInputPos;
              end;
        ...
        if Scripter.DeferObjectResolution or not Assigned(AVariable.DeclaredClass) then
          { assume que a variável não tem tipo definido, de modo que seu uso
            como objeto possa ser postergado para tempo de execução }
          FCurrentClass := UndefinedClass  { undefined variable instance }
        else
          FCurrentClass := AVariable.DeclaredClass;
     end
     else
        if (operation=doExec) and (not AHasSubnode) then
     ...
     begin
        ...
        begin
           ...
           begin
              if AVariable.Global then
                OpCode := inStoreGlobalIdxVar;

              if AVariable.Modifier = moVar then
                case OpCode of
                  inStoreIdxVar: OpCode := inStoreIdxVarRef;
                end;
  ...
  begin
     ...
     begin
           ...
           if Assigned(AProperty.Setter) then
              if AProperty.IndexCount = AIdxCount then
                 ...
                 begin
                    ...
                    with newinst^ do
                    begin
                       vInteger   := AProperty.atClass.Index;
                       vByte      := 1;
        ...
        if (operation = doRead) or AHasSubNode or (FCurrentClass = UndefinedClass) then
           if Assigned(AProperty) then
           begin
              if Assigned(AProperty.Getter) then
                 ...
                 CompileError(Format('Property ''%s'' cannot be read',[AElementName]),Parser.ScanningInputPos);
              ...
              begin
                ...
                FCurrentClass.SetImplicitElement(AProperty);
              end;
           end
           else
...
begin
   ...
   ElementName := StackPopAsString(stIdentifierList);
   ...
   case ElementType of
      ...
      etVariable: { V A R I Á V E L }
        if hasDefaultProperty then
          GenerateVariableCode(variable, ElementName, idxCount, True, index)
        else
          GenerateVariableCode(variable, ElementName, idxCount, hasSubnode, index);
      ...
      begin
         ...
         if Assigned(method_obj) then
            if (method_obj.ResultDataType=tkNone) and (hasSubNode or (operation=doRead)) then
               ...
               if hasSubnode and not (method_obj.ResultDataType in [tkClass, tkVariant]) then
                        ...
                        begin
                           ...
                           begin
                              ...
                              begin
                                 ...
                                 with newinst^ do
                                 begin
                                    vInteger   := method_obj.atClass.Index;
                                    vByte      := argcount;
                                    vSmallInt  := 1; // indica que é uma chamada Call e não um getter ou setter
                           ...
                           begin
                              { register the implicit class referencing }
                              FCurrentClass.SetImplicitElement(method_obj);
                           end;
                        end
                        else
                           begin
      ...
      etProperty: { P R O P E R T Y }
        if hasDefaultProperty then
          GeneratePropertyCode(property_obj, ElementName, idxCount, True, index)
        else
  ...
  if hasDefaultProperty then
  begin
    AppendInstruction(inLoadStackValue)^.vInteger := def_property_obj.IndexCount;
    GeneratePropertyCode(def_property_obj, def_property_obj.Name, def_property_obj.IndexCount, hasSubnode, -1);
  end;
end;
...
begin
   ...
   {$IFDEF NEWSTACK}
   StackPush(stContext, TValue.From<pSimplifiedCode>(LastInstruction));
   StackPush(stContext, TValue.From<pSimplifiedCode>(LastInstruction));
   {$ELSE}
   StackPush(stContext, PointerToVar(LastInstruction));
   StackPush(stContext, PointerToVar(LastInstruction));
   {$ENDIF}
end;
...
begin
  // Register the final address of try block in instruction TryFinally/TryExcept
  {$IFDEF NEXTGEN}
  StackPop(stContext).AsType<pSimplifiedCode>^.vInteger := CodeLine + 1;
  {$ELSE}
  pSimplifiedCode(VarToPointer(StackPop(stContext)))^.vInteger := CodeLine + 1;
  {$ENDIF}
end;
...
begin
  {$IFDEF NEXTGEN}
  StackPop(stContext).AsType<pSimplifiedCode>^.vInteger2 := CodeLine + 1;
  {$ELSE}
  pSimplifiedCode(VarToPointer(StackPop(stContext)))^.vInteger2 := CodeLine + 1;
  {$ENDIF}
end;
...
begin
   ...
   FCaseVarIndex := StackPopAsInt(stContext);
   { controle do nível de aninhamento do CASE }
   FCurrentCase := StackPopAsInt(stContext);
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if Pos('{$CLASS', UpcaseSource) > InsertPosition then
            InsertPosition := Pos('{$CLASS', UpcaseSource);

          if InsertPosition > 0 then
          begin
            {if there is a form/class directive, then go to the end of line}
            repeat
              Inc(InsertPosition);
...
function TatPascalRefactor.BuildMethodHeader(meth: TatMethod): string;
var
  Args: string;
begin
  Args := BuildMethodHint(meth);
  if Args > '' then
    Args := Format('(%s)', [Args]);

  {Finishing building parameters}
  if meth.ResultDataType = tkNone then
  begin
    Result := Format('procedure %s%s', [meth.Name, Args]);
  end else
  begin
//    if meth.ResultDataType
//      AType := ': ' + AInfo.ResultTypeDecl
//    else
//      AType := '';
    result := Format('function %s%s', [meth.Name, Args]);
  end;
end;

function TatPascalRefactor.BuildMethodHint(meth: TatMethod): string;
var
  c: integer;
  ArgDef: TatDataDef;
  PName: string;
...
begin
  result := '';
  for c := 0 to meth.ArgCount - 1 do
  begin
    PName := '';
    PType := '';
    PDefault := '';
    PModifier := '';
    if c < meth.ArgDefs.Count then
    begin
      ArgDef := meth.ArgDefs[c];
      PName := ArgDef.Name;
      PType := ArgDef.DataTypeName;
      PDefault := ArgDef.DefaultValueText;
    end;
    if PName = '' then
      PName := Format('Param%d', [c]);

    {Build the string}
    if c > 0 then
      result := result + '; ';
    if meth.IsByRefArg(c) then
      PModifier := 'var ';
    if PDefault <> '' then
      PDefault := ' = ' + PDefault;
    if PType <> '' then
    ...
    result := result + PModifier + PName + PType + PDefault;
  end;
end;

function TatPascalRefactor.BuildPropertyDeclaration(prop: TatProperty): string;
var
  Args: string;
begin
  Args := BuildPropertyHint(prop);
  if Args > '' then
    Args := Format('[%s]', [Args]);

  Result := Format('property %s%s', [prop.Name, Args]);
end;

function TatPascalRefactor.BuildRoutineHint(routine: TatRoutineInfo): string;
var
  c: integer;
  VarInfo: TatVariableInfo;
  PName: string;
  PType: string;
  PDefault: string;
  PModifier: string;
begin
  result := '';
  for c := 0 to routine.ArgCount - 1 do
  begin
    PName := '';
    PType := '';
    PDefault := '';
    PModifier := '';
    if c < routine.Variables.Count then
    begin
      VarInfo := routine.Variables[c];
      PName := VarInfo.VarName;
      PType := VarInfo.TypeDecl;
      PDefault := '';
    end;
    if PName = '' then
      PName := Format('Param%d', [c]);
...
function TatPascalRefactor.IsStatementEndChar(ch: Char): boolean;
begin
  result := (ch = ';');
end;

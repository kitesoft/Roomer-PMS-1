...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomizeActionBarsClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
    TCustomizeActionBars(CurrentObject).ExecuteTarget(TObject(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := TCustomizeActionBars(CurrentObject).HandlesTarget(TObject(VarToObject(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TCustomizeActionBars,__TCustomizeActionBarsCreate,true,0,'AOwner: TComponent');
    DefineMethod('ExecuteTarget',1,tkNone,nil,__TCustomizeActionBarsExecuteTarget,false,0,'Target: TObject');
    DefineMethod('HandlesTarget',1,tkVariant,nil,__TCustomizeActionBarsHandlesTarget,false,0,'Target: TObject');
  end;

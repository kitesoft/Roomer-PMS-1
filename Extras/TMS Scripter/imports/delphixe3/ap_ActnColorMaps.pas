...
type
  TatActnColorMapsLibrary = class(TatScripterLibrary)
    ...
    procedure __TThemedColorMapUpdateColors(AMachine: TatVirtualMachine);
  ...
  TThemedColorMapClass = class of TThemedColorMap;
...
procedure TatActnColorMapsLibrary.__TThemedColorMapUpdateColors(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    TThemedColorMap(CurrentObject).UpdateColors;
  end;
end;
...
begin
  ...
  With Scripter.DefineClass(TThemedColorMap) do
  begin
    DefineMethod('UpdateColors',0,tkNone,nil,__TThemedColorMapUpdateColors,false,0,'');
  end;

...
begin
  with AMachine do
  begin
    TMenuItem(CurrentObject).SetParentComponent(TComponent(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Menus.SameCaption(VarToStr(GetInputArg(0)),VarToStr(GetInputArg(1)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    ...
    DefineMethod('SetParentComponent',1,tkNone,nil,__TMenuItemSetParentComponent,false,0,'Value: TComponent');
  ...
  begin
    ...
    DefineMethod('SameCaption',2,tkVariant,nil,__SameCaption,false,0,'Text1: string; Text2: string');

...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TActionBarAccessibilityClass(CurrentClass.ClassRef).Create(TActionClient(VarToObject(GetInputArg(0))),TActionClient(VarToObject(GetInputArg(1)))));
...
begin
  ...
  begin
    DefineMethod('Create',2,tkClass,TActionBarAccessibility,__TActionBarAccessibilityCreate,true,0,'Parent: TActionClient; ActionClient: TActionClient');
  end;
  With Scripter.DefineClass(TActionMenuBarAccessibility) do
  begin
    DefineMethod('Get_accState',2,tkInteger,nil,__TActionMenuBarAccessibilityGet_accState,false,0,'Child: OleVariant; pvarState: OleVariant').SetVarArgs([0,1]);
    DefineMethod('Get_accRole',2,tkInteger,nil,__TActionMenuBarAccessibilityGet_accRole,false,0,'Child: OleVariant; pvarRole: OleVariant').SetVarArgs([0,1]);
    DefineMethod('Get_accDescription',2,tkInteger,nil,__TActionMenuBarAccessibilityGet_accDescription,false,0,'Child: OleVariant; pszDescription: WideString').SetVarArgs([0,1]);
  end;

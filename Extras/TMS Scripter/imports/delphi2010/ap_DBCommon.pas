...
type
  TatDBCommonLibrary = class(TatScripterLibrary)
    ...
    procedure __SQLRequiresParams(AMachine: TatVirtualMachine);
    procedure __NextSQLTokenEx(AMachine: TatVirtualMachine);
    procedure __GetTableNameFromSQLEx(AMachine: TatVirtualMachine);
    procedure __MetaDataFromUnicode(AMachine: TatVirtualMachine);
    procedure __MetaDataToUnicode(AMachine: TatVirtualMachine);
    procedure Init; override;
    class function LibraryName: string; override;
  end;

  TFilterExprClass = class of TFilterExpr;
  TFilterExpr2Class = class of TFilterExpr2;
  TExprParserClass = class of TExprParser;
  TExprParser2Class = class of TExprParser2;
  ...
  private
    ...
    FOriginalFieldName: Widestring;
  public
    constructor Create(ARecord: TFieldInfo);
    function ObjToRec: TFieldInfo;
  published
    property DatabaseName: Widestring read FDatabaseName write FDatabaseName;
    property TableName: Widestring read FTableName write FTableName;
    property OriginalFieldName: Widestring read FOriginalFieldName write FOriginalFieldName;
  end;
...
begin
  with AMachine do
  begin
    TFilterExpr(CurrentObject).DataSet:=TDataSet(VarToObject(GetInputArg(0)));
  end;
end;
...
AResult := Integer(DBCommon.NextSQLToken(Param0,Param1,TSQLToken(VarToInteger(GetInputArg(2)))));
    ReturnOutputArg(AResult);
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(DBCommon.GetIndexForOrderBy(GetInputArg(0),TDataSet(VarToObject(GetInputArg(1)))));
    ReturnOutputArg(AResult);
  end;
end;
...
3: AResult := DBCommon.AddParamSQLForDetail(TParams(VarToObject(GetInputArg(0))),GetInputArg(1),GetInputArg(2));
4: AResult := DBCommon.AddParamSQLForDetail(TParams(VarToObject(GetInputArg(0))),GetInputArg(1),GetInputArg(2),GetInputArg(3));
end;
    ReturnOutputArg(AResult);
  end;
end;
...
procedure TatDBCommonLibrary.__SQLRequiresParams(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := DBCommon.SQLRequiresParams(GetInputArg(0));
    ReturnOutputArg(AResult);
  end;
end;

procedure TatDBCommonLibrary.__NextSQLTokenEx(AMachine: TatVirtualMachine);
  var
...
Param1 := GetInputArg(1);
AResult := Integer(DBCommon.NextSQLTokenEx(Param0,Param1,TSQLToken(VarToInteger(GetInputArg(2))),IDENTIFIEROption(VarToInteger(GetInputArg(3)))));
    ReturnOutputArg(AResult);
...
procedure TatDBCommonLibrary.__GetTableNameFromSQLEx(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := DBCommon.GetTableNameFromSQLEx(GetInputArg(0),IDENTIFIEROption(VarToInteger(GetInputArg(1))));
    ReturnOutputArg(AResult);
  end;
end;

procedure TatDBCommonLibrary.__MetaDataFromUnicode(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := DBCommon.MetaDataFromUnicode(VarToStr(GetInputArg(0)));
    ReturnOutputArg(AResult);
  end;
end;

procedure TatDBCommonLibrary.__MetaDataToUnicode(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := string(DBCommon.MetaDataToUnicode(GetInputArg(0)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  With Scripter.DefineClass(TFilterExpr2) do
  begin
  end;
  With Scripter.DefineClass(TExprParser) do
  begin
    DefineMethod('Destroy',0,tkNone,nil,__TExprParserDestroy,false,0,'');
    DefineMethod('SetExprParams',4,tkNone,nil,__TExprParserSetExprParams,false,0,'Text: Widestring; Options: TFilterOptions; ParserOptions: TParserOptions; FieldName: Widestring');
    DefineProp('DataSize',tkInteger,__GetTExprParserDataSize,nil,nil,false,0);
  end;
  With Scripter.DefineClass(TExprParser2) do
  begin
  end;
  With Scripter.DefineClass(ClassType) do
  begin
    DefineMethod('GetFieldInfo',2,tkVariant,nil,__GetFieldInfo,false,0,'Origin: Widestring; FieldInfo: TFieldInfo').SetVarArgs([1]);
    DefineMethod('NextSQLToken',3,tkEnumeration,nil,__NextSQLToken,false,0,'p: PAnsiChar; Token: AnsiString; CurSection: TSQLToken').SetVarArgs([0,1]);
    DefineMethod('GetIndexForOrderBy',2,tkClass,TIndexDef,__GetIndexForOrderBy,false,0,'SQL: WideString; DataSet: TDataSet');
    DefineMethod('GetTableNameFromSQL',1,tkVariant,nil,__GetTableNameFromSQL,false,0,'SQL: WideString');
    DefineMethod('GetTableNameFromQuery',1,tkVariant,nil,__GetTableNameFromQuery,false,0,'SQL: Widestring');
    DefineMethod('AddParamSQLForDetail',4,tkVariant,nil,__AddParamSQLForDetail,false,1,'Params: TParams; SQL: WideString; Native: Boolean; QuoteChar: WideString = ''');
    DefineMethod('IsMultiTableQuery',1,tkVariant,nil,__IsMultiTableQuery,false,0,'SQL: WideString');
    DefineMethod('SQLRequiresParams',1,tkVariant,nil,__SQLRequiresParams,false,0,'SQL: WideString');
    DefineMethod('NextSQLTokenEx',4,tkEnumeration,nil,__NextSQLTokenEx,false,0,'p: PWideChar; Token: UnicodeString; CurSection: TSQLToken; IdOption: IDENTIFIEROption').SetVarArgs([0,1]);
    DefineMethod('GetTableNameFromSQLEx',2,tkVariant,nil,__GetTableNameFromSQLEx,false,0,'SQL: WideString; IdOption: IDENTIFIEROption');
    DefineMethod('MetaDataFromUnicode',1,tkVariant,nil,__MetaDataFromUnicode,false,0,'Value: string');
    DefineMethod('MetaDataToUnicode',1,tkVariant,nil,__MetaDataToUnicode,false,0,'Value: RawByteString');
    ...
    AddConstant('idKeepQuotes',idKeepQuotes);
    AddConstant('idMixCase',idMixCase);
    AddConstant('idMakeLowerCase',idMakeLowerCase);
    AddConstant('idMakeUpperCase',idMakeUpperCase);

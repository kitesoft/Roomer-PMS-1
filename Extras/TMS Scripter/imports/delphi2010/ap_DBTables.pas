...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TDBErrorClass(CurrentClass.ClassRef).Create(EDBEngineError(VarToObject(GetInputArg(0))),GetInputArg(1),VarToInteger(GetInputArg(2)),VarToStr(GetInputArg(3))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TBDEDataSet(CurrentObject).Translate(PAnsiChar(AnsiString(VarToStr(GetInputArg(0)))),PAnsiChar(AnsiString(VarToStr(GetInputArg(1)))),GetInputArg(2)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TQuery(CurrentObject).SQLBinary:=PAnsiChar(AnsiString(VarToStr(GetInputArg(0))));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',4,tkClass,TDBError,__TDBErrorCreate,true,0,'Owner: EDBEngineError; ErrorCode: DBIResult; NativeError: Longint; Message: string');
  ...
  begin
    ...
    DefineMethod('Translate',3,tkInteger,nil,__TBDEDataSetTranslate,false,0,'Src: PAnsiChar; Dest: PAnsiChar; ToOem: Boolean');

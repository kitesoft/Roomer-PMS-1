...
type
  TatUxThemeLibrary = class(TatScripterLibrary)
    ...
    procedure __BeginPanningFeedback(AMachine: TatVirtualMachine);
    procedure __UpdatePanningFeedback(AMachine: TatVirtualMachine);
    procedure __EndPanningFeedback(AMachine: TatVirtualMachine);
...
procedure TatUxThemeLibrary.__BeginPanningFeedback(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := UxTheme.BeginPanningFeedback(VarToInteger(GetInputArg(0)));
    ReturnOutputArg(AResult);
  end;
end;

procedure TatUxThemeLibrary.__UpdatePanningFeedback(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := UxTheme.UpdatePanningFeedback(VarToInteger(GetInputArg(0)),VarToInteger(GetInputArg(1)),VarToInteger(GetInputArg(2)),GetInputArg(3));
    ReturnOutputArg(AResult);
  end;
end;

procedure TatUxThemeLibrary.__EndPanningFeedback(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := UxTheme.EndPanningFeedback(VarToInteger(GetInputArg(0)),GetInputArg(1));
    ReturnOutputArg(AResult);
  end;
end;
...
AResult := Integer(UxTheme.OpenThemeDataEx(VarToInteger(GetInputArg(0)),StringToWideChar(VarToStr(GetInputArg(1)),@Param1Buf[0],Length(Param1Buf)),VarToInteger(GetInputArg(2))));
    ReturnOutputArg(AResult);
  end;
end;
...
procedure TatUxThemeLibrary.Init;
  ...
  begin
    ...
    begin
      AddConstant('MAX_THEMECOLOR',MAX_THEMECOLOR);
      AddConstant('MAX_THEMESIZE',MAX_THEMESIZE);
...
begin
  ...
  begin
    ...
    DefineMethod('BeginPanningFeedback',1,tkVariant,nil,__BeginPanningFeedback,false,0,'hwnd: HWND');
    DefineMethod('UpdatePanningFeedback',4,tkVariant,nil,__UpdatePanningFeedback,false,0,'hwnd: HWND; lTotalOverpanOffsetX: Integer; lTotalOverpanOffsetY: Integer; fInInertia: BOOL');
    DefineMethod('EndPanningFeedback',2,tkVariant,nil,__EndPanningFeedback,false,0,'hwnd: HWND; fAnimateBack: BOOL');
    DefineMethod('OpenThemeDataEx',3,tkInteger,nil,__OpenThemeDataEx,false,0,'hwnd: HWND; pszClassList: LPCWSTR; dwFlags: DWORD');
    DefineMethod('DrawThemeTextEx',9,tkInteger,nil,__DrawThemeTextEx,false,0,'hTheme: HTHEME; hdc: HDC; iPartId: Integer; iStateId: Integer; pszText: UnicodeString; cchText: Integer; dwTextFlags: DWORD; pRect: TRect; pOptions: TDTTOpts').SetVarArgs([7,8]);
    DefineMethod('SetWindowThemeNonClientAttributes',3,tkInteger,nil,__SetWindowThemeNonClientAttributes,false,0,'hwnd: HWND; dwMask: DWORD; dwAttributes: DWORD');
    DefineMethod('GetThemeBitmap',6,tkInteger,nil,__GetThemeBitmap,false,0,'hTheme: HTHEME; iPartId: Integer; iStateId: Integer; iPropId: Integer; dwFlags: ULONG; phBitmap: HBITMAP').SetVarArgs([5]);
    ...
    DefineMethod('EndBufferedPaint',2,tkInteger,nil,__EndBufferedPaint,false,0,'hBufferedPaint: HPAINTBUFFER; fUpdateTarget: BOOL');
    DefineMethod('GetBufferedPaintTargetRect',2,tkInteger,nil,__GetBufferedPaintTargetRect,false,0,'hBufferedPaint: HPAINTBUFFER; prc: TRect').SetVarArgs([1]);
    DefineMethod('GetBufferedPaintTargetDC',1,tkInteger,nil,__GetBufferedPaintTargetDC,false,0,'hBufferedPaint: HPAINTBUFFER');
    DefineMethod('GetBufferedPaintDC',1,tkInteger,nil,__GetBufferedPaintDC,false,0,'hBufferedPaint: HPAINTBUFFER');
    DefineMethod('BufferedPaintMakeOpaque',2,tkInteger,nil,__BufferedPaintMakeOpaque,false,0,'hBufferedPaint: HPAINTBUFFER; prc: TRect');
    DefineMethod('BufferedPaintStopAllAnimations',1,tkInteger,nil,__BufferedPaintStopAllAnimations,false,0,'hwnd: HWND');
    DefineMethod('EndBufferedAnimation',2,tkInteger,nil,__EndBufferedAnimation,false,0,'hbpAnimation: HANIMATIONBUFFER; fUpdateTarget: BOOL');
    DefineMethod('BufferedPaintRenderAnimation',2,tkVariant,nil,__BufferedPaintRenderAnimation,false,0,'hwnd: HWND; hdcTarget: HDC');
    DefineMethod('IsCompositionActive',0,tkVariant,nil,__IsCompositionActive,false,0,'');
    DefineMethod('GetThemeTransitionDuration',6,tkInteger,nil,__GetThemeTransitionDuration,false,0,'hTheme: HTHEME; iPartId: Integer; iStateIdFrom: Integer; iStateIdTo: Integer; iPropId: Integer; pdwDuration: DWORD').SetVarArgs([5]);

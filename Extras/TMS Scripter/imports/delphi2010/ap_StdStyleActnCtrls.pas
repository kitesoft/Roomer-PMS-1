...
begin
  with AMachine do
  begin
AResult := string(TStandardStyleActionBars(CurrentObject).GetStyleName);
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(StdStyleActnCtrls.StandardStyle));
  end;
end;
...
begin
  with AMachine do
  begin
    StdStyleActnCtrls.StandardStyle:=TStandardStyleActionBars(VarToObject(GetInputArg(0)));
  end;
end;

...
begin
  with AMachine do
  begin
    TCustomActionPopupMenuEx(CurrentObject).LoadMenu(TActionClients(VarToObject(GetInputArg(0))),TMenuItem(VarToObject(GetInputArg(1))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TPopupActionBarClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  ...
  begin
    DefineMethod('LoadMenu',2,tkNone,nil,__TCustomActionPopupMenuExLoadMenu,false,0,'Clients: TActionClients; AMenu: TMenuItem');
  end;
  With Scripter.DefineClass(TPopupActionBar) do
  begin
    DefineMethod('Create',1,tkClass,TPopupActionBar,__TPopupActionBarCreate,true,0,'AOwner: TComponent');
    DefineMethod('Popup',2,tkNone,nil,__TPopupActionBarPopup,false,0,'X: Integer; Y: Integer');

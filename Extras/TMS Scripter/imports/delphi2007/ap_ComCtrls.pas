...
type
  ...
  TNodeInfoWrapper = class(TatRecordWrapper)
  private
    FImageIndex: Integer;
    ...
    FCount: Integer;
  public
    constructor Create(ARecord: TNodeInfo);
    function ObjToRec: TNodeInfo;
  published
  ...
  TNodeDataInfoWrapper = class(TatRecordWrapper)
  private
    FImageIndex: Integer;
    FSelectedIndex: Integer;
    FStateIndex: Integer;
    ...
    FTextLen: Byte;
  public
    constructor Create(ARecord: TNodeDataInfo);
    function ObjToRec: TNodeDataInfo;
  published
    property ImageIndex: Integer read FImageIndex write FImageIndex;
    property SelectedIndex: Integer read FSelectedIndex write FSelectedIndex;
    property StateIndex: Integer read FStateIndex write FStateIndex;
    property OverlayIndex: Integer read FOverlayIndex write FOverlayIndex;
    property Count: Integer read FCount write FCount;
    property TextLen: Byte read FTextLen write FTextLen;
  end;
...
constructor TNodeInfoWrapper.Create(ARecord: TNodeInfo);
begin
  inherited Create;
  FImageIndex := ARecord.ImageIndex;
  FSelectedIndex := ARecord.SelectedIndex;
  FStateIndex := ARecord.StateIndex;
  FOverlayIndex := ARecord.OverlayIndex;
  FCount := ARecord.Count;
end;

function TNodeInfoWrapper.ObjToRec: TNodeInfo;
begin
  result.ImageIndex := FImageIndex;
  result.SelectedIndex := FSelectedIndex;
  result.StateIndex := FStateIndex;
  result.OverlayIndex := FOverlayIndex;
  result.Count := FCount;
end;

constructor TNodeDataInfoWrapper.Create(ARecord: TNodeDataInfo);
begin
  inherited Create;
  FImageIndex := ARecord.ImageIndex;
  FSelectedIndex := ARecord.SelectedIndex;
  ...
  FOverlayIndex := ARecord.OverlayIndex;
  FCount := ARecord.Count;
  FTextLen := ARecord.TextLen;
end;

function TNodeDataInfoWrapper.ObjToRec: TNodeDataInfo;
begin
  result.ImageIndex := FImageIndex;
  result.SelectedIndex := FSelectedIndex;
  result.StateIndex := FStateIndex;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomTabControlClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCustomTabControl(CurrentObject).Canvas));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTabSheetClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TTabSheet(CurrentObject).PageControl));
  end;
end;
...
begin
  with AMachine do
  begin
    TTabSheet(CurrentObject).PageControl:=TPageControl(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TPageControlClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TPageControl(CurrentObject).FindNextPage(TTabSheet(VarToObject(GetInputArg(0))),GetInputArg(1),GetInputArg(2)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TPageControl(CurrentObject).Pages[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TStatusPanelClass(CurrentClass.ClassRef).Create(TCollection(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TStatusPanel(CurrentObject).Assign(TPersistent(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TStatusPanelsClass(CurrentClass.ClassRef).Create(TCustomStatusBar(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TStatusPanels(CurrentObject).Add);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TStatusPanels(CurrentObject).AddItem(TStatusPanel(VarToObject(GetInputArg(0))),VarToInteger(GetInputArg(1))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TStatusPanels(CurrentObject).Insert(VarToInteger(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TStatusPanels(CurrentObject).Items[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    TStatusPanels(CurrentObject).Items[VarToInteger(GetArrayIndex(0))]:=TStatusPanel(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomStatusBarClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := TCustomStatusBar(CurrentObject).ExecuteAction(TBasicAction(VarToObject(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCustomStatusBar(CurrentObject).Canvas));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCustomStatusBar(CurrentObject).Panels));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomStatusBar(CurrentObject).Panels:=TStatusPanels(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TCustomStatusBar(CurrentObject).SimpleText));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomStatusBar(CurrentObject).SimpleText:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(THeaderSectionClass(CurrentClass.ClassRef).Create(TCollection(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    THeaderSection(CurrentObject).Assign(TPersistent(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(THeaderSectionsClass(CurrentClass.ClassRef).Create(TCustomHeaderControl(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(THeaderSections(CurrentObject).Add);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(THeaderSections(CurrentObject).AddItem(THeaderSection(VarToObject(GetInputArg(0))),VarToInteger(GetInputArg(1))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(THeaderSections(CurrentObject).Insert(VarToInteger(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(THeaderSections(CurrentObject).Items[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    THeaderSections(CurrentObject).Items[VarToInteger(GetArrayIndex(0))]:=THeaderSection(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomHeaderControlClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCustomHeaderControl(CurrentObject).Canvas));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTreeNodeClass(CurrentClass.ClassRef).Create(TTreeNodes(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TTreeNode(CurrentObject).Assign(TPersistent(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTreeNode(CurrentObject).getFirstChild);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTreeNode(CurrentObject).GetLastChild);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTreeNode(CurrentObject).GetNext);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTreeNode(CurrentObject).GetNextChild(TTreeNode(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTreeNode(CurrentObject).getNextSibling);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTreeNode(CurrentObject).GetNextVisible);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTreeNode(CurrentObject).GetPrev);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTreeNode(CurrentObject).GetPrevChild(TTreeNode(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTreeNode(CurrentObject).getPrevSibling);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTreeNode(CurrentObject).GetPrevVisible);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := TTreeNode(CurrentObject).HasAsParent(TTreeNode(VarToObject(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TTreeNode(CurrentObject).IndexOf(TTreeNode(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TTreeNode(CurrentObject).MoveTo(TTreeNode(VarToObject(GetInputArg(0))),TNodeAttachMode(VarToInteger(GetInputArg(1))));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TTreeNode(CurrentObject).Item[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    TTreeNode(CurrentObject).Item[VarToInteger(GetArrayIndex(0))]:=TTreeNode(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TTreeNode(CurrentObject).Owner));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TTreeNode(CurrentObject).Parent));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TTreeNode(CurrentObject).Text));
  end;
end;
...
begin
  with AMachine do
  begin
    TTreeNode(CurrentObject).Text:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TTreeNode(CurrentObject).TreeView));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTreeNodesEnumeratorClass(CurrentClass.ClassRef).Create(TTreeNodes(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTreeNodesEnumerator(CurrentObject).GetCurrent);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TTreeNodesEnumerator(CurrentObject).Current));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTreeNodesClass(CurrentClass.ClassRef).Create(TCustomTreeView(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTreeNodes(CurrentObject).AddChildFirst(TTreeNode(VarToObject(GetInputArg(0))),VarToStr(GetInputArg(1))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTreeNodes(CurrentObject).AddChild(TTreeNode(VarToObject(GetInputArg(0))),VarToStr(GetInputArg(1))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTreeNodes(CurrentObject).AddFirst(TTreeNode(VarToObject(GetInputArg(0))),VarToStr(GetInputArg(1))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTreeNodes(CurrentObject).Add(TTreeNode(VarToObject(GetInputArg(0))),VarToStr(GetInputArg(1))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TTreeNodes(CurrentObject).Assign(TPersistent(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    TTreeNodes(CurrentObject).Delete(TTreeNode(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTreeNodes(CurrentObject).GetFirstNode);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTreeNodes(CurrentObject).GetEnumerator);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTreeNodes(CurrentObject).Insert(TTreeNode(VarToObject(GetInputArg(0))),VarToStr(GetInputArg(1))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TTreeNodes(CurrentObject).Item[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TTreeNodes(CurrentObject).Owner));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomTreeViewClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomTreeView(CurrentObject).GetNodeAt(VarToInteger(GetInputArg(0)),VarToInteger(GetInputArg(1))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomTreeView(CurrentObject).LoadFromFile(VarToStr(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomTreeView(CurrentObject).LoadFromStream(TStream(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomTreeView(CurrentObject).SaveToFile(VarToStr(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomTreeView(CurrentObject).SaveToStream(TStream(VarToObject(GetInputArg(0))));
  end;
end;
...
IntToSet(Param1, VarToInteger(GetInputArg(1)), SizeOf(Param1));
    Case InputArgCount of
      1: TCustomTreeView(CurrentObject).Select(TTreeNode(VarToObject(GetInputArg(0))));
      2: TCustomTreeView(CurrentObject).Select(TTreeNode(VarToObject(GetInputArg(0))),Param1);
    end;
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomTreeView(CurrentObject).Deselect(TTreeNode(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  ...
  begin
    Case InputArgCount of
      1: TCustomTreeView(CurrentObject).Subselect(TTreeNode(VarToObject(GetInputArg(0))));
      2: TCustomTreeView(CurrentObject).Subselect(TTreeNode(VarToObject(GetInputArg(0))),GetInputArg(1));
    end;
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomTreeView(CurrentObject).GetSelections(TList(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomTreeView(CurrentObject).FindNextToSelect);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCustomTreeView(CurrentObject).Canvas));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCustomTreeView(CurrentObject).DropTarget));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomTreeView(CurrentObject).DropTarget:=TTreeNode(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCustomTreeView(CurrentObject).Selected));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomTreeView(CurrentObject).Selected:=TTreeNode(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCustomTreeView(CurrentObject).TopItem));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomTreeView(CurrentObject).TopItem:=TTreeNode(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCustomTreeView(CurrentObject).Selections[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTrackBarClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TProgressBarClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTextAttributesClass(CurrentClass.ClassRef).Create(TCustomRichEdit(VarToObject(GetInputArg(0))),TAttributeType(VarToInteger(GetInputArg(1)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TTextAttributes(CurrentObject).Assign(TPersistent(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TTextAttributes(CurrentObject).Charset));
  end;
end;
...
begin
  with AMachine do
  begin
    TTextAttributes(CurrentObject).Charset:=TFontCharset(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TTextAttributes(CurrentObject).Color));
  end;
end;
...
begin
  with AMachine do
  begin
    TTextAttributes(CurrentObject).Color:=TColor(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TTextAttributes(CurrentObject).Name));
  end;
end;
...
begin
  with AMachine do
  begin
    TTextAttributes(CurrentObject).Name:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TTextAttributes(CurrentObject).Pitch));
  end;
end;
...
begin
  with AMachine do
  begin
    TTextAttributes(CurrentObject).Pitch:=TFontPitch(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TParaAttributesClass(CurrentClass.ClassRef).Create(TCustomRichEdit(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TParaAttributes(CurrentObject).Assign(TPersistent(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TParaAttributes(CurrentObject).Alignment));
  end;
end;
...
begin
  with AMachine do
  begin
    TParaAttributes(CurrentObject).Alignment:=TAlignment(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TParaAttributes(CurrentObject).Numbering));
  end;
end;
...
begin
  with AMachine do
  begin
    TParaAttributes(CurrentObject).Numbering:=TNumberingStyle(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TConversion(CurrentObject).ConvertReadStream(TStream(VarToObject(GetInputArg(0))),PChar(VarToStr(GetInputArg(1))),VarToInteger(GetInputArg(2))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TConversion(CurrentObject).ConvertWriteStream(TStream(VarToObject(GetInputArg(0))),PChar(VarToStr(GetInputArg(1))),VarToInteger(GetInputArg(2))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomRichEditClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
AResult := Integer(TCustomRichEdit(CurrentObject).FindText(VarToStr(GetInputArg(0)),VarToInteger(GetInputArg(1)),VarToInteger(GetInputArg(2)),Param3));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomRichEdit(CurrentObject).Print(VarToStr(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCustomRichEdit(CurrentObject).DefAttributes));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomRichEdit(CurrentObject).DefAttributes:=TTextAttributes(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCustomRichEdit(CurrentObject).SelAttributes));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomRichEdit(CurrentObject).SelAttributes:=TTextAttributes(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCustomRichEdit(CurrentObject).Paragraph));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomUpDownClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomHotKeyClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TListColumnClass(CurrentClass.ClassRef).Create(TCollection(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TListColumn(CurrentObject).Assign(TPersistent(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TListColumn(CurrentObject).WidthType));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TListColumnsClass(CurrentClass.ClassRef).Create(TCustomListView(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TListColumns(CurrentObject).Add);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TListColumns(CurrentObject).Owner);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TListColumns(CurrentObject).Items[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    TListColumns(CurrentObject).Items[VarToInteger(GetArrayIndex(0))]:=TListColumn(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TListItemClass(CurrentClass.ClassRef).Create(TListItems(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TListItem(CurrentObject).Assign(TPersistent(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := integer(TRectWrapper.Create(TListItem(CurrentObject).DisplayRect(TDisplayCode(VarToInteger(GetInputArg(0))))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TListItem(CurrentObject).Caption));
  end;
end;
...
begin
  with AMachine do
  begin
    TListItem(CurrentObject).Caption:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TListItem(CurrentObject).ListView));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TListItem(CurrentObject).Owner));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TListItem(CurrentObject).SubItems));
  end;
end;
...
begin
  with AMachine do
  begin
    TListItem(CurrentObject).SubItems:=TStrings(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TListItemsEnumeratorClass(CurrentClass.ClassRef).Create(TListItems(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TListItemsEnumerator(CurrentObject).GetCurrent);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TListItemsEnumerator(CurrentObject).Current));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TListItemsClass(CurrentClass.ClassRef).Create(TCustomListView(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TListItems(CurrentObject).Add);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
case InputArgCount of
1: AResult := ObjectToVar(TListItems(CurrentObject).AddItem(TListItem(VarToObject(GetInputArg(0)))));
2: AResult := ObjectToVar(TListItems(CurrentObject).AddItem(TListItem(VarToObject(GetInputArg(0))),VarToInteger(GetInputArg(1))));
end;
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TListItems(CurrentObject).Assign(TPersistent(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TListItems(CurrentObject).GetEnumerator);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TListItems(CurrentObject).IndexOf(TListItem(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TListItems(CurrentObject).Insert(VarToInteger(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TListItems(CurrentObject).Item[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    TListItems(CurrentObject).Item[VarToInteger(GetArrayIndex(0))]:=TListItem(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TListItems(CurrentObject).Owner));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TWorkAreaClass(CurrentClass.ClassRef).Create(TCollection(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TWorkArea(CurrentObject).SetDisplayName(VarToStr(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := string(TWorkArea(CurrentObject).GetDisplayName);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TWorkArea(CurrentObject).Color));
  end;
end;
...
begin
  with AMachine do
  begin
    TWorkArea(CurrentObject).Color:=TColor(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TWorkAreas(CurrentObject).Add);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TWorkAreas(CurrentObject).Insert(VarToInteger(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TWorkAreas(CurrentObject).Items[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    TWorkAreas(CurrentObject).Items[VarToInteger(GetArrayIndex(0))]:=TWorkArea(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIconOptionsClass(CurrentClass.ClassRef).Create(TCustomListView(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomListViewClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomListView(CurrentObject).AddItem(VarToStr(GetInputArg(0)),TObject(VarToObject(GetInputArg(1))));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomListView(CurrentObject).Arrange(TListArrangement(VarToInteger(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomListView(CurrentObject).CopySelection(TCustomListControl(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomListView(CurrentObject).FindCaption(VarToInteger(GetInputArg(0)),VarToStr(GetInputArg(1)),GetInputArg(2),GetInputArg(3),GetInputArg(4)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomListView(CurrentObject).GetItemAt(VarToInteger(GetInputArg(0)),VarToInteger(GetInputArg(1))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomListView(CurrentObject).GetNearestItem(TPointWrapper(integer(GetInputArg(0))).ObjToRec,TSearchDirection(VarToInteger(GetInputArg(1)))));
    ReturnOutputArg(AResult);
  end;
end;
...
AResult := ObjectToVar(TCustomListView(CurrentObject).GetNextItem(TListItem(VarToObject(GetInputArg(0))),TSearchDirection(VarToInteger(GetInputArg(1))),Param2));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := string(TCustomListView(CurrentObject).GetSearchString);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TCustomListView(CurrentObject).StringWidth(VarToStr(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCustomListView(CurrentObject).Canvas));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCustomListView(CurrentObject).Column[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCustomListView(CurrentObject).DropTarget));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomListView(CurrentObject).DropTarget:=TListItem(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCustomListView(CurrentObject).ItemFocused));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomListView(CurrentObject).ItemFocused:=TListItem(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCustomListView(CurrentObject).Selected));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomListView(CurrentObject).Selected:=TListItem(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCustomListView(CurrentObject).TopItem));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCustomListView(CurrentObject).WorkAreas));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TAnimateClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TAnimate(CurrentObject).ResHandle));
  end;
end;
...
begin
  with AMachine do
  begin
    TAnimate(CurrentObject).ResHandle:=VarToInteger(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TAnimate(CurrentObject).ResName));
  end;
end;
...
begin
  with AMachine do
  begin
    TAnimate(CurrentObject).ResName:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TToolButtonClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TToolBarEnumeratorClass(CurrentClass.ClassRef).Create(TToolBar(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TToolBarEnumerator(CurrentObject).GetCurrent);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TToolBarEnumerator(CurrentObject).Current));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TToolBarClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TToolBar(CurrentObject).GetEnumerator);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := TToolBar(CurrentObject).TrackMenu(TToolButton(VarToObject(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TToolBar(CurrentObject).Buttons[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TToolBar(CurrentObject).Canvas));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TToolBar(CurrentObject).CustomizeKeyName));
  end;
end;
...
begin
  with AMachine do
  begin
    TToolBar(CurrentObject).CustomizeKeyName:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TToolBar(CurrentObject).CustomizeValueName));
  end;
end;
...
begin
  with AMachine do
  begin
    TToolBar(CurrentObject).CustomizeValueName:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCoolBandClass(CurrentClass.ClassRef).Create(TCollection(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TCoolBand(CurrentObject).Assign(TPersistent(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCoolBandsClass(CurrentClass.ClassRef).Create(TCoolBar(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCoolBands(CurrentObject).Add);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCoolBands(CurrentObject).FindBand(TControl(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCoolBands(CurrentObject).CoolBar));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCoolBands(CurrentObject).Items[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    TCoolBands(CurrentObject).Items[VarToInteger(GetArrayIndex(0))]:=TCoolBand(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCoolBarClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TMonthCalColorsClass(CurrentClass.ClassRef).Create(TCommonCalendar(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TMonthCalColors(CurrentObject).Assign(TPersistent(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCommonCalendarClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TMonthCalendarClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TDateTimePickerClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TPageScrollerClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TPageScroller(CurrentObject).GetButtonState(TPageScrollerButton(VarToInteger(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TComboExItem(CurrentObject).Assign(TPersistent(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TComboExItems(CurrentObject).Add);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TComboExItems(CurrentObject).Insert(VarToInteger(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TComboExItems(CurrentObject).ComboItems[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TComboBoxExStringsClass(CurrentClass.ClassRef).Create(TCustomComboBoxEx(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TComboBoxExStrings(CurrentObject).Add(VarToStr(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TComboBoxExStrings(CurrentObject).AddObject(VarToStr(GetInputArg(0)),TObject(VarToObject(GetInputArg(1)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := string(TComboBoxExStrings(CurrentObject).Get(VarToInteger(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TComboBoxExStrings(CurrentObject).GetObject(VarToInteger(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TComboBoxExStrings(CurrentObject).IndexOf(VarToStr(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TComboBoxExStrings(CurrentObject).IndexOfName(VarToStr(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TComboBoxExStrings(CurrentObject).Insert(VarToInteger(GetInputArg(0)),VarToStr(GetInputArg(1)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TComboBoxExStrings(CurrentObject).SortType));
  end;
end;
...
begin
  with AMachine do
  begin
    TComboBoxExStrings(CurrentObject).SortType:=TListItemsSortType(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TComboBoxExStrings(CurrentObject).ItemsEx));
  end;
end;
...
begin
  with AMachine do
  begin
    TComboBoxExStrings(CurrentObject).ItemsEx:=TComboExItems(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomComboBoxExClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCustomComboBoxEx(CurrentObject).Images));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomComboBoxEx(CurrentObject).Images:=TCustomImageList(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCustomComboBoxEx(CurrentObject).ItemsEx));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomComboBoxEx(CurrentObject).ItemsEx:=TComboExItems(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TCustomComboBoxEx(CurrentObject).SelText));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomComboBoxEx(CurrentObject).SelText:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TCustomComboBoxEx(CurrentObject).Style));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomComboBoxEx(CurrentObject).Style:=TComboBoxExStyle(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  ...
  begin
    ...
    AllowChangeTemp := AllowChange;
    DirectionTemp := Integer(Direction);
    if Assigned(Scripter) and (RoutineName > '') then
...
begin
  ...
  begin
    ...
    ButtonTemp := ObjectToVar(Button);
    if Assigned(Scripter) and (RoutineName > '') then
      Scripter.ExecuteSubroutine(RoutineName, [Sender,Index,ButtonTemp]);
    Button := TToolButton(VarToObject(ButtonTemp));
    if AssignedMethod(AfterCall) then
...
begin
  ...
  begin
    ...
    OrientationTemp := Integer(Orientation);
...
begin
  with AMachine do
  begin
    ComCtrls.CheckToolMenuDropdown(TToolButton(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TCustomTabControl,__TCustomTabControlCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TCustomTabControlDestroy,false,0,'');
    DefineMethod('IndexOfTabAt',2,tkInteger,nil,__TCustomTabControlIndexOfTabAt,false,0,'X: Integer; Y: Integer');
    DefineMethod('GetHitTestInfoAt',2,tkInteger,nil,__TCustomTabControlGetHitTestInfoAt,false,0,'X: Integer; Y: Integer');
    DefineMethod('TabRect',1,tkVariant,nil,__TCustomTabControlTabRect,false,0,'Index: Integer');
    DefineMethod('RowCount',0,tkInteger,nil,__TCustomTabControlRowCount,false,0,'');
    DefineMethod('ScrollTabs',1,tkNone,nil,__TCustomTabControlScrollTabs,false,0,'Delta: Integer');
  ...
  begin
    DefineMethod('Create',1,tkClass,TTabSheet,__TTabSheetCreate,true,0,'AOwner: TComponent');
  ...
  begin
    DefineMethod('Create',1,tkClass,TPageControl,__TPageControlCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TPageControlDestroy,false,0,'');
    DefineMethod('FindNextPage',3,tkClass,TTabSheet,__TPageControlFindNextPage,false,0,'CurPage: TTabSheet; GoForward: Boolean; CheckTabVisible: Boolean');
    DefineMethod('SelectNextPage',2,tkNone,nil,__TPageControlSelectNextPage,false,1,'GoForward: Boolean; CheckTabVisible: Boolean = True');
  ...
  begin
    DefineMethod('Create',1,tkClass,TStatusPanel,__TStatusPanelCreate,true,0,'Collection: TCollection');
    DefineMethod('Assign',1,tkNone,nil,__TStatusPanelAssign,false,0,'Source: TPersistent');
  ...
  begin
    DefineMethod('Create',1,tkClass,TStatusPanels,__TStatusPanelsCreate,true,0,'StatusBar: TCustomStatusBar');
    DefineMethod('Add',0,tkClass,TStatusPanel,__TStatusPanelsAdd,false,0,'');
    DefineMethod('AddItem',2,tkClass,TStatusPanel,__TStatusPanelsAddItem,false,0,'Item: TStatusPanel; Index: Integer');
    DefineMethod('Insert',1,tkClass,TStatusPanel,__TStatusPanelsInsert,false,0,'Index: Integer');
  ...
  begin
    DefineMethod('Create',1,tkClass,TCustomStatusBar,__TCustomStatusBarCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TCustomStatusBarDestroy,false,0,'');
    DefineMethod('ExecuteAction',1,tkVariant,nil,__TCustomStatusBarExecuteAction,false,0,'Action: TBasicAction');
    DefineMethod('FlipChildren',1,tkNone,nil,__TCustomStatusBarFlipChildren,false,0,'AllLevels: Boolean');
    DefineMethod('SetBounds',4,tkNone,nil,__TCustomStatusBarSetBounds,false,0,'ALeft: Integer; ATop: Integer; AWidth: Integer; AHeight: Integer');
  ...
  begin
    DefineMethod('Create',1,tkClass,THeaderSection,__THeaderSectionCreate,true,0,'Collection: TCollection');
    DefineMethod('Assign',1,tkNone,nil,__THeaderSectionAssign,false,0,'Source: TPersistent');
  ...
  begin
    DefineMethod('Create',1,tkClass,THeaderSections,__THeaderSectionsCreate,true,0,'HeaderControl: TCustomHeaderControl');
    DefineMethod('Add',0,tkClass,THeaderSection,__THeaderSectionsAdd,false,0,'');
    DefineMethod('AddItem',2,tkClass,THeaderSection,__THeaderSectionsAddItem,false,0,'Item: THeaderSection; Index: Integer');
    DefineMethod('Insert',1,tkClass,THeaderSection,__THeaderSectionsInsert,false,0,'Index: Integer');
  ...
  begin
    DefineMethod('Create',1,tkClass,TCustomHeaderControl,__TCustomHeaderControlCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TCustomHeaderControlDestroy,false,0,'');
    DefineMethod('FlipChildren',1,tkNone,nil,__TCustomHeaderControlFlipChildren,false,0,'AllLevels: Boolean');
  ...
  begin
    DefineMethod('Create',1,tkClass,TTreeNode,__TTreeNodeCreate,true,0,'AOwner: TTreeNodes');
    DefineMethod('Destroy',0,tkNone,nil,__TTreeNodeDestroy,false,0,'');
    DefineMethod('AlphaSort',1,tkVariant,nil,__TTreeNodeAlphaSort,false,1,'ARecurse: Boolean = False');
    DefineMethod('Assign',1,tkNone,nil,__TTreeNodeAssign,false,0,'Source: TPersistent');
    DefineMethod('Collapse',1,tkNone,nil,__TTreeNodeCollapse,false,0,'Recurse: Boolean');
    ...
    DefineMethod('DisplayRect',1,tkVariant,nil,__TTreeNodeDisplayRect,false,0,'TextOnly: Boolean');
    DefineMethod('EditText',0,tkVariant,nil,__TTreeNodeEditText,false,0,'');
    DefineMethod('EndEdit',1,tkNone,nil,__TTreeNodeEndEdit,false,0,'Cancel: Boolean');
    DefineMethod('Expand',1,tkNone,nil,__TTreeNodeExpand,false,0,'Recurse: Boolean');
    ...
    DefineMethod('GetNextChild',1,tkClass,TTreeNode,__TTreeNodeGetNextChild,false,0,'Value: TTreeNode');
    ...
    DefineMethod('GetPrevChild',1,tkClass,TTreeNode,__TTreeNodeGetPrevChild,false,0,'Value: TTreeNode');
    ...
    DefineMethod('HasAsParent',1,tkVariant,nil,__TTreeNodeHasAsParent,false,0,'Value: TTreeNode');
    DefineMethod('IndexOf',1,tkInteger,nil,__TTreeNodeIndexOf,false,0,'Value: TTreeNode');
    DefineMethod('MakeVisible',0,tkNone,nil,__TTreeNodeMakeVisible,false,0,'');
    DefineMethod('MoveTo',2,tkNone,nil,__TTreeNodeMoveTo,false,0,'Destination: TTreeNode; Mode: TNodeAttachMode');
  ...
  begin
    DefineMethod('Create',1,tkClass,TTreeNodesEnumerator,__TTreeNodesEnumeratorCreate,true,0,'ATreeNodes: TTreeNodes');
  ...
  begin
    DefineMethod('Create',1,tkClass,TTreeNodes,__TTreeNodesCreate,true,0,'AOwner: TCustomTreeView');
    DefineMethod('Destroy',0,tkNone,nil,__TTreeNodesDestroy,false,0,'');
    DefineMethod('AddChildFirst',2,tkClass,TTreeNode,__TTreeNodesAddChildFirst,false,0,'Parent: TTreeNode; S: string');
    DefineMethod('AddChild',2,tkClass,TTreeNode,__TTreeNodesAddChild,false,0,'Parent: TTreeNode; S: string');
    DefineMethod('AddFirst',2,tkClass,TTreeNode,__TTreeNodesAddFirst,false,0,'Sibling: TTreeNode; S: string');
    DefineMethod('Add',2,tkClass,TTreeNode,__TTreeNodesAdd,false,0,'Sibling: TTreeNode; S: string');
    DefineMethod('AlphaSort',1,tkVariant,nil,__TTreeNodesAlphaSort,false,1,'ARecurse: Boolean = False');
    DefineMethod('Assign',1,tkNone,nil,__TTreeNodesAssign,false,0,'Source: TPersistent');
    ...
    DefineMethod('Delete',1,tkNone,nil,__TTreeNodesDelete,false,0,'Node: TTreeNode');
    ...
    DefineMethod('Insert',2,tkClass,TTreeNode,__TTreeNodesInsert,false,0,'Sibling: TTreeNode; S: string');
  ...
  begin
    DefineMethod('Create',1,tkClass,TCustomTreeView,__TCustomTreeViewCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TCustomTreeViewDestroy,false,0,'');
    DefineMethod('AlphaSort',1,tkVariant,nil,__TCustomTreeViewAlphaSort,false,1,'ARecurse: Boolean = True');
    ...
    DefineMethod('GetHitTestInfoAt',2,tkInteger,nil,__TCustomTreeViewGetHitTestInfoAt,false,0,'X: Integer; Y: Integer');
    DefineMethod('GetNodeAt',2,tkClass,TTreeNode,__TCustomTreeViewGetNodeAt,false,0,'X: Integer; Y: Integer');
    DefineMethod('IsEditing',0,tkVariant,nil,__TCustomTreeViewIsEditing,false,0,'');
    DefineMethod('LoadFromFile',1,tkNone,nil,__TCustomTreeViewLoadFromFile,false,0,'FileName: string');
    DefineMethod('LoadFromStream',1,tkNone,nil,__TCustomTreeViewLoadFromStream,false,0,'Stream: TStream');
    DefineMethod('SaveToFile',1,tkNone,nil,__TCustomTreeViewSaveToFile,false,0,'FileName: string');
    DefineMethod('SaveToStream',1,tkNone,nil,__TCustomTreeViewSaveToStream,false,0,'Stream: TStream');
    DefineMethod('Select',2,tkNone,nil,__TCustomTreeViewSelect,false,1,'Node: TTreeNode; ShiftState: TShiftState = []');
    DefineMethod('Deselect',1,tkNone,nil,__TCustomTreeViewDeselect,false,0,'Node: TTreeNode');
    DefineMethod('Subselect',2,tkNone,nil,__TCustomTreeViewSubselect,false,1,'Node: TTreeNode; Validate: Boolean = False');
    DefineMethod('ClearSelection',1,tkNone,nil,__TCustomTreeViewClearSelection,false,1,'KeepPrimary: Boolean = False');
    DefineMethod('GetSelections',1,tkClass,TTreeNode,__TCustomTreeViewGetSelections,false,0,'AList: TList');
  ...
  begin
    DefineMethod('Create',1,tkClass,TTrackBar,__TTrackBarCreate,true,0,'AOwner: TComponent');
    DefineMethod('SetTick',1,tkNone,nil,__TTrackBarSetTick,false,0,'Value: Integer');
  end;
  With Scripter.DefineClass(TProgressBar) do
  begin
    DefineMethod('Create',1,tkClass,TProgressBar,__TProgressBarCreate,true,0,'AOwner: TComponent');
    DefineMethod('StepIt',0,tkNone,nil,__TProgressBarStepIt,false,0,'');
    DefineMethod('StepBy',1,tkNone,nil,__TProgressBarStepBy,false,0,'Delta: Integer');
  end;
  With Scripter.DefineClass(TTextAttributes) do
  begin
    DefineMethod('Create',2,tkClass,TTextAttributes,__TTextAttributesCreate,true,0,'AOwner: TCustomRichEdit; AttributeType: TAttributeType');
    DefineMethod('Assign',1,tkNone,nil,__TTextAttributesAssign,false,0,'Source: TPersistent');
  ...
  begin
    DefineMethod('Create',1,tkClass,TParaAttributes,__TParaAttributesCreate,true,0,'AOwner: TCustomRichEdit');
    DefineMethod('Assign',1,tkNone,nil,__TParaAttributesAssign,false,0,'Source: TPersistent');
  ...
  begin
    DefineMethod('ConvertReadStream',3,tkInteger,nil,__TConversionConvertReadStream,false,0,'Stream: TStream; Buffer: PChar; BufSize: Integer');
    DefineMethod('ConvertWriteStream',3,tkInteger,nil,__TConversionConvertWriteStream,false,0,'Stream: TStream; Buffer: PChar; BufSize: Integer');
  end;
  With Scripter.DefineClass(TCustomRichEdit) do
  begin
    DefineMethod('Create',1,tkClass,TCustomRichEdit,__TCustomRichEditCreate,true,0,'AOwner: TComponent');
    ...
    DefineMethod('FindText',4,tkInteger,nil,__TCustomRichEditFindText,false,0,'SearchStr: string; StartPos: Integer; Length: Integer; Options: TSearchTypes');
    DefineMethod('GetSelTextBuf',2,tkInteger,nil,__TCustomRichEditGetSelTextBuf,false,0,'Buffer: PChar; BufSize: Integer');
    DefineMethod('Print',1,tkNone,nil,__TCustomRichEditPrint,false,0,'Caption: string');
  ...
  begin
    DefineMethod('Create',1,tkClass,TCustomUpDown,__TCustomUpDownCreate,true,0,'AOwner: TComponent');
  end;
  ...
  begin
    DefineMethod('Create',1,tkClass,TCustomHotKey,__TCustomHotKeyCreate,true,0,'AOwner: TComponent');
  end;
  ...
  begin
    DefineMethod('Create',1,tkClass,TListColumn,__TListColumnCreate,true,0,'Collection: TCollection');
    DefineMethod('Destroy',0,tkNone,nil,__TListColumnDestroy,false,0,'');
    DefineMethod('Assign',1,tkNone,nil,__TListColumnAssign,false,0,'Source: TPersistent');
    DefineProp('WidthType',tkEnumeration,__GetTListColumnWidthType,nil,nil,false,0);
  end;
  With Scripter.DefineClass(TListColumns) do
  begin
    DefineMethod('Create',1,tkClass,TListColumns,__TListColumnsCreate,true,0,'AOwner: TCustomListView');
  ...
  begin
    DefineMethod('Create',1,tkClass,TListItem,__TListItemCreate,true,0,'AOwner: TListItems');
    DefineMethod('Destroy',0,tkNone,nil,__TListItemDestroy,false,0,'');
    DefineMethod('Assign',1,tkNone,nil,__TListItemAssign,false,0,'Source: TPersistent');
    ...
    DefineMethod('DisplayRect',1,tkVariant,nil,__TListItemDisplayRect,false,0,'Code: TDisplayCode');
    ...
    DefineMethod('MakeVisible',1,tkNone,nil,__TListItemMakeVisible,false,0,'PartialOK: Boolean');
    DefineMethod('Update',0,tkNone,nil,__TListItemUpdate,false,0,'');
    DefineMethod('SetPosition',1,tkNone,nil,__TListItemSetPosition,false,0,'Value: TPoint');
  ...
  begin
    DefineMethod('Create',1,tkClass,TListItemsEnumerator,__TListItemsEnumeratorCreate,true,0,'AListItems: TListItems');
  ...
  begin
    DefineMethod('Create',1,tkClass,TListItems,__TListItemsCreate,true,0,'AOwner: TCustomListView');
    ...
    DefineMethod('AddItem',2,tkClass,TListItem,__TListItemsAddItem,false,1,'Item: TListItem; Index: Integer = -1');
    DefineMethod('Assign',1,tkNone,nil,__TListItemsAssign,false,0,'Source: TPersistent');
    ...
    DefineMethod('Delete',1,tkNone,nil,__TListItemsDelete,false,0,'Index: Integer');
    ...
    DefineMethod('IndexOf',1,tkInteger,nil,__TListItemsIndexOf,false,0,'Value: TListItem');
    DefineMethod('Insert',1,tkClass,TListItem,__TListItemsInsert,false,0,'Index: Integer');
  ...
  begin
    DefineMethod('Create',1,tkClass,TWorkArea,__TWorkAreaCreate,true,0,'Collection: TCollection');
    DefineMethod('SetDisplayName',1,tkNone,nil,__TWorkAreaSetDisplayName,false,0,'Value: string');
  ...
  begin
    ...
    DefineMethod('Delete',1,tkNone,nil,__TWorkAreasDelete,false,0,'Index: Integer');
    DefineMethod('Insert',1,tkClass,TWorkArea,__TWorkAreasInsert,false,0,'Index: Integer');
  ...
  begin
    DefineMethod('Create',1,tkClass,TIconOptions,__TIconOptionsCreate,true,0,'AOwner: TCustomListView');
  end;
  With Scripter.DefineClass(TCustomListView) do
  begin
    DefineMethod('Create',1,tkClass,TCustomListView,__TCustomListViewCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TCustomListViewDestroy,false,0,'');
    DefineMethod('AddItem',2,tkNone,nil,__TCustomListViewAddItem,false,0,'Item: String; AObject: TObject');
    DefineMethod('AlphaSort',0,tkVariant,nil,__TCustomListViewAlphaSort,false,0,'');
    DefineMethod('Arrange',1,tkNone,nil,__TCustomListViewArrange,false,0,'Code: TListArrangement');
    ...
    DefineMethod('CopySelection',1,tkNone,nil,__TCustomListViewCopySelection,false,0,'Destination: TCustomListControl');
    DefineMethod('DeleteSelected',0,tkNone,nil,__TCustomListViewDeleteSelected,false,0,'');
    DefineMethod('FindCaption',5,tkClass,TListItem,__TCustomListViewFindCaption,false,0,'StartIndex: Integer; Value: string; Partial: Boolean; Inclusive: Boolean; Wrap: Boolean');
    DefineMethod('GetHitTestInfoAt',2,tkInteger,nil,__TCustomListViewGetHitTestInfoAt,false,0,'X: Integer; Y: Integer');
    DefineMethod('GetItemAt',2,tkClass,TListItem,__TCustomListViewGetItemAt,false,0,'X: Integer; Y: Integer');
    DefineMethod('GetNearestItem',2,tkClass,TListItem,__TCustomListViewGetNearestItem,false,0,'Point: TPoint; Direction: TSearchDirection');
    DefineMethod('GetNextItem',3,tkClass,TListItem,__TCustomListViewGetNextItem,false,0,'StartItem: TListItem; Direction: TSearchDirection; States: TItemStates');
    ...
    DefineMethod('Scroll',2,tkNone,nil,__TCustomListViewScroll,false,0,'DX: Integer; DY: Integer');
    DefineMethod('StringWidth',1,tkInteger,nil,__TCustomListViewStringWidth,false,0,'S: string');
    DefineMethod('UpdateItems',2,tkNone,nil,__TCustomListViewUpdateItems,false,0,'FirstIndex: Integer; LastIndex: Integer');
  ...
  begin
    DefineMethod('Create',1,tkClass,TAnimate,__TAnimateCreate,true,0,'AOwner: TComponent');
    DefineMethod('Play',3,tkNone,nil,__TAnimatePlay,false,0,'FromFrame: Word; ToFrame: Word; Count: Integer');
    DefineMethod('Reset',0,tkNone,nil,__TAnimateReset,false,0,'');
    DefineMethod('Seek',1,tkNone,nil,__TAnimateSeek,false,0,'Frame: Smallint');
    ...
    DefineProp('ResHandle',tkInteger,__GetTAnimateResHandle,__SetTAnimateResHandle,nil,false,0);
  ...
  begin
    DefineMethod('Create',1,tkClass,TToolButton,__TToolButtonCreate,true,0,'AOwner: TComponent');
    ...
    DefineMethod('SetBounds',4,tkNone,nil,__TToolButtonSetBounds,false,0,'ALeft: Integer; ATop: Integer; AWidth: Integer; AHeight: Integer');
    DefineProp('Index',tkInteger,__GetTToolButtonIndex,nil,nil,false,0);
  end;
  With Scripter.DefineClass(TToolBarEnumerator) do
  begin
    DefineMethod('Create',1,tkClass,TToolBarEnumerator,__TToolBarEnumeratorCreate,true,0,'AToolBar: TToolBar');
  ...
  begin
    DefineMethod('Create',1,tkClass,TToolBar,__TToolBarCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TToolBarDestroy,false,0,'');
    DefineMethod('FlipChildren',1,tkNone,nil,__TToolBarFlipChildren,false,0,'AllLevels: Boolean');
    DefineMethod('GetEnumerator',0,tkClass,TToolBarEnumerator,__TToolBarGetEnumerator,false,0,'');
    DefineMethod('TrackMenu',1,tkVariant,nil,__TToolBarTrackMenu,false,0,'Button: TToolButton');
  ...
  begin
    DefineMethod('Create',1,tkClass,TCoolBand,__TCoolBandCreate,true,0,'Collection: TCollection');
    DefineMethod('Destroy',0,tkNone,nil,__TCoolBandDestroy,false,0,'');
    DefineMethod('Assign',1,tkNone,nil,__TCoolBandAssign,false,0,'Source: TPersistent');
    DefineProp('Height',tkInteger,__GetTCoolBandHeight,nil,nil,false,0);
  end;
  With Scripter.DefineClass(TCoolBands) do
  begin
    DefineMethod('Create',1,tkClass,TCoolBands,__TCoolBandsCreate,true,0,'CoolBar: TCoolBar');
    DefineMethod('Add',0,tkClass,TCoolBand,__TCoolBandsAdd,false,0,'');
    DefineMethod('FindBand',1,tkClass,TCoolBand,__TCoolBandsFindBand,false,0,'AControl: TControl');
  ...
  begin
    DefineMethod('Create',1,tkClass,TCoolBar,__TCoolBarCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TCoolBarDestroy,false,0,'');
    DefineMethod('FlipChildren',1,tkNone,nil,__TCoolBarFlipChildren,false,0,'AllLevels: Boolean');
  end;
  ...
  begin
    DefineMethod('Create',1,tkClass,TMonthCalColors,__TMonthCalColorsCreate,true,0,'AOwner: TCommonCalendar');
    DefineMethod('Assign',1,tkNone,nil,__TMonthCalColorsAssign,false,0,'Source: TPersistent');
  end;
  With Scripter.DefineClass(TCommonCalendar) do
  begin
    DefineMethod('Create',1,tkClass,TCommonCalendar,__TCommonCalendarCreate,true,0,'AOwner: TComponent');
  ...
  begin
    DefineMethod('Create',1,tkClass,TMonthCalendar,__TMonthCalendarCreate,true,0,'AOwner: TComponent');
  end;
  ...
  begin
    DefineMethod('Create',1,tkClass,TDateTimePicker,__TDateTimePickerCreate,true,0,'AOwner: TComponent');
  ...
  begin
    DefineMethod('Create',1,tkClass,TPageScroller,__TPageScrollerCreate,true,0,'AOwner: TComponent');
    DefineMethod('GetButtonState',1,tkEnumeration,nil,__TPageScrollerGetButtonState,false,0,'Button: TPageScrollerButton');
  end;
  With Scripter.DefineClass(TComboExItem) do
  begin
    DefineMethod('Assign',1,tkNone,nil,__TComboExItemAssign,false,0,'Source: TPersistent');
  end;
  With Scripter.DefineClass(TComboExItems) do
  begin
    DefineMethod('Add',0,tkClass,TComboExItem,__TComboExItemsAdd,false,0,'');
    DefineMethod('Insert',1,tkClass,TComboExItem,__TComboExItemsInsert,false,0,'Index: Integer');
  ...
  begin
    DefineMethod('Create',1,tkClass,TComboBoxExStrings,__TComboBoxExStringsCreate,true,0,'Owner: TCustomComboBoxEx');
    DefineMethod('Destroy',0,tkNone,nil,__TComboBoxExStringsDestroy,false,0,'');
    DefineMethod('Add',1,tkInteger,nil,__TComboBoxExStringsAdd,false,0,'S: String');
    DefineMethod('AddObject',2,tkInteger,nil,__TComboBoxExStringsAddObject,false,0,'S: String; AObject: TObject');
    DefineMethod('Clear',0,tkNone,nil,__TComboBoxExStringsClear,false,0,'');
    DefineMethod('Delete',1,tkNone,nil,__TComboBoxExStringsDelete,false,0,'Index: Integer');
    DefineMethod('Exchange',2,tkNone,nil,__TComboBoxExStringsExchange,false,0,'Index1: Integer; Index2: Integer');
    DefineMethod('Get',1,tkVariant,nil,__TComboBoxExStringsGet,false,0,'Index: Integer');
    ...
    DefineMethod('GetObject',1,tkClass,TObject,__TComboBoxExStringsGetObject,false,0,'Index: Integer');
    DefineMethod('IndexOf',1,tkInteger,nil,__TComboBoxExStringsIndexOf,false,0,'S: String');
    DefineMethod('IndexOfName',1,tkInteger,nil,__TComboBoxExStringsIndexOfName,false,0,'Name: String');
    DefineMethod('Insert',2,tkNone,nil,__TComboBoxExStringsInsert,false,0,'Index: Integer; S: String');
    DefineMethod('Move',2,tkNone,nil,__TComboBoxExStringsMove,false,0,'CurIndex: Integer; NewIndex: Integer');
  ...
  begin
    DefineMethod('Create',1,tkClass,TCustomComboBoxEx,__TCustomComboBoxExCreate,true,0,'AOwner: TComponent');
  ...
  begin
    DefineMethod('InitCommonControl',1,tkVariant,nil,__InitCommonControl,false,0,'CC: Integer');
    DefineMethod('CheckCommonControl',1,tkNone,nil,__CheckCommonControl,false,0,'CC: Integer');
    DefineMethod('GetComCtlVersion',0,tkInteger,nil,__GetComCtlVersion,false,0,'');
    DefineMethod('CheckToolMenuDropdown',1,tkNone,nil,__CheckToolMenuDropdown,false,0,'ToolButton: TToolButton');

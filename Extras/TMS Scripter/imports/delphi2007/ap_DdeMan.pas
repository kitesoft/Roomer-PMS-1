...
begin
  with AMachine do
  begin
AResult := TDdeClientConv(CurrentObject).PokeData(VarToStr(GetInputArg(0)),PChar(VarToStr(GetInputArg(1))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    ...
    DefineMethod('PokeData',2,tkVariant,nil,__TDdeClientConvPokeData,false,0,'Item: string; Data: PChar');
    DefineMethod('ExecuteMacroLines',2,tkVariant,nil,__TDdeClientConvExecuteMacroLines,false,0,'Cmd: TStrings; waitFlg: Boolean');
    DefineMethod('ExecuteMacro',2,tkVariant,nil,__TDdeClientConvExecuteMacro,false,0,'Cmd: PChar; waitFlg: Boolean');

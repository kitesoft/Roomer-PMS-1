...
type
  TatTabsLibrary = class(TatScripterLibrary)
    ...
    procedure __TTabListCreate(AMachine: TatVirtualMachine);
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TScrollerClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
procedure TatTabsLibrary.__TTabListCreate(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTabListClass(CurrentClass.ClassRef).Create(TTabSet(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TTabList(CurrentObject).Insert(VarToInteger(GetInputArg(0)),VarToStr(GetInputArg(1)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TTabList(CurrentObject).Add(VarToStr(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TTabList(CurrentObject).Put(VarToInteger(GetInputArg(0)),VarToStr(GetInputArg(1)));
  end;
end;
...
begin
  with AMachine do
  begin
    TTabList(CurrentObject).AddStrings(TStrings(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTabSetClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
1: AResult := Integer(TTabSet(CurrentObject).ItemAtPos(TPointWrapper(integer(GetInputArg(0))).ObjToRec));
2: AResult := Integer(TTabSet(CurrentObject).ItemAtPos(TPointWrapper(integer(GetInputArg(0))).ObjToRec,GetInputArg(1)));
end;
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TScroller,__TScrollerCreate,true,0,'AOwner: TComponent');
  ...
  begin
    DefineMethod('Create',1,tkClass,TTabList,__TTabListCreate,true,0,'ATabSet: TTabSet');
    DefineMethod('Insert',2,tkNone,nil,__TTabListInsert,false,0,'Index: Integer; S: string');
    DefineMethod('Delete',1,tkNone,nil,__TTabListDelete,false,0,'Index: Integer');
    DefineMethod('Add',1,tkInteger,nil,__TTabListAdd,false,0,'S: string');
    DefineMethod('Put',2,tkNone,nil,__TTabListPut,false,0,'Index: Integer; S: string');
    DefineMethod('Clear',0,tkNone,nil,__TTabListClear,false,0,'');
    DefineMethod('AddStrings',1,tkNone,nil,__TTabListAddStrings,false,0,'Strings: TStrings');
  end;
  With Scripter.DefineClass(TTabSet) do
  begin
    DefineMethod('Create',1,tkClass,TTabSet,__TTabSetCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TTabSetDestroy,false,0,'');
    DefineMethod('ItemAtPos',2,tkInteger,nil,__TTabSetItemAtPos,false,1,'Pos: TPoint; IgnoreTabHeight: Boolean = False');
    DefineMethod('ItemRect',1,tkVariant,nil,__TTabSetItemRect,false,0,'Index: Integer');
    DefineMethod('ItemWidth',1,tkInteger,nil,__TTabSetItemWidth,false,0,'Index: Integer');
    DefineMethod('MinClientRect',0,tkVariant,nil,__TTabSetMinClientRect,false,0,'');
    DefineMethod('SelectNext',1,tkNone,nil,__TTabSetSelectNext,false,0,'Direction: Boolean');
  ...
  begin
    ...
    AddConstant('soLeftRight',soLeftRight);
    AddConstant('soUpDown',soUpDown);
    ...
    AddConstant('tsSoftTabs',tsSoftTabs);
    AddConstant('tsModernTabs',tsModernTabs);
    AddConstant('tsModernPopout',tsModernPopout);
  end;
end;

...
begin
  with AMachine do
  begin
AResult := Integer(TCustomMemo(CurrentObject).GetControlsAlignment);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TButtonClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TButton,__TButtonCreate,true,0,'AOwner: TComponent');

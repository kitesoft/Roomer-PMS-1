...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TAutoDispatchClass(CurrentClass.ClassRef).Create(TAutoObject(VarToObject(GetInputArg(0)))));
...
AResult := Integer(TAutoDispatch(CurrentObject).QueryInterface(TGUIDWrapper(integer(GetInputArg(0))).ObjToRec,Param1));
    ReturnOutputArg(AResult);
...
AResult := Integer(TAutoDispatch(CurrentObject).GetTypeInfoCount(Param0));
    ReturnOutputArg(AResult);
...
AResult := Integer(TAutoDispatch(CurrentObject).GetTypeInfo(VarToInteger(GetInputArg(0)),VarToInteger(GetInputArg(1)),Param2));
    ReturnOutputArg(AResult);
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TAutoDispatch(CurrentObject).GetAutoObject);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TAutoDispatch(CurrentObject).AutoObject));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TAutoObject(CurrentObject).AutoDispatch));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TRegistryClassClass(CurrentClass.ClassRef).Create(TAutoClassInfoWrapper(integer(GetInputArg(0))).ObjToRec));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TAutomationClass(CurrentClass.ClassRef).Create);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TAutomation(CurrentObject).StartMode));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(EOleSysErrorClass(CurrentClass.ClassRef).Create(VarToInteger(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(EOleExceptionClass(CurrentClass.ClassRef).Create(tagEXCEPINFOWrapper(integer(GetInputArg(0))).ObjToRec));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := OleAuto.CreateOleObject(VarToStr(GetInputArg(0)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := OleAuto.GetActiveOleObject(VarToStr(GetInputArg(0)));
    ReturnOutputArg(AResult);
  end;
end;
...
AResult := Integer(OleAuto.DllGetClassObject(TGUIDWrapper(integer(GetInputArg(0))).ObjToRec,TGUIDWrapper(integer(GetInputArg(1))).ObjToRec,Param2));
    ReturnOutputArg(AResult);
...
begin
  with AMachine do
  begin
AResult := Integer(OleAuto.DllCanUnloadNow);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(OleAuto.DllRegisterServer);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(OleAuto.DllUnregisterServer);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := OleAuto.VarFromInterface(IUnknown(VarToObject(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(OleAuto.VarToInterface(GetInputArg(0)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(OleAuto.VarToAutoObject(GetInputArg(0)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    OleAuto.DispInvokeError(VarToInteger(GetInputArg(0)),tagEXCEPINFOWrapper(integer(GetInputArg(1))).ObjToRec);
  end;
end;
...
begin
  with AMachine do
  begin
    OleAuto.OleError(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    OleAuto.OleCheck(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := integer(TGUIDWrapper.Create(OleAuto.StringToClassID(VarToStr(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := string(OleAuto.ClassIDToString(TGUIDWrapper(integer(GetInputArg(0))).ObjToRec));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := integer(TGUIDWrapper.Create(OleAuto.ProgIDToClassID(VarToStr(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := string(OleAuto.ClassIDToProgID(TGUIDWrapper(integer(GetInputArg(0))).ObjToRec));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(OleAuto.Automation));
  end;
end;
...
begin
  with AMachine do
  begin
    OleAuto.Automation:=TAutomation(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TAutoDispatch,__TAutoDispatchCreate,true,0,'AutoObject: TAutoObject');
    DefineMethod('QueryInterface',2,tkInteger,nil,__TAutoDispatchQueryInterface,false,0,'iid: TIID; obj').SetVarArgs([1]);
    ...
    DefineMethod('GetTypeInfoCount',1,tkInteger,nil,__TAutoDispatchGetTypeInfoCount,false,0,'ctinfo: Integer').SetVarArgs([0]);
    DefineMethod('GetTypeInfo',3,tkInteger,nil,__TAutoDispatchGetTypeInfo,false,0,'itinfo: Integer; lcid: TLCID; tinfo: ITypeInfo').SetVarArgs([2]);
  ...
  begin
    DefineMethod('Create',1,tkClass,TRegistryClass,__TRegistryClassCreate,true,0,'AutoClassInfo: TAutoClassInfo');
    DefineMethod('Destroy',0,tkNone,nil,__TRegistryClassDestroy,false,0,'');
    DefineMethod('UpdateRegistry',1,tkNone,nil,__TRegistryClassUpdateRegistry,false,0,'Register: Boolean');
  end;
  ...
  begin
    ...
    DefineMethod('RegisterClass',1,tkNone,nil,__TAutomationRegisterClass,false,0,'AutoClassInfo: TAutoClassInfo');
    DefineMethod('UpdateRegistry',1,tkNone,nil,__TAutomationUpdateRegistry,false,0,'Register: Boolean');
  ...
  begin
    DefineMethod('Create',1,tkClass,EOleSysError,__EOleSysErrorCreate,true,0,'ErrorCode: Integer');
    DefineProp('ErrorCode',tkInteger,__GetEOleSysErrorErrorCode,nil,nil,false,0);
  end;
  With Scripter.DefineClass(EOleException) do
  begin
    DefineMethod('Create',1,tkClass,EOleException,__EOleExceptionCreate,true,0,'ExcepInfo: TExcepInfo');
  ...
  begin
    DefineMethod('CreateOleObject',1,tkVariant,nil,__CreateOleObject,false,0,'ClassName: string');
    DefineMethod('GetActiveOleObject',1,tkVariant,nil,__GetActiveOleObject,false,0,'ClassName: string');
    DefineMethod('DllGetClassObject',3,tkInteger,nil,__DllGetClassObject,false,0,'CLSID: TCLSID; IID: TIID; Obj').SetVarArgs([2]);
    DefineMethod('DllCanUnloadNow',0,tkInteger,nil,__DllCanUnloadNow,false,0,'');
    DefineMethod('DllRegisterServer',0,tkInteger,nil,__DllRegisterServer,false,0,'');
    DefineMethod('DllUnregisterServer',0,tkInteger,nil,__DllUnregisterServer,false,0,'');
    DefineMethod('VarFromInterface',1,tkVariant,nil,__VarFromInterface,false,0,'Unknown: IUnknown');
    DefineMethod('VarToInterface',1,tkClass,IDispatch,__VarToInterface,false,0,'V: Variant');
    DefineMethod('VarToAutoObject',1,tkClass,TAutoObject,__VarToAutoObject,false,0,'V: Variant');
    DefineMethod('DispInvokeError',2,tkNone,nil,__DispInvokeError,false,0,'Status: HResult; ExcepInfo: TExcepInfo');
    ...
    DefineMethod('StringToClassID',1,tkVariant,nil,__StringToClassID,false,0,'S: string');
    DefineMethod('ClassIDToString',1,tkVariant,nil,__ClassIDToString,false,0,'ClassID: TCLSID');
    DefineMethod('ProgIDToClassID',1,tkVariant,nil,__ProgIDToClassID,false,0,'ProgID: string');
    DefineMethod('ClassIDToProgID',1,tkVariant,nil,__ClassIDToProgID,false,0,'ClassID: TCLSID');

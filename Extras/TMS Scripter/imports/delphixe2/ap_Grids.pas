...
type
  ...
  private
    ...
    procedure __TFixedCellClickEvent( Sender: TObject;  ACol: Longint;  ARow: Longint);
  ...
  THotTrackCellInfoWrapper = class(TatRecordWrapper)
  private
    FPressed: Boolean;
    FButton: TMouseButton;
  public
    constructor Create(ARecord: THotTrackCellInfo);
    function ObjToRec: THotTrackCellInfo;
  published
...
constructor THotTrackCellInfoWrapper.Create(ARecord: THotTrackCellInfo);
begin
  inherited Create;
  FPressed := ARecord.Pressed;
  FButton := ARecord.Button;
end;

function THotTrackCellInfoWrapper.ObjToRec: THotTrackCellInfo;
begin
  result.Pressed := FPressed;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TInplaceEditClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomGridClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TStringGridStringsClass(CurrentClass.ClassRef).Create(TStringGrid(VarToObject(GetInputArg(0))),VarToInteger(GetInputArg(1))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TStringGridStrings(CurrentObject).Add(VarToStr(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TStringGridStrings(CurrentObject).Assign(TPersistent(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    TStringGridStrings(CurrentObject).Insert(VarToInteger(GetInputArg(0)),VarToStr(GetInputArg(1)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TStringGridClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TStringGrid(CurrentObject).Cells[VarToInteger(GetArrayIndex(0)),VarToInteger(GetArrayIndex(1))]));
  end;
end;
...
begin
  with AMachine do
  begin
    TStringGrid(CurrentObject).Cells[VarToInteger(GetArrayIndex(0)),VarToInteger(GetArrayIndex(1))]:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TStringGrid(CurrentObject).Cols[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    TStringGrid(CurrentObject).Cols[VarToInteger(GetArrayIndex(0))]:=TStrings(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TStringGrid(CurrentObject).Objects[VarToInteger(GetArrayIndex(0)),VarToInteger(GetArrayIndex(1))]));
  end;
end;
...
begin
  with AMachine do
  begin
    TStringGrid(CurrentObject).Objects[VarToInteger(GetArrayIndex(0)),VarToInteger(GetArrayIndex(1))]:=TObject(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TStringGrid(CurrentObject).Rows[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    TStringGrid(CurrentObject).Rows[VarToInteger(GetArrayIndex(0))]:=TStrings(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TInplaceEditListClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TInplaceEditList(CurrentObject).ActiveList));
  end;
end;
...
begin
  with AMachine do
  begin
    TInplaceEditList(CurrentObject).ActiveList:=TWinControl(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TInplaceEditList(CurrentObject).EditStyle));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TInplaceEditList(CurrentObject).PickList));
  end;
end;
...
procedure TatGridsDispatcher.__TFixedCellClickEvent( Sender: TObject;  ACol: Longint;  ARow: Longint);
begin
  if DoOnExecuteEvent then
  begin
    if AssignedMethod(BeforeCall) then
      TFixedCellClickEvent(BeforeCall)(Sender,ACol,ARow);
    if Assigned(Scripter) and (RoutineName > '') then
      Scripter.ExecuteSubroutine(RoutineName, [Sender,ACol,ARow]);
    if AssignedMethod(AfterCall) then
      TFixedCellClickEvent(AfterCall)(Sender,ACol,ARow);
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TInplaceEdit,__TInplaceEditCreate,true,0,'AOwner: TComponent');
    ...
    DefineMethod('Move',1,tkNone,nil,__TInplaceEditMove,false,0,'Loc: TRect');
    DefineMethod('PosEqual',1,tkVariant,nil,__TInplaceEditPosEqual,false,0,'Rect: TRect');
    DefineMethod('SetFocus',0,tkNone,nil,__TInplaceEditSetFocus,false,0,'');
    DefineMethod('UpdateLoc',1,tkNone,nil,__TInplaceEditUpdateLoc,false,0,'Loc: TRect');
    DefineMethod('Visible',0,tkVariant,nil,__TInplaceEditVisible,false,0,'');
  end;
  With Scripter.DefineClass(TCustomGrid) do
  begin
    DefineMethod('Create',1,tkClass,TCustomGrid,__TCustomGridCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TCustomGridDestroy,false,0,'');
    DefineMethod('MouseCoord',2,tkVariant,nil,__TCustomGridMouseCoord,false,0,'X: Integer; Y: Integer');
  end;
  With Scripter.DefineClass(TCustomDrawGrid) do
  begin
    DefineMethod('CellRect',2,tkVariant,nil,__TCustomDrawGridCellRect,false,0,'ACol: Longint; ARow: Longint');
    DefineMethod('MouseToCell',4,tkNone,nil,__TCustomDrawGridMouseToCell,false,0,'X: Integer; Y: Integer; ACol: Longint; ARow: Longint').SetVarArgs([2,3]);
    ...
    DefineProp('DrawingStyle',tkVariant,nil,nil,nil,false,0);
  ...
  begin
    DefineMethod('Create',2,tkClass,TStringGridStrings,__TStringGridStringsCreate,true,0,'AGrid: TStringGrid; AIndex: Longint');
    DefineMethod('Add',1,tkInteger,nil,__TStringGridStringsAdd,false,0,'S: string');
    DefineMethod('Assign',1,tkNone,nil,__TStringGridStringsAssign,false,0,'Source: TPersistent');
    DefineMethod('Clear',0,tkNone,nil,__TStringGridStringsClear,false,0,'');
    DefineMethod('Delete',1,tkNone,nil,__TStringGridStringsDelete,false,0,'Index: Integer');
    DefineMethod('Insert',2,tkNone,nil,__TStringGridStringsInsert,false,0,'Index: Integer; S: string');
  end;
  With Scripter.DefineClass(TStringGrid) do
  begin
    DefineMethod('Create',1,tkClass,TStringGrid,__TStringGridCreate,true,0,'AOwner: TComponent');
  ...
  begin
    DefineMethod('Create',1,tkClass,TInplaceEditList,__TInplaceEditListCreate,true,0,'Owner: TComponent');
  ...
  DefineEventAdapter(TypeInfo(TFixedCellClickEvent), TatGridsDispatcher, @TatGridsDispatcher.__TFixedCellClickEvent);
  ...
  begin
    ...
    AddConstant('goFixedColClick',goFixedColClick);
    AddConstant('goFixedRowClick',goFixedRowClick);
    AddConstant('goFixedHotTrack',goFixedHotTrack);
    AddConstant('esSimple',esSimple);
    AddConstant('esEllipsis',esEllipsis);
    AddConstant('esPickList',esPickList);
    AddConstant('gdsClassic',gdsClassic);
    AddConstant('gdsThemed',gdsThemed);
    AddConstant('gdsGradient',gdsGradient);

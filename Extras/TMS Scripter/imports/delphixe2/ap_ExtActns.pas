...
type
  TatExtActnsLibrary = class(TatScripterLibrary)
    ...
    procedure __TRecipientCollectionCreate(AMachine: TatVirtualMachine);
    procedure __GetTRecipientCollectionRecipients(AMachine: TatVirtualMachine);
    procedure __SetTRecipientCollectionRecipients(AMachine: TatVirtualMachine);
    procedure __TSendMailCreate(AMachine: TatVirtualMachine);
  ...
  TRecipientItemClass = class of TRecipientItem;
  TRecipientCollectionClass = class of TRecipientCollection;
  ...
  private
    ...
    procedure __TMailSendStatusEvent( Sender: TObject; const Status: TMailSendStatus);
  end;
...
procedure TatExtActnsLibrary.__TRecipientCollectionCreate(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := ObjectToVar(TRecipientCollectionClass(CurrentClass.ClassRef).Create(TSendMail(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;

procedure TatExtActnsLibrary.__GetTRecipientCollectionRecipients(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TRecipientCollection(CurrentObject).Recipients[VarToInteger(GetArrayIndex(0))]));
  end;
end;

procedure TatExtActnsLibrary.__SetTRecipientCollectionRecipients(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    TRecipientCollection(CurrentObject).Recipients[VarToInteger(GetArrayIndex(0))]:=TRecipientItem(VarToObject(GetInputArg(0)));
  end;
end;

procedure TatExtActnsLibrary.__TSendMailCreate(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := ObjectToVar(TSendMailClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
procedure TatExtActnsDispatcher.__TMailSendStatusEvent( Sender: TObject; const Status: TMailSendStatus);
var
  StatusTemp: variant;
begin
  if DoOnExecuteEvent then
  begin
    if AssignedMethod(BeforeCall) then
      TMailSendStatusEvent(BeforeCall)(Sender,Status);
    StatusTemp := Integer(Status);
    if Assigned(Scripter) and (RoutineName > '') then
      Scripter.ExecuteSubroutine(RoutineName, [Sender,StatusTemp]);
    if AssignedMethod(AfterCall) then
      TMailSendStatusEvent(AfterCall)(Sender,Status);
  end;
end;
...
begin
  ...
  With Scripter.DefineClass(TRecipientItem) do
  begin
  end;
  With Scripter.DefineClass(TRecipientCollection) do
  begin
    DefineMethod('Create',1,tkClass,TRecipientCollection,__TRecipientCollectionCreate,true,0,'Action: TSendMail');
    DefaultProperty := DefineProp('Recipients',tkClass,__GetTRecipientCollectionRecipients,__SetTRecipientCollectionRecipients,TRecipientItem,false,1);
  end;
  With Scripter.DefineClass(TSendMail) do
  begin
    DefineMethod('Create',1,tkClass,TSendMail,__TSendMailCreate,true,0,'AOwner: TComponent');
  ...
  DefineEventAdapter(TypeInfo(TMailSendStatusEvent), TatExtActnsDispatcher, @TatExtActnsDispatcher.__TMailSendStatusEvent);
  ...
  begin
    ...
    AddConstant('rtPrimary',rtPrimary);
    AddConstant('rtCC',rtCC);
    AddConstant('rtBCC',rtBCC);
    AddConstant('mssSuccess',mssSuccess);
    AddConstant('mssAmbiguousRecipient',mssAmbiguousRecipient);
    AddConstant('mssAttachmentNotFound',mssAttachmentNotFound);
    AddConstant('mssAttachmentOpenFailure',mssAttachmentOpenFailure);
    AddConstant('mssBadRecipType',mssBadRecipType);
    AddConstant('mssFailure',mssFailure);
    AddConstant('mssInsufficientMemory',mssInsufficientMemory);
    AddConstant('mssInvalidRecips',mssInvalidRecips);
    AddConstant('mssLoginFailure',mssLoginFailure);
    AddConstant('mssTextTooLarge',mssTextTooLarge);
    AddConstant('mssTooManyFiles',mssTooManyFiles);
    AddConstant('mssTooManyRecipients',mssTooManyRecipients);
    AddConstant('mssUnknownRecipient',mssUnknownRecipient);
    AddConstant('mssUserAbort',mssUserAbort);
    AddConstant('mssStatusCodeError',mssStatusCodeError);
  end;
end;

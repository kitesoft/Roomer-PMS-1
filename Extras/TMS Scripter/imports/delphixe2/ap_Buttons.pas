...
type
  TatButtonsLibrary = class(TatScripterLibrary)
    procedure __TSpeedButtonActionLinkCreate(AMachine: TatVirtualMachine);
    ...
    procedure __TBitBtnActionLinkCreate(AMachine: TatVirtualMachine);
  ...
  TBitBtnActionLinkClass = class of TBitBtnActionLink;
...
procedure TatButtonsLibrary.__TSpeedButtonActionLinkCreate(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := ObjectToVar(TSpeedButtonActionLinkClass(CurrentClass.ClassRef).Create(TObject(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TSpeedButtonClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
procedure TatButtonsLibrary.__TBitBtnActionLinkCreate(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := ObjectToVar(TBitBtnActionLinkClass(CurrentClass.ClassRef).Create(TObject(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TBitBtnClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := integer(TRectWrapper.Create(Buttons.DrawButtonFace(TCanvas(VarToObject(GetInputArg(0))),TRectWrapper(integer(GetInputArg(1))).ObjToRec,VarToInteger(GetInputArg(2)),GetInputArg(3),GetInputArg(4),GetInputArg(5),GetInputArg(6))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TSpeedButtonActionLink,__TSpeedButtonActionLinkCreate,true,0,'AClient: TObject');
  end;
  With Scripter.DefineClass(TSpeedButton) do
  begin
    DefineMethod('Create',1,tkClass,TSpeedButton,__TSpeedButtonCreate,true,0,'AOwner: TComponent');
  ...
  With Scripter.DefineClass(TBitBtnActionLink) do
  begin
    DefineMethod('Create',1,tkClass,TBitBtnActionLink,__TBitBtnActionLinkCreate,true,0,'AClient: TObject');
  end;
  With Scripter.DefineClass(TBitBtn) do
  begin
    DefineMethod('Create',1,tkClass,TBitBtn,__TBitBtnCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TBitBtnDestroy,false,0,'');
    DefineMethod('Click',0,tkNone,nil,__TBitBtnClick,false,0,'');
  end;
  With Scripter.DefineClass(ClassType) do
  begin
    DefineMethod('DrawButtonFace',7,tkVariant,nil,__DrawButtonFace,false,0,'Canvas: TCanvas; Client: TRect; BevelWidth: Integer; Style: TButtonStyle; IsRounded: Boolean; IsDown: Boolean; IsFocused: Boolean');

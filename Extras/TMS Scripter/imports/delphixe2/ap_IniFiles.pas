...
type
  TatIniFilesLibrary = class(TatScripterLibrary)
    ...
    procedure __TCustomIniFileReadSubSections(AMachine: TatVirtualMachine);
...
procedure TatIniFilesLibrary.__TCustomIniFileReadSubSections(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    Case InputArgCount of
      2: TCustomIniFile(CurrentObject).ReadSubSections(VarToStr(GetInputArg(0)),TStrings(VarToObject(GetInputArg(1))));
      3: TCustomIniFile(CurrentObject).ReadSubSections(VarToStr(GetInputArg(0)),TStrings(VarToObject(GetInputArg(1))),GetInputArg(2));
    end;
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TMemIniFile(CurrentObject).Encoding));
  end;
end;
...
begin
  with AMachine do
  begin
    TMemIniFile(CurrentObject).Encoding:=TEncoding(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  ...
  begin
    ...
    DefineMethod('ReadSubSections',3,tkNone,nil,__TCustomIniFileReadSubSections,false,1,'Section: string; Strings: TStrings; Recurse: Boolean = False');

...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TDataBlockClass(CurrentClass.ClassRef).Create);
...
begin
  with AMachine do
  begin
AResult := TDataBlockInterpreter(CurrentObject).CallCreateObject(VarToStr(GetInputArg(0)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TDataDispatchClass(CurrentClass.ClassRef).Create(TCustomDataBlockInterpreter(VarToObject(GetInputArg(0))),VarToInteger(GetInputArg(1))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TTransportThread(CurrentObject).Semaphore));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TStreamedConnectionClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TSocketTransportClass(CurrentClass.ClassRef).Create);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TSocketTransport(CurrentObject).Host));
  end;
end;
...
begin
  with AMachine do
  begin
    TSocketTransport(CurrentObject).Host:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TSocketTransport(CurrentObject).Address));
  end;
end;
...
begin
  with AMachine do
  begin
    TSocketTransport(CurrentObject).Address:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TSocketTransport(CurrentObject).Socket));
  end;
end;
...
begin
  with AMachine do
  begin
    TSocketTransport(CurrentObject).Socket:=TCustomWinSocket(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TSocketTransport(CurrentObject).InterceptGUID));
  end;
end;
...
begin
  with AMachine do
  begin
    TSocketTransport(CurrentObject).InterceptGUID:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TSocketConnectionClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TWebConnectionClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    SConnect.GetPacketInterceptorList(TStringList(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  ...
  begin
    ...
    DefineMethod('CallCreateObject',1,tkVariant,nil,__TDataBlockInterpreterCallCreateObject,false,0,'Name: string');
  end;
  With Scripter.DefineClass(TDataDispatch) do
  begin
    DefineMethod('Create',2,tkClass,TDataDispatch,__TDataDispatchCreate,true,0,'Interpreter: TCustomDataBlockInterpreter; DispatchIndex: Integer');
  ...
  begin
    ...
    DefineProp('Semaphore',tkInteger,__GetTTransportThreadSemaphore,nil,nil,false,0);
  end;
  ...
  begin
    ...
    DefineMethod('Create',1,tkClass,TStreamedConnection,__TStreamedConnectionCreate,true,0,'AOwner: TComponent');
  ...
  begin
    DefineMethod('Create',1,tkClass,TSocketConnection,__TSocketConnectionCreate,true,0,'AOwner: TComponent');
  end;
  With Scripter.DefineClass(TWebConnection) do
  begin
    DefineMethod('Create',1,tkClass,TWebConnection,__TWebConnectionCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TWebConnectionDestroy,false,0,'');
  end;
  With Scripter.DefineClass(TPacketInterceptFactory) do
  begin
    DefineMethod('UpdateRegistry',1,tkNone,nil,__TPacketInterceptFactoryUpdateRegistry,false,0,'Register: Boolean');
  end;
  With Scripter.DefineClass(ClassType) do
  begin
    DefineMethod('LoadWinSock2',0,tkVariant,nil,__LoadWinSock2,false,0,'');
    DefineMethod('GetPacketInterceptorList',1,tkNone,nil,__GetPacketInterceptorList,false,0,'List: TStringList');

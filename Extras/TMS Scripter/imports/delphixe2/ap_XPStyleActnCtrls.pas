...
begin
  with AMachine do
  begin
AResult := string(TXPStyleActionBars(CurrentObject).GetStyleName);
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(XPStyleActnCtrls.XPStyle));
  end;
end;
...
begin
  with AMachine do
  begin
    XPStyleActnCtrls.XPStyle:=TXPStyleActionBars(VarToObject(GetInputArg(0)));
  end;
end;

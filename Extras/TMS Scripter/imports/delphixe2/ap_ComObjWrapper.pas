...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TComComponentClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TComComponent(CurrentObject).Description));
  end;
end;
...
begin
  with AMachine do
  begin
    TComComponent(CurrentObject).Description:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TComComponent(CurrentObject).GUID));
  end;
end;
...
begin
  with AMachine do
  begin
    TComComponent(CurrentObject).GUID:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TComComponent,__TComComponentCreate,true,0,'AOwner: TComponent');

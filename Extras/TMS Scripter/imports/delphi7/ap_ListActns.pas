...
type
  TatListActnsLibrary = class(TatScripterLibrary)
    ...
    procedure __TStaticListActionCreate(AMachine: TatVirtualMachine);
    procedure __TStaticListActionDestroy(AMachine: TatVirtualMachine);
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TListControlItemClass(CurrentClass.ClassRef).Create(TCollection(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
    TListControlItem(CurrentObject).Assign(TPersistent(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TListControlItems(CurrentObject).Add);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TListControlItems(CurrentObject).Items[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomListActionClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomListAction(CurrentObject).ExecuteTarget(TObject(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := TCustomListAction(CurrentObject).HandlesTarget(TObject(VarToObject(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TCustomListAction(CurrentObject).Strings[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomListAction(CurrentObject).Strings[VarToInteger(GetArrayIndex(0))]:=VarToStr(GetInputArg(0));
  end;
end;

procedure TatListActnsLibrary.__TStaticListActionCreate(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := ObjectToVar(TStaticListActionClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;

procedure TatListActnsLibrary.__TStaticListActionDestroy(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    TStaticListAction(CurrentObject).Destroy;
  end;
end;
...
begin
  ...
  begin
    ...
    ItemTemp := ObjectToVar(Item);
    if Assigned(Scripter) and (RoutineName > '') then
      Scripter.ExecuteSubroutine(RoutineName, [Sender,Index,ItemTemp]);
    Item := TListControlItem(VarToObject(ItemTemp));
    if AssignedMethod(AfterCall) then
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TListControlItem,__TListControlItemCreate,true,0,'Collection: TCollection');
    DefineMethod('Assign',1,tkNone,nil,__TListControlItemAssign,false,0,'Source: TPersistent');
  end;
  ...
  begin
    DefineMethod('Create',1,tkClass,TCustomListAction,__TCustomListActionCreate,true,0,'AOwner: TComponent');
    DefineMethod('ExecuteTarget',1,tkNone,nil,__TCustomListActionExecuteTarget,false,0,'Target: TObject');
    DefineMethod('HandlesTarget',1,tkVariant,nil,__TCustomListActionHandlesTarget,false,0,'Target: TObject');
  ...
  begin
    DefineMethod('Create',1,tkClass,TStaticListAction,__TStaticListActionCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TStaticListActionDestroy,false,0,'');
    DefineProp('Count',tkVariant,nil,nil,nil,false,0);
  end;

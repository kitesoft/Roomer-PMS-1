...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomCachedDataSetClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  ...
  begin
    Case InputArgCount of
      ...
      1: TCustomCachedDataSet(CurrentObject).LoadFromFile(VarToStr(GetInputArg(0)));
    end;
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TCustomCachedDataSet,__TCustomCachedDataSetCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TCustomCachedDataSetDestroy,false,0,'');
    DefineMethod('LoadFromFile',1,tkNone,nil,__TCustomCachedDataSetLoadFromFile,false,1,'AFileName: string = ''');
  end;

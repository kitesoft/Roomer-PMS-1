...
begin
  with AMachine do
  begin
AResult := TIBSchema(CurrentObject).Has_DEFAULT_VALUE(VarToStr(GetInputArg(0)),VarToStr(GetInputArg(1)));
...
begin
  with AMachine do
  begin
AResult := TIBSchema(CurrentObject).Has_COMPUTED_BLR(VarToStr(GetInputArg(0)),VarToStr(GetInputArg(1)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBDatabaseClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TIBDatabase(CurrentObject).GetFieldNames(VarToStr(GetInputArg(0)),TStrings(VarToObject(GetInputArg(1))));
  end;
end;
...
begin
  ...
  begin
    Case InputArgCount of
      1: TIBDatabase(CurrentObject).GetTableNames(TStrings(VarToObject(GetInputArg(0))));
      2: TIBDatabase(CurrentObject).GetTableNames(TStrings(VarToObject(GetInputArg(0))),GetInputArg(1));
    end;
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TIBDatabase(CurrentObject).IndexOfDBConst(VarToStr(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TIBDatabase(CurrentObject).AddTransaction(TIBTransaction(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TIBDatabase(CurrentObject).FindTransaction(TIBTransaction(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBDatabase(CurrentObject).FindDefaultTransaction);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := TIBDatabase(CurrentObject).Has_DEFAULT_VALUE(VarToStr(GetInputArg(0)),VarToStr(GetInputArg(1)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := TIBDatabase(CurrentObject).Has_COMPUTED_BLR(VarToStr(GetInputArg(0)),VarToStr(GetInputArg(1)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TIBDatabase(CurrentObject).DBParamByDPB[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    TIBDatabase(CurrentObject).DBParamByDPB[VarToInteger(GetArrayIndex(0))]:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBDatabase(CurrentObject).SQLObjects[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBDatabase(CurrentObject).Transactions[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBDatabase(CurrentObject).InternalTransaction));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBTransactionClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TIBTransaction(CurrentObject).AddDatabase(TIBDatabase(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TIBTransaction(CurrentObject).FindDatabase(TIBDatabase(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBTransaction(CurrentObject).FindDefaultDatabase);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBTransaction(CurrentObject).Databases[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBTransaction(CurrentObject).SQLObjects[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBBaseClass(CurrentClass.ClassRef).Create(TObject(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBBase(CurrentObject).Database));
  end;
end;
...
begin
  with AMachine do
  begin
    TIBBase(CurrentObject).Database:=TIBDatabase(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBBase(CurrentObject).Owner));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBBase(CurrentObject).Transaction));
  end;
end;
...
begin
  with AMachine do
  begin
    TIBBase(CurrentObject).Transaction:=TIBTransaction(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBTimerClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
Param2 := VarToInteger(GetInputArg(2));
    IBDatabase.GenerateDPB(TStrings(VarToObject(GetInputArg(0))),Param1,Param2);
...
Param1 := VarToStr(GetInputArg(1));
Param2 := VarToInteger(GetInputArg(2));
    IBDatabase.GenerateTPB(TStrings(VarToObject(GetInputArg(0))),Param1,Param2);
    SetInputArg(1,string(Param1));
    SetInputArg(2,Integer(Param2));
  end;
end;
...
begin
  ...
  begin
    ...
    DefineMethod('Has_DEFAULT_VALUE',2,tkVariant,nil,__TIBSchemaHas_DEFAULT_VALUE,false,0,'Relation: String; Field: String');
    DefineMethod('Has_COMPUTED_BLR',2,tkVariant,nil,__TIBSchemaHas_COMPUTED_BLR,false,0,'Relation: String; Field: String');
  end;
  With Scripter.DefineClass(TIBDatabase) do
  begin
    DefineMethod('Create',1,tkClass,TIBDatabase,__TIBDatabaseCreate,true,0,'AOwner: TComponent');
    ...
    DefineMethod('GetFieldNames',2,tkNone,nil,__TIBDatabaseGetFieldNames,false,0,'TableName: string; List: TStrings');
    DefineMethod('GetTableNames',2,tkNone,nil,__TIBDatabaseGetTableNames,false,1,'List: TStrings; SystemTables: Boolean = False');
    DefineMethod('IndexOfDBConst',1,tkInteger,nil,__TIBDatabaseIndexOfDBConst,false,0,'st: String');
    ...
    DefineMethod('Call',2,tkInteger,nil,__TIBDatabaseCall,false,0,'ErrCode: ISC_STATUS; RaiseError: Boolean');
    DefineMethod('AddTransaction',1,tkInteger,nil,__TIBDatabaseAddTransaction,false,0,'TR: TIBTransaction');
    DefineMethod('FindTransaction',1,tkInteger,nil,__TIBDatabaseFindTransaction,false,0,'TR: TIBTransaction');
    DefineMethod('FindDefaultTransaction',0,tkClass,TIBTransaction,__TIBDatabaseFindDefaultTransaction,false,0,'');
    DefineMethod('RemoveTransaction',1,tkNone,nil,__TIBDatabaseRemoveTransaction,false,0,'Idx: Integer');
    DefineMethod('RemoveTransactions',0,tkNone,nil,__TIBDatabaseRemoveTransactions,false,0,'');
    DefineMethod('Has_DEFAULT_VALUE',2,tkVariant,nil,__TIBDatabaseHas_DEFAULT_VALUE,false,0,'Relation: String; Field: String');
    DefineMethod('Has_COMPUTED_BLR',2,tkVariant,nil,__TIBDatabaseHas_COMPUTED_BLR,false,0,'Relation: String; Field: String');
  ...
  begin
    DefineMethod('Create',1,tkClass,TIBTransaction,__TIBTransactionCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TIBTransactionDestroy,false,0,'');
    DefineMethod('Call',2,tkInteger,nil,__TIBTransactionCall,false,0,'ErrCode: ISC_STATUS; RaiseError: Boolean');
    ...
    DefineMethod('AddDatabase',1,tkInteger,nil,__TIBTransactionAddDatabase,false,0,'db: TIBDatabase');
    DefineMethod('FindDatabase',1,tkInteger,nil,__TIBTransactionFindDatabase,false,0,'db: TIBDatabase');
    DefineMethod('FindDefaultDatabase',0,tkClass,TIBDatabase,__TIBTransactionFindDefaultDatabase,false,0,'');
    DefineMethod('RemoveDatabase',1,tkNone,nil,__TIBTransactionRemoveDatabase,false,0,'Idx: Integer');
  ...
  begin
    DefineMethod('Create',1,tkClass,TIBBase,__TIBBaseCreate,true,0,'AOwner: TObject');
  ...
  begin
    DefineMethod('Create',1,tkClass,TIBTimer,__TIBTimerCreate,true,0,'AOwner: TComponent');
  ...
  begin
    DefineMethod('GenerateDPB',3,tkNone,nil,__GenerateDPB,false,0,'sl: TStrings; DPB: string; DPBLength: Short').SetVarArgs([1,2]);
    DefineMethod('GenerateTPB',3,tkNone,nil,__GenerateTPB,false,0,'sl: TStrings; TPB: string; TPBLength: Short').SetVarArgs([1,2]);

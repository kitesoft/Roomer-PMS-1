...
begin
  with AMachine do
  begin
AResult := ObjectToVar(EIBErrorClass(CurrentClass.ClassRef).Create(VarToInteger(GetInputArg(0)),VarToStr(GetInputArg(1))));
...
begin
  with AMachine do
  begin
AResult := string(IB.StatusVectorAsText);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',2,tkClass,EIBError,__EIBErrorCreate,true,0,'ASQLCode: Long; Msg: string');
  ...
  begin
    DefineMethod('IBAlloc',3,tkNone,nil,__IBAlloc,false,0,'P; OldSize: Integer; NewSize: Integer').SetVarArgs([0]);
    ...
    DefineMethod('SetIBDataBaseErrorMessages',1,tkNone,nil,__SetIBDataBaseErrorMessages,false,0,'Value: TIBDataBaseErrorMessages');

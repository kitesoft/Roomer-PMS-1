...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TLocalConnectionClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TLocalConnection(CurrentObject).Providers[VarToStr(GetArrayIndex(0))]));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TLocalConnection,__TLocalConnectionCreate,true,0,'AOwner: TComponent');

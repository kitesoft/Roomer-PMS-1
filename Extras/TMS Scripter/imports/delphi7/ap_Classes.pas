...
type
  TatClassesLibrary = class(TatScripterLibrary)
    ...
    procedure __TParserCreate(AMachine: TatVirtualMachine);
  ...
  TSynchronizeRecordWrapper = class(TatRecordWrapper)
  private
    FFThread: TObject;
    FFSynchronizeException: TObject;
  public
    constructor Create(ARecord: TSynchronizeRecord);
    function ObjToRec: TSynchronizeRecord;
  published
    property FThread: TObject read FFThread write FFThread;
    property FSynchronizeException: TObject read FFSynchronizeException write FFSynchronizeException;
  end;
...
constructor TSynchronizeRecordWrapper.Create(ARecord: TSynchronizeRecord);
begin
  inherited Create;
  FFThread := ARecord.FThread;
  FFSynchronizeException := ARecord.FSynchronizeException;
end;

function TSynchronizeRecordWrapper.ObjToRec: TSynchronizeRecord;
begin
  result.FThread := FFThread;
  result.FSynchronizeException := FFSynchronizeException;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TStringStreamClass(CurrentClass.ClassRef).Create(VarToStr(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TWriter(CurrentObject).WriteStr(VarToStr(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    TWriter(CurrentObject).WriteString(VarToStr(GetInputArg(0)));
  end;
end;
...
procedure TatClassesLibrary.__TParserCreate(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := ObjectToVar(TParserClass(CurrentClass.ClassRef).Create(TStream(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TThreadClass(CurrentClass.ClassRef).Create(GetInputArg(0)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    Classes.BinToHex(PChar(VarToStr(GetInputArg(0))),PChar(VarToStr(GetInputArg(1))),VarToInteger(GetInputArg(2)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(Classes.HexToBin(PChar(VarToStr(GetInputArg(0))),PChar(VarToStr(GetInputArg(1))),VarToInteger(GetInputArg(2))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',2,tkClass,TFileStream,__TFileStreamCreate,true,0,'FileName: string; Mode: Word');
  ...
  begin
    DefineMethod('Create',1,tkClass,TStringStream,__TStringStreamCreate,true,0,'AString: string');
    DefineMethod('Read',2,tkInteger,nil,__TStringStreamRead,false,0,'Buffer; Count: Longint').SetVarArgs([0]);
    DefineMethod('ReadString',1,tkVariant,nil,__TStringStreamReadString,false,0,'Count: Longint');
    DefineMethod('Seek',2,tkInteger,nil,__TStringStreamSeek,false,0,'Offset: Longint; Origin: Word');
    DefineMethod('Write',2,tkInteger,nil,__TStringStreamWrite,false,0,'Buffer; Count: Longint');
  ...
  begin
    ...
    DefineMethod('WriteStr',1,tkNone,nil,__TWriterWriteStr,false,0,'Value: string');
    DefineMethod('WriteString',1,tkNone,nil,__TWriterWriteString,false,0,'Value: string');
    DefineMethod('WriteWideString',1,tkNone,nil,__TWriterWriteWideString,false,0,'Value: WideString');
  ...
  begin
    DefineMethod('Create',1,tkClass,TParser,__TParserCreate,true,0,'Stream: TStream');
  ...
  begin
    DefineMethod('Create',1,tkClass,TThread,__TThreadCreate,true,0,'CreateSuspended: Boolean');
  ...
  begin
    ...
    DefineMethod('LineStart',2,tkVariant,nil,__LineStart,false,0,'Buffer: PChar; BufPos: PChar');
    ...
    DefineMethod('BinToHex',3,tkNone,nil,__BinToHex,false,0,'Buffer: PChar; Text: PChar; BufSize: Integer');
    DefineMethod('HexToBin',3,tkInteger,nil,__HexToBin,false,0,'Text: PChar; Buffer: PChar; BufSize: Integer');

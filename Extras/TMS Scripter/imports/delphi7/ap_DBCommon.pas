...
IntToSet(Param2, VarToInteger(GetInputArg(2)), SizeOf(Param2));
    TExprParser(CurrentObject).SetExprParams(VarToStr(GetInputArg(0)),Param1,Param2,VarToStr(GetInputArg(3)));
  end;
end;
...
AResult := DBCommon.GetFieldInfo(VarToStr(GetInputArg(0)),Param1);
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(DBCommon.GetIndexForOrderBy(VarToStr(GetInputArg(0)),TDataSet(VarToObject(GetInputArg(1)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := string(DBCommon.GetTableNameFromSQL(VarToStr(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := string(DBCommon.GetTableNameFromQuery(VarToStr(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
3: AResult := string(DBCommon.AddParamSQLForDetail(TParams(VarToObject(GetInputArg(0))),VarToStr(GetInputArg(1)),GetInputArg(2)));
4: AResult := string(DBCommon.AddParamSQLForDetail(TParams(VarToObject(GetInputArg(0))),VarToStr(GetInputArg(1)),GetInputArg(2),VarToStr(GetInputArg(3))));
end;
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := DBCommon.IsMultiTableQuery(VarToStr(GetInputArg(0)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    ...
    DefineMethod('SetExprParams',4,tkNone,nil,__TExprParserSetExprParams,false,0,'Text: string; Options: TFilterOptions; ParserOptions: TParserOptions; FieldName: string');
    DefineProp('DataSize',tkInteger,__GetTExprParserDataSize,nil,nil,false,0);
  end;
  With Scripter.DefineClass(ClassType) do
  begin
    DefineMethod('GetFieldInfo',2,tkVariant,nil,__GetFieldInfo,false,0,'Origin: string; FieldInfo: TFieldInfo').SetVarArgs([1]);
    DefineMethod('NextSQLToken',3,tkEnumeration,nil,__NextSQLToken,false,0,'p: PChar; Token: string; CurSection: TSQLToken').SetVarArgs([0,1]);
    DefineMethod('GetIndexForOrderBy',2,tkClass,TIndexDef,__GetIndexForOrderBy,false,0,'SQL: string; DataSet: TDataSet');
    DefineMethod('GetTableNameFromSQL',1,tkVariant,nil,__GetTableNameFromSQL,false,0,'SQL: string');
    DefineMethod('GetTableNameFromQuery',1,tkVariant,nil,__GetTableNameFromQuery,false,0,'SQL: string');
    DefineMethod('AddParamSQLForDetail',4,tkVariant,nil,__AddParamSQLForDetail,false,1,'Params: TParams; SQL: string; Native: Boolean; QuoteChar: string = ''');
    DefineMethod('IsMultiTableQuery',1,tkVariant,nil,__IsMultiTableQuery,false,0,'SQL: string');

...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBDatabaseInfoClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TIBDatabaseInfo(CurrentObject).DBFileName));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TIBDatabaseInfo(CurrentObject).DBSiteName));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TIBDatabaseInfo(CurrentObject).Version));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBDatabaseInfo(CurrentObject).UserNames));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBDatabaseInfo(CurrentObject).BackoutCount));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBDatabaseInfo(CurrentObject).DeleteCount));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBDatabaseInfo(CurrentObject).ExpungeCount));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBDatabaseInfo(CurrentObject).InsertCount));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBDatabaseInfo(CurrentObject).PurgeCount));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBDatabaseInfo(CurrentObject).ReadIdxCount));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBDatabaseInfo(CurrentObject).ReadSeqCount));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBDatabaseInfo(CurrentObject).UpdateCount));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TIBDatabaseInfo,__TIBDatabaseInfoCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TIBDatabaseInfoDestroy,false,0,'');
    DefineMethod('Call',2,tkInteger,nil,__TIBDatabaseInfoCall,false,0,'ErrCode: ISC_STATUS; RaiseError: Boolean');
    DefineMethod('GetLongDatabaseInfo',1,tkInteger,nil,__TIBDatabaseInfoGetLongDatabaseInfo,false,0,'DatabaseInfoCommand: Integer');

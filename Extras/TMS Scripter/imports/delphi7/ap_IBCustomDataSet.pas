...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBDataSetUpdateObjectClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBStringFieldClass(CurrentClass.ClassRef).create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := string(TIBStringField(CurrentObject).GetAsString);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TIBStringField(CurrentObject).SetAsString(VarToStr(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBBCDFieldClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBDataLinkClass(CurrentClass.ClassRef).Create(TIBCustomDataSet(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBGeneratorFieldClass(CurrentClass.ClassRef).Create(TIBCustomDataSet(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := string(TIBGeneratorField(CurrentObject).ValueName);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TIBGeneratorField(CurrentObject).Assign(TPersistent(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBCustomDataSetClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TIBCustomDataSet(CurrentObject).CachedUpdateStatus);
    ReturnOutputArg(AResult);
  end;
end;
...
AResult := TIBCustomDataSet(CurrentObject).LocateNext(VarToStr(GetInputArg(0)),GetInputArg(1),Param2);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBCustomDataSet(CurrentObject).Current);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TIBCustomDataSet(CurrentObject).SQLType);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBCustomDataSet(CurrentObject).CreateBlobStream(TField(VarToObject(GetInputArg(0))),TBlobStreamMode(VarToInteger(GetInputArg(1)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
IntToSet(Param2, VarToInteger(GetInputArg(2)), SizeOf(Param2));
AResult := TIBCustomDataSet(CurrentObject).Locate(VarToStr(GetInputArg(0)),GetInputArg(1),Param2);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := TIBCustomDataSet(CurrentObject).Lookup(VarToStr(GetInputArg(0)),GetInputArg(1),VarToStr(GetInputArg(2)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TIBCustomDataSet(CurrentObject).UpdateStatus);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBCustomDataSet(CurrentObject).UpdateObject));
  end;
end;
...
begin
  with AMachine do
  begin
    TIBCustomDataSet(CurrentObject).UpdateObject:=TIBDataSetUpdateObject(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TIBCustomDataSet(CurrentObject).Plan));
  end;
end;
...
begin
  with AMachine do
  begin
    TIBDataSet(CurrentObject).BatchInput(TIBBatchInput(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
Param0 := TIBBatchOutput(VarToObject(GetInputArg(0)));
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBDataSet(CurrentObject).ParamByName(VarToStr(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBDSBlobStreamClass(CurrentClass.ClassRef).Create(TField(VarToObject(GetInputArg(0))),TIBBlobStream(VarToObject(GetInputArg(1))),TBlobStreamMode(VarToInteger(GetInputArg(2)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    ...
    UpdateAction := TIBUpdateAction(VarToInteger(UpdateActionTemp));
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TIBDataSetUpdateObject,__TIBDataSetUpdateObjectCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TIBDataSetUpdateObjectDestroy,false,0,'');
  end;
  With Scripter.DefineClass(TIBStringField) do
  begin
    DefineMethod('create',1,tkClass,TIBStringField,__TIBStringFieldcreate,true,0,'AOwner: TComponent');
    DefineMethod('CheckTypeSize',1,tkNone,nil,__TIBStringFieldCheckTypeSize,true,0,'Value: Integer');
    ...
    DefineMethod('GetValue',1,tkVariant,nil,__TIBStringFieldGetValue,false,0,'Value: string').SetVarArgs([0]);
    DefineMethod('SetAsString',1,tkNone,nil,__TIBStringFieldSetAsString,false,0,'Value: string');
  end;
  With Scripter.DefineClass(TIBBCDField) do
  begin
    DefineMethod('Create',1,tkClass,TIBBCDField,__TIBBCDFieldCreate,true,0,'AOwner: TComponent');
  end;
  With Scripter.DefineClass(TIBDataLink) do
  begin
    DefineMethod('Create',1,tkClass,TIBDataLink,__TIBDataLinkCreate,true,0,'ADataSet: TIBCustomDataSet');
    DefineMethod('Destroy',0,tkNone,nil,__TIBDataLinkDestroy,false,0,'');
  end;
  With Scripter.DefineClass(TIBGeneratorField) do
  begin
    DefineMethod('Create',1,tkClass,TIBGeneratorField,__TIBGeneratorFieldCreate,true,0,'ADataSet: TIBCustomDataSet');
    ...
    DefineMethod('Assign',1,tkNone,nil,__TIBGeneratorFieldAssign,false,0,'Source: TPersistent');
  end;
  With Scripter.DefineClass(TIBCustomDataSet) do
  begin
    DefineMethod('Create',1,tkClass,TIBCustomDataSet,__TIBCustomDataSetCreate,true,0,'AOwner: TComponent');
    ...
    DefineMethod('LocateNext',3,tkVariant,nil,__TIBCustomDataSetLocateNext,false,0,'KeyFields: string; KeyValues: Variant; Options: TLocateOptions');
    DefineMethod('RecordModified',1,tkNone,nil,__TIBCustomDataSetRecordModified,false,0,'Value: Boolean');
    ...
    DefineMethod('CreateBlobStream',2,tkClass,TStream,__TIBCustomDataSetCreateBlobStream,false,0,'Field: TField; Mode: TBlobStreamMode');
    DefineMethod('GetCurrentRecord',1,tkVariant,nil,__TIBCustomDataSetGetCurrentRecord,false,0,'Buffer: PChar');
    DefineMethod('Locate',3,tkVariant,nil,__TIBCustomDataSetLocate,false,0,'KeyFields: string; KeyValues: Variant; Options: TLocateOptions');
    DefineMethod('Lookup',3,tkVariant,nil,__TIBCustomDataSetLookup,false,0,'KeyFields: string; KeyValues: Variant; ResultFields: string');
  ...
  begin
    ...
    DefineMethod('BatchInput',1,tkNone,nil,__TIBDataSetBatchInput,false,0,'InputObject: TIBBatchInput');
    DefineMethod('BatchOutput',1,tkNone,nil,__TIBDataSetBatchOutput,false,0,'putObject: TIBBatchOutput').SetVarArgs([0]);
    DefineMethod('ExecSQL',0,tkNone,nil,__TIBDataSetExecSQL,false,0,'');
    DefineMethod('ParamByName',1,tkClass,TIBXSQLVAR,__TIBDataSetParamByName,false,0,'Idx: String');
  ...
  begin
    DefineMethod('Create',3,tkClass,TIBDSBlobStream,__TIBDSBlobStreamCreate,true,0,'AField: TField; ABlobStream: TIBBlobStream; Mode: TBlobStreamMode');
    DefineMethod('Destroy',0,tkNone,nil,__TIBDSBlobStreamDestroy,false,0,'');
    DefineMethod('Read',2,tkInteger,nil,__TIBDSBlobStreamRead,false,0,'Buffer; Count: Longint').SetVarArgs([0]);
    DefineMethod('Seek',2,tkInteger,nil,__TIBDSBlobStreamSeek,false,0,'Offset: Longint; Origin: Word');
    DefineMethod('SetSize',1,tkNone,nil,__TIBDSBlobStreamSetSize,false,0,'NewSize: Longint');
    DefineMethod('Write',2,tkInteger,nil,__TIBDSBlobStreamWrite,false,0,'Buffer; Count: Longint');
  end;

...
type
  TatMConnectLibrary = class(TatScripterLibrary)
    ...
    procedure __TSharedConnectionCreate(AMachine: TatVirtualMachine);
...
begin
  with AMachine do
  begin
    TCustomObjectBroker(CurrentObject).SetConnectStatus(VarToStr(GetInputArg(0)),GetInputArg(1));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := string(TCustomObjectBroker(CurrentObject).GetComputerForGUID(TGUIDWrapper(integer(GetInputArg(0))).ObjToRec));
...
AResult := string(TCustomObjectBroker(CurrentObject).GetComputerForProgID(Param0));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TCustomObjectBroker(CurrentObject).GetPortForComputer(VarToStr(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TDispatchConnectionClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TDCOMConnectionClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
procedure TatMConnectLibrary.__TSharedConnectionCreate(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := ObjectToVar(TSharedConnectionClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    MConnect.GetMIDASAppServerList(TStringList(VarToObject(GetInputArg(0))),VarToStr(GetInputArg(1)));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('SetConnectStatus',2,tkNone,nil,__TCustomObjectBrokerSetConnectStatus,false,0,'ComputerName: string; Success: Boolean');
    DefineMethod('GetComputerForGUID',1,tkVariant,nil,__TCustomObjectBrokerGetComputerForGUID,false,0,'GUID: TGUID');
    DefineMethod('GetComputerForProgID',1,tkVariant,nil,__TCustomObjectBrokerGetComputerForProgID,false,0,'ProgID');
    DefineMethod('GetPortForComputer',1,tkInteger,nil,__TCustomObjectBrokerGetPortForComputer,false,0,'ComputerName: string');
  end;
  ...
  begin
    DefineMethod('Create',1,tkClass,TDispatchConnection,__TDispatchConnectionCreate,true,0,'AOwner: TComponent');
  end;
  ...
  begin
    DefineMethod('Create',1,tkClass,TDCOMConnection,__TDCOMConnectionCreate,true,0,'AOwner: TComponent');
  end;
  ...
  begin
    ...
    DefineMethod('Create',1,tkClass,TSharedConnection,__TSharedConnectionCreate,true,0,'AOwner: TComponent');
  end;
  With Scripter.DefineClass(ClassType) do
  begin
    DefineMethod('GetMIDASAppServerList',2,tkNone,nil,__GetMIDASAppServerList,false,0,'List: TStringList; RegCheck: string');
  end;
end;

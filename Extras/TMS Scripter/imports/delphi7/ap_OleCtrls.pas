...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TEventDispatchClass(CurrentClass.ClassRef).Create(TOleControl(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
AResult := Integer(TEnumPropDesc(CurrentObject).StringToValue(VarToStr(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := string(TEnumPropDesc(CurrentObject).ValueToString(VarToInteger(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TOleControlClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TOleControl(CurrentObject).GetEnumPropDesc(VarToInteger(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
AResult := TOleControl(CurrentObject).GetHelpContext(VarToStr(GetInputArg(0)),Param1,Param2);
    ReturnOutputArg(AResult);
...
begin
  with AMachine do
  begin
    TOleControl(CurrentObject).GetObjectVerbs(TStrings(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := string(TOleControl(CurrentObject).GetPropDisplayString(VarToInteger(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TOleControl(CurrentObject).GetPropDisplayStrings(VarToInteger(GetInputArg(0)),TStrings(VarToObject(GetInputArg(1))));
  end;
end;
...
begin
  with AMachine do
  begin
    TOleControl(CurrentObject).SetPropDisplayString(VarToInteger(GetInputArg(0)),VarToStr(GetInputArg(1)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := OleCtrls.FontToOleFont(TFont(VarToObject(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    OleCtrls.OleFontToFont(GetInputArg(0),TFont(VarToObject(GetInputArg(1))));
  end;
end;

procedure TatOleCtrlsLibrary.__GetEmptyParam(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    ReturnOutputArg(OleCtrls.EmptyParam);
  end;
end;

procedure TatOleCtrlsLibrary.__SetEmptyParam(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    OleCtrls.EmptyParam:=GetInputArg(0);
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TEventDispatch,__TEventDispatchCreate,true,0,'Control: TOleControl');
  end;
  With Scripter.DefineClass(TEnumPropDesc) do
  begin
    DefineMethod('Destroy',0,tkNone,nil,__TEnumPropDescDestroy,false,0,'');
    DefineMethod('StringToValue',1,tkInteger,nil,__TEnumPropDescStringToValue,false,0,'S: string');
    DefineMethod('ValueToString',1,tkVariant,nil,__TEnumPropDescValueToString,false,0,'V: Integer');
  end;
  With Scripter.DefineClass(TOleControl) do
  begin
    DefineMethod('Create',1,tkClass,TOleControl,__TOleControlCreate,true,0,'AOwner: TComponent');
    ...
    DefineMethod('DefaultHandler',1,tkNone,nil,__TOleControlDefaultHandler,false,0,'Message').SetVarArgs([0]);
    DefineMethod('DoObjectVerb',1,tkNone,nil,__TOleControlDoObjectVerb,false,0,'Verb: Integer');
    DefineMethod('GetEnumPropDesc',1,tkClass,TEnumPropDesc,__TOleControlGetEnumPropDesc,false,0,'DispID: Integer');
    DefineMethod('GetHelpContext',3,tkVariant,nil,__TOleControlGetHelpContext,false,0,'Member: string; HelpCtx: Integer; HelpFile: string').SetVarArgs([1,2]);
    DefineMethod('GetObjectVerbs',1,tkNone,nil,__TOleControlGetObjectVerbs,false,0,'List: TStrings');
    DefineMethod('GetPropDisplayString',1,tkVariant,nil,__TOleControlGetPropDisplayString,false,0,'DispID: Integer');
    DefineMethod('GetPropDisplayStrings',2,tkNone,nil,__TOleControlGetPropDisplayStrings,false,0,'DispID: Integer; List: TStrings');
    DefineMethod('IsCustomProperty',1,tkVariant,nil,__TOleControlIsCustomProperty,false,0,'DispID: Integer');
    DefineMethod('IsPropPageProperty',1,tkVariant,nil,__TOleControlIsPropPageProperty,false,0,'DispID: Integer');
    DefineMethod('SetBounds',4,tkNone,nil,__TOleControlSetBounds,false,0,'ALeft: Integer; ATop: Integer; AWidth: Integer; AHeight: Integer');
    DefineMethod('SetPropDisplayString',2,tkNone,nil,__TOleControlSetPropDisplayString,false,0,'DispID: Integer; Value: string');
    ...
    DefineProp('Anchors',tkVariant,nil,nil,nil,false,0);
  end;
  ...
  begin
    DefineMethod('FontToOleFont',1,tkVariant,nil,__FontToOleFont,false,0,'Font: TFont');
    DefineMethod('OleFontToFont',2,tkNone,nil,__OleFontToFont,false,0,'OleFont: Variant; Font: TFont');

...
begin
  with AMachine do
  begin
AResult := Integer(TEvent(CurrentObject).WaitFor(VarToInteger(GetInputArg(0))));
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TSimpleEventClass(CurrentClass.ClassRef).Create);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    DefineMethod('WaitFor',1,tkEnumeration,nil,__TEventWaitFor,false,0,'Timeout: LongWord');

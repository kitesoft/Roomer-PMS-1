...
begin
  with AMachine do
  begin
AResult := Integer(TCanvas(CurrentObject).TextHeight(VarToStr(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TCanvas(CurrentObject).TextRect(TRectWrapper(integer(GetInputArg(0))).ObjToRec,VarToInteger(GetInputArg(1)),VarToInteger(GetInputArg(2)),VarToStr(GetInputArg(3)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TCanvas(CurrentObject).TextWidth(VarToStr(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TCanvas(CurrentObject).CanvasOrientation));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TCanvas(CurrentObject).Pixels[VarToInteger(GetArrayIndex(0)),VarToInteger(GetArrayIndex(1))]));
  end;
end;
...
begin
  with AMachine do
  begin
    TCanvas(CurrentObject).Pixels[VarToInteger(GetArrayIndex(0)),VarToInteger(GetArrayIndex(1))]:=TColor(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  ...
  begin
    ...
    DefineMethod('TextHeight',1,tkInteger,nil,__TCanvasTextHeight,false,0,'Text: string');
    DefineMethod('TextOut',3,tkNone,nil,__TCanvasTextOut,false,0,'X: Integer; Y: Integer; Text: string');
    DefineMethod('TextRect',4,tkNone,nil,__TCanvasTextRect,false,0,'Rect: TRect; X: Integer; Y: Integer; Text: string');
    DefineMethod('TextWidth',1,tkInteger,nil,__TCanvasTextWidth,false,0,'Text: string');
  ...
  begin
    ...
    DefineMethod('CreateGrayMappedBmp',1,tkInteger,nil,__CreateGrayMappedBmp,false,0,'Handle: HBITMAP');
    DefineMethod('CreateGrayMappedRes',2,tkInteger,nil,__CreateGrayMappedRes,false,0,'Instance: THandle; ResName: PChar');

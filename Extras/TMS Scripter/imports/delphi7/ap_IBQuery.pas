...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBQueryClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
    TIBQuery(CurrentObject).BatchInput(TIBBatchInput(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    TIBQuery(CurrentObject).GetDetailLinkFields(TList(VarToObject(GetInputArg(0))),TList(VarToObject(GetInputArg(1))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBQuery(CurrentObject).ParamByName(VarToStr(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TIBQuery(CurrentObject).Text));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TIBQuery,__TIBQueryCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TIBQueryDestroy,false,0,'');
    DefineMethod('BatchInput',1,tkNone,nil,__TIBQueryBatchInput,false,0,'InputObject: TIBBatchInput');
    DefineMethod('BatchOutput',1,tkNone,nil,__TIBQueryBatchOutput,false,0,'putObject: TIBBatchOutput').SetVarArgs([0]);
    DefineMethod('ExecSQL',0,tkNone,nil,__TIBQueryExecSQL,false,0,'');
    DefineMethod('GetDetailLinkFields',2,tkNone,nil,__TIBQueryGetDetailLinkFields,false,0,'MasterFields: TList; DetailFields: TList');
    DefineMethod('ParamByName',1,tkClass,TParam,__TIBQueryParamByName,false,0,'Value: string');

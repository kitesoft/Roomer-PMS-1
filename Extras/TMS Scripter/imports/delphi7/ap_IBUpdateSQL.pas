...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBUpdateSQLClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
    TIBUpdateSQL(CurrentObject).Apply(TUpdateKind(VarToInteger(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    TIBUpdateSQL(CurrentObject).ExecSQL(TUpdateKind(VarToInteger(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    TIBUpdateSQL(CurrentObject).SetParams(TUpdateKind(VarToInteger(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBUpdateSQL(CurrentObject).Query[TUpdateKind(VarToInteger(GetArrayIndex(0)))]));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBUpdateSQL(CurrentObject).SQL[TUpdateKind(VarToInteger(GetArrayIndex(0)))]));
  end;
end;
...
begin
  with AMachine do
  begin
    TIBUpdateSQL(CurrentObject).SQL[TUpdateKind(VarToInteger(GetArrayIndex(0)))]:=TStrings(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TIBUpdateSQL,__TIBUpdateSQLCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TIBUpdateSQLDestroy,false,0,'');
    DefineMethod('Apply',1,tkNone,nil,__TIBUpdateSQLApply,false,0,'UpdateKind: TUpdateKind');
    DefineMethod('ExecSQL',1,tkNone,nil,__TIBUpdateSQLExecSQL,false,0,'UpdateKind: TUpdateKind');
    DefineMethod('SetParams',1,tkNone,nil,__TIBUpdateSQLSetParams,false,0,'UpdateKind: TUpdateKind');

...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBClientDataSetClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  ...
  begin
    Case InputArgCount of
      2: TIBClientDataSet(CurrentObject).CloneCursor(TCustomClientDataSet(VarToObject(GetInputArg(0))),GetInputArg(1));
      3: TIBClientDataSet(CurrentObject).CloneCursor(TCustomClientDataSet(VarToObject(GetInputArg(0))),GetInputArg(1),GetInputArg(2));
    end;
  end;
end;
...
begin
  with AMachine do
  begin
AResult := string(TIBClientDataSet(CurrentObject).GetQuoteChar);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TIBClientDataSet,__TIBClientDataSetCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TIBClientDataSetDestroy,false,0,'');
    DefineMethod('CloneCursor',3,tkNone,nil,__TIBClientDataSetCloneCursor,false,1,'Source: TCustomClientDataSet; Reset: Boolean; KeepSettings: Boolean = False');

...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBStoredProcClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
    TIBStoredProc(CurrentObject).CopyParams(TParams(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBStoredProc(CurrentObject).ParamByName(VarToStr(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBStoredProc(CurrentObject).StoredProcedureNames));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TIBStoredProc,__TIBStoredProcCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TIBStoredProcDestroy,false,0,'');
    DefineMethod('CopyParams',1,tkNone,nil,__TIBStoredProcCopyParams,false,0,'Value: TParams');
    DefineMethod('ExecProc',0,tkNone,nil,__TIBStoredProcExecProc,false,0,'');
    DefineMethod('ParamByName',1,tkClass,TParam,__TIBStoredProcParamByName,false,0,'Value: string');

...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBExtractClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
AResult := string(TIBExtract(CurrentObject).GetArrayField(VarToStr(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := string(TIBExtract(CurrentObject).GetFieldType(VarToInteger(GetInputArg(0)),VarToInteger(GetInputArg(1)),VarToInteger(GetInputArg(2)),VarToInteger(GetInputArg(3)),VarToInteger(GetInputArg(4)),VarToInteger(GetInputArg(5))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := string(TIBExtract(CurrentObject).GetCharacterSets(VarToInteger(GetInputArg(0)),VarToInteger(GetInputArg(1)),GetInputArg(2)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TIBExtract(CurrentObject).Notification(TComponent(VarToObject(GetInputArg(0))),TOperation(VarToInteger(GetInputArg(1))));
  end;
end;
...
IntToSet(Param2, VarToInteger(GetInputArg(2)), SizeOf(Param2));
    Case InputArgCount of
      1: TIBExtract(CurrentObject).ExtractObject(TExtractObjectTypes(VarToInteger(GetInputArg(0))));
      2: TIBExtract(CurrentObject).ExtractObject(TExtractObjectTypes(VarToInteger(GetInputArg(0))),VarToStr(GetInputArg(1)));
      3: TIBExtract(CurrentObject).ExtractObject(TExtractObjectTypes(VarToInteger(GetInputArg(0))),VarToStr(GetInputArg(1)),Param2);
    end;
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBExtract(CurrentObject).DatabaseInfo));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBExtract(CurrentObject).Items));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TIBExtract,__TIBExtractCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TIBExtractDestroy,false,0,'');
    DefineMethod('GetArrayField',1,tkVariant,nil,__TIBExtractGetArrayField,false,0,'FieldName: String');
    DefineMethod('GetFieldType',6,tkVariant,nil,__TIBExtractGetFieldType,false,0,'FieldType: Integer; FieldSubType: Integer; FieldScale: Integer; FieldSize: Integer; FieldPrec: Integer; FieldLen: Integer');
    DefineMethod('GetCharacterSets',3,tkVariant,nil,__TIBExtractGetCharacterSets,false,0,'CharSetId: Short; Collation: Short; CollateOnly: Boolean');
    DefineMethod('Notification',2,tkNone,nil,__TIBExtractNotification,false,0,'AComponent: TComponent; Operation: TOperation');
    DefineMethod('ExtractObject',3,tkNone,nil,__TIBExtractExtractObject,false,2,'ObjectType: TExtractObjectTypes; ObjectName: String = ''; ExtractTypes: TExtractTypes = []');

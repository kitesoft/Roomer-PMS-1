...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TIBBlobStreamClass(CurrentClass.ClassRef).Create);
...
begin
  with AMachine do
  begin
    TIBBlobStream(CurrentObject).LoadFromFile(VarToStr(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    TIBBlobStream(CurrentObject).LoadFromStream(TStream(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    TIBBlobStream(CurrentObject).SaveToFile(VarToStr(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    TIBBlobStream(CurrentObject).SaveToStream(TStream(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBBlobStream(CurrentObject).Database));
  end;
end;
...
begin
  with AMachine do
  begin
    TIBBlobStream(CurrentObject).Database:=TIBDatabase(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TIBBlobStream(CurrentObject).Mode));
  end;
end;
...
begin
  with AMachine do
  begin
    TIBBlobStream(CurrentObject).Mode:=TBlobStreamMode(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TIBBlobStream(CurrentObject).Transaction));
  end;
end;
...
begin
  with AMachine do
  begin
    TIBBlobStream(CurrentObject).Transaction:=TIBTransaction(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  ...
  begin
    ...
    DefineMethod('Call',2,tkInteger,nil,__TIBBlobStreamCall,false,0,'ErrCode: ISC_STATUS; RaiseError: Boolean');
    ...
    DefineMethod('LoadFromFile',1,tkNone,nil,__TIBBlobStreamLoadFromFile,false,0,'Filename: string');
    DefineMethod('LoadFromStream',1,tkNone,nil,__TIBBlobStreamLoadFromStream,false,0,'Stream: TStream');
    DefineMethod('Read',2,tkInteger,nil,__TIBBlobStreamRead,false,0,'Buffer; Count: Longint').SetVarArgs([0]);
    DefineMethod('SaveToFile',1,tkNone,nil,__TIBBlobStreamSaveToFile,false,0,'Filename: string');
    DefineMethod('SaveToStream',1,tkNone,nil,__TIBBlobStreamSaveToStream,false,0,'Stream: TStream');
    DefineMethod('Seek',2,tkInteger,nil,__TIBBlobStreamSeek,false,0,'Offset: Longint; Origin: Word');
    DefineMethod('SetSize',1,tkNone,nil,__TIBBlobStreamSetSize,false,0,'NewSize: Long');
    DefineMethod('Truncate',0,tkNone,nil,__TIBBlobStreamTruncate,false,0,'');
    DefineMethod('Write',2,tkInteger,nil,__TIBBlobStreamWrite,false,0,'Buffer; Count: Longint');

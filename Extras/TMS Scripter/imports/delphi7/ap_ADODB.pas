...
type
  TatADODBLibrary = class(TatScripterLibrary)
    ...
    procedure __TADOConnectionExecute(AMachine: TatVirtualMachine);
    ...
    procedure __TADOConnectionOpen(AMachine: TatVirtualMachine);
    ...
    procedure __TCustomADODataSetGetDetailLinkFields(AMachine: TatVirtualMachine);
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TADOConnectionClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
procedure TatADODBLibrary.__TADOConnectionExecute(AMachine: TatVirtualMachine);
  var
  Param1: Integer;
  Param2: TExecuteOptions;
begin
  with AMachine do
  begin
...
IntToSet(Param2, VarToInteger(GetInputArg(2)), SizeOf(Param2));
    Case InputArgCount of
      2: TADOConnection(CurrentObject).Execute(GetInputArg(0),Param1);
      3: TADOConnection(CurrentObject).Execute(GetInputArg(0),Param1,Param2);
    end;
...
begin
  with AMachine do
  begin
    TADOConnection(CurrentObject).GetProcedureNames(TStrings(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    TADOConnection(CurrentObject).GetFieldNames(VarToStr(GetInputArg(0)),TStrings(VarToObject(GetInputArg(1))));
  end;
end;
...
begin
  ...
  begin
    Case InputArgCount of
      1: TADOConnection(CurrentObject).GetTableNames(TStrings(VarToObject(GetInputArg(0))));
      2: TADOConnection(CurrentObject).GetTableNames(TStrings(VarToObject(GetInputArg(0))),GetInputArg(1));
    end;
  end;
end;

procedure TatADODBLibrary.__TADOConnectionOpen(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    TADOConnection(CurrentObject).Open(GetInputArg(0),GetInputArg(1));
  end;
end;
...
begin
  with AMachine do
  begin
    TADOConnection(CurrentObject).OpenSchema(TSchemaInfo(VarToInteger(GetInputArg(0))),GetInputArg(1),GetInputArg(2),TADODataSet(VarToObject(GetInputArg(3))));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TADOConnection(CurrentObject).Commands[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TADOConnection(CurrentObject).DataSets[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TRDSConnectionClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TParameter(CurrentObject).Assign(TPersistent(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    TParameter(CurrentObject).LoadFromFile(VarToStr(GetInputArg(0)),TFieldType(VarToInteger(GetInputArg(1))));
  end;
end;
...
begin
  with AMachine do
  begin
    TParameter(CurrentObject).LoadFromStream(TStream(VarToObject(GetInputArg(0))),TFieldType(VarToInteger(GetInputArg(1))));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TParameter(CurrentObject).Parameters));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TParameters(CurrentObject).AddParameter);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TParameters(CurrentObject).AssignValues(TParameters(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TParameters(CurrentObject).CreateParameter(GetInputArg(0),TFieldType(VarToInteger(GetInputArg(1))),TParameterDirection(VarToInteger(GetInputArg(2))),VarToInteger(GetInputArg(3)),GetInputArg(4)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TParameters(CurrentObject).FindParam(GetInputArg(0)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TParameters(CurrentObject).GetParamList(TList(VarToObject(GetInputArg(0))),GetInputArg(1));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := TParameters(CurrentObject).IsEqual(TParameters(VarToObject(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TParameters(CurrentObject).ParamByName(GetInputArg(0)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := string(TParameters(CurrentObject).ParseSQL(VarToStr(GetInputArg(0)),GetInputArg(1)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TParameters(CurrentObject).Command));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TParameters(CurrentObject).Items[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    TParameters(CurrentObject).Items[VarToInteger(GetArrayIndex(0))]:=TParameter(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TADOCommandClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TADOCommand(CurrentObject).Assign(TPersistent(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomADODataSetClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    Case InputArgCount of
      ...
      1: TCustomADODataSet(CurrentObject).CancelBatch(TAffectRecords(VarToInteger(GetInputArg(0))));
    end;
  end;
end;
...
begin
  ...
  begin
    Case InputArgCount of
      1: TCustomADODataSet(CurrentObject).Clone(TCustomADODataSet(VarToObject(GetInputArg(0))));
      2: TCustomADODataSet(CurrentObject).Clone(TCustomADODataSet(VarToObject(GetInputArg(0))),TADOLockType(VarToInteger(GetInputArg(1))));
    end;
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomADODataSet(CurrentObject).CreateBlobStream(TField(VarToObject(GetInputArg(0))),TBlobStreamMode(VarToInteger(GetInputArg(1)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    Case InputArgCount of
      ...
      1: TCustomADODataSet(CurrentObject).DeleteRecords(TAffectRecords(VarToInteger(GetInputArg(0))));
    end;
  end;
end;

procedure TatADODBLibrary.__TCustomADODataSetGetDetailLinkFields(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    TCustomADODataSet(CurrentObject).GetDetailLinkFields(TList(VarToObject(GetInputArg(0))),TList(VarToObject(GetInputArg(1))));
  end;
end;
...
begin
  ...
  begin
IntToSet(Param2, VarToInteger(GetInputArg(2)), SizeOf(Param2));
AResult := TCustomADODataSet(CurrentObject).Locate(VarToStr(GetInputArg(0)),GetInputArg(1),Param2);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := TCustomADODataSet(CurrentObject).Lookup(VarToStr(GetInputArg(0)),GetInputArg(1),VarToStr(GetInputArg(2)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    Case InputArgCount of
      ...
      2: TCustomADODataSet(CurrentObject).SaveToFile(GetInputArg(0),TPersistFormat(VarToInteger(GetInputArg(1))));
    end;
  end;
end;
...
2: AResult := TCustomADODataSet(CurrentObject).Seek(GetInputArg(0),TSeekOption(VarToInteger(GetInputArg(1))));
end;
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    Case InputArgCount of
      ...
      1: TCustomADODataSet(CurrentObject).UpdateBatch(TAffectRecords(VarToInteger(GetInputArg(0))));
    end;
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TCustomADODataSet(CurrentObject).UpdateStatus);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TCustomADODataSet(CurrentObject).DesignerData));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomADODataSet(CurrentObject).DesignerData:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TCustomADODataSet(CurrentObject).IndexName));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomADODataSet(CurrentObject).IndexName:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCustomADODataSet(CurrentObject).IndexFields[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomADODataSet(CurrentObject).IndexFields[VarToInteger(GetArrayIndex(0))]:=TField(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TCustomADODataSet(CurrentObject).FilterGroup));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomADODataSet(CurrentObject).FilterGroup:=TFilterGroup(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    TADODataSet(CurrentObject).GetIndexNames(TStrings(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TADOTableClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TADOTable(CurrentObject).GetIndexNames(TStrings(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TADOQueryClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TADOStoredProcClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TADOBlobStreamClass(CurrentClass.ClassRef).Create(TBlobField(VarToObject(GetInputArg(0))),TBlobStreamMode(VarToInteger(GetInputArg(1)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    ...
    EventStatusTemp := Integer(EventStatus);
    if Assigned(Scripter) and (RoutineName > '') then
      Scripter.ExecuteSubroutine(RoutineName, [Connection,EventStatusTemp]);
    EventStatus := TEventStatus(VarToInteger(EventStatusTemp));
...
begin
  ...
  begin
    ...
    ConnectOptionsTemp := Integer(ConnectOptions);
    EventStatusTemp := Integer(EventStatus);
    if Assigned(Scripter) and (RoutineName > '') then
    ...
    ConnectOptions := TConnectOption(VarToInteger(ConnectOptionsTemp));
    EventStatus := TEventStatus(VarToInteger(EventStatusTemp));
    if AssignedMethod(AfterCall) then
...
begin
  with AMachine do
  begin
AResult := string(ADODB.DataLinkDir);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ADODB.GetProviderNames(TStrings(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  ...
  begin
case InputArgCount of
1: AResult := Integer(ADODB.GetDataLinkFiles(TStrings(VarToObject(GetInputArg(0)))));
2: AResult := Integer(ADODB.GetDataLinkFiles(TStrings(VarToObject(GetInputArg(0))),VarToStr(GetInputArg(1))));
end;
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TADOConnection,__TADOConnectionCreate,true,0,'AOwner: TComponent');
    ...
    DefineMethod('Execute',3,tkNone,nil,__TADOConnectionExecute,false,1,'CommandText: WideString; RecordsAffected: Integer; ExecuteOptions: TExecuteOptions = [eoExecuteNoRecords]').SetVarArgs([1]);
    DefineMethod('GetProcedureNames',1,tkNone,nil,__TADOConnectionGetProcedureNames,false,0,'List: TStrings');
    DefineMethod('GetFieldNames',2,tkNone,nil,__TADOConnectionGetFieldNames,false,0,'TableName: string; List: TStrings');
    DefineMethod('GetTableNames',2,tkNone,nil,__TADOConnectionGetTableNames,false,1,'List: TStrings; SystemTables: Boolean = False');
    DefineMethod('Open',2,tkNone,nil,__TADOConnectionOpen,false,0,'UserID: WideString; Password: WideString');
    DefineMethod('OpenSchema',4,tkNone,nil,__TADOConnectionOpenSchema,false,0,'Schema: TSchemaInfo; Restrictions: OleVariant; SchemaID: OleVariant; DataSet: TADODataSet');
  ...
  begin
    DefineMethod('Create',1,tkClass,TRDSConnection,__TRDSConnectionCreate,true,0,'AOwner: TComponent');
  ...
  begin
    DefineMethod('Assign',1,tkNone,nil,__TParameterAssign,false,0,'Source: TPersistent');
    DefineMethod('AppendChunk',1,tkNone,nil,__TParameterAppendChunk,false,0,'Val: OleVariant');
    DefineMethod('LoadFromFile',2,tkNone,nil,__TParameterLoadFromFile,false,0,'FileName: string; DataType: TDataType');
    DefineMethod('LoadFromStream',2,tkNone,nil,__TParameterLoadFromStream,false,0,'Stream: TStream; DataType: TDataType');
  ...
  begin
    ...
    DefineMethod('AssignValues',1,tkNone,nil,__TParametersAssignValues,false,0,'Value: TParameters');
    DefineMethod('CreateParameter',5,tkClass,TParameter,__TParametersCreateParameter,false,0,'Name: WideString; DataType: TDataType; Direction: TParameterDirection; Size: Integer; Value: OleVariant');
    DefineMethod('FindParam',1,tkClass,TParameter,__TParametersFindParam,false,0,'Value: WideString');
    DefineMethod('GetParamList',2,tkNone,nil,__TParametersGetParamList,false,0,'List: TList; ParamNames: WideString');
    DefineMethod('IsEqual',1,tkVariant,nil,__TParametersIsEqual,false,0,'Value: TParameters');
    DefineMethod('ParamByName',1,tkClass,TParameter,__TParametersParamByName,false,0,'Value: WideString');
    DefineMethod('ParseSQL',2,tkVariant,nil,__TParametersParseSQL,false,0,'SQL: string; DoCreate: Boolean');
  ...
  begin
    DefineMethod('Create',1,tkClass,TADOCommand,__TADOCommandCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TADOCommandDestroy,false,0,'');
    DefineMethod('Assign',1,tkNone,nil,__TADOCommandAssign,false,0,'Source: TPersistent');
  ...
  begin
    DefineMethod('Create',1,tkClass,TCustomADODataSet,__TCustomADODataSetCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TCustomADODataSetDestroy,false,0,'');
    DefineMethod('CancelBatch',1,tkNone,nil,__TCustomADODataSetCancelBatch,false,1,'AffectRecords: TAffectRecords = arAll');
    DefineMethod('CancelUpdates',0,tkNone,nil,__TCustomADODataSetCancelUpdates,false,0,'');
    DefineMethod('Clone',2,tkNone,nil,__TCustomADODataSetClone,false,1,'Source: TCustomADODataSet; LockType: TADOLockType = ltUnspecified');
    DefineMethod('CreateBlobStream',2,tkClass,TStream,__TCustomADODataSetCreateBlobStream,false,0,'Field: TField; Mode: TBlobStreamMode');
    DefineMethod('DeleteRecords',1,tkNone,nil,__TCustomADODataSetDeleteRecords,false,1,'AffectRecords: TAffectRecords = arAll');
    DefineMethod('GetDetailLinkFields',2,tkNone,nil,__TCustomADODataSetGetDetailLinkFields,false,0,'MasterFields: TList; DetailFields: TList');
    DefineMethod('IsSequenced',0,tkVariant,nil,__TCustomADODataSetIsSequenced,false,0,'');
    DefineMethod('LoadFromFile',1,tkNone,nil,__TCustomADODataSetLoadFromFile,false,0,'FileName: WideString');
    DefineMethod('Locate',3,tkVariant,nil,__TCustomADODataSetLocate,false,0,'KeyFields: string; KeyValues: Variant; Options: TLocateOptions');
    DefineMethod('Lookup',3,tkVariant,nil,__TCustomADODataSetLookup,false,0,'KeyFields: string; KeyValues: Variant; ResultFields: string');
    DefineMethod('Requery',1,tkNone,nil,__TCustomADODataSetRequery,false,1,'Options: TExecuteOptions = []');
    DefineMethod('SaveToFile',2,tkNone,nil,__TCustomADODataSetSaveToFile,false,2,'FileName: WideString = ''; Format: TPersistFormat = pfADTG');
    DefineMethod('Seek',2,tkVariant,nil,__TCustomADODataSetSeek,false,1,'KeyValues: Variant; SeekOption: TSeekOption = soFirstEQ');
    DefineMethod('Supports',1,tkVariant,nil,__TCustomADODataSetSupports,false,0,'CursorOptions: TCursorOptions');
    DefineMethod('UpdateBatch',1,tkNone,nil,__TCustomADODataSetUpdateBatch,false,1,'AffectRecords: TAffectRecords = arAll');
  ...
  begin
    ...
    DefineMethod('GetIndexNames',1,tkNone,nil,__TADODataSetGetIndexNames,false,0,'List: TStrings');
    DefineProp('IndexDefs',tkVariant,nil,nil,nil,false,0);
  end;
  With Scripter.DefineClass(TADOTable) do
  begin
    DefineMethod('Create',1,tkClass,TADOTable,__TADOTableCreate,true,0,'AOwner: TComponent');
    DefineMethod('GetIndexNames',1,tkNone,nil,__TADOTableGetIndexNames,false,0,'List: TStrings');
    DefineProp('IndexDefs',tkVariant,nil,nil,nil,false,0);
  end;
  With Scripter.DefineClass(TADOQuery) do
  begin
    DefineMethod('Create',1,tkClass,TADOQuery,__TADOQueryCreate,true,0,'AOwner: TComponent');
  ...
  begin
    DefineMethod('Create',1,tkClass,TADOStoredProc,__TADOStoredProcCreate,true,0,'AOwner: TComponent');
    DefineMethod('ExecProc',0,tkNone,nil,__TADOStoredProcExecProc,false,0,'');
  end;
  With Scripter.DefineClass(TADOBlobStream) do
  begin
    DefineMethod('Create',2,tkClass,TADOBlobStream,__TADOBlobStreamCreate,true,0,'Field: TBlobField; Mode: TBlobStreamMode');
    DefineMethod('Destroy',0,tkNone,nil,__TADOBlobStreamDestroy,false,0,'');
    DefineMethod('Write',2,tkInteger,nil,__TADOBlobStreamWrite,false,0,'Buffer; Count: Longint');
  ...
  begin
    DefineMethod('CreateUDLFile',3,tkNone,nil,__CreateUDLFile,false,0,'FileName: WideString; ProviderName: WideString; DataSourceName: WideString');
    DefineMethod('DataLinkDir',0,tkVariant,nil,__DataLinkDir,false,0,'');
    DefineMethod('GetProviderNames',1,tkNone,nil,__GetProviderNames,false,0,'Names: TStrings');
    DefineMethod('PromptDataSource',2,tkVariant,nil,__PromptDataSource,false,0,'ParentHandle: THandle; InitialString: WideString');
    DefineMethod('PromptDataLinkFile',2,tkVariant,nil,__PromptDataLinkFile,false,0,'ParentHandle: THandle; InitialFile: WideString');
    DefineMethod('GetDataLinkFiles',2,tkInteger,nil,__GetDataLinkFiles,false,1,'FileNames: TStrings; Directory: string = ''');

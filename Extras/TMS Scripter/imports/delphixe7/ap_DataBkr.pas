...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TProviderDataModuleClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
    TProviderDataModule(CurrentObject).RegisterProvider(TCustomProvider(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    TProviderDataModule(CurrentObject).UnRegisterProvider(TCustomProvider(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TProviderDataModule(CurrentObject).Providers[VarToStr(GetArrayIndex(0))]));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TProviderDataModule,__TProviderDataModuleCreate,true,0,'AOwner: TComponent');
    ...
    DefineMethod('RegisterProvider',1,tkNone,nil,__TProviderDataModuleRegisterProvider,false,0,'Value: TCustomProvider');
    DefineMethod('UnRegisterProvider',1,tkNone,nil,__TProviderDataModuleUnRegisterProvider,false,0,'Value: TCustomProvider');

...
type
  TatExtDlgsLibrary = class(TatScripterLibrary)
    ...
    procedure __StandardEncodingFromName(AMachine: TatVirtualMachine);
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TOpenPictureDialogClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TOpenTextFileDialogClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
procedure TatExtDlgsLibrary.__StandardEncodingFromName(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := ObjectToVar(ExtDlgs.StandardEncodingFromName(VarToStr(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TOpenPictureDialog,__TOpenPictureDialogCreate,true,0,'AOwner: TComponent');
    DefineMethod('Execute',1,tkVariant,nil,__TOpenPictureDialogExecute,false,0,'ParentWnd: HWND');
  end;
  ...
  begin
    DefineMethod('Create',1,tkClass,TOpenTextFileDialog,__TOpenTextFileDialogCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TOpenTextFileDialogDestroy,false,0,'');
    DefineMethod('Execute',1,tkVariant,nil,__TOpenTextFileDialogExecute,false,0,'ParentWnd: HWND');
  end;
  ...
  begin
    DefineMethod('StandardEncodingFromName',1,tkClass,TEncoding,__StandardEncodingFromName,false,0,'Name: string');
  end;
end;

...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TOleContainerClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
    TOleContainer(CurrentObject).CreateLinkToFile(VarToStr(GetInputArg(0)),GetInputArg(1));
  end;
end;
...
begin
  with AMachine do
  begin
    TOleContainer(CurrentObject).CreateObject(VarToStr(GetInputArg(0)),GetInputArg(1));
  end;
end;
...
begin
  with AMachine do
  begin
    TOleContainer(CurrentObject).CreateObjectFromFile(VarToStr(GetInputArg(0)),GetInputArg(1));
  end;
end;
...
begin
  with AMachine do
  begin
    TOleContainer(CurrentObject).LoadFromFile(VarToStr(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    TOleContainer(CurrentObject).LoadFromStream(TStream(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    TOleContainer(CurrentObject).SaveAsDocument(VarToStr(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    TOleContainer(CurrentObject).SaveToFile(VarToStr(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    TOleContainer(CurrentObject).SaveToStream(TStream(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TOleContainer(CurrentObject).ObjectVerbs));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TOleContainer(CurrentObject).OleClassName));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TOleContainer(CurrentObject).SourceDoc));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TOleContainer(CurrentObject).State));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TOleFormClass(CurrentClass.ClassRef).Create(TCustomForm(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TOleContainer,__TOleContainerCreate,true,0,'AOwner: TComponent');
    ...
    DefineMethod('CreateLinkToFile',2,tkNone,nil,__TOleContainerCreateLinkToFile,false,0,'FileName: string; Iconic: Boolean');
    DefineMethod('CreateObject',2,tkNone,nil,__TOleContainerCreateObject,false,0,'OleClassName: string; Iconic: Boolean');
    DefineMethod('CreateObjectFromFile',2,tkNone,nil,__TOleContainerCreateObjectFromFile,false,0,'FileName: string; Iconic: Boolean');
    DefineMethod('CreateObjectFromInfo',1,tkNone,nil,__TOleContainerCreateObjectFromInfo,false,0,'CreateInfo: TCreateInfo');
    DefineMethod('DestroyObject',0,tkNone,nil,__TOleContainerDestroyObject,false,0,'');
    DefineMethod('DoVerb',1,tkNone,nil,__TOleContainerDoVerb,false,0,'Verb: Integer');
    ...
    DefineMethod('LoadFromFile',1,tkNone,nil,__TOleContainerLoadFromFile,false,0,'FileName: string');
    DefineMethod('LoadFromStream',1,tkNone,nil,__TOleContainerLoadFromStream,false,0,'Stream: TStream');
    ...
    DefineMethod('SaveAsDocument',1,tkNone,nil,__TOleContainerSaveAsDocument,false,0,'FileName: string');
    DefineMethod('SaveToFile',1,tkNone,nil,__TOleContainerSaveToFile,false,0,'FileName: string');
    DefineMethod('SaveToStream',1,tkNone,nil,__TOleContainerSaveToStream,false,0,'Stream: TStream');
  ...
  begin
    DefineMethod('Create',1,tkClass,TOleForm,__TOleFormCreate,true,0,'Form: TCustomForm');
    DefineMethod('Destroy',0,tkNone,nil,__TOleFormDestroy,false,0,'');
  end;
  With Scripter.DefineClass(ClassType) do
  begin
    DefineMethod('DestroyMetaPict',1,tkNone,nil,__DestroyMetaPict,false,0,'MetaPict: HGlobal');

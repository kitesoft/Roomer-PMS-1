...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomStaticListActionClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TCustomStaticListAction(CurrentObject).Items));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomStaticListAction(CurrentObject).Items:=TStaticListItems(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TCustomStaticListAction,__TCustomStaticListActionCreate,true,0,'AOwner: TComponent');

...
type
  ...
  private
    ...
    procedure __TInquireEvent( Sender: TObject; var idIcon: Integer; var idName: Integer; var idInfo: Integer; var lData: NativeInt);
    procedure __TNewInquireEvent( Sender: TObject; var lData: NativeInt; var hIcon: HICON; var AppletName: string; var AppletInfo: string);
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TAppletModuleClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TAppletApplicationClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TAppletApplication(CurrentObject).HandleException(TObject(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TAppletApplication(CurrentObject).Modules[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    TAppletApplication(CurrentObject).Modules[VarToInteger(GetArrayIndex(0))]:=TAppletModule(VarToObject(GetInputArg(0)));
  end;
end;
...
procedure TatCtlPanelDispatcher.__TInquireEvent( Sender: TObject; var idIcon: Integer; var idName: Integer; var idInfo: Integer; var lData: NativeInt);
var
  idIconTemp: variant;
  idNameTemp: variant;
  idInfoTemp: variant;
...
procedure TatCtlPanelDispatcher.__TNewInquireEvent( Sender: TObject; var lData: NativeInt; var hIcon: HICON; var AppletName: string; var AppletInfo: string);
var
  lDataTemp: variant;
  hIconTemp: variant;
...
begin
  ...
  begin
    ...
    AppletNameTemp := string(AppletName);
    AppletInfoTemp := string(AppletInfo);
    if Assigned(Scripter) and (RoutineName > '') then
    ...
    AppletName := VarToStr(AppletNameTemp);
    AppletInfo := VarToStr(AppletInfoTemp);
    if AssignedMethod(AfterCall) then
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(CtlPanel.Application));
  end;
end;
...
begin
  with AMachine do
  begin
    CtlPanel.Application:=TAppletApplication(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TAppletModule,__TAppletModuleCreate,true,0,'AOwner: TComponent');
  ...
  begin
    DefineMethod('Create',1,tkClass,TAppletApplication,__TAppletApplicationCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TAppletApplicationDestroy,false,0,'');
    DefineMethod('HandleException',1,tkNone,nil,__TAppletApplicationHandleException,false,0,'Sender: TObject');
  ...
  begin
    DefineMethod('CPlApplet',4,tkInteger,nil,__CPlApplet,false,0,'hwndCPl: THandle; uMsg: DWORD; lParam1: Longint; lParam2: Longint');

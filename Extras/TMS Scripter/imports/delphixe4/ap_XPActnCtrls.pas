...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TXPStylePopupMenuClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TXPStyleCustomizePopupClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TXPStylePopupMenu,__TXPStylePopupMenuCreate,true,0,'AOwner: TComponent');
  ...
  begin
    DefineMethod('Create',1,tkClass,TXPStyleCustomizePopup,__TXPStyleCustomizePopupCreate,true,0,'AOwner: TComponent');
  end;

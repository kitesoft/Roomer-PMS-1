...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TThemedPopupMenuClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TThemedCustomizePopupClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TThemedPopupMenu,__TThemedPopupMenuCreate,true,0,'AOwner: TComponent');
  end;
  ...
  begin
    DefineMethod('Create',1,tkClass,TThemedCustomizePopup,__TThemedCustomizePopupCreate,true,0,'AOwner: TComponent');
  end;

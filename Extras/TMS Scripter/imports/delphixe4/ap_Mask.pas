...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomMaskEditClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TCustomMaskEdit(CurrentObject).EditText));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomMaskEdit(CurrentObject).EditText:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TCustomMaskEdit(CurrentObject).Text));
  end;
end;
...
begin
  with AMachine do
  begin
    TCustomMaskEdit(CurrentObject).Text:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TCustomMaskEdit,__TCustomMaskEditCreate,true,0,'AOwner: TComponent');

...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TDBCtrlGridLinkClass(CurrentClass.ClassRef).Create(TDBCtrlGrid(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TDBCtrlPanelClass(CurrentClass.ClassRef).CreateLinked(TDBCtrlGrid(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TDBCtrlGridClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TDBCtrlGrid(CurrentObject).DoKey(TDBCtrlGridKey(VarToInteger(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := TDBCtrlGrid(CurrentObject).ExecuteAction(TBasicAction(VarToObject(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TDBCtrlGrid(CurrentObject).GetTabOrderList(TList(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := TDBCtrlGrid(CurrentObject).UpdateAction(TBasicAction(VarToObject(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TDBCtrlGrid(CurrentObject).Canvas));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TDBCtrlGridLink,__TDBCtrlGridLinkCreate,true,0,'DBCtrlGrid: TDBCtrlGrid');
  end;
  With Scripter.DefineClass(TDBCtrlPanel) do
  begin
    DefineMethod('CreateLinked',1,tkClass,TDBCtrlPanel,__TDBCtrlPanelCreateLinked,true,0,'DBCtrlGrid: TDBCtrlGrid');
  end;
  With Scripter.DefineClass(TDBCtrlGrid) do
  begin
    DefineMethod('Create',1,tkClass,TDBCtrlGrid,__TDBCtrlGridCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TDBCtrlGridDestroy,false,0,'');
    DefineMethod('DoKey',1,tkNone,nil,__TDBCtrlGridDoKey,false,0,'Key: TDBCtrlGridKey');
    DefineMethod('ExecuteAction',1,tkVariant,nil,__TDBCtrlGridExecuteAction,false,0,'Action: TBasicAction');
    DefineMethod('GetTabOrderList',1,tkNone,nil,__TDBCtrlGridGetTabOrderList,false,0,'List: TList');
    DefineMethod('KeyDown',2,tkNone,nil,__TDBCtrlGridKeyDown,false,0,'Key: Word; Shift: TShiftState').SetVarArgs([0]);
    DefineMethod('SetBounds',4,tkNone,nil,__TDBCtrlGridSetBounds,false,0,'ALeft: Integer; ATop: Integer; AWidth: Integer; AHeight: Integer');
    DefineMethod('UpdateAction',1,tkVariant,nil,__TDBCtrlGridUpdateAction,false,0,'Action: TBasicAction');

...
AResult := ShlObj.SHGetSpecialFolderPath(VarToInteger(GetInputArg(0)),StringToWideChar(VarToStr(GetInputArg(1)),@Param1Buf[0],Length(Param1Buf)),VarToInteger(GetInputArg(2)),GetInputArg(3));
...
begin
  with AMachine do
  begin
AResult := ShlObj.SHGetSpecialFolderPathA(VarToInteger(GetInputArg(0)),PAnsiChar(AnsiString(VarToStr(GetInputArg(1)))),VarToInteger(GetInputArg(2)),GetInputArg(3));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
SetLength(Param1Buf, Length(VarToStr(GetInputArg(1))) + 1);
AResult := ShlObj.SHGetSpecialFolderPathW(VarToInteger(GetInputArg(0)),StringToWideChar(VarToStr(GetInputArg(1)),@Param1Buf[0],Length(Param1Buf)),VarToInteger(GetInputArg(2)),GetInputArg(3));
    ReturnOutputArg(AResult);
  end;
end;
...
AResult := Integer(ShlObj.SHCreateItemInKnownFolder(TGUIDWrapper(integer(GetInputArg(0))).ObjToRec,VarToInteger(GetInputArg(1)),StringToWideChar(VarToStr(GetInputArg(2)),@Param2Buf[0],Length(Param2Buf)),TGUIDWrapper(integer(GetInputArg(3))).ObjToRec,Param4));
    ReturnOutputArg(AResult);
...
begin
  With Scripter.DefineClass(ClassType) do
  begin
    DefineMethod('SHGetSpecialFolderPath',4,tkVariant,nil,__SHGetSpecialFolderPath,false,0,'hwndOwner: HWND; lpszPath: PWideChar; nFolder: Integer; fCreate: BOOL');
    DefineMethod('SHGetSpecialFolderPathA',4,tkVariant,nil,__SHGetSpecialFolderPathA,false,0,'hwndOwner: HWND; lpszPath: PAnsiChar; nFolder: Integer; fCreate: BOOL');
    DefineMethod('SHGetSpecialFolderPathW',4,tkVariant,nil,__SHGetSpecialFolderPathW,false,0,'hwndOwner: HWND; lpszPath: PWideChar; nFolder: Integer; fCreate: BOOL');
    DefineMethod('SHLoadInProc',1,tkInteger,nil,__SHLoadInProc,false,0,'rclsid: TCLSID');
    DefineMethod('SHGetSettings',2,tkNone,nil,__SHGetSettings,false,0,'lpss: TShellFlagState; dwMask: DWORD').SetVarArgs([0]);
    DefineMethod('SHCreateItemInKnownFolder',5,tkInteger,nil,__SHCreateItemInKnownFolder,false,0,'kfid: TIID; dwKFFlags: DWORD; pszItem: LPCWSTR; riid: TIID; ppv').SetVarArgs([4]);

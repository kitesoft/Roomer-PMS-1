...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TToolWindowClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TToolWindow(CurrentObject).EdgeInner));
  end;
end;
...
begin
  with AMachine do
  begin
    TToolWindow(CurrentObject).EdgeInner:=TEdgeStyle(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TToolWindow(CurrentObject).EdgeOuter));
  end;
end;
...
begin
  with AMachine do
  begin
    TToolWindow(CurrentObject).EdgeOuter:=TEdgeStyle(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TToolDockObjectClass(CurrentClass.ClassRef).Create(TControl(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TToolDockFormClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TToolWindow,__TToolWindowCreate,true,0,'AOwner: TComponent');
  ...
  begin
    DefineMethod('Create',1,tkClass,TToolDockObject,__TToolDockObjectCreate,true,0,'AControl: TControl');
  end;
  With Scripter.DefineClass(TToolDockForm) do
  begin
    DefineMethod('Create',1,tkClass,TToolDockForm,__TToolDockFormCreate,true,0,'AOwner: TComponent');
  end;

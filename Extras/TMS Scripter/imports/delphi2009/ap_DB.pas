...
type
  TatDBLibrary = class(TatScripterLibrary)
    ...
    procedure __TParamObjectGetImplementation(AMachine: TatVirtualMachine);
    procedure __GetTParamObjectIsInstanceOwner(AMachine: TatVirtualMachine);
    procedure __TParamDataSetObjectCreate(AMachine: TatVirtualMachine);
    procedure __TParamDataSetObjectDestroy(AMachine: TatVirtualMachine);
    procedure __GetTParamDataSetObjectDataSet(AMachine: TatVirtualMachine);
    procedure __TParamParamsObjectCreate(AMachine: TatVirtualMachine);
    procedure __TParamParamsObjectDestroy(AMachine: TatVirtualMachine);
    procedure __GetTParamParamsObjectParams(AMachine: TatVirtualMachine);
    procedure __TParamStreamObjectCreate(AMachine: TatVirtualMachine);
    procedure __TParamStreamObjectDestroy(AMachine: TatVirtualMachine);
    procedure __GetTParamStreamObjectStream(AMachine: TatVirtualMachine);
  ...
  TParamDataSetObjectClass = class of TParamDataSetObject;
  TParamParamsObjectClass = class of TParamParamsObject;
...
begin
  with AMachine do
  begin
    TParam(CurrentObject).SetStream(TStream(VarToObject(GetInputArg(0))),GetInputArg(1));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TParamObjectClass(CurrentClass.ClassRef).Create(GetInputArg(0),GetInputArg(1)));
    ReturnOutputArg(AResult);
  end;
end;

procedure TatDBLibrary.__TParamObjectGetImplementation(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := ObjectToVar(TParamObject(CurrentObject).GetImplementation);
    ReturnOutputArg(AResult);
  end;
end;

procedure TatDBLibrary.__GetTParamObjectIsInstanceOwner(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    ReturnOutputArg(TParamObject(CurrentObject).IsInstanceOwner);
  end;
end;

procedure TatDBLibrary.__TParamDataSetObjectCreate(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := ObjectToVar(TParamDataSetObjectClass(CurrentClass.ClassRef).Create(TDataSet(VarToObject(GetInputArg(0))),GetInputArg(1),GetInputArg(2)));
    ReturnOutputArg(AResult);
  end;
end;

procedure TatDBLibrary.__TParamDataSetObjectDestroy(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    TParamDataSetObject(CurrentObject).Destroy;
  end;
end;

procedure TatDBLibrary.__GetTParamDataSetObjectDataSet(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TParamDataSetObject(CurrentObject).DataSet));
  end;
end;

procedure TatDBLibrary.__TParamParamsObjectCreate(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := ObjectToVar(TParamParamsObjectClass(CurrentClass.ClassRef).Create(TParams(VarToObject(GetInputArg(0))),GetInputArg(1),GetInputArg(2)));
    ReturnOutputArg(AResult);
  end;
end;

procedure TatDBLibrary.__TParamParamsObjectDestroy(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    TParamParamsObject(CurrentObject).Destroy;
  end;
end;

procedure TatDBLibrary.__GetTParamParamsObjectParams(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TParamParamsObject(CurrentObject).Params));
  end;
end;

procedure TatDBLibrary.__TParamStreamObjectCreate(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := ObjectToVar(TParamStreamObjectClass(CurrentClass.ClassRef).Create(TStream(VarToObject(GetInputArg(0))),VarToInteger(GetInputArg(1)),GetInputArg(2),GetInputArg(3)));
    ReturnOutputArg(AResult);
  end;
end;

procedure TatDBLibrary.__TParamStreamObjectDestroy(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    TParamStreamObject(CurrentObject).Destroy;
  end;
end;

procedure TatDBLibrary.__GetTParamStreamObjectStream(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TParamStreamObject(CurrentObject).Stream));
  end;
end;
...
begin
  ...
  begin
    ...
    DefineMethod('SetStream',2,tkNone,nil,__TParamSetStream,false,0,'Stream: TStream; AInstanceOwner: Boolean');
  ...
  begin
    DefineMethod('Create',2,tkClass,TParamObject,__TParamObjectCreate,true,0,'AInstanceOwner: Boolean; AInstanceOwnerSet: Boolean');
    DefineMethod('GetImplementation',0,tkClass,TObject,__TParamObjectGetImplementation,false,0,'');
    DefineProp('IsInstanceOwner',tkVariant,__GetTParamObjectIsInstanceOwner,nil,nil,false,0);
  end;
  With Scripter.DefineClass(TParamDataSetObject) do
  begin
    DefineMethod('Create',3,tkClass,TParamDataSetObject,__TParamDataSetObjectCreate,true,0,'DataSet: TDataSet; AInstanceOwner: Boolean; AInstanceOwnerSet: Boolean');
    DefineMethod('Destroy',0,tkNone,nil,__TParamDataSetObjectDestroy,false,0,'');
    DefineProp('DataSet',tkClass,__GetTParamDataSetObjectDataSet,nil,TDataSet,false,0);
  end;
  With Scripter.DefineClass(TParamParamsObject) do
  begin
    DefineMethod('Create',3,tkClass,TParamParamsObject,__TParamParamsObjectCreate,true,0,'Params: TParams; AInstanceOwner: Boolean; AInstanceOwnerSet: Boolean');
    DefineMethod('Destroy',0,tkNone,nil,__TParamParamsObjectDestroy,false,0,'');
    DefineProp('Params',tkClass,__GetTParamParamsObjectParams,nil,TParams,false,0);
  end;
  With Scripter.DefineClass(TParamStreamObject) do
  begin
    DefineMethod('Create',4,tkClass,TParamStreamObject,__TParamStreamObjectCreate,true,0,'Stream: TStream; AKnownSize: Integer; AInstanceOwner: Boolean; AInstanceOwnerSet: Boolean');
    DefineMethod('Destroy',0,tkNone,nil,__TParamStreamObjectDestroy,false,0,'');
    DefineProp('Stream',tkClass,__GetTParamStreamObjectStream,nil,TStream,false,0);
  end;

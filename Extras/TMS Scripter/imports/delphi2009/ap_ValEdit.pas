...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TValueListEditorClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
AResult := TValueListEditor(CurrentObject).FindRow(VarToStr(GetInputArg(0)),Param1);
    ReturnOutputArg(AResult);
...
begin
  with AMachine do
  begin
AResult := Integer(TValueListEditor(CurrentObject).InsertRow(VarToStr(GetInputArg(0)),VarToStr(GetInputArg(1)),GetInputArg(2)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TValueListEditor(CurrentObject).Cells[VarToInteger(GetArrayIndex(0)),VarToInteger(GetArrayIndex(1))]));
  end;
end;
...
begin
  with AMachine do
  begin
    TValueListEditor(CurrentObject).Cells[VarToInteger(GetArrayIndex(0)),VarToInteger(GetArrayIndex(1))]:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TValueListEditor(CurrentObject).ItemProps[GetArrayIndex(0)]));
  end;
end;
...
begin
  with AMachine do
  begin
    TValueListEditor(CurrentObject).ItemProps[GetArrayIndex(0)]:=TItemProp(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TValueListEditor(CurrentObject).Keys[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    TValueListEditor(CurrentObject).Keys[VarToInteger(GetArrayIndex(0))]:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TValueListEditor(CurrentObject).Values[VarToStr(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    TValueListEditor(CurrentObject).Values[VarToStr(GetArrayIndex(0))]:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TValueListStringsClass(CurrentClass.ClassRef).Create(TValueListEditor(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TValueListStrings(CurrentObject).Assign(TPersistent(VarToObject(GetInputArg(0))));
  end;
end;
...
1: AResult := TValueListStrings(CurrentObject).KeyIsValid(VarToStr(GetInputArg(0)));
2: AResult := TValueListStrings(CurrentObject).KeyIsValid(VarToStr(GetInputArg(0)),GetInputArg(1));
end;
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TValueListStrings(CurrentObject).ItemProps[GetArrayIndex(0)]));
  end;
end;
...
begin
  with AMachine do
  begin
    TValueListStrings(CurrentObject).ItemProps[GetArrayIndex(0)]:=TItemProp(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TItemPropClass(CurrentClass.ClassRef).Create(TValueListEditor(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TValueListEditor,__TValueListEditorCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TValueListEditorDestroy,false,0,'');
    DefineMethod('DeleteRow',1,tkNone,nil,__TValueListEditorDeleteRow,false,0,'ARow: Integer');
    DefineMethod('FindRow',2,tkVariant,nil,__TValueListEditorFindRow,false,0,'KeyName: string; Row: Integer').SetVarArgs([1]);
    DefineMethod('InsertRow',3,tkInteger,nil,__TValueListEditorInsertRow,false,0,'KeyName: string; Value: string; Append: Boolean');
  ...
  begin
    DefineMethod('Create',1,tkClass,TValueListStrings,__TValueListStringsCreate,true,0,'AEditor: TValueListEditor');
    DefineMethod('Destroy',0,tkNone,nil,__TValueListStringsDestroy,false,0,'');
    DefineMethod('Assign',1,tkNone,nil,__TValueListStringsAssign,false,0,'Source: TPersistent');
    DefineMethod('KeyIsValid',2,tkVariant,nil,__TValueListStringsKeyIsValid,false,1,'Key: string; RaiseError: Boolean = True');
    DefineMethod('Clear',0,tkNone,nil,__TValueListStringsClear,false,0,'');
    DefineMethod('Delete',1,tkNone,nil,__TValueListStringsDelete,false,0,'Index: Integer');
    DefineMethod('Exchange',2,tkNone,nil,__TValueListStringsExchange,false,0,'Index1: Integer; Index2: Integer');
  ...
  begin
    DefineMethod('Create',1,tkClass,TItemProp,__TItemPropCreate,true,0,'AEditor: TValueListEditor');

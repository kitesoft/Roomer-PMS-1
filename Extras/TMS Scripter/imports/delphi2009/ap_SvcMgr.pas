...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TEventLoggerClass(CurrentClass.ClassRef).Create(VarToStr(GetInputArg(0))));
...
begin
  ...
  begin
    Case InputArgCount of
      1: TEventLogger(CurrentObject).LogMessage(VarToStr(GetInputArg(0)));
      2: TEventLogger(CurrentObject).LogMessage(VarToStr(GetInputArg(0)),VarToInteger(GetInputArg(1)));
      3: TEventLogger(CurrentObject).LogMessage(VarToStr(GetInputArg(0)),VarToInteger(GetInputArg(1)),VarToInteger(GetInputArg(2)));
      4: TEventLogger(CurrentObject).LogMessage(VarToStr(GetInputArg(0)),VarToInteger(GetInputArg(1)),VarToInteger(GetInputArg(2)),VarToInteger(GetInputArg(3)));
    end;
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TDependenciesClass(CurrentClass.ClassRef).Create(TPersistent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TDependencies(CurrentObject).Items[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    TDependencies(CurrentObject).Items[VarToInteger(GetArrayIndex(0))]:=TDependency(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TServiceThreadClass(CurrentClass.ClassRef).Create(TService(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
case InputArgCount of
1: AResult := ObjectToVar(TServiceClass(CurrentClass.ClassRef).CreateNew(TComponent(VarToObject(GetInputArg(0)))));
2: AResult := ObjectToVar(TServiceClass(CurrentClass.ClassRef).CreateNew(TComponent(VarToObject(GetInputArg(0))),VarToInteger(GetInputArg(1))));
end;
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    Case InputArgCount of
      1: TService(CurrentObject).LogMessage(VarToStr(GetInputArg(0)));
      2: TService(CurrentObject).LogMessage(VarToStr(GetInputArg(0)),VarToInteger(GetInputArg(1)));
      3: TService(CurrentObject).LogMessage(VarToStr(GetInputArg(0)),VarToInteger(GetInputArg(1)),VarToInteger(GetInputArg(2)));
      4: TService(CurrentObject).LogMessage(VarToStr(GetInputArg(0)),VarToInteger(GetInputArg(1)),VarToInteger(GetInputArg(2)),VarToInteger(GetInputArg(3)));
    end;
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TService(CurrentObject).Param[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TService(CurrentObject).ServiceThread));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TService(CurrentObject).Status));
  end;
end;
...
begin
  with AMachine do
  begin
    TService(CurrentObject).Status:=TCurrentStatus(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TServiceApplicationClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TServiceApplication(CurrentObject).Title));
  end;
end;
...
begin
  with AMachine do
  begin
    TServiceApplication(CurrentObject).Title:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(SvcMgr.Application));
  end;
end;
...
begin
  with AMachine do
  begin
    SvcMgr.Application:=TServiceApplication(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TEventLogger,__TEventLoggerCreate,true,0,'Name: String');
    DefineMethod('Destroy',0,tkNone,nil,__TEventLoggerDestroy,false,0,'');
    DefineMethod('LogMessage',4,tkNone,nil,__TEventLoggerLogMessage,false,3,'Message: String; EventType: DWord = 1; Category: Word = 0; ID: DWord = 0');
  end;
  ...
  begin
    DefineMethod('Create',1,tkClass,TDependencies,__TDependenciesCreate,true,0,'Owner: TPersistent');
  ...
  begin
    DefineMethod('Create',1,tkClass,TServiceThread,__TServiceThreadCreate,true,0,'Service: TService');
    DefineMethod('ProcessRequests',1,tkNone,nil,__TServiceThreadProcessRequests,false,0,'WaitForMessage: Boolean');
  end;
  With Scripter.DefineClass(TService) do
  begin
    DefineMethod('CreateNew',2,tkClass,TService,__TServiceCreateNew,true,1,'AOwner: TComponent; Dummy: Integer = 0');
    ...
    DefineMethod('LogMessage',4,tkNone,nil,__TServiceLogMessage,false,3,'Message: String; EventType: DWord = 1; Category: Integer = 0; ID: Integer = 0');
  ...
  begin
    DefineMethod('Create',1,tkClass,TServiceApplication,__TServiceApplicationCreate,true,0,'AOwner: TComponent');

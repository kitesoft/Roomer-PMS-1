...
type
  TatAxCtrlsLibrary = class(TatScripterLibrary)
    procedure __TOleStreamDestroy(AMachine: TatVirtualMachine);
    ...
    procedure __TActiveXControlCreate(AMachine: TatVirtualMachine);
    ...
    procedure __GetTActiveXControlConnectionPoint(AMachine: TatVirtualMachine);
    procedure __TActiveXControlFactoryDestroy(AMachine: TatVirtualMachine);
    procedure __TActiveXControlFactoryCreateControl(AMachine: TatVirtualMachine);
    ...
    procedure __TActiveFormControlCreate(AMachine: TatVirtualMachine);
    procedure __TActiveFormControlDestroy(AMachine: TatVirtualMachine);
    ...
    procedure __TActiveFormCreate(AMachine: TatVirtualMachine);
    procedure __TActiveFormDestroy(AMachine: TatVirtualMachine);
    procedure __GetTActiveFormActiveFormControl(AMachine: TatVirtualMachine);
    procedure __GetTActiveFormConnectionPoint(AMachine: TatVirtualMachine);
    procedure __TActiveFormFactoryDestroy(AMachine: TatVirtualMachine);
    ...
    procedure __TPropertyPageImplDestroy(AMachine: TatVirtualMachine);
    ...
    procedure __TOleGraphicCreate(AMachine: TatVirtualMachine);
...
procedure TatAxCtrlsLibrary.__TOleStreamDestroy(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    TOleStream(CurrentObject).Destroy;
  end;
end;
...
procedure TatAxCtrlsLibrary.__TActiveXControlCreate(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := ObjectToVar(TActiveXControlClass(CurrentClass.ClassRef).Create);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TActiveXControl(CurrentObject).Control));
  end;
end;

procedure TatAxCtrlsLibrary.__GetTActiveXControlConnectionPoint(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TActiveXControl(CurrentObject).ConnectionPoint));
  end;
end;
...
procedure TatAxCtrlsLibrary.__TActiveXControlFactoryCreateControl(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := ObjectToVar(TActiveXControlFactory(CurrentObject).CreateControl(VarToInteger(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TActiveXControlFactory(CurrentObject).AddVerb(VarToInteger(GetInputArg(0)),VarToStr(GetInputArg(1)));
  end;
end;
...
procedure TatAxCtrlsLibrary.__TActiveFormControlCreate(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := ObjectToVar(TActiveFormControlClass(CurrentClass.ClassRef).Create);
    ReturnOutputArg(AResult);
  end;
end;

procedure TatAxCtrlsLibrary.__TActiveFormControlDestroy(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    TActiveFormControl(CurrentObject).Destroy;
  end;
end;
...
procedure TatAxCtrlsLibrary.__TActiveFormCreate(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := ObjectToVar(TActiveFormClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;

procedure TatAxCtrlsLibrary.__TActiveFormDestroy(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    TActiveForm(CurrentObject).Destroy;
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TActiveForm(CurrentObject).ActiveFormControl));
  end;
end;

procedure TatAxCtrlsLibrary.__GetTActiveFormConnectionPoint(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TActiveForm(CurrentObject).ConnectionPoint));
  end;
end;

procedure TatAxCtrlsLibrary.__TActiveFormFactoryDestroy(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    TActiveFormFactory(CurrentObject).Destroy;
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TPropertyPageClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TPropertyPage(CurrentObject).EnumCtlProps(TGUIDWrapper(integer(GetInputArg(0))).ObjToRec,TStrings(VarToObject(GetInputArg(1))));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TPropertyPage(CurrentObject).OleObjects));
  end;
end;
...
begin
  with AMachine do
  begin
    TPropertyPage(CurrentObject).OleObjects:=TInterfaceList(VarToObject(GetInputArg(0)));
  end;
end;

procedure TatAxCtrlsLibrary.__TPropertyPageImplDestroy(AMachine: TatVirtualMachine);
begin
  with AMachine do
  begin
    TPropertyPageImpl(CurrentObject).Destroy;
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TPropertyPageImpl(CurrentObject).PropertyPage));
  end;
end;
...
begin
  with AMachine do
  begin
    TPropertyPageImpl(CurrentObject).PropertyPage:=TPropertyPage(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TActiveXPropertyPage(CurrentObject).PropertyPageImpl));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCustomAdapterClass(CurrentClass.ClassRef).Create);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TAdapterNotifierClass(CurrentClass.ClassRef).Create(TCustomAdapter(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TFontAdapterClass(CurrentClass.ClassRef).Create(TFont(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TPictureAdapterClass(CurrentClass.ClassRef).Create(TPicture(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;

procedure TatAxCtrlsLibrary.__TOleGraphicCreate(AMachine: TatVirtualMachine);
  var
  AResult: variant;
begin
  with AMachine do
  begin
AResult := ObjectToVar(TOleGraphicClass(CurrentClass.ClassRef).Create);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TOleGraphic(CurrentObject).Assign(TPersistent(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    TOleGraphic(CurrentObject).LoadFromFile(VarToStr(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    TOleGraphic(CurrentObject).LoadFromStream(TStream(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    TOleGraphic(CurrentObject).SaveToStream(TStream(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TStringsAdapterClass(CurrentClass.ClassRef).Create(TStrings(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TReflectorWindowClass(CurrentClass.ClassRef).Create(VarToInteger(GetInputArg(0)),TControl(VarToObject(GetInputArg(1)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Destroy',0,tkNone,nil,__TOleStreamDestroy,false,0,'');
    DefineMethod('Read',2,tkInteger,nil,__TOleStreamRead,false,0,'Buffer; Count: Longint').SetVarArgs([0]);
    DefineMethod('Write',2,tkInteger,nil,__TOleStreamWrite,false,0,'Buffer; Count: Longint');
    DefineMethod('Seek',2,tkInteger,nil,__TOleStreamSeek,false,0,'Offset: Longint; Origin: Word');
  end;
  ...
  begin
    DefineMethod('Create',0,tkClass,TActiveXControl,__TActiveXControlCreate,true,0,'');
    ...
    DefineMethod('ObjQueryInterface',2,tkInteger,nil,__TActiveXControlObjQueryInterface,false,0,'IID: TGUID; Obj').SetVarArgs([1]);
    DefineProp('Control',tkClass,__GetTActiveXControlControl,nil,TWinControl,false,0);
    DefineProp('ConnectionPoint',tkClass,__GetTActiveXControlConnectionPoint,nil,TConnectionPoint,false,0);
  end;
  ...
  begin
    ...
    DefineMethod('CreateControl',1,tkClass,TWinControl,__TActiveXControlFactoryCreateControl,false,0,'Parent: HWND');
    DefineMethod('AddVerb',2,tkNone,nil,__TActiveXControlFactoryAddVerb,false,0,'Verb: Integer; VerbName: string');
    DefineMethod('UpdateRegistry',1,tkNone,nil,__TActiveXControlFactoryUpdateRegistry,false,0,'Register: Boolean');
  ...
  begin
    DefineMethod('Create',0,tkClass,TActiveFormControl,__TActiveFormControlCreate,true,0,'');
    DefineMethod('Destroy',0,tkNone,nil,__TActiveFormControlDestroy,false,0,'');
    ...
    DefineMethod('ObjQueryInterface',2,tkInteger,nil,__TActiveFormControlObjQueryInterface,false,0,'IID: TGUID; Obj').SetVarArgs([1]);
  end;
  With Scripter.DefineClass(TActiveForm) do
  begin
    DefineMethod('Create',1,tkClass,TActiveForm,__TActiveFormCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TActiveFormDestroy,false,0,'');
    DefineProp('ActiveFormControl',tkClass,__GetTActiveFormActiveFormControl,nil,TActiveFormControl,false,0);
    DefineProp('ConnectionPoint',tkClass,__GetTActiveFormConnectionPoint,nil,TConnectionPoint,false,0);
  end;
  With Scripter.DefineClass(TActiveFormFactory) do
  begin
    DefineMethod('Destroy',0,tkNone,nil,__TActiveFormFactoryDestroy,false,0,'');
  end;
  With Scripter.DefineClass(TPropertyPage) do
  begin
    DefineMethod('Create',1,tkClass,TPropertyPage,__TPropertyPageCreate,true,0,'AOwner: TComponent');
    ...
    DefineMethod('EnumCtlProps',2,tkNone,nil,__TPropertyPageEnumCtlProps,false,0,'PropType: TGUID; PropNames: TStrings');
  ...
  begin
    DefineMethod('Destroy',0,tkNone,nil,__TPropertyPageImplDestroy,false,0,'');
  ...
  begin
    DefineMethod('Create',1,tkClass,TAdapterNotifier,__TAdapterNotifierCreate,true,0,'Adapter: TCustomAdapter');
  end;
  With Scripter.DefineClass(TFontAdapter) do
  begin
    DefineMethod('Create',1,tkClass,TFontAdapter,__TFontAdapterCreate,true,0,'Font: TFont');
  end;
  With Scripter.DefineClass(TPictureAdapter) do
  begin
    DefineMethod('Create',1,tkClass,TPictureAdapter,__TPictureAdapterCreate,true,0,'Picture: TPicture');
  end;
  With Scripter.DefineClass(TOleGraphic) do
  begin
    DefineMethod('Create',0,tkClass,TOleGraphic,__TOleGraphicCreate,true,0,'');
    DefineMethod('Assign',1,tkNone,nil,__TOleGraphicAssign,false,0,'Source: TPersistent');
    DefineMethod('LoadFromFile',1,tkNone,nil,__TOleGraphicLoadFromFile,false,0,'Filename: string');
    DefineMethod('LoadFromStream',1,tkNone,nil,__TOleGraphicLoadFromStream,false,0,'Stream: TStream');
    DefineMethod('SaveToStream',1,tkNone,nil,__TOleGraphicSaveToStream,false,0,'Stream: TStream');
    DefineMethod('LoadFromClipboardFormat',3,tkNone,nil,__TOleGraphicLoadFromClipboardFormat,false,0,'AFormat: Word; AData: THandle; APalette: HPALETTE');
    DefineMethod('SaveToClipboardFormat',3,tkNone,nil,__TOleGraphicSaveToClipboardFormat,false,0,'AFormat: Word; AData: THandle; APalette: HPALETTE').SetVarArgs([0,1,2]);
  ...
  begin
    DefineMethod('Create',1,tkClass,TStringsAdapter,__TStringsAdapterCreate,true,0,'Strings: TStrings');
  end;
  With Scripter.DefineClass(TReflectorWindow) do
  begin
    DefineMethod('Create',2,tkClass,TReflectorWindow,__TReflectorWindowCreate,true,0,'ParentWindow: HWND; Control: TControl');
  end;

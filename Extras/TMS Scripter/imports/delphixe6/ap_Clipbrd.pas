...
type
  ...
  EClipboardExceptionClass = class of EClipboardException;
...
begin
  with AMachine do
  begin
    TClipboard(CurrentObject).Assign(TPersistent(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TClipboard(CurrentObject).GetComponent(TComponent(VarToObject(GetInputArg(0))),TComponent(VarToObject(GetInputArg(1)))));
...
begin
  with AMachine do
  begin
    TClipboard(CurrentObject).SetComponent(TComponent(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TClipboard(CurrentObject).AsText));
  end;
end;
...
begin
  with AMachine do
  begin
    TClipboard(CurrentObject).AsText:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(Clipbrd.Clipboard);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(Clipbrd.SetClipboard(TClipboard(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    ...
    DefineMethod('Assign',1,tkNone,nil,__TClipboardAssign,false,0,'Source: TPersistent');
    DefineMethod('Clear',0,tkNone,nil,__TClipboardClear,false,0,'');
    DefineMethod('Close',0,tkNone,nil,__TClipboardClose,false,0,'');
    DefineMethod('GetComponent',2,tkClass,TComponent,__TClipboardGetComponent,false,0,'Owner: TComponent; Parent: TComponent');
    DefineMethod('GetAsHandle',1,tkInteger,nil,__TClipboardGetAsHandle,false,0,'Format: Word');
    DefineMethod('HasFormat',1,tkVariant,nil,__TClipboardHasFormat,false,0,'Format: Word');
    DefineMethod('Open',0,tkNone,nil,__TClipboardOpen,false,0,'');
    DefineMethod('SetComponent',1,tkNone,nil,__TClipboardSetComponent,false,0,'Component: TComponent');
    DefineMethod('SetAsHandle',2,tkNone,nil,__TClipboardSetAsHandle,false,0,'Format: Word; Value: THandle');
    DefineMethod('GetTextBuf',2,tkInteger,nil,__TClipboardGetTextBuf,false,0,'Buffer: PChar; BufSize: Integer');
    DefineMethod('SetTextBuf',1,tkNone,nil,__TClipboardSetTextBuf,false,0,'Buffer: PChar');
  ...
  With Scripter.DefineClass(EClipboardException) do
  begin
  end;
  With Scripter.DefineClass(ClassType) do
  begin
    DefineMethod('Clipboard',0,tkClass,TClipboard,__Clipboard,false,0,'');
    DefineMethod('SetClipboard',1,tkClass,TClipboard,__SetClipboard,false,0,'NewClipboard: TClipboard');

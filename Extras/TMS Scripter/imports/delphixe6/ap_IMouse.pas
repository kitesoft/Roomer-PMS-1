...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TPanningWindowClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
AResult := TPanningWindow(CurrentObject).StartPanning(VarToInteger(GetInputArg(0)),TControl(VarToObject(GetInputArg(1))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TPanningWindow(CurrentObject).PanControl));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := IMouse.StartPan(VarToInteger(GetInputArg(0)),TControl(VarToObject(GetInputArg(1))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TPanningWindow,__TPanningWindowCreate,true,0,'AOwner: TComponent');
    ...
    DefineMethod('StartPanning',2,tkVariant,nil,__TPanningWindowStartPanning,false,0,'AHandle: THandle; AControl: TControl');
  ...
  begin
    DefineMethod('StartPan',2,tkVariant,nil,__StartPan,false,0,'WndHandle: THandle; AControl: TControl');

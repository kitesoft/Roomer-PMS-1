...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TServerEventDispatchClass(CurrentClass.ClassRef).Create(TOleServer(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TOleServerClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;

...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TMediaPlayerClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TMediaPlayer(CurrentObject).ErrorMessage));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TMediaPlayer(CurrentObject).Mode));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TMediaPlayer(CurrentObject).NotifyValue));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TMediaPlayer(CurrentObject).TimeFormat));
  end;
end;
...
begin
  with AMachine do
  begin
    TMediaPlayer(CurrentObject).TimeFormat:=TMPTimeFormats(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TMediaPlayer,__TMediaPlayerCreate,true,0,'AOwner: TComponent');

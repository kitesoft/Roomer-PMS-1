...
begin
  with AMachine do
  begin
    TDockCaptionDrawer(CurrentObject).DrawDockCaption(TCanvas(VarToObject(GetInputArg(0))),TRectWrapper(integer(GetInputArg(1))).ObjToRec,TParentFormStateWrapper(integer(GetInputArg(2))).ObjToRec);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TDockCaptionDrawerClass(CurrentClass.ClassRef).Create(TDockCaptionOrientation(VarToInteger(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TDockCaptionDrawer(CurrentObject).DockCaptionPinButton));
  end;
end;
...
begin
  with AMachine do
  begin
    TDockCaptionDrawer(CurrentObject).DockCaptionPinButton:=TDockCaptionPinButton(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCaptionedDockTreeClass(CurrentClass.ClassRef).Create(TWinControl(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := integer(TParentFormStateWrapper.Create(TCaptionedDockTreeClass(CurrentClass.ClassRef).GetParentFormState(TControl(VarToObject(GetInputArg(0))))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    DefineMethod('DrawDockCaption',3,tkNone,nil,__TDockCaptionDrawerDrawDockCaption,false,0,'Canvas: TCanvas; CaptionRect: TRect; State: TParentFormState');
    DefineMethod('DockCaptionHitTest',2,tkInteger,nil,__TDockCaptionDrawerDockCaptionHitTest,false,0,'CaptionRect: TRect; MousePos: TPoint');
    DefineMethod('Create',1,tkClass,TDockCaptionDrawer,__TDockCaptionDrawerCreate,true,0,'DockCaptionOrientation: TDockCaptionOrientation');
  ...
  begin
    DefineMethod('Create',1,tkClass,TCaptionedDockTree,__TCaptionedDockTreeCreate,true,0,'DockSite: TWinControl');
    DefineMethod('Destroy',0,tkNone,nil,__TCaptionedDockTreeDestroy,false,0,'');
    DefineMethod('GetParentFormState',1,tkVariant,nil,__TCaptionedDockTreeGetParentFormState,true,0,'Control: TControl');
  end;

...
begin
  with AMachine do
  begin
    THTTPServer(CurrentObject).WebModuleCreate(TObject(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    THTTPServer(CurrentObject).WebModuleDestroy(TObject(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(HTTPIntr.HTTPServer));
  end;
end;
...
begin
  with AMachine do
  begin
    HTTPIntr.HTTPServer:=THTTPServer(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('WebModuleCreate',1,tkNone,nil,__THTTPServerWebModuleCreate,false,0,'Sender: TObject');
    DefineMethod('WebModuleDestroy',1,tkNone,nil,__THTTPServerWebModuleDestroy,false,0,'Sender: TObject');
  end;
  With Scripter.DefineClass(ClassType) do
  begin
    DefineMethod('TerminateExtension',1,tkVariant,nil,__TerminateExtension,false,0,'dwFlags: DWORD');

...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TColorGridClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
AResult := Integer(TColorGrid(CurrentObject).ColorToIndex(TColor(VarToInteger(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TColorGrid(CurrentObject).ForegroundColor));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TColorGrid(CurrentObject).BackgroundColor));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TColorGrid,__TColorGridCreate,true,0,'AOwner: TComponent');
    DefineMethod('ColorToIndex',1,tkInteger,nil,__TColorGridColorToIndex,false,0,'AColor: TColor');

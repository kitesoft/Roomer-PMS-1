...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TComServerClass(CurrentClass.ClassRef).Create);
...
begin
  with AMachine do
  begin
    TComServer(CurrentObject).SetServerName(VarToStr(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TComServer(CurrentObject).StartMode));
  end;
end;
...
AResult := Integer(ComServ.DllGetClassObject(TGUIDWrapper(integer(GetInputArg(0))).ObjToRec,TGUIDWrapper(integer(GetInputArg(1))).ObjToRec,Param2));
    ReturnOutputArg(AResult);
...
begin
  with AMachine do
  begin
AResult := Integer(ComServ.DllCanUnloadNow);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(ComServ.DllRegisterServer);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(ComServ.DllUnregisterServer);
    ReturnOutputArg(AResult);
  end;
end;
...
AResult := Integer(ComServ.DllInstall(GetInputArg(0),StringToWideChar(VarToStr(GetInputArg(1)),@Param1Buf[0],Length(Param1Buf))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(ComServ.ComServer));
  end;
end;
...
begin
  with AMachine do
  begin
    ComServ.ComServer:=TComServer(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  ...
  begin
    ...
    DefineMethod('SetServerName',1,tkNone,nil,__TComServerSetServerName,false,0,'Name: string');
    DefineMethod('UpdateRegistry',1,tkNone,nil,__TComServerUpdateRegistry,false,0,'Register: Boolean');
  ...
  begin
    DefineMethod('DllGetClassObject',3,tkInteger,nil,__DllGetClassObject,false,0,'CLSID: TGUID; IID: TGUID; Obj').SetVarArgs([2]);
    ...
    DefineMethod('DllInstall',2,tkInteger,nil,__DllInstall,false,0,'bInstall: WordBool; pszCmdLine: LPCWSTR');

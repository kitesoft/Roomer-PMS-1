...
begin
  with AMachine do
  begin
AResult := Winsafer.SaferCloseLevel(VarToInteger(GetInputArg(0)));
...
AResult := Winsafer.SaferGetLevelInformation(VarToInteger(GetInputArg(0)),VarToInteger(GetInputArg(1)),VarToInteger(GetInputArg(2)),Param3);
    ReturnOutputArg(AResult);
...
AResult := Winsafer.SaferiIsExecutableFileType(StringToWideChar(VarToStr(GetInputArg(0)),@Param0Buf[0],Length(Param0Buf)),GetInputArg(1));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  ...
  begin
    DefineMethod('SaferCloseLevel',1,tkVariant,nil,__SaferCloseLevel,false,0,'hLevelHandle: THandle');
    DefineMethod('SaferGetLevelInformation',4,tkVariant,nil,__SaferGetLevelInformation,false,0,'LevelHandle: THandle; dwInfoType: SAFER_OBJECT_INFO_CLASS; dwInBufferSize: DWORD; lpdwOutBufferSize: DWORD').SetVarArgs([3]);
    DefineMethod('SaferiIsExecutableFileType',2,tkVariant,nil,__SaferiIsExecutableFileType,false,0,'szFullPathname: LPCWSTR; bFromShellExecute: BOOLEAN');

...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TJPEGImageClass(CurrentClass.ClassRef).Create);
...
begin
  with AMachine do
  begin
    TJPEGImage(CurrentObject).Assign(TPersistent(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    TJPEGImage(CurrentObject).LoadFromStream(TStream(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    TJPEGImage(CurrentObject).SaveToStream(TStream(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    TJPEGImage(CurrentObject).LoadFromClipboardFormat(VarToInteger(GetInputArg(0)),VarToInteger(GetInputArg(1)),VarToInteger(GetInputArg(2)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TJPEGImage(CurrentObject).CompressionQuality));
  end;
end;
...
begin
  with AMachine do
  begin
    TJPEGImage(CurrentObject).CompressionQuality:=TJPEGQualityRange(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TJPEGImage(CurrentObject).PixelFormat));
  end;
end;
...
begin
  with AMachine do
  begin
    TJPEGImage(CurrentObject).PixelFormat:=TJPEGPixelFormat(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TJPEGImage(CurrentObject).Performance));
  end;
end;
...
begin
  with AMachine do
  begin
    TJPEGImage(CurrentObject).Performance:=TJPEGPerformance(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TJPEGImage(CurrentObject).Scale));
  end;
end;
...
begin
  with AMachine do
  begin
    TJPEGImage(CurrentObject).Scale:=TJPEGScale(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  ...
  begin
    ...
    DefineMethod('Assign',1,tkNone,nil,__TJPEGImageAssign,false,0,'Source: TPersistent');
    DefineMethod('LoadFromStream',1,tkNone,nil,__TJPEGImageLoadFromStream,false,0,'Stream: TStream');
    DefineMethod('SaveToStream',1,tkNone,nil,__TJPEGImageSaveToStream,false,0,'Stream: TStream');
    DefineMethod('LoadFromClipboardFormat',3,tkNone,nil,__TJPEGImageLoadFromClipboardFormat,false,0,'AFormat: Word; AData: THandle; APalette: HPALETTE');
    DefineMethod('SaveToClipboardFormat',3,tkNone,nil,__TJPEGImageSaveToClipboardFormat,false,0,'AFormat: Word; AData: THandle; APalette: HPALETTE').SetVarArgs([0,1,2]);

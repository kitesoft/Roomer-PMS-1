...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTabPageClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TTabbedNotebookClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := Integer(TTabbedNotebook(CurrentObject).GetIndexForPage(VarToStr(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    TTabbedNotebook(CurrentObject).TabFontChanged(TObject(VarToObject(GetInputArg(0))));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TTabbedNotebook(CurrentObject).TopFont));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TTabPage,__TTabPageCreate,true,0,'AOwner: TComponent');
  end;
  With Scripter.DefineClass(TTabbedNotebook) do
  begin
    DefineMethod('CMTabFontChanged',1,tkNone,nil,__TTabbedNotebookCMTabFontChanged,false,0,'Message: TMessage').SetVarArgs([0]);
    DefineMethod('Create',1,tkClass,TTabbedNotebook,__TTabbedNotebookCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TTabbedNotebookDestroy,false,0,'');
    DefineMethod('GetIndexForPage',1,tkInteger,nil,__TTabbedNotebookGetIndexForPage,false,0,'PageName: string');
    DefineMethod('TabFontChanged',1,tkNone,nil,__TTabbedNotebookTabFontChanged,false,0,'Sender: TObject');

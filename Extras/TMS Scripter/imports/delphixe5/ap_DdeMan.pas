...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TDdeClientConvClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
AResult := TDdeClientConv(CurrentObject).SetLink(VarToStr(GetInputArg(0)),VarToStr(GetInputArg(1)));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := TDdeClientConv(CurrentObject).PokeDataLines(VarToStr(GetInputArg(0)),TStrings(VarToObject(GetInputArg(1))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := TDdeClientConv(CurrentObject).ExecuteMacroLines(TStrings(VarToObject(GetInputArg(0))),GetInputArg(1));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := TDdeClientConv(CurrentObject).PokeData(VarToStr(GetInputArg(0)),PAnsiChar(AnsiString(VarToStr(GetInputArg(1)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := TDdeClientConv(CurrentObject).ExecuteMacro(PAnsiChar(AnsiString(VarToStr(GetInputArg(0)))),GetInputArg(1));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := string(TDdeClientConv(CurrentObject).RequestData(VarToStr(GetInputArg(0))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TDdeClientConv(CurrentObject).DataMode));
  end;
end;
...
begin
  with AMachine do
  begin
    TDdeClientConv(CurrentObject).DataMode:=TDataMode(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TDdeClientItemClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TDdeServerConvClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TDdeServerItemClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TDdeMgrClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
AResult := string(TDdeMgr(CurrentObject).GetExeName);
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(string(TDdeMgr(CurrentObject).AppName));
  end;
end;
...
begin
  with AMachine do
  begin
    TDdeMgr(CurrentObject).AppName:=VarToStr(GetInputArg(0));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(DdeMan.ddeMgr));
  end;
end;
...
begin
  with AMachine do
  begin
    DdeMan.ddeMgr:=TDdeMgr(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TDdeClientConv,__TDdeClientConvCreate,true,0,'AOwner: TComponent');
    ...
    DefineMethod('SetLink',2,tkVariant,nil,__TDdeClientConvSetLink,false,0,'Service: string; Topic: string');
    ...
    DefineMethod('PokeDataLines',2,tkVariant,nil,__TDdeClientConvPokeDataLines,false,0,'Item: string; Data: TStrings');
    DefineMethod('ExecuteMacroLines',2,tkVariant,nil,__TDdeClientConvExecuteMacroLines,false,0,'Cmd: TStrings; waitFlg: Boolean');
    DefineMethod('PokeData',2,tkVariant,nil,__TDdeClientConvPokeData,false,0,'Item: string; Data: PAnsiChar');
    DefineMethod('ExecuteMacro',2,tkVariant,nil,__TDdeClientConvExecuteMacro,false,0,'Cmd: PAnsiChar; waitFlg: Boolean');
    DefineMethod('RequestData',1,tkVariant,nil,__TDdeClientConvRequestData,false,0,'Item: string');
  ...
  begin
    DefineMethod('Create',1,tkClass,TDdeClientItem,__TDdeClientItemCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TDdeClientItemDestroy,false,0,'');
  end;
  With Scripter.DefineClass(TDdeServerConv) do
  begin
    DefineMethod('Create',1,tkClass,TDdeServerConv,__TDdeServerConvCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TDdeServerConvDestroy,false,0,'');
    DefineMethod('ExecuteMacro',1,tkInteger,nil,__TDdeServerConvExecuteMacro,false,0,'Data: HDdeData');
  end;
  With Scripter.DefineClass(TDdeServerItem) do
  begin
    DefineMethod('Create',1,tkClass,TDdeServerItem,__TDdeServerItemCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TDdeServerItemDestroy,false,0,'');
    DefineMethod('PokeData',1,tkInteger,nil,__TDdeServerItemPokeData,false,0,'Data: HDdeData');
  ...
  begin
    DefineMethod('Create',1,tkClass,TDdeMgr,__TDdeMgrCreate,true,0,'AOwner: TComponent');
  ...
  begin
    DefineMethod('GetPasteLinkInfo',3,tkVariant,nil,__GetPasteLinkInfo,false,0,'Service: string; Topic: string; Item: string').SetVarArgs([0,1,2]);

...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TCheckListBoxClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  ...
  begin
    Case InputArgCount of
      1: TCheckListBox(CurrentObject).CheckAll(TCheckBoxState(VarToInteger(GetInputArg(0))));
      2: TCheckListBox(CurrentObject).CheckAll(TCheckBoxState(VarToInteger(GetInputArg(0))),GetInputArg(1));
      3: TCheckListBox(CurrentObject).CheckAll(TCheckBoxState(VarToInteger(GetInputArg(0))),GetInputArg(1),GetInputArg(2));
    end;
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TCheckListBox(CurrentObject).State[VarToInteger(GetArrayIndex(0))]));
  end;
end;
...
begin
  with AMachine do
  begin
    TCheckListBox(CurrentObject).State[VarToInteger(GetArrayIndex(0))]:=TCheckBoxState(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TCheckListBox,__TCheckListBoxCreate,true,0,'AOwner: TComponent');
    DefineMethod('Destroy',0,tkNone,nil,__TCheckListBoxDestroy,false,0,'');
    DefineMethod('CheckAll',3,tkNone,nil,__TCheckListBoxCheckAll,false,2,'AState: TCheckBoxState; AllowGrayed: Boolean = True; AllowDisabled: Boolean = True');

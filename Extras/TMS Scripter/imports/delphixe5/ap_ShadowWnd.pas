...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TShadowWindowClass(CurrentClass.ClassRef).Create(TComponent(VarToObject(GetInputArg(0)))));
...
begin
  with AMachine do
  begin
AResult := ObjectToVar(TShadowWindowClass(CurrentClass.ClassRef).CreateShadow(TComponent(VarToObject(GetInputArg(0))),TControlSide(VarToInteger(GetInputArg(1)))));
    ReturnOutputArg(AResult);
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(ObjectToVar(TShadowWindow(CurrentObject).Control));
  end;
end;
...
begin
  with AMachine do
  begin
    TShadowWindow(CurrentObject).Control:=TControl(VarToObject(GetInputArg(0)));
  end;
end;
...
begin
  with AMachine do
  begin
    ReturnOutputArg(Integer(TShadowWindow(CurrentObject).Side));
  end;
end;
...
begin
  with AMachine do
  begin
    TShadowWindow(CurrentObject).Side:=TControlSide(VarToInteger(GetInputArg(0)));
  end;
end;
...
begin
  ...
  begin
    DefineMethod('Create',1,tkClass,TShadowWindow,__TShadowWindowCreate,true,0,'AOwner: TComponent');
    DefineMethod('CreateShadow',2,tkClass,TShadowWindow,__TShadowWindowCreateShadow,true,0,'AOwner: TComponent; ControlSide: TControlSide');
    DefineMethod('Destroy',0,tkNone,nil,__TShadowWindowDestroy,false,0,'');
    DefineMethod('SetBounds',4,tkNone,nil,__TShadowWindowSetBounds,false,0,'ALeft: Integer; ATop: Integer; AWidth: Integer; AHeight: Integer');

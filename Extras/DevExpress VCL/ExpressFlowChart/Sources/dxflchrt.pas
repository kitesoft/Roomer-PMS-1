...
begin
  ...
  begin
    ...
    if cxPtInRegion(ARegion, P) then
...
begin
  ...
  begin
    ADC := Owner.Canvas.Handle;
    APrevPen := SelectObject(ADC, GetStockObject(BLACK_PEN));
    APrevBrush := SelectObject(ADC, GetStockObject(BLACK_BRUSH));
    try
      APoints := @FPoints.List^[0];
      for I := 0 to FCounts.Count - 1 do
      begin
        ACount := Integer(FCounts[I]);
        Polygon(ADC, APoints^, ACount);
        Inc(APoints, ACount);
      end;
    finally
      SelectObject(ADC, APrevBrush);
      SelectObject(ADC, APrevPen);
    end;
  end;
end;
...
begin
  Result := (Zoom > 0) and (ssCtrl in AShift);
end;
...
begin
  ...
  P := Point(X, Y);
  if IsScrollBarsArea(P) then Exit;
...
procedure TdxFlowChartDragHelper.DoDragMove(X, Y: Integer; State: TDragState);
  ...
  procedure MoveObjects(ADragStartPoint, ADragFinishPoint: TPoint);
  begin
    ADragStartPoint := Owner.ChartPoint(ADragStartPoint);
    ADragFinishPoint := Owner.ChartPoint(ADragFinishPoint);
    Owner.MoveObjects(ADragFinishPoint.X - ADragStartPoint.X,
      ADragFinishPoint.Y - ADragStartPoint.Y);
  end;
  ...
  begin
    if DynamicMoving then
      MoveObjects(FMobile, Point(X, Y))
    else
    begin
      DrawRgn;
      OffsetRgn(FRegion, X - FMobile.X, Y - FMobile.Y);
      DrawRgn;
    end;
    FMobile := Point(X, Y);
  end;
...
procedure TdxFlowChartDragHelper.DoDragPoint(X, Y: Integer; State: TDragState);
  ...
  begin
    ...
    if CanRemovePoint(X, Y) then
      ...
      FDragConnection.Points[AIndex] := Owner.ChartPoint(X, Y);
  end;
...
begin
  RealFont.Size := Max(MulDiv(Font.Size, Owner.RealZoom, 100), 1);
end;

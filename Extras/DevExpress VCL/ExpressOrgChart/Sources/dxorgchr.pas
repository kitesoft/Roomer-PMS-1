...
{$ENDIF}
  ...
  cxLookAndFeels, cxLookAndFeelPainters, dxGDIPlusAPI, dxGDIPlusClasses, dxTouch;
...
type
  ...
  TdxCustomOrgChart = class(TcxControl, IdxSkinSupport, IdxRotateClient)
  private
  ...
  TdxOrgChartCustomCustomizeForm = class(TForm)
  protected
  ...
  TdxOrgChartCustomizeFormManager = class(TObject)
  private
    FList: TList;
  public
...
begin
  ...
  if Result then
  begin
    with TdxOrgChartCustomCustomizeFormClass(FList.Last).Create(nil) do
    try
...
procedure TdxCustomOrgChart.InitScrollBarsParameters;

  function DoSetScrollBarInfo(AKind: TScrollBarKind; APage, AMax: Integer): Integer;
  var
    AScrollBar: IcxControlScrollBar;
  begin
    AScrollBar := GetScrollBar(AKind);
    if Assigned(AScrollBar) and not IsZoomed then
    ...
    SetScrollBarInfo(AKind, 0, AMax, 1, APage, Result, True, True);
  end;
...
var
  ANewX, ANewY: Integer;
begin
  ANewX := DoSetScrollBarInfo(sbHorizontal, ClientBoundsWidth, FullWidth);
  ANewY := DoSetScrollBarInfo(sbVertical, ClientBoundsHeight, FullHeight);
...
function TdxCustomOrgChart.PaintNode(ACanvas: TcxCanvas;
  ...
  begin
    ...
    LookAndFeelPainter.DrawExpandButton(ACanvas, ARect, Value);
  end;
...
begin
  ...
  if HasButton(ANode) then
    AOffset := Max(0, LookAndFeelPainter.ExpandButtonSize)
  else
    AOffset := 0;
...
begin
  ...
  begin
    ...
    begin
      AcxCanvas := TcxCanvas.Create(ACanvas);
      try
        ABitmapRect := cxRectBounds(P.X, P.Y, DoZoom(Images.Width), DoZoom(Images.Height));
        cxDrawImage(AcxCanvas.Handle, ABitmapRect, ABitmapRect, nil, Images, Node.ImageIndex, idmNormal, (ocImageFiltering in Options));
      finally
        FreeAndNil(AcxCanvas);
      end;
    end
    else
      Images.Draw(ACanvas, P.X, P.Y, Node.ImageIndex)
  end;
...
begin
  Adjust(FullWidth - ClientBoundsWidth, ALeft);
  Adjust(FullHeight - ClientBoundsHeight, ATop);
  ...
  begin
    ...
    begin
      if HScrollBar <> nil then
        HScrollBar.Position := LeftEdge;
      if VScrollBar <> nil then
        VScrollBar.Position := TopEdge;
      UpdateScrollBars;
      Invalidate;
      Update;
    end;
...
begin
  ...
  begin
    ...
    if X >= ClientBoundsWidth - OtScrollUnit then NewX := OtScrollUnit;
    if Y <= OtScrollUnit then NewY := -OtScrollUnit;
    if Y >= ClientBoundsHeight - OtScrollUnit then NewY := OtScrollUnit;
...
procedure TdxCustomOrgChart.DragOver(Source: TObject;
...
begin
  ...
  begin
    ...
    Color := Painter.GetNodeColor(Selected.Color, False);
    Font := Self.Font;
    Font.Color := Painter.GetNodeTextColor(False);
...
procedure TdxCustomOrgChart.Scroll(AScrollBarKind: TScrollBarKind;
  AScrollCode: TScrollCode; var AScrollPos: Integer);
const
  CodeHorzMap: array[TScrollCode] of TdxOcNavigate =
    (ocnLineLeft, ocnLineRight, ocnPageLeft, ocnPageRight,
     ocnLeftPosition, ocnLeftPosition, ocnLeft, ocnRight, ocnLeftPosition);
  CodeVertMap: array[TScrollCode] of TdxOcNavigate =
    (ocnLineUp, ocnLineDown, ocnPageUp, ocnPageDown, ocnTopPosition,
     ocnTopPosition, ocnTop, ocnBottom, ocnTopPosition);
begin
  case AScrollBarKind of
    sbHorizontal:
      DoNavigate(CodeHorzMap[AScrollCode], AScrollPos);
    sbVertical:
      DoNavigate(CodeVertMap[AScrollCode], AScrollPos);
  end;
end;
...
procedure TdxOrgChartGdiPlusPainter.DrawFrameDiamond(
...
begin
  APoints[0].X := R.Left;
  APoints[0].Y := R.Top + (R.Bottom - R.Top) shr 1;
  APoints[1].X := R.Left + (R.Right - R.Left) shr 1;
  APoints[1].Y := R.Top;
  ...
  try
    ...
    AGraphics.Polygon(APoints, GetLineColor, GetBrushColor(ACanvas), Chart.LineWidth);
  finally
    dxGpEndPaint(AGraphics);
  end;
end;

procedure TdxOrgChartGdiPlusPainter.DrawFrameEllipse(
  ACanvas: TcxCanvas; ANode: TdxOcNode; const R: TRect);
var
  AGraphics: TdxGPGraphics;
begin
  ...
  try
    AGraphics.SmoothingMode := smAntiAlias;
    AGraphics.Ellipse(R, GetLineColor, GetBrushColor(ACanvas), Chart.LineWidth);
  finally
    dxGpEndPaint(AGraphics);
  end;
end;

procedure TdxOrgChartGdiPlusPainter.DrawFrameRectangle(
...
begin
  ...
  try
    ...
    if ARect3D then
      AGraphics.Rectangle(
        cxRectOffset(R, AShadowSize, AShadowSize),
        GetLineColor, clNone, Chart.LineWidth, psSolid, 200);
    AGraphics.Rectangle(R, GetLineColor, GetBrushColor(ACanvas), Chart.LineWidth);
  finally
    AGraphics.Free;
  end;
end;

procedure TdxOrgChartGdiPlusPainter.DrawFrameRoundRect(
...
begin
  ...
  try
    ...
    AGraphics.RoundRect(R, GetLineColor, GetBrushColor(ACanvas),
      ARadius div 2, ARadius div 2, Chart.LineWidth);
  finally
    dxGpEndPaint(AGraphics);
  end;
end;

procedure TdxOrgChartGdiPlusPainter.DrawLine(
  ACanvas: TcxCanvas; X1, Y1, X2, Y2: Integer; AWidth: Single);
var
  AGraphics: TdxGPGraphics;
begin
  AGraphics := dxGpBeginPaint(ACanvas.Handle,
    cxRect(Min(X1, X2), Min(Y1, Y2), Max(X1, X2), Max(Y1, Y2)));
  try
    AGraphics.SmoothingMode := smAntiAlias;
    AGraphics.Line(X1, Y1, X2, Y2, GetLineColor, AWidth);
  finally
    dxGpEndPaint(AGraphics);
  end;
end;
...
begin
  ...
  ATopY := R.Top;
  ALeftX := R.Left;
  if R.Right > Owner.ClientBoundsWidth then
    Inc(ALeftX, Owner.ClientBoundsWidth - R.Right);
  if R.Bottom > Owner.ClientBoundsHeight then
    Inc(ATopY, Owner.ClientBoundsHeight - R.Bottom);
  ALeftX := Max(ALeftX, 0);
  ATopY := Max(ATopY, 0);

  if (ALeftX <> R.Left) or (ATopY <> R.Top) then
    Owner.SetLeftTop(Owner.LeftEdge + R.Left - ALeftX, Owner.TopEdge + R.Top - ATopY);
end;
...
begin
  APreview.SetLeftTop(0,0);
  APreview.Antialiasing := ASource.Antialiasing;
  APreview.LookAndFeel.MasterLookAndFeel := ASource.LookAndFeel;
  APreview.DefaultNodeWidth := ASource.DefaultNodeWidth;
  APreview.DefaultNodeHeight := ASource.DefaultNodeHeight;
  APreview.DefaultImageAlign := ASource.DefaultImageAlign;
  APreview.LineWidth := ASource.LineWidth;
  APreview.IndentX := ASource.IndentX;
  APreview.IndentY := ASource.IndentY;
...
begin
  if ASelected then
  begin
    Result := Chart.SelectedNodeColor;
    if Result = clDefault then
      Result := LookAndFeelPainter.DefaultSelectionColor;
  end
  else
    if ACurrectColor = clDefault then
      Result := GetBackgroundColor
    else
      Result := ACurrectColor;
end;

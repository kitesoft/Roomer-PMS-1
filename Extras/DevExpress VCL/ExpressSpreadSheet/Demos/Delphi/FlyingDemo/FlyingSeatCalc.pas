...
begin
  ...
  begin
    fLeftForm := GetFormula(efLsPrime, efLsLbs, rbLsKgs,
                            rbLsLbs, '', ' / 2' + dxFormatSettings.DecimalSeparator + '2');        // Get the Left hand seat formula
    fRightForm := GetFormula(efRsPrime, efRsLbs, rbRsKgs,
                             rbRsLbs, '', ' / 2' + dxFormatSettings.DecimalSeparator + '2');       // Get the right hand seta formula
  end
  else
  begin                                                    // Result in Lbs
    fLeftForm := GetFormula(efLsPrime, efLsLbs, rbLsKgs,
                            rbLsLbs, ' * 2' + dxFormatSettings.DecimalSeparator + '2', '');        // Get Left seat
    fRightForm := GetFormula(efRsPrime, efRsLbs, rbRsKgs,
                             rbRsLbs, ' * 2' + dxFormatSettings.DecimalSeparator + '2', '');       // Get right seat
  end;

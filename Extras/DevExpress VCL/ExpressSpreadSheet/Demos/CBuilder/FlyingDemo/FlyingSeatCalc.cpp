...
void GetValidChar(char &Key, String AText)      // Get a valid numeric char
{
  ...
  if ((((Key == dxFormatSettings->DecimalSeparator) && (AText.Pos(dxFormatSettings->DecimalSeparator) > 0))) || // Scan for a valid Key
    ((Key != dxFormatSettings->DecimalSeparator) && (StrScan(NumKeys, Key) == NULL)))

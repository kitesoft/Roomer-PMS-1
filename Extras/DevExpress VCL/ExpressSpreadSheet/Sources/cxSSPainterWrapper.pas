...
uses
  Types, SysUtils, Classes, Windows, Graphics,
  dxCore, cxGraphics, cxGeometry, cxExcelConst, cxSSTypes;
...
type
  ...
  protected
    ...
    procedure DrawLine(const AVertex: array of TRect;
      AStyle: TcxSSEdgeLineStyle; AOrientation: TdxOrientation);
  ...
  public
    ...
    procedure Line(const AVertex: TRect; AStyle: TcxSSEdgeLineStyle;
      AOrientation: TdxOrientation; ABkColor, AFgColor: TColor); overload;
    procedure Line(const AVertex: array of TRect; AStyle: TcxSSEdgeLineStyle;
      AOrientation: TdxOrientation; ABkColor, AFgColor: Word; IsSelected: Boolean); overload;
    ...
    procedure SelectFonT(AFont: TFont); overload; // SelectFont conflicts with C++ macro
    procedure SelectFonT(const AFont: TcxFontHandle; ACheckSettings: Boolean = True); overload; // SelectFont conflicts with C++ macro
    procedure SelectFonT(const AFont: TcxFontHandle; AColor: Word); overload; // SelectFont conflicts with C++ macro
...
var
  APenBrushes: array[0..9, TdxOrientation] of TcxFillBrushHandle;
  DefaultPens: array[0..16, TdxOrientation] of TcxFillBrushHandle;
...
function CheckText(const AText: string): string;
begin
  Result := StringReplace(AText, #160, ' ', [rfReplaceAll, rfIgnoreCase]);
end;
...
begin
  Result := cxPoint(cxTextExtent(FFontHandle, AText));
end;
...
procedure TcxCanvasWrapper.Line(const AVertex: TRect; AStyle: TcxSSEdgeLineStyle;
  AOrientation: TdxOrientation; ABkColor, AFgColor: TColor);
begin
  SetBkColor(ABkColor);
...
procedure TcxCanvasWrapper.Line(const AVertex: array of TRect; AStyle: TcxSSEdgeLineStyle;
  AOrientation: TdxOrientation; ABkColor, AFgColor: Word; IsSelected: Boolean);
begin
...
begin
  ...
  begin
    SetLength(TextBricks, 1);
    TextBricks[0].Text := CheckText(AText);
    FontHandle := FFontHandle;
    HorzAlign := haCenter;
...
var
  ...
  I: Byte;
const
  AOffset: array[TdxOrientation, 0..1] of Byte = ((0, 1), (1, 0));
begin
  case AStyle of
...
var
  ...
  begin
    ...
    TextBrick.Text := '';
    Count := 0;
    while (StartPos <= Length(AText)) and (AText[StartPos] = ' ') and (AWordCount > 1) do
      Inc(StartPos);
    EndPos := StartPos;
    ...
    begin
      with TextBrick do
      begin
        TextBrick.Text := CheckText(Copy(AText, StartPos, Count));
        ...
        begin
          ...
          begin
            while (AWidth > ExTextExtent(@AText[StartPos], Count).CX) and (BreakExtra < AWidth) do
            begin
              Inc(BreakExtra);
              SetTextJustification(DC, BreakExtra, BreakCount);
            end;
          end;
          SetTextJustification(DC, 0, 1);
        end;
      end;
      StartPos := EndPos;
    end;
...
begin
  ...
  try
    ...
    APos := 1;
    LineHeight := TextHeight(dxMeasurePattern);
...
const
  ...
  begin
    ...
    for K := orHorizontal to orVertical do
    begin
      for I := 0 to High(APatterns) do
      begin
        if K = orHorizontal then
...
begin
  ...
  begin
    WN_CreatePens;
    for K := Low(TdxOrientation) to High(TdxOrientation) do

...
begin
  ...
  begin
    ...
    Result.Y := Min(Lines.Count, YPos div ACanvas.TextHeight(dxMeasurePattern));
    S := Lines[Result.Y];
    try
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        FIsUpdate := True;
        System.Insert(dxCRLF, S, X + 1);
        Lines[Y] := S;
      end
      else
        Lines.Insert(Y + 1, '');
    end;
    dxMessagesController.KillMessages(Handle, WM_SYSCHAR, WM_SYSCHAR, False);
...
procedure TcxSSInplaceTextEdit.KeyDown(var Key: Word; Shift: TShiftState);
  ...
  begin
    ...
    begin
      Windows.SetFocus(TWinControl(Owner).Handle);
      if not Error then
...
var
  ...
  APos: TPoint;
begin
  if (Owner is TWinControl) and Assigned(TWinControlAccess(Owner).OnKeyDown) then
...
begin
  if (Owner is TWinControl) and Assigned(TWinControlAccess(Owner).OnKeyPress) then
...
begin
  if (Owner is TWinControl) and Assigned(TWinControlAccess(Owner).OnKeyUp) then
...
begin
  ...
  try
    ...
    LineCount := 1;
    while Pos(dxCRLF, AStr) <> 0 do
    begin
      Delete(AStr, Pos(dxCRLF, AStr), Length(dxCRLF));
      Inc(LineCount);
    end;
    ...
    Result.cy := ACanvas.TextHeight(dxMeasurePattern) * LineCount;

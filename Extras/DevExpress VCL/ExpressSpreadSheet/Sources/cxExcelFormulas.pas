...
begin
  FStringList.Add(IntToStr(PWord(@FTokens^[FOffset])^));
  Inc(FOffset, 2);
end;
...
begin
  ...
  try
    ...
    begin
      SetLength(AnsiS, ASize);
      Move(FTokens^[FOffset + 1], AnsiS[1], ASize);
      S := dxAnsiStringToString(AnsiS);
    end
    else
      begin

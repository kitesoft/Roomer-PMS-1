...
uses
  ...
  Windows, Graphics, dxCore, cxGraphics, cxExcelConst, cxSSTypes, cxSSStyles,
...
var
  ...
  procedure SetEdgeVertex(const ARect: TRect; var AStyle: TcxSSEdgeLineStyle;
    var AVertex: TcxEdgeVertex; AOrientation: TdxOrientation);
  var
    I: Byte;
  const
    AOfs: array[TdxOrientation, 0..1] of Byte = ((0, 1), (1, 0));
  begin
    ...
    if cxLineWidth[AStyle] = 3 then
      OffsetRect(AVertex[0], -AOfs[AOrientation, 0], -AOfs[AOrientation, 1]);
    if AStyle = lsDouble then
      for I := 1 to 2 do
      begin
        AVertex[I] := AVertex[0];
        OffsetRect(AVertex[I], AOfs[AOrientation, 0] * I, AOfs[AOrientation, 1] * I);
      end
    else
    begin
      if AOrientation = orHorizontal then
...
begin
  ...
  begin
    ...
    SetEdgeVertex(DisplayRect, Edges[eTop].Style, TopEdgeVertex, orHorizontal);
    SetEdgeVertex(DisplayRect, Edges[eLeft].Style, LeftEdgeVertex, orVertical);
  end;
end;

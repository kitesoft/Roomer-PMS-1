...
uses
  Types, Classes, Math, SysUtils, Windows, Dialogs,
...
procedure TcxSSDataStorage.DeleteCells(ACells: TRect; ACellsModify: TcxSSCellsModify);
  ...
  begin
    for I := ACells.Left to ACells.Right do
      ...
      begin
        L := FCols^[I].CellsCount - ACells.Bottom - 1;
        FCols^[I].CellsCount := ACells.Top;
        if L > 0 then
        begin
          FCols^[I].CellsCount := ACells.Top + L;
          Move(FCols^[I].Cells^[ACells.Bottom + 1], FCols^[I].Cells^[ACells.Top], L * SizeOf(Pointer));
          FillChar(FCols^[I].Cells^[FCols^[I].CellsCount], L * SizeOf(Pointer), 0);
        end;
      end;
  end;
...
{      else
    ...
    begin
      if DataType = dtFunction then
      begin
        if (FuncRecPtr <> nil) and not FuncRecPtr^.IsBadFunction then

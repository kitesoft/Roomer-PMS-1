...
uses
   Windows,  Classes, SysUtils, Math, Variants, Types, Dialogs,
   dxCore, cxClasses, cxExcelConst, cxSSData, cxSSUtils, cxSSTypes, cxSSRes;
...
type
  ...
  protected
    ...
    function CheckPageIndex(Sender: TcxSSFunctionHandler; APage: Integer): Boolean;
    function CreateCalculator: TcxSSFunctionHandler;
    procedure DoRecalc;
    function GetCellValue(Sender: TcxSSFunctionHandler;
      APage: Word; ACol, ARow: Integer; var AValue: Boolean; var AIsBlank: Boolean): Boolean; overload;
    function GetCellValue(Sender: TcxSSFunctionHandler;
      APage: Word; ACol, ARow: Integer; var AValue: Double; var AIsBlank: Boolean): Boolean; overload;
    function GetCellValue(Sender: TcxSSFunctionHandler;
      APage: Word; ACol, ARow: Integer; var AValue: string; var AIsBlank: Boolean): Boolean; overload;
  ...
  public
    ...
    property IsBlankCell: Boolean read FIsBlankCell; 
...
uses
  cxFormats, cxSSheet, cxExcelFormulas;
...
begin
  ...
  try
    ...
    begin
      ...
      FFuncList.Remove(ACell.FuncRecPtr);
...
begin
  ...
  begin
    ...
    begin
      ...
      CalcResult := TcxTokensStack.Clone(CalcResult);
    end;
...
var
  ACalculator: TcxSSFunctionHandler;
{$IFDEF DEBUGCALCULATION}
  AHandle: TObject;
{$ENDIF}
begin
  ...
  begin
    ...
    try
      ...
      begin
        ACalculator := CreateCalculator;
        Inc(IsBusy);
        try
          if FuncTree.Size = 0 then
            ACalculator.StringToTokens(DataStorages[Page][Col, Row].Text, AFuncRecPtr);
          States := fsSource;
          if FuncTree.Size > 0 then
          begin
          {$IFDEF DEBUGCALCULATION}
          ...
          {$ENDIF}
            ACalculator.EvaluateExpression(AFuncRecPtr);
          {$IFDEF DEBUGCALCULATION}
            TraceOut(AHandle);
          {$ENDIF}
          end;
  ...
  begin
    ...
    begin
      if TcxSSBookAccess(Owner).ShowFormulas then
        ...
        Result := TcxSSFunctionHandler.ErrorCodeToStr(ecName);
      Exit;
    end;
    if TcxSSBookAccess(Owner).ShowFormulas then
    begin
...
begin
  ...
  begin
    ATokens := AFuncRecPtr^.FuncTree.Tokens;
    ATok := Pointer(TdxNativeInt(@ATokens[AOfs]) + 1);
    if IsErr then Break;
...
function TcxSSFormulasCache.CheckPageIndex(
  Sender: TcxSSFunctionHandler; APage: Integer): Boolean;
begin
  Result := APage < Length(Sheets);
  if not Result then
...
function TcxSSFormulasCache.GetCellValue(Sender: TcxSSFunctionHandler;
 APage: Word; ACol, ARow: Integer; var AValue: Boolean; var AIsBlank: Boolean): Boolean;
var
  F: Double;
  ACell: TcxSSCellRec;
{$IFDEF DEBUGCALCULATION}
...
{$ENDIF}
    AValue := False;
    AIsBlank := True;
    Result := CheckPageIndex(Sender, APage);
    ...
    begin
      ...
      begin
        if ACell.FuncRecPtr^.IsBadFunction then
        begin
          Sender.SetError(ecName);
          Result := False;
          Exit;
        end
        else
...
function TcxSSFormulasCache.GetCellValue(Sender: TcxSSFunctionHandler;
  APage: Word; ACol, ARow: Integer; var AValue: Double; var AIsBlank: Boolean): Boolean;
var
  AStrValue: string;
  ABool: Boolean;
begin
  AValue := 0;
  Result := GetCellValue(Sender, APage, ACol, ARow, AStrValue, AIsBlank);
  if  Result then
  begin
    if AStrValue <> '' then
    begin
...
function TcxSSFormulasCache.GetCellValue(Sender: TcxSSFunctionHandler;
 APage: Word; ACol, ARow: Integer; var AValue: string; var AIsBlank: Boolean): Boolean;
var
  ACell: TcxSSCellRec;
{$IFDEF DEBUGCALCULATION}
  AHandle: TObject;
...
begin
  if AList.Count > 0 then
    APrior := TcxStackItemObject(AList.Last).Priority
  else
    APrior := -MaxInt;
...
begin
  ...
  if AList.Count > 0 then
  begin
    with TcxStackItemObject(AList.Last) do
    begin
      Result := Item;
      Free;
    end;
    ...
    begin
      case StackItemType(AItem) of
        ...
        begin
          ...
          if not Error then
            Owner.GetCellValue(Self, APage, ACol, ARow, Result, FIsBlankCell);
        end;
      else
        SetError(ecValue);
      end;
  ...
  try
    ...
    begin
      case StackItemType(AItem) of
        ...
        begin
          GetRefParams(AItem.Tokens, APage, ACol, ARow);
          if not Error and not Owner.GetCellValue(Self, APage, ACol, ARow, Result, FIsBlankCell) and not FIsBlankCell then
            SetError(ecValue);
        end;
      else
        SetError(ecValue);
...
begin
  ...
  begin
    for J := ARange.Top to ARange.Bottom do
      ...
      begin
        Owner.GetCellValue(Self, APage, I, J, AStringValue, FIsBlankCell);
  ...
  begin
    for J := ARange.Top to ARange.Bottom do
      for I := ARange.Left to ARange.Right do
         if Owner.GetCellValue(Self, APage, I, J, AFloatValue, FIsBlankCell) then
         begin
           TcxFloatFuncCallBack(AFunc)(Self, AFloatValue);
           Inc(FCallBackIndex);
         end;
...
var
  S: string;
begin
  S := StringReplace(FStack.StackTokensToStr(ATokens), '"', '""', [rfReplaceAll]);
  FStringList.Add('"' + S + '"');
end;
...
procedure fnAnd(Sender: TcxSSFunctionHandler; const AValue: string);
var
  B: Boolean;
  F: Double;
begin
  ...
  begin
    if cxTryStrToFloat(AValue, F) then
    begin
      if F = 0 then
        Sender.FloatValue := 0;
    end
    else
      if not cxTryStrToBool(AValue, B) or not B then
        Sender.FloatValue := 0;
  end;
end;
...
begin
  Sender.FloatValue := 1;
  Sender.EnumParamValues(@fnAnd, [vtString]);
  Sender.SetBooleanResult(Sender.FloatValue > 0);
end;
...
begin
  with Sender do
  begin
    if not Sender.IsBlankCell and (not IsValueAssigned or (Value > FloatValue)) then
      FloatValue := Value;
  end;
end;
...
const
  ...
  begin
    try
      APrecision := IntPower(IntPowerBase, ADigits);
    except
      on EOverflow do
        APrecision := 100;
    end;
    ...
    A := Abs(AValue * APrecision);
    B := Abs(ATempValue + Sign(AValue) * LimitFracValue);

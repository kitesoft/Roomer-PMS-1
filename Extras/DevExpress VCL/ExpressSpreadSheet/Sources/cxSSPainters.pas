...
uses
  Types, Classes, Windows, Graphics, SysUtils,
  dxCore,
  cxSSViewInfo, cxSSTypes, cxSSStyles, cxSSUtils, cxExcelConst, cxSSPainterWrapper;
  ...
  begin
    ...
    begin
      if Edges[eTop].Style = lsDefault then
        Canvas.Line(TopEdgeVertex, lsDefault, orHorizontal, 56, 56,
          IsSelected and not FHideSelection)
      else
        if Edges[eTop].Style <> lsNone then
        Canvas.Line(TopEdgeVertex, Edges[eTop].Style, orHorizontal, BrushBkColor,
          Edges[eTop].Color, IsSelected and not FHideSelection);
      if Edges[eLeft].Style = lsDefault then
        Canvas.Line(LeftEdgeVertex, lsDefault, orVertical, 56, 56,
          IsSelected and not FHideSelection)
      else
        if Edges[eLeft].Style <> lsNone then
        Canvas.Line(LeftEdgeVertex, Edges[eLeft].Style, orVertical, BrushBkColor,

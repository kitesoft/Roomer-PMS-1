...
type
  ...
  public
    ...
    procedure SetColStyle(APage, ACol, AWidth: Word; AOriginalSize: Integer; IsLocked: Boolean; IsHidden: Boolean = False); virtual;
    ...
    procedure SetRowStyle(APage, ARow, AHeight: Word; AOriginalSize: Integer; IsHidden: Boolean = False); virtual;
...
begin
  ...
  while Count > 0 do
  begin
    Result := (Result shl AShift or Result shr -AShift) xor AByte^;
    Inc(AByte);
    Dec(Count);
  end;
end;
...
begin
  ASrc := Pointer(TdxNativeInt(ASrc) + ALen);
end;
...
var
  ...
  begin
    ...
    begin
      ...
      FCurrentSheet.SetColRowSize(htCol, I,
         Round(AWidth), AWords^[2], ALocked, not Boolean(AWords^[4] and $01));
    end;
  end;

  procedure ReadRowFormatInfo(AData: Pointer);
  begin
    if ((PWordArray(AData)^[6] and $80) = $80) and (PWordArray(AData)^[3] > 32768) then Exit;
    FCurrentSheet.SetColRowSize(htRow, PWordArray(AData)^[0],
      Round(PWordArray(AData)^[3] / 20 * 1.325 * ScaledFactor), PWordArray(AData)^[3], False,
         (PWordArray(AData)^[6] and $20) = 0)
  end;
  
  procedure ReadStdColWidth(AData: Pointer);
  begin
     FCurrentSheet.SetDefaultSize(htCol, Round(PWord(AData)^ * 705 / 25782{ * ScaledFactor}));
  end;
  ...
  var
    ...
    begin
      ACount := ALen;
      AEndsCount := (TdxNativeUInt(StartPtr) + BufSize) - TdxNativeUInt(ACurrentPtr);
      try
        if IsWideChar then
        begin
          ...
          Inc(TdxNativeInt(ACurrentPtr), ACount);
        end
        else
        begin
    ...
    var
      ...
      begin
        ...
        begin
          ...
          Inc(TdxNativeInt(ACurrentPtr), 2);
        end;
        ExtRstLen := PInteger(ACurrentPtr)^;
        Inc(TdxNativeInt(ACurrentPtr), SizeOf(Integer));
        Result  := ReadStringChars(ALen, (AOptions and $01) <> 0);
        Inc(TdxNativeInt(ACurrentPtr), ExtRstLen);
        ExtRstLen := TdxNativeInt(ACurrentPtr) - (TdxNativeInt(StartPtr) + BufSize);
        if ExtRstLen > 0 then
        begin
          Dec(TdxNativeInt(ACurrentPtr), ExtRstLen);
          if PWord(ACurrentPtr)^ <> brcContinue then
            raise EdxException.Create(cxGetResourceString(@scxSpreadSheetErrorReadSST));
          StartPtr := @PWordArray(ACurrentPtr)^[2];
          BufSize := PWordArray(ACurrentPtr)^[1];
          ACurrentPtr := StartPtr;
          Inc(TdxNativeInt(ACurrentPtr), Abs(ExtRstLen));
        end;
      end;
    ...
    begin
      Result := '';
      FormatTokens := 0;
      if ((TdxNativeInt(StartPtr) + BufSize) - TdxNativeInt(ACurrentPtr)) < 3 then
      begin
        if PWord(ACurrentPtr)^ <> brcContinue then
          raise EdxException.Create(cxGetResourceString(@scxSpreadSheetErrorReadSST));
        StartPtr := @PWordArray(ACurrentPtr)^[2];
      ...
      try
        ...
        if  FormatTokens > (TdxNativeUInt(StartPtr) + BufSize - TdxNativeUInt(ACurrentPtr)) then
        begin
          FormatTokens := FormatTokens - (TdxNativeUInt(StartPtr) + BufSize - TdxNativeUInt(ACurrentPtr));
        ...
        Inc(TdxNativeInt(ACurrentPtr), FormatTokens);
      end;
    end;

  var
    I: Integer;
    AbsPtr: TdxNativeUInt;
  ...
  begin
    ...
    BufSize := ASize - 8;
    ACurrentPtr := StartPtr;
    AbsPtr := TdxNativeUInt(StartPtr);
    for I := 0 to AStringCount - 1 do
    begin
      ProgressUpdate(StartPtr);
      FSharedStringTable.Add(ReadNextString);
      if (TdxNativeUInt(ACurrentPtr) - TdxNativeUInt(AbsPtr)) > TdxNativeUInt(FExtRecordSize) then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        while I >= 0 do 
        begin
          if TdxNativeUInt(FStyleList.Objects[I]) = FormatIndex then
          begin
  ...
  begin
    ...
    begin
      ...
      Inc(TdxNativeInt(ACoords), 4 * 2);
      CurrentSheet.SetMergedCells(ARect, True);
    end;
  end;

  procedure AddCell(ACol, ARow: Integer; AText: string;
    AStyleIndex: Integer; IsFormula: Boolean = False; IsBlank: Boolean = False; AIsText: Boolean = False);
  var
    AFormat: Integer;
  begin
    ...
    begin
      ...
      AFormat := GetCellStyle.GetFormat;
      if AIsText then
        GetCellStyle.SetFormatStyle($23);
      if (AText <> '') or IsBlank then
        SetTextEx(AText, IsFormula, False);
      GetCellStyle.SetFormatStyle(AFormat)
    end;
  end;
  ...
  begin
    AddCell(PWordArray(AData)^[1], PWordArray(AData)^[0],
      FSharedStringTable[PInteger(@PByteArray(AData)^[6])^], PWordArray(AData)^[2], False, False, True);
  end;
  ...
  begin
    ...
    Inc(TdxNativeInt(AData), 4);
    for I := AColFirst to  AColLast do
    begin
      AddCell(I, ARow, '', PWord(AData)^, False, True);
      Inc(TdxNativeInt(AData), 2);
    end;
  end;
  ...
  begin
    ...
    begin
      Result := Pointer(TdxNativeInt(MemoryData) + APosition);
      FExtRecordSize := Result^.RecSize;
      ANextRec := Pointer(TdxNativeUInt(Result) + Result^.RecSize + 4);
      while ANextRec^.RecType = brcContinue do
      begin
        FExtRecordSize := FExtRecordSize + ANextRec^.RecSize + 4;
        ANextRec := Pointer(TdxNativeUInt(ANextRec) + ANextRec^.RecSize + 4);
      end;
    end
    else
      Result := nil;
  ...
  begin
    ...
    begin
      Result := TdxNativeInt(FBoundSheets.Objects[I]) = APosition;
...
begin
  ...
  try
      ...
      case ABiffRecHeader^.RecType of
        ...
        begin
          ...
          begin
            ...
            begin
              APosition := TdxNativeUInt(FBoundSheets.Objects[APage]);
              ABiffRecHeader :=  Pointer(TdxNativeUInt(MemoryData) + TdxNativeUInt(FBoundSheets.Objects[APage]));
              ProgressUpdate(ABiffRecHeader);
              Inc(APage);
            end;}
            Inc(FCurrentPage);
    ...
    ProgressUpdate(Pointer(TdxNativeInt(MemoryData) + DataSize));
  end;
end;
...
begin
  ALeft :=
    Round((TdxNativeUInt(APos) - TdxNativeUInt(MemoryData)) * 100 / DataSize);
...
procedure TcxExcelFileWriter.SetColStyle(APage, ACol, AWidth: Word;
  AOriginalSize: Integer; IsLocked: Boolean;
...
begin
  ...
  begin
    ...
    if AOriginalSize = 0 then
      AOriginalSize := Round(AWidth * 36.6 / ScaledFactor);
    WordData[4] := AOriginalSize;
    if IsLocked then
...
begin
  SelectPage(APage);
  ANode := GetEnd(FStorage.Pages.SheetNodesList);
  ANode := CreateXlsListNode(brcSTANDARDWIDTH, 2, ANode);
  with ANode^ do
    WordData[2] := Round(ASize * 25782 / 705);
end;
...
procedure TcxExcelFileWriter.SetRowStyle(APage, ARow, AHeight: Word;
  AOriginalSize: Integer; IsHidden: Boolean = False);
var
  ANode: PxlsTreeNode;
begin
  SelectPage(APage);
  ...
  begin
    ...
    WordData[4] := 256;
    if AOriginalSize = 0 then
      AOriginalSize := Round((AHeight * 20) / (1.325 * ScaledFactor));
    WordData[5] := AOriginalSize;
...
begin
  ...
  FStorage.RootHeader := CreateXlsListNode(brcTABID, 1024 * 4, FStorage.RootHeader);
  ...
  FStorage.RootHeader := CreateXlsListNode(brcExternSheet, 1024 * 4, FStorage.RootHeader);
end;
...
begin
  ...
  begin
    ...
    if not FIsError then
      Inc(TdxNativeInt(AData), SectorSize[IsSmallFile]);
  end;
end;

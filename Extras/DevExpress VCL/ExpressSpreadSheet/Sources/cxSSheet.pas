...
uses
  Types, Classes, SysUtils, Math,  Windows, Messages, Variants,
...
type
  ...
  private
    ...
    procedure SetColRowSize(const AKind: TcxSSHeaderType;
      const AColumn, ASize, AOriginalSize: Integer; ALocked: Boolean; AVisible: Boolean = True);
...
uses
  cxFormats, cxSSDesigner, cxLibraryConsts;
...
begin
  ...
  if AFormat <> $31 then
  begin
    if FormatIsDateTime(AFormat) then
    begin
      if cxTryStrToDate(AString, ADateTime) or cxTryStrToTime(AString, ADateTime) or cxTryStrToFloat(AString, Double(ADateTime)) then
        Result := TcxSSUtils.FormatText(ADateTime, AFormat, ADig, AColor)
    end
    else
...
begin
  ...
  Owner.BeginUpdate;
  try
    History.StartComplexAction(cxGetResourceString(@scxChangeDeleteCells));
    try
      if ACellsModify in [msAllRow, msAllCol] then
        Owner.FormulasCache.UpdateRef(PageIndex,  ACells, True, ACellsModify = msAllCol);
...
begin
  ...
  Owner.BeginUpdate;
  try
    History.StartComplexAction(cxGetResourceString(@scxChangeInsertCells));
    try
      if ACellsModify in [msAllRow, msAllCol] then
        Owner.FormulasCache.UpdateRef(PageIndex,  ACells, False, ACellsModify = msAllCol);
...
var
  ...
  I, J: Integer;
begin
  Result := not CellReadOnly(Owner, Self, DataStorage.Col, DataStorage.Row);
  if not Result then Exit;
  Result := True;
...
procedure TcxSSBookSheet.ApplyAutoHeight(ARow: Integer);
  ...
  begin
    ...
    begin
      ...
      Result := Canvas.TextHeight(dxMeasurePattern) + 2;
...
procedure TcxSSBookSheet.MouseMove(Shift: TShiftState; X, Y: Integer);
  ...
  begin
    ...
    if ANeedViewInfoUpdate then
      ViewInfo.TopLeft := Point(Owner.HScrollBar.GetData.Position,
        Owner.VScrollBar.GetData.Position);
  end;
    ...
    try
      ...
      try
        ...
        try
          ...
          CellEditor.Error := True;
          raise;
        end;
      finally
        FIsEditMode := False;
        Free;
        if CellEditor.Error then
        begin
          ACell := DataStorage.Cells[FEditingPos.X, FEditingPos.Y];
          try
            if ACell.DataType = dtFunction then
              OwnerSheet.Owner.FormulasCache.DestroyFunction(ACell.FuncRecPtr);
          finally
            ACell.FuncRecPtr := nil;
            ACell.DataType := dtText;
            DataStorage.Cells[FEditingPos.X, FEditingPos.Y] := ACell;
            CellEditor.SetFocus;
          end;
        end;
      end;
...
begin
  ...
  case AScrollBarKind of
    sbHorizontal:
    begin
      FSheetViewInfo.LeftCol := AScroller.GetData.Position;
      AScrollPos := FSheetViewInfo.LeftCol;
    end;
    sbVertical:
    begin
      FSheetViewInfo.TopRow := AScroller.GetData.Position;
...
begin
  ...
  begin
    ...
    OriginalSize[AColumn] := AOriginalSize;
  end;
end;
...
begin
  ...
  case ACell.DataType of
    ...
    dtControl:
      Result := TdxNativeInt(ACell.Control);
  end;
end;
    ...
    if J < 255 then
      with AHeader.Data^[J] do
        if (hsHidden in States) or (not (hsDefault in States)) or not (hsLockProtect in States) then
          AWriter.SetColStyle(APage, J, Size, OriginalSize, (hsLockProtect in States) <> DefaultStyle.Locked, hsHidden in States);
     ...
     if J < $FFFF then
       with AHeader.Data^[J] do
         if (hsHidden in States) or (not (hsDefault in States)) then
           AWriter.SetRowStyle(APage, J, Size, OriginalSize, hsHidden in States);
    end;
  end;
...
procedure TcxCustomSpreadSheetBook.DoGestureScroll(AScrollKind: TScrollBarKind; ANewScrollPos: Integer);
begin
...
begin
  ...
  try
    ...
    DC := AColPos - R.Left;
    DR := ARowPos - R.Top;
    if not CanModify(Self, ASheet, Rect(AColPos, ARowPos, AColPos + DX, ARowPos + DY)) then
      Exit; 

...
{$ENDIF}
  ...
  dxCore, dxCoreClasses, dxMessages, cxControls, cxContainer, cxGraphics, cxVariants,
  ...
  cxData, cxDataUtils, cxCustomData, cxStyles, cxGeometry, cxMemo, 
...
const
  ...
  tlhc_HitAtBackground        = 4;
  tlhc_HitAtBand              = 5;
  tlhc_HitAtBandContainer     = 6;
  tlhc_HitAtBandCustomizing   = 7;
  tlhc_HitAtBandHeader        = 8;
  tlhc_HitAtButton            = 9;
  tlhc_HitAtCheckButton       = 10;
  tlhc_HitAtColumn            = 11;
  tlhc_HitAtColumnCustomizing = 12;
  tlhc_HitAtColumnHeader      = 13;
  tlhc_HitAtFooter            = 14;
  tlhc_HitAtFooterItem        = 15;
  tlhc_HitAtGroupFooter       = 16;
  tlhc_HitAtGroupFooterItem   = 17;
  tlhc_HitAtImage             = 18;
  tlhc_HitAtIndent            = 19;
  tlhc_HitAtIndicator         = 20;
  tlhc_HitAtNode              = 21;
  tlhc_HitAtNodePreview       = 22;
  tlhc_HitAtSeparator         = 23;
  tlhc_HitAtSizingHorz        = 24;
  tlhc_HitAtSizingVert        = 25;
  tlhc_HitAtStateImage        = 26;
  tlhc_HitAtNavigator         = 27;
...
type
  ...
  TcxTreeListNodeState = (nsDeleting, nsHasChildren, nsValuesAssigned, nsCollapsed, nsValidIndexes, nsInternalDelete, nsHidden,
    nsHeightAssigned, nsInserting, nsEditing, nsCheck, nsCheckStateInvalid, nsDisabled, nsSaveExpanded, nsReserved1);
 ...
 { TcxTreeListHitTest }
  ...
  TcxCustomTreeList = class(TcxExtEditingControl, IcxNavigator,
    IcxStoredParent, IcxStoredObject, IcxTreeListDesignTimeOperations, IdxSkinSupport)
  private
...
uses
  Types, CommCtrl, RTLConsts, cxLike, cxFormats, dxThreading;
...
begin
  ...
  if ExtPaintStyle then
    Painter.DrawHeaderEx(ACanvas, R, TextBounds, Neighbors, Borders,
      State, GetAlignHorz, GetAlignVert, GetMultiline, GetShowEndEllipsis, GetText, ACanvas.Font,
...
begin
  ...
  begin
    ...
    if (Images <> nil) and (FVertTreeLine.Right >= (DisplayRect.Right - cxTextOffset)) then
...
begin
  ...
  begin
    if cxIsTouchModeEnabled then
      ...
      R := cxRectInflate(GlyphRect, cxTextOffset, cxTextOffset);
...
begin
  ...
  try
    ...
    begin
      ...
      if not AColumn.IsVisibleInQuickCustomizationPopup then
        Continue;
...
begin
  ...
  try
    ...
    begin
      ...
      if not ABand.IsVisibleInQuickCustomizationPopup then
        Continue;
      AItem := CheckListBox.Items.Add;
...
var
  AImage, AMask: TcxAlphaBitmap;
  AIsAlphaUsed: Boolean;
begin
  if IsImageAssigned(AImages, AImageIndex) then
  begin
    cxPrepareBitmapForDrawing(nil, AImages, AImageIndex, False, clNone, AImage, AMask, AIsAlphaUsed);
    ABitmap.PixelFormat := pf32bit;
    ABitmap.Width := AImage.Width;
    ABitmap.Height := AImage.Height;
...
begin
  if Controller.SelectionLockCount > 0 then Exit; 
  if not IsDestroying then
  begin
...
begin
  ...
  dxAdjustToTouchableSize(FDefaultIndentSize);
  BeginUpdate;
  try
...
begin
  ...
  begin
    ...
    AScrollRect.Bottom := ClientBounds.Bottom - ViewInfo.FooterHeight;
...
var
  ACF428B6B956621A2AABFDC664C62EB5B: TcxTreeListNode;
begin
  if (FocusedNode = nil) and (FocusedNodeIndex >= 0) then
  begin
    ACF428B6B956621A2AABFDC664C62EB5B := FindNearestFocusableNode(FocusedNodeIndex);
    if ACF428B6B956621A2AABFDC664C62EB5B <> nil then
      ACF428B6B956621A2AABFDC664C62EB5B.Focused := True;
  end;
end;
...
begin
  ...
  while ANode <> nil do
  begin
    if ANode.Visible and (ANode <> AExclude) then
...
begin
  ...
  begin
    ...
    while AChild <> nil do
    begin
      if AChild.Visible and (AState <> AChild.CheckState) then
      begin
        AState := cbsGrayed;
        Break;
      end;
...
begin
  ...
  Result := inherited FocusNextCell(AForward, ANextRow, AShift);
end;
...
begin
  ABounds.Right := ABounds.Left + Max(AImages.Width + cxTreeListIndentOffsetSize div 2, LevelInfo[ANode.Level].Size.cx);
...
begin
  ...
  if cxInRange(TopNode.VisibleIndex, ATop, ABottom) then
    TreeList.ScrollWindow(0, -AOffsets[TopNode.VisibleIndex - ATop])
  else
  begin
    InitOrigin;
    if cxInRange(LastNode.VisibleIndex, ATop, ABottom) then
      TreeList.ScrollWindow(0, AOffsets[ATop - TopNode.VisibleIndex])
    else
...
begin
  ...
  begin
    ...
    begin
      ...
      Result := CalculateHeight(MultilineEdit.CalculatedHeight + cxTextOffset);
    end
    else

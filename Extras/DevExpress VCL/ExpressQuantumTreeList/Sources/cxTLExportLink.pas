...
{$ENDIF}
  ...
  cxExport, cxXLSExport, dxXLSXExport, cxHtmlXmlTxtExport, cxMaskEdit;
...
begin
  ...
  ACells.Capacity := Nodes.Count * Cells.Count;
...
procedure cxExportTLToXLSX(const AFileName: string; ATreeList: TcxCustomTreeList;
  ...
  const AFileExt: string = 'xlsx');
begin
  cxExportTLToFile(AFileName, ATreeList, cxExportToXlsx, AExpand, ASaveAll,
    AUseNativeFormat, [], AFileExt);
end;

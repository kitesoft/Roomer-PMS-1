...
begin
  ...
  try
    TreeList.DataLocate := True;
    if (ANode = nil) or (TcxDBTreeListNode(ANode).KeyValue <> GetFieldValue(DBKeyField)) then
    begin
      CheckBrowseMode;
...
begin
  ...
  if Result and AIgnoreSimpleCurrency and ((Field <> nil) and IsSimpleCurrencyField(Field)) then
    Result := False;
end;
...
begin
  ...
  Inc(LoadingIndex);
  if (CursorNode = nil) and (Result.KeyValue = CursorKey) then
    CursorNode := Result;
...
procedure TcxCustomDBTreeList.CheckChanges;
begin
  FDataBusy := True;
  try
...
begin
  if (FocusedNode <> nil) and VarEquals(TcxDBTreeListNode(FocusedNode).KeyValue, AKeyValue) then
   ...
   if (Loader.CursorNode <> nil) and VarEquals(Loader.CursorNode.KeyValue, AKeyValue) then
    ...
    if OptionsData.SmartLoad and VarEquals(AKeyValue, RootValue) then
       Result := nil
     else
     begin

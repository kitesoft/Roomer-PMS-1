...
type
  ...
  public
    ...
    function FindWord(const AWord: WideString; AUncheckedOnly: Boolean; out AItem: TdxSpellCheckerSpelledWord): Boolean;
  ...
  private
    ...
    FTextChangedLock: TRTLCriticalSection;
    function CanContinue: Boolean;
    function GetTextChanged: Boolean;
    procedure SetTextChanged(AValue: Boolean);
  protected
  ...
  public
    ...
    property TextChanged: Boolean read GetTextChanged write SetTextChanged;
  end;
...
uses
  cxRichEdit, cxRichEditUtils, dxOffice11, cxTextEdit, ComCtrls, RichEdit, Dialogs, ComObj,
...
function TdxSpellCheckerCheckAsYouTypeRichEditAdapter.GetUnderlineRect(APosition, ALength: Integer): TRect;
...
begin
  ...
  InitializeCriticalSection(FTextChangedLock);
  inherited Create(False, True);
end;

destructor TdxSpellCheckerCheckAsYouTypeThread.Destroy;
begin
  inherited Destroy;
  DeleteCriticalSection(FTextChangedLock);
end;
    ...
    begin
      ...
      repeat
        ...
        try
          ...
          CheckMode.SpellingDone(not CanContinue);
        end;
...
function TdxSpellCheckerCheckAsYouTypeThread.GetTextChanged: Boolean;
begin
  EnterCriticalSection(FTextChangedLock);
  try
    Result := FTextChanged;
  finally
    LeaveCriticalSection(FTextChangedLock);
  end;
end;

procedure TdxSpellCheckerCheckAsYouTypeThread.SetTextChanged(AValue: Boolean);
begin
  EnterCriticalSection(FTextChangedLock);
  try
    FTextChanged := AValue;
...
procedure TdxSpellCheckerCheckAsYouTypeMode.CalculateBounds(AWord: TdxSpellCheckerMisspelledWord);
  ...
  procedure DoAddUnderlineRect(const R: TRect);
  begin
    Lock;
    try
      AWord.AddRect(R);
...
begin
  ...
  begin
    ...
    DoAddUnderlineRect(Adapter.GetUnderlineRect(ACharIndex, AWordPartLength));
  end;
end;
...
begin
  ...
  begin
    MisspelledWords.DropCheckedFlag;
    Parser.UpdateMeaningfulCharacters;
...
begin
  ...
  begin
    AItem := Items[I];
    Result := cxRectIsEqual(AItem.Bounds, R);
    if Result then
      Break;
  end;
end;
...
function TdxSpellCheckerCheckAsYouTypeEditAdapter.GetUnderlineRect(APosition, ALength: Integer): TRect;
  ...
  begin
    ...
    try
      if cxPointIsEqual(cxInvalidPoint, P) then
        Result := CheckMode.MisspelledWords.ItemAtIndex(Adapter.SelStart, AItem)
      else
...
procedure TdxSpellCheckerCheckAsYouTypeManager.SetMisspelledItem(
  AValue: TdxSpellCheckerMisspelledWord);
begin
  ...
  begin
    ...
    if AValue <> nil then
    begin
      FMisspelledItem := TdxSpellCheckerMisspelledWord.Create(0, '');

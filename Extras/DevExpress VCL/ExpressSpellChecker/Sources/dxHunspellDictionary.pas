...
type
  ...
  private
    ...
    function CheckCompoundWord(const AWord: PWideChar; AWordLength: Integer;
      var AInfo: TdxSpellInfo): TdxHunspellWordStem;
    function CheckForbiddenAndCompoundOnly(var AWordStem: TdxHunspellWordStem;
    ...
    function SpellSharps(ABase, APosition: PWideChar; AIndex, ANumRepeat: Integer;
      var AInfo: TdxSpellInfo): TdxHunspellWordStem;
    //setters
  ...
  protected
    ...
    procedure UpdateWordChars(var AWordChars: WideString); override;
  ...
  private
    ...
    procedure GenerateNgramSuggestions(AWord: PWideChar;
      var AExistCapitalizedWords, AOnlyCompoundSuggestions: Boolean);
    function GetAffixManager: TdxHunspellAffixManager; {$IFDEF DELPHI9} inline; {$ENDIF}
    function GetDictionary: TdxHunspellDictionary; {$IFDEF DELPHI9} inline; {$ENDIF}
    function GetPhoneTable: TdxPhoneTable; {$IFDEF DELPHI9} inline; {$ENDIF}
  ...
  protected
    ...
    function CanAddToSuggestions(const ATestWord: WideString): Boolean; override;
    ...
    function PrepareWord(const AWord: WideString): WideString; override;
...
begin
  Result := FileExists(DictionaryPath) and FileExists(GrammarPath);
  if not Result then
    Exit;
...
procedure TdxHunspellDictionary.UpdateWordChars(var AWordChars: WideString);
var
  P: PWideChar;
begin
...
function TdxHunspellSuggestionBuilder.CanAddToSuggestions(const ATestWord: WideString): Boolean;
begin
...
procedure TdxHunspellSuggestionBuilder.DoNearMissStrategy(AWord: PWideChar;
  var AOnlyCompoundSuggestions: Boolean);
var
  I: Boolean;
  ASavedWord: WideString;
...
procedure TdxHunspellSuggestionBuilder.InsertSuggestion(AWord: PWideChar;
  ASuggestions: TdxSpellCheckerSuggestionList = nil);
begin
...
function TdxHunspellSuggestionBuilder.PrepareWord(const AWord: WideString): WideString;
var
...
begin
  ...
  if (FAbbreviationCount <> 0) and AffixManager.SugsWithDots then
    for I := 0 to Suggestions.Count - 1 do

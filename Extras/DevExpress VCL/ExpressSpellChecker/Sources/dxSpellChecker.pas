...
{$ENDIF}
  ...
  dxCore, cxControls, cxLookAndFeels, cxEdit, cxTextEdit, cxRichEdit,
  dxSpellCheckerRules, dxSpellCheckerAlgorithms, dxHunspellTypes;
...
type
  ...
  TdxSpellCheckerSingleWordParser = class(TdxCustomSpellCheckerTextHelper)
  private
    FSpellChecker: TdxCustomSpellChecker;
  protected
    property SpellChecker: TdxCustomSpellChecker read FSpellChecker;
  ...
  TdxSpellCheckerTextParser = class(TdxSpellCheckerSingleWordParser)
  private
  ...
  public
    ...
    procedure UpdateMeaningfulCharacters;
  ...
  private
    ...
    function GetWordStruct(AWord: PWideChar): Pointer; {$IFDEF DELPHI9} inline; {$ENDIF}
  ...
  protected
    ...
    procedure PopulateRules(ARules: TdxSpellCheckerRules;
      AParser: TdxCustomSpellCheckerTextHelper); virtual;
  ...
  protected
    ...
    class function IsCheckSupported(AControl: TWinControl): Boolean;
  ...
  protected
    ...
    function GetAdapterClass(AControl: TWinControl): TdxSpellCheckerCustomEditAdapterClass; override;
    procedure Undo; virtual;
    function WideCapitalizeCase(const AWord: WideString): WideString;
  ...
  protected
    ...
    function CanCheckControlInContainer(AControl: TControl): Boolean; virtual;
    ...
    function InternalIsValidWord(const AWord: WideString): Boolean; virtual;
    ...
    procedure SpellingOptionsChanged; virtual;
    procedure ValidateRules(AParser: TdxCustomSpellCheckerTextHelper); virtual;
    procedure UpdateByDictionary(ADictionary: TdxCustomSpellCheckerDictionary); virtual;
  ...
  public
    ...
    function IsValidWord(const AWord: WideString): Boolean; overload;
...
uses
  ComCtrls, Dialogs, cxGeometry, dxOffice11, dxSpellCheckerStrs, dxSpellCheckerUtils,
...
function GetAdjustedRichEditText(const AText: WideString; AUseShortLineBreak: Boolean): WideString;
var
  ALen: Integer;
begin
  ALen := Length(AText);
...
function TdxSpellCheckerSingleWordParser.GetPrevWord: WideString;
begin
  Result := '';
end;

function TdxSpellCheckerSingleWordParser.WordExists(const AWord: WideString): Boolean;
begin
...
function TdxSpellCheckerTextParser.GetNextWord(var AStart, ALength: Integer; out ADelimiters: WideString): WideString;
  ...
  procedure SkipFirstDelimiters(var AStart: Integer; var ADelimiters: WideString);
  begin
  ...
  function InternalGetNextWord(var AStart, ALength, APos: Integer; var ADelimiters: WideString): WideString;
  begin
    ALength := 0;
...
begin
  ...
  APos := AStart;
  repeat
    Result := InternalGetNextWord(AStart, ALength, APos, ADelimiters);
    if ALength = 0 then
    begin
      ALength := 0;
      Break;
    ...
    begin
      ADelimiters := ADelimiters + Result;
      Inc(AStart, ALength);
    end
    else
      Break;
...
function TdxSpellCheckerTextParser.GetSentence(APos: Integer): TdxSpellCheckerSentence;
  ...
  begin
    ...
    while (AEndPos < ALength) and (not IsSentenceDelimiters(Text[AEndPos]) or (Text[AEndPos] = '.')) do
      Inc(AEndPos);
    if Text[AEndPos] = #0 then
      Dec(AEndPos);
    Result := Copy(Text, AStartPos, AEndPos - AStartPos + 1);
  end;
...
procedure TdxSpellCheckerTextParser.UpdateMeaningfulCharacters;
begin
...
begin
  if CanAddToSuggestions(ATestWord) then
    if IsCaseSensitive then
      ...
      FSuggestions.Add(WidePatternCase(FOriginalWord, ATestWord), Dictionary, 2);
end;
...
begin
  AWideText := dxAnsiStringToWideString(AText);
  Check(AWideText);
  AText := dxWideStringToAnsiString(AWideText);
end;
...
begin
  if Supports(AEdit, IcxContainerInnerControl, AIntf) then
    Check(TcxCustomTextEdit(AIntf.ControlContainer))
  else
  begin
    if AEdit is TCustomRichEdit then
      AEditAdapter := TdxSpellCheckerRichEditAdapter.Create(AEdit)
    else
...
procedure TdxCustomSpellChecker.AddWordToUserDictionary(const AWord: WideString);
var
...
function TdxCustomSpellChecker.CanCheckControlInContainer(AControl: TControl): Boolean;
begin
  Result := (AControl is TCustomEdit) or
    ((AControl is TcxCustomTextEdit) and
      TdxSpellCheckerCustomManager.IsCheckSupported(TcxCustomTextEdit(AControl)));
end;
...
var
  AParser: TdxSpellCheckerSingleWordParser;
begin
  BeginManualCheck;
  try
    AParser := CreateSingleWordParser;
    try
      ValidateRules(AParser);
...
begin
  Result := IsValidWord(dxAnsiStringToWideString(AWord));
end;
...
function TdxCustomSpellChecker.InternalIsValidWord(const AWord: WideString): Boolean;
begin
  Result := False;
...
procedure TdxCustomSpellChecker.ValidateRules(AParser: TdxCustomSpellCheckerTextHelper);
begin
  Rules.Clear;
...
procedure TdxSpellCheckerSpellingOptions.PopulateRules(
  ARules: TdxSpellCheckerRules; AParser: TdxCustomSpellCheckerTextHelper);
begin
  if AParser = nil then
    Exit;
...
begin
  Result := Edit.ActiveProperties.HideSelection;
end;
...
begin
  Edit.ActiveProperties.HideSelection := AValue;
end;
...
begin
  Result := GetAdjustedRichEditText(inherited GetText,
    TcxCustomRichEdit(Edit).RichVersion >= 200);
end;
...
function TdxSpellCheckerRichEditAdapter.GetText: WideString;
begin
  Result := GetAdjustedRichEditText(inherited GetText,
    {$IFDEF DELPHI12}True{$ELSE}False{$ENDIF});
end;
...
begin
  if Active then
  begin
    if IsCheckSupported(AControl) then
      Edit := AControl
    else
      Edit := nil;
  end;
...
class function TdxSpellCheckerCustomManager.IsCheckSupported(AControl: TWinControl): Boolean;
var
  AIntf: IdxSpellCheckerControl;
begin
  Result := Assigned(AControl) and AControl.HandleAllocated and
    Supports(GetInnerControlContainer(AControl), IdxSpellCheckerControl, AIntf) and
    AIntf.SupportsSpelling;
end;
...
function TdxSpellCheckerCustomCheckMode.IsValidWord(
  const AWord: WideString): Boolean;
begin
  Result := SpellChecker.InternalIsValidWord(AWord);
end;
...
begin
  AControl := GetInnerControlContainer(AControl);
  if AControl is TcxCustomRichEdit then

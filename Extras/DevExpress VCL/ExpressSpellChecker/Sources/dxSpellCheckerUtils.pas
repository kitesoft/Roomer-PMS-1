...
begin
  ...
  Result := ShiftPointer(Result, SizeOf(Cardinal) div 2)
end;
...
begin
  if S <> nil then
  begin
    S := ShiftPointer(S, -SizeOf(Cardinal) div 2);
    FreeMem(S, PCardinal(S)^);
  end;
end;
...
function IsSpecialAlpha(AChar: WideChar; ALangID: LCID): Boolean; {$IFDEF DELPHI9} inline; {$ENDIF}
begin
  Result := AChar = #$00DF;
  if Result then

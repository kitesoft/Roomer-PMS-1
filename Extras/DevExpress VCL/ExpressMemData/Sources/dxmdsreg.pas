...
{$ELSE}
  DsgnIntf,
{$ENDIF}
  Windows, Classes, DB, dxmdaset, dxmdsedt, dxmdatps, ShellAPI, SysUtils, dxCore, dxCoreReg;
...
function TdxMemDataComponentEditor.IsAssignable: Boolean;
begin
  Result := True;
end;

procedure TdxMemDataComponentEditor.DoAssign(AObject: TObject);
var
  AActive: Boolean;
...
function TdxMemDataComponentEditor.GetAssignItemCaption: string;
begin
  Result := 'Assign Data From';
end;

function TdxMemDataComponentEditor.GetAssignTypeClass: TClass;
begin
  Result := TDataSet;
end;

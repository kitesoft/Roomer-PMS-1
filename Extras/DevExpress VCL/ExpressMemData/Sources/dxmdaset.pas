...
{$ENDIF}
  Types, DB, SqlTimSt, Classes, SysUtils, dxCore, dxCoreClasses;
...
{$ENDIF}
  ...
  protected
    function ActiveBuffer: TdxMemDataBuffer; {$IFDEF DELPHI9}inline;{$ENDIF}
...
var
  ...
  AC842B2439266188AADBFE54E0361EF78: TSQLTimeStamp;
begin
  case AField.DataType of
    ftString, ftGuid:
    ...
    ftTimeStamp:
    begin
      AC842B2439266188AADBFE54E0361EF78 := GetTimeStampValue(ABuffer, AField);
      Result := VarSQLTimeStampCreate(AC842B2439266188AADBFE54E0361EF78);
    end;
    ftBCD:
    begin
  ...
  try
    ...
    if AExhangeList <> nil then
      for I := 0 to AExhangeList.Count - 1 do
        SynchronizeList(ADataList, TList(AExhangeList.List[I]));
  finally
    ADatalist.Free;
  end;
end;
  ...
  begin
    ...
    begin
      if (MemData.FBlobList.Last <> nil) and (dxrField <> nil) then
        MemData.SetInternalBlobData(MemData.FBlobList.Last, dxrField.Field.Offset, dxrField.BlobData);
    end;
  end;
end;
...
begin
  ...
  if Result then
    AIndex := Integer(FIndexList.List[AIndex]);
end;

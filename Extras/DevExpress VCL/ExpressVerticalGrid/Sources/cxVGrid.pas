...
{$ENDIF}
  ...
  cxInplaceContainer, cxEdit, cxButtons, cxNavigator, cxFilterControl,
...
type
  ...
  TcxRowCaptionInfo = class(TPersistent, IUnknown, IcxHotTrackElement, IcxHintableObject)
  private
    FCaption: string;
    FCaptionRect: TRect;
  ...
  private
    ...
    function GetCalcHelper: TcxvgCustomPaintStyleCalcHelper; {$IFDEF DELPHI9} inline; {$ENDIF}
    function GetVerticalGrid: TcxCustomVerticalGrid; {$IFDEF DELPHI9} inline; {$ENDIF}
  protected
    procedure GetDefaultViewParams(Index: Integer; AData: TObject; out AParams: TcxViewParams); override;
...
uses
  Math, Variants, Types, dxThemeConsts,
...
var
  ...
  begin
    ...
    begin
      ...
      ARecalcChild := ARecalcChild or ((ASaveColor <> HeaderInfo.IndentViewParams.Color) and (Row is TcxCategoryRow));
    end;
...
begin
  Result := Painter.DefaultContentColor;
end;
...
begin
  Result := Painter.ExpandButtonAreaSize + 4;
end;
...
begin
  Result := Painter.DefaultContentEvenColor;
  if Result = clDefault then
    Result := inherited GetContentEvenColor(AFocused);
end;

function TcxvgSkinCalcHelper.GetContentOddColor(AFocused: Boolean): TColor;
begin
  Result := Painter.DefaultContentOddColor;
  if Result = clDefault then
    Result := inherited GetContentOddColor(AFocused);
end;
...
procedure TcxvgCustomViewInfo.AddBottomValueSide(ARowViewInfo: TcxCustomRowViewInfo;
  const R: TRect);
begin
  with R do
    if HorzLineWidth > 0 then
      ARowViewInfo.ValuesLinesInfo.Add(Left, Bottom, Right - Left, HorzLineWidth, HorzLineBrush);
end;

procedure TcxvgCustomViewInfo.AddRightValueSide(ARowViewInfo: TcxCustomRowViewInfo;
  const R: TRect; ALast: Boolean);
begin
  with R do
    if VertLineWidth > 0 then
      if ALast then
        ARowViewInfo.ValuesLinesInfo.Add(Right, Top, VertLineWidth, Bottom - Top + HorzLineWidth, BandBorderColor)
      else
        ARowViewInfo.ValuesLinesInfo.Add(Right, Top, VertLineWidth, Bottom - Top + HorzLineWidth, VertLineBrush);
end;
...
begin
  ...
  dxAdjustToTouchableSize(FExplorerButtonAreaSize);
end;
...
begin
  ...
  begin
    ...
    FButtonAreaSize := LookAndFeel.Painter.ExpandButtonAreaSize;
...
begin
  if VerticalGrid.Images <> nil then
    FImageSize := cxSize(VerticalGrid.Images.Width, VerticalGrid.Images.Height)
  else
    FImageSize := cxNullSize;
  Result := cxTextHeight(VerticalGrid.Font) + 2 * cxTextOffset;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if not VerticalGrid.OptionsView.ShowEmptyRowImage then W := 0;
        FImageIndex := -1;
      end;
    end
    else W := 0;
...
begin
  ...
  begin
    ...
    if ABottomLineNeeded then
      AddBottomValueSide(Self, R);
    AddRightValueSide(Self, R, ALast);
  end;
end;
...
begin
  with Row.VerticalGrid do
    if not IsDestroying then
      Controller.HitTestController.CheckDestroyingItem(Self);
  inherited Destroy;
end;
...
begin
  ...
  begin
    ...
    cxRectIntersect(AVisibleCaptionBounds, ATextRect, Row.VerticalGrid.ClientBounds);
...
begin
  ...
  if AElement = nil then
    ...
    Result := AElement.PropertiesValue;
end;
...
begin
  if not FLockUpdate and (FFocusedRow <> nil) then
    FFocusedRow.Refresh;
end;
...
begin
  with ACaptionInfo do
  begin
    Canvas.SetParams(ViewParams);
    if not ATransparent then
      Canvas.FillRect(CaptionRect, ViewParams);
...
procedure TcxEditorPropertiesCollection.Notify(Item: TCollectionItem;
...
begin
  Result := VarToStr(DisplayEditProperties[Index].GetDisplayText(EditContainer.GetDisplayValue(DisplayEditProperties[Index], Index)));
...
begin
  ...
  ItemHeight := cxTextHeight(AVerticalGrid.Font) + cxTextOffset * 2;
end;
...
begin
  Accept := IsOwnerDragDrop(Source) and
...
begin
  ...
  try
    ...
    if ItemIndex = Index then
      Painter.Canvas.InvertRect(Rect);
  finally
...
function TcxVerticalGridItemsCustomizeListBox.IsOwnerDragDrop(
  ADragObject: TObject): Boolean;
begin
  Result := (ADragObject is TcxvgDragRowObject) and
    (TcxvgDragRowObject(ADragObject).Row = DragRow);
end;
...
begin
  ...
  if ViewInfo.UseCategoryExplorerStyle and (LookAndFeel.SkinPainter = nil) then
    Result := Max(Result, ViewInfo.ExplorerButtonAreaSize.cy + 2);
  end;
end;
...
begin
  ...
  VerticalGrid.SetScrollBarInfo(sbHorizontal,
    ...
    ScrollBarPos, not IsHideHScrollBar, ViewInfo.CanHScrollBarHide);
end;
...
begin
  ...
  if GetValue(AStyleIndex) <> nil then
...
begin
  ...
  if Assigned(FOnGetHeaderStyle) then
    FOnGetHeaderStyle(Control, ARow, AStyle);
  GetViewParams(AStyleIndexes[VerticalGrid.Controller.Focused or VerticalGrid.IsFocused], ARow, AStyle, Result);
end;
...
var
  ANavigatorInfo: TcxcustomNavigatorSiteViewInfo;
begin
  ANavigatorInfo := TcxvgCustomViewInfo(ViewInfo).NavigatorSiteViewInfo;
  Result := PtInRect(ANavigatorInfo.BoundsRect, HitPoint);
  if Result then
  begin
    FNewHitTestItem := ANavigatorInfo;

...
TdxRibbonRecentDocumentsController::TdxRibbonRecentDocumentsController(TdxRibbonBackstageViewGalleryControl* ARecentPaths, TdxRibbonBackstageViewGalleryControl* ACurrentFolder, TdxRibbonBackstageViewGalleryControl* ARecentDocuments) :
...
{
  const String NamesMap[rs2013 + 1] = {"RIBBONAPPGLYPH", "RIBBONAPPGLYPH2010", "RIBBONAPPGLYPH2010"};
  ...
  Ribbon->ApplicationButton->Glyph->LoadFromResourceName((unsigned int)HInstance, NamesMap[Ribbon->Style]);
...
void __fastcall TfrmRibbonNotepadMain::FontColorChanged(TObject* Sender)
{
...
{
  return new TdxRibbonRecentDocumentsController(bvgcRecentDocuments, bvgcRecentPaths, bvgcCurrentFolder);
}
//---------------------------------------------------------------------------

void TfrmRibbonNotepadMain::DoUpdateControls(TfrmNotepadChild* AActiveChild)
{
  const SelectionContextIndex = 0;
  ...
  bbFontColor->Enabled = AActiveChild != NULL;
...
void __fastcall TfrmRibbonNotepadMain::bbFontColorClick(TObject *Sender)
{
...
{
  const	String NameMap[6] = {"Blue", "Black", "Silver", "DarkGray", "LightGray", "White"};
  for (int I = 0; I <= 5; I++)
...
void TfrmRibbonNotepadMain::SetGlyph(Graphics::TBitmap* ABitmap, TcxImageList *AImageList, Integer AGlyphIndex)
{
...
{
  const String DisplayNameMap[6] = {"Blue", "Black", "Silver", "Dark Gray", "Light Gray", "White"};
  ...
  for (int I = 0; I <= dxRibbonSkinsManager()->SkinCount - 1; I++)
  {
	...
	{
	#ifdef EXPRESSSKINS
	  if (ASkin is TdxSkinRibbonPainter)
	  {
		if (!((TdxSkinRibbonPainter*)(ASkin))->Painter->IsInternalPainter &&

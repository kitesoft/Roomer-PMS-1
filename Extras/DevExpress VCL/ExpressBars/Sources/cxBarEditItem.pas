...
type
  ...
  private
    ...
    procedure DoDrawEditButtonBorder(Sender: TcxEditButtonViewInfo;
      ACanvas: TcxCanvas; out ABackgroundRect, AContentRect: TRect; var AHandled: Boolean);
  ...
  protected
    ...
    function NeedFocusOnClick: Boolean;
...
uses
  Variants, Forms, SysUtils, cxBarEditItemValueEditor, cxClasses, cxEditPaintUtils,
  cxEditRepositoryItems, cxEditUtils, cxLookAndFeelPainters, cxTextEdit,
  cxVariants, dxBarCustomCustomizationForm, dxOffice11, dxBarStrs, dxBarSkinConsts, dxCore, dxFading, Math;//, cxDWMApi;
...
function TcxBarEditItemControl.GetViewData(
  out AIsViewDataCreated: Boolean): TcxCustomEditViewData;
begin
  ...
  begin
    ...
    try
      ...
      if AAssignRepositoryItem then
        Edit.RepositoryItem := Item.RepositoryItem;
      SetEditButtonsWidth;
    finally
...
begin
  ...
  AAssignRepositoryItem := NeedAssignRepositoryItem(Item.Properties, Item.RepositoryItem, AProperties);
  ...
  InitEditProperties(AProperties, AAssignRepositoryItem);
...
begin
  ...
  try
    ...
    if Item.CaptionIsEditValue then
      AViewData.EditValueToDrawValue(Caption, AViewInfo)
    else
      AViewData.EditValueToDrawValue(Item.EditValue, AViewInfo);
    if not Item.CaptionIsEditValue and NeedEditShowCaption then
      TcxCustomTextEditViewInfo(AViewInfo).Text := Caption;
    cxPaintCanvas.BeginPaint(Canvas.Canvas);
    try
      AViewData.Calculate(cxPaintCanvas, ABounds, P, cxmbNone, [], AViewInfo, AIsMouseEvent);
    finally
...
begin
  ...
  try
    ...
    AViewData := CreateEditViewData(not AIsMinSize or Item.CaptionIsEditValue);
    CalculateEditViewInfo(Rect(0, 0, MaxInt, MaxInt), Point(-1, -1), False, AEditViewInfo, False);
    cxPaintCanvas.BeginPaint(Canvas.Canvas);
    try
      AConstantPartSize := AViewData.GetEditConstantPartSize(cxPaintCanvas,
      ...
      if Item.CaptionIsEditValue then
        Result := AViewData.GetEditContentSize(cxPaintCanvas, Caption, DefaultcxEditSizeProperties)
      else
        Result := cxNullSize
    finally
      cxPaintCanvas.EndPaint;
    ...
    if Item.IsEditHasContent and (Result.cx < MinContentWidth) then
...
procedure TcxBarEditItemControl.InitEditContentParams(
  ...
  begin
    ...
    OffsetRect(AExternalBorderBounds, -AExternalBorderBounds.Left,
      -AExternalBorderBounds.Top);
    if IsPopupSideward then
      Dec(AExternalBorderBounds.Left, GetEditOffset);
    ABorderOffsets := Painter.EditControlBorderOffsets(GetPaintType);
    OffsetRect(AExternalBorderBounds, -ABorderOffsets.Left,
      -ABorderOffsets.Top);
  end;

...
uses
  Classes, Messages, DB, dxCore, dxCoreClasses, cxClasses, dxBar;
...
uses
  Windows, SysUtils, Forms, dxBarDBNavStrs, Math, StrUtils;
...
procedure TdxBarDBNavButton.DefineProperties(Filer: TFiler);
  ...
  begin
    Result := ((Filer.Ancestor = nil) or (TdxBarDBNavButton(Filer.Ancestor).Caption <> Caption)) and
      IsCaptionStored;
  end;

  function IsHintNeedStored: Boolean;
  begin
    Result := ((Filer.Ancestor = nil) or (TdxBarDBNavButton(Filer.Ancestor).Hint <> Hint)) and
      IsHintStored;
  end;

begin
  inherited DefineProperties(Filer);
  Filer.DefineProperty('CaptionEx', ReadCaption, WriteCaption,
    IsCaptionNeedStored);
  Filer.DefineProperty('HintEx', ReadHint, WriteHint,
    IsHintNeedStored);
end;

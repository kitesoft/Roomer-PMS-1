...
{$ENDIF}
  ...
  dxBar, dxBarAccessibility, dxCommon, dxBarSkinConsts, cxControls, dxCore,
  cxDateUtils;
...
uses
  ...
  cxClasses, cxGraphics, cxGeometry, cxFormats, cxLookAndFeels;
...
type
  ...
  TPlaceForm = class(TCustomForm, IdxSkinSupport, IdxSkinSupport2)
  private
...
begin
  ...
  begin
    Default;
    if not FIsControlRecreating and not InPlaceControl and IsSizeChanged and
      ((Message.Msg = WM_SIZE) or (Message.Msg = WM_WINDOWPOSCHANGED) and (Message.lParam = 0)) then
    begin
      SaveControlSize;
      UpdateEx;
    end;
...
begin
  Result := (Parent.Kind <> bkBarQuickControl) and (Control <> nil) and
    not BarManager.Designing;
end;

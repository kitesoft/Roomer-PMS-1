...
uses
  ...
  dxBar, dxStatusBar, dxRibbonSkins, dxRibbon, dxRibbonFormCaptionHelper;
...
type
  ...
  public
    ...
    function GetColorScheme(ABarControl: TCustomdxBarControl;
      out AColorScheme: TdxCustomRibbonSkin; out AColorSchemeAccent: TdxRibbonColorSchemeAccent): Boolean;
...
begin
  if GetColorScheme(ABarControl, AColorScheme, AColorSchemeAccent) then
  begin
    Dec(ABeginGroupRect.Top, GetToolbarContentOffsets(nil, dsTop, False).Top);
    AColorScheme.DrawStatusBarToolbarSeparator(DC, ABeginGroupRect, AColorSchemeAccent);
  end;
end;
...
begin
  ...
  begin
    ...
    Dec(ABounds.Top, GetToolbarContentOffsets(ABarControl.Bar, dsTop, False).Top);
    ...
    AColorScheme.DrawStatusBarPanel(ACanvas.Handle,
      ...
      ABounds, GetIsPanelLowered(ABarControl), AColorSchemeAccent);
  end;
end;
  ...
  begin
    ...
    try
      ...
      AColorScheme.DrawStatusBarPanel(DC,
        dxMapWindowRect(AStatusBar.Handle, ABarControl.Handle, AStatusBar.ClientBounds, False),
        cxRectBounds(0, 0, ABarControl.Width, AStatusBar.Height),
        GetIsPanelLowered(ABarControl), AColorSchemeAccent);
    finally
...
var
  ...
  AColorSchemeAccent: TdxRibbonColorSchemeAccent;
begin
  if GetColorScheme(ABarItemControl.Parent, AColorScheme, AColorSchemeAccent) then
  begin
    AColor1 := AColorScheme.GetPartColor(rspStatusBarText, DXBAR_DISABLED, AColorSchemeAccent);
    AColor2 := AColor1;
  end
  else
...
function TdxRibbonStatusBarBarControlPainter.GetColorScheme(ABarControl: TCustomdxBarControl;
  out AColorScheme: TdxCustomRibbonSkin; out AColorSchemeAccent: TdxRibbonColorSchemeAccent): Boolean;
begin
  Result := TdxBarControl(ABarControl).DockControl <> nil;
  if Result then
  begin
    with TdxRibbonStatusBarDockControl(TdxBarControl(ABarControl).DockControl) do
    begin
      AColorSchemeAccent := Ribbon.ColorSchemeAccent;
      AColorScheme := Ribbon.ColorScheme;
    end;
  end;
end;

function TdxRibbonStatusBarBarControlPainter.GetStatusBar(
  ABarControl: TCustomdxBarControl): TdxRibbonStatusBar;
begin
  Result := TdxRibbonStatusBarDockControl(TdxBarControl(ABarControl).DockControl).StatusBar;
end;
...
begin
  ...
  if ColorScheme = nil then
    ...
    ColorScheme.DrawStatusBarPanel(DC,
      dxMapWindowRect(StatusBar.Handle, Handle, StatusBar.ClientBounds),
      R, Panel.Bevel <> dxpbRaised, Ribbon.ColorSchemeAccent);
end;
...
begin
  ...
  begin
    ...
    if ABarControl.IsValid then
      Result := ABarControl.GetSize(MaxInt).cx + 2;
  end;
end;
  ...
  begin
    ...
    begin
      Result := ABarControl.GetSize(Panel.Width).cy + cxMarginsHeight(
        ABarControl.Painter.GetToolbarContentOffsets(ABarControl.Bar, dsNone, False));
    end;
  end;
end;
...
class procedure TdxRibbonStatusBarPainter.DrawBorder(
  AStatusBar: TdxCustomStatusBar; ACanvas: TcxCanvas; var R: TRect);
begin
  if TdxRibbonStatusBar(AStatusBar).IsRibbonValid then
  begin
    TdxRibbonStatusBar(AStatusBar).Ribbon.ColorScheme.DrawStatusBar(
      ACanvas.Handle, R, TdxRibbonStatusBar(AStatusBar).Ribbon.ColorSchemeAccent);
  end
  else
  begin
...
begin
  ...
  begin
    ARibbonStatusBar.Ribbon.ColorScheme.DrawStatusBarPanel(
      ACanvas.Handle, ARibbonStatusBar.ClientBounds, R, ABevel <> dxpbRaised,
      ARibbonStatusBar.Ribbon.ColorSchemeAccent);
  end
  else
...
begin
  ...
  if ARibbonStatusBar.IsRibbonValid then
  begin
    ARibbonStatusBar.Ribbon.ColorScheme.DrawStatusBarPanelSeparator(
      ACanvas.Handle, R, ARibbonStatusBar.Ribbon.ColorSchemeAccent);
  end
  else
...
begin
  ...
  begin
    ...
    try
      ...
      ARibbonStatusBar.Ribbon.ColorScheme.DrawStatusBarGripBackground(
        ACanvas.Handle, AGripRect, ARibbonStatusBar.Ribbon.ColorSchemeAccent);
      Dec(AGripRect.Bottom, 3);
      ARibbonStatusBar.Ribbon.ColorScheme.DrawStatusBarSizeGrip(
        ACanvas.Handle, AGripRect, ARibbonStatusBar.Ribbon.ColorSchemeAccent);
    finally
...
begin
  if IsRibbonValid then
    AColor := Ribbon.ColorScheme.GetPartColor(rspStatusBarText, Ord(not Active), Ribbon.ColorSchemeAccent)
  else
    AColor := Font.Color;
end;
...
begin
  ...
  begin
    FRisedSideStates[True] := ViewInfo.Panels[I].Bevel = dxpbRaised;
    Break;
  end;
...
begin
  if IsRibbonValid and (Color = clDefault) then
    inherited Color := Ribbon.ColorScheme.GetPartColor(rfspRibbonForm, 0, Ribbon.ColorSchemeAccent);

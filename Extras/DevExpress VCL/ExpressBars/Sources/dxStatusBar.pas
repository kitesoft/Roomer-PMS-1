...
uses
  ...
  dxThemeManager, dxCore, cxClasses, cxGraphics, cxControls, cxLookAndFeels,
...
type
  ...
  protected
    ...
    property OnChange: TNotifyEvent read FOnChange write FOnChange;
  public
    constructor Create(AId: TdxStatusBarKeyboardState; ACode: Integer; const AActiveCaption: string;
      AActiveFontColor: TColor; const AInactiveCaption: string; AInactiveFontColor: TColor; AChangeHandler: TNotifyEvent);
    procedure Assign(Source: TPersistent); override;
...
uses
  ActnList, StdActns, Types, Math,
  dxThemeConsts, dxUxTheme, dxOffice11, cxGeometry;
...
begin
  ...
  begin
    ...
    then
      Dec(R.Right, Painter.GripAreaSize.cx);
  end;
end;
...
begin
  ...
  begin
    ...
    Inc(Result, cxMarginsWidth(Painter.PanelContentExtents(
      Owner.StatusBarControl, Painter.GetPanelBevel(Owner) <> dxpbNone)));
  end
  else
    Result := 20;
end;
  ...
  begin
    ...
    begin
      ...
      R.Right := R.Left + FKeyInfos[I].GetRectWidth(ACanvas) +
        cxMarginsWidth(APainter.PanelContentExtents(Owner.StatusBarControl, not FullRect));
      ARect := R;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Inc(Result, cxMarginsWidth(Painter.PanelContentExtents(Owner.StatusBarControl, not FullRect)));
        // key separator
        if I <> ALastKeyIndex then
          Inc(Result, Painter.SeparatorSizeEx(StatusBarControl));
      end;
    end;
  end
  else
...
procedure TdxStatusBarViewInfo.AddCalculatedItems(AAutoWidthObject: TcxAutoWidthObject);
  ...
  procedure AddAutoWidthPanel(APanel: TdxStatusBarPanel;
    APanelIndex: Integer; var ANonFixedExists: Boolean);
  var
...
begin
  ...
  begin
    ...
    Result.Left := Result.Right - Painter.GripAreaSize.cx;
  end
  else
    Result := cxNullRect;
end;
...
begin
  Result := cxRectSetRight(SizeGripRect,
    SizeGripRect.Right, cxRectWidth(SizeGripRect) + Painter.SeparatorSize);
end;
...
begin
  Result := Max(Painter.GripAreaSize.cy, cxTextHeight(PanelFont) +
...
procedure TdxCustomStatusBar.LookAndFeelChangeHandler(
...
class function TdxStatusBarPainter.PanelContentExtents(
...
class procedure TdxStatusBarPainter.DrawSimplePanel(
  ...
  begin
    ...
    try
      ...
      ACanvas.Font.Color := ATextColor;
      InflateRect(R, -cxTextOffset, 0);
      ACanvas.DrawText(AText, R, cxSingleLine or cxAlignVCenter or cxAlignLeft,
        AStatusBar.Enabled);
    finally
      ACanvas.RestoreState;
    end;
...
begin
  ...
  try
    DrawFrameControl(ACanvas.Handle, R, DFC_SCROLL, DFCS_SCROLLSIZEGRIP);
    ACanvas.ExcludeClipRect(R);
    ACanvas.FillRect(AStatusBar.ViewInfo.SizeGripAreaRect, AStatusBar.Color);
  finally
...
class procedure TdxStatusBarOffice11Painter.DrawSizeGrip(AStatusBar: TdxCustomStatusBar;
  ACanvas: TcxCanvas; R: TRect);
begin
  Office11DrawSizeGrip(ACanvas.Handle, R);
end;
...
begin
  ATheme := OpenTheme(totStatus);
  DrawThemeBackground(ATheme, ACanvas.Handle, SP_GRIPPER, 0, @R);
end;

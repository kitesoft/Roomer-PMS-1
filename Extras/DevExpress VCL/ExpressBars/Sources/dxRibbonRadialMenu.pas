...
uses
  ...
  Controls, ExtCtrls, ImgList, IniFiles, Contnrs, Math, Types,
  dxCore, dxCoreClasses, cxClasses, cxGraphics, cxControls, cxContainer,
  cxLookAndFeels, dxGDIPlusApi, dxGDIPlusClasses, cxGeometry,
  dxBar, dxBarSkinConsts, dxRibbon, dxAnimation, dxBarStrs,
...
type
  ...
  private
    ...
    procedure SetGlyph(AGlyph: TdxSmartImage);
  ...
  private
    ...
    FInternalItemLinks: TdxBarInternalItemLinks;
...
//    function GetVisibleItemLink(ASectorIndex: Integer): TdxBarItemLink;
...
procedure TdxRibbonCustomRadialMenu.InternalShowRadialMenu(X, Y: Integer; AMode: TdxRadialMenuMode);
  ...
  var
    ...
    ANestedAItemLinks:  TdxBarItemLinks;
  begin
    Result := 0;
    for I := 0 to GetVisibleItemCount(AItemLinks, FSectorCount) - 1 do
    begin
//      ACaptionSize := cxTextWidth(ItemLinks.BarControl.Font, AItemLinks.VisibleItems[I].Caption) div 2;
      ACaptionSize := cxRectWidth(cxGetTextRect(ItemLinks.BarControl.Canvas.Handle, AItemLinks.VisibleItems[I].Caption, 2));
      Result := Max(Result, ACaptionSize);
      ANestedAItemLinks := GetOwnedLinks(AItemLinks.VisibleItems[I].Item);
      if ANestedAItemLinks <> nil then
        Result := Max(Result, GetMaximumCaptionSize(ANestedAItemLinks));
    end;
  end;
  ...
  begin
    ACaptionSize := GetMaximumCaptionSize(ItemLinks);
    Result := Round(ACaptionSize / Sin(pi / FSectorCount)) * 2;
    Result := Max(DefaultMenuSize, Result);
  end;
  ...
  begin
    ...
    with ItemLinks.BarControl as TdxRibbonRadialMenuControl do
    begin
      FRadialMenu := Self;
      if Ribbon <> nil then
        TdxCustomRibbonAccess(Ribbon).AddAffiliatedBarControlForAccessibility(ItemLinks.BarControl);
      FInnerSize := Self.FInnerSize;
      if Self.MenuSize = 0 then
        FMenuSize := GetMaximumMenuSize
      else
        FMenuSize := Self.FMenuSize;
      FSectorCount := Self.FSectorCount;

      ABounds := ExpandRectFromCenter(dxPointF(P), dxSizeF(FMenuSize, FMenuSize));
      ...
      FMenuMode := AMenuMode;
      InitializeColors(GetSkin(Ribbon));
...
procedure TdxRibbonCustomRadialMenu.SetGlyph(AGlyph: TdxSmartImage);
begin
...
begin
  ...
  FInternalItemLinks := TdxBarInternalItemLinks.Create(BarManager, Self);
  ...
  FInternalItemLinks.Add(ACoreItem);
end;
...
procedure TdxRibbonRadialMenuControl.CreateParams(
...
begin
  ...
  FSmallRingRect := cxRectInflate(FCoreRingRect, -SmallRingOffset, -SmallRingOffset);
  FFullRingRect := ClientRect;
  FItemContentRingRect := cxRectContent(FFullRingRect, Rect(ThickRingSize, ThickRingSize, ThickRingSize, ThickRingSize));
  FThickRingRect := cxRectInflate(FFullRingRect, -ThickRingSize div 2, -ThickRingSize div 2);
end;
...
begin
  inherited;
  UpdateLayer;
  CallNotify(FRadialMenu.FOnPopup, FRadialMenu);
end;
...
begin
  Result := (AKey <> VK_MENU); // or BarNavigationController.KeyTipsHandlingMode
end;
...
//  ItemRingDefaultColor = $EADAF1;
//  BackArrowColor = $925C97;
//  DropDownItemRingNormalColor = RadialMenuBaseColor;

  BackgroundColor = $FEFEFE;
//  ItemTextColor = $3F3F3F;
//  DropDownItemRingHotColor = $444444;
...
begin
  ...
  AHSV := dxColorToHSV(FBaseAccentColor);
...
procedure TdxRibbonRadialMenuControl.DrawRadialItemBackGround(ACanvas: TcxCanvas; AGPGraphics: TdxGPGraphics; AItem: TdxBarItem; ASector: TdxSector);
  ...
  begin
    ...
    dxPointsOffset(APoints, dxPointF(FItemContentRingRect.Right + ThickRingSize / 3, FMenuCenter.Y));

    A := DegToRad(ASector.StartAngle + ASector.SweepAngle / 2);
    dxRingRotatePoints(dxPointF(FMenuCenter), APoints, A);
    AGPGraphics.Polygon(APoints, clWhite, clWhite, 2, psSolid, 170);
  end;
...
procedure TdxRibbonRadialMenuControl.DrawRadialItemContent(ACanvas: TcxCanvas; AGPGraphics: TdxGPGraphics; AItem: TdxBarItem; ARect: TRect);
  ...
  function GetCurrentImage(AItem: TdxBarItem; out ACurrentGlyph: TBitmap; out ACurrentImages: TCustomImageList; out ACurrentImageIndex: Integer): Boolean;
  begin
    if AItem = nil then
      Result := False
    else
    begin
      ...
      Result := IsGlyphAssigned(ACurrentGlyph) or TdxBarItemAccess(AItem).IsImageIndexLinked(ACurrentImages, ACurrentImageIndex);
    end;
  end;
  ...
  begin
    Result := cxRectBounds(ARect.Left, ARect.Top, cxRectWidth(ARect), ItemImageSize);
    Result := cxRectCenter(Result, ItemImageSize, ItemImageSize);
  ...
  begin
    ...
    begin
      ATempBitmap := TcxAlphaBitmap.CreateSize(GetImageRect);
      ATempBitmap.Clear;
      AEnabled := AItem.Enabled and ((GetClickableItem(AItem) = nil) or (GetClickableItem(AItem).Enabled));
      cxDrawImage(ATempBitmap.cxCanvas, ATempBitmap.ClientRect, ACurrentGlyph, ACurrentImages, ACurrentImageIndex, AEnabled);
      ATempImage := TdxSmartImage.CreateFromBitmap(ATempBitmap);
      AGPGraphics.Draw(ATempImage, GetImageRect);
      ATempImage.Free;
      ATempBitmap.free;
    end;

    ARect.Top := ARect.Top + ItemImageSize;
  end;
  ...
  procedure DrawText(var ARect: TRect; ATextColor: TColor; AAlpha: Integer);
    ...
    var
      ...
      AGPBitmapanvas: TdxGPGraphics;
    begin
      ABitmap := TcxAlphaBitmap.CreateSize(ARect);
      try
        ABitmap.Clear;
        AGPBitmapanvas := dxGpBeginPaint(ABitmap.Canvas.Handle, ABitmap.ClientRect);
        try
          dxGPDrawText(AGPBitmapanvas, AText, ABitmap.ClientRect, ACanvas.Font, ATextColor, taCenter, taAlignTop, True, TextRenderingHintAntiAliasGridFit, AAlpha)
        finally
          dxGpEndPaint(AGPBitmapanvas);
        end;
        ABitmap.RecoverAlphaChannel(0);

        ASmartImage := TdxSmartImage.CreateFromBitmap(ABitmap);
        try
          cxDrawImage(ACanvas, ARect, ASmartImage, ifmNormal);
        finally
          ASmartImage.Free;
        end;
      finally
...
procedure TdxRibbonRadialMenuControl.DrawRadialItems(ACanvas: TcxCanvas; AGPGraphics: TdxGPGraphics);
  ...
  begin
    ...
    AItemSize := dxSizeF(GetItemRectWidth(R), ItemHeight);
...
begin
  ACanvas.Font := Font;
  ACanvas.Font.Size := Min(8, Font.Size);
  ...
  try
    ...
    //Background
     AGPGraphics.Ellipse(FFullRingRect, FBaseBackgroundColor, FBaseBackgroundColor);

    // Items
    AGPGraphics.Ellipse(FThickRingRect, FItemRingDefaultColor, 0, ThickRingSize, psSolid, 255, 0);
...
begin
  ...
  try
    ...
    //Background
     AGPGraphics.Ellipse(FSmallRingRect, FBaseBackgroundColor, FBaseBackgroundColor);
    ...
    begin
      AGlyphRect := cxRectCenter(ClientRect, GetCurrentCoreImage.Size);
      ARegion := TcxRegion.Create(dxCreateRoundRgn(cxRectInflate(FSmallRingRect, -2, -2)));
      ACanvas.SetClipRegion(ARegion, roSet);
      cxDrawImage(ACanvas, AGlyphRect, GetCurrentCoreImage, ifmNormal);
    end;
  finally
    dxGpEndPaint(AGPGraphics);
  end;
...
begin
  ...
  begin
    ...
    try
      ...
      UpdateWindowLayer(Handle, ABasicBitmap);
    finally
      ARotateBitmap.Free;
      ABasicBitmap.Free;
    end;
...
{
function TdxRibbonRadialMenuControl.GetVisibleItemLink(ASectorIndex: Integer): TdxBarItemLink;
begin
  Result := TdxBarItemLink(FInternalVisibleLinks[ASectorIndex]);
end;
}

function TdxRibbonRadialMenuControl.GetCoreItemLink: TdxBarItemLink;
begin
  Result := FInternalItemLinks[0];
end;
...
begin
  ...
  begin
    ...
    begin
      ABackArrowBitmap := TcxAlphaBitmap.CreateSize(18, 13, True);
      try
        AGPGraphics := dxGpBeginPaint(ABackArrowBitmap.Canvas.Handle, ABackArrowBitmap.ClientRect);
        try
          AGPGraphics.SmoothingMode := smAntiAlias;
          AGPGraphics.Line(2, 6, 18, 6, FBackArrowColor, 3);
          AGPGraphics.Line(10, -2, 1, 7, FBackArrowColor, 3);
          AGPGraphics.Line(10, 14, 1, 5, FBackArrowColor, 3);
        finally
          dxGpEndPaint(AGPGraphics);
        end;
        FBackArrowImage := TdxSmartImage.CreateFromBitmap(ABackArrowBitmap);
      finally
    ...
    case FExpandState of
      rmsExpanded:
        begin
          FreeAndNil(FHideAnimation);
          FAnimationStep := 0;
          FShowAnimation := TdxAnimationTransition.Create(650, ateLinear, AnimationShowCycle);
      ...
      rmsCollapsed:
        ...
        begin
          ...
          FHideAnimation := TdxAnimationTransition.Create(500, ateLinear, AnimationHideCycle);
...
begin
  CallNotify(FRadialMenu.FOnCloseUp, FRadialMenu);
  Free;
end;
...
begin
  ...
  begin
    ...
    ALast := FDropDownsList.Last;
    FDropDownsList.Remove(ALast);
    SynchronizeInternalItems;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      AMoreButtonsLink := FInternalItemLinks.Add(AMoreButtonsItem);
...
begin
  ...
  CallNotify(FRadialMenu.FOnExpand, FRadialMenu);
end;

procedure TdxRibbonRadialMenuControl.HideAnimationTerminated(Sender: TObject);
begin
  CallNotify(FRadialMenu.FOnCollapse, FRadialMenu);
  FHideAnimation := nil;
  if FMenuMode = rmmAutoHide then

...
type
  ...
  TdxTabbedMDIManager = class(TcxCustomComponent, IcxControlComponentState, IdxSkinSupport,
  ...
  protected
    ...
    procedure TabPropertiesGetTabHintHandler(Sender: TObject; ATabIndex: Integer;
...
uses
  Math, dxOffice11, dxTabbedMDIStrs, Types, dxMessages;
...
begin
  RedrawWindow(AWnd, nil, 0, RDW_INVALIDATE or RDW_ERASE or RDW_ALLCHILDREN or
...
type
  ...
  TdxTabbedMDIClientControl = class(TcxControl, IcxControlComponentState,
    IcxTabControl, IcxMouseTrackingCaller, IcxMouseTrackingCaller2)
  private
    FWindowProcObject: TcxWindowProcLinkedObject;
  ...
  try
    ...
    if AForm.WindowState = wsNormal then
      ...
      if AForm.WindowState = wsMaximized then
        FParent.FTabbedMDIManager.FMDIChildsMaximized := True
      else  //wsMinimized
        FParent.FTabbedMDIManager.FMinimizedMDIChilds.Add(AForm);
  finally
...
begin
  ...
  FTabbedMDIManager.FMinimizedMDIChilds.Remove(FForm);
end;
...
begin
  ...
  FMinimizedMDIChilds := TList.Create;
end;

destructor TdxTabbedMDIManager.Destroy;
begin
  Active := False;
  FreeAndNil(FMinimizedMDIChilds);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          FMinimizedMDIChilds.Add(AChildForm);
      end;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      if FMDIChildsMaximized then
        ...
        for I := 0 to FMinimizedMDIChilds.Count - 1 do
          TForm(FMinimizedMDIChilds[I]).WindowState := wsMinimized;
      UpdateScrollBarsArea;
    end;
  end;
end;
...
procedure TdxTabbedMDIManager.TabPropertiesGetTabHintHandler(Sender: TObject; ATabIndex: Integer;
  var AHint: string; var ACanShow: Boolean);
begin
  if Assigned(FOnGetTabHint) then
    FOnGetTabHint(Self, TabProperties.GetPage(ATabIndex), AHint, ACanShow);
end;

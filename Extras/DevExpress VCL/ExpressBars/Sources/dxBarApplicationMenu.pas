...
{$ENDIF}
  ...
  dxCore, cxClasses, cxGraphics, cxGeometry, cxControls, cxContainer, cxLookAndFeels,
...
function TdxBarApplicationMenuItemLinks.GetOptionsValue(
  AOption: TdxBarItemOptionValue; AItemLinks: TdxBarItemLinks): Variant;
begin
...
begin
  case AOption of
    ioShowDescriptions:
      Result := (AItemLinks <> Self) or not GetIsExtraPaneVisible;
    ioShowShortCuts:
      Result := False;
    else {ioSize}
      Result := misLarge;
...
begin
  ...
  try
    ...
    ACanvas.DrawTexT(Caption, HeaderTextRect, taLeftJustify, vaCenter, False, False);
  finally
...
procedure TdxBarExtraPaneControl.SetAccelSelectedItem(
  AItemControl: TdxBarItemControl; ADuplicate: Boolean);
begin
...
begin
  Result := cxRectHeight(ItemsRect) div
    Painter.GetButtonHeight(Painter.GetSmallIconSize, GetTextSize);
end;
...
begin
  if not HasExtraPane then
    ...
    if ExtraPane.Size <> 0 then
      Result := MulDiv(ExtraPane.Size, TextSize, Painter.SubMenuControlNormalItemHeight)
    else
...
function TdxBarApplicationMenuButtonItem.GetGlyph: TBitmap;
begin
  if FExternalItem <> nil then

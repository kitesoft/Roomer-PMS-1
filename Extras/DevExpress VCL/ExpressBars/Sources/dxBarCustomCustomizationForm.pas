...
{$ENDIF}
  ...
  cxControls, cxClasses, dxMessages, dxBar, cxLookAndFeelPainters, cxLookAndFeels;
...
{$ENDIF}
  ...
  cxGraphics, Math, cxGeometry, dxBarCustForm;
...
begin
  if AItem.Glyph.Empty then
    with AItem.GetCurrentImages do
...
begin
  ...
  ListBox.ItemHeight := cxTextHeight(ListBox.Canvas.Handle) + 2;
...
begin
  case Message.Msg of
    DXM_BAR_LB_SYNCHRONIZE: SynchronizeBarListBox(TdxBar(Message.WParam), Message.LParam);
    DXM_BAR_LB_SYNCHRONIZESELECTION: Owner.SynchronizeListBoxSelection(ListBox);
    DXM_BAR_LB_UPDATEEVENTS: Owner.UpdateToolBarsEvents;
  end;
...
begin
  ...
  begin
    ...
    case Message.Msg of
      ...
      CN_DRAWITEM: CNDrawItem(TWMDrawItem(Message).DrawItemStruct^);
      DXM_BAR_LB_SYNCHRONIZE: SynchronizeItemList;
      DXM_BAR_LB_SYNCHRONIZESELECTION: Owner.SynchronizeListBoxSelection(ListBox);
      DXM_BAR_LB_UPDATEEVENTS: Owner.UpdateItemsListEvents;
    end;
  end;
end;

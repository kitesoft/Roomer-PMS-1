...
{$ENDIF}
  ...
  dxCore, dxCoreClasses, cxClasses, cxGraphics, cxGeometry, dxTouch, cxScrollBar,
  dxBar, cxAccessibility, dxBarAccessibility, dxRibbon, dxRibbonForm, cxControls,
  cxLookAndFeelPainters, dxRibbonSkins, dxAnimation;
...
type
  ...
  protected
    ...
    function GetTextFlags(AnAlignment: TAlignment): DWORD;
  public
...
{$IFDEF DELPHI6}
  Types,
{$ENDIF}
  Math, cxContainer, dxBarSkinConsts, dxBarStrs, dxOffice11, cxLookAndFeels;
...
begin
  FGalleryItem.GalleryChanged;
  FGalleryItem.Recalculate;
end;
...
procedure TdxRibbonGalleryGroupItem.SetImageIndex(Value: TcxImageIndex);
begin
...
begin
  AItem.Name := CreateUniqueName(nil, Group, AItem, 'TdxRibbonGalleryGroup', '', Count + 1);
end;
...
procedure TdxRibbonGalleryGroup.ItemsChange(Sender: TObject;
...
begin
  AItem.Name := CreateUniqueName(nil, GalleryItem, AItem, 'TdxRibbonGallery', '', Count + 1);
end;
  ...
  begin
    ...
    begin
      ScrollBar.SetScrollParams(0, Max(0, AGalleryHeight - 1),
        ViewInfo.LayoutOffset, Max(0, ClientHeight), False);
...
begin
  ...
  begin
    ...
    SendMessage(Parent.Handle, WM_LBUTTONDOWN, ShiftStateToKeys(KeyboardStateToShiftState),
...
function TdxRibbonGalleryGroupElementViewInfo.GetTextFlags(
  AnAlignment: TAlignment): DWORD;
const
  AlignmentsHorz: array[TAlignment] of DWORD = (DT_LEFT, DT_RIGHT, DT_CENTER);
begin
  Result := DT_SINGLELINE or DT_VCENTER or AlignmentsHorz[AnAlignment];
end;
...
procedure TdxRibbonGalleryGroupItemViewInfo.Paint(ACanvas: TcxCanvas);
  ...
  begin
    ...
    try
      ACanvas.Pen.Color := Painter.DropDownGalleryGetPartColor(DXBAR_GALLERYGROUPITEM_OUTERBORDER, AState);
      ARect := FImageBounds;
      ACanvas.Canvas.Rectangle(ARect);
      ACanvas.Pen.Color := Painter.DropDownGalleryGetPartColor(DXBAR_GALLERYGROUPITEM_INNERBORDER, AState);
...
begin
  ...
  if IsSmallButtonSelected then
    Painter.DropDownGalleryDrawItem(ACanvas.Handle, Bounds, CalculateSmallButtonState)
  else
  ...
  if IsDrawSelected then
    DrawSelectionRect(FImageBounds, CalculateSelectionState);
  FChanged := False;
end;
...
procedure TdxRibbonGalleryGroupItemViewInfo.DrawItemText(ACanvas: TcxCanvas);
  ...
  begin
    ...
    if FDescriptionRowCount = 1 then
      Result := DT_SINGLELINE or DT_VCENTER or Result;
  end;
...
begin
  Result := dxGetImageSize(GetCurrentGlyph, GroupItem.Group.Images, 0);
end;
...
begin
  ...
  try
    ACanvas.SetClipRegion(TcxRegion.Create(GalleryBounds), roIntersect);
...
procedure TdxInRibbonGalleryControlViewInfo.Paint(ACanvas: TcxCanvas);
  ...
  begin
    ...
    try
      ...
      cxBitBlt(ACanvas.Canvas.Handle, ABitmap.Canvas.Handle, GetControlBounds, cxNullPoint, SRCCOPY);
    finally
      ABitmap.Free;
    end;
  end;
...
procedure TdxInRibbonGalleryControlViewInfo.DoScrolling(ARowDelta: Integer);
  ...
  begin
    if ARowDelta < 0 then
      ...
      ANewTopVisibleRow := Min(FTopVisibleRow + 1,
        Ceil(GetGroupItemCount(Control.GetGroups.Count - 1) / AColumnCount));
  ...
  procedure DrawOnBitmap(ABitmap: TcxBitmap32; AColumnCount: Integer;
    AGroupItemsList: TObjectList; AOffset: Integer = 0);
  begin
    CalculateLayout(GetTopLayoutIndent, AColumnCount, AGroupItemsList);

    ABitmap.Width := cxRectWidth(GalleryBounds);
    FAnimationBitmapHeight := RowCount * Groups[0].GetRowHeight;
    ABitmap.Height := FAnimationBitmapHeight;

    ABitmap.Clear;
    try
      ABitmap.cxCanvas.WindowOrg := Point(GalleryBounds.Left,
        GalleryBounds.Top - AOffset * (RowCount - 1) * ABitmap.Height div RowCount + 1);
...
var
  AColumnCount: Integer;
  AFirstVisibleRowIndex, ALastVisibleRowIndex: Integer;
  AGroupItemsList: TObjectList;
  AStartBitmap, AEndBitmap: TcxBitmap32;
...
begin
  ...
  begin
    ...
    try
      if ARowDelta > 0 then
        PopulateGroupItemList(AFirstVisibleRowIndex, ALastVisibleRowIndex, AColumnCount,
          AGroupItemsList)
      else
        PopulateGroupItemList(AFirstVisibleRowIndex + 1, ALastVisibleRowIndex, AColumnCount,
          AGroupItemsList);
      ...
      FAnimation := TdxImageAnimationTransition.Create(AStartBitmap, AEndBitmap, 350 div RowCount,
        AAnimationMode, ateLinear, AEndBitmap.Height div RowCount);
...
begin
  ...
  AGroupItemsList := TObjectList.Create(False);
  try
    PopulateGroupItemList(FTopVisibleRow,
      FTopVisibleRow + (RowCount - 1), AColumnCount, AGroupItemsList);
...
begin
  ...
  while (I <= Control.GetGroups.Count - 1) and
    (ACurrentGroupItem + GetGroupItemCount(I) <= AFirstGroupItem) do
  begin
    ACurrentGroupItem := ACurrentGroupItem + GetGroupItemCount(I);
    Inc(I);
  end;
  AList.Clear;
...
begin
  ...
  begin
    ...
    FFilterBandContentRect := cxRectContent(FFilterBandRect,
      Painter.DropDownGalleryGetContentOffsets(DXBAR_GALLERYFILTERBAND));
  end;
end;
...
begin
  if GalleryItem.IsFilterVisible then
  begin
    FillRectByColor(Control.Canvas.Handle, cxRectInflate(FFilterBandRect, 0, 0,
      0, FilterBandOffset), Painter.DropDownGalleryGetPartColor(DXBAR_DROPDOWNBORDER_INNERLINE));
...
procedure TdxRibbonOnMenuGalleryControlViewInfo.DrawFilterCaption;
  ...
  begin
    Result := DT_SINGLELINE or DT_LEFT or DT_TOP or DT_END_ELLIPSIS;
  end;
...
begin
  ...
  ACanvas.Font.Color := Painter.DropDownGalleryGetPartColor(
    DXBAR_GALLERYFILTERBANDTEXT, GetFilterSkinState);
...
begin
  ...
  AContentOffset := Painter.DropDownGalleryGetContentOffsets(DXBAR_GALLERYFILTERBAND);
  Inc(Result, AContentOffset.Top + AContentOffset.Bottom);
end;
...
procedure TdxRibbonGalleryScrollBarPainter.Paint(ACanvas: TcxCanvas);
  ...
  begin
    Result := (BarNavigationController.SelectedObject <> nil) and
      (BarNavigationController.SelectedObject.GetHelper = GetGalleryControl.IAccessibilityHelper.GetHelper) or
      ScrollBar.IsDropDownButtonUnderMouse;
    Result := Result and not GetGalleryControl.IsDroppedDown and
      not (ViewInfo.HotPart in [sbpLineUp, sbpLineDown]);
  end;
...
begin
  ...
  begin
    ...
    ScrollBar.GetRibbonSkin.DrawBackground(ACanvas.Handle, ViewInfo.DropDownButtonRect,
      DXBAR_INRIBBONGALLERYSCROLLBAR_DROPDOWNBUTTON, ScrollBar.GetButtonSkinState(ADropDownButtonState));
  end;
  ...
  begin
    ...
    begin
      if not ScrollBar.IsButtonEnabled(GetButtonKind) then
        AState := cxbsDisabled;
      ScrollBar.GetRibbonSkin.DrawBackground(ACanvas.Handle, R, GetButtonSkinPart,
        ScrollBar.GetButtonSkinState(AState));
    end;
  end
  else
    ScrollBar.GetBarPainter.DropDownGalleryDrawScrollBarPart(ACanvas.Handle, R, APart, AState);
end;

procedure TdxRibbonGalleryScrollBarPainter.DrawScrollBarBackground(
...
begin
  ...
  begin
    ...
    ScrollBar.GetBarPainter.DropDownGalleryDrawScrollBarBackground(ACanvas.Handle, R);
end;
...
begin
  inherited Create(AOwner);
  Supports(AOwner, IInterfaceComponentReference, AComponentRefrence);
  FRibbonScrollBar := AComponentRefrence.GetComponent as TdxRibbonGalleryScrollBar;
end;
...
begin
  ...
  begin
    AButtonHeight := Painter.DropDownGalleryGetSizingBandHeight(
      AGalleryControl.BarManager.ImageOptions.GlyphSize, AGalleryControl.Font);
...
procedure TdxRibbonDropDownGalleryControlPainter.SubMenuControlDrawBorder(
  ...
  begin
    ...
    case ADropDownGallery.Resizing of
      ...
      gsrWidthAndHeight:
        Painter.DropDownGalleryDrawBottomSizeGrip(DC, R);
    end;
  end;
  ...
  begin
    ...
    case ADropDownGallery.Resizing of
      ...
      gsrWidthAndHeight:
        Painter.DropDownGalleryDrawTopSizeGrip(DC, R);
    end;
  end;
...
begin
  Result := (AScrollKind = InternalGalleryItemControl.ScrollBar.Kind) and
     InternalGalleryItemControl.ScrollBar.Enabled;
end;
...
begin
  ...
  begin
    ...
    for I := 0 to ItemLinks.Count - 1 do
      ItemLinks[I].ItemRect := cxRectOffset(ItemLinks[I].ItemRect, 0, AGalleryHeight);
  end
  else
    AGalleryHeight := cxRectHeight(VisibleItemsRect);
  InternalGalleryItemLink.ItemRect := cxRectSetHeight(VisibleItemsRect, AGalleryHeight);
...
begin
  ...
  begin
    FGalleryItemItemLinks.BeginCalcItemRects;
    try
      CalcControlsPositions;
    finally
      FGalleryItemItemLinks.EndCalcItemRects;
    end;
...
var
  AGalleryWidth, AGalleryHeight: Integer;
begin
  Result := inherited GetItemsPaneSize;
  AGalleryWidth := InternalGalleryItemControl.Width;
  AGalleryHeight := InternalGalleryItemControl.Height;
  if Result.cx > AGalleryWidth then
    AGalleryHeight := InternalGalleryItemControl.ViewInfo.GetHeight(Result.cx)
  else
    Result.cx := AGalleryWidth;
  Inc(Result.cy, AGalleryHeight);
end;
...
function TdxRibbonDropDownGalleryControl.GetMinSize: TSize;
  ...
  procedure IncByOffset(var R: TSize; const AOffset: TRect);
  begin
    Inc(R.cx, AOffset.Left + AOffset.Right);
    Inc(R.cy, AOffset.Top + AOffset.Bottom);
  end;
...
begin
  ...
  for I := 0 to ItemLinks.VisibleItemCount - 1 do
    AItemsHeight := AItemsHeight + cxRectHeight(ItemLinks[I].ItemRect);
  IncByOffset(Result, GetClientOffset);
  IncByOffset(Result, GetItemsRectOffset);
  inc(Result.cy, AItemsHeight);
end;
...
procedure TdxRibbonDropDownGalleryControl.DoResizing;
  ...
  begin
    ...
    SetWindowPos(Handle, 0, ANewRect.Left, ANewRect.Top, cxRectWidth(ANewRect),
      cxRectHeight(ANewRect), AFlags);
  end;
...
begin
  Result := FGalleryItemItemLinks[0];
end;

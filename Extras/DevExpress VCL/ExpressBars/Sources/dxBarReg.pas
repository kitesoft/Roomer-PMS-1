...
uses
  ...
  dxCoreReg, cxLibraryReg, cxPropEditors, cxDesignWindows, cxControls, dxBar,
...
uses
  ...
  dxBarStrs, dxBarCustomCustomizationForm, dxBarCustForm, dxBarPopupMenuEd, dxBarConverter;
...
function TdxBarManagerEditor.IsAssignable: Boolean;
begin
  Result := True;
end;

procedure TdxBarManagerEditor.DoAssign(AObject: TObject);
begin
  dxBarConvert(AObject as TMenu, BarManager, Designer);
end;

function TdxBarManagerEditor.GetAssignItemCaption: string;
begin
  Result := 'Convert Menu';
end;

function TdxBarManagerEditor.GetAssignTypeClass: TClass;
begin
  Result := TMenu;
end;
...
{$ENDIF}
  ...
  RegisterComponents(dxBarProductPage, [TdxBarManager,
    TdxBarPopupMenu, TdxBarDockControl, TdxBarScreenTipRepository]);
  RegisterNoIcon([
    TdxBarGroup, TdxBarButton, TdxBarLargeButton,

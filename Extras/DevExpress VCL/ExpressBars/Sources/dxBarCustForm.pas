...
{$ENDIF}
  ...
  dxCore, dxMessages, cxClasses, dxBar, dxBarCustomCustomizationForm;
...
//      SendMessage(LAllCommands.Handle, WM_SETREDRAW, WPARAM(False), 0);
      LAllCommands.Items.BeginUpdate;
      try
...
begin
  case Message.Msg of
    DXM_BAR_LB_SYNCHRONIZE: SynchronizeCommandList;
    DXM_BAR_LB_SYNCHRONIZESELECTION: SynchronizeListBoxSelection(LAllCommands);
  end;
...
begin
  case Message.Msg of
    DXM_BAR_LB_SYNCHRONIZE: SynchronizeGroupList;
    DXM_BAR_LB_SYNCHRONIZESELECTION: SynchronizeListBoxSelection(lbGroups);
    DXM_BAR_LB_UPDATEEVENTS: UpdateGroupEvents;
  end;
...
begin
  case Message.Msg of
    DXM_BAR_LB_SYNCHRONIZE: SynchronizeGroupItemList;
    DXM_BAR_LB_SYNCHRONIZESELECTION: SynchronizeGroupListSelection;
    DXM_BAR_LB_UPDATEEVENTS: UpdateGroupItemEvents;
  end;
...
begin
  if Key = VK_DELETE then
    DeleteSelectedObjects(LAllCommands);
end;

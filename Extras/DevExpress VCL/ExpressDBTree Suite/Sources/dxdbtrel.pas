...
uses 
  ...
  dxdbtree, dxtree, dbctrls, dxCore, dxCommon, cxControls, cxGeometry, cxGraphics;
...
uses
  dxlocate, commctrl, DBConsts, cxLookAndFeelPainters;
...
begin
  AEditRect := cxGetWindowRect((Owner as TWinControl).Handle);
  ADropDownRect := cxGetWindowRect(Handle);
...
begin
  ...
  begin
    ...
    cxLookAndFeelPaintersManager.GetPainter(lfsFlat).CalculatePopupPanelClientRect(
      AClientRect, APanelRect, FGripRect, FCloseButtonRect, FCorner,
      cxSimpleRect, GetSysPanelBorders);
...
begin
  inherited;
  if TCustomdxVTreeViewEdit(Owner).FListVisible then
    cxLookAndFeelPaintersManager.GetPainter(lfsFlat).DrawPopupNCPanel(
      Handle, FMouseAboveCloseButton, FCloseButtonIsTracking, FCorner,
      FCloseButtonRect, FGripRect, clWindowFrame);
end;

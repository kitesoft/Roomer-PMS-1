...
uses
  Types, Windows, Messages, ShellAPI, TypInfo, SysUtils, Classes, Forms, Controls,
  Contnrs, SyncObjs, dxCore, dxCoreClasses, dxMessages;
...
type
  ...
  TcxHashedStringList = class(TObject)
  private
    FList: TStringList;
  ...
  protected
    ...
    function AddItem(AParentComponent, AOwner: TComponent): TcxComponentCollectionItem;
...
uses
  RTLConsts, Math, Graphics, cxGraphics, cxControls;
...
begin
  inherited;
  FPixelsPerInch := cxGetCurrentDPI;
  FScalable := True;
end;
...
begin
  inherited;
  if Scalable and (cxGetCurrentDPI <> PixelsPerInch) then
    ChangeScale(cxGetCurrentDPI, PixelsPerInch);
  PixelsPerInch := cxGetCurrentDPI;
end;
...
begin
  Result := AddItem(ParentComponent, ParentComponent.Owner);
  SetItemName(Result);
end;
...
function TcxComponentCollection.AddItem(AParentComponent, AOwner: TComponent): TcxComponentCollectionItem;
begin
  Result := FItemClass.Create(AOwner);
  Result.SetParentComponent(AParentComponent);
end;

function TcxComponentCollection.AddInternalItem: TcxComponentCollectionItem;
begin
  Result := AddItem(ParentComponent, nil);
end;
...
begin
  ...
  AItem.Name := CreateUniqueName(AOwnerParent, ParentComponent, AItem, GetItemPrefixName, GetItemSuffixName);
end;

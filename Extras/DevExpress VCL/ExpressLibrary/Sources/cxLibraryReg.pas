...
uses
  ...
  cxContainer, cxLookAndFeels, cxClasses, cxStyles, cxControls,
  cxLookAndFeelPainters, dxScreenTip,
  cxDesignWindows, cxStyleRepositoryEditor, cxImageListEditor, dxCoreReg,
  dxGallery, cxScrollBar;
...
type
  ...
  TdxSmartImageGraphicProperty = class(TGraphicProperty)
  public
    procedure Edit; override;
  end;
...
uses
  VCLEditors, Types, Math, Dialogs, ImgList, ToolsApi,
  cxGraphics, cxPropEditors, cxLocalization, dxCore,
    ...
    begin
      ...
      try
        for I := 0 to ADialog.Files.Count - 1 do
          ImageCollection.Items.Add.Picture.LoadFromFile(ADialog.Files[I]);
      finally
...
begin
  if (AGraphic = nil) or (AGraphic is GraphicClass) then
    SetOrdValue(TdxNativeInt(AGraphic))
  else
  begin
    ABitmap := cxGetAsBitmap(AGraphic);
    try
      SetOrdValue(TdxNativeInt(ABitmap));
    finally
      ABitmap.Free;
    end;
  end;
...
procedure TdxSmartImageGraphicProperty.Edit;
var
  AEditor: TPictureEditor;
begin
  AEditor := TPictureEditor.Create(nil);
  try
    AEditor.GraphicClass := TdxSmartImage;
    AEditor.Picture.Graphic := TGraphic(Pointer(GetOrdValue));
    if AEditor.Execute then
      SetOrdValue(TdxNativeInt(AEditor.Picture.Graphic));
  finally
    AEditor.Free;
  end;
end;
...
var
  AGallery: IdxGallery;
begin
  if Supports(TObject(GetOrdValue), IdxGallery, AGallery) then
...
{$ENDIF}
  ...
  RegisterComponents(dxLibraryProductPage, [TdxScreenTipRepository]);
...
{$ENDIF}
  ...
  RegisterNoIcon([TcxImageCollectionItem, TdxScreenTip]);
{$IFDEF DELPHI7}

...
type
  ...
  TcxScrollBar = class(TCustomControl, IcxScrollBarOwner, IdxSkinSupport)
  private
...
uses
  Consts, Math, dxCore, dxUxTheme, dxThemeConsts, cxControls, cxGeometry;
...
begin
  ...
  begin
    ...
    begin
      FPartInfos[sbpPageUp].Bounds := Rect(FPartInfos[sbpLineUp].Bounds.Right, Bounds.Top,
        FPartInfos[sbpThumbnail].Bounds.Left, Bounds.Bottom);
      FPartInfos[sbpPageDown].Bounds := Rect(FPartInfos[sbpThumbnail].Bounds.Right, Bounds.Top,
        FPartInfos[sbpLineDown].Bounds.Left, Bounds.Bottom);
    end
    else
    begin
...
begin
  ...
  if FKind = sbHorizontal then
  begin
    FPartInfos[sbpLineUp].Bounds := cxRectBounds(Bounds.Left, Bounds.Top, FArrowButtonLength, GetHeight);
    FPartInfos[sbpLineDown].Bounds := cxRectOffsetHorz(FPartInfos[sbpLineUp].Bounds, FLength - FArrowButtonLength);
  end
  else
  begin
...
begin
  ...
  begin
    ...
    begin
      AThumbLength := GetSystemMetrics(IfThen(FKind = sbHorizontal, SM_CXHTHUMB, SM_CYVTHUMB));
    ...
    if FKind = sbHorizontal then
      FPartInfos[sbpThumbnail].Bounds := cxRectBounds(FPartInfos[sbpLineUp].Bounds.Right + AThumbPos, Bounds.Top, AThumbLength, GetHeight)
    else
...
begin
  Result := IfThen(FKind = sbHorizontal, GetScrollBarSize.cy, GetScrollBarSize.cx);
end;
...
begin
  ...
  FHelper.MouseDown(Button, Shift, X, Y); Exit;
end;
...
procedure TcxScrollBarController.DoMouseDown(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  APart: TcxScrollBarPart;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        FScrollBar.Position := FSavePosition;
    ...
    begin
      ...
      begin
        ...
        FLastMousePos.X := X;
        if (ADelta < 0) and (FSaveThumbnailPos.X + ADelta < ViewInfo.PartInfo[sbpLineUp].Bounds.Right) then
          ADelta := ViewInfo.PartInfo[sbpLineUp].Bounds.Right - FSaveThumbnailPos.X
        else
          if (ADelta > 0) and (FSaveThumbnailPos.X + ASize + ADelta > ViewInfo.PartInfo[sbpLineDown].Bounds.Left) then
            ADelta := ViewInfo.PartInfo[sbpLineDown].Bounds.Left - (FSaveThumbnailPos.X + ASize);
      ...
      begin
        ...
        FLastMousePos.Y := Y;
        if (ADelta < 0) and (FSaveThumbnailPos.Y + ADelta < ViewInfo.PartInfo[sbpLineUp].Bounds.Bottom) then
          ADelta := ViewInfo.PartInfo[sbpLineUp].Bounds.Bottom - FSaveThumbnailPos.Y
        else
          if (ADelta > 0) and (FSaveThumbnailPos.Y + ASize + ADelta > ViewInfo.PartInfo[sbpLineDown].Bounds.Top) then
            ADelta := ViewInfo.PartInfo[sbpLineDown].Bounds.Top - (FSaveThumbnailPos.Y + ASize);
        UpdateThumbnail(0, ADelta);
      end;
      ANewPos := GetPositionFromThumbnail;
      if ANewPos <> FScrollBar.Position then
      begin
        FScrollBar.FPosition := ANewPos;
        DoScroll(sbpThumbnail);
        FScrollBar.Change;
      end;
    end;
...
begin
  ...
  begin
    ...
    ADistance := ViewInfo.PartInfo[sbpLineDown].Bounds.Left - ViewInfo.PartInfo[sbpLineUp].Bounds.Right - AThumbnailSize;
    Result := FScrollBar.Min + MulDiv(ATotal, ViewInfo.PartInfo[sbpThumbnail].Bounds.Left - ViewInfo.PartInfo[sbpLineUp].Bounds.Right,
      ADistance);
  end
  else
  begin
...
var
  ANewPos: Longint;
begin
  ANewPos := FScrollBar.Position;
  case AScrollCode of
    scLineUp:
      Dec(ANewPos, FScrollBar.SmallChange);
    scLineDown:
      Inc(ANewPos, FScrollBar.SmallChange);
    scPageUp:
      Dec(ANewPos, FScrollBar.LargeChange);
    scPageDown:
      Inc(ANewPos, FScrollBar.LargeChange);
    scTop:
        ANewPos := FScrollBar.Min;
    scBottom:
        ANewPos := FScrollBar.Max;
  end;
  ANewPos := EnsureRange(ANewPos, FScrollBar.Min, FScrollBar.Max);
  FScrollBar.Scroll(AScrollCode, ANewPos);
  begin
    ANewPos := EnsureRange(ANewPos, FScrollBar.Min, FScrollBar.Max);
    if ANewPos <> FScrollBar.Position then
    begin
      if AScrollCode <> scTrack then
        FScrollBar.Position := ANewPos
      else
      begin
        FScrollBar.FPosition := ANewPos;
        FScrollBar.Repaint;
...
var
  ABitmap: TcxBitmap;
begin
  ABitmap := TcxBitmap.CreateSize(BoundsRect);
  try
    ABitmap.cxCanvas.WindowOrg := BoundsRect.TopLeft;
    DoPaint(ABitmap.cxCanvas);
    cxBitBlt(ADC, ABitmap.cxCanvas.Handle, BoundsRect, BoundsRect.TopLeft, SRCCOPY);
  finally
    ABitmap.Free;
  end;
end;
...
begin
  R := ARect;
  ANCOrigin := ScreenToClient(cxGetWindowRect(Handle).TopLeft);
...
begin
  LookAndFeelPainter.DrawScrollBarPart(ACanvas, FScrollBar.Kind = sbHorizontal, R, APart, AState);
end;

procedure TcxScrollBarPainter.DrawScrollBarBackground(ACanvas: TcxCanvas; const R: TRect);
begin
  LookAndFeelPainter.DrawScrollBarBackground(ACanvas, R, FScrollBar.Kind = sbHorizontal);
end;
...
begin
  Result := LookAndFeelPainter.ScrollBarMinimalThumbSize(FScrollBar.Kind = sbVertical);
end;

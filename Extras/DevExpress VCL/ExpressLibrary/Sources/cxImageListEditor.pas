...
uses
  Types, Math, cxImageListEditorView, dxCore, cxFormats;
...
begin
  inherited;
  FAlphaUsed := dxIsAlphaUsed(Image);
end;
...
begin
  for I := 0 to DataItems.Count - 1 do
    TcxImageListAccess(FImageList).ConvertImageInfoTo32bit(TcxImageInfo(DataItems[I].Data));
...
procedure TcxImageListEditor.ExportImages(const AFileName: string; AFormat: TcxImageType = itBitmap);
  ...
  begin
    Result := MessageDlg(Format('File %s is already exists.'#13#10'Do you want to replace it?',
...
begin
  ...
  begin
    ...
    if not IsAnyImageSelected then
      SelectAllImages;
    AExportImage := TcxBitmap.CreateSize(ImageList.Width * GetSelectionCount, ImageList.Height, pf32bit);
    try
...
function TcxImageListEditor.InternalAddImage(AImage, AMask: TBitmap; AFileName: string;
  ...
  const
    scxBitmapSplitQuery = 'The bitmap in the file %s is too large.'#13#10 +
...
begin
  ...
  if ((ImageWidth <> AValue.cx) or (ImageHeight <> AValue.cy)) and
    ...
    begin
      Result := True;
      ImageList.SetSize(AValue.cx, AValue.cy);

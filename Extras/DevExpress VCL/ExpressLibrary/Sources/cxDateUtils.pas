...
uses
  Variants, SysUtils, Windows, Controls, Classes, Graphics,
...
begin
  ...
  begin
    case AnsiLowerCase(ADateFormat[APos])[1] of
      ...
      'd':
        begin
          ACountChar := GetCountOfChar(ADateFormat, 'd', APos);
          if ACountChar = 3 then
            AddToDateParth(Result, dxFormatSettings.ShortDayNames[ADayOfWeek], ACurrentSeparator)
          else
            if ACountChar = 4 then
              AddToDateParth(Result, dxFormatSettings.LongDayNames[ADayOfWeek], ACurrentSeparator)
            else
...
function cxGetLocalTimeSeparator: Char;
begin
  Result := dxGetLocaleInfo(GetThreadLocale, LOCALE_STIME)[1];
end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if (Result >= MaxInt) or (Result <= -MaxInt - 1) or (ADay <= 0) or (AYear > dxMaxYear) then
          Result := NullDate;
      end
      else

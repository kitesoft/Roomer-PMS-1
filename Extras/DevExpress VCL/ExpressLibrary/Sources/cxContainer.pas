...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, Windows, Messages, Classes, Controls, Forms, Graphics, Menus, StdCtrls, SysUtils,
  dxCore, dxCoreClasses, dxMessages, dxThemeManager, dxUxTheme, cxClasses, cxControls,
  dxTouch, cxGraphics, cxLookAndFeels, cxScrollBar, cxLookAndFeelPainters;
...
type
  ...
  protected
    ...
    function GetScrollBarEnabled(AScrollBar: IcxControlScrollBar;
      const AScrollBarinfo: TcxScrollBarInfo): Boolean; virtual;
  ...
  public
    ...
    function HasInnerControl: Boolean; {$IFDEF DELPHI9} inline; {$ENDIF}
    function HasInnerControlHandle: Boolean; {$IFDEF DELPHI9} inline; {$ENDIF}
...
uses
  dxThemeConsts, dxOffice11, cxDWMApi, cxGeometry, dxHooks;
...
function InternalIsWindowVisible(AControl: TWinControl): Boolean;
begin
  Result := AControl.HandleAllocated and IsWindowVisible(AControl.Handle);
end;
...
procedure SetRegionToWindow(AWindowHandle: THandle; ANewRegion: TcxRegionHandle);

  function IsWindowRegionChanged(AWindowHandle: THandle; ANewRegion: TcxRegionHandle): Boolean;
  var
    ARgnType: Integer;
    ACurrentRegion: HRGN;
  begin
...
begin
  Result := HasInnerControlHandle and not (csDestroying in ComponentState) and
...
begin
  AWindowFromPoint := WindowFromPoint(GetMouseCursorPos);
  if HandleAllocated and IsChildEx(Handle, AWindowFromPoint) then
    ACursorPos := GetMouseCursorClientPos
  else
    ACursorPos := cxInvalidPoint;
  Result := RefreshContainer(ACursorPos, cxmbNone, KeyboardStateToShiftState, AIsMouseEvent);
end;
...
begin
  Result := inherited CanFocusOnClick and
    not (HasInnerControlHandle and FInnerControl.Focused or InnerControlMouseDown);
end;
...
begin
  if ViewInfo.FCalculated and HasInnerControl and not EqualRect(InnerControl.BoundsRect, ViewInfo.ClientRect) then
...
begin
  Result := (AScrollBar.Control <> nil) and (AScrollBar.Control.Parent <> nil) and
    AScrollBar.Control.Parent.Enabled and
...
procedure TcxCustomInnerListBox.SetExternalScrollBarsParameters;
  ...
  begin
    ...
    repeat
      ...
      R.TopLeft := Parent.ScreenToClient(AScrollBarInfo.rcScrollBar.TopLeft);
      R.Right := cxRectWidth(AScrollBarInfo.rcScrollBar);
      R.Bottom := cxRectHeight(AScrollBarInfo.rcScrollBar);

      if (R.Left < 0) or (R.Right > Width) or (R.Top < 0) or (R.Bottom > Height) then
        AScrollBarVisible := False
      else
...
begin
  ...
  begin
    ...
    try
      P := cxClientToParent(Self, Point(X, Y), Container);
      Container.MouseDown(Button, Shift, P.X, P.Y);
    finally
...
begin
  ...
  begin
    P := cxClientToParent(Self, Point(X, Y), Container);
    Container.MouseMove(Shift, P.X, P.Y);
  end;
end;
...
begin
  ...
  begin
    P := cxClientToParent(Self, Point(X, Y), Container);
    Container.MouseUp(Button, Shift, P.X, P.Y);
  end;
end;
...
procedure cxContainerWndProcHook(ACode: Integer; wParam: WPARAM; lParam: LPARAM; var AHookResult: LRESULT);
  ...
  begin
    ...
    Result := (AForm1 <> AForm2) and (AForm1 <> nil) and (AForm2 <> nil) and
      (AForm1.HandleAllocated) and (AForm2.HandleAllocated) and (IsChildEx(AForm1.Handle, AForm2.Handle) or IsChildEx(AForm2.handle, AForm1.Handle));
  end;
...
var
  AHandled: Boolean;
begin
  if (ACode = HC_ACTION) and IsMouseDownMessage(WParam) and
    cxPopupVisibleController.IsAnyVisible then
  begin

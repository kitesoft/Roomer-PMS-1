...
{$ENDIF}
  ...
  dxCore, dxMessages, cxClasses, cxGeometry, dxGDIPlusClasses, dxGDIPlusApi;
...
{$ENDIF}
  ...
  protected
    ...
    procedure DoCombine(ARegionHandle: TcxRegionHandle; AOperation: TcxRegionOperation);
  public
  ...
  public
    ...
    procedure Rectangle(R: TRect; const AParams: TcxViewParams;
      const ABorders: TcxBorders = []; ABorderColor: TColor = clDefault;
    ...
    procedure FrameRect(const R: TRect; Color: TColor = clDefault;
      ...
      AExcludeFrame: Boolean = False); overload;
  ...
  protected
    ...
    procedure ConvertImageInfoTo32bit(AImageInfo: TcxImageInfo);
  ...
  public
    ...
    class function HSLToColor(const AHSL: TdxHSL): TColor;
    class function HSVToColor(const AHSV: TdxHSV): TColor;
  end;
...
function cxColorIsValid(AColor: TColor): Boolean; {$IFDEF DELPHI9}inline;{$ENDIF}
...
procedure cxDrawImage(ADC: THandle; AGlyphRect, ABackgroundRect: TRect; AGlyph: TBitmap;
  ...
  ATransparentColor: TColor = clNone; AUseLeftBottomPixelAsTransparent: Boolean = True); overload;
procedure cxDrawImage(ACanvas: TcxCanvas; const ARect: TRect;
  AGlyph: TBitmap; AImages: TCustomImageList; AImageIndex: Integer; // image info
  AFitMode: TcxImageFitMode; ADrawMode: TcxImageDrawMode = idmNormal); overload;
procedure cxDrawImage(ACanvas: TcxCanvas; const ARect: TRect;
  AGlyph: TBitmap; AImages: TCustomImageList; AImageIndex: Integer; AEnabled: Boolean); overload;
procedure cxDrawPicture(ACanvas: TcxCanvas; const ARect: TRect;
  APicture: TPicture; AFitMode: TcxImageFitMode);
procedure cxDrawImage(ACanvas: TcxCanvas; const ARect: TRect;
  AImage: TGraphic; AFitMode: TcxImageFitMode); overload;
...
procedure cxPrepareBitmapForDrawing(AGlyph: TBitmap; AImages: TCustomImageList; AImageIndex: Integer;
  AUseLeftBottomPixelAsTransparent: Boolean; ATransparentColor: TColor;
  out AImageBitmap, AMaskBitmap: TcxAlphaBitmap; out AIsAlphaUsed: Boolean);
...
function cxTextSize(AFont: TFont; const AText: string): TSize; overload;
...
function cxPtInRegion(ARegionHandle: HRGN; const P: TPoint): Boolean; overload; {$IFDEF DELPHI9}inline;{$ENDIF}
function cxPtInRegion(ARegionHandle: HRGN; const X, Y: Integer): Boolean; overload; {$IFDEF DELPHI9}inline;{$ENDIF}
...
uses
  Messages, Math, Contnrs, cxControls, dxUxTheme, dxOffice11,
  dxThemeConsts, dxThemeManager, cxDrawTextUtils;
...
begin
  Result := MulDiv(AValue, cxGetCurrentDPI, 96);
end;
...
begin
  if IsImageAssigned(AImage) then
    ...
    if IsImageAssigned(AImageList, AImageIndex) then
      Result := Size(AImageList.Width, AImageList.Height)
    else
      Result := cxNullSize;
end;
...
begin
  if IsPictureAssigned(APicture) then
    Result := Size(APicture.Graphic.Width, APicture.Graphic.Height)
  else
    Result := cxNullSize;
end;
...
procedure CommonAlphaBlend(ADestDC, ASrcDC: HDC; const ADestRect, ASrcRect: TRect; ASmoothImage: Boolean; AConstantAlpha: Byte = $FF);
  ...
  const
    iModes: array[Boolean] of TdxGPInterpolationMode = (imLowQuality, imHighQualityBicubic);
  begin
...
procedure cxSetBitmapParams(ABitmap: TBitmap; AWidth, AHeight: Integer; AFormat: TPixelFormat); {$IFDEF DELPHI9} inline; {$ENDIF}
begin
...
procedure cxDrawImage(ACanvas: TcxCanvas; const ARect: TRect;
  AGlyph: TBitmap; AImages: TCustomImageList; AImageIndex: Integer; // image info
  AFitMode: TcxImageFitMode; ADrawMode: TcxImageDrawMode = idmNormal);
var
  AGlyphRect: TRect;
  AImageSize: TSize;
begin
  AImageSize := dxGetImageSize(AGlyph, AImages, AImageIndex);
  if cxSizeIsEqual(AImageSize, cxNullSize) then
    Exit;
  AGlyphRect := cxGetImageRect(ARect, AImageSize, AFitMode);
  cxDrawImage(ACanvas.Handle, AGlyphRect, AGlyphRect, AGlyph, AImages, AImageIndex, ADrawMode);
end;

procedure cxDrawImage(ACanvas: TcxCanvas; const ARect: TRect;
  AGlyph: TBitmap; AImages: TCustomImageList; AImageIndex: Integer; AEnabled: Boolean);
begin
  cxDrawImage(ACanvas, ARect, AGlyph, AImages, AImageIndex, ifmNormal, EnabledImageDrawModeMap[AEnabled]);
end;
...
begin
  if APicture <> nil then
    cxDrawImage(ACanvas, ARect, APicture.Graphic, AFitMode);
end;

procedure cxDrawImage(ACanvas: TcxCanvas; const ARect: TRect;
  AImage: TGraphic; AFitMode: TcxImageFitMode);
var
  AGlyphRect: TRect;
  AImageSize: TSize;
begin
  if IsImageAssigned(AImage) then
  begin
    AImageSize := cxSize(AImage.Width, AImage.Height);
    AGlyphRect := cxGetImageRect(ARect, AImageSize, AFitMode);
    ACanvas.StretchDraw(AGlyphRect, AImage);
  end;
end;
...
procedure cxGetTextLines(const AText: string; ACanvas: TcxCanvas; const ARect: TRect; ALines: TStrings);
  ...
  procedure GetNextWordPos(const AWideText: WideString; ALength: Integer; const ACurrentWord: TcxWordPosition; var ANextWord: TcxWordPosition);
  ...
  procedure FindNextWordStart(const AWideText: WideString; var ALineStart: Integer; var ANextWord: TcxWordPosition);
  begin
  ...
  function CheckNewLineSymbol(const AWideText: WideString; var ANextWord: TcxWordPosition; var ALineStart: Integer): Boolean;
  var
    I: Integer;
  begin
    Result := False;
...
function cxTextSize(AFont: TFont; const AText: string): TSize;
var
  ARect: TRect;
begin
  ARect := cxNullRect;
  cxGetTextRect(ARect, AText, AFont, 0);
  Result := Size(ARect.Right, ARect.Bottom);
end;
...
procedure cxPrintWindow(AWindow, ADC: THandle; ADrawNC: Boolean; ADoubleBuffered: Boolean = False); overload;

  function HasNonClientArea(AWindow: THandle): Boolean;
  begin
    Result := not cxSizeIsEqual(cxSize(cxGetWindowRect(AWindow)), cxSize(cxGetClientRect(AWindow)));
  end;
  ...
  begin
    ...
    begin
      if (GetWindowLong(AWindow, GWL_STYLE) and WS_CHILD = 0) and (GetMenu(AWindow) <> 0) then
...
begin
  ...
  begin
    if GetWindowOrgEx(ADC, AWindowOrg) and not cxPointIsEqual(AWindowOrg, cxNullPoint) then
    begin
      ...
      try
        ...
        DoPrintWindow(AMemDC, PRF_NONCLIENT or PRF_ERASEBKGND or PRF_CLIENT);
        cxBitBlt(ADC, AMemDC, cxRect(cxSize(AWindowRect)), cxNullPoint, SRCCOPY);
      finally
      ...
      DoPrintWindow(ADC, PRF_NONCLIENT or PRF_ERASEBKGND or PRF_CLIENT);
  end
  else
    DoPrintWindow(ADC, PRF_ERASEBKGND or PRF_CLIENT);
end;
...
procedure cxPaintControlTo(ADrawingControl: TWinControl; ACanvas: TcxCanvas;
  ...
  begin
    ...
    begin
      ...
      cxPrintWindow(AChildHandle, ACanvas.Handle, True);
      MoveWindowOrg(ACanvas.Handle, -ADrawingRect.Left, -ADrawingRect.Top);
    end;
  end;
...
begin
  DoCombine(ARegion.Handle, AOperation);
...
begin
  ...
  try
    DoCombine(ARegionHandle, AOperation);
  finally
...
procedure TcxRegion.DoCombine(ARegionHandle: TcxRegionHandle; AOperation: TcxRegionOperation);
const
  ...
  begin
    ...
    begin
      ...
      begin
        ADrawImage := ADrawImageListItem or (AMenuItem.Checked and AMenuItem.Bitmap.Empty);
        ACheckBkgRect := ARect;
        ACheckBkgRect :=  cxRectCenterVertically(ARect, Min(IfThen(AImageList <> nil,
          AImageList.Height + 6, 22), cxRectHeight(ARect)));
        ...
        begin
          ...
          begin
            ...
            FillRect(cxRectInflate(ACheckBkgRect, -1, -1));
          ...
          begin
            if ADrawBackground then
              ExcludeClipRect(Handle, ACheckBkgRect.Left, ACheckBkgRect.Top,
                ACheckBkgRect.Right, ACheckBkgRect.Bottom);
...
begin
  FillRect(ABitmap.Canvas.Handle, Rect(0, 0, ABitmap.Width, ABitmap.Height), GetStockObject(BLACK_BRUSH));
end;
...
procedure TcxBitmap32.AlphaBlend(ADestBitmap: TcxBitmap32; const ARect: TRect; ASmoothImage: Boolean; AConstantAlpha: Byte = $FF);
begin
  cxAlphaBlend(ADestBitmap, Self, ARect, ClientRect, ASmoothImage, AConstantAlpha);
end;
...
begin
  if IsGlyphAssigned(FImage) then
    FImage.Dormant;
  if IsGlyphAssigned(FMask) then
    FMask.Dormant;
end;
...
function TcxImageList.AddBitmap(AImage, AMask: TBitmap; AMaskColor: TColor; AStretch, ASmooth: Boolean): Integer;
var
...
begin
  ...
  begin
    ...
    TcxBitmap32(ASizedImage).CopyBitmap(AImage);
    TcxBitmap32(ASizedImage).Resize(Width, Height, AStretch, ASmooth, AMaskColor);
  end
  else
    ASizedImage := AImage;
  ...
  begin
    ...
    TcxBitmap32(ASizedMask).CopyBitmap(AMask);
    TcxBitmap32(ASizedMask).Resize(Width, Height, AStretch, False);
  end
  else
    ASizedMask := AMask;
...
procedure TcxImageList.AddImages(AImages: TCustomImageList);
begin
  if AImages <> nil then
    CopyImages(AImages);
end;

procedure TcxImageList.CopyImages(AImages: TCustomImageList; AStartIndex, AEndIndex: Integer);

  procedure InternalCopy(AcxImages: TcxImageList);
  begin
    if AEndIndex < 0 then
...
var
  AcxImages: TcxImageList;
begin
  BeginUpdate;
  try
    if AImages is TcxImageList then
      InternalCopy(TcxImageList(AImages))
    else
    begin
      AcxImages := TcxImageList.Create(nil);
      try
...
procedure TcxImageList.CancelUpdate;
begin
  Dec(FLockCount);
end;

procedure TcxImageList.EndUpdate;
begin
  Dec(FLockCount);
...
begin
  ...
  begin
    ...
    begin
      ...
      if AMask <> nil then
      begin
        if not TcxImageInfoItem(FImages.Items[AIndex]).ImageInfo.Mask.Empty then
          ASourceMask := TcxImageInfoItem(FImages.Items[AIndex]).ImageInfo.Mask
        else
        begin
          if TcxImageInfoItem(FImages.Items[AIndex]).ImageInfo.FInternalMask.Empty then
            GetImageInfo(Handle, AIndex, nil, TcxImageInfoItem(FImages.Items[AIndex]).ImageInfo.FInternalMask);
          ASourceMask := TcxImageInfoItem(FImages.Items[AIndex]).ImageInfo.FInternalMask;
        end;
...
procedure TcxImageList.SetSize(AWidth, AHeight: Integer);
begin
  AWidth := Max(AWidth, 1);
...
function TcxImageList.AddImageInfo(AImageInfo: TcxImageInfo): Integer;
begin
...
procedure TcxImageList.ConvertImageInfoTo32bit(AImageInfo: TcxImageInfo);
var
  AImageBitmap, AMaskBitmap: TcxAlphaBitmap;
begin
  AImageBitmap := TcxAlphaBitmap.CreateSize(Width, Height);
  AMaskBitmap := TcxAlphaBitmap.CreateSize(Width, Height);

  try
    AImageBitmap.CopyBitmap(AImageInfo.FImage);
    if not AImageBitmap.IsAlphaUsed then
      if AImageInfo.FMaskColor <> clNone then
        AImageBitmap.RecoverTransparency(AImageInfo.FMaskColor)
      else
      begin
        AImageBitmap.TransformBitmap(btmSetOpaque);
        if IsGlyphAssigned(AImageInfo.FMask) then
        begin
          AMaskBitmap.CopyBitmap(AImageInfo.FMask);
          AImageBitmap.Filter(AMaskBitmap);
        end
      end;

    AImageInfo.Image := AImageBitmap;
    AImageInfo.Mask := nil;
...
procedure TcxImageList.ConvertTo32bit;
var
  I: Integer;
  AImageInfo: TcxImageInfo;
  AImageInfos: TList;
begin
  AImageInfos := TObjectList.Create;
  try
    for I := 0 to Count - 1 do
    begin
      AImageInfo := TcxImageInfo.Create;
      GetImageInfo(I, AImageInfo);
      AImageInfos.Add(AImageInfo);
    end;

    BeginUpdate;
    try
      Clear;
      for I := 0 to AImageInfos.Count - 1 do
      begin
        AImageInfo := TcxImageInfo(AImageInfos[I]);
        ConvertImageInfoTo32bit(AImageInfo);
...
procedure TcxImageList.SynchronizeHandle;
var
  I: Integer;
...
begin
  TcxImageInfoItem(FImages.Items[AIndex]).ImageInfo.Dormant;
end;
...
function TcxImageList.GetHeight: Integer;
begin
  Result := inherited Height;
end;

procedure TcxImageList.SetHeight(AValue: Integer);
begin
  SetSize(Width, AValue);
end;

function TcxImageList.GetWidth: Integer;
begin
  Result := inherited Width;
end;

procedure TcxImageList.SetWidth(AValue: Integer);
begin
...
begin
  ...
  begin
    if AHSL.L <= 0.5 then
      ...
      M2 := AHSL.L * (1 - AHSL.S) + AHSL.S;

    M1 := 2 * AHSL.L - M2;
    R := HueToColor(AHSL.H + 1 / 3, M1, M2);
    G := HueToColor(AHSL.H, M1, M2);
    B := HueToColor(AHSL.H - 1 / 3, M1, M2);
  end;
  Result := RGB(R, G, B);
end;
...
var
  ...
  AFrac: Double;
begin
  AMax := Round(AHSV.V * 255);
  AMin := Round(AMax * (1 - AHSV.S));
  ASector := Trunc(AHSV.H / 60) mod 6;
  AFrac := AHSV.H / 60 - ASector;
  AMid_1 := Round(AMax * (1 - AFrac * AHSV.S));
  AMid_2 := Round(AMax * (1 - (1 - AFrac) * AHSV.S));
  case ASector of

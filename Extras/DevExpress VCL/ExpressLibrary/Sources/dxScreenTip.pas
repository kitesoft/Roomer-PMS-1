...
{$ENDIF}
  ...
  dxCore, cxClasses, cxGraphics, cxControls, cxLookAndFeels, cxLookAndFeelPainters, dxCustomHint;
...
type
  ...
  TdxScreenTipRepository = class(TcxCustomComponent)
  private
  ...
  public
    constructor Create(APainter: TdxScreenTipPainter;
  ...
  public
    constructor Create(ABand: TdxCustomScreenTipBand; const AHintText, AShortCut: string);
    procedure Calculate(ACanvas: TCanvas; APainter: TdxScreenTipPainter; ATop, AWidth: Integer);
    function GetHeight(ACanvas: TCanvas; const AWidth: Integer;
      APainter: TdxScreenTipPainter): Integer;
  ...
  public
    constructor Create(AScreenTip: TdxScreenTip; APainter: TdxScreenTipPainter;
  ...
  protected
    ...
    procedure DestroyHelper; override;
  public
    constructor Create(AScreenTip: TdxScreenTip; APainter: TcxCustomLookAndFeelPainter;
      const AHintText: string; AShortCut: string = '');
  end;
  ...
  protected
    procedure DoShowHint(var AHintStr: string; var ACanShow: Boolean;
      var AHintInfo: THintInfo); override;
    procedure GetScreenTipInfo(AControl: TControl;
...
uses
  StrUtils, Math, Menus, ActnList, cxGeometry, dxDrawRichTextUtils;
...
procedure TdxScreenTipCollection.SetItemName(AItem: TcxComponentCollectionItem);
begin
...
procedure TdxCustomScreenTipBand.SetGlyph(Value: TBitmap);
begin
  FGlyph.Assign(Value);
  Changed;
end;
...
begin
  ...
  begin
    if not Description.Glyph.Empty then
      Result := cxGetValueCurrentDPI(318)
    else
      Result := cxGetValueCurrentDPI(210);
  end;
end;
...
begin
  FHelper := TdxScreenTipLikeHintViewInfoHelper.Create(FPainter, FHint,
    FShortCut, FCursorPos);
end;
...
procedure TdxScreenTipBandViewInfo.Calculate(ACanvas: TCanvas;
  APainter: TdxScreenTipPainter; ATop, AWidth: Integer);
var
  AHeight: Integer;
begin
...
function TdxScreenTipBandViewInfo.GetHeight(ACanvas: TCanvas;
  const AWidth: Integer; APainter: TdxScreenTipPainter): Integer;
  ...
  begin
    ...
    begin
      ...
      if (Band.Glyph.Width <= AAvailWidth)  then
      begin
        FGlyphSize.cy := Band.Glyph.Height;
        FGlyphSize.cx := Band.Glyph.Width;
      end
      else
      begin
        FGlyphSize.cy := Round(AAvailWidth * Band.Glyph.Height / Band.Glyph.Width);
        FGlyphSize.cx := AAvailWidth;
      end;
    end
    else
...
begin
  if not Band.GlyphFixedWidth then
    Result := Band.Glyph.Width
  else
    if Band.BandType = stbDescription then
      Result := cxGetValueCurrentDPI(108) - GetHorzIndent
    else
...
begin
  if Band.BandType = stbDescription then
    Result := cxGetValueCurrentDPI(14)
  else
    Result := cxGetValueCurrentDPI(6);
end;
...
begin
  ...
  if HasGlyph then
    cxDrawImage(ACanvas.Handle, GlyphBounds, GlyphBounds, Band.Glyph, nil, -1, idmNormal);
...
begin
  ...
  begin
    GetTextHelper(Band.BandType).Init(ACanvas, AText);
...
begin
  Result := DT_LEFT or DT_NOPREFIX or IfThen(AIsCalculate, DT_CALCRECT);
  if Band.BandType = stbDescription then
...
function TdxScreenTipViewInfoHelper.CreateBandViewInfo(
  ABand: TdxCustomScreenTipBand): TdxScreenTipBandViewInfo;
begin
  Result := TdxScreenTipBandViewInfo.Create(ABand, FHintText, FShortCut);
end;
...
begin
  ...
  begin
    Inc(ARect.Right);
    Inc(ARect.Bottom);
    OffsetRect(ARect, -FBorderWidth, -FBorderWidth);
  end;
...
procedure TdxScreenTipWindow.ShowHint(X, Y: Integer; ACaption, AHint: string;
  AMaxWidth: Integer = 0);
var
  ARect: TRect;
begin
...
function TdxScreenTipWindow.CalcHintRect(MaxWidth: Integer; const AHint: string;
  AData: Pointer): TRect;

  procedure GetScreenTipInfo(AHintStyle: TcxCustomHintStyle;
    out AScreenTip: TdxScreenTip; out AShortCut: string);
  var
    AControl: Tcontrol;
  begin
  ...
  begin
    ...
    begin
      if (AScreenTip <> nil) and not AScreenTip.UseHintAsHeader and (AScreenTip.Header.Text <> '') then
        ...
        AShortCut := '';
      Result := TdxScreenTipLikeHintViewInfo.Create(GetPainter, AHintText, AShortCut, cxNullPoint);
    end;
  end;
...
function TdxScreenTipStyle.GetScreenTipBounds(AScreenTip: TdxScreenTip;
  const AHint: string = ''): TRect;
var
...
procedure TdxScreenTipPainter.DrawBackground(ACanvas: TcxCanvas;
  const ARect: TRect);
begin
  FPainter.ScreenTipDrawBackground(ACanvas, ARect);
end;

procedure TdxScreenTipPainter.DrawFooterLine(ACanvas: TcxCanvas;
  const ARect: TRect);
begin
...
constructor TdxScreenTipLikeHintViewInfoHelper.Create(APainter: TdxScreenTipPainter;
  const AHint, AShortCut: string; const ACursorPos: TPoint);
begin

...
type
  TdxHookType = (htKeyboard, htMouse, htWndProc, htGetMessage, htCBT);
  TdxHookProc = procedure (ACode: Integer; wParam: WPARAM; lParam: LPARAM; var AHookResult: LRESULT);
...
type
  ...
  private
    ...
    function ProcessHookProcs(ACode: Integer; wParam: WPARAM; lParam: LPARAM): LRESULT;
  protected
...
var
  dxHookController: TdxHookController;
...
function TdxSystemHook.ProcessHookProcs(ACode: Integer; wParam: WPARAM; lParam: LPARAM): LRESULT;
var
...
begin
  ...
  for I := FHooks.Count - 1 downto 0 do
    try
      TdxHookProc(FHooks.List[I])(ACode, wParam, lParam, AHookResult);
    except
      Application.HandleException(Application);
    end;
  Result := CallNextHookEx(FHandle, ACode, wParam, lParam);
...
initialization
  dxHookController := TdxHookController.Create

finalization
  FreeAndNil(dxHookController);

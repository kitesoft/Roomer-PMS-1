...
begin
  ...
  Inc(TdxNativeInt(Result), SizeOf(Word));
end;
...
begin
  ...
  begin
    ...
    Result := Result  + ASize + (Ceil(FUnionCellsCount / xlsMaxMergedCellsInBlock) * 6);
  end;
end;
...
begin
  ...
  with AStyle do
  begin
    PWord(@XF^[8])^ := (Byte(AlignText) + 1);
    if not SingleLine then
      PWord(@XF^[8])^ := PWord(@XF^[8])^ or $8;
...
begin
  ...
  if FUnionCellsCount > 0 then
  begin
    C := Min(FUnionCellsCount, xlsMaxMergedCellsInBlock);
    ASize := C shl 3 + 2;
    xlsWriteBuffer([XLS_MergeCells, ASize, C]);
    for I := 0 to FUnionCellsCount - 1 do
    begin
      if (I > 0) and (I mod xlsMaxMergedCellsInBlock = 0) then
      begin
        C := Min(FUnionCellsCount - I, xlsMaxMergedCellsInBlock);
        ASize := C shl 3 + 2;
        xlsWriteBuffer([XLS_MergeCells, ASize, C]);
      end;
      FStream.WriteBuffer(FUnionCells^[I], SizeOf(TcxMergeRect));
    end;
  end;
  xlsWriteBuf(cxXLS_EOF);
end;

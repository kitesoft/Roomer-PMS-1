...
interface
uses
  SysUtils, Math, Classes, Windows, Variants,
{$IFNDEF NONDB}
  FMTBcd, SqlTimSt,
{$ENDIF}
...
begin
  if AText <> '' then
    SetData(ACol, ARow, Length(AText) * SizeOf(WideChar), cxDataTypeWideString, AText[1])
  else
      ...
      begin
        ...
        begin
          ...
          begin
            if Cache[J, I].DataSize > 0 then
            begin
              SetLength(AWideStringValue, Cache[J, I].DataSize div SizeOf(WideChar));
...
begin
  ...
  begin
    ...
    begin
      if Cache[ACol, ARow].DataSize > 0 then
      begin
        SetLength(AWideStringValue, Cache[ACol, ARow].DataSize div SizeOf(WideChar));

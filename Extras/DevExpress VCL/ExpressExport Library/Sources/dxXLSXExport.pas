...
uses
  ...
  cxExportStrs, dxCustomTree, dxXMLDoc, dxGdiPlusClasses {$IFDEF DELPHI12}, AnsiStrings {$ENDIF};
...
type
  ...
  protected
    ...
    procedure InitializeItem(AItem: TdxPackedItem; const AName: AnsiString; AStream: TStream);
    procedure WriteItems; override;
  public
    procedure AddDirectory(const APath, ARelativeDir: AnsiString);
    procedure AddFile(const AFileName, ARelativeDir: AnsiString; AStream: TStream);
  ...
  protected
    procedure CheckEncodedText(var AText: TdxXMLString); override;
  end;
...
begin
  Result := {$IFDEF DELPHI12}AnsiStrings.{$ENDIF}StringReplace(AString, AOldPattern, ANewPattern, AFlags);
end;
...
var
  ...
  AName: string;
begin
  AName := StringReplace(dxAnsiStringToString(Name), '/', PathDelim, [rfReplaceAll]);
  S := APath;
  if (Length(S) > 0) and (S[Length(S)] <> '\') then
    S := S + '\';
  if IsDirectory then
    ForceDirectories(S + AName)
  else
  begin
    ForceDirectories(S + ExtractFilePath(AName));
    if Size = 0  then Exit;  
    with TFileStream.Create(S + AName, fmCreate) do
    try
...
procedure TdxPackedStreamWriter.AddDirectory(const APath, ARelativeDir: AnsiString);
begin
  InitializeItem(Add, APath, nil);
end;

procedure TdxPackedStreamWriter.AddFile(
  const AFileName, ARelativeDir: AnsiString; AStream: TStream);
begin
  InitializeItem(Add, AFileName, AStream);
end;

procedure TdxPackedStreamWriter.AddImage(
  const APath: AnsiString; AImage: TdxPNGImage; AIndex: Integer);
var
...
procedure TdxPackedStreamWriter.InitializeItem(
  AItem: TdxPackedItem; const AName: AnsiString; AStream: TStream);
begin
  AItem.FName := dxAnsiStringReplace(AName, PathDelim, '/', [rfReplaceAll]);
  if AStream <> nil then
  begin
...
procedure TdxXLSXInternalXMLNode.CheckEncodedText(var AText: TdxXMLString);
begin
  if Length(AText) > MAXSHORT then
    AText := Copy(AText, 1, MAXSHORT);
  inherited CheckEncodedText(AText);
end;
...
begin
  Inc(FCount);
  AnsiName := dxWideStringToAnsiString(AName, CP_UTF8);
  if Length(AnsiName) > 31 then
    SetLength(AnsiName, 31);
  ANode := AddNode(FSheets, sSheet, [sRID, sSheetID], ['rId' +
    dxIntToStr(FCount - 1), dxIntToStr(FCount)]);
...
begin
  ...
  ANode.AddChild('t').Text := dxWideStringToAnsiString(AValue, CP_UTF8);
end;

procedure TdxXLSXSharedStrings.SaveToStream(AStream: TStream);
begin
//  Document.SaveToXmlStrings;

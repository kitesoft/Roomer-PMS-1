...
uses
  Windows, Classes, SysUtils, Math, Graphics, dxCore, dxGdiPlusClasses, cxGraphics, 
  cxGeometry, Variants, {$IFNDEF NONDB} FMTBcd, SqlTimSt, {$ENDIF} cxExportStrs;
...
function GraphicToPNGImage(const AClientRect, ADrawRect: TRect;
  AGraphic: TGraphic): TdxPNGImage;
...
const
  ...
  cxCheckForDuplicateImages: Boolean = False;
...
begin
  ...
  try
    ...
    for I := 0 to Length(ABits) - 1 do
      ABits[I].rgbReserved := Transparency[PtInRect(ADrawRect, Point(I mod B.Width, I div B.Width))];
    Result.LoadFromBits(B.Width, B.Height, ABits, True);
  finally
    B.Free;
  end;
end;
...
function FindGraphic(const AGraphic: TGraphic; const AMemStream: TMemoryStream;
  var AGraphicText: AnsiString): Boolean;

  function IsEqualGraphic(ACandidate: TGraphic; const AData: AnsiString): Boolean;
  var
    ASize, ALen: Integer;
  begin
    Move(AData[1], ASize, SizeOf(ASize));
    ALen := Length(AData) - ASize - SizeOf(ASize);
    Result := AMemStream.Size = ALen;
    if Result then
      Result := CompareMem(@AData[1 + ASize + SizeOf(ASize)], AMemStream.Memory, ALen);
  end;
...
begin
  Result := False;
  for I := 0 to GraphicCache.Count - 1 do
  begin
    AGraphicText := dxStringToAnsiString(GraphicCache[I]);
    Result := IsEqualGraphic(TGraphic(GraphicCache.Objects[I]), AGraphicText);
...
begin
  ...
  try
    ...
    if not cxCheckForDuplicateImages or (AMemStream.Size >= MaxWord) or
      not FindGraphic(ASource, AMemStream, AGraphicText) then
    begin
      L := Length(AName);
      ...
      if cxCheckForDuplicateImages and (AMemStream.Size < MaxWord) then
        GraphicCache.AddObject(dxAnsiStringToString(AGraphicText), ASource);
    end;
  finally
    AMemStream.Free;
  end;
...
begin
  ...
  begin
    ...
    FHandle := TdxNativeInt(CreateFileW(PWideChar(AName), GENERIC_READ or GENERIC_WRITE,
...
initialization
  StylesCache := nil;
  GraphicCache := TStringList.Create;

finalization
  FreeAndNil(StylesCache);
  FreeAndNil(GraphicCache);

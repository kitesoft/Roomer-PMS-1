...
type                                 
  ...
  TdxWizardControlButtonClickEvent = procedure (Sender: TObject;
    AKind: TdxWizardControlButtonKind; var AHandled: Boolean) of object;
  ...
  TdxWizardControlCustomPersistent = class(TPersistent)
  private
  ...
  public
    ...
    destructor Destroy; override;
    procedure Draw(ACanvas: TcxCanvas; const ARect: TRect);
  ...
  TdxWizardControlHeaderAssignedValue = (wchvBackgroundImage, wchvDescriptionFont,
    wchvDescriptionVisibility, wchvDescriptionOffset, wchvGlyph, wchvGlyphVisibility, wchvTitleFont);
  ...
  published
    ...
    property Glyph: TdxSmartImage read FGlyph write SetGlyph stored IsGlyphStored;
  ...
  private
    ...
    procedure WriteDescrption(Writer: TWriter);
  ...
  protected
    ...
    property Visible: Boolean read FVisible write SetVisible default True;
  public
    constructor Create(AButtons: TdxWizardControlButtons); virtual;
    destructor Destroy; override;

    property Buttons: TdxWizardControlButtons read FButtons;
  published
  ...
  { TdxWizardControlBackButtonViewInfo }

  TdxWizardControlBackButtonViewInfo = class(TdxWizardControlButtonViewInfo)
  protected
    function IsEnabled: Boolean; override;
  end;

  { TdxWizardControlNextButtonViewInfo }

  TdxWizardControlNextButtonViewInfo = class(TdxWizardControlButtonViewInfo)
  protected
    function IsEnabled: Boolean; override;
  end;
  ...
  protected
    ...
    function GetIndentBetweenElemenets: Integer; virtual; abstract;
  ...
  public
    ...
    property GlyphRect: TRect read FGlyphRect;
    property GlyphSize: TSize read GetGlyphSize;
    property IndentBetweenElemenets: Integer read GetIndentBetweenElemenets;
  ...
  private
    function GetWatermakImage: TdxWizardControlImage;
    function GetWatermakVisibility: TdxWizardControlElementVisibility;
  protected
  ...
  public
    ...
    property WatermakImage: TdxWizardControlImage read GetWatermakImage;
    property WatermakVisibility: TdxWizardControlElementVisibility read GetWatermakVisibility;
  end;
  ...
  protected
    ...
    function GetButtonKind(AButton: TdxWizardControlCustomButton; out AKind: TdxWizardControlButtonKind): Boolean;
    procedure ProcessAccel(ACell: TdxWizardControlCellViewInfo); virtual;
    procedure ProcessButtonClick(AButton: TdxWizardControlCustomButton); virtual;
  ...
  { TdxWizardControlDesignSelector }

  TdxWizardControlDesignSelector = class(TcxDesignSelector)
  private
  ...
  private
    ...
    FDesignSelector: TdxWizardControlDesignSelector;
  ...
  {$ENDIF}
    ...
    // DesignSelector
    procedure InvalidateDesignSelector;
    procedure UpdateDesignSelectorBounds;

    // Events
    function DoButtonClick(AKind: TdxWizardControlButtonKind): Boolean; virtual;
    ...
    property DesignSelector: TdxWizardControlDesignSelector read FDesignSelector;
...
uses
  Types, ShellAPI, dxWizardControlViewStyleWizard97, dxWizardControlViewStyleAero;
...
procedure TdxWizardControlCustomPersistent.DoAssign(Source: TPersistent);
begin
  // do nothing
end;
...
begin
  ...
  FHeaderTitleFont.Color := clDefault;
  FHeaderDescriptionFont := TFont.Create;
  FHeaderDescriptionFont.Color := clDefault;
end;
...
begin
  LookAndFeelPainter.DrawButton(ACanvas, ABounds, '', AState);
end;
...
begin
  Result := LookAndFeelPainter.ButtonFocusRect(ACanvas, ABounds);
end;
...
begin
  if AButtonState = cxbsPressed then
    Result := Point(LookAndFeelPainter.ButtonTextShift, LookAndFeelPainter.ButtonTextShift)
  else
    Result := cxNullPoint;
end;
...
begin
  Result := LookAndFeelPainter.LabelLineHeight;
end;
...
function TdxWizardControlCellViewInfo.IsEnabled: Boolean;
begin
  Result := True;
end;
...
begin
  FBounds := WizardControl.ClientRect;
...
begin
  Result := Max(cxGetValueCurrentDPI(dxWizardControlMinHeight),
...
begin
  Result := Max(cxGetValueCurrentDPI(dxWizardControlMinWidth),
...
begin
  ...
  R.Right := GlyphRect.Left - IndentBetweenElemenets;
  FTitleRect := R;
  ...
  begin
    ...
    FDescriptionRect := R;
    FDescriptionRect.Top := TitleRect.Bottom + cxTextSpace;
...
begin
  ...
  if BackgroundImageIsAvailable then
    BackgroundImage.Draw(ACanvas, Bounds);
end;
...
begin
  ...
  try
    ...
    begin
      ...
      ACanvas.DrawTexT(Header.Title, TitleRect, taLeftJustify, vaCenter, False, True);
    end;
    ...
    begin
      ...
      ACanvas.DrawTexT(Header.Description, DescriptionRect, taLeftJustify, vaCenter, True, True);
    end;
  finally
...
begin
  ...
  begin
    ...
    if DescriptionIsVisible then
      Inc(Result, 2 * cxTextHeight(DescriptionFont) + cxTextSpace);
    if GlyphIsVisible then
...
begin
  ...
  Result := cxGetValueCurrentDPI(Result);
end;
...
begin
  if Glyph <> nil then
    Result := cxGetValueCurrentDPI(Glyph.Size)
  else
    Result := cxNullSize;
end;
...
begin
  ...
  FNextButtonViewInfo := AddButtonCell(Buttons.Next, TdxWizardControlNextButtonViewInfo);
...
begin
  ...
  case Button.GlyphAlignment of
    blGlyphLeft:
      FCaptionRect.Left := GlyphRect.Right + cxGetValueCurrentDPI(cxTextOffset);
    blGlyphRight:
      FCaptionRect.Right := GlyphRect.Left - cxGetValueCurrentDPI(cxTextOffset);
    blGlyphTop:
      FCaptionRect.Top := GlyphRect.Bottom + cxGetValueCurrentDPI(cxTextOffset);
    blGlyphBottom:
      FCaptionRect.Bottom := GlyphRect.Top - cxGetValueCurrentDPI(cxTextOffset);
  end;
end;
...
begin
  ...
  begin
    if not Button.Glyph.Empty then
      ...
      AGlyphBitmap := nil;
    try
      cxDrawImage(ACanvas.Handle, cxRectOffset(GlyphRect, AOffset),
        cxRectOffset(GlyphRect, AOffset), AGlyphBitmap, WizardControl.Buttons.Images,
        Button.ImageIndex, EnabledImageDrawModeMap[State = cxbsDisabled]);
    finally
      AGlyphBitmap.Free;
    end;
  end;
  ...
  try
    ...
    begin
      cxDrawText(ACanvas, Button.Caption, cxRectOffset(CaptionRect, AOffset),
  ...
  begin
    ...
    begin
      ...
      if (not Button.Glyph.Empty) or IsImageAvailable then
        Result := Result + GlyphSize.cy + cxGetValueCurrentDPI(cxTextOffset);
    end;
...
begin
  ...
  if AActualButtonHeight <= 0 then
    AActualButtonHeight := AutoheightValue;
  Result := cxGetValueCurrentDPI(AActualButtonHeight);
end;
...
begin
  ...
  if AActualButtonWidth <= 0 then
    AActualButtonWidth := dxWizardControlDefaultButtonWidth;
  Result := cxGetValueCurrentDPI(AActualButtonWidth);
end;
...
begin
  ...
  if CanFocus then
  begin
    Result := (ACharCode = VK_RETURN) and IsDefault;
    if Result then
...
function TdxWizardControlButtonViewInfo.IsEnabled: Boolean;
begin
...
begin
  if (not Button.Glyph.Empty) or IsImageAvailable then
    Result := dxGetImageSize(Button.Glyph, WizardControl.Buttons.Images, Button.ImageIndex)
  else
    Result := cxNullSize;
...
function TdxWizardControlBackButtonViewInfo.IsEnabled: Boolean;
begin
  Result := inherited IsEnabled and WizardControl.CanGoToPrevPage;
end;

{ TdxWizardControlNextButtonViewInfo }

function TdxWizardControlNextButtonViewInfo.IsEnabled: Boolean;
begin
  Result := inherited IsEnabled and WizardControl.CanGoToNextPage;
end;
...
begin
  ...
  try
    ...
    ACanvas.DrawTexT(InfoPanel.Caption, Bounds, cxAlignVCenter or cxShowEndEllipsis,
      InfoPanel.Enabled or Painter.IsSkinUsed);
  finally
    ACanvas.RestoreState;
  end;
end;
...
begin
  Result := cxTextHeight(Font) + cxGetValueCurrentDPI(cxTextOffset * 2);
end;

function TdxWizardControlInfoPanelViewInfo.MeasureWidth: Integer;
begin
  Result := cxTextWidth(Font, InfoPanel.Caption) + cxGetValueCurrentDPI(cxTextOffset * 2);
end;
...
constructor TdxWizardControlImage.Create;
begin
  inherited Create;
...
begin
  FImage.Draw(ACanvas.Handle, ARect);
end;
...
function TdxWizardControlImage.GetImage: TdxSmartImage;
begin
...
procedure TdxWizardControlImage.SetImage(AValue: TdxSmartImage);
begin
...
constructor TdxWizardControlHeader.Create;
begin
  inherited Create;
  FBackgroundImage := TdxWizardControlImage.Create;
...
procedure TdxWizardControlHeader.SetGlyph(AValue: TdxSmartImage);
begin
...
constructor TdxWizardControlTitle.Create;
begin
  inherited Create;
  FGlyph := TdxSmartImage.Create;
...
procedure TdxWizardControlTitle.SetGlyph(AValue: TdxSmartImage);
begin
  FGlyph.Assign(AValue);
end;
...
begin
  if IsImageAvailable then
    WatermakImage.Draw(ACanvas, Bounds);
end;

function TdxWizardControlWatermarkViewInfo.MeasureHeight: Integer;
begin
  if IsImageAvailable then
    Result := cxGetValueCurrentDPI(WatermakImage.Height)
  else
    Result := 0;
end;
...
begin
  if IsImageAvailable then
    Result := cxGetValueCurrentDPI(WatermakImage.Width)
  else
    Result := 0;
end;
...
begin
  Result := IsImageVisible and not WatermakImage.Empty;
end;

function TdxWizardControlWatermarkViewInfo.GetWatermakImage: TdxWizardControlImage;
begin
...
function TdxWizardControlWatermarkViewInfo.GetWatermakVisibility: TdxWizardControlElementVisibility;
begin
...
constructor TdxWizardControlPageHeader.Create;
begin
  inherited Create;
...
begin
  ...
  Filer.DefineProperty('Description', ReadDescription, WriteDescrption, IsDescriptionStored and (Description = ''));
...
procedure TdxWizardControlPageHeader.WriteDescrption(Writer: TWriter);
begin
...
constructor TdxWizardControlCustomButton.Create(AButtons: TdxWizardControlButtons);
begin
  inherited Create;
  FEnabled := True;
  FVisible := True;
...
procedure TdxWizardControlCustomButton.SetGlyph(AValue: TdxSmartImage);
begin
  FGlyph.Assign(AValue);
end;
...
procedure TdxWizardControlCustomButton.SetImageIndex(Value: TcxImageIndex);
begin
...
constructor TdxWizardControlButtons.Create;
begin
  inherited Create;
  ...
  FCommon := TdxWizardControlOptionsButtons.Create;
...
procedure TdxWizardControlButtons.SetImages(Value: TCustomImageList);
begin
  cxSetImageList(Value, FImages, FImageChangeLink, FreeNotificator);
end;
...
constructor TdxWizardControlOptionsViewStyleAero.Create;
begin
  inherited Create;
  FTitle := TdxWizardControlTitle.Create;
...
constructor TdxWizardControlWatermark.Create;
begin
  inherited Create;
  FBackgroundImage := TdxWizardControlImage.Create;
  FBackgroundImage.OnChange := BackgroundImageChanged;
...
constructor TdxWizardControlInfoPanel.Create;
begin
  inherited Create;
  FFont := TFont.Create;
  FFont.Color := clDefault;
...
function TdxWizardControlController.GetButtonKind(AButton: TdxWizardControlCustomButton;
  out AKind: TdxWizardControlButtonKind): Boolean;
begin
  Result := True;
  if AButton = WizardControl.Buttons.Back then
    AKind := wcbkBack
  else

  if AButton = WizardControl.Buttons.Cancel then
    AKind := wcbkCancel
  else

  if AButton = WizardControl.Buttons.Next then
    AKind := wcbkNext
  else

  if AButton = WizardControl.Buttons.Help then
    AKind := wcbkHelp
  else

  if AButton = WizardControl.Buttons.Finish then
    AKind := wcbkFinish
  else
    Result := False;
end;
...
begin
  case AKey of
    VK_RETURN:
      if [ssCtrl, ssAlt] * AShiftState = [] then
        ProcessClick(FocusedCell);
...
procedure TdxWizardControlController.ProcessButtonClick(AButton: TdxWizardControlCustomButton);
var
  AButtonKind: TdxWizardControlButtonKind;
begin
  if GetButtonKind(AButton, AButtonKind) and AButton.Enabled then
  begin
    if not WizardControl.DoButtonClick(AButtonKind) then
      case AButtonKind of
        wcbkBack:
...
begin
  ...
  begin
    ...
    begin
      case AKey of
        VK_LEFT:
          ProcessButtonClick(WizardControl.Buttons.Back);
        VK_RIGHT:
          ProcessButtonClick(WizardControl.Buttons.Next);
      end;
    end;
end;
...
begin
  if ACell is TdxWizardControlButtonViewInfo then
    ProcessButtonClick(TdxWizardControlButtonViewInfo(ACell).Button)
  else
...
begin
  ...
  begin
    if AInfoPanel.Hyperlink <> '' then
      ShellExecute(WizardControl.Handle, 'open', PChar(AInfoPanel.Hyperlink), '', '', SW_SHOW);
  end;
end;
...
procedure TdxWizardControlDesignSelector.MouseDown(
...
function TdxWizardControlDesignSelector.GetDesignHitTest(X, Y: Integer; Shift: TShiftState): Boolean;
begin
  Result := True;
end;

function TdxWizardControlDesignSelector.GetPage: TdxWizardControlCustomPage;
begin
  Result := (Owner as TdxCustomWizardControl).ActivePage;
end;

function TdxWizardControlDesignSelector.GetSelected: Boolean;
var
  APage: TdxWizardControlCustomPage;
begin
  APage := Page;
  Result := (APage <> nil) and (APage.FDesignHelper <> nil) and
    APage.FDesignHelper.IsObjectSelected(APage, APage);
end;
  ...
  begin
    ...
    try
      ...
      try
        ...
        SendMessage(WizardControl.Handle, WM_SETREDRAW, 0, 0);
        APage.Visible := True;
        SendMessage(WizardControl.Handle, WM_SETREDRAW, 1, 0);
      finally
        APageBitmap.Free;
      end;
    finally
...
begin
  ...
  FButtons := TdxWizardControlButtons.Create;
  FButtons.OnChange := ChangeHandler;
  FHeader := TdxWizardControlHeader.Create;
  FHeader.OnChange := ChangeHandler;
  FInfoPanel := TdxWizardControlInfoPanel.Create;
  FInfoPanel.OnChange := ChangeHandler;
  FOptionsAnimate := TdxWizardControlOptionsAnimate.Create;
  FOptionsAnimate.OnChange := ChangeHandler;
  FOptionsViewStyleAero := TdxWizardControlOptionsViewStyleAero.Create;
  FOptionsViewStyleAero.OnChange := ChangeHandler;
  FWatermark := TdxWizardControlWatermark.Create;
  FWatermark.OnChange := ChangeHandler;
  FAnimationController := TdxWizardControlAnimationController.Create(Self);

  if IsDesigning then
  begin
    FDesignSelector := TdxWizardControlDesignSelector.Create(Self);
    FDesignSelector.Parent := Self;
  end;
...
procedure TdxCustomWizardControl.ChangeScale(M, D: Integer);
begin
  inherited ChangeScale(M, D);
...
function TdxCustomWizardControl.DoButtonClick(AKind: TdxWizardControlButtonKind): Boolean;
begin
  Result := False;
  if Assigned(OnButtonClick) then
    OnButtonClick(Self, AKind, Result);
end;
...
procedure TdxCustomWizardControl.InvalidateDesignSelector;
begin
...
procedure TdxCustomWizardControl.UpdateDesignSelectorBounds;
var
  ARect : TRect;
begin
  if DesignSelector <> nil then
  begin
    if ActivePage <> nil then
    begin
      ARect := cxRectInflate(ViewInfo.PageArea,
        -cxDesignSelectorIndentFromBorder,
        -cxDesignSelectorIndentFromBorder);
      ARect.Left := ARect.Right - cxDesignSelectorSize;
      ARect.Top := ARect.Bottom - cxDesignSelectorSize;
    end
    else
      ARect := cxNullRect;

    DesignSelector.BoundsRect := ARect;
...
begin
  ...
  begin
    if not ((AComponent is TdxWizardControlCustomPage) or (AComponent is TdxWizardControlDesignSelector)) then
...
begin
  ...
  FPageVisible := True;
  ControlStyle := ControlStyle + [csAcceptsControls];
  FHeader := TdxWizardControlPageHeader.Create;
  FHeader.OnChange := ChangeHandler;
  FOptionsSize := TdxWizardControlOptionsSize.Create;
  FOptionsSize.OnChange := ChangeHandler;
  FWatermark := TdxWizardControlPageWatermark.Create;
  FWatermark.OnChange := ChangeHandler;
...
begin
  if IsObjectSelected then
    Activate;
  if Active then
    WizardControl.InvalidateDesignSelector;
end;
...
procedure TdxWizardControlCustomPage.WMWindowPosChanged(var Message: TWMWindowPosChanged);
begin
  inherited;
  if Message.WindowPos^.flags and SWP_NOZORDER = 0 then
  begin
    if WizardControl <> nil then
      WizardControl.UpdateDesignSelectorBounds;
  end;
end;

...
uses
  ...
  dxSkinsCore, cxGraphics, cxGeometry, dxScreenTip, dxCore;
...
type
  ...
  private
    FBarElementCacheList: TdxSkinElementCacheList;
  ...
  protected
    ...
    property BarElementCacheList: TdxSkinElementCacheList read FBarElementCacheList;
  ...
  public
    ...
    procedure DropDownGalleryDrawItem(DC: HDC; const R: TRect; AState: Integer); override;
    procedure DropDownGalleryDrawScrollBarBackground(DC: HDC; const R: TRect); override;
    ...
    function DropDownGalleryGetPartColor(APart: Integer; AState: Integer = 0): TColor; override;
  end;

implementation

uses
  Math, dxOffice11, dxBarSkinConsts, dxSkinsStrs, dxFading;
...
begin
  ...
  FBarElementCacheList := TdxSkinElementCacheList.Create
end;

destructor TdxBarSkinPainter.Destroy;
begin
  FreeAndNil(FBarElementCacheList);
  inherited Destroy;
end;
...
begin
  Result := IsBarElementSkinned(ABarItemControl.Parent) and
    (GetSkinElementTextColorHot(GetTextColorElement(ABarItemControl.Parent)) <> nil);
end;
...
begin
  ...
  cxPaintCanvas.BeginPaint(DC);
  try
    cxPaintCanvas.SetClipRegion(TcxRegion.Create(ABarControlAccess.ClientBounds), roSubtract);
    InternalDrawDockedBarBackground(ABarControl, cxPaintCanvas, R, False);
  finally
    cxPaintCanvas.EndPaint;
  end;
end;
...
begin
  ...
  if Assigned(AElement) and AElement.IsAlphaUsed then
    TdxBarControlAccess(ABarControl).FillBackground(DC, R, 0, clNone, True);
  DrawSkinElement(AElement, DC, R, 0, GetSkinElementState);
end;
...
begin
  ...
  try
    ...
    DrawSkinElement(StatusBar, cxPaintCanvas.Handle, AWholeR);
  finally
    cxPaintCanvas.EndPaint;
  end;  
end;
...
begin
  ...
  begin
    ...
    try
      ...
      OffsetRect(AWholeR, -(ASourceR.Left - ADestR.Left), -(ASourceR.Top - ADestR.Top));
      InternalDrawDockedBarBackground(ABarControl, cxPaintCanvas, AWholeR, not cxRectIsEqual(ASourceR, ADestR));
    finally
      cxPaintCanvas.EndPaint;
    end;
  end;
...
begin
  ...
  begin
    ...
    try
      cxPaintCanvas.SetClipRegion(TcxRegion.Create(ADestR), roIntersect);
      DrawSkinElement(FloatingBar, cxPaintCanvas.Handle, ABarControl.ClientRect);
    finally
      cxPaintCanvas.EndPaint;
    end;
  end;
...
procedure TdxBarSkinPainter.DropDownGalleryDrawItem(DC: HDC;
...
begin
  ...
  if ASkinElement = nil then
    inherited DropDownGalleryDrawItem(DC, R, AState)
  else
  begin
...
procedure TdxBarSkinPainter.DropDownGalleryDrawScrollBarBackground(DC: HDC; const R: TRect);
var
  ASkinInfo: TdxSkinLookAndFeelPainterInfo;
  ASkinElement: TdxSkinElement;
  R1: TRect;
begin
  ...
  if ASkinElement = nil then
    inherited DropDownGalleryDrawScrollBarBackground(DC, R)
  else
  begin
    R1 := R;
    Dec(R1.Left, Max(ASkinElement.Size.cx, ASkinElement.Image.Margins.Left));
    ASkinElement.Draw(DC, R1);
  end;
end;
...
function TdxBarSkinPainter.DropDownGalleryGetPartColor(APart, AState: Integer): TColor;
var
  ASkinInfo: TdxSkinLookAndFeelPainterInfo;
begin
  if GetSkinPainterData(ASkinInfo) then
    ...
    Result := inherited DropDownGalleryGetPartColor(APart, AState);
end;
...
begin
  ...
  try
    ...
    DrawSkinElement(PopupMenu, DC, cxRectInflate(R,
      TdxBarSubMenuControlAccess(ABarSubMenuControl).GetClientOffset));
  finally
...
begin
  ...
  FSkinPainter.DrawEditorButtonGlyph(ADrawParams.Canvas, ARect, cxbkEllipsisBtn, GetState);
end;
...
begin
  ...
  FSkinPainter.DrawEditorButtonGlyph(ADrawParams.Canvas, ARect, cxbkComboBtn, GetState);
end;
...
begin
  ...
  try
    ...
    FSkinPainter.DrawButton(cxPaintCanvas, R, ACaption, State[APressed]);
  finally
    cxPaintCanvas.EndPaint;
  end;
end;
...
begin
  FSkinPainter.DrawEditorButton(ADrawParams.Canvas, ARect,
    ButtonKind[AButtonIndex = secButtonUp], GetSpinEditButtonState(ADrawParams, AButtonIndex));
  FSkinPainter.DrawEditorButtonGlyph(ADrawParams.Canvas, ARect,
    ButtonKind[AButtonIndex = secButtonUp], GetSpinEditButtonState(ADrawParams, AButtonIndex));
end;
...
begin
  ...
  begin
    DrawBackground(BarStaticControl, Canvas.Handle, ARect, 0, False);
    DrawSkinElement(ItemSeparator, Canvas.Handle, ARect);
    ACaptionRect := ARect;
...
begin
  ...
  DrawSkinElement(AElement, DC, R, 0, StatesMap[AState <> msNone]);
end;
...
begin
  cxPaintCanvas.BeginPaint(DC);
  try
    FSkinPainter.DrawSizeGrip(cxPaintCanvas, R, clNone);
  finally
    cxPaintCanvas.EndPaint;
  end;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      Result := DrawSkinElement(AElement, cxPaintCanvas.Handle, ARect, AImageIndex, AState);
    end;
  finally
    cxPaintCanvas.EndPaint;
  end;
...
begin
  ...
  begin
    ...
    if (Result <> nil) and ((Result.Value = clDefault) or (Result.Value = clNone)) then
      Result := nil;
  end
  else
    Result := nil;
...
begin
  if ABarControl = nil then
    ...
    Result := GetBarElement(ABarControl,
      TCustomdxBarControlAccess(ABarControl).IsRealVertical);
end;
...
var
  ABarControlAccess: TCustomdxBarControlAccess;
begin
  ABarControlAccess := TCustomdxBarControlAccess(ABarControl);
  if ABarControlAccess.IsMainMenu and ABarControl.IsRealVertical then
    Result := Bar[False]
  else
...
begin
  ...
  begin
    ...
    if cxRectWidth(ARect) < (AElement.Image.Margins.Left + AElement.Image.Margins.Right) then
      R.Left := ARect.Left + cxRectWidth(ARect) - AElement.Image.Margins.Margin.Left - AElement.Image.Margins.Right;
    ACanvas.SaveClipRegion;
    try
      ACanvas.SetClipRegion(TcxRegion.Create(ARect), roIntersect);
...
procedure TdxBarSkinPainter.InternalDrawDockedBarBackground(
  ...
  begin
    ...
    if AElement <> nil then
      FBarElementCacheList.DrawElement(ACanvas.Handle, AElement, R);
  end;
  ...
  begin
    ...
    DrawSkinElementParentBackground(ACanvas.Handle, R, ABarControl, AElement);
    DrawSkinElement(AElement, ACanvas.Handle, R)
  end;
  ...
  begin
    ...
    try
      DrawBarElement(ABarControlAccess, cxPaintCanvas,
        GetBarElementRect(ABarControlAccess, ABitmapRect));
      if not AClientArea then
      begin
        if ABarControlAccess.MarkExists then
          DrawMarkElementNCPart(ABarControlAccess, cxPaintCanvas, AWholeR);
        if ABarControlAccess.CanMoving then
          DrawBarDragElement(ABarControlAccess, cxPaintCanvas,
...
begin
  ...
  cxBitBlt(ACanvas.Handle,
    ABarControlAccess.BackgroundTempBitmap.Canvas.Handle,

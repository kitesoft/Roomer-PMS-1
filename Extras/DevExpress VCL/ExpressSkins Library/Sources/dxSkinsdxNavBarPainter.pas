...
begin
  ...
  if AElement = nil then
    ...
    Result := AElement.ContentOffset.Top + AElement.ContentOffset.Bottom;
end;
...
begin
  ...
  if AElement = nil then
    ...
    Result := GetSkinElementOffsets(AElement);
end;
...
begin
  ...
  if not TdxCustomNavBarAccess(NavBar).IsInternal then
    Result.Left := Result.Left + GetSkinElementOffsets(GetSkinHelper.NavPaneGroupClient).Left;
end;
...
procedure TdxNavBarSkinNavPanePainter.DoDrawSizeGrip(ACanvas: TcxCanvas;
  const ARect: TRect);
begin
  if not DrawSkinElement(FSkinBasedPainterHelper.NavPaneSizeGrip, ACanvas.Canvas, ARect) then
    inherited;
end;
...
begin
  if GetHelper.NavBarGroupButtonCaption <> nil then
    Result := cxMarginsHeight(GetHelper.NavBarGroupButtonCaption.ContentOffset.Rect)
  else
    Result := inherited GetGroupCaptionHeightAddon;
end;

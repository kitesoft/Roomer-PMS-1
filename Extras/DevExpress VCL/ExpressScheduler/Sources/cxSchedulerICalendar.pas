...
type
  ...
  protected
    ...
    class function ToString(AProperty: TcxSchedulerICalendarCustomProperty;
      AValue: Variant; out ATranslateValue: string): Boolean;{$IFDEF DELPHI14} reintroduce;{$ENDIF} virtual;
  end;
  ...
  protected
    ...
    function ToString: string;{$IFDEF DELPHI14} reintroduce;{$ENDIF} virtual;
  ...
  protected
    ...
    class function ToString(AProperty: TcxSchedulerICalendarRecurrenceCustomProperty; out AValue: string): Boolean;{$IFDEF DELPHI14} reintroduce;{$ENDIF} virtual;
  end;
  ...
  protected
    ...
    function GetStandatdDate: TDateTime;
...
const
  ...
  cxSchedulerICalendarPropertyParametrFormat = ';%s=%s';
  cxSchedulerICalendarPropertyValueParametrFormat = '%s=%s';
  ...
  cxSchedulerICalendarParamertDelimiter = ';';
...
begin
  ...
  AStrings := StrSplit(cxSchedulerICalendarParamertDelimiter, AParameters);
  try
...
begin
  ...
  for I := 0 to CustomParameterCount - 1 do
    if CustomParameters[I].ConvertPropertyToParameterValue(Self, AValue) then
      AParametrs := AParametrs + Format(cxSchedulerICalendarPropertyParametrFormat,
  ...
  for I := 0 to CustomParameterCount - 1 do
    if CustomParameters[I].ToString(Self, AValue, ATranslateValue) then
      Break;
  Result := Format(cxSchedulerICalendarPropertyFormat, [TokenName + AParametrs, ATranslateValue]);
end;
...
begin
  S := AParameterValue;
  ALen := Length(cxSchedulerICalendarQuote);
  Delete(S, 1, ALen);
  Delete(S, Length(S) - ALen + 1, ALen);
  ADateTimeProperty := TcxSchedulerICalendarDateTimeCustomProperty(AProperty);
...
begin
  ...
  if Length(ATranslateValue) > 8 then
  begin
  ...
  begin
    TcxSchedulerICalendarDateTimeCustomProperty(AProperty).UseDateFormat := True;
    AHour := 0;
    AMinute := 0;
    ASecond := 0;
  end;
...
function TcxSchedulerICalendarTimeZoneObject.GetStandatdDate: TDateTime;
begin
...
begin
  Result := GetStandatdDate = GetDaylightDate;
end;
...
var
  ...
  ADate: Variant;
begin
  ADateProperty := TcxSchedulerICalendarDateTimeCustomProperty.Create(AProperty.ICalendar, nil);
  try
  ...
  for I := 0 to ValueParameterCount - 1 do
    ...
    begin
      if Length(VarToStr(Result)) > 0 then
        Result := Result + cxSchedulerICalendarParamertDelimiter;
      Result := Result + Format(cxSchedulerICalendarPropertyValueParametrFormat,
...
var
  ...
  I: Integer;
begin
  AStrinngs := StrSplit(cxSchedulerICalendarParamertDelimiter, VarToStr(AValue));
  try
    for I := 0 to AStrinngs.Count - 1 do
      SetValueParameter(AStrinngs[I]);
  finally
    AStrinngs.Free;
  end;
end;
...
begin
  ...
  try
    ...
    begin
      S := AStrings[I];
      if (Length(S) > 0) and (Ord(S[1]) = VK_TAB) then
      begin
        Delete(S, 1, 1);

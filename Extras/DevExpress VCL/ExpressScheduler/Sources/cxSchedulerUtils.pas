...
uses
  ...
  dxOffice11, cxDateUtils, cxSchedulerStrs, dxSkinsCore;
...
type
  ...
  public
    ...
    class procedure DrawShadow(ACanvas: TcxCanvas; const ABounds, AVisibleBounds: TRect); virtual;
...
class function TcxSchedulerDateTimeHelper.AddDSTInfo(const ATimeZone: Integer;
  const AYear: Integer; const AStandardDate, ADaylightDate: TDateTime): Integer;
begin
  Result := AddDSTInfo(TimeZoneInformations[CheckTimeIndex(ATimeZone)],
...
begin
  ...
  if (ATimeZone >= 0) and (ATimeZone < Length(TimeZoneInformations)) then
    AInfo := @TimeZoneInformations[ATimeZone]
  else
...
begin
  ...
  begin
    ...
    for I := AFirstEntry to ALastEntry do
      if ARegistry.ReadBinaryData(IntToStr(I), ADSTInfo, SizeOf(ADSTInfo)) = SizeOf(ADSTInfo) then 
...
class procedure TcxSchedulerPainterHelper.DrawShadow(
  ACanvas: TcxCanvas; const ABounds, AVisibleBounds: TRect);
var
  R: TRect;
begin
  R := ABounds;
  Inc(R.Right, ShadowSize - 1);
  Inc(R.Bottom, ShadowSize - 1);
  ACanvas.SaveClipRegion;
  try
    ACanvas.IntersectClipRect(AVisibleBounds);
    Shadow.Draw(ACanvas.Handle, R);
  finally
...
begin
  ...
  Shadow := TdxSkinImage.Create(nil);

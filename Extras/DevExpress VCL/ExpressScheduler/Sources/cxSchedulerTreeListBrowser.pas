...
{$ENDIF}
  ...
  Contnrs, SysUtils, Types, cxInplaceContainer, dxCore, cxControls, cxGraphics,
  ...
  cxScrollBar, cxSchedulerTimeGridView, cxTL, cxTLData, cxCustomData, cxVariants, dxMessages;
...
type
  ...
  TcxSchedulerTreeListBrowserColumnStartDataProvider = class(TcxSchedulerTreeListBrowserColumnDataProvider)
  protected
    function GetCaption: string; override;
    function GetPropertiesClass: TcxCustomEditPropertiesClass; override;
    function GetValue(const AEvent: TcxSchedulerEvent): Variant; override;
  ...
  TcxSchedulerTreeListBrowserColumnFinishDataProvider = class(TcxSchedulerTreeListBrowserColumnStartDataProvider)
  protected
    function GetCaption: string; override;
    function GetValue(const AEvent: TcxSchedulerEvent): Variant; override;
    procedure SetValue(const AEvent: TcxSchedulerEvent; const AValue: Variant); override;
...
function TcxSchedulerTreeListBrowserColumnStartDataProvider.GetPropertiesClass: TcxCustomEditPropertiesClass; 
begin
  Result := TcxDateEditProperties;
end;

procedure TcxSchedulerTreeListBrowserColumnStartDataProvider.InitializeProperties;
begin
...
begin
  ...
  if ADone then
  begin
    AViewInfo.Painter.DrawIndicatorItem(ACanvas, AViewInfo.BoundsRect, AViewInfo.Kind,
      AViewInfo.ViewParams.Color, TcxTreeListIndicatorAccess(AViewInfo).DrawBackgroundHandler);
...
begin
  ...
  begin
    if HandleAllocated then
      PostMessage(Handle, WM_GROUPUNGROUP, wParam(KEY = VK_RIGHT), 0);
    Key := 0;
  end;
  inherited;

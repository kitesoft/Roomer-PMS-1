...
type
 ...
 { TcxSchedulerBackgroundSlotCellViewInfo }
  ...
  public
    ...
    procedure DrawShadow(ACanvas: TcxCanvas; const ABounds, AVisibleBounds: TRect); virtual;
    function DrawShadowFirst: Boolean; virtual;
    function GetEventSelectionExtends: TRect; virtual;
...
uses
  cxDrawTextUtils, cxLibraryConsts, cxVariants, cxSchedulerDateNavigator, dxCore, cxFormats;
...
begin
  ...
  begin
    AItem := Items[I];
    if AItem.FVisible and not cxRectIsEmpty(AItem.Bounds) and ACanvas.RectVisible(AItem.Bounds) then
    begin
...
begin
  if Transparent then
    ...
    Canvas.Rectangle(ARect, FViewParams, ABorders, ABorderColor, ALineWidth);
end;
...
begin
  ...
  begin
    ...
    DrawText(Rect(TextR.Left, TextR.Top, R.Left - cxTextOffset, TextR.Bottom),
      Copy(AText, 1, APos - 1), cxRightTopAlign, LargeFont, TextColor);
    R1 := Rect(R.Left, TextR.Top, TextR.Right, TextR.Top + LH - 3);
    DrawText(R1, Copy(AText, APos + 1, J - APos), cxAlignCenter, Font, TextColor);
...
function TcxSchedulerEventCellViewInfo.CalculateClipRect(const ABounds, AVisibleRect: TRect): TRect;
  ...
  function GetBoundsExtends: TRect;
  begin
    if Selected then
    begin
      Result := ExternalPainter.GetEventSelectionExtends;
      if ViewData.ShowTimeLine then
      begin
        Result.Bottom := Max(Result.Bottom, cxTimeLineWidth);
        Result.Top := Max(Result.Top, cxTimeLineWidth);
      end;
    end
    else
      Result := cxNullRect;
...
begin
  Result := inherited CalculateClipRect(cxRectInflate(ABounds, GetBoundsExtends), AVisibleRect);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FRotateBitmap.Rotate(raPlus90, True);
      Painter.DrawSchedulerGroupSeparator(FRotateBitmap.cxCanvas,
        FRotateBitmap.ClientRect, Color, DrawRotateBackground);
      FRotateBitmap.Rotate(raPlus90, True);
    end;
    Canvas.Draw(Bounds.Left, Bounds.Top, FRotateBitmap);
  end
  else
    Painter.DrawSchedulerGroupSeparator(Canvas, Bounds, Color, DrawBackground);
end;
...
begin
  if ATransparent then
    ...
    ACanvas.Rectangle(ARect, AViewParams, ABorders,  clBtnShadow, 1);
end;

procedure TcxSchedulerExternalPainter.DrawShadow(ACanvas: TcxCanvas;
  const ABounds, AVisibleBounds: TRect);
begin
  PainterHelper.DrawShadow(ACanvas, ABounds, AVisibleBounds);
end;
...
begin
  ...
  with ViewInfo do
  begin
    Result := GetFontHeight(StylesAdapter.GetEventParams(nil)) + 3 * cxTextOffset;
    ...
    begin
      ...
      Result := Max(AIconHeight + cxTextOffset + 3 * cxEventImagesOffset, Result);
    end
    else
...
begin
  ...
  begin
    ...
    Result := (AResource = nil) or not AResource.ReadOnly;
    if Result then
      Result := not AEvent.IsGroup or not AEvent.HasChildren;
  end;
end;

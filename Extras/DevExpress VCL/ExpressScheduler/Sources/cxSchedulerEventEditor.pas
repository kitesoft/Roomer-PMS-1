...
type
  ...
  TcxSchedulerEventEditorForm = class(TcxSchedulerCustomEventEditor)
    ...
    lbLocation: TcxLabel;
    lbResource: TcxLabel;
    lbShowTimeAs: TcxLabel;
    lbStartTime: TcxLabel;
    lbSubject: TcxLabel;
    meMessage: TcxMemo;
    pnlButtons: TPanel;
    pnlCaption: TPanel;
...
uses
  Variants, DateUtils, Math, cxClasses, cxSchedulerStrs, cxSchedulerUtils,
...
begin
  OnChanged(nil);
  deEnd.Date := deStart.Date + dxDateOf(FDuration) - Ord(cbAllDayEvent.Checked);
  if cbAllDayEvent.Checked and (FDuration < 1) then
    deEnd.Date := deEnd.Date + 1;
  FDuration := (dxDateOf(deEnd.Date) + dxTimeOf(teEnd.Time)) -

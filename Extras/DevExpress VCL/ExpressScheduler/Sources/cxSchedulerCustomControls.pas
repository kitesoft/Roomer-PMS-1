...
uses
  ...
  cxDateUtils, cxStorage, cxTextEdit, cxNavigator, cxVariants, dxUxTheme, dxTouch;
...
type
 ...
 { TcxSchedulerNavigatorButton }
  ...
  protected
    ...
    procedure DoSchedulerDragOver(const P: TPoint; AState: TDragState;
      var AAccept: Boolean);
    procedure DragOver(Source: TObject; X, Y: Integer; State: TDragState;
  ...
  protected
    ...
    procedure UpdateViewClonesResources;
  ...
  TcxSchedulerHintController = class(TcxIUnknownObject, IUnknown,
  ...
  public
    ...
    procedure Activate(const AHintRect: TRect;
      ...
      AAutoHide: Boolean = True); virtual;
...
function TcxSchedulerSubControl.GetScrollBar(
  AKind: TScrollBarKind): IcxControlScrollBar;
begin
...
begin
  ...
  begin
    ...
    ACanvas.SaveState;
    try
      APainter.DrawNavigatorButtonGlyph(ACanvas, AImages, AImageIndex,
...
begin
  ...
  begin
    ...
    if Scheduler.IsSizeGripVisible then
...
begin
  ...
  Scheduler.NotififySchedulerChanged;
  SaveState;
end;
...
procedure TcxCustomScheduler.PaintControl(
  AControl: TcxSchedulerSubControl; ASpecialPaint: Boolean = False);
begin
...
begin
  ...
  if AllDay and not AIsAllDaySelection then
    AEvent.Finish := Scheduler.SelFinish + 1
  else
  ...
  begin
    ...
    begin
      if AScrollBarKind = sbVertical then
        HintController.HintWindowAnchor := GetMouseCursorPos.Y
      else
        HintController.HintWindowAnchor := GetMouseCursorPos.X;
    end;
    R := HintController.CalcHintRect(cxscMaxHintWidth, AHintText, cxAlignRight);
    if AScrollBarKind = sbVertical then
    begin
      R := cxRectSetRight(R, ClientToScreen(VScrollBar.BoundsRect.TopLeft).X - 5);
      R := cxRectSetTop(R, HintController.HintWindowAnchor - ((R.Bottom - R.Top) div 2))
    end
    else
    begin
      R := cxRectSetBottom(R, ClientToScreen(HScrollBar.BoundsRect.TopLeft).Y - 5);
      R := cxRectSetLeft(R, HintController.HintWindowAnchor - ((R.Right - R.Left) div 2));
    end;
...
begin
  Result := Resources[AResourceIndex].Color;
    ...
    cxcsEvent:
      ...
      begin
        ...
        begin
          ...
          begin
            AParams.Color := Painter.DefaultSchedulerEventColor(AEvent.AllDayEvent or (AEvent.Duration >= 1));

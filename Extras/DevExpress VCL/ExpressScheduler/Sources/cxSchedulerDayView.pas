...
type
  ...
  private
    ...
    FAllDayScrollBar: TcxControlScrollBarHelper;
  ...
  protected
    ...
    property AllDayScrollBar: TcxControlScrollBarHelper read FAllDayScrollBar;
  ...
  protected
    ...
    function IsDrawShadowFirst: Boolean; virtual;
...
begin
  Event := AEvent;
  AFinish := Round(dxTimeOf(Event.Finish) / MinuteToTime);
...
begin
  TcxCustomSchedulerAccess(Scheduler).ScrollBarsManager.UnRegisterScrollBar(FAllDayScrollBar);
...
begin
  if cxRectPtIn(ViewInfo.FAllDayAreaBounds, APoint) then
    ...
    inherited GetGestureClient(APoint);
end;
...
begin
  FAllDayScrollBar := TcxControlScrollBarHelper.Create(Scheduler);
  FAllDayScrollBar.OnScroll := DoAllDayScroll;
  TcxCustomSchedulerAccess(Scheduler).ScrollBarsManager.RegisterScrollBar(FAllDayScrollBar);
end;
...
begin
  FIsDetailInfo := ViewData.AutoHeight or (not IsHeaderEvent and
    ((dxTimeOf(ContentFinish) <> 0) or (dxTimeOf(ContentStart) <> 0))) or
...
begin
  if ViewData.DrawShadows and not Hidden and not Selected and ShowTimeLine then
    ExternalPainter.DrawShadow(Canvas, Bounds, ViewData.VisibleRect);
end;

procedure TcxSchedulerDayViewEventCellViewInfo.DrawState;
begin
  Canvas.Rectangle(FTimeLineRect, ViewData.TimeLineParams, cxBordersAll,
...
begin
  ...
  if Selected then
  begin
    Dec(Result.Top, cxTimeLineWidth);
    Inc(Result.Bottom, cxTimeLineWidth);
  end
  else
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      Inc(ARect.Right);
      Inc(ARect.Top);
      ARect.Left := ARect.Right - GetScrollBarSize.cx;
      BoundsRect := ARect;
      Enabled := Max > Min;
      Calculate;
    end;
...
begin
  ACount := Min(FRowCount - 1,
    dxTimeOf(ATime) / MinuteToTime / TimeScale - FPrintStartRow);
  Result := Round(ACount);
  ...
  begin
    if HitAtTimeRuler then
      ...
      SetCloneEventsTimeDelta(Controller.StartDragHitTime,
        dxDateOf(Controller.StartDragHitTime) + Time, False)
    else
...
begin
  for Result := 0 to ViewInfo.ColCount - 1 do
    with ViewInfo do
    begin
      if (GetColumnDate(Result) >= dxDateOf(ADate)) and

...
uses
  Types, Windows, Classes, SysUtils, Graphics, DB, cxDB, cxDBData, cxSchedulerStorage,
...
type
  ...
  protected
    ...
    procedure RemoveDeletedEvents(AList: TList);
...
uses
  Variants, cxVariants, cxSchedulerStrs, cxSchedulerUtils;
...
begin
  ...
  if (DataSet = nil) or not DataSet.ControlsDisabled then
    UpdatingEvent := nil;
  if not SmartRefresh and (LockCount = 0) then
    FullRefresh;
end;
...
begin
  ...
  try
    ...
    AEvent.EndEditing;
    // todo: for autoincrement fields if key field was modified after post
    ...
    if not VarEquals(AEvent.ID, DataController.KeyField.Value) then
...
procedure TcxSchedulerDBStorage.RemoveDeletedEvents(AList: TList);
var
  I: Integer;
  AEvent: TcxSchedulerEvent; 
begin

...
type
  ...
  TcxSchedulerTimeGridScrollBar = class(TcxControlScrollBarHelper)
  private
    FResourceIndex: Integer;
  public
...
begin
  ...
  begin
    ...
    with AEvent do
    begin
      AllDay := (dxTimeOf(Start) = 0) and (dxTimeOf(Finish) = 0) and (Duration >= 1);
...
begin
  inherited SetBounds(ALeft + TreeBrowser.DisplayWidth, ATop,
    AWidth - TreeBrowser.DisplayWidth, AHeight);
  if TreeBrowser.Visible then
    TreeBrowser.SetBounds(ALeft, ATop, TreeBrowser.Width, AHeight);
end;
...
var
  ...
  AMilestoneWidth: Integer;

  procedure CalculateForward;
  var
    AActualLast,  AStartValue, AFinishValue: TDateTime;
  begin
    ALast := AFinish;
    AFirst := ALast;
    AActualLast := ALast;
    ...
    if not View.IsSnapEventsToTimeSlots and (EventFinish < AActualLast) then
    begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          AStart := AFirst;
        if not TimeBuilder.IsPeriodChanged(AFirst, ALast) or (EventStart < TimeBuilder.CalculateDateTimeDateTimeWithoutCheckWorkDays(AFirst, 1)) then
...
begin
  ...
  try
    ...
    for AResourceIndex := 0 to Max(ResourceCount - 1, 0) do
      ...
      begin
        TcxSchedulerControlEventAccess(Events.AbsoluteItems[AEventIndex]).VisibleIndex := -1;
...
begin
  ResourceViewShift[TcxSchedulerTimeGridScrollBar(Sender).ResourceIndex].FShift := -ScrollPos;
  View.Changed;
end;
...
begin
  ...
  begin
    ...
    if ResourceViewShiftList.Count > 0 then
      Dec(Result.Right, GetScrollBarSize.cx);
end;
...
begin
  ...
  if not View.IsSnapEventsToTimeSlots and not AViewData.IsMilestone then
...
begin
  ...
  CheckTime := (dxTimeOf(WorkStart) > 0) or
    (dxTimeOf(WorkFinish) < EncodeTime(23, 59, 59, 0)) and (MinorUnit = suHour);
  WorkFinish := RoundTime(WorkFinish);
  WorkStart := RoundTime(WorkStart);
  CheckMidnight := AIsWorkTimeOnly and CheckTime and
    (dxTimeOf(WorkFinish) - EncodeTime(23, 59, 59, 0) <= SecondToTime);
end;
...
begin
  inherited Create;
  FView := ATimeGridView;
  FScrollBar := TcxSchedulerTimeGridScrollBar.Create(ATimeGridView.Scheduler);
  FScrollBar.Kind := sbVertical;
  FScrollBar.FResourceIndex := AResourceIndex;
  FScrollBar.OnScroll := ATimeGridView.ViewInfo.DoResourceVerticalScroll;
  TcxCustomSchedulerAccess(ATimeGridView.Scheduler).ScrollBarsManager.RegisterScrollBar(FScrollBar);
end;
...
begin
  ...
  FScrollBar.SetScrollParams(0, FEventMaxLine, -Shift, FScrollBar.LargeChange, False);
  AHeight := ResourceBounds.Bottom - ResourceBounds.Top;
  FScrollBar.BoundsRect := Bounds(ResourceBounds.Right - GetScrollBarSize.cx,
    ResourceBounds.Top, GetScrollBarSize.cx, AHeight);
  FScrollBar.Calculate;
...
procedure TcxSchedulerTimeGridResourceScroll.DestroyScrollBar;
begin
  if FScrollBar <> nil then
  begin
    TcxCustomSchedulerAccess(FView.Scheduler).ScrollBarsManager.UnRegisterScrollBar(FScrollBar);
...
begin
  if ((FScrollBar = nil) or (Abs(AValue) <= FScrollBar.Max)) and (AValue <= 0) then
    FShift := AValue;
end;

...
uses
  ...
  Dialogs, DemoBasicMain, cxStyles, cxGraphics, cxEdit, cxScheduler,
...
{$ELSE}
  if AIndex = -1 then
    ...
    AListView.Selected := AListView.Items[AIndex];
{$ENDIF}
end;
...
begin
  if not AViewInfo.Selected and miUseColorScheme.Checked and (AViewInfo.Resource <> nil) then
    ACanvas.Brush.Color := GetMiddleRGB(GetResourceColor(AViewInfo.Resource), 0, 80);
end;
...
begin
  ...
  begin
    ...
    AParams.Color := GetMiddleRGB(GetResourceColor(AViewInfo.Resource), 0, 80);

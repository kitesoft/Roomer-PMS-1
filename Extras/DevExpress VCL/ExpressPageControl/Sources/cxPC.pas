...
uses
  ...
  Menus,
  dxCore, dxCoreClasses, cxClasses, cxControls, cxContainer, cxGraphics, cxLookAndFeels,
  cxLookAndFeelPainters, dxCustomHint;
...
type
  ...
  protected
    ...
    property Controller: TcxCustomTabControlController read GetController;
    property HotTrackNavigatorButton: TcxPCCustomHeaderButtonViewInfo read FHotTrackNavigatorButton
      write SetHotTrackNavigatorButton;
    property HotTrackTabButton: TcxPCCustomTabButtonViewInfo read FHotTrackTabButton
      write SetHotTrackTabButton;
    ...
    property Painter: TcxPCCustomPainter read GetPainter;
    property PressedNavigatorButton: TcxPCCustomHeaderButtonViewInfo read FPressedNavigatorButton
      write SetPressedNavigatorButton;
    property PressedTabButton: TcxPCCustomTabButtonViewInfo read FPressedTabButton
      write SetPressedTabButton;
  ...
  public
    ...
    function CanDrawParentBackground: Boolean;
    function GetHeaderButtonsDistance(AButtonInfo1,
      AButtonInfo2: TcxPCCustomHeaderButtonViewInfo): Integer;
    function GetLookAndFeel: TcxLookAndFeel;
  ...
  protected
    ...
    function GetButtonHeight: Integer; virtual; abstract;
    ...
    function GetButtonWidth(Button: TcxPCNavigatorButton): Integer; virtual; abstract;
  ...
  private
    ...
    procedure SetDragImage(const Value: TBitmap);
  ...
  protected
    ...
    function DoCanChange(NewTabIndex: Integer): Boolean; virtual;
    ...
    property DragImage: TBitmap read FDragImage write SetDragImage;
  ...
  TcxCustomTabControl = class(TcxControl, IcxTabControl, IcxControlComponentState,
    IcxMouseTrackingCaller, IcxMouseTrackingCaller2, IdxSkinSupport)
  private
    ...
    procedure PropertiesPaintDragImage(Sender: TObject; ABitmap: TBitmap; var ADone: Boolean);
  ...
  protected
    ...
    procedure PropertiesDrawTabExHandler(AControl: TcxCustomTabControlProperties;
      ATab: TcxTab; Font: TFont);
    procedure PropertiesDrawTabHandler(AControl: TcxCustomTabControlProperties;
      ATab: TcxTab; var DefaultDraw: Boolean);
    procedure PropertiesGetImageIndexHandler(Sender: TObject; TabIndex: Integer;
      var ImageIndex: Integer);
    procedure PropertiesGetTabHintHandler(Sender: TObject; ATabIndex: Integer;
  ...
  private
    ...
    procedure LockNotifyEvents(ALock: Boolean);
  protected
...
procedure TcxPCImageList.Draw(ACanvas: TcxCanvas; X, Y, AIndex: Integer;
  ...
  begin
    ABitmap.cxCanvas.CopyRect(cxRect(RotatedImageSize), ACanvas.Canvas,
      cxRectOffset(cxRect(RotatedImageSize), X, Y));
  end;
...
const
  ...
  begin
    Result:= TcxBitmap.CreateSize(cxRect(RotatedImageSize), pf32bit);
    Result.HandleType := bmDDB;
    if AIsNativePainting then
      Result.Transparent := True;
    DrawBitmapBackground(Result);
    Result.Rotate(ABackRotateAngle[ImageRotationAngle]);
...
begin
  ...
  begin
    ...
    if AButton.Visible and (AButton.Position in [pcbpTabs, pcbpTabsAndHeader]) and
      HasButton(AButton) then
...
begin
  ...
  FIsTextTooLong := TextSize(Self, Caption, Canvas.Font).cx > AContentWidth - AImageAreaWidth -
    AImageTextDistance - AButtonsAreaWidth;
end;
...
begin
  Result := IsAccel(Key, Caption) and ControlViewInfo.IsTabAccessible(VisibleIndex);
  if Result then
...
var
  ...
  begin
    FRowCount := 1;
    FTopOrLeftPartRowCount := IfThen(Properties.TabPosition in [tpTop, tpLeft], 1);
    CalculateLongitudinalTabPositions;
    if not IsTooSmallControlSize then
    begin
      SetTabRows;
    ...
    begin
      ...
      if IsTabButtonImagesAssigned then
        FTabButtonHeight := Max(FTabButtonHeight,
          Properties.TabButtonImages.Height +
          Painter.GetTabButtonGlyphOffset.Top +
          Painter.GetTabButtonGlyphOffset.Bottom);
    end;
  end;
...
begin
  Result := Painter.GetButtonsDistance(GetButtonTypeForDistance(AButtonInfo1),
    GetButtonTypeForDistance(AButtonInfo2));
end;
...
begin
  Result := ClientRect;
  case Properties.TabPosition of
    tpTop:
...
procedure TcxCustomTabControlViewInfo.CreateHeaderButtons;
  ...
  begin
    ...
    if IsHeaderButtonImagesAssigned then
      FHeaderButtonHeight := Max(FHeaderButtonHeight, Properties.HeaderButtonImages.Height +
        Painter.GetHeaderButtonGlyphOffset.Top + Painter.GetHeaderButtonGlyphOffset.Bottom);
  end;
...
procedure TcxPCCustomPainter.DrawHeaderButtonGlyph(ACanvas: TcxCanvas;
  AHeaderButtonInfo: TcxPCHeaderButtonViewInfo);

  function NeedDrawImage: Boolean;
  begin
    Result := IsImageAssigned(ViewInfo.Properties.HeaderButtonImages,
      AHeaderButtonInfo.ImageIndex);
  end;
...
begin
  ...
  begin
    AGlyphRect := cxRectCenter(AHeaderButtonInfo.Bounds,
      ViewInfo.Properties.CustomButtons.FHeaderImages.RotatedImageSize);
    HeaderButtonImagePainter.Draw(ACanvas, AGlyphRect.Left, AGlyphRect.Top,
      AHeaderButtonInfo.ImageIndex, clNone, IsNativePainting,
      (AHeaderButtonInfo.State <> nbsDisabled), False);
  end;
end;
...
procedure TcxPCCustomPainter.DrawTabButtons(ACanvas: TcxCanvas; TabVisibleIndex: Integer);
  ...
  procedure InternalDrawTabButton(ACanvas: TcxCanvas;
    const ARect: TRect; AButtonInfo: TcxPCCustomTabButtonViewInfo);
  var
    AGlyphRect: TRect;
  begin
    if AButtonInfo.GetHitTestIndex = pchtTabCloseButton then
      if UseLookAndFeelTabButton then
        GetLookAndFeelPainter.DrawSmallCloseButton(
          ACanvas, ARect, NavigatorBtnStateToLookAndFeelBtnState[AButtonInfo.State])
      else
        DoDrawTabCloseButton(ACanvas, ARect, AButtonInfo.State)
    else
    begin
      if UseLookAndFeelTabButton then
        GetLookAndFeelPainter.DrawSmallButton(ACanvas,
          cxRectInflate(ARect, GetButtonDrawOffsets),
          NavigatorBtnStateToLookAndFeelBtnState[AButtonInfo.State])
      else
        DoDrawTabButton(ACanvas, ARect, AButtonInfo.State);
      if NeedDrawImage(AButtonInfo) then
      begin
        AGlyphRect := cxRectCenter(ARect,
          dxGetImageSize(nil, ViewInfo.Properties.TabButtonImages, AButtonInfo.ImageIndex));
        TabButtonImagePainter.Draw(ACanvas, AGlyphRect.Left, AGlyphRect.Top,
          AButtonInfo.ImageIndex, clNone, IsNativePainting,
          (AButtonInfo.State <> nbsDisabled), False);
      end;
    end;
  end;
...
begin
  ImagePainter.Draw(ACanvas, ARect.Left, ARect.Top, AImageIndex,
...
begin
  ...
  begin
    Result := Result + TabViewInfo[ATabVisibleIndex].ButtonInfos[I].GetWidth;
  end;
  Result := Result + GetTabButtonDistance * (TabViewInfo[ATabVisibleIndex].ButtonsCount - 1) +
    GetCloseButtonOffset(ATabVisibleIndex).Left + GetCloseButtonOffset(ATabVisibleIndex).Right;
end;
...
procedure TcxPCCustomPainter.PaintHeadersArea(ACanvas: TcxCanvas);
  ...
  begin
    Result := (ViewInfo.TabPosition in [tpTop, tpBottom]) or
      not ViewInfo.IsTooSmallControlSize;
  end;
...
begin
  if ControlViewInfo.IsTabButtonImagesAssigned then
    Result := ControlViewInfo.Properties.TabButtonImages.Width +
      ControlViewInfo.Painter.GetTabButtonGlyphOffset.Left +
      ControlViewInfo.Painter.GetTabButtonGlyphOffset.Right
  else
...
begin
  ControlViewInfo.Properties.DoCloseButtonClick(FTabViewInfo.Index);
end;
...
begin
  if ControlViewInfo.IsHeaderButtonImagesAssigned then
    Result := ControlViewInfo.Properties.HeaderButtonImages.Width +
      ControlViewInfo.Painter.GetHeaderButtonGlyphOffset.Left +
      ControlViewInfo.Painter.GetHeaderButtonGlyphOffset.Right
  else
...
begin
  if (PageControl <> nil) and (PageControl.Properties.FLockCount > 0) then
...
function TcxPageControlProperties.DoCanChange(NewTabIndex: Integer): Boolean;
begin
...
procedure TcxPageControlProperties.AddDeferredEvent(AEvent: TNotifyEvent; ASender: TObject);
var
...
procedure TcxPageControlProperties.LockNotifyEvents(ALock: Boolean);
var
  I: Integer;
begin
  if ALock then
...
begin
  ...
  begin
    ...
    if not AMouseEvent or IsTabPressable then
      ATabViewInfo.Select(ssCtrl in AShift);
...
begin
  if (FMouseDownTabVisibleIndex = ATabVisibleIndex) and
    ViewInfo.IsTabAccessible(ATabVisibleIndex) then
  begin
...
begin
  Result := Properties.AllowTabDragDrop and HitTest.HitAtTab and not HitTest.HitAtTabButton;
  if Result then
...
begin
  ...
  FButtons := TcxPCButtons.Create(Self, TcxPCButton);
...
begin
  ...
  FCustomButtons := TcxPCCustomButtons.Create(Self);
end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        Self.AllowDisabledTabAccess := AllowDisabledTabAccess;
        Self.AllowTabDragDrop := AllowTabDragDrop;
        Self.CloseButtonMode := CloseButtonMode;
        Self.CloseTabWithMiddleClick := CloseTabWithMiddleClick;
...
function TcxCustomTabControlProperties.DoCanChange(NewTabIndex: Integer): Boolean;
begin
  Result := True;
...
begin
  ...
  try
    ...
    begin
      ...
      AImage.TransparentColorValue := ABitmap.TransparentColor;
      AImage.TransparentColor := True;
      cxBitBlt(AImage.Canvas.Handle, ABitmap.Canvas.Handle, AImage.ClientBounds, cxNullPoint, SRCCOPY);
    end;
  finally
    ABitmap.Free;
  end;
...
procedure TcxCustomTabControlProperties.SetDragImage(const Value: TBitmap);
begin
...
procedure TcxCustomTabControl.PropertiesPaintDragImage(Sender: TObject; ABitmap: TBitmap; var ADone: Boolean);
begin
  if Assigned(FOnPaintDragImage) then
...
begin
  ...
  FProperties.OnPaintDragImage := PropertiesPaintDragImage;
...
begin
  ...
  begin
    ...
    if NewPainterClass = nil then
      OutError('SetStyleByName', Format(cxGetResourceString(@scxPCStyleNameError),
        [StyleName]))
    else
      Style := NewPainterClass.GetStyleID;
  end;
...
begin
  if IsLoading or IsDestroying or ViewInfo.FUpdating then Exit;
  ViewInfo.Calculate;
  Realign;
  Invalidate;
end;

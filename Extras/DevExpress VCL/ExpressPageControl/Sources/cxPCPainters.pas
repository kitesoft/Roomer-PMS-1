...
{$ENDIF}
  ...
  cxPC, dxCore, cxLookAndFeelPainters, dxThemeManager;
...
type
  ...
  protected
    ...
    function GetButtonContentPosition(const ARect: TRect; AButton: TcxPCNavigatorButton;
      AState: TcxPCNavigatorButtonState): TPoint; virtual;
    ...
    procedure InternalDrawEdge(ACanvas: TcxCanvas; const ARect: TRect;
      ASunken: Boolean; AThinFrame: Boolean = False);
...
{    FMDC: HDC;
  ...
  protected
    ...
    function GetButtonContentPosition(const ARect: TRect; AButton: TcxPCNavigatorButton;
      AState: TcxPCNavigatorButtonState): TPoint; override;
    function GetButtonHeight: Integer; override;
    procedure GetButtonNativePartAndState(AState: TcxPCNavigatorButtonState;
...
//    procedure PrepareBackgroundBitmap; virtual;
  ...
  protected
    ...
    procedure InternalPaintTabsRowsDelimiter(ACanvas: TcxCanvas; var ARowRect: TRect;
  ...
  protected
    ...
    procedure DrawGradientBackground(ACanvas: TcxCanvas; ARect: TRect;
  ...
  protected
    ...
    function InternalGetTabClipRegion(ACanvas: TcxCanvas; ATabVisibleIndex: Integer;
...
uses
  Math, cxPCPaintersFactory, dxThemeConsts,
  dxUxTheme, dxOffice11, cxGeometry;
...
begin
  if GetHeaderButtonHeightCorrection <> 0 then
...
begin
  ...
  if (ATabViewInfo.GetRelativeTextRotationAngle = raMinus90) or
    IsNativePainting and (ATabViewInfo.GetRelativeTextRotationAngle = raPlus90) then
    ExchangeLongWords(Result.Left, Result.Right);
end;
...
const
  ATabNativePart: array[TcxTabPositionWithinRow] of Integer = (
  ...
  begin
    if ViewInfo.IsTabsContainer then
      ...
      if ALineIndexBoundsA[ATabViewInfo.VisibleRow].Left = ATabViewInfo.VisibleIndex then
        ...
        if (ALineIndexBoundsA[ATabViewInfo.VisibleRow].Right = ATabViewInfo.VisibleIndex) and
          (ViewInfo.RowCount > 1) then
...
var
  ...
  begin
    ...
    begin
      ...
      begin
        if MultiLine or (ViewInfo.NavigatorButtonCount = 0) then
        begin
...
begin
  with Result do
  begin
    ExtendedTabsRect := ViewInfo.ControlBounds;
...
begin
  if ViewInfo.IControl.IsDestroying then
    ...
    Result := ViewInfo.GetLookAndFeel.NativeStyle and
...
begin
  ...
  if ViewInfo.IsTabsContainer then
    ACanvas.FillRect(cxRectInflate(GetPageFrameRect, -GetFrameWidth, -GetFrameWidth), cxPCTabBodyColor);
end;
...
procedure TcxPCTabsPainter.PrepareTabBackground(ABitmap: TcxBitmap;
...
begin
  if IsNativePainting and CanDrawParentBackground and ViewInfo.ParentBackground then
...
begin
  if (TabPropertyChanged = tpcIsMainTab) and
     (not TabViewInfo[TabVisibleIndex].IsMainTab) then
    InvalidateTabExtendedTabsRect(TabVisibleIndex)
  else
    InvalidateTabRect(TabVisibleIndex);
end;

function TcxPCTabsPainter.GetButtonsDistance(
...
function TcxPCTabsPainter.GetNativeButtonsDistance(
  AButton1, AButton2: TcxPCNavigatorButton): Integer;
begin
  if IsWinVistaOrLater and IsOneOfButtons(AButton1, AButton2, nbGoDialog) and
    IsOneOfButtons(AButton1, AButton2, nbClose) then
    Result := 1
  else
    Result := inherited GetButtonsDistance(AButton1, AButton2);
end;
...
begin
  ...
  if (ATabViewInfo.PaintingPosition in [tpLeft, tpRight]) and
    (ViewInfo.GetTextRotationAngle <> raMinus90) then
...
begin
  ...
  ACanvas.FillRect(cxRectInflate(AFullRect, -ABorderWidth, -ABorderWidth),
    GetTabBodyColor(ATabVisibleIndex));
end;
...
begin
  if AButtonInfo.IsNavigatorButton then
    PaintButton(ACanvas, AButtonInfo.Bounds, AButtonInfo.State,
      TcxPCNavigatorButtonViewInfo(AButtonInfo).ButtonType)
  else
  begin
...
begin
  ...
  APainter.DrawButtonCross(ACanvas, ARect,
    APainter.ButtonSymbolColor(NavigatorBtnStateToLookAndFeelBtnState[AState]),
      NavigatorBtnStateToLookAndFeelBtnState[AState]);
end;
...
function TcxPCStandardPainter.GetCloseButtonSize: TSize;
begin
  if UseLookAndFeelTabButton then
    Result := GetLookAndFeelPainter.SmallCloseButtonSize
  else
    Result := cxSize(GetButtonWidth(nbClose), GetButtonHeight);
end;
...
begin
  Result := TextSize(TabViewInfo[ATabVisibleIndex], TabViewInfo[ATabVisibleIndex].Caption).cx;
end;
...
begin
  Result := IfThen(IsAssignedImages, GetTabImageAreaHeight + 6);
  Result := Max(Result, GetMaxTabCaptionHeight + 8);
end;

procedure TcxPCStandardPainter.InternalDrawEdge(ACanvas: TcxCanvas; const ARect: TRect;
  ASunken: Boolean; AThinFrame: Boolean = False);
const
...
begin
  ...
  begin
    ...
    ACanvas.DrawFocusRect(cxRectInflate(R, -1, -1));
  end;
end;
...
begin
  ...
  try
    ...
    DrawTabContentBackground(ABitmap.cxCanvas, ATabRect,
      GetTabBodyColor(ATabVisibleIndex), ATabVisibleIndex);
...
begin
  ...
  AContentLeftTopCorner := cxPointOffset(
    GetButtonContentPosition(ARect, AType, AState), ARect.TopLeft);
  DrawButtonContent(ACanvas, AType, AContentParameters, AContentLeftTopCorner);
end;

procedure TcxPCStandardPainter.PaintButtonBackground(ACanvas: TcxCanvas; const ARect: TRect;
  AState: TcxPCNavigatorButtonState);
begin
...
procedure TcxPCStandardPainter.PaintButtonFrame(ACanvas: TcxCanvas; var ARect: TRect;
  AButtonState: TcxPCNavigatorButtonState);
begin
  InternalDrawEdge(ACanvas, ARect, AButtonState = nbsPressed);
  InflateRect(ARect, -StandardPainterTabBorderWidth, -StandardPainterTabBorderWidth);
end;
...
var
  IsY: Boolean;
begin
  IsY := ViewInfo.TabPosition in [tpLeft, tpRight];
  ...
  begin
    with ALeftTopCorner do
      if AParameters.LiteStyle then
        if ViewInfo.TabPosition in [tpTop, tpBottom] then
        begin
          InternalPolyLine([Point(X + 1, Y), Point(X + CloseButtonCrossSize - 2, Y + CloseButtonCrossSize - 3)], ACanvas.Pen.Color, ACanvas);
          InternalPolyLine([Point(X, Y), Point(X + CloseButtonCrossSize - 3, Y + CloseButtonCrossSize - 3)], ACanvas.Pen.Color, ACanvas);

          InternalPolyLine([Point(X, Y + CloseButtonCrossSize - 3), Point(X + CloseButtonCrossSize - 3, Y)], ACanvas.Pen.Color, ACanvas);
          InternalPolyLine([Point(X + 1, Y + CloseButtonCrossSize - 3), Point(X + CloseButtonCrossSize - 2, Y)], ACanvas.Pen.Color, ACanvas);
        end
        else
        begin
          InternalPolyLine([Point(X, Y), Point(X + CloseButtonCrossSize - 3, Y + CloseButtonCrossSize - 3)], ACanvas.Pen.Color, ACanvas);
          InternalPolyLine([Point(X, Y + 1), Point(X + CloseButtonCrossSize - 3, Y + CloseButtonCrossSize - 2)], ACanvas.Pen.Color, ACanvas);

          InternalPolyLine([Point(X, Y + CloseButtonCrossSize - 3), Point(X + CloseButtonCrossSize - 3, Y)], ACanvas.Pen.Color, ACanvas);
          InternalPolyLine([Point(X, Y + CloseButtonCrossSize - 2), Point(X + CloseButtonCrossSize - 3, Y + 1)], ACanvas.Pen.Color, ACanvas);
        end
      else
      begin
        InternalPolyLine([Point(X + 1, Y), Point(X + CloseButtonCrossSize - 1, Y + CloseButtonCrossSize - 2)], ACanvas.Pen.Color, ACanvas);
        InternalPolyLine([Point(X, Y), Point(X + CloseButtonCrossSize - 1, Y + CloseButtonCrossSize - 1)], ACanvas.Pen.Color, ACanvas);
        InternalPolyLine([Point(X, Y + 1), Point(X + CloseButtonCrossSize - 2, Y + CloseButtonCrossSize - 1)], ACanvas.Pen.Color, ACanvas);

        InternalPolyLine([Point(X, Y + CloseButtonCrossSize - 2), Point(X + CloseButtonCrossSize - 2, Y)], ACanvas.Pen.Color, ACanvas);
        InternalPolyLine([Point(X, Y + CloseButtonCrossSize - 1), Point(X + CloseButtonCrossSize - 1, Y)], ACanvas.Pen.Color, ACanvas);
        InternalPolyLine([Point(X + 1, Y + CloseButtonCrossSize - 1), Point(X + CloseButtonCrossSize - 1, Y + 1)], ACanvas.Pen.Color, ACanvas);
      end;
  end;
...
var
  AIsY: Boolean;
begin
  AIsY := ViewInfo.TabPosition in [tpLeft, tpRight];
...
function TcxPCStandardPainter.GetButtonCenteredContentPosition(const ARect: TRect;
  AButton: TcxPCNavigatorButton; AState: TcxPCNavigatorButtonState): TPoint;
  ...
  begin
    ...
    begin
      ...
      if (AButton = nbGoDialog) or (AButton in [nbTopLeft, nbBottomRight]) and
        (ViewInfo.TabPosition in [tpLeft, tpRight]) then
          RotateSize(Result);
    end;
  end;
...
begin
  Result := ExtraFlatPainterTabContentWOffset[TabViewInfo[ATabVisibleIndex].IsMainTab];
end;
...
const
  ...
  begin
    ...
    begin
      ...
      NormalTabsRect := GetExtendedRect(ViewInfo.ControlBounds,
        ANormalTabsRectCorrection, ViewInfo.TabPosition);
...
procedure TcxPCExtraFlatPainter.PaintButtonFrame(ACanvas: TcxCanvas; var ARect: TRect;
  AButtonState: TcxPCNavigatorButtonState);
begin
  if AButtonState in [nbsHotTrack, nbsPressed] then
  begin
...
procedure TcxPCExtraFlatPainter.DrawTabBackground(ACanvas: TcxCanvas; ARect: TRect;
  ATabVisibleIndex: Integer);
begin
...
procedure TcxPCExtraFlatPainter.InternalPaintFrame(ACanvas: TcxCanvas;
...
procedure TcxPCExtraFlatPainter.InternalPaintMainTabFrame(ACanvas: TcxCanvas; var R: TRect;
...
procedure TcxPCExtraFlatPainter.InternalPaintTabsRowsDelimiter(ACanvas: TcxCanvas; var ARowRect: TRect;
  AColors: array of TColor);
var
  I: Integer;
begin
  for I := 0 to GetTabsRowsDelimiterWidth - 1 do
    case ViewInfo.TabPosition of
      tpTop:
        DrawBorder(ACanvas, ARowRect, [bBottom], [AColors[I]]);
      tpBottom:
        DrawBorder(ACanvas, ARowRect, [bTop], [AColors[I]]);
      tpLeft:
        DrawBorder(ACanvas, ARowRect, [bRight], [AColors[I]]);
      tpRight:
        DrawBorder(ACanvas, ARowRect, [bLeft], [AColors[I]]);
    end;
end;
...
procedure TcxPCExtraFlatPainter.PaintTabsRowsDelimiter(ACanvas: TcxCanvas; var ARowRect: TRect;
  ARowIndex: Integer);
begin
  if ViewInfo.TabPosition in [tpTop, tpLeft] then
...
function TcxPCUltraFlatPainter.GetButtonContentPosition(const ARect: TRect; 
  AButton: TcxPCNavigatorButton; AState: TcxPCNavigatorButtonState): TPoint;
begin
  Result := GetButtonCenteredContentPosition(ARect, AButton, AState);
end;
...
begin
  Result := GetExtendedRect(TabViewInfo[ViewInfo.FocusedTabVisibleIndex].FullRect,
    Rect(1, 1, 1, 0), ViewInfo.TabPosition);
end;

procedure TcxPCUltraFlatPainter.PaintButtonFrame(ACanvas: TcxCanvas; var ARect: TRect;
  AButtonState: TcxPCNavigatorButtonState);
...
procedure TcxPCUltraFlatPainter.PaintTabsRowsDelimiter(ACanvas: TcxCanvas; var ARowRect: TRect;
  ARowIndex: Integer);
begin
...
procedure TcxPCFlatPainter.PaintButtonFrame(ACanvas: TcxCanvas; var ARect: TRect;
  AButtonState: TcxPCNavigatorButtonState);
begin
  if AButtonState = nbsPressed then
...
begin
  if (ViewInfo.MainTabVisibleIndex <> -1) and
    (GetTabColor(ViewInfo.MainTabVisibleIndex) <> clDefault) then
      Result := GetTabColor(ViewInfo.MainTabVisibleIndex)
  else
...
begin
  Result := not (TabViewInfo[ATabVisibleIndex].IsMainTab or
...
procedure TcxPCOffice11Painter.PaintButtonBackground(ACanvas: TcxCanvas; const ARect: TRect;
  AState: TcxPCNavigatorButtonState);
var
  AColor1, AColor2: TColor;
  ATabsRowRect: TRect;
  ...
  case AState of
    ...
    nbsPressed, nbsHotTrack:
      begin
        ...
        FillGradientRect(ACanvas.Handle, ATabsRowRect, AColor1, AColor2,
          ViewInfo.TabPosition in [tpLeft, tpRight]);
      end;
  end;
  ACanvas.RestoreClipRegion;
...
procedure TcxPCOffice11Painter.PaintButtonFrame(ACanvas: TcxCanvas; var ARect: TRect;
  AButtonState: TcxPCNavigatorButtonState);
...
procedure TcxPCOffice11Painter.PaintTabsRowsDelimiter(ACanvas: TcxCanvas; var ARowRect: TRect;
  ARowIndex: Integer);
begin
...
begin
  if AButtonState = nbsHotTrack then
    Result := GetMiddleRGB(clBtnFace(*ViewInfo.Color*), clWhite, 35)
  else
    if AButtonState = nbsPressed then
      Result := GetMiddleRGB(clBtnFace(*ViewInfo.Color*), clBlack, 90)
    else
      Result := clBtnFace(*ViewInfo.Color*);
end;
...
begin
  if (AButtonState = nbsDisabled) or not ViewInfo.IsEnabled then
    ...
    if AButtonState = nbsPressed then
      Result := GetMiddleRGB(ViewInfo.Color, clWhite, 10)
    else
      Result := clBtnText;
end;
...
function TcxPCSlantedPainter.GetCloseButtonSize: TSize;
begin
 // Result := inherited GetCloseButtonSize;
  Result := cxSize(14, 15);
end;
...
procedure TcxPCSlantedPainter.PaintButtonFrame(ACanvas: TcxCanvas; var ARect: TRect;
  AButtonState: TcxPCNavigatorButtonState);
...
procedure TcxPCSlantedPainter.DrawTabContentBackground(ACanvas: TcxCanvas; const ABounds: TRect;
  ABackgroundColor: TColor; ATabVisibleIndex: Integer);
var
  R: TRect;
begin
  R := TabViewInfo[ATabVisibleIndex].FullRect;
  ...
  try
    ACanvas.SetClipRegion(TcxRegion.Create(ABounds), roIntersect);
    DrawBackground(ACanvas, R, ATabVisibleIndex,
      ViewInfo.TabPosition in [tpLeft, tpRight], False);
  finally
    ACanvas.RestoreClipRegion;
  end;
...
begin
  ...
  if (spLeft in ViewInfo.TabSlants.Positions) and (not ViewInfo.ActuallyRotate or (ViewInfo.TabSlants.Kind = skCutCorner)) then
  begin
    if ViewInfo.TabSlants.Kind = skSlant then
  ...
  if (spRight in ViewInfo.TabSlants.Positions) and (not ViewInfo.ActuallyRotate or (ViewInfo.TabSlants.Kind = skCutCorner)) then
  begin
    if ViewInfo.TabSlants.Kind = skSlant then
...
function TcxPCSlantedPainter.InternalGetTabClipRegion(ACanvas: TcxCanvas; ATabVisibleIndex: Integer;
  AForContent: Boolean): TcxRegion;
var
...
begin
  ...
  Result := DirectionalGetFigureRegion(ATabViewInfo.FullRect,
    APoints, ALinePositions, ATabViewInfo.PaintingPosition, AForContent);
end;

procedure TcxPCSlantedPainter.PaintTabFrame(ACanvas: TcxCanvas; ATabVisibleIndex: Integer;
  const R: TRect);
var
  ARHeight, ARWidth: Integer;
  ATabPaintingPosition: TcxTabPosition;
...
begin
  ...
  GetTabFramePolyline(ATabVisibleIndex, APoints, ALinePositions);
  DirectionalPolyline(ACanvas, R, APoints, ATabPaintingPosition, GetFrameColor);
end;
...
begin
  ...
  DirectionalPolyline(ACanvas, R, [Point(R.Left, R.Bottom), Point(R.Left, R.Bottom)],
    ATabViewInfo.PaintingPosition, GetMiddleRGB(GetFrameColor, AColor, 50));

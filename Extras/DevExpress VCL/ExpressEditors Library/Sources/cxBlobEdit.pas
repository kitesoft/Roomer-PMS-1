...
{$ENDIF}
  ...
  cxFilterControlUtils, dxUxTheme;
...
type
  ...
  public
    ...
    property MemoScrollBars: TcxScrollStyle
...
uses
  Variants, Math, cxDrawTextUtils, cxEditUtils, cxGeometry, cxVariants, dxCore, dxThemeConsts;
  ...
  if AFullText and (AProperties.BlobEditKind = bekMemo) and VarIsStr(Value) then
    ...
    begin
      ...
      if (AProperties.BlobPaintStyle = bpsText) and
        (AProperties.BlobEditKind = bekMemo) and dxVarIsBlob(Value) then
      begin
        Result := dxVariantToWideString(Value);
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        if Transparent then
          imgBlobImages.Draw(ACanvas.Canvas, IconRect.Left, IconRect.Top, ImageIndex, Enabled)
        else
          cxEditUtils.DrawGlyph(ACanvas, imgBlobImages, ImageIndex, IconRect, BackgroundColor, Enabled);
      finally
...
begin
  ...
  if not IsDestroying and HasPopupWindow and (FStorage is TcxPopupMemo) then
    TcxPopupMemo(FStorage).Style.Color := GetBackgroundColor;
end;
  ...
  case ActiveProperties.BlobEditKind of
    bekPict:
      begin
        ...
        with (FStorage as TcxPopupImage) do
        begin
          FBlobEdit := Self;
    ...
    bekMemo:
      begin
        ...
        with (FStorage as TcxPopupMemo) do
        begin
          FBlobEdit := Self;
          ...
          Style.LookAndFeel.MasterLookAndFeel := Self.PopupControlsLookAndFeel;
          Style.Font.Assign(Self.VisibleFont);
          Style.Color := Self.GetBackgroundColor;
        end;
        InitControls;
      end;
  end;
...
begin
  ...
  begin
    if FStorage is TcxPopupMemo then
      TcxPopupMemo(FStorage).Style.Font.Assign(VisibleFont);
    if FOkButton <> nil then
...
begin
  if (ActiveProperties.BlobEditKind = bekMemo) and (FStorage <> nil) then
    Result := (FStorage as TcxPopupMemo).Text
  else
...
begin
  if HasPopupWindow and (ActiveProperties.BlobEditKind in [bekMemo, bekPict]) then
    if ActiveProperties.BlobEditKind = bekMemo then
      ...
      Result := (FStorage as TcxPopupImage).EditValue
  else
    Result := EditValue;
end;
...
begin
  with ActiveProperties do
    ...
    begin
      with (FStorage as TcxPopupImage).Picture do
...
begin
  if (ActiveProperties.BlobEditKind = bekMemo) and (FStorage <> nil) then
    (FStorage as TcxPopupMemo).Text := ADisplayValue;
  inherited InternalValidateDisplayValue(ADisplayValue);
end;

procedure TcxCustomBlobEdit.DestroyPopupControls;
begin
  if not ((FStorage is TcxPopupImage) and TcxPopupImage(FStorage).IsDialogShowed) then
  begin
...
begin
  ...
  if not Result then
    Exit;
  if HasPopupWindow then
    (FStorage as TcxPopupMemo).EditingText := Value
  else
    EditValue := Value;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      imgBlobImages.AddMasked(ABitmap, clFuchsia);
    end;
  finally
    ABitmap.Free;
  end;

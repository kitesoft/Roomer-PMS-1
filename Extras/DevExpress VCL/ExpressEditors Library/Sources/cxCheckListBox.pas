...
uses
  ...
  Forms, Math, ImgList, dxCore, cxClasses, cxControls, cxContainer, cxGraphics, cxVariants,
...
type
  ...
  TcxCustomCheckListBox = class(TcxCustomEditContainer)
  private
...
uses
  RTLConsts, Consts, cxGeometry, cxEditPaintUtils, cxEditUtils, dxThemeManager;
...
begin
  ...
  begin
    ...
    ACheckBorderOffset := GetEditCheckBorderOffset(Style.LookAndFeel.Kind,
      FNativeStyle, IsGlyphAssigned(Glyph) and (GlyphCount > 0),
...
begin
  ...
  if VerifyImages(Container.Images) then
    Result := Max(Result, Container.Images.Height + Metrics.ImageFrameWidth * 2);
  dxAdjustToTouchableSize(Result);
end;
...
begin
  ...
  try
    DrawEditCheck(ABitmap.cxCanvas, ABitmap.ClientRect, AState, ACheckState, Glyph,
    ...
    cxBitBlt(Canvas.Handle, ABitmap.Canvas.Handle, R, cxNullPoint, SRCCOPY);
  finally
    ABitmap.Free;
  end;
end;
...
procedure TcxCustomCheckListBox.DrawItem(Control: TWinControl; Index: Integer;
  ...
  begin
    ...
    begin
      FInnerCheckListBox.Canvas.Font.Color := LookAndFeelPainter.DefaultSelectionTextColor;
      FInnerCheckListBox.Canvas.Brush.Color := LookAndFeelPainter.DefaultSelectionColor;
    end
    else
    begin
      FInnerCheckListBox.Canvas.Brush.Color := ViewInfo.BackgroundColor;
      FInnerCheckListBox.Canvas.Font.Color := VisibleFont.Color;
    end;
    if not (AIsItemEnabled or (odSelected in State)) then
      FInnerCheckListBox.Canvas.Font.Color := GetDisabledTextColor;
  end;
  ...
  procedure DrawCheckRegion(const ACheckRegion: TRect; AIsItemEnabled: Boolean);
    ...
    begin
      ...
      if ShowChecks and (ImageLayout = ilAfterChecks) then
        Inc(AGlyphOffset, FInnerCheckListBox.GetCheckAreaSize.cx);
  ...
  begin
    ...
    if IsImageAssigned(Images, AImageIndex) then
      cxDrawImage(FInnerCheckListBox.Canvas.Handle, GetGlyphRect, GetGlyphRect,
...
begin
  ...
  begin
    FInnerCheckListBox.Canvas.Font.Assign(VisibleFont);
    ACheckRegion := Rect;
    ATextRect := Rect;

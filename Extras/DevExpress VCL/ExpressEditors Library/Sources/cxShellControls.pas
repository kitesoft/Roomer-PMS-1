...
uses
  ...
  dxThemeManager, dxUxTheme, dxThemeConsts, cxEdit;
...
begin
  ...
  Result := TcxShellItemInfo(ItemProducer.Items[Item.Index]).CanRename and
...
begin
  ...
  begin
   ...
   // ListView_SetIconSpacing(Handle, LargeImages.Width + 30, LargeImages.Height + 44); // XP thumbnail
    CreateDropTarget;
...
begin
  if FSorting then
  begin
    if FItemProducer.SortColumn = Column.Index then
      FItemProducer.SortDescending := not FItemProducer.SortDescending;
    FItemProducer.SortColumn := Column.Index;
    SortColumnChanged;
  end;
...
procedure TcxCustomInnerShellListView.AdvancedDrawItem(Sender: TCustomListView; Item: TListItem;
  ...
  begin
    ...
    if (Item.Selected or (cdsHot in State)) and not IsWinXP and IsXPManifestEnabled and
      AreVisualStylesAvailable([totMenu]) then
    begin
  ...
  begin
    ...
    if IsWinVistaOrLater and not cxIsVCLThemesEnabled then
      cxDrawImage(ACanvas.Handle, ARect, AIconRect, nil, ItemProducer.FThumbnails,
        AShellItem.ThumbnailIndex, idmNormal)
    else
  ...
  procedure DrawItemText(ACanvas: TcxCanvas; ATextRect: TRect;
    AText: string);
  var
    AFlags: Integer;
  begin
...
begin
  ...
  begin
    AShellItem := TcxShellItemInfo(ItemProducer.Items[Item.Index]);
    ...
    try
      ...
      DrawItemBackground(ABitmap.cxCanvas, ABounds);

      if IsWinXP then
        DrawThumbnailFrame(ABitmap.cxCanvas, AIconRect);

      if AShellItem.HasThumbnail then
        DrawThumbnail(ABitmap.cxCanvas, AIconRect, AShellItem)
      else
        DrawIcon(ABitmap.cxCanvas, AIconRect, AShellItem);

      if not ADefaultDrawText and not (Item.Selected and IsEditing) then
        DrawItemText(ABitmap.cxCanvas, ATextRect, Item.Caption);

      cxBitBlt(Canvas.Handle, ABitmap.Canvas.Handle, ABounds,
        ABounds.TopLeft, SRCCOPY);
    finally
...
begin
  ...
  begin
    ...
    FLargeImages.Handle := SHGetFileInfo('C:\', 0, AFileInfo, SizeOf(AFileInfo),
...
procedure TcxCustomInnerShellListView.InitializeSorting;
begin
...
begin
  case FLargeIconSize of
    isDefault: Result := SHIL_LARGE;
    isExtraLarge: Result := SHIL_EXTRALARGE;
  else // isJumbo
...
begin
  ...
  begin
    ...
    try
      ...
      Result :=  TTreeNode(TcxShellItemInfo(AItemProducer.Items[AItemIndex]).Data);
      APIDL := GetNextItemID(APIDL);
    finally
      DisposePidl(APID);
    end;
...
function TcxShellTreeItemProducer.CheckUpdates: Boolean;
  ...
  begin
    ...
    begin
      AItem := TcxShellItemInfo(ANewItems.Extract(ANewItems.First));

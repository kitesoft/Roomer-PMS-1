...
uses
  ...
  ShellApi, ShlObj, SyncObjs, SysUtils, cxClasses;
...
{$IFNDEF DELPHI11}
  FSHCreateItemFromIDList: function(pidl: PItemIDList; const riid: TIID;
...
begin
  if Assigned(FSHCreateItemFromIDList) then
    Result := FSHCreateItemFromIDList(pidl, riid,  ppv)
  else
  begin
    Result := E_NOTIMPL;
    FSHCreateItemFromIDList := GetProcAddress(ShellLibrary, 'SHCreateItemFromIDList');
    if Assigned(FSHCreateItemFromIDList) then
      Result := FSHCreateItemFromIDList(pidl, riid, ppv);
  end;
end;
...
begin
  ...
  try
    if Failed(AIFolder.GetUIObjectOf(AWnd, AItemPIDLList.Count,
      APIDLs[0], IID_IContextMenu, nil, AContextMenu)) then
        Exit;
...
begin
  ...
  try
    try
      ...
      begin
        ATempCustomPath := PWideChar({$IFNDEF DELPHI14}WideString{$ENDIF}(CustomPath));
  ...
  if Source is TcxShellOptions then
    ...
    begin
      ...
      try
        ...
        Self.ShowNonFolders := ShowNonFolders;
        Self.ShowToolTip := ShowToolTip;
        Self.TrackShellChanges := TrackShellChanges;
      finally
        EndUpdate;
      end;
    end

...
uses
  ...
  cxGeometry, cxLookAndFeels, cxLookAndFeelPainters, cxScrollBar, cxExtEditConsts, cxHeader, cxEdit;
...
type
  ...
  TcxBaseInnerListView = class(TListView, IUnknown,
...
begin
  Result := inherited IsPanArea(APoint) and
   ((ViewStyle <> vsReport) or not PtInHeader);
end;

procedure TcxListViewContainer.LookAndFeelChanged(Sender: TcxLookAndFeel;
...
function TcxBaseInnerListView.DoMouseWheel(Shift: TShiftState;
...
procedure TcxBaseInnerListView.DragOver(Source: TObject; X, Y: Integer;
...
begin
  ...
  for I := 0 to Columns.Count do
    DrawHeaderSection(FHeaderHandle, I, Canvas, GetLookAndFeel, SmallImages);
end;
...
procedure TcxBaseInnerListView.LookAndFeelChanged(Sender: TcxLookAndFeel;
  AChangedValues: TcxLookAndFeelValues);
begin
  HeaderInvalidate;
end;
...
begin
  inherited;
  if UsecxScrollBars and not FContainer.ScrollBarsCalculating then
    FContainer.SetScrollBarsParameters;
end;

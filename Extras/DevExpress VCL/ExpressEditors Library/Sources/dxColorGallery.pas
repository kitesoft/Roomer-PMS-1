...
uses
  ...
  dxCore, cxVariants, cxClasses, cxControls, cxGraphics, dxGalleryControl,
...
uses
  Controls, dxOffice11, Variants, cxEditConsts, Math;
...
begin
  ...
  try
    ...
    ABitmap.cxCanvas.FrameRect(ABitmap.ClientRect,
      LookAndFeelPainter.GetGalleryItemImageFrameColor, 1, ABorders);
...
begin
  ...
  AItemSize := IfThen(ItemSize > 0, ItemSize, dxcExtendedSchemeItemSize);
...
begin
  ColumnCount := 5;
  AGroup := AddColorGroup('');
  AItemSize := IfThen(ItemSize > 0, ItemSize, dxcStandardSchemeItemSize);
...
//      ColorAccentMap1: TdxColorAccentArray = (caLight80, caLight80, caLight80, caLight80, caLight80);
...
begin
  ...
  AItemSize := IfThen(ItemSize > 0, ItemSize, dxcOfficeSchemeItemSize);

...
{$ENDIF}
  ...
  StdCtrls, SysUtils,
  dxCoreClasses, cxClasses, cxContainer, cxControls, cxFilter, cxGraphics,
...
type
  ...
  TcxNavigatorButtonViewInfo = class(TcxIUnknownObject, IcxHintableObject, IdxFadingObject)
  private
  ...
  protected
    ...
    procedure CalculateBounds(const AInfoPanelText: string;
      AInfoPanelWidth, AHeight, AButtonIndent: Integer);
    ...
    function GetMinButtonSize(AButtonIndex: Integer): TSize;
...
uses
  ...
  cxGeometry, dxCore, Math, RTLConsts;
...
begin
  ...
  begin
    ...
    Result := APainter.NavigatorButtonGlyphSize;
end;
...
begin
  ...
  AContentRect := cxRectContent(ABounds, APainter.NavigatorButtonGlyphPadding);
  ...
  try
    ...
    begin
      AImageRect := cxRectCenter(AContentRect, Button.GetImageSize);
...
begin
  Painter.DrawNavigatorButton(ACanvas, R, AState, clWindow);
end;

procedure TcxNavigatorButtonViewInfo.DrawGlyph(
  ACanvas: TcxCanvas; const R: TRect; AState: TcxButtonState);
begin
  Painter.DrawNavigatorButtonGlyph(ACanvas, Button.InternalImages,
    Button.InternalImageIndex, R, AState <> cxbsDisabled,
    Button.IsUserImageListUsed);
end;
...
begin
  ...
  ACanvas.FillRect(Bounds, AViewParams);
...
constructor TcxNavigatorHintHelper.Create(
  ANavigatorViewInfo: TcxNavigatorViewInfo);
begin
  inherited Create;
  FNavigatorViewInfo := ANavigatorViewInfo;
end;
...
begin
  ...
  if (ButtonCount = 0) and (AInfoPanelText <> '') then
    FInfoPanelViewInfo.Calculate(ANavigatorBounds, AInfoPanelText)
  else
  begin
    AFoundInfoPanelPos := False;
    for I := 0 to ButtonCount - 1 do
    begin
      if not AFoundInfoPanelPos and (AInfoPanelText <> '') and
        (Buttons[I].Button.DefaultIndex >= NBDI_NEXT) then
      begin
        AFoundInfoPanelPos := True;
        AButtonBounds.Left := AButtonBounds.Right;
        AButtonBounds.Right := AButtonBounds.Right + AInfoPanelWidth;
        FInfoPanelViewInfo.Calculate(AButtonBounds, AInfoPanelText);
      end;
      AButtonBounds.Left := AButtonBounds.Right;
      AButtonBounds.Right := AButtonBounds.Right + Buttons[I].Size.cx + AButtonIndent;
...
begin
  ...
  AMinHeight := Max(AInfoPanelSize.cy, Buttons.Size.cy);
  AButtonIndent := 0;
  ...
  begin
    if ButtonCount > 0 then
      AButtonIndent := (AWidth - AMinWidth) div ButtonCount;
...
begin
  ...
  with APainter.NavigatorButtonGlyphPadding do
  begin
    Inc(Result.cx, Left + Right);
    Inc(Result.cy, Top + Bottom);
  end;
  with APainter.NavigatorButtonMinSize do
  begin
...
begin
  ...
  begin
    ...
    R2 := R1;
    InflateRect(R2, 1, 1);
    InternalInvalidate(ANavigatorControl.Handle, R2, R1, False);
  end;
end;
...
begin
  Result := Max(Max(FNavigatorSize.cy,
    GetLookAndFeelPainter.NavigatorButtonMinSize.cy),
...
begin
  ...
  if AOwnerControl.HScrollBarVisible then
  begin
  ...
  begin
    ...
    if AOwnerControl.VScrollBarVisible then
      Inc(Result.Right, AOwnerControl.VScrollBar.Width);
...
procedure TcxCustomNavigator.SetBorderStyle(
  const Value: TcxNavigatorBorderStyle);
begin

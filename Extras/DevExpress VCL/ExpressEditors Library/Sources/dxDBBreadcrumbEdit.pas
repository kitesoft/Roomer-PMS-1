...
begin
  ...
  if DataBusy or (DataSet.State = dsEdit) or (DataSet.State = dsSetKey) then
    Exit;
  StructureChanged;
end;
...
constructor TdxDBBreadcrumbEditDataBinding.Create(
  AOwner: TPersistent; AControl: IdxDBBreadCrumbEdit);
begin
...
procedure TdxDBBreadcrumbEditDataBinding.DoSetFieldValue(
  const AValue: string; var AFieldValue: string);
begin
...
function TdxDBBreadcrumbEditController.FindNodeByKeyValue(
  const AKeyValue: Variant): TdxDBBreadcrumbEditNode;
begin
...
function TdxDBBreadcrumbEditController.ProcessParentKeyValueChanges(
  const ANewParentKeyValue: Variant): Boolean;
var
  ANode: TdxDBBreadcrumbEditNode;
begin

...
//    procedure AdjustScrollWidth;
  ...
  TcxMCListBox = class(TcxCustomEditContainer)
  private
...
begin
  Dec(X, Container.InnerHeader.GetSplitLineOffset);
...
begin
  ...
  for I := 0 to Container.HeaderSections.Count - 1 do
    DrawLine(Container.HeaderSections[I].Right, AColumnLineTop, Height);
end;
...
var
  P: TPoint;
begin
  P := cxParentToClient(FInnerListBox, APos, Self);
  Result := FInnerListBox.ItemAtPos(P, AExisting);
end;
...
begin
  inherited;
  FullRepaint;
  cxRecreateControlWnd(FInnerListBox);
end;
...
procedure TcxMCListBox.DrawItem(Control: TWinControl; Index: Integer; ARect: TRect; State: TOwnerDrawState);
  ...
  begin
    FInnerListBox.DrawLine(GetHeaderSectionRect(AColumnIndex, InnerListBox.VScrollBarVisible).Right,
...
begin
  ...
  ATextHeight := cxTextHeight(FInnerListBox.Canvas.Handle);

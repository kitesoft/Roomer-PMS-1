...
{$ENDIF}
  ...
  cxDrawTextUtils, ImgList, dxCoreClasses, cxLookAndFeels, Menus;

type
  TdxAlertWindowAnimation = (awaNone, awaSlide, awaFade, awaMove);
...
type
  ...
  public
    ...
    function Add(const ACaption, AText: string; AImageIndex: TcxImageIndex): TdxAlertWindowMessage; overload;
  ...
  public
    ...
    procedure Assign(Source: TPersistent); override;
  published
    property CaptionButtons: TdxAlertWindowCaptionButtons
      read FCaptionButtons write SetCaptionButtons default dxAlertWindowDefaultCaptionButtons;
...
// #BA: Don't change properties order
  ...
  protected
    ...
    procedure JumpToFinalState; override;
  public
    procedure StartAnimation(const ARect: TRect; AShowing: Boolean;
      ADirection: TdxAlertWindowMovingDirection; ATime: Cardinal); overload;
  ...
  private
    ...
    procedure AnimationComplete(AAnimation: TdxAlertWindowAnimation);
  ...
  TdxAlertWindow = class(TCustomForm, IcxMouseTrackingCaller,
    IcxMouseTrackingCaller2, IdxAlertWindow, IdxSkinSupport2)
  private
  ...
  protected
    ...
    procedure CalculateAutoSize(AWidth, AHeight: Integer); virtual;
  ...
  public
    constructor Create(AAlertWindow: TdxAlertWindow; APosition: TdxAlertWindowPosition);
  ...
  public
    function Add(AAlertWindow: TdxAlertWindow; APosition: TdxAlertWindowPosition): TdxAlertWindowPositionInfo; overload;
  ...
  private
    ...
    function GetLastInitialGrid(APosition: TdxAlertWindowPosition): TdxAlertWindowPositionInfo;
    function GetNextWindowForShowing: TdxAlertWindow;
    function GetPosition: TdxAlertWindowPosition;
    function HasWindow(AAlertWindow: TdxAlertWindow): Boolean;
  ...
  public
    ...
    procedure AddItem;
    procedure DeleteItem(AAlertWindow: TdxAlertWindow);
  ...
  TdxAlertWindowManager = class(TcxCustomComponent, IdxSkinSupport, IcxLookAndFeelContainer)
  private
...
uses
  Types, dxCore, cxEditConsts, ShellAPI;

function AdjustSizeByIndent(AValue: Integer; ANeedIndent: Boolean = True): Integer;
begin
  Result := AValue;
  if ANeedIndent and (Result > 0) then
    Inc(Result, dxAlertWindowIndentBetweenRegions);
end;

procedure AddSize(AElementSize: Integer; var ALayoutSize: Integer);
begin
  Inc(ALayoutSize, AdjustSizeByIndent(AElementSize, ALayoutSize > 0));
end;
...
function GetSuitableHeightForText(AFont: TFont; const AText: String;
  AWidth: Integer; AFormat: Cardinal): Integer;
var
  ARect: TRect;
...
begin
  ...
  begin
    ...
    cxMakeTextRows(cxScreenCanvas.Canvas, PChar(AText), Length(AText), ARect,
      ATextParams, ATextRows, Result);
...
function GetSuitableWidthForText(AFont: TFont; const AText: String;
  AHeight: Integer; AFormat: Cardinal): Integer;

  function GetNextSubString(const AText, ASeparator: string;
    AInitialPosition: Integer; var ASubString: string): Integer;
  begin
    Result := Pos(ASeparator, Copy(AText, AInitialPosition, Length(AText)));
    if Result = 0 then
      Result := Length(AText);
    ASubString := Copy(AText, AInitialPosition, Result);
  end;

  function GetMaxTextWidth(const AText: String): Integer;
  var
    I: Integer;
    ASubString: String;
    ALineCount: Integer;
  begin
    ...
    I := 1;
    ALineCount := 1;
    while (I < Length(AText)) and (ALineCount * cxTextHeight(AFont) < AHeight) do
    begin
      I := I + GetNextSubString(AText, #13, I, ASubString);
      Result := Max(Result, cxTextExtent(AFont, ASubString).cx);
      Inc(ALineCount);
    end;
  end;

  function GetMinTextWidth(const AText: String): Integer;
  var
    I: Integer;
    ASubString: String;
  begin
    ...
    begin
      GetNextSubString(AText, #13, I, ASubString);
      I := I + GetNextSubString(ASubString, ' ', 0, ASubString);
      Result := Max(Result, cxTextExtent(AFont, ASubString).cx);
    end;
  end;

  function IsPlacedText(AText: String; AWidth, AHeight: Integer): Boolean;
  begin
    Result := GetSuitableHeightForText(AFont, AText, AWidth, AFormat) <= AHeight;
  end;

  function NextDivisionOfWidth(const AText: String;
    AMinWidth, AWidth, AHeight, ARatio: Integer): Integer;
  begin
    if ((AWidth div ARatio) <= 2) or (AWidth <= AMinWidth + 2) then
      Result := AWidth
    else
    begin
      Result := AWidth - (AWidth div ARatio);
      if IsPlacedText(AText, Result, AHeight) and (Result > AMinWidth) then
        Result := NextDivisionOfWidth(AText, AMinWidth, Result, AHeight, ARatio)
      else
        Result := NextDivisionOfWidth(AText, AMinWidth, AWidth, AHeight, ARatio * 2);
    end;
  end;

begin
  Result := NextDivisionOfWidth(AText, GetMinTextWidth(AText),
    GetMaxTextWidth(AText), AHeight, 2);
end;
...
procedure TdxAlertWindow.CalculateAutoSize(AWidth, AHeight: Integer);
...
begin
  ...
  begin
    ...
    SetBounds(Left, Top, CalculateNewWidth(AWidth), CalculateNewHeight(AHeight));
  end;
end;
...
begin
  ...
  begin
    if not Visible then
      CalculateAutoSize(OptionsSize.Width, OptionsSize.Height);
    CalculateViewInfo;
...
function TdxAlertWindow.DoCustomDrawBackground(ACanvas: TcxCanvas;
  AViewInfo: TdxAlertWindowViewInfo): Boolean;
begin
  Result := False;
...
function TdxAlertWindow.DoCustomDrawButton(ACanvas: TcxCanvas;
  AViewInfo: TdxAlertWindowCustomButtonViewInfo): Boolean;
begin
  Result := False;
...
function TdxAlertWindow.DoCustomDrawMessageCaptionText(ACanvas: TcxCanvas;
  AViewInfo: TdxAlertWindowCaptionTextViewInfo): Boolean;
begin
  Result := False;
...
function TdxAlertWindow.DoCustomDrawMessageImage(ACanvas: TcxCanvas;
  AViewInfo: TdxAlertWindowImageViewInfo): Boolean;
begin
  Result := False;
...
function TdxAlertWindow.DoCustomDrawMessageText(ACanvas: TcxCanvas;
  AViewInfo: TdxAlertWindowMessageTextViewInfo): Boolean;
begin
  Result := False;
...
function TdxAlertWindow.DoCustomDrawNavigationPanelText(ACanvas: TcxCanvas;
  AViewInfo: TdxAlertWindowNavigationPanelTextViewInfo): Boolean;
begin
  Result := False;
...
procedure TdxAlertWindow.LookAndFeelOptionsChanged(Sender: TcxLookAndFeel;
...
procedure TdxAlertWindow.MouseMove(Shift: TShiftState; X, Y: Integer);
  ...
  begin
    Result := (Abs(FDraggingPoint.X - X) > Mouse.DragThreshold) or
      (Abs(FDraggingPoint.Y - Y) > Mouse.DragThreshold);
...
procedure TdxAlertWindow.SetOptionsCaptionButtons(
  AValue: TdxAlertWindowOptionsCaptionButtons);
begin
...
procedure TdxAlertWindow.SetOptionsNavigationPanel(
  AValue: TdxAlertWindowOptionsNavigationPanel);
begin
...
begin
  ...
  begin
    ARegion := CreateRoundRectRgn(0, 0, cxRectWidth(BoundsRect) + 1,
      cxRectHeight(BoundsRect) + 1, ACornerRadius, ACornerRadius);
...
procedure TdxAlertWindowOptionsAnimate.SetShowingAnimation(
  AValue: TdxAlertWindowAnimation);
begin
...
procedure TdxAlertWindowFadingAnimationController.StartAnimation(
  AStartALphaValue, AFinishAlphaValue: Byte; ATime: Cardinal);
var
...
procedure TdxAlertWindowMovingAnimationController.StartAnimation(
  const AStartPoint, AFinishPoint: TPoint; ATime: Cardinal);
...
begin
  ...
  begin
    FFramesCount := Trunc(ARealTimeAnimation / GetAnimationTimerInterval);
    FStepMovingX := GetAnimationTimerInterval *
      (AFinishPoint.X - FWorkingPointX) / ARealTimeAnimation;
    FStepMovingY := GetAnimationTimerInterval *
      (AFinishPoint.Y - FWorkingPointY) / ARealTimeAnimation;
    StartAnimationTimer;
  end
  else
  begin
...
begin
  ...
  P := Point(Min(0, GetCurrentPoint.X - BaseRect.Left),
    Min(0, GetCurrentPoint.Y - BaseRect.Top));
...
procedure TdxAlertWindowSlidingAnimationController.StartAnimation(const ARect: TRect;
  AShowing: Boolean; ADirection: TdxAlertWindowMovingDirection; ATime: Cardinal);
const
...
begin
  FBaseRect := ARect;
  AOffset := Point(AOffsetHor[ADirection] * cxRectWidth(ARect),
    AOffsetVert[ADirection] * cxRectHeight(ARect));
  if AShowing then
    StartAnimation(cxPointOffset(ARect.TopLeft, AOffset), ARect.TopLeft, ATime)
  else
    StartAnimation(ARect.TopLeft, cxPointOffset(ARect.TopLeft,
      cxPointInvert(AOffset)), ATime);
end;
...
function TdxAlertWindowMessageList.Add(const ACaption, AText: string;
  AImageIndex: TcxImageIndex): TdxAlertWindowMessage;
begin
  Result := Add;
...
function TdxAlertWindowCustomViewInfo.GetHitTest(
  const APoint: TPoint): TdxAlertWindowCustomViewInfo;
begin
...
procedure TdxAlertWindowViewInfo.CalculateCustomSize(
  AOptionsSize: TdxAlertWindowOptionsSize; var AWidth, AHeight: Integer);
...
begin
  Painter.DrawAlertWindowBackground(ACanvas, Bounds);
end;
...
begin
  if IsImageAssigned then
    Result := OptionsMessage.Images.Height
  else
    Result := 0;
end;
...
begin
  if IsImageAssigned then
    Result := OptionsMessage.Images.Width
  else
    Result := 0;
end;
...
begin
  if IsImageAssigned then
    OptionsMessage.Images.Draw(ACanvas.Canvas, Bounds.Left, Bounds.Top, ImageIndex);
end;
...
begin
  Result := (OptionsMessage.Images <> nil) and (ImageIndex >= 0);
end;
...
begin
  Result := FormatText(OptionsMessage.Caption.AlignHorz, OptionsMessage.Caption.AlignVert) or
...
function TdxAlertWindowNavigationPanelViewInfo.GetHitTest(
  const APoint: TPoint): TdxAlertWindowCustomViewInfo;
begin
  Result := nil;
  if cxRectPtIn(Bounds, APoint) then
...
begin
  Result := GetSuitableHeightForText(Font, Text, AWidth,
    CXTO_WORDBREAK or CXTO_EDITCONTROL or CXTO_END_ELLIPSIS);
end;
...
begin
  if AHeight = -1 then
    ...
    Result := GetSuitableWidthForText(Font, Text, AHeight,
      CXTO_WORDBREAK or CXTO_EDITCONTROL or CXTO_END_ELLIPSIS);
end;
...
begin
  Result := FormatText(OptionsMessage.Text.AlignHorz, OptionsMessage.Text.AlignVert);
...
begin
  Painter.DrawAlertWindowButton(ACanvas, Bounds, State, Kind);
end;

function TdxAlertWindowCustomButtonViewInfo.GetContentOffsets: TRect;
begin
  Result := Painter.AlertWindowButtonContentOffsets(Kind);
end;
...
begin
  Result := Painter.AlertWindowButtonGlyphSize(Kind);
end;
...
function TdxAlertWindowButtonsViewInfoList.Add(
  AButton: TdxAlertWindowCustomButtonViewInfo): Integer;
begin
  Result := inherited Add(AButton);
end;

function TdxAlertWindowButtonsViewInfoList.Add(
  AOwner: IdxAlertWindow; AButtonItem: TdxAlertWindowButton): Integer;
begin
  Result := Add(TdxAlertWindowButtonViewInfo.Create(AOwner, AButtonItem));
end;

function TdxAlertWindowButtonsViewInfoList.Add(AOwner: IdxAlertWindow;
  AButtonViewInfoClass: TdxAlertWindowCustomButtonViewInfoClass): Integer;
begin
  Result := Add(AButtonViewInfoClass.Create(AOwner));
end;

function TdxAlertWindowButtonsViewInfoList.GetItem(
  AIndex: Integer): TdxAlertWindowCustomButtonViewInfo;
begin
...
procedure TdxAlertWindowOptionsNavigationPanel.SetVisibility(
  AValue: TdxAlertWindowNavigationPanelVisibility);
begin
...
function TdxAlertWindowCustomButtonsViewInfo.GetHitTest(
  const APoint: TPoint): TdxAlertWindowCustomViewInfo;
var
  I: Integer;
begin
...
constructor TdxAlertWindowButtonViewInfo.Create(AOwner: IdxAlertWindow;
...
begin
  ...
  if IsImageAssigned then
    cxDrawImage(ACanvas.Handle, cxRectCenter(Bounds, GlyphSize.cx, GlyphSize.cy),
      Bounds, nil, OptionsButtons.Images, ButtonItem.ImageIndex, idmNormal);
end;
...
begin
  if IsImageAssigned then
    Result := cxSize(OptionsButtons.Images.Width, OptionsButtons.Images.Height)
  else
...
function TdxAlertWindowController.ButtonHitTest(
  const APoint: TPoint): TdxAlertWindowCustomButtonViewInfo;
var
  AViewInfo: TdxAlertWindowCustomViewInfo;
begin
...
procedure TdxAlertWindowController.SetPressedButton(
  AValue: TdxAlertWindowCustomButtonViewInfo);
var
  AButton: TdxAlertWindowCustomButtonViewInfo;
begin
  if (AValue <> nil) and not AValue.Enabled then
...
begin
  ...
  try
    ...
    begin
      ...
      cxControls.ShowPopupMenu(AlertWindow.GetContainer,
        AlertWindow.GetOptionsCaptionButtons.PopupMenu, APoint.X, APoint.Y);
    end;
  finally
...
begin
  Painter.DrawAlertWindowButton(ACanvas, Bounds, State, Kind, Pinned);
end;
...
begin
  inherited Create;
  FAlertWindow := AOwner;
  FOptionsAnimate := TdxAlertWindowOptionsAnimate.Create(AOwner);
...
procedure TdxAlertWindowAnimationHelper.AnimationComplete(AAnimation: TdxAlertWindowAnimation);
begin
...
begin
  ...
  case FVisibilityAnimation of
    awaFade:
      FadingAnimationController.StartAnimation(FAlertWindow.AlphaBlendValue, 0,
        OptionsAnimate.HidingAnimationTime);
    awaMove:
      MovingAnimationController.StartAnimation(BoundsRect,
        CalculateFinishRectMoving.TopLeft, OptionsAnimate.HidingAnimationTime);
    awaSlide:
      SlidingAnimationController.StartAnimation(BoundsRect, False,
        GetRealMovingDirection(awvtHiding), OptionsAnimate.HidingAnimationTime);
    awaNone:
      AlertWindow.VisibilityTransitionComplete;
  end;
end;
...
procedure TdxAlertWindowAnimationHelper.SetOptionsAnimate(
  AValue: TdxAlertWindowOptionsAnimate);
begin
...
begin
  if (AlertWindow.VisibilityTransition = awvtShowing) and
    not AlertWindow.Visible then
  ...
  case FVisibilityAnimation of
    awaFade:
      begin
        AlphaBlendValue := 0;
        Show;
        FadingAnimationController.StartAnimation(0, OptionsAnimate.AlphaBlendValue,
          OptionsAnimate.ShowingAnimationTime)
      end;
    awaMove:
      begin
        AlphaBlendValue := OptionsAnimate.AlphaBlendValue;
        MovingAnimationController.StartAnimation(CalculateStartRectMoving,
          BoundsRect.TopLeft, OptionsAnimate.ShowingAnimationTime);
      end;
    awaSlide:
      begin
        AlphaBlendValue := OptionsAnimate.AlphaBlendValue;
        SlidingAnimationController.StartAnimation(BoundsRect, True,
          GetRealMovingDirection(awvtShowing), OptionsAnimate.ShowingAnimationTime);
      end;
    awaNone:
      begin
        AlphaBlendValue := OptionsAnimate.AlphaBlendValue;
...
begin
  ...
  begin
    if AlertWindow.MouseInControl then
      DoStartFadingAnimation(OptionsAnimate.HotTrackAlphaBlendValue,
        OptionsAnimate.HotTrackFadeInTime)
    else
      DoStartFadingAnimation(OptionsAnimate.AlphaBlendValue,
        OptionsAnimate.HotTrackFadeOutTime);
  end
  else
  begin
    FadingAnimationController.StopAnimation;
...
procedure TdxAlertWindowOptionsCaptionButtons.SetCaptionButtons(
  AValue: TdxAlertWindowCaptionButtons);
begin
...
begin
  ...
  begin
    if FPopupMenu <> nil then
      FPopupMenu.RemoveFreeNotification(FreeNotificator);
    FPopupMenu := AValue;
    if FPopupMenu <> nil then
      FPopupMenu.FreeNotification(FreeNotificator);
    Changed;
  end;
end;
...
constructor TdxAlertWindowPositionInfo.Create(AAlertWindow: TdxAlertWindow;
...
function TdxAlertWindowPositionInfo.CompareAlertWindow(
  AAlertWindow: TdxAlertWindow): Boolean;
begin
...
begin
  Result := not (cxRectIsEqual(FAlertWindow.BoundsRect, InitialBounds) or
    (FAlertWindow.VisibilityTransition = awvtShowing));
end;
...
begin
  inherited Create(Aowner);
  FOptionsAnimate := TdxAlertWindowOptionsAnimate.Create(Self);
  FOptionsBehavior := TdxAlertWindowOptionsBehavior.Create(Self);
  FOptionsButtons := TdxAlertWindowOptionsButtons.Create(Self);
  FOptionsCaptionButtons := TdxAlertWindowOptionsCaptionButtons.Create(Self);
  FOptionsMessage := TdxAlertWindowOptionsMessage.Create(Self);
...
procedure TdxAlertWindowManager.AlertWindowButtonClickHandler(
...
procedure TdxAlertWindowManager.AlertWindowCaptionButtonClickHandler(
  AAlertWindow: TdxAlertWindow; AButton: TdxAlertWindowCaptionButton;
...
procedure TdxAlertWindowManager.AlertWindowCustomDrawButtonHandler(
  AAlertWindow: TdxAlertWindow; ACanvas: TcxCanvas;
  AViewInfo: TdxAlertWindowCustomButtonViewInfo; var ADone: Boolean);
begin
...
procedure TdxAlertWindowManager.AlertWindowCustomDrawMessageCaptionTextHandler(
  AAlertWindow: TdxAlertWindow; ACanvas: TcxCanvas;
  AViewInfo: TdxAlertWindowCaptionTextViewInfo; var ADone: Boolean);
begin
...
procedure TdxAlertWindowManager.AlertWindowCustomDrawMessageImageHandler(
  AAlertWindow: TdxAlertWindow; ACanvas: TcxCanvas;
  AViewInfo: TdxAlertWindowImageViewInfo; var ADone: Boolean);
begin
...
procedure TdxAlertWindowManager.AlertWindowCustomDrawNavigationPanelTextHandler(
  AAlertWindow: TdxAlertWindow; ACanvas: TcxCanvas;
  AViewInfo: TdxAlertWindowNavigationPanelTextViewInfo;
  var ADone: Boolean);
begin
  ADone := DoCustomDrawNavigationPanelText(AAlertWindow, ACanvas, AViewInfo);
end;

procedure TdxAlertWindowManager.AlertWindowDragBeginHandler(
  AAlertWindow: TdxAlertWindow; var AAllow: Boolean);
begin
...
procedure TdxAlertWindowManager.AlertWindowMouseDownHandler(
  AAlertWindow: TdxAlertWindow; AButton: TMouseButton; AShift: TShiftState; X,
  Y: Integer);
begin
...
procedure TdxAlertWindowManager.DoButtonClick(AAlertWindow: TdxAlertWindow;
...
function TdxAlertWindowManager.DoCaptionButtonClick(AAlertWindow: TdxAlertWindow;
  AButton: TdxAlertWindowCaptionButton): Boolean;
begin
  Result := False;
  if Assigned(FOnCaptionButtonClick) then
...
function TdxAlertWindowManager.DoCustomDrawBackground(AAlertWindow: TdxAlertWindow;
  ACanvas: TcxCanvas; AViewInfo: TdxAlertWindowViewInfo): Boolean;
begin
  Result := False;
  if Assigned(FOnCustomDrawBackground) then
...
function TdxAlertWindowManager.DoCustomDrawButton(AAlertWindow: TdxAlertWindow;
  ACanvas: TcxCanvas; AViewInfo: TdxAlertWindowCustomButtonViewInfo): Boolean;
begin
  Result := False;
  if Assigned(FOnCustomDrawButton) then
...
function TdxAlertWindowManager.DoCustomDrawMessageCaptionText(AAlertWindow: TdxAlertWindow;
...
function TdxAlertWindowManager.DoCustomDrawMessageImage(AAlertWindow: TdxAlertWindow;
  ACanvas: TcxCanvas; AViewInfo: TdxAlertWindowImageViewInfo): Boolean;
begin
  Result := False;
  if Assigned(FOnCustomDrawMessageImage) then
...
function TdxAlertWindowManager.DoCustomDrawMessageText(AAlertWindow: TdxAlertWindow;
  ACanvas: TcxCanvas; AViewInfo: TdxAlertWindowMessageTextViewInfo): Boolean;
begin
  Result := False;
  if Assigned(FOnCustomDrawMessageText) then
...
function TdxAlertWindowManager.DoCustomDrawNavigationPanelText(AAlertWindow: TdxAlertWindow;
...
procedure TdxAlertWindowManager.DoMeasureMessageText(
  AAlertWindow: TdxAlertWindow; var AWidth, AHeight: Integer);
begin
  if Assigned(FOnMeasureMessageText) then
...
procedure TdxAlertWindowManager.DoMouseDown(AAlertWindow: TdxAlertWindow;
...
procedure TdxAlertWindowManager.DoMouseMove(AAlertWindow: TdxAlertWindow;
...
procedure TdxAlertWindowManager.DoMouseUp(AAlertWindow: TdxAlertWindow;
  AButton: TMouseButton; AShift: TShiftState; X, Y: Integer);
begin
  if Assigned(FOnMouseUp) then
...
function TdxAlertWindowManager.GetOptionsAnimate: TdxAlertWindowOptionsAnimate;
begin
...
begin
  ...
  if HotWindow = AComponent then
    FHotWindow := nil;
  if Result then
    FInitialLayout.DeleteItem(TdxAlertWindow(AComponent));
end;
...
procedure TdxAlertWindowManager.SetOptionsAnimate(AValue: TdxAlertWindowOptionsAnimate);
begin
  FOptionsAnimate.Assign(AValue);
end;
...
procedure TdxAlertWindowManager.SetOptionsCaptionButtons(
  AValue: TdxAlertWindowOptionsCaptionButtons);
begin
  FOptionsCaptionButtons.Assign(AValue);
end;
...
procedure TdxAlertWindowManager.SetOptionsNavigationPanel(
  AValue: TdxAlertWindowOptionsNavigationPanel);
begin
  FOptionsNavigationPanel.Assign(AValue);
end;
...
function TdxAlertWindowManager.Show(const ACaption,
  AText: string; AImageIndex: TcxImageIndex = -1): TdxAlertWindow;
begin
  Result := InternalAdd;
  Result.BeginUpdate;
  try
...
function TdxAlertWindowPositionInfoList.Add(AAlertWindow: TdxAlertWindow;
  APosition: TdxAlertWindowPosition): TdxAlertWindowPositionInfo;
begin
  Result := TdxAlertWindowPositionInfo.Create(AAlertWindow, APosition);
  inherited Add(Result);
end;
...
begin
  ...
  if (AAlertWindow <> nil) and ((FManager.WindowMaxCount = 0) or
    (FPositionInfoList.Count < FManager.WindowMaxCount)) then
  begin
    CalculateNextPosition(AAlertWindow);
    FManager.DoBeforeShow(AAlertWindow);
    AAlertWindow.Show;
    AddItem;
  end;
end;
...
procedure TdxAlertWindowInitialLayout.CalculateNextPosition(AAlertWindow: TdxAlertWindow);
  ...
  function CalculatePlaceRect(const AInitialPlaceRect, ALastWindowInGridRect: TRect): TRect;
  begin
    Result := AInitialPlaceRect;
    if ((Position = awpTopLeft) or (Position = awpTopRight)) then
    begin
      if (ALastWindowInGridRect.Bottom + cxRectHeight(Result) <= WorkArea.Bottom) then
        Result := cxRectSetTop(Result, ALastWindowInGridRect.Bottom);
    end
    else
      if ALastWindowInGridRect.Top - cxRectHeight(Result) >= WorkArea.Top then
        Result := cxRectSetTop(Result, ALastWindowinGridRect.Top - cxRectHeight(Result));
  end;

  function InitialPlaceRect(APosition: TdxAlertWindowPosition): TRect;
  var
    P: TPoint;
  begin
    case APosition of
      awpTopLeft:
        P := WorkArea.TopLeft;
      awpTopRight:
        P := cxPointOffset(cxRectRightTop(WorkArea), -AAlertWindow.Width, 0);
      awpBottomLeft:
        P := cxPointOffset(cxRectLeftBottom(WorkArea), 0, -AAlertWindow.Height);
      awpBottomRight:
        P := cxPointOffset(WorkArea.BottomRight, -AAlertWindow.Width, -AAlertWindow.Height);
    end;
    Result := cxRectSetOrigin(AAlertWindow.BoundsRect, P);
  end;

var
  ALastWindowInGrid: TdxAlertWindowPositionInfo;
begin
  ALastWindowInGrid := GetLastInitialGrid(Position);
  if ALastWindowInGrid <> nil then
    AAlertWindow.BoundsRect := CalculatePlaceRect(InitialPlaceRect(Position),
      ALastWindowInGrid.InitialBounds)
  else
    AAlertWindow.BoundsRect := InitialPlaceRect(Position);
  FPositionInfoList.Add(AAlertWindow, Position);
end;
...
function TdxAlertWindowInitialLayout.GetLastInitialGrid(
  APosition: TdxAlertWindowPosition): TdxAlertWindowPositionInfo;
var
  I: Integer;
begin
  Result := nil;
  for I := FPositionInfoList.Count - 1 downto 0 do
    if not FPositionInfoList[I].IsPositionChanged and
      (FPositionInfoList[I].Position = APosition) then
    begin
      Result := FPositionInfoList[I];
      Break;
    end;
end;
...
begin
  ...
  for I := 0 to FManager.Count - 1 do
    if not FManager[I].Visible and not HasWindow(FManager[I]) then
    begin
      Result := FManager[I];
      Break;
    end;
...
begin
  Result := FManager.WindowPosition;
...
function TdxAlertWindowInitialLayout.HasWindow(AAlertWindow: TdxAlertWindow): Boolean;
var
  I: Integer;
begin
  Result := False;
  for I := 0 to FPositionInfoList.Count - 1 do
  begin
    if FPositionInfoList[I].CompareAlertWindow(AAlertWindow) then
    begin
      Result := True;
      Break;
    end;
...
function TdxAlertWindowInitialLayout.RemoveItemWithWindow(
  AAlertWindow: TdxAlertWindow): Integer;
var
  I: Integer;
begin
  Result := -1;
  ...
  begin
    ...
    begin
      Result := I;
      FPositionInfoList.FreeAndDelete(I);
      Break;
    end;
  end;
end;
...
function TdxAlertWindowCustomTextViewInfo.FormatText(AAlignHorz: TAlignment;
  AAlignVert: TcxAlignmentVert): Cardinal;
const
...
function TdxAlertWindowNavigationPanelTextViewInfo.CalculateAutoHeight(
...
begin
  ...
  Result := StringReplace(Result, sdxAlertWindowMessageIndexMacro,
    IntToStr(MessageIndex + 1), [rfReplaceAll]);
  Result := StringReplace(Result, sdxAlertWindowMessageCountMacro,
    IntToStr(MessageCount), [rfReplaceAll]);
end;
...
begin
  Result := cxRectContent(FBounds, GetViewInfo.Painter.AlertWindowContentOffsets);
end;
...
begin
  ...
  AddSize(MessageTextPlace.CalculatedHeight, Result);
  Result := Max(Result, ImagePlace.CalculatedHeight);
  AddSize(NavigationPanelPlace.CalculatedHeight, Result);
  AddSize(ButtonsPlace.CalculatedHeight, Result);
  Inc(Result, cxMarginsHeight(GetViewInfo.Painter.AlertWindowContentOffsets));
end;
...
begin
  Result := ContentCalculator.Bounds;
  Result := cxRectOffsetVert(Result,
    -AdjustSizeByIndent(GetContentCalculator.ButtonsPlace.CalculatedHeight));
  Result.Top := Result.Bottom - CalculatedHeight;
...
begin
  Result := ContentCalculator.Bounds;
  Inc(Result.Left, AdjustSizeByIndent(GetContentCalculator.ImagePlace.CalculatedWidth));
  Dec(Result.Right, AdjustSizeByIndent(
    GetContentCalculator.CaptionButtonsPlace.CalculatedWidth));
  Result := cxRectSetHeight(Result,
    Max(CalculatedHeight, GetContentCalculator.CaptionButtonsPlace.CalculatedHeight));
end;
...
function TdxAlertWindowMessageTextPlace.GetBounds: TRect;
  ...
  begin
    ...
    AddSize(GetContentCalculator.NavigationPanelPlace.CalculatedHeight, Result);
  end;
...
begin
  Result := ContentCalculator.Bounds;
  AddSize(GetContentCalculator.ImagePlace.CalculatedWidth, Result.Left);
  AddSize(CaptionHeight, Result.Top);
  Dec(Result.Bottom, AdjustSizeByIndent(BottomSpace));
end;
...
constructor TdxAlertWindowNavigationPanelContentCalculator.Create(
  AViewInfo: TdxAlertWindowCustomViewInfo);
begin
  inherited Create(AViewInfo);
...
begin
  ...
  Inc(Result, (cxTextSpace shl 1));
  AddSize(PreviousButtonPlace.CalculatedWidth, Result);
  AddSize(NextButtonPlace.CalculatedWidth, Result);
end;
...
constructor TdxAlertWindowCustomContentCalculator.Create(
  AViewInfo: TdxAlertWindowCustomViewInfo);
begin
  FViewInfo := AViewInfo;
end;

...
type
  ...
  protected
    property OnShellOptionsChanged: TdxShellBreadcrumbEditShellOptionsChangeEvent
...
var
  ATempPath: string;
begin
  ATempPath := ExcludeTrailingPathDelimiter(Path);
  Result := ExtractFileName(ATempPath);
  if Result = '' then
    Result := ATempPath;
end;
...
begin
  Result := GetPidlName(AbsolutePIDL);
end;
...
begin
  Result := (AFolder <> nil) and AFolder.IsFolder;
end;
...
begin
  ...
  Result.Handle := SHGetFileInfo('C:\', 0,
    AFileInfo, SizeOf(AFileInfo), SHGFI_SYSICONINDEX or SHGFI_SMALLICON);
...
procedure TdxShellBreadcrumbEditShellOptions.DoFolderChanged(
...
begin
  APidl := PathToAbsolutePIDL(APath, ShellOptions.Root, ViewOptions);
  if APidl = nil then
  begin
    ...
    begin
      repeat
        ...
        begin
          ATempPidl := InternalParseDisplayName(ASavedNode.ShellFolder, ASavedPath, ViewOptions);

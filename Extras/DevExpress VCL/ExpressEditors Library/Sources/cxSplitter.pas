...
uses
  Windows, Classes, Controls, Forms, Graphics, Messages, SysUtils, Types,
  cxClasses, cxControls, cxExtEditConsts, cxExtEditUtils, cxGraphics, cxLookAndFeels;
...
type
  ...
  protected
    ...
    property PositionAfterOpen: TcxPositionAfterOpen read FPositionAfterOpen write FPositionAfterOpen default 30;
...
uses
  Math, cxContainer, dxThemeConsts, dxThemeManager, dxUxTheme, cxGeometry;
...
begin
  ...
  if (SplitterDirection = cxsdLeftToRight) or (SplitterDirection = cxsdRightToLeft) then
    FArrowRect := Rect(Result.Left,
      Result.Top + (cxRectHeight(Result) div 2) - 7,
      Result.Left + SplitterDefaultSize - 1,
      Result.Top + (cxRectHeight(Result) div 2) + 7)
  else
    FArrowRect := Rect(Result.Left + (cxRectWidth(Result) div 2) - 7,
      Result.Top,
      Result.Left + (cxRectWidth(Result) div 2) + 7,
      Result.Top + SplitterDefaultSize - 1);
  HotZoneRect := Result;
end;

procedure TcxMediaPlayer9Style.DrawBackground(
  ACanvas: TcxCanvas; const ARect: TRect; const AHighlighted, AClicked: Boolean);
var
  FRect: TRect;
...
begin
  case SplitterDirection of
    cxsdLeftToRight, cxsdRightToLeft:
      FRect := cxRectTransform(ARect, 0, 1, 0, -1);
    cxsdTopToBottom, cxsdBottomToTop:
      FRect := cxRectTransform(ARect, 1, 0, -1, 0);
  end;
...
function TcxCustomSplitter.FindControl: TControl;
  ...
  {$IFDEF DELPHI11}
    if AlignWithMargins then
      Result := cxRectTransform(Result, Margins.Left, Margins.Top,
  ...
  {$IFDEF DELPHI11}
    if AControl.AlignWithMargins then
      Result := cxRectTransform(Result, AControl.Margins.Left, AControl.Margins.Top,
...
begin
  ...
  begin
    ...
    begin
      R := cxRectInflate(Result.BoundsRect, GetControlOffsets(Result));
      ...
      if PtInRect(R, P) then
        ...
        begin
          ...
          case Result.Align of
            alLeft:
              if (R.Right = R.Left) and (R.Left = Width) then Exit;
            alRight:
              if (R.Right = R.Left) and (R.Right = Left) then Exit;
            alTop:
              if (R.Bottom = R.Top) and (R.Top = Height) then Exit;
            alBottom:
              if (R.Bottom = R.Top) and (R.Bottom = Top) then Exit;
          end;
        end;
    end;
  end;
...
begin
  ...
  begin
    ...
    LookAndFeel.SkinPainter.DrawSplitter(FDrawBitmap.cxCanvas, R, smsInHotZone in FMouseStates,
...
begin
  if (FControl = nil) or (FNewSize = FOldSize) then
    Exit;
  Parent.DisableAlign;
  try
    case Align of
...
begin
  ...
  case Align of
    alLeft:
      S := FControl.Width + ASplit;
    alRight:
      S := FControl.Width - ASplit;
    alTop:
      S := FControl.Height + ASplit;
    alBottom:
      S := FControl.Height - ASplit;
  else
    S := 0;
...
begin
  if (FControl <> nil) and not (csLoading in ComponentState) then
    case Align of
      alLeft:
        begin
          Left := FControl.Left + FControl.Width + GetMargin;
          FOldSize := FControl.Width;
        end;
      alTop:
        begin
          Top := FControl.Top + FControl.Height + GetMargin;
          FOldSize := FControl.Height;
        end;
      alRight:
        begin
          Left := FControl.Left - Width - GetMargin;
          FOldSize := FControl.Width;
        end;
      alBottom:
        begin
          Top := FControl.Top - Height - GetMargin;
          FOldSize := FControl.Height;
        end;
    end;
end;

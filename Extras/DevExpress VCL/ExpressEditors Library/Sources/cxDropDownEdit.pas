...
uses
  ...
  dxCore, dxCoreClasses, dxMessages, cxClasses, cxContainer, cxControls,
...
begin
  ...
  begin
    ...
    begin
      Result := True;
      if ILookupData.GetActiveControl <> nil then
...
begin
  Result := not(ActiveProperties.ImmediateDropDownWhenKeyPressed and ActiveProperties.PopupWindowCapturesFocus and
...
begin
  if IsInplace and ActiveProperties.PopupSizeable then
    ...
    Result := ActiveProperties.PopupHeight;
end;
...
begin
  ...
  UpdateDrawValue;
  with ActiveProperties do
    if UseLookupData and FindSelection and ImmediateDropDownWhenKeyPressed and
...
begin
  ...
  SetBounds(Left, Top, APreferredWidth, APreferredHeight);
end;
...
var
  AStyle: TListBoxStyle;
begin
  ItemHeight := GetItemHeight;
  if Edit.IsOnMeasureItemEventAssigned then
    AStyle := lbOwnerDrawVariable
  else
    AStyle := lbOwnerDrawFixed;

...
type
  ...
  protected
    ...
    function NeedDrawTick(AIndex: Integer): Boolean; override;
  ...
  protected
    function GetPositionAfterSliding(X, Y: Integer): Integer; override;
...
begin
  ...
  FFrequency := TdxZoomTrackBarRangeOptions(Source).Frequency;
  FLineSize := TdxZoomTrackBarRangeOptions(Source).LineSize;
  FPageSize := TdxZoomTrackBarRangeOptions(Source).PageSize;
end;
...
function TdxCustomZoomTrackBarViewInfo.NeedDrawTick(AIndex: Integer): Boolean;
begin
  Result := 
    ((FrequencyLeft > 0) and (AIndex <= CenterPosition) and (((AIndex - Min) mod FrequencyLeft) = 0)) or
    ((FrequencyRight > 0) and (AIndex > CenterPosition) and (((AIndex - CenterPosition) mod FrequencyRight) = 0))
end;
...
begin
  ...
  if AEditProperties.ReverseDirection then
    AIsInc := not AIsInc;
  if AIsLineSize then
  begin
...
function TdxCustomZoomTrackBar.GetPositionAfterSliding(X, Y: Integer): Integer;
...
begin
  ...
  begin
    AInitialThumbOffCentering := Trunc((CenterPosition - StoredPosition) * ViewInfo.PixelsPerPositionFirstRange);
    if ViewInfo.GetMouseDelta(X, Y, MouseDownPos) <= AInitialThumbOffCentering then
      Result := StoredPosition + Round(ViewInfo.GetMouseDelta(X, Y, MouseDownPos) / ViewInfo.PixelsPerPositionFirstRange)
    else
    begin
      AMouseDeltaRemainder := ViewInfo.GetMouseDelta(X, Y, MouseDownPos) - AInitialThumbOffCentering;
      Result := StoredPosition + Round(AInitialThumbOffCentering / ViewInfo.PixelsPerPositionFirstRange + AMouseDeltaRemainder / ViewInfo.PixelsPerPositionSecondRange);
    end;
  end
  else
  begin
    AInitialThumbOffCentering := Trunc((CenterPosition - StoredPosition) * ViewInfo.PixelsPerPositionSecondRange);
    if ViewInfo.GetMouseDelta(X, Y, MouseDownPos) >= AInitialThumbOffCentering then
      Result := StoredPosition + Round(ViewInfo.GetMouseDelta(X, Y, MouseDownPos) / ViewInfo.PixelsPerPositionSecondRange)
    else
    begin
      AMouseDeltaRemainder := ViewInfo.GetMouseDelta(X, Y, MouseDownPos) - AInitialThumbOffCentering;
      Result := StoredPosition + Round(AMouseDeltaRemainder / ViewInfo.PixelsPerPositionFirstRange + AInitialThumbOffCentering / ViewInfo.PixelsPerPositionSecondRange);
    end;
  end;
  if Abs(ViewInfo.GetMouseDelta(X, Y, MouseDownPos)) > OffsetForJumpToCenterRenewal then
    FCanThumbJumpToCenter := True;
  if NeedJumpToCenter(ViewInfo.GetMouseDelta(X, Y, MouseDownPos), AInitialThumbOffCentering) then
    Result := CenterPosition;
end;

procedure TdxCustomZoomTrackBar.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
begin
  Result := TdxZoomTrackBarProperties(FProperties);
end;

...
{$ENDIF}
  ...
  cxGraphics, cxLookAndFeels, cxLookAndFeelPainters, dxCustomHint, ComCtrls, CommCtrl;
...
uses
  ...
  cxGeometry, cxEditUtils, cxExtEditUtils;
...
procedure TcxHintViewInfo.Paint(ACanvas: TcxCanvas);
  ...
  begin
    if not Owner.Icon.Empty then
      DrawIconEx(ACanvas.Handle, FIconOrigin.X, FIconOrigin.Y, Owner.Icon.Handle,
        IconSize, IconSize, 0, 0, DI_NORMAL);
  end;

  procedure InternalDrawHintText(ABounds: TRect; const AText: string; AFont: TFont;
    AColor: TColor; AFlags: Cardinal);
  begin
    ACanvas.Font.Assign(AFont);
    if AColor <> clDefault then
      ACanvas.Font.Color := AColor;
    DrawText(ACanvas.Handle, PChar(AText), Length(AText), ABounds,
  ...
  begin
    ...
    if Caption <> '' then
      InternalDrawHintText(FCaptionBounds, Caption, CaptionFont, FPainter.ScreenTipGetTitleTextColor,
        DT_WORDBREAK or DT_NOPREFIX or DT_VCENTER);

    InternalDrawHintText(FHintBounds, Hint, HintFont, FPainter.ScreenTipGetDescriptionTextColor,
...
begin
  Result := not Owner.AbsolutePosition and
    not (CalculatedCallOutPos in [cxbpNone, cxbpTopLeft, cxbpTopRight]);
end;
...
function TcxHintWindow.CalcHintRect(MaxWidth: Integer; const AHint: string;
  AData: Pointer): TRect;
begin
  CalculateController;
...
begin
  if FStandardHint or HasWindowRegion then
...
procedure TcxHintWindow.CalculateController;
  ...
  begin
    ...
    Result := (AController <> nil) and
      Supports(AController.HintStyle, IcxHint, AHintData);
    if Result then
...
const
  LI_LARGE = 1;
  LI_SMALL = 0;
  ARealIconTypes: array[TcxRealHintIconType] of MakeIntResource = (
    IDI_APPLICATION, IDI_INFORMATION, IDI_WARNING, IDI_ERROR, IDI_QUESTION,
    IDI_WINLOGO);
  AIconSize: array[TcxHintIconSize] of Integer = (LI_LARGE, LI_LARGE, LI_SMALL);

  function InternalLoadIcon(AIconID: MakeIntResource; out AHandle: HICON): Boolean;
  begin
  {$IFDEF DELPHI11}
    Result := Succeeded(LoadIconMetric(0, MakeIntResourceW(AIconID), AIconSize[FIconSize], AHandle));
  {$ELSE}
    AHandle := 0;
    Result := False;
  {$ENDIF}
...
begin
  ...
  begin
    AIconID := ARealIconTypes[TcxRealHintIconType(Ord(FIconType) - 1)];

...
uses
  ...
  Menus, RichEdit, StdCtrls, SysUtils, cxDataUtils,
  dxCore, dxCoreClasses, dxMessages, cxClasses, cxContainer,
  cxControls, cxEdit, cxDrawTextUtils, cxGraphics, cxLookAndFeels, cxMemo,
  cxScrollbar, cxTextEdit, dxGDIPlusApi, cxRichEditUtils, cxExtEditConsts,
  dxDrawRichTextUtils;
...
const
  cxMaxVersionRichEditClass = {$IFNDEF DELPHI12}recRichEdit20{$ELSE}recRichEdit41{$ENDIF};
...
type
  ...
  public
    ...
    property SelectionBar: Boolean read FSelectionBar write SetSelectionBar
      default False;
    property StreamModes: TcxRichEditStreamModes read GetStreamModes
...
procedure cxDrawRichEdit(ADC: HDC; const ARect: TRect; const AText: string;
  ...
  ATextColor: TColor; ACalculateHeight: Boolean; out AHeight: Integer); overload;
...
{$IFDEF DELPHI18}
  AnsiStrings, 
{$ENDIF}
  cxEditPaintUtils, cxEditUtils, cxVariants,
  cxDWMAPI, dxUxTheme, dxThemeConsts, dxThemeManager, cxGeometry;
...
const
  ...
  cxRichNoError = $0000;
  cxRichEditZoomDenominator = 64 * 100;
...
begin
  ...
  Result.ParentWindow := cxMessageWindow.Handle;
...
begin
  ...
  begin
    ...
    if Alignment <> TcxCustomRichEditProperties(AProperties).Alignment then
    begin
    ...
    AdvancedTypography := TcxCustomRichEditProperties(AProperties).AdvancedTypography;
    ZoomFactor := TcxCustomRichEditProperties(AProperties).ZoomFactor;
    HandleNeeded;

    SetRichDefAttributes(RichRenderer, AFont, ATextColor);

    if not RichRenderer.MemoMode then
      dxRichLoadFromString(RichLines, AText)
    else
      Text := AText;
...
const
  ...
  begin
    Result := cxRectOffset(R, -R.Left, -R.Top);
    Result.Right := cxPixelsToTwips(Result.Right);
    if ACalculateHeight then
      Result.Bottom := TwipsPerInch
    else
      Result.Bottom := cxPixelsToTwips(Result.Bottom (*65535*));
  end;
...
begin
  ...
  cxDrawRichEdit(ADC, ARect, RichRenderer.Handle, 0, -1, ACalculateHeight, AHeight);
end;

procedure cxDrawRichEdit(ACanvas: TCanvas; const ARect: TRect; ARichEdit: TcxRichEdit;
  AMinCharIndex, AMaxCharIndex: Integer; ACalculateHeight: Boolean; out AHeight: Integer); overload;
begin
  cxDrawRichEdit(ACanvas.Handle, ARect, ARichEdit.InnerControl.Handle, AMinCharIndex,
    AMaxCharIndex, ACalculateHeight, AHeight);
end;
...
begin
  ...
  {$ENDIF}
    ...
    EditStream.dwCookie := TdxNativeUInt(@StreamInfo);
...
begin
  ...
  begin
    ...
    InternalSetMemoMode;
    if GetZoomFactor <> FZoomFactor then
      SendMessage(Handle, EM_SETZOOM, Trunc(FZoomFactor * cxRichEditZoomDenominator), cxRichEditZoomDenominator);
  end;
end;
...
procedure TcxRichInnerEdit.CheckZoomFactor;
begin
...
function TcxRichInnerEdit.GetZoomFactor: Double;
var
  ANumerator, ADenominator: TdxNativeInt;
begin
  Result := FZoomFactor;
  if HandleAllocated then
  begin
    if Boolean(SendMessage(Handle, EM_GETZOOM, WPARAM(@ANumerator), LPARAM(@ADenominator))) and (ADenominator > 0) then
...
begin
  if RichLines.IsModification or not ((Container <> nil) and Container.Focused and not Container.DoEditing) then
    inherited;
end;
...
begin
  if (Container <> nil) and not Container.IsDestroying and
    (Container.ComponentState * [csLoading, csReading] = []) then
      Container.DoEditing;
  inherited;
end;
...
begin
  if HandleAllocated and MemoMode then
    ...
    if (Container <> nil) and not Container.IsInplace then
      SetRichDefAttributes(Self, Container.VisibleFont, Container.VisibleFont.Color);
end;
...
{$IFNDEF DELPHI12}
  const
    UseCRLFFlag: array[Boolean] of DWORD = (GT_DEFAULT, GT_USECRLF);
    CodePageMap: array[Boolean] of UINT = (CP_ACP, 1200);
    TextInfoFlagsMap: array[Boolean] of DWORD = (GTL_NUMBYTES, GTL_NUMCHARS);
{$ENDIF}
...
begin
{$IFNDEF DELPHI12}
  if FIsPrinting and (RichVersion >= 200) and HandleAllocated then
  begin
    ZeroMemory(@ATextInfo, SizeOf(ATextInfo));
    ATextInfo.codepage := CodePageMap[IsWindowUnicode(Handle)];
    ATextInfo.flags := TextInfoFlagsMap[IsWindowUnicode(Handle)];
...
{$IFNDEF DELPHI12}
  ...
  begin
    ZeroMemory(@ATextInfo, SizeOf(ATextInfo));
    ATextInfo.codepage := CodePageMap[IsWindowUnicode(Handle)];
    ATextInfo.flags := GTL_PRECISE or TextInfoFlagsMap[IsWindowUnicode(Handle)];
...
//  inherited;
  ...
  cxDrawRichEdit(Message.WParam, ClientRect, Handle, ACharIndex, -1, False, AHeight);
end;
...
begin
  ...
  begin
    ...
    try
      ...
      cxDrawRichEdit(ADC, Rect(0, 0, AWidth, 0), VarToStr(AEditValue),
...
begin
  P := GetMouseCursorPos;
  PostMessage(FEdit.Container.Handle, WM_CONTEXTMENU, FEdit.Handle, dxPointToLParam(P));
  Result := S_OK;
end;
...
begin
  if (MemoMode or not PlainText) and dxIsRichText(VarToStr(AEditValue)) then
...
procedure TcxCustomRichEditProperties.SetZoomFactor(const Value: Double);
begin
  if (ZoomFactor <> Value) and (Value > 1 / 64) and (Value < 64) then
  begin
    FZoomFactor := Value;
    Changed;
  end;
...
begin
  ...
  begin
    ...
    AdvancedTypography := ActiveProperties.AdvancedTypography;
    ...
    ZoomFactor := ActiveProperties.ZoomFactor;
...
begin
  inherited;
  InnerRich.DefAttributes.Assign(Font);
end;

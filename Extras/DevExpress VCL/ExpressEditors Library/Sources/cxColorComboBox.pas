...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Variants, Windows, Messages, TypInfo, SysUtils, Classes, Controls, Graphics,
  StdCtrls, Forms, Dialogs, Types, cxClasses, cxControls, cxContainer, cxGraphics,
  cxDataStorage, cxDataUtils, cxVariants, dxCoreClasses,
...
uses
  dxCore, cxGeometry, cxExtEditUtils, dxThemeConsts, dxThemeManager, dxUxTheme;
...
begin
  ...
  DrawColorBox(ACanvas, ColorBoxRect, ColorBoxFrameColor, ColorBoxColor,
    BkColor, ARealDefaultColorStyle);
...
begin
  ...
  if IsRectEmpty(ABounds) or (ABounds.Right = MaxInt) or (ABounds.Bottom = MaxInt) then
    Exit;
...
begin
  ...
  Result.cx := Result.cx + ActiveProperties.ColorBoxWidth;
...
begin
  if AProperties is TcxCustomColorComboBoxProperties then
    ...
    begin
      ...
      CustomColors.Assign(Self.CustomColors);
...
begin
  ...
  if ((AFoundIndex <> -1) and (not ANeedDescription or ShowDescriptions) or
    ((AFoundIndex = -1) and (ColorComboStyle = cxccsComboList))) and
...
procedure TcxCustomColorComboBoxProperties.InternalGetColorComboBoxDisplayValue(
  ...
  out ADescription: string; out AColorFound: Boolean);
begin
  AColorFound := not IsVarEmpty(AEditValue);
...
begin
  Result := inherited CanSelectItem(AFindSelection) or
    (ItemIndex <> -1);
end;
...
function TcxCustomColorComboBox.DoOnSelectCustomColor(var AColor: TColor; out AColorDescription: string; ANeedFireEvent: Boolean): Boolean;
begin
...
begin
  ...
  AKey := DoSelectCustomColor(AColorProvided, ActiveProperties.ColorDialogType = cxcdtCustom);
...
begin
  ...
  if (APrevKey = VK_RETURN) and not (ssAlt in Shift) and
      (ItemIndex = -1) and ActiveProperties.StrToColor(Text, AColor) then

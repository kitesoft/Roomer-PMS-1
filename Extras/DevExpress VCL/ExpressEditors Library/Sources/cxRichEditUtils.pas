...
function cxPixelsToTwips(AValue: Integer): Integer;
...
begin
  ...
  begin
    dwMask := GetMask;
    ANewValue := cxPixelsToTwips(Value);
    case Index of
      0:
        begin
...
begin
  ...
  begin
    ...
    wNumberingTab := cxPixelsToTwips(Value);
  end;
  SetAttributes(AParagraph);
end;
...
begin
  ...
  with AParagraph do
  begin
    rgxTabs[Index] := cxPixelsToTwips(AWidth) or Ord(AAlignment) shl 24 or Ord(ALeader) shr 28;
    dwMask := PFM_TABSTOPS;
...
begin
  ...
  begin
    dwMask := GetMask;
    case Index of
      0: dySpaceBefore := cxPixelsToTwips(Value);
      1: dySpaceAfter := cxPixelsToTwips(Value);
    else
      dyLineSpacing := cxPixelsToTwips(Value);
    end;
  end;
  SetAttributes(AParagraph);
end;
...
begin
  ...
  AFormat.yOffset := cxPixelsToTwips(Value);
  SetAttributes(AFormat);
end;
...
begin
  ...
  AFormat.yHeight := cxPixelsToTwips(Value);
  SetAttributes(AFormat);
end;
...
procedure TcxRichEditTableParams.InsertTable(ARichEdit: TcxCustomEdit; AColumnCount, ARowCount: Integer);
  ...
  begin
    ...
    ACell.dxWidth := cxPixelsToTwips(AParams.Width);
    ACell.dxBrdrLeft := cxPixelsToTwips(AParams.BorderWidth.Left);
    ACell.dyBrdrTop := cxPixelsToTwips(AParams.BorderWidth.Top);
    ACell.dxBrdrRight := cxPixelsToTwips(AParams.BorderWidth.Right);
    ACell.dyBrdrBottom := cxPixelsToTwips(AParams.BorderWidth.Bottom);
...
begin
  ...
  try
    ...
    PcxRichEditTableRowParams(ARows).dxCellMargin := cxPixelsToTwips(CellMargins);
    PcxRichEditTableRowParams(ARows).dyHeight := cxPixelsToTwips(RowHeight);
    PcxRichEditTableRowParams(ARows).dxIndent := cxPixelsToTwips(RowIndent);

...
uses
  ...
  dxCore, cxContainer, cxControls, cxGraphics, cxDataUtils,
...
begin
  DrawCustomEdit(ACanvas, Self, False);
  DrawLabel(ACanvas);
end;
...
procedure TcxCustomLabelViewInfo.DrawLabel(ACanvas: TcxCanvas);
  ...
  function GetTextDrawingParameters(ACanvas: TCanvas; ADrawingRect: TRect;
    ...
    begin
      ...
      begin
        ...
        ARealAngle := (Angle mod 360 + 360) mod 360;
        ARadian := ARealAngle * PI / 180;

        AWidth := Abs(ATextSize.cx * Cos(ARadian)) + Abs(ATextSize.cy * Sin(ARadian)) + DepthDeltaSize.cx;
        AHeight := Abs(ATextSize.cx * Sin(ARadian)) + Abs(ATextSize.cy * Cos(ARadian)) + DepthDeltaSize.cy;

        if ARealAngle >= 180 then
        begin
          AOffsetX := Abs(ATextSize.cy * Sin(ARadian));
          AOffsetY := Abs(ATextSize.cy * Cos(ARadian));
          X := IfThen(ARealAngle >= 270, AOffsetX, AWidth);
          Y := IfThen(ARealAngle >= 270, 0, AOffsetY);
        end
        else
        begin
          AOffsetX := Abs(ATextSize.cx * Cos(ARadian));
          AOffsetY := Abs(ATextSize.cx * Sin(ARadian));
        ...
        case Alignment.Horz of
          ...
          Result.X := Round((ADrawingRect.Left + ADrawingRect.Right - (ATextSize.cx * Cos(ARadian) + ATextSize.cy * Sin(ARadian))) / 2);
        end;

        case Alignment.Vert of
          taTopJustify:
            Result.Y := Round(ADrawingRect.Top + Y);
          taBottomJustify:
            Result.Y := Round(ADrawingRect.Bottom - AHeight + Y);
        else
          Result.Y := Round((ADrawingRect.Top + ADrawingRect.Bottom + (ATextSize.cx * Sin(ARadian) - ATextSize.cy * Cos(ARadian))) / 2);
        end;
      end;
    end;
  ...
  begin
    ...
    begin
      ...
      ATextFlags := PrepareTextFlag(Alignment.Horz, TcxAlignmentVert(Ord(Alignment.Vert)),
  ...
  begin
    ...
    begin
      ...
      try
        ...
        Painter.DrawLabelLine(ACanvas, ALineRect,
          LineOuterColor, LineInnerColor, LineOrientation = orVertical);
      finally
  ...
  begin
    Result := not cxPointIsEqual(FInplaceOffset, Point(Left, Top)) or
              not cxPointIsEqual(FWindowOrg, ACanvas.WindowOrg);
    if Result then
    begin
  ...
  function AdjustTextRectWithOffset(const ATextRect: TRect): TRect;
  begin
    if IsInplace then
      Result := cxRectContent(ATextRect, DrawTextOffsets)
    else
      Result := ATextRect;
  end;
...
begin
  ...
  try
    ...
    ATextRect := AdjustTextRectWithOffset(LabelTextRect);
    ...
    begin
      ...
      begin
        ...
        InternalDrawLabel(DrawBitmap.cxCanvas, DrawBitmap.ClientRect,
          ATextRect, ALineRect, AContentRect.TopLeft);

        FIsDrawBitmapDirty := not ACanvas.RectFullyVisible(AContentRect);
      end;
      cxBitBlt(ACanvas.Handle, DrawBitmap.Canvas.Handle, AContentRect,
        cxNullPoint, SRCCOPY);
     end;
  finally
...
begin
  ...
  ATextFlags := PrepareTextFlag(taLeftJustify, vaTop, False,
...
begin
  ...
  try
    ...
    Result := CalculateLabelMinSize(ACanvas, GetEditDisplayText,
      Rect(0, 0, Result.cx, Result.cy), ARealAngle, Properties.CalculateDepthDelta,
...
procedure TcxCustomLabelProperties.ChangeHandler(Sender: TObject);
begin
  Changed;
end;

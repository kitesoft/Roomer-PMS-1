...
type
  ...
  protected
    ...
    procedure InternalGetImageComboBoxDisplayValue(AItem: TcxImageComboBoxItem;
      out AText: string; out AImageIndex: TcxImageIndex;
  ...
  public
    ...
    property DefaultImageIndex: TcxImageIndex
...
begin
  ...
  begin
    ...
    begin
      if Properties.MultiLineText then
        AFlags := DrawTextBiDiModeFlags(DT_LEFT or DT_EXPANDTABS or DT_NOPREFIX or DT_WORDBREAK)
      else
      ...
      begin
        AImageRect := GetImageRect(AImages, Rect);
        AImages.Draw(Canvas.Canvas, AImageRect.Left, AImageRect.Top, AImageIndex, Enabled);
      end;
      if not IsRectEmpty(Rect) then
        cxDrawText(Canvas, AData, Rect, AFlags);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    H := Max(cxTextHeight(Canvas.Handle) + 2, H);
  Height := H;
...
begin
  ...
  begin
    ...
    if IsDescriptionsAssigned or Properties.ShowDescriptions then
      if Properties.ImageAlign = iaLeft then
      begin
        Result.Right := Result.Left + AImages.Width + 4;
        R.Left := Result.Right;
      end
      else
      begin
        Result.Left := Result.Right - (AImages.Width + 4);
        R.Right := Result.Left;
      end;
    Result := cxRectCenter(Result, cxSize(AImages.Width, AImages.Height));
  end
  else
    Result := cxEmptyRect;
end;
...
var
  AImages: TCustomImageList;
  I, AImageAreaWidth: Integer;
begin
  AImages := GetImages;
  if AImages <> nil then
    AImageAreaWidth := AImages.Width + 8
  else
    AImageAreaWidth := 4;
  Result := 0;
  for I := 0 to Properties.Items.Count - 1 do
    Result := Max(Result, Canvas.TextWidth(Properties.Items[I].Description));
  Inc(Result, AImageAreaWidth);
...
begin
  ...
  begin
    ...
    try
      ...
      if Transparent or IsWinVistaOrLater and NativeStyle then

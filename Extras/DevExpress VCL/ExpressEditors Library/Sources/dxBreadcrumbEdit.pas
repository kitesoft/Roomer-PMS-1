...
type
  ...
  TdxBreadcrumbEditChange = (bcecAnimation, bcecButtons,
    bcecLayout, bcecNodes, bcecPathEditor, bcecProgress, bcecSelection);
  ...
  published
    ...
    property AnimationRestartDelay: Cardinal read FAnimationRestartDelay
      write SetAnimationRestartDelay default dxBreadcrumbEditDefaultAnimationRestartDelay;
    property AnimationSpeed: Cardinal read FAnimationSpeed
      write SetAnimationSpeed default dxBreadcrumbEditDefaultAnimationSpeed;
  ...
  public
    ...
    property ViewItemsCount: Integer read GetViewItemCount;
  end;
  ...
  TdxBreadcrumbEditProgressBarController = class(TObject)
  private
  ...
  TdxBreadcrumbEditViewInfo = class(TObject)
  private
  ...
  { TdxBreadcrumbEditAutoCompleteInnerListBoxSizeGrip }

  TdxBreadcrumbEditAutoCompleteInnerListBoxSizeGrip = class(TcxSizeGrip)
  protected
    procedure Paint; override;
    procedure WMNCHitTest(var Message: TWMNCHitTest); message WM_NCHITTEST;
  end;

  { TdxBreadcrumbEditAutoCompleteInnerListBox }

  TdxBreadcrumbEditAutoCompleteInnerListBox = class(TdxCustomListBox)
  private
  ...
  TdxBreadcrumbEditAutoCompleteWindow = class(TcxCustomPopupWindow)
  private
    FDisplayRowsCount: Integer;
    FInnerListBox: TdxBreadcrumbEditAutoCompleteInnerListBox;
    FLookAndFeel: TcxLookAndFeel;
    FOwnerEditor: TdxBreadcrumbEditPathEditor;
  ...
  protected
    ...
    function CreateInnerListBox: TdxBreadcrumbEditAutoCompleteInnerListBox; virtual;
    ...
    procedure SynchronizeSuggestions;
    //
    property InnerListBox: TdxBreadcrumbEditAutoCompleteInnerListBox read FInnerListBox;
    property OwnerEditor: TdxBreadcrumbEditPathEditor read FOwnerEditor;
  ...
  private
    ...
    procedure DoGetDefaultButtonWidthEvent(Sender: TcxCustomEditViewData;
  ...
  TdxBreadcrumbEditPathEditingController = class(TObject)
  private
    FController: TdxBreadcrumbEditController;
  ...
  protected
    ...
    function FindRootNodeForPath(var APath: string; out ANode: TdxBreadcrumbEditNode): Boolean;
...
uses
  Math, StdCtrls, cxGeometry, cxEditConsts, Variants, StrUtils;
...
function cxImageListSize(AImageList: TCustomImageList): TSize;
begin
  Result := cxSize(AImageList.Width, AImageList.Height);
end;
...
procedure TdxBreadcrumbEditNode.ReadData(AStream: TStream);
begin
  Name := dxReadStr(AStream, dxIsUnicodeStream(AStream));
...
begin
  if Parent <> nil then
    Result := Parent.Path + PathDelim + Name
  else
    Result := Name;
end;
...
begin
  Painter.DrawBreadcrumbEditButton(ACanvas, Bounds, State, IsFirst, IsLast);
end;
...
begin
  cxDrawImage(ACanvas.Handle,
    ...
    Button.ImageIndex, EnabledImageDrawModeMap[GetIsEnabled]);
end;

function TdxBreadcrumbEditButtonViewItem.GetContentOffsets: TRect;
begin
  Result := Painter.BreadcrumbEditButtonContentOffsets(IsFirst, IsLast);
end;
...
begin
  if IsGlyphAssigned(Button.Glyph) then
    Result := cxSize(Button.Glyph.Width, Button.Glyph.Height)
  else
    if IsImageAssigned(Owner.Properties.ButtonImages, Button.ImageIndex) then
      Result := cxImageListSize(Owner.Properties.ButtonImages)
    else
      Result := cxSize(dxBreadcrumbEditDefaultImageSize, dxBreadcrumbEditDefaultImageSize);
end;
...
begin
  ...
  begin
    if not ANode.IsHidden then
      AItems.Add(ANode.Name, ANode.ImageIndex, ANode);
    ANode := ANode.Next;
  end;
end;
...
begin
  Result := FTextSize.cy + cxMarginsHeight(Painter.BreadcrumbEditNodeTextOffsets);
end;
...
begin
  FTextSize := cxTextExtent(Font, Node.Name);
  FMaxWidth := DelimiterSize + Max(Painter.BreadcrumbEditNodeDelimiterSize,
    FTextSize.cx + cxMarginsWidth(Painter.BreadcrumbEditNodeTextOffsets));
  FMinWidth := Min(dxBreadcrumbEditMinReducedNodeWidth, MaxWidth);
  FWidth := MaxWidth;
end;
...
begin
  if cxRectIsEmpty(ButtonRect) then
    Painter.DrawBreadcrumbEditNode(ACanvas, DelimiterRect, DelimiterState, False)
  else
    if cxRectIsEmpty(DelimiterRect) then
      Painter.DrawBreadcrumbEditNode(ACanvas, ButtonRect, State, False)
    else
    begin
      Painter.DrawBreadcrumbEditNode(ACanvas, ButtonRect, State, True);
      Painter.DrawBreadcrumbEditNodeDelimiter(ACanvas, DelimiterRect, DelimiterState);
    end;
end;
...
begin
  ...
  R := cxRectContent(ButtonRect, Painter.BreadcrumbEditNodeTextOffsets);
  if not cxRectIsEmpty(R) then
  begin
    ACanvas.Font := Font;
    ACanvas.Font.Color := GetTextColor;
    ACanvas.DrawTexT(Node.Name, R, AlignMap[cxRectWidth(R) > FTextSize.cx], vaCenter, False, True);
  end;
end;

procedure TdxBreadcrumbEditNodeViewItem.DrawDelimiterGlyph(ACanvas: TcxCanvas);
begin
  Painter.DrawBreadcrumbEditNodeDelimiterGlyph(ACanvas, DelimiterRect, DelimiterState);
end;
...
begin
  if Node.HasChildren then
    Result := Painter.BreadcrumbEditNodeDelimiterSize
  else
    Result := 0;
end;
...
begin
  ...
  AItems.Add(Node.Name, Node.ImageIndex, Node);
...
begin
  ...
  if not IsLastNode then
    FMaxWidth := Painter.BreadcrumbEditNodeDelimiterSize;
  FMinWidth := MaxWidth;
  FWidth := MaxWidth;
end;
...
begin
  if IsNextNodeHidden then
    Painter.DrawBreadcrumbEditNodeMoreButtonGlyph(ACanvas, DelimiterRect, DelimiterState)
  else
...
begin
  Result := Index + 1 = Owner.ViewItemsCount;
end;
...
procedure TdxCustomBreadcrumbEditProperties.Notification(
...
procedure TdxCustomBreadcrumbEditProperties.PropertiesChanged(
...
procedure TdxCustomBreadcrumbEditProperties.SetPathEditor(
  AValue: TdxBreadcrumbEditPathEditorProperties);
begin
  FPathEditor.Assign(AValue)
end;

procedure TdxCustomBreadcrumbEditProperties.SetProgressBar(
  AValue: TdxBreadcrumbEditProgressBarProperties);
begin
...
begin
  ...
  begin
    ...
    if AInfo.ViewItem <> nil then Break;
  end;
end;
...
procedure TdxBreadcrumbEditButtonsPartViewInfo.Calculate(R: TRect);
  ...
  begin
    if AButtonViewItem.Button.Width > 0 then
      Result := cxGetValueCurrentDPI(AButtonViewItem.Button.Width)
    else
...
begin
  FBounds := R;
  for I := ViewItemsCount - 1 downto 0 do
  begin
    AButtonViewItem := TdxBreadcrumbEditButtonViewItem(ViewItems[I]);
    AButtonViewItem.Calculate(cxRectSetRight(R, R.Right, GetButtonWidth(AButtonViewItem)));
    R.Right := AButtonViewItem.Bounds.Left;
  end;

  if ViewItemsCount > 0 then
  begin
    FSeparatorRect := cxRectSetRight(R, R.Right, Painter.BreadcrumbEditButtonAreaSeparatorSize);
    R.Right := SeparatorRect.Left;
  end
  else
...
begin
  ...
  Painter.DrawBreadcrumbEditButtonAreaSeparator(ACanvas, SeparatorRect, Owner.State);
end;
...
begin
  Result := dxBreadcrumbEditDefaultImageSize +
    cxMarginsHeight(Painter.BreadcrumbEditButtonContentOffsets(True, True));
end;
...
begin
  Painter.DrawBreadcrumbEditDropDownButton(ACanvas, Bounds, State, False);
end;

procedure TdxBreadcrumbEditDropDownButtonViewItem.DrawContent(ACanvas: TcxCanvas);
begin
  Painter.DrawBreadcrumbEditDropDownButtonGlyph(ACanvas, Bounds, State, False);
end;
...
var
  AWidth: Integer;
begin
  if Properties.PathEditor.Enabled then
    AWidth := GetDropDownButtonWidth
  else
    AWidth := 0;

  DropDownButton.Calculate(cxRectSetRight(R, R.Right, AWidth));
...
begin
  ...
  begin
    AImageAreaSize := Properties.Images.Width + 2 * dxBreadcrumbEditImageBorder;
    FImageRect := cxRectCenter(cxRectSetWidth(R, AImageAreaSize), cxImageListSize(Properties.Images));
    Inc(R.Left, AImageAreaSize);
  end;
end;
...
begin
  ...
  begin
    ...
    ANodeViewItem.Calculate(cxRectSetWidth(R, ANodeViewItem.Width));
    R.Left := ANodeViewItem.Bounds.Right;
  end;
end;
...
var
  AIndex: Integer;
begin
  for AIndex := 0 to ViewItemsCount - 1 do
    ViewItems[AIndex].CalculateSizes;

  AIndex := 0;
  while (GetContentWidth > AContentWidth) and (AIndex + dxBreadcrumbEditMinVisibleNodesCount < ViewItemsCount) do
  begin
    HideNodeViewItem(ViewItems[AIndex]);
    Inc(AIndex);
  end;
  while (GetContentWidth > AContentWidth) and (AIndex < ViewItemsCount) do
  begin
...
function TdxBreadcrumbEditNodesAreaPartViewInfo.IsViewItemValid(
  AItem: TdxBreadcrumbEditViewItem): Boolean;
begin
...
procedure TdxBreadcrumbEditNodesAreaPartViewInfo.UpdateViewItemsState(
  AInfo: TdxBreadcrumbEditHitTestInfo);
begin
...
begin
  Result := Painter.BreadcrumbEditDropDownButtonWidth;
end;
...
begin
  if Properties.Images <> nil then
    Result := Properties.Images.Height
  else
    Result := dxBreadcrumbEditDefaultImageSize;
end;
...
begin
  if ViewItemsCount > 0 then
    Result := ViewItems[ViewItemsCount - 1]
  else
    Result := nil;
end;
...
begin
  FBounds := cxRectContent(R, Painter.BreadcrumbEditProgressChunkPadding);
  FOverlaySize := Painter.BreadcrumbEditProgressChunkOverlaySize;
  ResetOverlayPosition;
end;

procedure TdxBreadcrumbEditProgressBarPartViewInfo.DrawContent(ACanvas: TcxCanvas);
begin
  if not IsRectEmpty(ChunkRect) then
  begin
    ACanvas.SaveClipRegion;
    try
      ACanvas.IntersectClipRect(ChunkRect);
      Painter.DrawBreadcrumbEditProgressChunk(ACanvas, ChunkRect);
      Painter.DrawBreadcrumbEditProgressChunkOverlay(ACanvas, OverlayRect);
    finally
      ACanvas.RestoreClipRegion;
    end;
  end;
...
var
  I: Integer;
begin
  for I := 0 to NodesAreaViewInfo.ViewItemsCount - 1 do
  begin
...
begin
  ...
  if not (bBottom in Properties.Borders) then Result.Bottom := 0;
  if not (bRight in Properties.Borders) then Result.Right := 0;
  if not (bLeft in Properties.Borders) then Result.Left := 0;
  if not (bTop in Properties.Borders) then Result.Top := 0;
end;
...
procedure TdxBreadcrumbEditAutoCompleteInnerListBox.DoLayoutChanged;
begin
  ItemHeight := CalculateItemHeight;
  inherited DoLayoutChanged;
end;

procedure TdxBreadcrumbEditAutoCompleteInnerListBox.DoSelectItem;
begin
  if Assigned(OnSelectItem) then OnSelectItem(Self);
end;

procedure TdxBreadcrumbEditAutoCompleteInnerListBox.InitScrollBarsParameters;
begin
...
function TdxBreadcrumbEditAutoCompleteInnerListBox.IsSizeGripVisible: Boolean;
begin
  Result := True;
end;

procedure TdxBreadcrumbEditAutoCompleteInnerListBox.MouseDown(
...
procedure TdxBreadcrumbEditAutoCompleteInnerListBox.MouseMove(Shift: TShiftState; X, Y: Integer);
begin
...
function TdxBreadcrumbEditAutoCompleteInnerListBox.PointInSizeGrip(const P: TPoint): Boolean;
begin
  Result := ControlAtPos(P, False, True) = SizeGrip;
end;

function TdxBreadcrumbEditAutoCompleteInnerListBox.ProcessNavigationKey(
...
function TdxBreadcrumbEditAutoCompleteInnerListBox.GetSizeGripBounds: TRect;
begin
  Result.Left := VScrollBar.Left;
  Result.Right := Result.Left + VScrollBar.Width;
  Result.Bottom := ClientBounds.Bottom;
  Result.Top := Result.Bottom - cxRectWidth(Result);
end;

function TdxBreadcrumbEditAutoCompleteInnerListBox.GetSizeGripClass: TcxSizeGripClass;
begin
  Result := TdxBreadcrumbEditAutoCompleteInnerListBoxSizeGrip;
end;

function TdxBreadcrumbEditAutoCompleteInnerListBox.GetVScrollBarBounds: TRect;
begin
  Result := ClientBounds;
  Result.Left := Result.Right;
  Result.Right := Result.Left + VScrollBar.Width;
  Result.Bottom := GetSizeGripBounds.Top;
end;

procedure TdxBreadcrumbEditAutoCompleteInnerListBox.WMNCHitTest(var Message: TWMNCHitTest);
begin
...
procedure TdxBreadcrumbEditAutoCompleteInnerListBoxSizeGrip.Paint;
begin
  inherited Paint;
  cxPaintCanvas.BeginPaint(Canvas);
  try
    LookAndFeel.Painter.DrawSizeGrip(cxPaintCanvas, ClientRect);
  finally
    cxPaintCanvas.EndPaint;
  end;
end;

procedure TdxBreadcrumbEditAutoCompleteInnerListBoxSizeGrip.WMNCHitTest(var Message: TWMNCHitTest);
begin
  Message.Result := HTTRANSPARENT;
end;
...
constructor TdxBreadcrumbEditAutoCompleteWindow.Create(AOwnerControl: TWinControl);
begin
  inherited Create(AOwnerControl);
  OwnerParent := AOwnerControl;
  FDisplayRowsCount := dxBreadcrumbEditDefaultAutoCompleteDropDownRows;
  FLookAndFeel := TcxLookAndFeel.Create(Self);
  FLookAndFeel.OnChanged := LookAndFeelChanged;
  FOwnerEditor := AOwnerControl as TdxBreadcrumbEditPathEditor;
...
destructor TdxBreadcrumbEditAutoCompleteWindow.Destroy;
begin
...
procedure TdxBreadcrumbEditAutoCompleteWindow.AdjustClientRect(var Rect: TRect);
begin
  InflateRect(Rect,
    -dxBreadcrumbEditSuggestionsWindowBorderSize,
    -dxBreadcrumbEditSuggestionsWindowBorderSize);
end;

procedure TdxBreadcrumbEditAutoCompleteWindow.CalculateConstaints;
begin
  Constraints.MinWidth := 2 * dxBreadcrumbEditSuggestionsWindowBorderSize + GetSystemMetrics(SM_CXVSCROLL);
  Constraints.MinHeight := Constraints.MinWidth;
end;

procedure TdxBreadcrumbEditAutoCompleteWindow.CalculateSize;
begin
  if Adjustable then
  begin
    InnerListBox.LayoutChanged;
    SetSize(OwnerEditor.InnerControl.Width,
      InnerListBox.CalculateContentSize(DisplayRowsCount).cy +
      2 * dxBreadcrumbEditSuggestionsWindowBorderSize);
  end;
end;

function TdxBreadcrumbEditAutoCompleteWindow.CreateInnerListBox: TdxBreadcrumbEditAutoCompleteInnerListBox;
begin
  Result := TdxBreadcrumbEditAutoCompleteInnerListBox.Create(nil);
end;

procedure TdxBreadcrumbEditAutoCompleteWindow.CreateParams(var Params: TCreateParams);
begin
...
begin
  inherited DoClosed;
  PostMessage(OwnerEditor.Handle, DXM_BREADCRUMBEDIT_HIDESUGGESTIONS, 0, 0);
end;

procedure TdxBreadcrumbEditAutoCompleteWindow.LookAndFeelChanged(
  Sender: TcxLookAndFeel; AChangedValues: TcxLookAndFeelValues);
begin
  Invalidate;
end;

procedure TdxBreadcrumbEditAutoCompleteWindow.Paint;
begin
  Canvas.FrameRect(ClientRect, LookAndFeel.Painter.GetContainerBorderColor(False));
end;

procedure TdxBreadcrumbEditAutoCompleteWindow.ProcessNavigationKey(var Key: Word; Shift: TShiftState);
begin
  InnerListBox.ProcessNavigationKey(Key, Shift);
end;

procedure TdxBreadcrumbEditAutoCompleteWindow.Popup(AFocusedControl: TWinControl);
var
  P: TPoint;
begin
  CalculateConstaints;
...
procedure TdxBreadcrumbEditAutoCompleteWindow.SelectItemHandler(Sender: TObject);
begin
  OwnerEditor.EditValue := SelectedPath;
  OwnerEditor.PostEditValue;
end;

procedure TdxBreadcrumbEditAutoCompleteWindow.SynchronizeSuggestions;
var
  I: Integer;
begin
  FPreviouslyEnteredText := OwnerEditor.EditingValue;
  InnerListBox.BeginUpdate;
  try
    InnerListBox.Clear;
    for I := 0 to OwnerEditor.AutoCompleteSuggestions.Count - 1 do
      InnerListBox.Add(OwnerEditor.AutoCompleteSuggestions[I]);
    SelectedPath := PreviouslyEnteredText;
  finally
    InnerListBox.EndUpdate;
  end;
end;

procedure TdxBreadcrumbEditAutoCompleteWindow.WMExitSizeMove(var Message: TMessage);
begin
  inherited;
  OwnerEditor.StoreAutoCompleteWindowCustomizedSize;
end;

procedure TdxBreadcrumbEditAutoCompleteWindow.WMNCHitTest(var Message: TWMNCHitTest);
begin
...
function TdxBreadcrumbEditAutoCompleteWindow.GetSelectedPath: string;
begin
...
procedure TdxBreadcrumbEditAutoCompleteWindow.SetSelectedPath(const AValue: string);
begin
  InnerListBox.ItemIndex := InnerListBox.Items.IndexOfCaption(AValue, False);
end;

procedure TdxBreadcrumbEditAutoCompleteWindow.SetSize(AWidth, AHeight: Integer);
begin
  SetBounds(Left, Top, AWidth, AHeight);
end;

procedure TdxBreadcrumbEditAutoCompleteWindow.SetSize(const ASize: TSize);
begin
  ...
  case Key of
    ...
    VK_UP, VK_DOWN:
      begin
        ...
        begin
          AutoCompleteWindow.ProcessNavigationKey(Key, Shift);
          PreviewEditValue(AutoCompleteWindow.SelectedPath, False);
        end
        else
          if DroppedDown then
          begin
...
begin
  ADefaultWidth := Style.LookAndFeel.Painter.BreadcrumbEditDropDownButtonWidth;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FAutoCompleteWindow.InnerListBox.Font := Style.Font;
      FAutoCompleteWindow.LookAndFeel.MasterLookAndFeel := Style.LookAndFeel;
    end;
    AutoCompleteWindow.DisplayRowsCount := Properties.AutoCompleteDropDownRows;
    ...
    AutoCompleteWindow.SynchronizeSuggestions;
...
procedure TdxBreadcrumbEditPathEditorViewInfo.DrawEditButton(
  ACanvas: TcxCanvas; AButtonVisibleIndex: Integer);
const
...
begin
  ...
  Painter.DrawBreadcrumbEditDropDownButton(ACanvas,
    AButtonInfo.Bounds, StatesMap[AButtonInfo.Data.State], True);
  Painter.DrawBreadcrumbEditDropDownButtonGlyph(ACanvas,
    AButtonInfo.Bounds, StatesMap[AButtonInfo.Data.State], True);
end;

procedure TdxBreadcrumbEditPathEditorViewInfo.GetColorSettingsByPainter(
  out ABackgroundColor, ATextColor: TColor);
begin
...
begin
  ...
  begin
    AImageAreaSize := AViewInfo.Images.Width + 2 * dxBreadcrumbEditImageBorder;
    AViewInfo.ImageRect := cxRectSetWidth(AViewInfo.ClientRect, AImageAreaSize);
    AViewInfo.ImageRect := cxRectCenter(AViewInfo.ImageRect, cxImageListSize(AViewInfo.Images));
    Inc(AViewInfo.ClientRect.Left, AImageAreaSize);
    Inc(AViewInfo.TextRect.Left, AImageAreaSize);
  end;
end;

procedure TdxBreadcrumbEditPathEditorViewData.CalculateButtonNativeInfo(
...
begin
  ...
  begin
    MinContentSize.cy := Max(MinContentSize.cy, APathEditorViewInfo.Images.Height);
    Inc(Result.cx, APathEditorViewInfo.Images.Width + 5);
  end
end;
...
procedure TdxBreadcrumbEditPathEditorListBox.DrawItem(
...
begin
  ...
  begin
    ...
    begin
      ...
      InflateRect(Rect, -cxTextOffset, -cxTextOffset);
...
begin
  if Images <> nil then
  begin
    Result := cxRectSetWidth(R, 2 * dxBreadcrumbEditImageBorder + Images.Width);
    Result := cxRectCenter(Result, cxImageListSize(Images));
  end
  else
    Result := cxNullRect;
end;
...
begin
  ...
  if Images <> nil then
    Result := Max(Result, 2 * dxBreadcrumbEditImageBorder + Images.Height);
end;
...
begin
  if Properties.RecentPathsAutoPopulate then
  begin
    if Properties.RecentPaths.IndexOfPath(ANode.Path) < 0 then
      Properties.RecentPaths.Add(ANode.Path, ANode.ImageIndex);
  end;
end;
...
begin
  ...
  begin
    ...
    ACompletePath := Copy(APath, 1, LastDelimiter(PathDelim, APath));
...
procedure TdxBreadcrumbEditPathEditingController.PopulateCustomSuggestions(
...
function TdxBreadcrumbEditController.DoSelectPath(
  const APath: string; ANode: TdxBreadcrumbEditNode): Boolean;
begin
...
function TdxBreadcrumbEditController.FindRootNodeForPath(
  var APath: string; out ANode: TdxBreadcrumbEditNode): Boolean;
var
  ANodeName: string;
begin
...
begin
  Result := ASubPath <> '';
  APathDelimPosition := Pos(PathDelim, ASubPath);
...
begin
  Result := DoSelectPath(ANode.Path, ANode);
end;
...
procedure TdxBreadcrumbEditController.ShowNodeDropDownMenu(
  ANodeViewItem: TdxBreadcrumbEditNodeViewItem);
begin
...
procedure TdxCustomBreadcrumbEdit.CMEnabledChanged(var Message: TMessage);
begin
  inherited;
  Controller.UpdateState;
end;
...
procedure TdxCustomBreadcrumbEdit.SetTransparent(AValue: Boolean);
begin
  if AValue <> Transparent then
  begin
    FTransparent := AValue;
    if AValue then
...
procedure TdxBreadcrumbEdit.ReadData(AStream: TStream);
begin
  BeginUpdate;
  try
...
procedure TdxBreadcrumbEdit.WriteData(AStream: TStream);
begin
  BeginUpdate;
  try

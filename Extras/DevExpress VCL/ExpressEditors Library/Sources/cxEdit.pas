...
uses
  ...
  dxCore, dxCoreClasses, dxMessages, dxGDIPlusClasses,
  cxClasses, cxContainer, cxControls, cxCustomData,
  cxDataStorage, cxDataUtils, cxGraphics, cxLookAndFeels, cxVariants, dxCustomHint,
  cxLookAndFeelPainters, dxFading, dxThemeManager, dxUxTheme, cxEditPaintUtils;
...
type
  ...
  TcxEditRepository = class(TcxCustomComponent)
  private
    FItems: TList;
  ...
  TcxEditDrawButtonBorderEvent = procedure (Sender: TcxEditButtonViewInfo;
    ACanvas: TcxCanvas; out ABackgroundRect, AContentRect: TRect; var AHandled: Boolean) of object;
  ...
  TcxEditValidateInfo = class(TObject)
  private
    FErrorIcon: TdxSmartImage;
    FErrorText: string;
    FErrorType: TcxEditErrorType;

    function GetErrorIcon: TGraphic;
    procedure SetErrorType(AValue: TcxEditErrorType);
    procedure SetErrorIcon(AValue: TGraphic);
  protected
  ...
  public
    ...
    property ErrorIcon: TGraphic read GetErrorIcon write SetErrorIcon;
  ...
  private
    ...
    function DoDrawButtonBackground(ACanvas: TcxCanvas;
      const ARect: TRect; AViewInfo: TcxEditButtonViewInfo): Boolean;
    function DoDrawButtonBorder(ACanvas: TcxCanvas;
      AViewInfo: TcxEditButtonViewInfo; out ABackgroundRect, AContentRect: TRect): Boolean;
    function DoPrepareButtonFadingImage(ASender: TcxEditButtonViewInfo;
      AState: TcxEditButtonState; out AImage: TcxBitmap32): Boolean;
  ...
  protected
    ...
    procedure CalculateNativeInfo(out AThemedObjectType: TdxThemedObjectType;
      out ANativePart: Integer; out ABoundsRect: TRect);
...
//    property GradientButtons;
  ...
  TcxCustomEditContainer = class(TcxContainer, IcxTransparentControl)
  private
  {$IFDEF DELPHI14}
  ...
  TcxCustomEdit = class(TcxCustomEditContainer, IcxEditRepositoryItemListener,
    IdxSkinSupport, IdxSpellCheckerControl, IdxFadingObject, IcxEditOwner)
  private
    FAnchorX: Integer;
    FAnchorY: Integer;
  ...
  protected
    ...
    procedure InvalidateRect(const R: TRect; AEraseBackground: Boolean = True);
    ...
    property IsUserAction: Boolean read FIsUserAction write FIsUserAction;
  ...
  TcxCustomEditingController = class
  private
...
uses
  ...
  dxThemeConsts, cxGeometry, cxDWMApi, Math, cxLibraryConsts;
...
function GetEditPopupWindowControlsLookAndFeelKind(AEdit: TcxCustomEdit): TcxLookAndFeelKind;
const
  APopupControlLookAndFeelKindMap: array [TcxEditButtonStyle] of TcxLookAndFeelKind =
    (lfStandard, lfStandard, lfFlat, lfFlat, lfFlat, lfUltraFlat, lfOffice11);
begin
  with AEdit do
    if IsInplace then
...
function TcxCustomEdit.InternalValidateEdit(AValidationOptions: TcxEditValidationOptions; AForceValidation: Boolean): Boolean;
  ...
  function GetValueToValidate: Variant;
  begin
...
begin
  ...
  try
    ...
    if (FValidationError <> APrevValidationError) or FValidationError then
    ...
    if FValidationError then
      try
        ...
        if not InternalCompareString(ADisplayValue, GetValueToValidate, True) then
        begin
...
procedure TcxCustomEdit.InvalidateRect(const R: TRect; AEraseBackground: Boolean);
begin
  InternalInvalidateRect(Self, R, AEraseBackground);
end;
...
begin
  Result := ViewInfo.FadingAvailable and not IsInplace;
end;
...
procedure TcxCustomEdit.ChangeScale(M, D: Integer);
begin
...
begin
  ...
  Filer.DefineProperty('Width', ReadWidth, WriteWidth,
    not (CanAutoSize and CanAutoWidth and not CanAutoHeight) and HasWidth);
...
var
  APrevIsUserAction: Boolean;
begin
  APrevIsUserAction := IsUserAction;
  IsUserAction := False;
  try
    if Assigned(dxISpellChecker) then
      dxISpellChecker.CheckFinish;
    ...
    IsUserAction := APrevIsUserAction;
  end;
end;
...
begin
  ...
  if (Key <> 0) and Assigned(dxISpellChecker2) then
    dxISpellChecker2.KeyUp(Key, Shift);
end;
...
procedure TcxCustomEdit.DoSetSize;
  ...
  begin
    ALeft := Left;
    ATop := Top;
    if UseAnchors and IsAutoWidth and not IsAutoHeight then
    begin
...
begin
  ...
  try
    AViewData.Calculate(Canvas, GetControlRect(Self), P, Button, Shift, AViewInfo, AIsMouseEvent);
  finally
    AViewData.Free;
  end;
end;
...
begin
  ...
  with Properties do
    if Assigned(FOnButtonClick) then
      FOnButtonClick(Self, AEditButtonViewInfo.ButtonIndex);
...
begin
  ...
  if IsAutoWidth and not IsAutoHeight then
...
begin
  Result := not IsInplace or ActiveProperties.UseMouseWheel or (ssCtrl in Shift);
end;
...
begin
  ...
  if Result then
  begin
    PrepareDisplayValue(DataBinding.StoredValue, V1, False);
    PrepareDisplayValue(EditValue, V2, False);
    Result := not VarEquals(V1, V2);
  end;
end;
...
begin
  ...
  begin
    ...
    if IsAutoWidth then
      ANewWidth := ASize.cx;
    if not (Align in [alLeft, alRight, alClient, alCustom]) then
      ANewHeight := ASize.cy;
  end;
end;
...
begin
  Result := CanAutoSize and CanAutoHeight;
end;
...
begin
  Result := EditViewInfo.FadingAvailable and (EditOwner <> nil);
end;
...
begin
  Result := TcxCustomEditButtons(inherited Collection);
end;

function TcxCustomEditButton.GetGlyph: TBitmap;
begin
  if FGlyph = nil then
...
procedure TcxCustomEditButton.SetGlyph(Value: TBitmap);
begin
  if Value <> nil then
    Glyph.Assign(Value)
  else
...
function TcxEditValidateInfo.GetErrorIcon: TGraphic;
begin
  Result := FErrorIcon;
end;
...
begin
  ...
  begin
    FErrorType := AValue;
    case FErrorType of
      eetError: ErrorIcon.Assign(cxEditErrorIcon);
      eetWarning: ErrorIcon.Assign(cxEditWarningIcon);
      eetInfo: ErrorIcon.Assign(cxEditInfoIcon);
//    eetNone: ErrorImage.Clear;
    end;
  end;
end;

procedure TcxEditValidateInfo.SetErrorIcon(AValue: TGraphic);
begin
...
procedure TcxCustomEditViewInfo.CalculateNativeInfo(
  out AThemedObjectType: TdxThemedObjectType; out ANativePart: Integer; out ABoundsRect: TRect);
begin
  ABoundsRect := Bounds;
  ...
  ABoundsRect.Left := FErrorBounds.Right;
end;
...
begin
  ...
  try
    ...
    case GetDrawBackgroundStyle of
      ...
      dbsCustomEdit:
        DrawCustomEdit(ACanvas, Self, True);
      dbsCustom:
        begin
...
procedure TcxCustomEditViewInfo.DrawButtonBorder(ACanvas: TcxCanvas;
  AViewInfo: TcxEditButtonViewInfo; var ARect: TRect; out AContentRect: TRect;
  var APenColor, ABrushColor: TColor);
var
  AButtonStyle: TcxEditButtonStyle;
begin
...
procedure TcxCustomEditViewInfo.DrawButtonContent(ACanvas: TcxCanvas;
  ...
  var
    ...
    ASize: TSize;
  begin
    if IsGlyphAssigned(AViewInfo.Glyph) or IsImageAssigned(AViewInfo.Images, AViewInfo.ImageIndex) then
    begin
      ASize := dxGetImageSize(AViewInfo.Glyph, AViewInfo.Images, 0);
...
begin
  ...
  try
    ...
    if UseSkins then
    begin
      Painter.DrawEditorButtonGlyph(ACanvas, AContentRect,
        EditBtnKindToEditBtnPainterKind[AViewInfo.Data.Kind],
        EditBtnStateToButtonState[AViewInfo.Data.State],
        EditBtnPositionMap[AViewInfo.Data.LeftAlignment]);
...
begin
  ...
  begin
    CalculateNativeInfo(AThemedObjectType, ANativePart, ABorderRect);
    ATheme := OpenTheme(AThemedObjectType);
    AContentRect := GetContentRect(ATheme, ANativePart, ABorderRect);
    DrawBorder(ATheme, ANativePart, ABorderRect, AContentRect);
  end;
...
var
  ...
  AThemedObjectType: TdxThemedObjectType;
begin
  Result := FErrorData.IsError and IsImageAssigned(FErrorData.ErrorIcon);
...
var
  I: Integer;
begin
  if FErrorData.IsError and PtInRect(FErrorBounds, P) then
    Result := ecpErrorIcon
  else
...
function TcxCustomEditViewInfo.FadingAvailable: Boolean;
begin
...
begin
  Shift := Shift - [ssShift, ssAlt, ssCtrl {$IFDEF DELPHI14}, ssTouch, ssPen{$ENDIF}];
...
//  KBounds := ABounds;
  ...
  begin
    if Edit.FValidationError then
      AViewInfo.FErrorData.ErrorType := eetError
    else
      AViewInfo.FErrorData.ErrorType := eetNone;
    if IsImageAssigned(Properties.ErrorIcon) then
      AViewInfo.FErrorData.ErrorIcon.Assign(Properties.ErrorIcon);
    AViewInfo.FErrorData.ErrorText := Edit.FValidationErrorText;
  end;

  AViewInfo.FNeedShowErrorIcon := AViewInfo.FErrorData.IsError and
    (evoShowErrorIcon in Properties.ValidationOptions) and IsImageAssigned(AViewInfo.FErrorData.ErrorIcon);
...
begin
  ...
  begin
    if Style.BorderStyle in [ebsUltraFlat, ebsOffice11] then
      Result := GetContainerBorderColor(AIsHighlightBorder or
        IsDesigning and Enabled, Style.BorderStyle = ebsOffice11)
    else
...
begin
  ...
  begin
    AButtonViewInfo := AViewInfo.ButtonsInfo[I];
    if AButtonViewInfo.Data.LeftAlignment then
      Result.Left := Max(Result.Left, AButtonViewInfo.Bounds.Right - AViewInfo.Bounds.Left)
    else
      Result.Right := Max(Result.Right, AViewInfo.Bounds.Right - AButtonViewInfo.Bounds.Left);
  end;
end;
...
begin
  ...
  AViewInfo.FEditorBounds := AViewInfo.Bounds;
  ...
  begin
    ...
    AViewInfo.FErrorBounds.Right := AViewInfo.FErrorBounds.Left + AViewInfo.FErrorData.ErrorIcon.Width + 3; 
...
begin
  ...
  begin
    ...
    if not AButton.Visible then
      Continue;
    if AButton.Kind = bkGlyph then
    begin
      if IsGlyphAssigned(AButton.Glyph) then
        AMaxButtonContentHeight := Max(AMaxButtonContentHeight, AButton.Glyph.Height)
      else
        if IsImageAssigned(Properties.Images, AButton.ImageIndex) then
          AMaxButtonContentHeight := Max(AMaxButtonContentHeight, Properties.Images.Height);
    end;
    ...
    begin
      AMaxButtonContentHeight := Max(AMaxButtonContentHeight,
        ACanvas.TextHeight(AButton.VisibleCaption));
    end;
  end;
...
begin
  ...
  FEdit.Left := cxInvisibleCoordinate; //don't show
  FEdit.Parent := GetEditParent;       //here
  FEditPlaceBounds := cxEmptyRect;
  AssignOverridableEditEvents;
  UpdateEdit;
  UpdateEditValue;
  AssignStaticEditEvents;
  //make visible here to prevent flickering
  if not IsRectEmpty(FEditPlaceBounds) then
    FEdit.BoundsRect := FEditPlaceBounds; //show
end;

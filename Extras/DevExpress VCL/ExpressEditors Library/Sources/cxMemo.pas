...
uses
  ...
  dxCoreClasses, cxClasses, cxLookAndFeelPainters, cxContainer, cxControls, cxDataStorage, cxDataUtils,
  cxEdit, cxDrawTextUtils, cxGraphics, cxLookAndFeels, cxTextEdit, cxFilterControlUtils, cxGeometry;
...
uses
  Variants, Clipbrd, Math, cxEditConsts, cxEditUtils, cxScrollBar, dxMessages;
...
begin
  ...
  if AProperties is TcxCustomMemoProperties then
    with TcxCustomMemoProperties(AProperties) do
    begin
  ...
  with Edit do
    ...
    begin
      ...
      try
        ...
        Container.ChangeEventsCatcher.OnChangeEvent := False;
      end;
    end
    else
      Text := VarToStr(Value);
...
begin
  ...
  begin
    ...
    InflateRect(R, -1, 0);
    cxScreenCanvas.Font := Style.Font;
    ATextHeight := cxDrawText(cxScreenCanvas, AText, R, DT_EDITCONTROL or DT_WORDBREAK or DT_CALCRECT or DT_NOPREFIX);
...
begin
  ...
  begin
    ...
    begin
      FPrevHeight := Height;
      CalculateHeight;
      TEditingControllerAccess(EditingController).MultilineEditTextChanged;
    end;
    APrevModified := ModifiedAfterEnter;
    inherited;
    if (Message.NotifyCode = EN_CHANGE) and ModifiedAfterEnter and not APrevModified then
      TEditingControllerAccess(EditingController).StartEditAutoHeight(FPrevHeight <> CalculatedHeight);
  end
  else
    inherited;
end;

...
type
  ...
  protected
    ...
    function PrepareValue(const AValue: TcxEditValue; AEditFocused : Boolean): Variant; override;
...
procedure TcxCustomTimeEditProperties.DoPrepareDisplayValue(const AEditValue: TcxEditValue;
  ...
  begin
    if AEditFocused or FUseTimeFormatWhenUnfocused and
      ((AValue = 0) or not ShowDate or (dxDateOf(AValue) = 0)) then
        Result := FormatDateTime(cxTimeEditFormats[TimeFormat, Use24HourFormat, 0], AValue)
    else
      if (AValue = 0) or not ShowDate or (dxDateOf(AValue) = 0) then
...
begin
  if VarIsNull(AEditValue) then
    if not AEditFocused then
      ...
      ADisplayValue := TrimRight(FormatDateTime(cxTimeEditFormats[TimeFormat, FUse24HourFormat, 0], 0))
  else
...
function TcxCustomTimeEditProperties.PrepareValue(const AValue: TcxEditValue;

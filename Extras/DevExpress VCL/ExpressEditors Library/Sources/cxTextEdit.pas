...
{$ENDIF}
  ...
  cxFormats, cxGraphics, cxVariants, cxFilterControlUtils, cxEdit, cxDrawTextUtils,
  dxUxTheme;
...
uses
  Themes, FMTBcd, Types, Math,
  cxGeometry, cxLookAndFeelPainters, cxEditConsts, cxEditUtils, dxThemeConsts, dxThemeManager, cxDWMApi;
...
begin
  ...
  begin
    ...
    DrawCustomEdit(ACanvas, AViewInfo, not ARealTransparent);
    R := ClientRect;
...
begin
  if Edit.PasswordChar <> Value then
  begin
...
begin
  ...
  begin
    ...
    if (not (cxIsVCLThemesEnabled and Container.IsNativeStyle) or
        ((Container.ActiveProperties.Alignment.Horz <> taLeftJustify) and Container.ActiveProperties.UseLeftAlignmentOnEditing)) and
    ...
    try
      ...
      try
        ...
        begin
          ...
          APasswordChar := PasswordChar;
          if (Container.ActiveProperties.EchoMode = eemPassword) and (APasswordChar = #0) then
            APasswordChar := '*';
          if APasswordChar <> #0 then
            S := StringOfChar(APasswordChar, Length(S));
          case Container.ActiveProperties.Alignment.Horz of
            taLeftJustify: ALeft := 0;
            taRightJustify: ALeft := ClientWidth - TextWidth(S) - ARightMargin + ALeftMargin;
          else
            ALeft := (ClientWidth - ARightMargin - ALeftMargin - TextWidth(S)) div 2 + ALeftMargin;
          end;
          TextRect(R, ALeft, 0, S);
        end;
      finally
        ACanvas.Handle := 0;
...
begin
  ...
  begin
    ...
    TcxCustomTextEditViewInfo(AViewInfo).HasPopupWindow := (Edit <> nil) and Edit.HasPopupWindow;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if TryStrToBcd(ADisplayValue, V) then
          ADisplayValue := BcdToStrF(V, ffFixed, APrecision,
            dxFormatSettings.CurrencyDecimals);
      end
      else
        if AEditFormat = '' then
        begin
          ...
          ADisplayValue := FormatFloat(AEditFormat, AValue);
    end
    else
      if DisplayFormat <> '' then
        ADisplayValue := FormatFloat(DisplayFormat, AValue)
      else
        ADisplayValue := VarToStr(AValue);
  end;
...
begin
  ...
  if {$IFNDEF DELPHI12}not IsInplace or{$ENDIF}(AMessage.Msg <> WM_CONTEXTMENU) then
    inherited;
end;
...
begin
  ...
  if Focused then
  begin
    APrevKeyboardAction := IsUserAction;
    IsUserAction := True;
    try
      S := '';
      ...
      IsUserAction := APrevKeyboardAction;
    end;
  end
  else
  begin
...
begin
  if SelLength = 0 then
    Exit;
  APrevKeyboardAction := IsUserAction;
  IsUserAction := Focused or FIsChangeBySpellChecker;
  try
    AText := DisplayValue;
...
begin
  ...
  begin
    ...
    UseLeftAlignmentOnEditing := ActiveProperties.UseLeftAlignmentOnEditing;
    AutoSelect := ActiveProperties.AutoSelect and not IsInplace;
    CharCase := ActiveProperties.FCharCase;
    EchoMode := ActiveProperties.EchoMode;
...
begin
  ...
  Result := ActiveProperties.IsDisplayValueValid(ADisplayValue, InternalFocused) and
    not (IsUserAction and not DoEditing);
  if Result then

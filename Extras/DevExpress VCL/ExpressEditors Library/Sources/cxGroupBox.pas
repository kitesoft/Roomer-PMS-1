...
procedure TcxGroupBoxViewInfo.CalculateRectsForStandardPainter(ACanvas: TcxCanvas; AShadowWidth: Integer);
  ...
  begin
    case Edit.FAlignment of
      ...
      alTopRight, alBottomRight:
        OffsetRect(R, Edit.Width - R.Right - R.Left - AShadowWidth - cxCaptionRectLeftBound, 0);
      alTopLeft, alBottomLeft:
        OffsetRect(R, cxCaptionRectLeftBound, 0);
    end;
    case Edit.FAlignment of
      alCenterCenter:
  ...
  begin
    ...
    case Edit.FAlignment of
      alLeftTop, alRightTop: OffsetRect(R, 0, cxCaptionRectLeftBound);
      alLeftBottom, alRightBottom: OffsetRect(R, 0, - cxCaptionRectLeftBound);
    end;
  end;
...
begin
  ...
  FCheckBoxRect := GetCheckBoxRect(ACanvas);
  ACombinedRect := GetCombinedCaptionRect;
  ACaptionRectIndent := GetCaptionRectIndent;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        Painter.DrawGroupBoxCaption(ACanvas, ACaptionRect, TextRect, ACaptionPos);
      end;
    end;
...
begin
  ...
  begin
    if Edit.FAlignment in [alTopLeft, alTopCenter, alTopRight, alCenterCenter, alBottomLeft, alBottomCenter, alBottomRight] then
    begin
      ...
      TextRect.Left := FCheckBoxRect.Right + cxCheckBoxCaptionIndent;
    ...
    begin
      ...
      if Edit.FAlignment in [alRightTop, alRightCenter, alRightBottom] then
      begin
        FCheckBoxRect := cxRectSetTop(FCheckBoxRect, ACombinedRect.Top);
        TextRect := cxRectSetTop(TextRect, FCheckBoxRect.Bottom + cxCheckBoxCaptionIndent);
      end
      else
      begin
        FCheckBoxRect := cxRectSetBottom(FCheckBoxRect, ACombinedRect.Bottom);
        TextRect := cxRectSetBottom(TextRect, FCheckBoxRect.Top - cxCheckBoxCaptionIndent);
      end;
    end;
  end;
end;
...
begin
  ...
  if CheckBoxVisible then
  begin
    ACheckBoxSize := GetEditCheckSize(ACanvas, NativeStyle, nil, 0, Painter);
...
begin
  if CheckBoxVisible then
    DrawEditCheck(ACanvas, FCheckBoxRect, FCheckBoxCheckState, FCheckBoxViewState, nil, 0,
      Edit.Style.BorderStyle, NativeStyle, BorderColor, BackgroundColor, DrawBackground(ACanvas),
      IsDesigning, Focused, True, GetPainter);
...
begin
  cxDrawText(ACanvas, Edit.Caption, TextRect, ETO_CLIPPED, clDefault,
  ...
  begin
    ...
    begin
      if IsPanelStyle then
        ...
        cxLookAndFeelPaintersManager.GetPainter(lfsNative).DrawGroupBoxFrame(
          ACanvas, GetThemeBackgroundRect(ACanvas), Enabled, cxgpCenter);
    end;
  end
  else
  begin
...
begin
  ...
  begin
    ...
    if not IsRectEmpty(TextRect) then
      Result.Right := Result.Right + cxCheckBoxCaptionIndent;
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if Painter.IsGroupBoxTransparent(False, ACaptionPos) or
           Painter.IsGroupBoxTransparent(True, ACaptionPos)
        then
          DrawParentBackground(ACanvas, ControlRect);
...
begin
  if Source is TcxButtonGroupItem then
    Self.Enabled := TcxButtonGroupItem(Source).Enabled;
...
begin
  ...
  if not TcxCustomEditProperties(GetOwner).ChangedLocked then
  begin
    if Item <> nil then
    begin
...
begin
  Result := ViewInfo.GetCaptionRectIndent;
end;
...
begin
  ...
  try
    R := ViewInfo.GetCaptionRectIndent;
    R1 := ViewInfo.CaptionRect;
    case ViewInfo.CaptionPosition of
      cxgpLeft:
    ...
    ACanvas.SetClipRegion(TcxRegion.Create(R1), roIntersect);

...
{$ENDIF}
  ...
  StdCtrls, cxFilter, cxEdit, cxClasses, cxContainer, cxControls, cxGraphics,
  cxLookAndFeels, cxLookAndFeelPainters, cxDropDownEdit, cxTextEdit, cxFilterControlUtils,
  cxDataStorage, cxFormats, cxListBox, dxMessages;
...
type
  ...
  TcxFilterControlDropDownMenuInnerListBox = class(TdxCustomDropDownInnerListBox)
  private
  ...
  TcxFilterControlDropDownMenu = class(TdxCustomDropDownListBox)
  private
    FControl: TcxCustomFilterControl;
  ...
  protected
    ...
    procedure CreateConditionList(AList: TStrings);
  ...
  public
    ...
    procedure Popup(const ABounds: TRect; const ACaption: string = ''; AKey: Char = #0); reintroduce;
  end;
...
uses
  cxVariants, cxFilterConsts, cxFilterControlStrs, cxCustomData, cxGeometry, ImgList, CommCtrl, dxCore, cxDrawTextUtils, StrUtils;
...
var
  ...
  FilterControlImages: TcxImageList;
...
function TcxFilterControlDropDownMenuInnerListBox.CheckAccelerators(AKey: Word): Boolean;
var
  I: Integer;
begin
  Result := False;
...
function TcxFilterControlDropDownMenuInnerListBox.GetTextFlags: Integer;
begin
...
function TcxFilterControlDropDownMenuInnerListBox.IsIncSearch: Boolean;
begin
  Result := IncrementalSearch and (SearchText <> '') and (Count > 0);
end;

function TcxFilterControlDropDownMenuInnerListBox.IsIncSearchChar(AChar: Char): Boolean;
begin
  Result := (AChar >= ' ') or (AChar = #8);
end;

procedure TcxFilterControlDropDownMenuInnerListBox.ClearIncrementalSearch;
begin
  if SearchText <> '' then
  begin
...
function TcxFilterControlDropDownMenuInnerListBox.DoIncrementalSearch(var Key: Char): Boolean;
var
  ASearchText: string;
begin
  if ShowShortCut then
...
procedure TcxFilterControlDropDownMenuInnerListBox.DrawItemText(const R: TRect;
  AItem: TdxCustomListBoxItem; AState: TcxButtonState);
var
  ABounds: TRect;
begin
  if ShowShortCut then
  ...
  begin
    ...
    if IsIncSearch and AnsiStartsText(SearchText, AItem.Caption) then
      ...
      cxTextOut(Canvas.Canvas, AItem.Caption, ABounds, GetTextFlags);
  end;
end;

function TcxFilterControlDropDownMenuInnerListBox.ProcessKeyPress(var Key: Char): Boolean;
begin
...
function TcxFilterControlDropDownMenuInnerListBox.ProcessNavigationKey(
...
begin
  ...
  begin
    ...
    case Key of
      VK_UP:
        ProcessNextItemWithText(GetValidIndex(ItemIndex - 1), 0);
      VK_DOWN:
        ProcessNextItemWithText(GetValidIndex(ItemIndex + 1), Count - 1);
    end;
    Key := 0;
  end
  else
...
begin
  ...
  Images := FilterControlImages;
end;

procedure TcxFilterControlDropDownMenu.Popup(const ABounds: TRect; const ACaption: string = ''; AKey: Char = #0);
begin
...
begin
  ...
  try
    Items.Clear;
    Items.Add(cxGetResourceString(@cxSFilterAddCondition), 28);
    Items.Add(cxGetResourceString(@cxSFilterAddGroup), 27);
    if Control.FocusedRow <> Control.FRoot then
    begin
      Items.AddSeparator;
      Items.Add(cxGetResourceString(@cxSFilterRemoveRow), 6);
    end
    else
      if Control.FRoot.Group.RowCount > 0 then
      begin
        Items.AddSeparator;
        Items.Add(cxGetResourceString(@cxSFilterClearAll), 17);
      end;
  finally
    EndUpdate;
  end;
...
const
  ImageMap: array[fboAnd..fboNotOr] of Integer = (4, 8, 5, 7);
var
...
procedure TcxFilterControlDropDownMenu.CreateConditionList(AList: TStrings);
const
  ImageMap: array[TcxFilterControlOperator] of Integer =
    (-1, 18, 17, 21, 22, 19, 20, 24, 26, 15, 14, 13, 16, 9, 10,
...
begin
  ...
  try
    Items.Clear;
    for I := 0 to AList.Count - 1 do
      Items.Add(AList[I], ImageMap[TcxFilterControlOperator(TdxNativeInt(AList.Objects[I]))], AList.Objects[I]);
  finally
    EndUpdate;
  end;
end;
...
procedure TcxCustomFilterControl.DoLayoutChange;
  ...
  var
    ADelta: Integer;
  begin
    ADelta := ClientBounds.Right - (ViewInfo.ContentWidth - LeftOffset) - 2;
...
begin
  ...
  Recalculate;
  FLeftOffset := Max((ViewInfo.ContentWidth + 2) - ClientBounds.Right, 0);
  LayoutChanged;
end;
...
begin
  ...
  try
    FillConditionList(AList);
    DropDownMenu.CreateConditionList(AList);
  finally
    AList.Free;
  end;
  with FocusedRow.Condition do
    DropDownMenu.Popup(OperatorRect, OperatorText, AKey);
end;
...
begin
  ...
  begin
    ...
    BoundsRect := FocusedRow.Condition.Values[AIndex].ValueRect;
...
begin
  ...
  if FMaxRowWidth > FContentWidth then
    FContentWidth := FMaxRowWidth;
  CalcButtonState;
  CalcFocusRect;
end;
...
procedure LoadImageListData(AImageList: TCustomImageList; AStream: TStream);
var
  AStreamAdapter: TStreamAdapter;
  ATempStream: TMemoryStream;
  ARetry: Boolean;
  AValue, ABitCount: Byte;
begin
  AImageList.Clear;
  AStreamAdapter := TStreamAdapter.Create(AStream);
  try
    AImageList.Handle := ImageList_Read(AStreamAdapter);
  finally
    AStreamAdapter.Free;
  end;
  if not AImageList.HandleAllocated then
  begin
    ARetry := False;
    ATempStream := TMemoryStream.Create;
    try
      AStream.Position := 0;
      ATempStream.LoadFromStream(AStream);
      ATempStream.Position := 18;
      if (ATempStream.Read(AValue, 1) = 1) and (AValue = 1) then
      begin
        ATempStream.Position := 56;
        if ATempStream.Read(ABitCount, 1) = 1 then
        begin
          AValue := AValue or ABitCount;
          ATempStream.Position := 18;
          ARetry := ATempStream.Write(AValue, 1) = 1;
        end;
      end;
      if ARetry then
      begin
        ATempStream.Position := 0;
        AStreamAdapter := TStreamAdapter.Create(ATempStream);
        try
          AImageList.Handle := ImageList_Read(AStreamAdapter);
        finally
          AStreamAdapter.Free;
...
begin
  FilterControlImages := TcxImageList.Create(nil);
  FilterControlImages.Width := 21;
  FilterControlImages.Height := 16;
  AResourceStream := TResourceStream.Create(HInstance, 'FILTERCONTROLICONS', RT_RCDATA);
  try
    LoadImageListData(FilterControlImages, AResourceStream);
  finally

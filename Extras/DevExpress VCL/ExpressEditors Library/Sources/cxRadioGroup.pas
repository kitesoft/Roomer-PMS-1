...
{$ENDIF}
  ...
  dxUxTheme, cxClasses, cxContainer, cxControls, cxDataStorage, cxGraphics, cxLookAndFeels,
  cxEdit, cxTextEdit, cxGroupBox, cxDropDownEdit, cxFilterControlUtils, dxFading;
...
type
  ...
  TcxRadioButton = class(TRadioButton, IdxFadingObject,
...
uses
  ...
  cxGeometry, cxEditConsts, cxEditPaintUtils, cxEditUtils;
...
begin
  InternalInvalidateRect(Self, GetControlRect(Self), False);
end;

procedure TcxRadioButton.InvalidateRadioButton;
begin
  InternalInvalidateRect(Self, cxRectInflate(ButtonRect, 1, 1), False);
end;
...
begin
  Result := (LookAndFeel.SkinPainter <> nil) or dxFader.Contains(Self);
end;
...
begin
  R := cxRectContent(R, GetTextRectCorrection);
end;
...
begin
  ...
  AButtonRect := ButtonRect;
  APainter.DrawRadioButton(ACanvas, AButtonRect.Left, AButtonRect.Top,
    ButtonStateMap[AState], Checked, Focused, GetBackgroundColor,
    csDesigning in ComponentState);
end;

function TcxRadioButton.GetButtonRect: TRect;
begin
  Result := GetRadioButtonRect(LookAndFeel.GetAvailablePainter(totButton).RadioButtonSize,
    IsNativeStyle);
end;
...
procedure TcxCustomRadioGroupViewInfo.DrawButtonCaption(ACanvas: TcxCanvas;
  ...
  begin
    if AButtonViewInfo.Data.NativeStyle then
      R := cxRectContent(R, ANativeStyleTextRectCorrection)
    else
      R := cxRectContent(R, ATextRectCorrection);
  ...
  try
    ...
    begin
      if UseSkins then
        ...
        APainter := GetPainterClass(False, LookAndFeelKindMap[AButtonViewInfo.Data.Style]);

      APainter.DrawRadioButton(ACanvas, AGlyphRect.Left, AGlyphRect.Top,
        ButtonStateMap[AButtonViewInfo.Data.State],
        ItemIndex = AButtonViewInfo.ButtonIndex, False, GetBackgroundColor, IsDesigning);
    end;
  finally

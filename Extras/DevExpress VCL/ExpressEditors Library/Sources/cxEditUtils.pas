...
procedure DrawCustomEdit(ACanvas: TcxCanvas; AViewInfo: TcxCustomEditViewInfo; ACanDrawBackground: Boolean);
...
procedure RestoreCanvasFont(ACanvas: TcxCanvas; const ASavedLogFont: TLogFont); deprecated; // Use ACanvas.RestoreState;
procedure SaveCanvasFont(ACanvas: TcxCanvas; out ALogFont: TLogFont); deprecated; // Use ACanvas.SaveState;
...
uses
  ...
  cxGeometry, cxDWMApi, dxCore, Math, dxFading;
...
begin
  ...
  begin
    ...
    begin
      if (Data.Kind = bkGlyph) and IsGlyphAssigned(Glyph) then
        Result := Glyph.Width + GetEditButtonTotalBorderExtent
      else
        if (Data.Kind = bkGlyph) and IsImageAssigned(Images, ImageIndex) then
          Result := Images.Width + GetEditButtonTotalBorderExtent
        else
        begin
          if Data.Kind = bkText then
            ...
            ACaptionWidth := 0;
          Result := Max(ACaptionWidth, GetSystemMetrics(SM_CYHSCROLL));
        end;
...
procedure DrawCustomEdit(ACanvas: TcxCanvas;
  AViewInfo: TcxCustomEditViewInfo; ACanDrawBackground: Boolean);
var
  AImageRect, AErrorBounds: TRect;
  AErrorIcon: TGraphic;
begin
  if not AViewInfo.IsInplace and (AViewInfo.Edit <> nil) then
  begin
    if AViewInfo.IsBackgroundPartiallyTransparent or dxFader.Contains(AViewInfo.Edit) then
      cxDrawTransparentControlBackground(AViewInfo.Edit, ACanvas, AViewInfo.Bounds);
  end;
  if not dxFader.DrawFadeImage(AViewInfo.Edit, ACanvas.Handle, AViewInfo.Bounds) then 
    DrawCustomEditBackground(ACanvas, AViewInfo, ACanDrawBackground);

  if (AViewInfo.ErrorData.ErrorType <> eetNone) and IsImageAssigned(AViewInfo.ErrorData.ErrorIcon) then
  begin
    AErrorBounds := TcxCustomEditViewInfoAccess(AViewInfo).FErrorBounds;
    AErrorIcon := AViewInfo.ErrorData.ErrorIcon;
    AImageRect := cxRectCenter(AErrorBounds,
      Min(cxRectWidth(AErrorBounds), AErrorIcon.Width),
      Min(cxRectHeight(AErrorBounds), AErrorIcon.Height));
    ACanvas.StretchDraw(AImageRect, AErrorIcon);
  end;

...
{$ENDIF}
  ...
  dxCore, cxClasses, cxGraphics, cxControls, cxContainer, cxDataStorage, cxDataUtils,
  cxEdit, cxDropDownEdit, cxTextEdit, cxMaskEdit, cxButtons, cxDateUtils, dxCalendarUtils,
...
{ TcxMonthListBox - Bug in Delphi5: Must be in the interface part }
  ...
  private
    ...
    function IsMouseInMonthOrYearRegion(AMousePos: TPoint): Boolean;
  ...
  protected
    ...
    procedure DrawDateRegion(ACanvas: TcxCanvas);
    procedure DrawDayNumbers(ACanvas: TcxCanvas; const R: TRect);
    procedure DrawMonths(ACanvas: TcxCanvas);
    procedure DrawWeekNumbers(ACanvas: TcxCanvas);
    ...
    //
    property CalendarTable: TcxCustomCalendarTable read GetCalendarTable;
    property HotTrackRegion: TcxCalendarHotTrackRegion read FHotTrackRegion write SetHotTrackRegion;
  ...
  public
    ...
    property TimeFormat: TcxTimeEditTimeFormat read GetTimeFormat write SetTimeFormat default tfHourMinSec;
    property Use24HourFormat: Boolean read GetUse24HourFormat write SetUse24HourFormat default True;
...
begin
  ANewWidth := LookAndFeelPainter.ClockSize.cx;
  ANewHeight := LookAndFeelPainter.ClockSize.cy;
  Result := True;
end;
...
begin
  ...
  LookAndFeelPainter.DrawClock(Canvas, ClientRect, TDateTime(FTime), BackgroundColor);
end;
...
procedure TcxClock.LookAndFeelChanged(Sender: TcxLookAndFeel;
...
procedure TcxClock.SetTransparent(Value: Boolean);
begin
...
begin
  ...
  FSelectDate := Date;
  ADate := CalendarTable.FromDateTime(FSelectDate);
  ADate.Day := 1;
  FFirstDate := CalendarTable.ToDateTime(ADate);
  Width := 20;
  Height := 20;
...
begin
  ...
  FTimeEdit.ActiveProperties.OnChange := TimeChanged;
  FTimeEdit.Style.LookAndFeel.MasterLookAndFeel := LookAndFeel;
...
var
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if FFlat and (Kind = ckDate) and (LookAndFeelPainter.LookAndFeelStyle <> lfsOffice11) then
          Inc(R.Top);
        Dec(R.Bottom);
      end;

      if not AIsTransparent then
        Canvas.FillRect(R, GetHeaderColor);
      cxLookAndFeelPaintersManager.GetPainter(lfsUltraFlat).CalculateArrowPoints(
        R, P, AArrowDirectionMap[AArrow], False);
      Canvas.Brush.Color := LookAndFeelPainter.DefaultDateNavigatorHeaderTextColor(False);
      Canvas.Pen.Color := LookAndFeelPainter.DefaultDateNavigatorHeaderTextColor(False);
      Canvas.Polygon(P);
  ...
  var
    ATextSize: TSize;
  begin
    Canvas.Font.Color := LookAndFeelPainter.DefaultDateNavigatorHeaderTextColor(AIsHighlighted);
...
begin
  ...
  AIsTransparent := (ASkinPainter <> nil) or (LookAndFeelPainter.LookAndFeelStyle = lfsNative);
  ...
  begin
    ...
    if LookAndFeelPainter.LookAndFeelStyle = lfsNative then
      ...
      if not AIsTransparent then
        Canvas.FillRect(FViewInfo.HeaderRegion, GetHeaderColor);
  DrawArrows;
      ...
      if Kind = ckDateTime then
        ...
        if LookAndFeelPainter.LookAndFeelStyle = lfsNative then
          Canvas.FrameRect(Rect(AHeaderRect.Left, AHeaderRect.Top - Office11HeaderOffset,
            ...
            BackgroundColor, Office11HeaderOffset)
        else
...
begin
  Result := (Kind = ckDateTime) or FTodayButton.Visible or FClearButton.Visible;
end;
...
procedure TcxCustomCalendar.SetButtonsEnabled;
begin
  FTodayButton.Enabled := not IsShowOnlyValidDates or IsDateInRange(Date);
  FNowButton.Enabled := FTodayButton.Enabled;
end;
...
begin
  ...
  if Kind = ckDateTime then
  begin
    ADate := PosToDateTime(GetMouseCursorClientPos);
    if ADate <> NullDate then
    begin
      SelectDate := ADate;
      DoDateTimeChanged;
...
begin
  Result := inherited IsMouseWheelHandleNeeded(Shift, WheelDelta, MousePos) or
    IsMouseInMonthOrYearRegion(MousePos);
end;
...
begin
  ...
  if not Result and IsMouseInMonthOrYearRegion(MousePos) then
  ...
  case Key of
    ...
    VK_HOME:
      ...
      begin
        ...
        SelectDate := SelectDate - dxDayOfWeekOffset(SelectDate);
    VK_END:
      if Shift = [ssCtrl] then
      begin
      ...
      else SelectDate := SelectDate + (6 - dxDayOfWeekOffset(SelectDate));
...
begin
  ...
  if (Kind = ckDate) and PtInRect(ClientRect, P) and
    not PtInRect(GetMonthRegion, P) and not PtInRect(GetYearRegion, P) and
    CanShowDate(PosToDateTime(P)) then
  begin
...
procedure TcxCustomCalendar.DrawButtons(ACanvas: TcxCanvas);
var
  R, R1: TRect;
begin
  R := Rect(0, Height - FButtonsRegionHeight, Width, Height);
  R1 := R;
  InternalPolyLine(ACanvas, [R1.TopLeft, Point(R1.Right - 1, R1.Top)], ContainerBorderColor, True);
  Inc(R1.Top);
  if not FFlat then
  begin
  ...
  ACanvas.FillRect(R1, BackgroundColor);
  ACanvas.ExcludeClipRect(R);
end;

procedure TcxCustomCalendar.DrawDateRegion(ACanvas: TcxCanvas);
var
  ATextSize: TSize;
  R: TRect;
  S: string;
begin
  ...
  ATextSize := ACanvas.TextExtent(S);
  TrueTextRect(ACanvas.Canvas, R, R.Left + (R.Right - R.Left - ATextSize.cx) div 2,
    R.Top + (R.Bottom - R.Top - ATextSize.cy) div 2, S);
  ACanvas.Brush.Style := bsSolid;
  ACanvas.ExcludeClipRect(FViewInfo.CurrentDateRegion);
end;

procedure TcxCustomCalendar.DrawMonths(ACanvas: TcxCanvas);
var
  ASeparator2Color, AColor: TColor;
  ASize: TSize;
  ATextRect, R: TRect;
...
begin
  ...
  if (ASeparator2Color = clNone) or (ASeparator2Color = clDefault) then
    ASeparator2Color := BackgroundColor;
  ...
  begin
    ...
    ACanvas.FillRect(Rect(Left - 8, Top, Left, Bottom + 2));
    ACanvas.FillRect(Rect(Right, Top, Right + 8, Bottom + 2));

    InternalPolyLine(ACanvas, [Point(Left, Bottom), Point(Right - 1, Bottom)],
      LookAndFeelPainter.DefaultDateNavigatorSeparator1Color, True);
    InternalPolyLine(ACanvas, [Point(Left, Bottom + 1),
      Point(Right - 1, Bottom + 1)], ASeparator2Color, True);
    if (Kind = ckDate) and ShowButtonsRegion then
      InternalPolyLine(ACanvas, [Point(Left, ClientHeight - FButtonsRegionHeight - 1),
    ...
    begin
      ...
      ACanvas.Brush.Color := BackgroundColor;
      ACanvas.FillRect(Rect(R.Left, Top, AWeekNumbersDelimiterPos, R.Bottom));
      InternalPolyLine(ACanvas, [Point(AWeekNumbersDelimiterPos, R.Top),
        Point(AWeekNumbersDelimiterPos, Bottom - 1)], BackgroundColor, True);
      if (Kind = ckDate) and ShowButtonsRegion then
      begin
        InternalPolyLine(ACanvas, [Point(AWeekNumbersDelimiterPos, Bottom),
          Point(AWeekNumbersDelimiterPos, ClientHeight - FButtonsRegionHeight - 1)], clBtnShadow, True);
        InternalPolyLine(ACanvas, [Point(AWeekNumbersDelimiterPos + 1, ClientHeight - FButtonsRegionHeight - 1),
          Point(AWeekNumbersDelimiterPos + 1, ClientHeight - FButtonsRegionHeight - 1)], clBtnShadow, True);
      end
      else
      begin
        InternalPolyLine(ACanvas, [Point(AWeekNumbersDelimiterPos, Bottom),
          Point(AWeekNumbersDelimiterPos, R.Bottom - 3)],
          LookAndFeelPainter.DefaultDateNavigatorSeparator1Color, True);
        InternalPolyLine(ACanvas, [Point(AWeekNumbersDelimiterPos, R.Bottom - 2),
          Point(AWeekNumbersDelimiterPos, R.Bottom - 2)], BackgroundColor, True);
      end;
      InternalPolyLine(ACanvas, [Point(AWeekNumbersDelimiterPos + 1, Bottom),
        Point(AWeekNumbersDelimiterPos + 1, Bottom)],
        LookAndFeelPainter.DefaultDateNavigatorSeparator1Color, True);
      InternalPolyLine(ACanvas, [Point(AWeekNumbersDelimiterPos + 1, Bottom + 1),
        Point(AWeekNumbersDelimiterPos + 1, Bottom + 1)], ASeparator2Color, True);
      InternalPolyLine(ACanvas, [Point(AWeekNumbersDelimiterPos + 1, Bottom + 2),
        Point(AWeekNumbersDelimiterPos + 1, R.Bottom - 2)], BackgroundColor, True);
    end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        FOnGetDayOfWeekState(Self, J, [cdsDefault], ACanvas.Font, AColor);
        ACanvas.Brush.Color := AColor;
      end;

      S := cxGetDayOfWeekName(J, ACanvas.Font.Charset);
      ASize := ACanvas.TextExtent(S);
      TrueTextRect(ACanvas.Canvas, ATextRect, ATextRect.Right - 3 - ASize.cx,
        (ATextRect.Top + ATextRect.Bottom - ASize.cy) div 2, S);
    finally
       ACanvas.RestoreState;
    end;
  end;
  DrawDayNumbers(ACanvas, R);
end;

procedure TcxCustomCalendar.DrawDayNumbers(ACanvas: TcxCanvas; const R: TRect);

  function GetDayState(const ADate, AFirstDate, ALastDate: Double): TCustomDrawState;
  begin
    Result := [];
    if ADate = Date then
    ...
    if (ADate = FSelectDate) or ((FSelectDate = NullDate) and (cdsMarked in Result)) then
      Include(Result, cdsSelected)
    else
      if (ADate < AFirstDate) or (ADate > ALastDate) then
        Include(Result, cdsGrayed);
  end;

  function GetDayTextColor(AState: TCustomDrawState): TColor;
  begin
    if cdsSelected in AState then
      Result := LookAndFeelPainter.DefaultDateNavigatorSelectionTextColor
    else
      if cdsGrayed in AState then
        Result := LookAndFeelPainter.DefaultDateNavigatorInactiveTextColor
      else
        if cdsMarked in AState then
          Result := LookAndFeelPainter.DefaultDateNavigatorTodayTextColor
        else
          Result := LookAndFeelPainter.DefaultDateNavigatorTextColor;
  end;

  function GetDayBrushColor(AState: TCustomDrawState): TColor;
  begin
    if cdsSelected in AState then
...
var
  ADate, ALastDate: Double;
  ASize: TSize;
  ATextRect, ASideRect: TRect;
  I, J: Integer;
  S: string;
  AColor: TColor;
...
begin
  ...
  begin
    ...
    begin
      ...
      AState := GetDayState(ADate, FirstDate, ALastDate);
      ...
      if CanShowDate(ADate) and CalendarTable.IsValidDate(ADate) then
      begin
        // draw frame around current date
        if cdsMarked in AState then
        begin
          ACanvas.FrameRect(ATextRect, LookAndFeelPainter.DefaultDateNavigatorTodayFrameColor);
          InflateRect(ATextRect, -1, -1);
        end;

        ACanvas.SaveState;
        try
          ACanvas.Font.Color := GetDayTextColor(AState);
          ACanvas.Brush.Color := GetDayBrushColor(AState);

          AColor := ACanvas.Brush.Color;
          if Assigned(FOnGetDayOfWeekState) then
            FOnGetDayOfWeekState(Self, dxDayOfWeek(ADate), AState, ACanvas.Font, AColor);
          if Assigned(FOnGetDayState) then
            FOnGetDayState(Self, ADate, AState, ACanvas.Font, AColor);
          ACanvas.Brush.Color := AColor;

          ASize := ACanvas.TextExtent(S);
          TrueTextRect(ACanvas.Canvas, ATextRect, ASideRect.Right - 3 - ASize.cx,
            (ASideRect.Top + ASideRect.Bottom - ASize.cy) div 2, S);
        finally
          ACanvas.RestoreState;
        end;
      end
      else
        ACanvas.FillRect(ATextRect, GetDayBrushColor([cdsDefault]));
    end;
  end;
  DrawWeekNumbers(ACanvas);
  ACanvas.ExcludeClipRect(Rect(R.Left + FSideWidth, R.Top, R.Right - FSideWidth, R.Bottom - 1));
end;

procedure TcxCustomCalendar.DrawWeekNumbers(ACanvas: TcxCanvas);
var
  I: Integer;
  R: TRect;
begin
  ...
  ACanvas.Brush.Color := BackgroundColor;
  ACanvas.Font := Font;
  ACanvas.Font.Size := MulDiv(ACanvas.Font.Size, 2, 3);
  ACanvas.Font.Color := LookAndFeelPainter.DefaultDateNavigatorTextColor;
  for I := 0 to 5 do
  begin
    ...
    ACanvas.DrawTexT(IntToStr(CalendarTable.GetWeekNumber(GetDateFromCell(0, I),
...
procedure TcxCustomCalendar.Paint;
  ...
  begin
    ...
    InternalPolyLine(Canvas, [
      Point(X, FViewInfo.CalendarRect.Top),
      Point(X, FTimeEdit.Top + FTimeEdit.Height - 1)],
      LookAndFeelPainter.DefaultDateNavigatorSeparator1Color, True);
  end;

begin
  if Kind = ckDateTime then
    DrawDateRegion(Canvas);
  DrawHeader;
  DrawMonths(Canvas);
  if (Kind = ckDate) and ShowButtonsRegion then
  begin
    Canvas.ExcludeClipRect(
      cxRectBounds(FSideWidth + FSpaceWidth,
        ClientHeight - FButtonsRegionHeight - 1,
        FDateRegionWidth - FSideWidth * 2, 1));
  end;
  if Kind = ckDateTime then
    DrawButtons(Canvas);
  Canvas.FillRect(ClientRect, BackgroundColor);
  if Kind = ckDateTime then
...
begin
  ...
  FButtonsHeight := MulDiv(cxTextHeight(Canvas.Handle), 20, 13) + 1;
  if Kind = ckDateTime then
  begin
...
begin
  ...
  case AArrow of
    caPrevMonth:
      AFirstDate := CalendarTable.AddMonths(FirstDate, -1);
    caNextMonth:
      AFirstDate := CalendarTable.AddMonths(FirstDate, 1);
    caPrevYear:
      AFirstDate := CalendarTable.AddYears(FirstDate, -1);
    caNextYear:
      AFirstDate := CalendarTable.AddYears(FirstDate, 1);
    else
      AFirstDate := NullDate;
...
var
  R: TRect;
begin
  if (FSelectDate <> Value) and CanShowDate(Value) and cxIsDateValid(Value) then
  begin
    FSelectDate := Value;
    if ARepositionVisibleDates and (Value <> NullDate) then
      FirstDate := dxDateOf(FSelectDate);
...
function TcxCustomCalendar.IsMouseInMonthOrYearRegion(AMousePos: TPoint): Boolean;
begin
  AMousePos := ScreenToClient(AMousePos);
  Result := not FMonthListBox.IsVisible and (PtInRect(FViewInfo.MonthRegion, AMousePos) or
    PtInRect(FViewInfo.YearRegion, AMousePos));
end;
...
begin
  ...
  if ActiveProperties.Kind = ckDateTime then
    ADate := dxDateOf(ADate) + cxSign(ADate) * dxTimeOf(TDateTime(Calendar.FTimeEdit.Time))
  else
...
begin
  if Date = NullDate then
    Exit;
  IsUserAction := ModifiedAfterEnter;
  try
...
begin
  ...
  if ActiveProperties.Kind = ckDateTime then
    Calendar.FTimeEdit.Time := TTime(dxTimeOf(ADate));

  ADate := dxDateOf(ADate);
  Calendar.FirstDate := ADate;
  Calendar.SelectDate := ADate;
  Calendar.Font.Assign(VisibleFont);
  Calendar.Font.Color := clBtnText;
  Calendar.SetSize; // force recalculate size

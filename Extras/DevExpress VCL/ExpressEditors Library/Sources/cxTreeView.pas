...
type
  ...
  TcxBaseInnerTreeView = class(TTreeView, IUnknown,
  ...
  protected
    ...
    property OnInnerTreeViewWndProc: TcxInnerTreeViewWndProcEvent read FOnInnerTreeViewWndProc
      write FOnInnerTreeViewWndProc;
    //
...
begin
  if (FContainer <> nil) and (FContainer.InnerControlMenuHandler(Message)
...
begin
  ...
  if not EqualRect(APrevBoundsRect, InnerTreeView.BoundsRect) and
    InnerTreeView.HandleAllocated then
      dxMessagesController.KillMessages(InnerTreeView.Handle, WM_MOUSEMOVE);
end;

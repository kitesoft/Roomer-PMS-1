...
type
  ...
  protected
    ...
    function PrepareValue(const AValue: TcxEditValue; AEditFocused: Boolean): Variant; virtual;
...
procedure TcxSpinEditViewInfo.DrawHotFlatButtonBorder(ACanvas: TcxCanvas; AButtonViewInfo: TcxEditButtonViewInfo;
  ...
  begin
    if AButtonViewInfo.ButtonIndex > 1 then
      Result := AFastButtonPositionA[ButtonsPosition, TcxSpinEditButton(AButtonViewInfo.ButtonIndex)]
    else
...
var
  ...
  begin
    ...
    begin
      ...
      AFastButtonWidth := MulDiv(ADefaultButtonHeight, 3, 2) + Integer(Odd(ADefaultButtonHeight));
  end;
...
begin
  ...
  if not Result then
    Exit;
  try
    EditValue := PrepareValue(EditValue, AEditFocused);
...
function TcxCustomSpinEditProperties.PrepareValue(const AValue: TcxEditValue;
  AEditFocused : Boolean): Variant;
begin
  if VarIsSoftNull(AValue) then
    if UseNullString and VarIsNull(AValue) and not AEditFocused then
      Result := Null
    else
  ...
  begin
    ...
    begin
      ...
      if AValue < InternalMinValue then
        ...
        begin
          if not InternalIncrement(AValue, Self.GetIncrement(AButton)) then
            Exit;
        end;
    end;
    Result := True;
...
begin
  ...
  with ActiveProperties do
    if IsEditValueValid(AValue, Focused) then
      SetInternalValue(PrepareValue(AValue, Focused))
    else

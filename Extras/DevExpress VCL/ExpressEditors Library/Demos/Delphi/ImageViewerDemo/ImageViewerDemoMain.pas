...
uses
  ...
  cxColorComboBox, ExtCtrls, cxMaskEdit, cxDropDownEdit,
  dxLayoutLookAndFeels, dxZoomTrackBar, cxSpinEdit;
...
type
  TImageViewerDemoMainForm = class(TForm)
    ...
    ztbZoom: TdxZoomTrackBar;
    lbZoom: TcxLabel;
    pbMain: TPaintBox;
    ceBackColor: TdxColorEdit;
    lbBackColor: TcxLabel;
    sbResize: TcxButton;
    ...
    procedure sbHidePanelClick(Sender: TObject);
    procedure sbInfoClick(Sender: TObject);
    procedure miFiveColumnsClick(Sender: TObject);
    procedure ztbZoomPropertiesChange(Sender: TObject);
    procedure pbMainPaint(Sender: TObject);
    procedure ceBackColorPropertiesChange(Sender: TObject);
...
begin
  ...
  ceBackColor.ColorValue := clWindow;
end;
...
begin
  if (dxGalleryControl.Gallery.Groups[0] <> nil) and (dxGalleryControl.Gallery.Groups[0].Items[0] <> nil) then
    dxGalleryControl.Gallery.ClickItem(dxGalleryControl.Gallery.Groups[0].Items[0]);
end;

procedure TImageViewerDemoMainForm.Draw;

  procedure CalculateRect(var ARect: TRect);
  begin
    ARect := cxNullRect;
    ARect.Right := Round(MulDiv(FGlyph.Width, ztbZoom.Position, 100));
    ARect.Bottom := Round(MulDiv(FGlyph.Height, ztbZoom.Position, 100));
    ARect := cxRectCenter(pbMain.BoundsRect, cxRectWidth(ARect), cxRectHeight(ARect));
  end;
...
begin
  CalculateRect(ARect);
  pbMain.Canvas.Brush.Color := ceBackColor.ColorValue;
  pbMain.Canvas.FillRect(pbMain.ClientRect);
  pbMain.Canvas.StretchDraw(ARect, FGlyph);
end;
...
procedure TImageViewerDemoMainForm.sbHidePanelClick(Sender: TObject);
begin
  if gbRightPanel.Visible then
  begin
    gbRightPanel.Visible := False;
    cxSplitter1.Enabled := False;
  end
  else
  begin
    gbRightPanel.Visible := True;
    cxSplitter1.Enabled := True;
...
procedure TImageViewerDemoMainForm.ztbZoomPropertiesChange(
  Sender: TObject);
begin
  lbZoom.Caption := IntToStr(ztbZoom.Position) + '%';
  pbMain.Invalidate;
end;

procedure TImageViewerDemoMainForm.pbMainPaint(Sender: TObject);
begin
  Draw;
end;

procedure TImageViewerDemoMainForm.ceBackColorPropertiesChange(
  Sender: TObject);
begin
  pbMain.Invalidate;
end;

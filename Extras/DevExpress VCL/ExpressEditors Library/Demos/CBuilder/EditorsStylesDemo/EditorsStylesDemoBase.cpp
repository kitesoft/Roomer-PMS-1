...
const
  String AExtEditorNames[eetCheckGroupBox+1] = {"TcxLabel",
    ...
    "TcxCalcEdit", "TcxDateEdit", "TcxTextEdit", "TcxRichEdit", "TdxZoomTrackBar", "TdxCheckGroupBox"};
...
TcxExtEditorTypes GetcxControlType(String AcxControlName)
{
  ...
  for (int i=0; i < eetCheckGroupBox + 1; i++)
...
void cxGetEditorsNamesListByTypes(TStrings* AEditorsNames, TcxExtEditorTypes AExtEditorTypes)
{
  if (AEditorsNames) {
    AEditorsNames->Clear();
    for (int i=eetLabel; i < eetCheckGroupBox + 1; i++)

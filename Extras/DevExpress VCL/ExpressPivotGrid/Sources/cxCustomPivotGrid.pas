...
{$ENDIF}
  ...
  {$IFNDEF NONDB}
     FMTBcd, SqlTimSt,
  {$ENDIF}
  dxCore, dxCoreClasses, dxMessages, cxClasses, cxControls, cxGraphics,
  cxCustomData, cxDataStorage, cxDataUtils, dxCustomHint,
  ...
  cxFilterControlDialog, cxNavigator, dxCustomTree;
...
{$ENDIF}
  ...
  public
    ...
    property UniqueName: Variant read FUniqueName write FUniqueName;
...
//    property GrandTotalWidth: Integer read FGrandTotalWidth write SetGrandTotalWidth default 0;
  ...
  published
    ...
    property FormStyle: TcxPivotGridCustomizationFormStyle read FFormStyle
      write SetFormStyle default cfsDefault;
...
// ViewInfo
  ...
  TcxPivotGridCustomCellViewInfo = class(TObject, IUnknown, IcxHintableObject)
  private
  ...
  public
    ...
    constructor Create(APainter: TcxCustomLookAndFeelPainter; const ABounds,
...
// PivotGrid Base Classes
  ...
  protected
    ...
    procedure CreateFieldsFromStructure;
    ...
    procedure CreateMembers(AField: TcxPivotGridField;
      AParent: TcxPivotGridGroupItem; AHasChildren: Boolean; ADataFields: TcxPivotGridFields); virtual;
    ...
    procedure CreateRootLayout(APivotGrid: TcxCustomPivotGrid; ARowFields,
      AColumnFields, ADataFields, AFilterFields: TcxPivotGridFields); virtual;
    ...
    procedure InitializeCrossCells(ARowsGroup, AColumnsGroup: TcxPivotGridGroupItem;
      ADataFields: TcxPivotGridFields; AOLAPType: TcxPivotGridOLAPFieldType); virtual;
    procedure InitializeCrossCellKPIValue(ACell: TcxPivotGridCrossCell; ADataFieldIndex: Integer;
      const AFormattedValue, ANativeValue: Variant);
    procedure InitializeCrossCellValues(ACell: TcxPivotGridCrossCell;
      const AFormattedValues, ANativeValues: Variant; IsInitialized: Boolean);
    procedure InitializeExpanding(ADataBuilder: TcxPivotGridDataBuilder; AField: TcxPivotGridField;
      AnExpandingItem, ACrossGroup: TcxPivotGridGroupItem; ADataFields: TcxPivotGridFields); virtual;
    procedure InitializeLayoutKPI(ARowFields, AColumnFields, ADataFields,
      AFilterFields: TcxPivotGridFields; ARowsGroup, AColumnsGroup: TcxPivotGridGroupItem); virtual;
    procedure CreateLayoutTotals(AColumns, ARows: TcxPivotGridGroupItem; ASummaryFields: TcxPivotGridFields; AnExpandedMembers: TList); virtual;
    procedure InitializeStructureMembers(AField: TcxPivotGridOLAPField; AList: TcxPivotGridVariantList); virtual;
    procedure InitializeTotal(ADataFields, AFilterFields: TcxPivotGridFields; AColumns, ARows: TcxPivotGridGroupItem); virtual;
    ...
    procedure QueryLayout(ARowFields, AColumnFields, ADataFields,
      AFilterFields: TcxPivotGridFields); virtual;
    procedure QueryLayoutTotals(AColumnFields, ARowFields, ADataFields,
      AFilterFields: TcxPivotGridFields; AnExpandedMembers: TList); virtual;
    procedure QueryRootMembers(ARowField: TcxPivotGridField; AParentMembers: TList;
      AFilters: TcxPivotGridFields; ADataFields: TcxPivotGridFields); virtual;
  ...
  {$ENDIF}
    ...
    property OnPrefilterDialogShow: TNotifyEvent read FOnPrefilterDialogShow
      write FOnPrefilterDialogShow;
...
uses
  ...
  cxEditDataRegisteredRepositoryItems, cxPivotGridAdvancedCustomization;
...
begin
  ...
  begin
    if not HitTest.HitAtCustomizationForm and PtInRect(PivotGrid.ClientBounds, P) then
    begin
      for I := 0 to DragDropAreaCount - 1 do
      begin
        AInfo := DragDropAreas[I];
        Result := PtInRect(AInfo.Bounds, P) and DragField.IsCompatibleWidth(AInfo);
...
begin
  ...
  begin
    ...
    begin
      ...
      ALeft := ASize.cx + cxPivotGridHorzSpace;
      Inc(ASize.cx, cxRectWidth(AField.ViewInfo.Bounds) + cxPivotGridHorzSpace);
      ARgn.Combine(TcxRegion.Create(ALeft - cxPivotGridHorzSpace,
        ASize.cy div 2, ALeft, ASize.cy div 2 + 1), roAdd);
...
begin
  SizableObject.SetSizeDelta(ADelta);
  PivotGrid.Modified;
end;
...
begin
  ...
  if PivotGrid.OLAPDataSource <> nil then
    PivotGrid.OLAPDataSource.InitializeStructureMembers(TcxPivotGridOLAPField(Self), FGroupValueList)
  else
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      Result := Max(cxPivotGridDefaultFieldWidth, ViewInfo.MeasureWidth);
    end;
  end;
...
begin
  ...
  if (Result = '') and (Structure <> nil) and Structure.Currency then
...
begin
  Result := LookAndFeelPainter.ExpandButtonSize + cxPivotGridDoubleSpace * 2;
end;
...
constructor TcxPivotGridCustomCellViewInfo.Create(APainter: TcxCustomLookAndFeelPainter;
...
function TcxPivotGridCustomCellViewInfo.QueryInterface(const IID: TGUID; out Obj): HResult;
begin
...
function TcxPivotGridCustomCellViewInfo._AddRef: Integer;
begin
  Result := -1;
end;

function TcxPivotGridCustomCellViewInfo._Release: Integer;
begin
  Result := -1;
end;
...
constructor TcxPivotGridEditContainerViewInfo.Create(
  APainter: TcxCustomLookAndFeelPainter; const ABounds, AVisibleRect: TRect;
...
constructor TcxPivotGridHeaderCellViewInfo.Create(APainter: TcxCustomLookAndFeelPainter;
  const ABounds, AVisibleRect: TRect; const AViewParams: TcxViewParams);
begin
  inherited Create(APainter, ABounds, AVisibleRect, AViewParams);
 // FAlignVert := vaCenter;
...
begin
  Create(APainter, ABounds, AVisibleRect, AViewParams);
...
begin
  ...
  begin
    ...
    begin
      ...
      FButtonAreaRect := cxRectSetSize(FButtonRect, ExpandButtonAreaSize, ExpandButtonAreaSize);
    end;
    CheckClipping(Bounds.Right - FButtonRect.Right <  cxPivotGridDoubleSpace);
    FTextBounds.Left := FButtonAreaRect.Right + cxTextOffset + 2;
  end;
...
begin
  ...
  DrawHeaderText;
  if HasButton then
    Painter.DrawExpandButton(Canvas, FButtonRect, Expanded);
...
procedure TcxPivotGridHeaderCellViewInfo.DrawHeaderText;
  ...
  begin
    ...
    begin
      ...
      cxDrawText(Canvas, DisplayText, TextBounds, cxFlagsToDTFlags(cxAlignmentsHorz[AlignHorz] or
...
begin
  ...
  begin
    ...
    if HasButton and PtInRect(FButtonAreaRect, AHitTest.HitPoint) then
...
procedure TcxPivotGridFieldHeaderCellViewInfo.Initialize(
  ACanvas: TcxCanvas; APainter: TcxCustomLookAndFeelPainter;
  const AViewParams: TcxViewParams);
...
begin
  ...
  if FFilterState = cxbsNormal then
    Inc(Result, IfThen(FIsSmartTag,
      Painter.FilterSmartTagSize.cx, Painter.FilterDropDownButtonSize.X) + cxTextOffset);
  if HasButton then
...
begin
  ...
  if FIsSmartTag then
    ASize := Painter.FilterSmartTagSize
  else
  begin
    ASize.cx := Painter.FilterDropDownButtonSize.X;
    ASize.cy := Painter.FilterDropDownButtonSize.Y;
  end;
...
begin
  ...
  begin
    if FIsSmartTag then
      Painter.DrawFilterSmartTag(Canvas, FilterBounds, GetSmartTagState, FFilterActive)
    else
  ...
  if FProperties <> nil then
    ...
    if DisplayText <> '' then
      cxDrawText(Canvas, DisplayText, TextBounds,
        cxFlagsToDTFlags(cxAlignmentsHorz[Align] or cxAlignmentsVert[FAlignVert])); 
end;
...
var
  ABounds: TRect;
begin
  ABounds := cxRectCenter(Bounds, FPainter.CheckButtonSize);
  FPainter.DrawFilterActivateButton(ACanvas, ABounds, FState, GetChecked);
end;
...
constructor TcxPivotGridPrefilterViewInfo.Create(APrefilter: TcxPivotGridPrefilter;
  APainter: TcxCustomLookAndFeelPainter; const ABounds,
...
begin
  ...
  if AField <> nil then
    Result := AField.GetCellViewInfoClass.Create(Painter, ABounds, DataCellsBounds, FViewParams)
  else
...
begin
  FFilterHeight := 0;
  FDataFieldsWidth := 0;
  AContentHeight := Max(cxTextHeight(GetFont(gs_FieldHeader)), Painter.ExpandButtonAreaSize);
  FFieldHeaderHeight := Max(Painter.HeaderHeight(AContentHeight), GetIconsSize.cx + cxPivotGridDoubleSpace);
  for I := 0 to PivotGrid.FieldCount - 1 do
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      ABounds.Left := Bounds.Left + cxPivotGridHorzSpace +
        MulDiv(RowHeadersWidth - cxPivotGridHorzSpace, I, RowFieldsCount) ;
      ABounds.Right := Bounds.Left + cxPivotGridHorzSpace +
        MulDiv(RowHeadersWidth - cxPivotGridHorzSpace, I + 1, RowFieldsCount) - cxPivotGridHorzSpace;
    end
    else
    begin
      Inc(ALeft, AWidth);
...
begin
  ...
  begin
    ...
    if (AArea <> faRow) or (OptionsView.RowTotalsLocation <> rtlTree) then
      AddDragDropAreaInfo(ABounds.Right + cxPivotGridHalfSpace, AAreaBounds,
        cxRectSetYPos(AAreaBounds, R.Top, R.Bottom), AArea, AIndex, nil);
end;
...
begin
  ...
  if (ItemCount > 0)  and not result then
    FExpanded := FExpanded;
end;
...
procedure TcxPivotGridOLAPStructureNode.ReadData(AStream: TStream);
var
  AReader: TcxReader;
begin
...
procedure TcxPivotGridCustomOLAPDataSource.CreateMembers(AField: TcxPivotGridField;
...
procedure TcxPivotGridCustomOLAPDataSource.CreateRootLayout(APivotGrid: TcxCustomPivotGrid; ARowFields,
  AColumnFields, ADataFields, AFilterFields: TcxPivotGridFields);
var
...
begin
  ...
  begin
    QueryRootMembers(ARowFields[0], nil, AFilterFields, ADataFields);
    CreateMembers(ARowFields.GetFirstField, ADataBuilder.FRows, ADataBuilder.RowFields.Count > 1, ADataFields);
  end;
  if AColumnFields.Count > 0 then
  begin
    QueryRootMembers(AColumnFields[0], nil, AFilterFields, ADataFields);
    CreateMembers(AColumnFields.GetFirstField, ADataBuilder.FColumns, ADataBuilder.ColumnFields.Count > 1, ADataFields);
  end;
  with ADataBuilder do
  begin
    QueryLayout(ARowFields, AColumnFields, ADataFields, AFilterFields);
    InitializeCrossCells(Rows, Columns, SummaryFields, oftMeasure);

    QueryLayoutTotals(AColumnFields, ARowFields, ADataFields, AFilterFields, nil);
    CreateLayoutTotals(Columns, Rows, SummaryFields, nil);
    QueryLayoutTotals(ARowFields, AColumnFields, ADataFields, AFilterFields, nil);
    CreateLayoutTotals(Rows, Columns, SummaryFields, nil);
    InitializeTotal(SummaryFields, AFilterFields, Columns, Rows);

    InitializeLayoutKPI(ARowFields, AColumnFields, ADataFields, AFilterFields, Rows, Columns);
...
procedure TcxPivotGridCustomOLAPDataSource.InitializeCrossCells(ARowsGroup,
  AColumnsGroup: TcxPivotGridGroupItem; ADataFields: TcxPivotGridFields; AOLAPType: TcxPivotGridOLAPFieldType);
begin
end;

procedure TcxPivotGridCustomOLAPDataSource.InitializeCrossCellKPIValue(ACell: TcxPivotGridCrossCell; ADataFieldIndex: Integer;
  const AFormattedValue, ANativeValue: Variant);
var
  I: Integer;
begin
  for I := 0 to Integer(High(TcxPivotGridSummaryType)) do
    ACell.SummaryCells[ADataFieldIndex].SetSummaryByIndex(I, AFormattedValue);
end;
...
procedure TcxPivotGridCustomOLAPDataSource.InitializeLayoutKPI(ARowFields, AColumnFields, ADataFields,
  AFilterFields: TcxPivotGridFields; ARowsGroup, AColumnsGroup: TcxPivotGridGroupItem);
begin
end;

procedure TcxPivotGridCustomOLAPDataSource.CreateLayoutTotals(AColumns, ARows: TcxPivotGridGroupItem;
  ASummaryFields: TcxPivotGridFields; AnExpandedMembers: TList);
begin
end;

procedure TcxPivotGridCustomOLAPDataSource.InitializeStructureMembers(
  AField: TcxPivotGridOLAPField; AList: TcxPivotGridVariantList); 
begin
end;

procedure TcxPivotGridCustomOLAPDataSource.InitializeTotal(ADataFields, AFilterFields: TcxPivotGridFields;
...
procedure TcxPivotGridCustomOLAPDataSource.QueryLayout(
  ARowFields, AColumnFields, ADataFields, AFilterFields: TcxPivotGridFields);
begin
end;

procedure TcxPivotGridCustomOLAPDataSource.QueryLayoutTotals(AColumnFields, ARowFields, ADataFields,
  AFilterFields: TcxPivotGridFields; AnExpandedMembers: TList);
begin
end;

procedure TcxPivotGridCustomOLAPDataSource.QueryRootMembers(ARowField: TcxPivotGridField; AParentMembers: TList;
...
begin
  ...
  if FGraphicType = gtServerDefined then
...
begin
  ...
  begin
    ...
    if AImageIndex > 0 then
      cxPivotGridKPIImageList.Draw(Canvas.Canvas, Bounds, AImageIndex, False, False, True);
  end;
end;
...
begin
  ...
  if (Field = AField) and (PivotGrid <> nil) and (not IsTotal or AIncludeTotals) then
    ...
    begin
      ...
      if (ItemCount > 0) or ((GroupItem <> nil) and (GroupItem.ItemCount > 0)) then
        Inc(Result, LookAndFeelPainter.ExpandButtonSize + cxPivotGridDoubleSpace * 2);
  ...
  if IsGrandTotal and ((AField = Field) or (ItemCount = 0)) then
   Inc(Result, cxTextWidth(PivotGrid.Styles.GetRowHeaderParams(Self).Font, Value) + cxPivotGridDoubleSpace)
  else
    for I := 0 to ItemCount - 1 do
...
begin
  ...
  if Result and AField.Visible then
    case AField.Area of
      faColumn: AType := fltColumn;
      faRow: AType := fltRow;
      faFilter: AType := fltFilter;
      faData: AType := fltData;
    end;
end;
...
procedure TcxPivotGridCustomCustomizationForm.SetDragFieldToController(
  AField: TcxPivotGridField);
begin
...
begin
  Result := Visible and Form.CalculateHitTest(AHitTest);
end;
...
begin
  ...
  AIsStringField := (not (DataField.SummaryType in [stCount, stCustom])) and ((DataField.DataBinding.ValueTypeClass = TcxStringValueType) or
    (DataField.DataBinding.ValueTypeClass = TcxWideStringValueType));
  InitializeValues;
  BaseCalculation;
  if ACount <> 0 then
  begin
  ...
  try
    ...
    for I := 1 to Values.Items.Count - 1 do
      if (Values.Items[I].State = AStates[AIsInclude]) then
        Filter.Values.AddUnique(GetFieldValueByValueIndex(I));
  finally
    Filter.EndUpdate;
  end;
end;
...
begin
  ...
  if Field.UseEditProperties and (Field.SummaryVariation = svNone) and
    (Field.Area <> faData) and (Field.GroupInterval = giDefault)  then
  ...
  if AValue.FUnUsed then
    ...
    Result.State := TcxCheckBoxState(Ord(not Filter.HasFilter or
...
const
  ADefWidth: array[Boolean, Boolean] of Integer = (
...
begin
  ...
  if Result = 0 then
    Result := ADefWidth[AColumnItem, RowTotalsLocation = rtlTree];
...
begin
  AWidth := cxPivotGridDefaultFieldMinWidth;
  for I := 0 to PivotGrid.ViewData.RowCount - 1 do
  begin
    ARow := PivotGrid.ViewData.Rows[I];
    AWidth := Max(AWidth, ARow.MeasureWidth(ARow.Field, False) + cxTextOffset * 2);
  end;
...
begin
  if PivotGrid.HitTest.FResizeRows then
    Result := GetActualItemWidth(False)
  else
    Result := GetActualItemWidth(True)
end;
...
var
  AWidth: Integer;
begin
  AWidth := Max(cxPivotGridDefaultFieldMinWidth, GetActualWidth + ADelta);
  if PivotGrid.HitTest.FResizeRows then
    RowGrandTotalWidth := AWidth
  else

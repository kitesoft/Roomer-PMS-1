...
uses
  ...
  cxCustomData, cxTreeView, ComCtrls;
...
procedure TcxFieldListListBox.DrawItem(
...
begin
  ...
  begin
    ...
    begin
      ...
      AFilterState := FilterStateMap[FilterState <> cxbsDisabled];
      PaintTo(Self.Canvas, R, AState, AFilterState, ASortOrder, False,
...
begin
  ...
  begin
    ...
    if I = Count then
      R.Top := ATop;
    ATop := R.Bottom;
    if R.Top >= ClientRect.Bottom then Break;
    AddDropArea(R, FieldByIndex(I), I);
  end;
}
...
begin
  ...
  begin
    ...
    begin
      ...
      if PtInRect(R, P) or (R.Bottom > ClientBounds.Bottom) then
        Break
      else
        Inc(AInfo.AreaIndex, GetAreaIndexOffset(FieldByIndex(AIndex),
          TcxPivotGridCustomizationForm(FieldChooser).GetImmediateUpdate));
...
begin
  ItemHeight := LookAndFeelPainter.HeaderHeight(cxTextHeight(Font));
end;
...
begin
  ...
  FieldList.ItemHeight := Max(Painter.HeaderHeight(cxTextHeight(Font)), FieldItemHeight);
...
begin
  Result := PivotGrid.LookAndFeelPainter.HeaderHeight(cxTextHeight(FieldViewInfo.Font));
end;
...
begin
  inherited CreateNew(nil);
  Constraints.MinWidth := Round(cxPivotGridCustomizationMinWidth * 1.3);
  Constraints.MinHeight := Round(cxPivotGridCustomizationMinHeight * 2.5);
...
begin
  ...
  InflateRect(R, -cxPivotGridControlsIndent, -cxPivotGridControlsIndent);
  // caption
  FieldListCaption := TLabel.Create(Self);
  FieldListCaption.Parent := Self;
  FieldListCaption.Transparent := True;
  FieldListCaption.BoundsRect :=  cxRectSetHeight(R, FontHeight);
  FieldListCaption.Caption := cxGetResourceString(@scxDragItems);
  Inc(R.Top, FieldListCaption.Height + cxPivotGridControlsIndent);
  FieldListCaption.Anchors := [akLeft, akTop];
  // bottom pane
  FieldList := TcxPivotGridOLAPTreeView.Create(Self);
  FieldList.Parent := Self;
  FieldList.ToolTips := False;
  FieldList.Anchors := AnchorAlign[alClient];
...
procedure TcxPivotGridOLAPCustomizationForm.OnGetTreeViewNodeImageIndex(
...
begin
  inherited Init;
  Width := Round(cxPivotGridCustomizationMinWidth * 1.3);
  Height := Round(cxPivotGridCustomizationMinHeight * 2.8);
end;

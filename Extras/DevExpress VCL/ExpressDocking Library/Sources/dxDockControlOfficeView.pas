...
begin
  ...
  if DockControl.SideContainer is TdxVertContainerDockSite then
    if IsSwitched then
    begin
      APoints[0] := Point(ARect.Right - 4, ARect.Top + 2);
      APoints[1] := Point(ARect.Left + 2, ARect.Top + 2);
    end
    else
    begin
      APoints[0] := Point(ARect.Right - 4, ARect.Bottom - 4);
      APoints[1] := Point(ARect.Left + 2, ARect.Bottom - 4);
    end
  else
    if IsSwitched then
    begin
      APoints[0] := Point(ARect.Left + 2, ARect.Top + 2);
      APoints[1] := Point(ARect.Left + 2, ARect.Bottom - 4);
    end
    else
    begin
      APoints[0] := Point(ARect.Right - 4, ARect.Top + 2);
      APoints[1] := Point(ARect.Right - 4, ARect.Bottom - 4);
    end;

  APoints[2] := Point(ARect.Left + 5, ARect.Top + 5);
...
begin
  cxLookAndFeelPaintersManager.GetPainter(lfsOffice11).DrawArrow(ACanvas, ARect, AState, AArrow);
end;

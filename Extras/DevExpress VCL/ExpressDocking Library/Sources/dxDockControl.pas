...
procedure TdxCustomDockControl.WMCheckActiveDockControl(var Message: TMessage);
  ...
  begin
    if TCustomFormAccess(AForm).FormStyle = fsMDIForm then
      Result := GetActiveWindow = AForm.Handle 
    else
      Result := AForm.Active;
  end;

begin
  if (Controller.ActiveDockControl <> Self) and CanActive and (ParentForm <> nil) and IsFormActive(ParentForm) then
    Activate;
end;
    ...
    begin
      ...
      begin
        if AControl <> nil then
          ...
          ADeltaSize := GetDifferentSize div ValidChildCount;

        for I := 0 to ChildCount - 1 do
        begin
          if (Children[I] <> AControl) and IsValidChild(Children[I]) then
          OriginalSizes[I] := Max(OriginalSizes[I] + ADeltaSize, MinSizes[I]);
        end;

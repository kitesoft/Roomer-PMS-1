...
uses
  ...
  dxDockControl, cxLookAndFeelPainters, cxGraphics;
...
function TdxDockControlXPPainter.GetCaptionButtonSize: TSize;
begin
  if OpenTheme(totTab) <> 0 then
    Result := cxSize(14, 14)
  else
...
begin
  if OpenTheme(totTab) <> 0 then
    Result := Max(2 * 7 + GetFont.Size, 2 * 2 + GetImageHeight)
  else
...
procedure TdxDockControlXPPainter.DrawCaptionText(
...
procedure TdxDockControlXPPainter.DrawHideBar(

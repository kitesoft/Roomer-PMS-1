...
function TdxTabContainerZone.GetTabPosition: TcxTabPosition;
begin
...
function TdxTabContainerZone.GetTabRect(AControl: TdxCustomDockControl): TRect;
  ...
  begin
    Result := TdxTabContainerDockSiteAccess(TabContainer).Painter.GetImageWidth +
  ...
  if (DockIndex < 0) and (AControl = AContainerAccess.GetLastValidChild) then
    ...
    begin
      ...
      begin
        ...
        if GetTabPosition in [tpTop, tpBottom] then
...
var
  ATabContainerAccess: TdxTabContainerDockSiteAccess;
begin
  Result := False;
  ...
  begin
    ...
    begin
      ATabContainerAccess := TdxTabContainerDockSiteAccess(TabContainer);
      Result := inherited IsZonePoint(Pt) and PtInRect(ATabContainerAccess.WindowRectToClient(
      GetTabRect(dxDockingController.DockingDockControl)), TabContainer.ScreenToClient(pt));
    end;
  end;
end;
...
function TdxDockPanelClientZone.GetTabPosition: TcxTabPosition;
var
  AProperties: TdxTabContainerDockSiteProperties;
begin
  if TdxDockPanelAccess(Owner).ShowSingleTab then
...
begin
  Result := TdxCustomDockControlAccess(DockPanel).IsCaptionPoint(DockPanel.ScreenToClient(pt));
end;

...
{$ENDIF}
  SysUtils, Classes, DB, ADODB, dxServerModeData, dxServerModeADODataSource, ServerModeDemoConnection;

type
  TServerModeDemoDataDM = class(TDataModule)
    ADOConnection: TADOConnection;
    ADOQuery: TADOQuery;
    ServerModeQueryDataSource: TdxServerModeADOQueryDataSource;
  private
...
function GetCaption: string;
begin
  Result := 'ExpressQuantumGrid ADO Server Mode Query Demo';
end;
...
begin
  Result := 'This demo shows a grid control''s capabilities when bound to a large ' +
    'amount of data in Server Mode via an ADO connection. The data is retrieved ' +
    'using an ADO query.';
end;

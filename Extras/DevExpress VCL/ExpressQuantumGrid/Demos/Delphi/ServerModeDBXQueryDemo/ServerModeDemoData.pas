...
{$ENDIF}
  SysUtils, Classes, DB, SqlExpr, dxServerModeData, ServerModeDemoConnection,
{$IFDEF DELPHI11}
  DBXCommon,
{$ENDIF}
{$IFNDEF DELPHI8}
...
{$ELSE}
  DBXMSSQL,
{$ENDIF}
  dxServerModeDBXDataSource, FMTBcd;

type
  TServerModeDemoDataDM = class(TDataModule)
    SQLConnection: TSQLConnection;
    SQLQuery: TSQLQuery;
    ServerModeQueryDataSource: TdxServerModeDBXQueryDataSource;
  private
...
function GetCaption: string;
begin
  Result := 'ExpressQuantumGrid dbExpress Server Mode Query Demo';
end;
...
begin
  Result := 'This demo shows a grid control''s capabilities when bound to a large ' +
    'amount of data in Server Mode via a dbExpress connection. The data is retrieved ' +
    'using a dbExpress query.';
end;

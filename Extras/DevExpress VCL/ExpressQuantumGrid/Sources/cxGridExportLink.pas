...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, Variants, FMTBcd, SqlTimSt, Windows, Classes, SysUtils, Graphics, DB,
  dxCore, cxGeometry, cxControls, cxClasses, cxGraphics, cxStyles, cxLookAndFeels,
  cxLookAndFeelPainters, cxCustomData, cxDataStorage, cxDataUtils, cxDBData,
  dxLayoutContainer, cxVariants, cxExport, cxEdit, cxImage, cxCalendar, cxCurrencyEdit,
  cxSpinEdit, cxCalc, cxTimeEdit, cxMaskEdit, cxGrid, cxGridCustomView, cxGridCustomTableView,
  cxGridTableView, cxGridBandedTableView, cxGridCardView, cxGridLevel, cxGridRows,
...
type
  ...
  protected
    ...
    function SetCellAsGraphic(const ABounds: TRect; AColumn, ARow: Integer;
  ...
  protected
    ...
    function AddVisualDataItem(const AItemBounds: TRect; AStyle: Integer;
      ARecord: TcxCustomGridRecord; AColumn: TcxCustomGridTableItem): TcxExportVisualItem;
    ...
    function AddVisualItemEx(const AItemBounds: TRect; const ADisplayText: string;
      ...
      ABorderColor: TColor = clDefault; AIsBackground: Boolean = False; AWordWrap: Boolean = True): TcxExportVisualItem; overload;
    function AddVisualItemEx(const AItemBounds: TRect; const ADisplayText: string;
  ...
  TcxGridLayoutViewExport = class(TcxGridTableViewExport)
  private
  ...
  protected
    ...
    //
    procedure GetItemBordersInfo(const AItemInfo: TdxCustomLayoutItemViewInfo;
      var ABorders: TcxBorders; var ABorderColor: TColor);
...
uses
  Contnrs, dxXLSXExport, cxXLSExport, cxHtmlXmlTxtExport, Math, Dialogs,
    ...
    if AGridView is TcxGridTableView then
      ...
      if AGridView is TcxGridCardView then
        ...
        if AGridView is TcxGridChartView then
          ...
          if AGridView is TcxGridLayoutView then
...
begin
  ...
  FInternalProviderSupported := Supports(FProvider, IcxCellInternalCache, FInternalProvider);
...
function TcxGridCustomExport.SetCellAsGraphic(const ABounds: TRect; AColumn, ARow: Integer;
  ARecord: TcxCustomGridRecord; AItem: TcxCustomGridTableItem): Boolean;
var
  AClientRect, R: TRect;
  AGraphic: TGraphic;
  APicture: TPicture;
  AProperties: TcxImagePropertiesAccess;
begin
  Result := (AItem.GetProperties is TcxImageProperties) and ExportImagesAsGraphic;
  if not Result then Exit;
  AProperties := TcxImagePropertiesAccess(AItem.GetProperties);
  ...
  try
    ...
    if (AGraphic <> nil) and not cxRectIsEmpty(ABounds) then
    begin
      AClientRect := Rect(0, 0, cxRectWidth(ABounds), cxRectHeight(ABounds));
      if AProperties.GetRealStretch(Size(APicture.Width, APicture.Height), cxRectSize(AClientRect)) then
      begin
      ...
      Provider.SetCellPNGImage(AColumn, ARow, GraphicToPNGImage(AClientRect, R, APicture.Graphic));
    end;
  finally
    APicture.Free;
  end;
...
begin
  ...
  if AItem <> nil then
  begin
    if not Provider.SupportGraphic or not SetCellAsGraphic(ARealBounds, R.Left, R.Top, ARecord, AItem) then
...
begin
  ...
  AViewInfo := ARecordViewInfoClass.Create(
    ViewInfo.RecordsViewInfo, ARow) as TcxGridDataRowViewInfo;
  try
    ...
    ALeft := AddIndents(ATop, RecordHeight{AViewInfo.ContentHeight}, ARow, ARow.Expandable);
    ...
    begin
      ...
      begin
        ...
        if AIsFooter then
          ...
          AValue := AValues^[J];
        ADisplayText := AItem.FormatValue(AValue, True);
        if ADisplayText = '' then
        begin
          ADisplayText := cxXMLEmptyChar;
          AParams.TextColor := AParams.Color;
        end;
        AFooterCell := AddVisualItemEx(GetFooterItemBounds(I, ALineIndex, R, AIsFooter), ADisplayText,
          AParams, AColumn.FooterAlignmentHorz, cxBordersAll, FooterCellBorderColor);
...
begin
  ...
  begin
    ...
    begin
      if AItem.Data2 is TcxCustomGridTableItem then
      begin
...
function TcxGridTableViewExport.GetGroupRowColumnIntersection(
  const ARowBounds: TRect; AColumn: TcxCustomGridTableItem): TRect;
var
  I: Integer;
begin
  Result := ARowBounds;
...
begin
  for I := 0 to PatternsList.Count - 1 do
    with Patterns[I] do
    begin
      Bounds.Top := (Bounds.Top div ARowHeight) * (AHeight div RecordRowCount);
      Bounds.Bottom := (Bounds.Bottom div ARowHeight) * (AHeight div RecordRowCount);
    end;
...
procedure TcxGridLayoutViewExport.CreateExportCache;
...
begin
  try
    ...
    try
      dxLayoutStoreItemStates(AList, Container);
      dxLayoutSetItemStates(Container, True, False, False, False);
      UpdateGridViewSize;
      inherited CreateExportCache;
      dxLayoutRestoreItemStates(AList, Container);
    finally
      AList.Free;
    end;
  finally
...
procedure TcxGridLayoutViewExport.CreateExportCells;
var
...
procedure TcxGridLayoutViewExport.ExportCardCaption(
  AInfo: TcxGridLayoutViewRecordCaptionViewInfo);
var
  P: TcxViewParams;
begin
  if not AInfo.Visible then Exit;
  P := AInfo.Params;
  P.Color := Painter.DefaultGroupColor; 
  AddVisualItemEx(GetRelativeBounds(AInfo.Bounds), AInfo.Text, P,
    AInfo.AlignmentHorz, cxBordersAll, Painter.DefaultGridLineColor);
end;

procedure TcxGridLayoutViewExport.ExportEditItem(
  const ABounds: TRect; AInfo: TcxGridLayoutViewItemViewInfo);
var
  AStyle: TcxCacheCellStyle;
  AViewParams: TcxViewParams;
begin
  AStyle := GetContentParams(Card, AInfo.Item,
    AViewParams, GridLines, GridLineColor);
  AddVisualDataItem(ABounds,
    Provider.RegisterStyle(AStyle), Card, AInfo.Item);
end;

procedure TcxGridLayoutViewExport.ExportLayoutItem(
  AInfo: TdxCustomLayoutItemViewInfo);
var
  I: Integer;
  R: TRect;
  AParams: TcxViewParams;
...
begin
  ...
  begin
    ...
    AItem := AddVisualItemEx(R, '',  AParams, taLeftJustify, [], clDefault, True);
    GetItemBordersInfo(AInfo, AItem.ItemBorders, AItem.ItemBorderColor);
  end;
  GetItemBordersInfo(AInfo, AItem.ItemBorders, AItem.ItemBorderColor);
  //
  ...
  begin
    ...
    AddVisualItemEx(R, AInfo.CaptionViewInfo.Text, AParams,
      AInfo.CaptionViewInfo.AlignHorz, [], clDefault, False, False);
  end;
  //
  if AInfo is TcxGridLayoutItemViewInfo then
    ExportEditItem(GetRelativeBounds(TcxGridLayoutItemViewInfo(AInfo).ControlViewInfo.Bounds),
...
procedure TcxGridLayoutViewExport.GetItemBordersInfo(
  const AItemInfo: TdxCustomLayoutItemViewInfo;
  var ABorders: TcxBorders; var ABorderColor: TColor);
begin
  ABorders := [];
  ABorderColor := clDefault;
  if not TdxLayoutItemViewInfoAccess(AItemInfo).HasBorder and
    (AItemInfo.ParentViewInfo <> nil) then Exit; 
...
begin
  Result := cxRectOffset(cxRectOffset(AItemBounds,
    CardViewInfo.Bounds.TopLeft, False), CardBounds.TopLeft);
end;

function TcxGridLayoutViewExport.HasCaption(
  AInfo: TdxCustomLayoutItemViewInfo): Boolean;
begin
  Result := not (TdxLayoutItemViewInfoAccess(
    AInfo).Item.CaptionOptions is TdxLayoutNonLabeledItemCaptionOptions)
end;
...
procedure ExportGridToXLSX(const AFileName: string; AGrid: TcxGrid;
  AExpand: Boolean; ASaveAll: Boolean; AUseNativeFormat: Boolean; const AFileExt: string);
begin
  ExportGridToFile(AFileName, cxExportToXLSX, AGrid,
    AExpand, ASaveAll, AUseNativeFormat, '', '', '', AFileExt);
end;

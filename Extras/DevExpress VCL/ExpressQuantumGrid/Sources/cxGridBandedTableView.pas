...
uses
  Types, Variants,
  Windows, Classes, Graphics, Controls, Forms, ComCtrls, Menus,
  dxCore, cxClasses, cxControls, cxGraphics, cxLookAndFeels, cxLookAndFeelPainters,
  cxStyles, cxStorage, cxPC,
...
type
  ...
  private
    function GetBandsViewInfo: TcxGridBandsViewInfo; {$IFDEF DELPHI9} inline; {$ENDIF}
  protected
    procedure DrawItems; override;
  ...
  private
    ...
    FWidth: Integer;
    function GetBand: TcxGridBand;
    function GetBorderOverlapSize: Integer; {$IFDEF DELPHI9} inline; {$ENDIF}
  ...
  public
    ...
    property GridView: TcxGridBandedTableView read GetGridView;
  published
    property BandCaptionsInColumnAlternateCaption: Boolean read FBandCaptionsInColumnAlternateCaption
      write SetBandCaptionsInColumnAlternateCaption default False;
  ...
  begin
    ...
    if not Result and not SourceBand.Visible and
      ...
      case SourceBand.FixedKind of
        fkLeft:
          Result := (DestInsertPosition = posLeft) and
        ...
        fkRight:
          Result := (DestInsertPosition = posRight) and
      ...
      case DestInsertPosition of
        posLeft, posRight:
          Result := (DestBand.ParentBand <> SourceBand.ParentBand) or
            (DestBand.Position.VisibleColIndex <> GetNearestColIndex(DestInsertPosition = posLeft));
        posTop:
...
var
  ...
  begin
    ...
    begin
      ...
      if FDestInsertPosition = posRight then Inc(Result);
  ...
  begin
    if ABand.VisibleForCustomization then
      ...
      begin
        ...
        ItemObject := ABand;
        Text := ABand.GetAlternateCaption;
      end;
...
var
  ...
  AData: TData;
begin
  if not AFollowVisualOrder then
  begin
    Result := inherited FindNextItem(AFocusedItemIndex, AGoForward, AGoOnCycle,
      AFollowVisualOrder, ACycleChanged, ARecord);
    Exit;
  end;
  ...
  try
    ...
    try
      ...
      if FindNextCustomItem(-1, AColumns.Count, True, False,
        ...
        if AGoOnCycle then
        begin
          Result := FindNextItem(-1, AGoForward, False, AFollowVisualOrder, ACycleChanged, ARecord);
          ACycleChanged := True;
        end
        else
          Result := -1;
...
begin
  ...
  try
    ...
    AAutoWidthObject.AvailableWidth := GridViewInfo.ClientWidth +
      BandsViewInfo.GetRootItemsHorizontalBorderOverlapSize;
...
function CompareSummaryItems(Item1, Item2: Pointer): Integer;
var
  ASummaryItem1, ASummaryItem2: TcxDataSummaryItem;
begin
  ASummaryItem1 := TcxDataSummaryItem(Item1);
  ASummaryItem2 := TcxDataSummaryItem(Item2);
...
procedure TcxGridBandedFooterViewInfo.PrepareSummaryItems(ASummaryItems: TList);
begin
  inherited;
  ASummaryItems.Sort(CompareSummaryItems);
end;

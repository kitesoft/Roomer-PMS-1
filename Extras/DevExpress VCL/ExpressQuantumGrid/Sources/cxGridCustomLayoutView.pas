...
uses
  Types, Classes, Controls, Graphics, Windows, Forms, StdCtrls, Contnrs,
  dxCoreClasses, cxClasses, cxCustomData, cxLookAndFeelPainters, cxControls,
...
type
  ...
  protected
    ...
    property FirstCalculate: Boolean read FFirstCalculate;
  public
    constructor Create(AGridViewInfo: TcxCustomGridViewInfo); override;
    function NeedCalculate: Boolean;
  ...
  public
    class function GetExpandButtonSize(ALookAndFeelPainter: TcxCustomLookAndFeelPainter): Integer; virtual;
  ...
  private
    function GetOptionsCustomize: TcxGridCustomLayoutViewOptionsCustomize; {$IFDEF DELPHI9} inline; {$ENDIF}
    function GetOptionsView: TcxGridCustomLayoutViewOptionsView; {$IFDEF DELPHI9} inline; {$ENDIF}
    function GetViewInfo: TcxGridCustomLayoutViewViewInfo; {$IFDEF DELPHI9} inline; {$ENDIF}
    procedure SetOptionsCustomize(Value: TcxGridCustomLayoutViewOptionsCustomize); {$IFDEF DELPHI9} inline; {$ENDIF}
    procedure SetOptionsView(Value: TcxGridCustomLayoutViewOptionsView); {$IFDEF DELPHI9} inline; {$ENDIF}
  protected
...
uses
  Math, SysUtils, cxGeometry, cxGridCommon;
...
constructor TcxGridCustomLayoutViewItemFilterButtonViewInfo.Create(
  AGridViewInfo: TcxCustomGridViewInfo);
begin
  inherited Create(AGridViewInfo);
  FFirstCalculate := True;
end;

function TcxGridCustomLayoutViewItemFilterButtonViewInfo.NeedCalculate: Boolean;
begin
  Result := FirstCalculate or Visible;
end;

procedure TcxGridCustomLayoutViewItemFilterButtonViewInfo.AfterCalculateBounds(var ABounds: TRect);
begin
  inherited AfterCalculateBounds(ABounds);
  FFirstCalculate := False;
end;
...
function TcxGridCustomLayoutViewItemFilterButtonViewInfo.GetDropDownWindow: TcxCustomGridPopup;
begin
  Result := TcxGridCustomLayoutView(GridView).Controller.FilterPopup;
end;

function TcxGridCustomLayoutViewItemFilterButtonViewInfo.GetDropDownWindowValue: TcxGridFilterPopup;
begin
...
begin
  Result := FFirstCalculate and PtInRect(Bounds, GridViewInfo.MousePos);
end;
...
begin
  Result := TcxGridCustomLayoutView(GridView).OptionsView.ShowItemFilterButtons = sfbAlways;
end;

{ TcxGridCustomLayoutRecordViewInfo }

class function TcxGridCustomLayoutRecordViewInfo.GetExpandButtonSize(ALookAndFeelPainter: TcxCustomLookAndFeelPainter): Integer;
begin
...
function TcxGridCustomLayoutView.IsRecordPixelScrolling: Boolean;
begin
  Result := False;
end;

...
function TcxGridLayoutViewCustomizationFormGroupViewInfo.CalculatePadding: TRect;
begin
...
constructor TcxGridViewLayoutControl.Create(AOwner: TComponent);
begin
...
procedure TcxGridLayoutViewCustomizationForm.FormKeyDown(Sender: TObject;
...
begin
  if (Controller = nil) or (Controller.GridView.Control = nil) then
    Result := 0
  else
...
begin
  FLayoutLookAndFeel.Assign(TcxGridLayoutViewAccess(GridView).LayoutLookAndFeel);
...
begin
  ...
  FGridViewLayoutControl.MenuItems := GridView.Container.MenuItems;

  FGridViewLayoutControl.OptionsItem.SizableHorz := TdxLayoutContainerAccess(GridView.Container).IsSizableHorz;
  FGridViewLayoutControl.OptionsItem.SizableVert := TdxLayoutContainerAccess(GridView.Container).IsSizableVert;
...
procedure TcxGridLayoutViewCustomizationForm.FormShortCut(var Msg: TWMKey;
  var Handled: Boolean);
begin
  inherited;
  if not Handled then
        ...
        begin
          ...
          if Handled then
            if tvItems.Selected <> nil then
              tvItems.Selected.EndEdit(False)
            else

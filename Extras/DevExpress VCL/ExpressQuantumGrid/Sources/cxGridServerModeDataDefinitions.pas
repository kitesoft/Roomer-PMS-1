...
begin
  ...
  ADescriptor.DateTimeGrouping := AConvertMap[TCustomGridTableItemAccess(AItem).GetDateTimeGrouping];
end;
...
begin
  ...
  try
    ...
    AHasDisplayText := AProperties.GetEditValueSource(False) = evsValue;
    for I := 0 to AValueList.Count - 1 do
    begin
      AItem := AValueList[I];
      if AHasDisplayText then
        ADisplayText := AProperties.GetDisplayText(AValueList[I].Value, True)
      else
        ADisplayText := AValueList[I].DisplayText;
...
begin
  ...
  try
    ...
    BeginUpdateFields;
    try
      ACreateAllFields := not HasDataSourcePersistentFields;
      for I := 0 to DataSource.Fields.Count - 1 do
      begin
        if not (ACreateAllFields or IsDataSourcePersistentField(DataSource.Fields[I])) then
          Continue;

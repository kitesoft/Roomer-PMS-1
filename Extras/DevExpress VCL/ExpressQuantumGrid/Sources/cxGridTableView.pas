...
type
  ...
  private
    ...
    function GetPopupRowHeight: Integer;

    function GetController: TcxGridTableController; {$IFDEF DELPHI9} inline; {$ENDIF}
    function GetDataWidth: Integer;
    function GetGridView: TcxGridTableView; {$IFDEF DELPHI9} inline; {$ENDIF}
    ...
    function GetRecordsViewInfo: TcxGridRowsViewInfo; {$IFDEF DELPHI9} inline; {$ENDIF}
    function GetViewData: TcxGridViewData; {$IFDEF DELPHI9} inline; {$ENDIF}
  protected
    ...
    function HasFirstBorderOverlap: Boolean; {$IFDEF DELPHI9} inline; {$ENDIF}
    procedure Offset(DX, DY: Integer); override;
  ...
  public
    ...
    function CanPopupAutoHeight: Boolean; override;
...
uses
  RTLConsts, SysUtils, Math, Contnrs, dxOffice11, cxVariants, cxDataUtils,
  cxFilterControlUtils, cxLibraryConsts, cxGridRows, cxGridStrs, dxCore, cxGeometry;
...
begin
  ...
  begin
    ATopRecordIndex := TopRecordIndex + Max(1, GetPageRecordCount - 1);
...
begin
  ...
  begin
    AVisibleRowCount := GetPageVisibleRecordCount(TopRecordIndex, False);
...
function TcxGridTableViewInfo.GetPopupRowHeight: Integer;
begin
  Result := RecordsViewInfo.DataRowHeight;
  if GridView.Preview.Visible then
    Inc(Result, GridView.Preview.GetFixedHeight);
end;
...
var
  ARowCount: Integer;
begin
  ARowCount := (AHeight - GetNonRecordsAreaHeight(True)) div GetPopupRowHeight;
  ...
  Result := GetNonRecordsAreaHeight(True) + ARowCount * GetPopupRowHeight;
end;

function TcxGridTableViewInfo.GetPopupHeight(ADropDownRowCount: Integer): Integer;
begin
  Result := GetNonRecordsAreaHeight(True) + ADropDownRowCount * GetPopupRowHeight;
...
procedure TcxGridColumn.SetHeaderImageIndex(Value: TcxImageIndex);
begin
...
function TcxGridColumn.IsFilterRowIncrementalFiltering: Boolean;
begin
...
function TcxGridColumn.HasGlyph: Boolean;
begin
...
begin
  Result := inherited IsEqualHeightRecords and
    ...
    (GroupedColumnCount = 0) and not IsMaster;
end;
...
function TcxGridTableView.CanPopupAutoHeight: Boolean;
begin
  Result := inherited CanPopupAutoHeight or
    (Preview.Visible and Preview.AutoHeight);
end;

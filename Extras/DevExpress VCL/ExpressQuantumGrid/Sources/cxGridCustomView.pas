...
type
  ...
  private
    FGridView: TcxCustomGridView;
    function GetControl: TcxControl; {$IFDEF DELPHI9} inline; {$ENDIF}
    ...
    function GetPainter: TcxCustomGridPainter; {$IFDEF DELPHI9} inline; {$ENDIF}
  ...
  TcxCustomGridCustomizationForm = class(TForm, IcxGridCustomizationForm)
  private
...
{$ENDIF}
  ...
  TcxGridIndexes = array of Integer;
  ...
  private
    ...
    function GetCheckedIndexes: TcxGridIndexes;
    function GetGridView: TcxCustomGridView;
    procedure SetChecked(Index: Integer; Value: Boolean);
    procedure SetCheckedIndexes(const Value: TcxGridIndexes);
    procedure SetPressedItemIndex(Value: Integer);
  ...
  protected
    ...
    function CalculateClientHeight(AItems: TStrings): Integer; virtual;
    function CalculateClientWidth(AItems: TStrings): Integer; virtual;
    function CalculateItemHeight: Integer; virtual;
    procedure DoAction(AItemIndex: Integer); virtual;
    procedure DrawItemCheck(ACanvas: TcxCanvas; AIndex: Integer;
      const ACheckAreaRect: TRect; AState: TOwnerDrawState); virtual;
    ...
    function CanHaveCheck(AItemIndex: Integer): Boolean; virtual;
    procedure ChangeChecked(AItemIndex: Integer); virtual;
    function GetCheckAreaHeight: Integer;
    function GetCheckAreaWidth: Integer;
    function HasCheck(AItemIndex: Integer): Boolean; virtual;
    function IsCheck(AItemIndex: Integer): Boolean;
    property Checked[Index: Integer]: Boolean read GetChecked write SetChecked;
    property CheckedIndexes: TcxGridIndexes read GetCheckedIndexes write SetCheckedIndexes;
    property PressedItemIndex: Integer read FPressedItemIndex write SetPressedItemIndex;
  ...
  public
    ...
    procedure IMEStartComposition; virtual;
  ...
  protected
    ...
    procedure RebuildDataGroupRecursive(AParentNode: TcxDataGroupNode; var ABeginRecordListIndex: Integer);
  ...
  private
    ...
    FSize: TSize;
    FViewInfo: TcxCustomGridViewInfo;
    function GetContainer: TcxControl; {$IFDEF DELPHI9} inline; {$ENDIF}
    function GetController: TcxCustomGridController; {$IFDEF DELPHI9} inline; {$ENDIF}
    function GetGridView: TcxCustomGridView; {$IFDEF DELPHI9} inline; {$ENDIF}
  ...
  protected
    ...
    procedure CheckOverpan(AScrollKind: TScrollBarKind;
      ANewDataPos, AMinDataPos, AMaxDataPos: Integer; ADeltaX, ADeltaY: Integer); override;
  ...
  {$ENDIF}
    ...
    function IsMenuKey(var Message: TWMKey): Boolean; override;
  ...
  private
    ...
    function GetCalculated: Boolean; {$IFDEF DELPHI9} inline; {$ENDIF}
    function GetCanvas: TcxCanvas;
    function GetClientBounds: TRect;
  ...
  private
    ...
    function GetInternalItem(Index: Integer): TcxCustomGridViewInfoCacheItem; {$IFDEF DELPHI9} inline; {$ENDIF}
  ...
  protected
    ...
    property ShowLockedStateImageOptions: TcxCustomGridShowLockedStateImageOptions
      read FShowLockedStateImageOptions write SetShowLockedStateImageOptions;
    property SuppressHintOnMouseDown: Boolean
      read FSuppressHintOnMouseDown write FSuppressHintOnMouseDown default True;
  public
    constructor Create(AGridView: TcxCustomGridView); override;
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
  ...
  private
    FGridView: TcxCustomGridView;
    function GetItem(Index: Integer): TcxUpdateControlInfo; {$IFDEF DELPHI9} inline; {$ENDIF}
  public
    constructor Create(AGridView: TcxCustomGridView);
  ...
  private
    ...
    function GetClone(Index: Integer): TcxCustomGridView; {$IFDEF DELPHI9} inline; {$ENDIF}
    function GetCloneCount: Integer; {$IFDEF DELPHI9} inline; {$ENDIF}
  ...
  protected
    ...
    procedure GridBeginLockedStatePaint(AMode: TcxGridShowLockedStateImageMode);
    procedure GridEndLockedStatePaint;
  ...
  TcxCustomGridViewAccess = class
    ...
    class function CanBeUsedAsDetail(AInstance: TcxCustomGridView): Boolean;
    class function CanBeUsedAsMaster(AInstance: TcxCustomGridView): Boolean;
    class function CanFocus(AInstance: TcxCustomGridView): Boolean;
    class procedure Deactivate(AInstance: TcxCustomGridView);
...
uses
  RTLConsts, 
  SysUtils, Math, cxGeometry, cxScrollBar, cxVariants, cxLibraryConsts,
  cxGrid, cxGridLevel, cxGridStrs, dxCore;
...
type
  ...
  private
    ...
    function GetItem(Index: Integer): TcxCustomGridHitTest; {$IFDEF DELPHI9} inline; {$ENDIF}
  protected
...
{$IFDEF DELPHI9}
  {$IFNDEF DELPHI11}
function TcxCustomGridCustomizationForm.GetOwnerWindow: HWND;
begin
...
procedure TcxCustomGridPopup.UpdateInnerControlsHeight(var AClientHeight: Integer);
begin
end;
...
procedure TcxCustomGridPopup.CorrectBoundsWithDesktopWorkArea(var APosition: TPoint);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      Height := AClientHeight + NCHeight;
    end;
    if (AHeight <> Height) and (Y = ADesktopWorkArea.Top) then
...
begin
  ...
  FCheckedIndexes := TdxFastList.Create;
  FPopup := APopup;
  FPressedItemIndex := -1;
  Init;
  Parent := FPopup;
...
begin
  ...
  begin
    ...
    begin
      ACanvas.Brush.Color := LookAndFeelPainter.DefaultSelectionColor;
      ACanvas.Font.Color := LookAndFeelPainter.DefaultSelectionTextColor;
    end;
  end;    
...
function TcxGridPopupListBox.CalculateClientHeight(AItems: TStrings): Integer;
begin
  Result := GetVisibleItemCount(AItems) * ItemHeight;
end;

function TcxGridPopupListBox.CalculateClientWidth(AItems: TStrings): Integer;
var
  I: Integer;
begin
  Result := 0;
  ...
  Result := Max(Result, GetScrollBarSize.cx);
end;

function TcxGridPopupListBox.CalculateItemHeight: Integer;
begin
  Result := cxTextHeight(Canvas.Handle) + 2 * FItemTextOffsetVert;
  if ShowChecks then
...
begin
  ...
  ACanvas.FillRect(ACheckAreaRect, GetBackgroundColor);
...
begin
  ...
  ClientHeight := CalculateClientHeight(AItems);
  ClientWidth := CalculateClientWidth(AItems);
  IntegralHeight := True;
...
begin
  Result := crDefault;
  if GridView.CanGetHitTest then
  begin
...
procedure TcxCustomGridController.IMEStartComposition;
begin
end;
...
begin
  ...
  begin
    ...
    for I := 0 to AParentNode.ChildCount - 1 do
      RebuildDataGroupRecursive(AParentNode.Children[I], ABeginRecordListIndex);
...
begin
  ...
  for I := 0 to FRootNode.ChildCount - 1 do
    RebuildDataGroupRecursive(FRootNode.Children[I], ABeginRecordListIndex);
  DataGroups.Rebuild;
end;
...
{$IFDEF DELPHI14}
procedure TcxGridSite.DoGetGestureOptions(var Gestures: TInteractiveGestures;
...
function TcxGridSite.IsMenuKey(var Message: TWMKey): Boolean;
begin
  Result := inherited IsMenuKey(Message);
  if not Result then
  begin
    Message.Result := 0;
...
begin
  SetWindowRgn(Handle, CreateRectRgnIndirect(AVisibleRect), False);
...
begin
  ...
  if Result or TcxCustomGrid(Container).UpdateLocked then
    Exit;
...
procedure TcxGridSite.WndProc(var Message: TMessage);
  ...
  function IsUnwantedMouseMessage: Boolean;
  begin
    if Locked then
    begin
      case Message.Msg of
        WM_MOUSEFIRST..WM_MOUSELAST, WM_MOUSEHOVER, WM_MOUSELEAVE:
          Result := True;
      else
        Result := False;
      end;
...
begin
  if IsUnwantedMouseMessage then
    Exit;
...
begin
  Result := not WasMovedOnMouseDown and Controller.StartDragAndDrop(P);
end;
...
begin
  inherited;
  Controller.IMEStartComposition;
end;
...
begin
  ...
  FActiveViewInfos := TdxFastList.Create;
...
begin
  if Site.HScrollBarVisible then
    Result := GetSystemMetrics(SM_CYHSCROLL) // Site.HScrollBar.Height
  else
    Result := 0;
end;
...
begin
  ...
  case Index of
    vsBackground:
      begin
        AParams.Color := LookAndFeelPainter.DefaultContentColor;
        AParams.TextColor := LookAndFeelPainter.DefaultEditorTextColor(True);
...
begin
  Result := Assigned(Control) and not TcxCustomGrid(Control).UpdateLocked;
end;

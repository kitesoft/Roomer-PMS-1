...
{$ENDIF}
  ...
  cxLabel, cxPC, cxVariants, cxCustomData, cxData, cxDataStorage, dxOffice11,
  cxGridCommon, cxGridCustomView, dxGDIPlusClasses, dxGDIPlusApi;
...
//  function GetTransparentValues: Boolean; override;
  ...
  TcxGridCharStackedAreaDiagramViewInfo = class(TcxGridChartAreaDiagramViewInfo)
  private
...
{$ENDIF}
  ...
  cxTextEdit, cxLibraryConsts, cxGeometry, DateUtils;
...
procedure TcxGridChartSortBySeriesComboBox.DrawItem(AControl: TcxCustomComboBox;
  ...
  begin
    Result := ARect;
    AKeySize := cxTextHeight(ACanvas.Handle) - 2 * KeyOffset;
    with Result do
    begin
...
begin
  ...
  begin
    ...
    ACanvas.Rectangle(GetKeyBounds, AParams, cxBordersAll, clWindowText, 1, True);
...
begin
  ...
  with ACanvas do
  begin
    Rectangle(R, AParams, cxBordersAll, LookAndFeelPainter.DefaultChartToolBoxDataLevelInfoBorderColor,
      ABorderSize);
  ...
  begin
    ...
    try
      ...
      AGraphics.Pie(R, AStartAngle, ASweepAngle,
        RealPenColor, Canvas.Brush.Color, PenWidth - 0.2, PenStyle);
    finally
...
begin
  ...
  try
    AGraphics.SmoothingMode := SmoothingMode;                    
    AGraphics.Polyline(Points, RealPenColor, PenWidth - 0.2, PenStyle);
  finally
    dxGpEndPaint(AGraphics);
  end;
end;
...
begin
  Canvas.Rectangle(ViewInfo.LegendKeyBounds, ViewInfo.LegendKeyParams,
    ViewInfo.LegendKeyBorders, Dark(ViewInfo.LegendKeyParams.Color, 75));
end;
...
class procedure TcxGridChartLineDiagramValuePainter.DrawLines(ACanvas: TcxCanvas;
  const APoints: TPointArray; AStyle: TcxGridChartLineStyle; AWidth: Integer;
...
begin
  with ValueViewInfo do
    Diagram.FOnCustomDrawValueCaption(Diagram, ACanvas, Self, Result);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Diagram.AxisValue.MinMaxValues = mmvAuto) and (I = 0) then
      begin
        AMinValue := Min(0, AMinSummaryValue);
        AMaxValue := Max(0, AMaxSummaryValue);
      end
      else
      begin
...
begin
  ...
  for AValueIndex := 0 to VisibleValueCount - 1 do
    ...
    begin
      ...
      ACategoryFinish := GetUnitOffset(ACategoryOffset + ColumnDiagramValueSizeUnits);
...
begin
  Result := SeriesGroupCount * ColumnDiagramValueSizeUnits + 2 * ColumnDiagramValueOffsetUnits;
end;
...
begin
  ...
  ACaptionWidth := CaptionViewInfo.CalculateWidth + cxTextOffset * 2;
  ACaptionHeight := CaptionViewInfo.CalculateHeight + cxTextOffset * 2;
...
{ TcxGridCharStackedAreaDiagramViewInfo }

procedure TcxGridCharStackedAreaDiagramViewInfo.CalculateValues;
var
  R: TRect;
  AEmptyPointFound: Boolean;
...
function TcxGridCharStackedAreaDiagramViewInfo.ExcludeEachSeriesArea: Boolean;
begin
  Result := True;
end;

function TcxGridCharStackedAreaDiagramViewInfo.GetValue(ASeriesIndex, AIndex: Integer): Variant;
var
  I: Integer;
begin
  Result := GetStackedValue(ASeriesIndex, AIndex);
...
function TcxGridCharStackedAreaDiagramViewInfo.GetSeriesCountAt(AValueIndex: Integer): Integer;
var
  ASeriesIndex: Integer; 
begin
  Result := 0;
...
function TcxGridCharStackedAreaDiagramViewInfo.GetDiagram: TcxGridChartStackedAreaDiagram;
begin
...
begin
  ...
  if not GridView.OptionsView.TransparentCaptions then
    Inc(ACaptionHeight, cxTextOffset * 2);
...
begin
  Result := TcxGridCharStackedAreaDiagramViewInfo;
end;

...
type
  ...
  protected
    ...
    function ConvertPoints(P: PByte; ACount: Integer; AIsSmallPoints: Boolean): TPointArray;
    ...
    procedure ProcessMetaFileObject(ATable: PHandleTable;
...
begin
  if AFont1.HandleAllocated and AFont2.HandleAllocated then
    Result := AFont1.Handle = AFont2.Handle
  else
    Result := False;

  if not Result then
    Result := (AFont1.Name = AFont2.Name) and (AFont1.Height = AFont2.Height) and
      (AFont1.Size = AFont2.Size) and (AFont1.Style = AFont2.Style);
end;
...
function TdxPSPDFMetaFileExportProvider.ConvertPoints(
  P: PByte; ACount: Integer; AIsSmallPoints: Boolean): TPointArray;
var
  I: Integer;
begin
...
begin
  ...
  PageContent.BeginPaint(PDFCanvas.WindowOrg, PDFCanvas.Region.Handle);
  try
    ...
    begin
      ...
      begin
        APoints := ConvertPoints(ACursor, APolygon^.aPolyCounts[I], ASmallPoints);
        PageContent.WritePoints(APoints);
        APoints := nil;
      end;
...
begin
  ...
  try
    for I := 0 to ACount - 1 do
    begin
      APoints := ConvertPoints(ACursor, APolyline^.aPolyCounts[I], ASmallPoints);
      PageContent.Polyline(APoints, PenWidth, Pen.Color);
...
begin
  PDFCanvas.Pie(ConvertRect(ARecord.rclBox),
    ConvertPoint(ARecord.ptlStart), ConvertPoint(ARecord.ptlEnd), BackgroundColor);
end;
...
begin
  ...
  try
    ...
    ARect := ConvertRect(AInfo.TextBounds);
    if AInfo.Options and ETO_CLIPPED = 0 then
      ATextWidth := cxRectWidth(ARect) + 1
    else
      ATextWidth := AInfo.TextWidth;

    if CompareFonts(PDFCanvas.Font, AInfo.Font) then
      PDFCanvas.DrawTextLine(ARect, ARect, ATextWidth, AInfo.Text, State.TextColor)
    else
    begin
...
begin
  Result := (Options = AInfo.Options) and CompareFonts(Font, AInfo.Font) and
...
procedure TdxPSPDFMetaFileTextInfo.Initialize(const R: PEMRExtTextOut; AIsUnicode: Boolean; AFont: TFont);
  ...
  function ExtractText(const R: PEMRExtTextOut; AIsUnicode: Boolean): WideString;
  var
  ...
  begin
    ...
    begin
      ...
      Result := dxAnsiStringToWideString(ATextA, dxGetCodePageFromCharset(AFont.Charset));
    end;
...
begin
  ...
  begin
    FTextBounds := Bounds(0, 0, TextWidth, Abs(AFont.Size));
    FTextBounds := cxRectOffset(FTextBounds, R^.emrtext.ptlReference);
  end;
  Inc(FTextBounds.Bottom, Abs(AFont.Size));
end;

...
type
  ...
  public
    ...
    procedure IntersectClipRgnEx(ARegion: TcxRegionHandle);
...
type
  ...
  private
    ...
    procedure CorrectTextRowsExtends;
  ...
  public
    ...
    function CalculateTextOutParams(const ABounds: TRect; AMaxLineCount: Integer; const AText: string): TRect; virtual;
...
procedure TdxPSPDFExportCustomTextHelper.CorrectTextRowsExtends;
var
  ARow: PcxTextRow;
  I: Integer;
begin
...
begin
  inherited Create;
  FProvider := AProvider;
  FRegion := TcxRegion.Create(Bounds(0, 0, cxMaxRegionSize, cxMaxRegionSize));
...
procedure TdxPSPDFReportRenderCanvas.DrawTextLine(const ATextRect: TRect;
  ...
  begin
    ALineThickness := 1 / PageContent.CoordsConversionFactor;
    ...
    try
      ...
      begin
        ...
        try
          ...
          PageContent.DrawText(ATextRect, AText, ADirection, CalculateCharsSpacing, ABreakExtraSpacing);
        finally
          PageContent.EndPaint;
        end;
      end;
...
var
  R1: TRect;
begin
  Region.Combine(AbsoluteRect(R), roSubtract);
...
var
  R1: TRect;
begin
  Region.Combine(AbsoluteRect(R), roIntersect);
  Result := GetRgnBox(Region.Handle, R1);
end;

procedure TdxPSPDFReportRenderCanvas.IntersectClipRgnEx(ARegion: TcxRegionHandle);
var
...
begin
  Region.Combine(AbsoluteRect(R), roSet);
end;
...
procedure TdxPSPDFReportRenderCanvas.SelectFonT(AFont: TFont; ATextColor: TColor);
  ...
  function GetRealFontSize(AFont: TFont): Integer;
  var
    ATempFont: TFont;
  begin
  ...
  function GetRealTextColor(ATextColor: TColor): TColor;
  begin
    Result := ATextColor;
...
begin
  ...
  PageContent.FontColor := GetRealTextColor(ATextColor);
  PageContent.FontSize := GetRealFontSize(Font);
end;

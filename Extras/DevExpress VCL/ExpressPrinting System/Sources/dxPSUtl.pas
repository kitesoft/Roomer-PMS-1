...
uses
  ...
  IniFiles, {$IFDEF DELPHI16} UITypes,{$ENDIF} cxClasses, cxGraphics, dxPSGlbl,
  dxPrnDev, dxCore, cxTextEdit, cxLabel, cxDropDownEdit, cxGeometry, cxButtons,
  cxLookAndFeels, cxControls, ExtCtrls;
...
procedure CheckDialogFormHelpContext(AForm: TCustomForm;
  AHelpBtn, ACancelBtn, AOkBtn: TControl);
...
procedure PlaceButtons(const AButtons: array of TcxButton; ABtnOffsetX, ARightOrigin, ATopOrigin: Integer);
...
begin
  ...
  if AForm.HelpContext = 0 then
  begin
    AOkBtn.BoundsRect := ACancelBtn.BoundsRect;
    ACancelBtn.BoundsRect := AHelpBtn.BoundsRect;
    AHelpBtn.Visible := False;
  end;
end;
...
begin
  ...
  begin
    ...
    try
      ...
      FShellLargeImages.Handle := SHGetFileInfo('', 0, FileInfo,
        SizeOf(FileInfo), SHGFI_SYSICONINDEX or SHGFI_LARGEICON);
    finally
      if FileInfo.hIcon <> 0 then
        Windows.DestroyIcon(FileInfo.hIcon);
    end;
  end;
...
begin
  ...
  begin
    ...
    try
      FillChar(FileInfo, SizeOf(FileInfo), 0);    
      FShellSmallImages.Handle := SHGetFileInfo('', 0, FileInfo,
...
var
  ...
  procedure PlaceButton(AButton: TcxButton; AOriginX, AOriginY: Integer);
  begin

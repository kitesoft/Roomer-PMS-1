...
type
  ...
  public
    ...
    destructor Destroy; override;
    function CanEncodeText(const S: WideString): Boolean; override;
...
function TdxPSPDFTrueTypeFont.CanEncodeText(const S: WideString): Boolean;
begin
  Result := True;
end;
...
function TdxPSPDFCIDFont.CanEncodeText(const S: WideString): Boolean;
var
  I: Integer;
begin
  Result := True;
  for I := 1 to Length(S) do
    Result := Result and (TTFFile.GlyphIndex[Word(S[I])] <> 0);
end;
...
function TdxPSPDFFontsManager.GetFontForSymbol(
  const ASymbol: WideChar; ACurrentFont: TdxPSPDFCustomFont): TdxPSPDFCustomFont;
var
...
begin
  ...
  begin
    ...
    Result := ACurrentFont.Owner[ACurrentFont.Owner.Add(FCachedFont, True, True)];
  end
  else
    Result := ACurrentFont;
end;

function TdxPSPDFFontsManager.GetFontInfoForSymbol(
  const ASymbol: WideChar; out AInfo: TdxPSPDFFontGlyphsInfo): Boolean;

...
uses
  Types, Classes, Windows, Messages, Graphics, Controls, StdCtrls, ExtCtrls, ImgList,
  ...
  dxPSGlbl, dxBase, dxPSCore, dxPSRes, cxDrawTextUtils, cxListBox, dxCore,
...
uses
  Themes, Variants, SysUtils, Forms, Math,
  dxThemeManager, cxContainer, cxVariants, cxDataUtils,
  dxPSUtl, dxPSEngn, dxPSImgs, dxExtCtrls;
...
procedure TdxStylesListBox.DoMeasureItem(AControl: TcxListBox; AIndex: Integer;
  var Height: Integer);
var
  AHeight: Integer;
...
begin
  ...
  if Assigned(AStyle) then
    AStyleFont := AStyle.Font
  else
  begin
    ReportLinkStyles.GetDefaultViewParamsByCaption(AText, nil, AParams);
    AStyleFont := AParams.Font;
  end;
  if Assigned(AStyleFont) then
    Canvas.Font := AStyleFont;
  Height := Max(cxTextHeight(Canvas.Handle), AHeight) + 2;
end;

function TdxStylesListBox.DrawItem(ACanvas: TcxCanvas; AIndex: Integer;
...
begin
  ...
  try
    AText := Items[AIndex];
    AStyle := ReportLinkStyles.StylesByCaption[AText];
    CalculateStyleBarViewInfo(AText, -1, AStyle, ARect, odSelected in AState, AViewInfo);
...
procedure TdxStylesListBox.CalculateStyleBarViewInfoSizes(AnIndex: Integer;
  ...
  begin
    ...
    if Assigned(AStyleFont) then
      Canvas.Font := AStyleFont;
    AFontInfoMaxWidth := Canvas.TextWidth(FormatFontInfo(AStyleFont)) + 2;
  end;
  ...
  try
    ...
    try
      ...
      begin
        ...
        CaptionMaxWidth := Max(W, CaptionMaxWidth);
      end;
      if AnIndex <> -1 then 
      begin
        WidthList.Count := 1;
    ...
    if CaptionMaxWidth <> 0 then
      Inc(CaptionMaxWidth, 6);  
...
begin
  ...
  ReportLinkStyles.GetDefaultViewParamsByCaption(AViewInfo.StyleCaption, nil, AParams);
  if Assigned(AStyle) then
  begin
  ...
  AViewInfo.BarFont.Color :=
    GetStyleBarItemTextColor(ASelected, AViewInfo.BarFont.Color, AViewInfo.BarColor);
  AViewInfo.StyleTextColor :=
    GetStyleBarItemTextColor(ASelected, AParams.TextColor, AViewInfo.BarColor);
...
function TdxStylesListBox.GetStyleBarItemTextColor(
  ASelected: Boolean; AColor, ABkColor: TColor): TColor;
begin
  if ASelected then
  ...
  begin
    ...
    if Result = ABkColor then
      Result := InvertColor(Result);
  end;
end;

function TdxStylesListBox.AreToolTipsNeeded: Boolean;
begin
  Result := IsHotTrackStyleViolateControlBounds and
    GetParentForm(Self).Active and dxPSGlbl.CanShowHints;
end;

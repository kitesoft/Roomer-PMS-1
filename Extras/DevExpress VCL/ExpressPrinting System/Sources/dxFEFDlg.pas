...
uses
  ...
  {$IFDEF DELPHI16} UITypes,{$ENDIF} dxPSForm, dxBkgnd, cxLookAndFeelPainters,
  cxButtons, cxControls, cxContainer, cxEdit, cxGroupBox, cxLabel, cxGraphics,
  cxTextEdit, cxMaskEdit, cxDropDownEdit, cxColorComboBox, cxPC, dxExtCtrls,
  dxPSReportRenderCanvas, cxLookAndFeels;
...
type
  ...
  TdxFEFDialog = class(TCustomdxPSForm)
    ...
    dgPattern: TDrawGrid;
    dgTexture: TDrawGrid;
    lblBackground: TcxLabel;
    lblForeground: TcxLabel;
    lblPaintMode: TcxLabel;
    lblSample: TcxLabel;
    miCopy: TMenuItem;
    miCut: TMenuItem;
    miDelete: TMenuItem;
    ...
    pnlPatternName: TcxLabel;
    pnlPictureName: TcxLabel;
    pnlPreview: TcxGroupBox;
    pnlTextureName: TcxLabel;
    tshPattern: TcxTabSheet;
    tshPicture: TcxTabSheet;
    tshTexture: TcxTabSheet;
    bvlPictureHolder: TBevel;
    ilMenu: TcxImageList;
...
uses
 {$IFDEF USEJPEGIMAGE} 
  Jpeg,
 {$ENDIF}
  ClipBrd, cxClasses, dxPSGlbl, dxPcPrVw, dxPSPopupMan, dxPSImgs, dxPSRes, dxPSUtl;
...
begin
  ...
  sbxPicture.Parent := bvlPictureHolder.Parent;
  sbxPicture.BoundsRect := bvlPictureHolder.BoundsRect;
...
begin
  inherited Loaded;
  dgTexture.DefaultColWidth := (dgTexture.Width - GetSystemMetrics(SM_CXHSCROLL) - 1) div dgTexture.ColCount - 1;
  dgTexture.DefaultRowHeight := dgTexture.DefaultColWidth;
  dgTexture.Height := dgTexture.Height - 1;
  dgTexture.Width := dgTexture.Width - 1;

  dgPattern.DefaultColWidth := (dgPattern.Width - 1) div dgPattern.ColCount - 1;
  dgPattern.DefaultRowHeight := (dgPattern.Height - 1) div dgPattern.RowCount - 1;
  dgPattern.Height := (dgPattern.DefaultRowHeight + 1) * dgPattern.RowCount + 4;
  dgPattern.Width := (dgPattern.DefaultColWidth + 1) * dgPattern.ColCount;
...
begin
  ...
  AIniFile.WriteInteger(ASectionName, sdxActivePage, PageControl1.ActivePage.PageIndex);
end;
...
procedure TdxFEFDialog.LoadImage(var AImage: TGraphic; AWhat: Integer);
  ...
  begin
    Result := Pos('ico', GraphicFilter(TGraphic)) > 0;
    if Result then
...
begin
  FreeAndNil(FbmpCurrentPattern);
  FbmpCurrentPattern := dxCreateMappedBmp(FbmpPattern,
    [clWhite, clBlack], [BackColor, ForeColor]);
end;
...
begin
  FApplied := False;
  FPreviewWhat := TdxFillAs(PageControl1.ActivePage.PageIndex + 1);
  FreeAndNil(FPicture);

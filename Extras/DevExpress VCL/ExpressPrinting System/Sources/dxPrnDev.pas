...
procedure dxDrawPrinter(ACanvas: TCanvas; ARect: TRect; const AText: string;
  AImageList: TCustomImageList; AImageIndex: Integer; ADrawBackground: Boolean = True);
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        ANeeded := 0;
        if GetPrinter(HPrinter, 9, nil, 0, @ANeeded) or (ANeeded <> 0) then
        begin
          PI9 := GlobalAllocPtr(GPTR, ANeeded);
          if PI9 <> nil then 
          try
            if GetPrinter(HPrinter, 9, PI9, ANeeded, @ANeeded) then
            begin
...
procedure dxSetDefaultPrinter(ADevice: PChar);
  ...
  procedure InternalSendMessageTimeOut(ALParam: LPARAM;
    dwNeeded: TdxNativeUInt);
  begin
    SendMessageTimeout(HWND_BROADCAST, WM_SETTINGCHANGE, 0,
      ALParam, SMTO_NORMAL, 1000, {$IFDEF DELPHI16}@{$ENDIF}dwNeeded);
  end;
        ...
        try
          if GetPrinter(HPrinter, 2, PI2, dwNeeded, @dwNeeded) then 
            ...
            begin
              ...
              InternalSendMessageTimeOut(LPARAM(PChar('windows')), dwNeeded);
            end
              ...
              begin
                ...
                begin
                  dwNeeded := Length(S);
                  InternalSendMessageTimeOut(0, dwNeeded);
                end;
              end;  
        finally
          GlobalFreePtr(PI2);
...
begin
  ...
  Offset := (ARect.Bottom - ARect.Top - AImageList.Height) div 2;
  Inc(ARect.Left, Offset);
  Inc(ARect.Top, Offset);
  AImageList.Draw(ACanvas, ARect.Left, ARect.Top, AImageIndex);
  // Text
  Inc(Offset);
  Inc(ARect.Left, Offset + AImageList.Width + Offset);
  ACanvas.Brush.Style := bsClear;
  ACanvas.TextRect(ARect, ARect.Left + Offset, ARect.Top + Offset, AText);
...
begin
  ...
  {$IFDEF DELPHI12}
    @FGetDefaultPrinter := GetProcAddress(LibHandle, 'GetDefaultPrinterW');
    @FSetDefaultPrinter := GetProcAddress(LibHandle, 'SetDefaultPrinterW');
  {$ELSE}

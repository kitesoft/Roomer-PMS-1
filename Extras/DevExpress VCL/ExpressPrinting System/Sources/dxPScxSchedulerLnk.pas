...
{$ENDIF}
  ...
  dxCore, cxGraphics, cxClasses, cxControls, cxStyles, cxLookAndFeels, cxGeometry,
  cxContainer, cxLookAndFeelPainters, cxDateUtils, dxMessages,
  cxEdit, cxLabel, cxButtons, cxTextEdit, cxMaskEdit, cxGroupBox, cxDrawTextUtils,
  cxDropDownEdit, cxCalendar, cxCheckBox, cxImageComboBox, cxTimeEdit, cxSpinEdit, cxPC,
  cxScheduler, cxSchedulerUtils, cxSchedulerStorage, cxSchedulerCustomControls,
  cxSchedulerCustomResourceView, cxSchedulerDayView, cxSchedulerDateNavigator,
  cxSchedulerWeekView, cxSchedulerTimeGridView, cxSchedulerYearView, cxSchedulerGanttView,
  dxPSCore, dxPSGlbl, dxExtCtrls, dxPScxCommon, dxPrevw, dxPgsdlg,
...
type
  ...
  TcxfmSchedulerReportLinkDesignWindow = class(TdxfmCustomcxControlReportLinkDesignWindow)
    ...
    pcPrintStyleOptions: TcxPageControl;
    tshWeekly: TcxTabSheet;
    tshMonthly: TcxTabSheet;
    tshTrifold: TcxTabSheet;
    ...
    lblPrintStyleWeeklyDaysLayout: TLabel;
    tshYearly: TcxTabSheet;
    cbxPrintStyleYearlyMonthPerPage: TcxComboBox;
    lblPrintStyleYearlyMonthPerPage: TLabel;
    lblPrintStyleYearlyResourceCountPerPage: TLabel;
    ...
    lblPrintStyleYearlyInclude: TLabel;
    cbxPrintStyleYearlyLayout: TcxComboBox;
    lblPrintStyleYearlyLayout: TLabel;
    ...
    tshTimeLine: TcxTabSheet;
    lblPrintStyleTimeLineResourceCountPerPage: TLabel;
    sePrintStyleTimeLineResourceCountPerPage: TcxSpinEdit;
    chbxPrintStyleTimeLineTaskPad: TcxCheckBox;
    lblPrintStyleTimeLineInclude: TLabel;
    ...
    lblPrintStyleTimeLinePrintFrom: TLabel;
    lblPrintStyleTimeLinePrintTo: TLabel;
    ...
    pnlPreview: TPanel;
    pbPreview: TPaintBox;
    gbPreview: TcxGroupBox;
    pnlPrintStylesPreview: TcxGroupBox;
    pbxPrintStylesPreview: TPaintBox;
    pnlPrintStylesPreviewContainer: TPanel;
...
//    FSmallFont: Boolean;
  ...
  protected
    ...
    function GetCaptionRect(ACanvas: TdxPSReportRenderCustomCanvas): TRect; virtual;
  ...
  protected
    ...
    function GetPolygone(ABounds: TRect): TPointArray; virtual;
  public
    procedure DrawContent(ACanvas: TdxPSReportRenderCustomCanvas;
      DrawRect: TRect; const OriginRect: TRect; AStage: TdxPSRenderStages); override;
  end;
  ...
  protected
    function GetPolygone(ABounds: TRect): TPointArray; override;
  end;
...
procedure TcxfmSchedulerReportLinkDesignWindow.HideOptionsButtons;
var
  I: Integer;
begin
  for I := 0 to pcPrintStyleOptions.PageCount - 1 do
    pcPrintStyleOptions.Pages[I].TabVisible := False;
end;
...
begin
  ...
  try
    ...
    try
      ...
      AMsg := nil;
      with ALink do
        R1 := Bounds(0, 0, PageWidth, PageHeight);
...
begin
  ...
  if Font.Charset in [ARABIC_CHARSET, CHINESEBIG5_CHARSET, GB2312_CHARSET] then
    ...
    Result := WideString(dxFormatSettings.ShortDayNames[FDayIndex])[1];
end;
...
begin
  ...
  begin
    Self.DisplayText := TcxSchedulerTimeGridMinorScaleCell(AViewInfo).DisplayText;
    Self.FTextRect := TcxSchedulerTimeGridMinorScaleCell(AViewInfo).TextBounds;
  end;
end;
...
begin
  ...
  begin
    ...
    try
      ...
      DrawCellText(ACanvas, R, FDisplayText, taRight, taTop, F);
    finally
      F.Free;
    end;
  end;
...
begin
  ...
  if not FTriangle then
    Renderer.FillRectByBrush(ACanvas, R, StateBrushes[FState])
  else
  begin
    ...
    APoints[1] := Point(R.Right, R.Top);
    APoints[2] := Point(R.Right, R.Top + R.Right - R.Left);
    Renderer.Polygone(ACanvas, APoints, StateBrushes[FState].Color, clNone);
    APoints[1] := R.TopLeft;
...
begin
  ...
  try
    ...
    FCaptionLeft := FContentBounds.Left;
    if IsNeedCreateTimeCells then
    begin
      H := Renderer.CalcTextHeight(ACanvas, '0' + DateTimeHelper.TimeAMString +
        DateTimeHelper.TimePMString , False, CaptionCell.Font);
      W := Max(Renderer.CalcTextWidth(ACanvas, '00:00' + DateTimeHelper.TimeAMString, CaptionCell.Font),
        Renderer.CalcTextWidth(ACanvas, '00:00' + DateTimeHelper.TimePMString, CaptionCell.Font));
      AHalfWidth := cxRectWidth(ContentBounds) div 2;
      if W < AHalfWidth then
      begin
        StartCell.BoundsRect := cxRectBounds(FCaptionLeft, 0, W, H);
        if W * 2 < AHalfWidth then
        begin
          FinishCell.BoundsRect := cxRectBounds(StartCell.BoundsRect.Right, 0, W, H);
          FCaptionLeft := FinishCell.BoundsRect.Right + 2;
        end
        else
        begin
          FCaptionLeft := StartCell.BoundsRect.Right + 2;
      ...
      for I := 0 to DataItemCount - 1 do
        ...
        begin
          DataItems[I].Left := FCaptionLeft;
          Inc(FCaptionLeft, DataItems[I].Width + 2);
        end;
    end;
    CaptionCell.BoundsRect := GetCaptionRect(ACanvas);
...
procedure TcxSchedulerReportEventCell.CalculateTimeLine(
  ACanvas: TdxPSReportRenderCustomCanvas);
...
begin
  Result := CreateStringCell(EventViewInfo.Caption,
    FHasDetailInfo and not IsHeaderEvent, cdidSchedulerEventCaption);
  with Result do
  begin
    if HasDetailInfo then TextAlignY := taTop;
    if IsHeaderEvent and not HasImages then TextAlignX := taCenterX;
  end;
end;
...
function TcxSchedulerReportEventCell.GetCaptionRect(
  ACanvas: TdxPSReportRenderCustomCanvas): TRect;
begin
  if HasImages then
    Result := GetRelativeRect(EventViewInfo.CaptionRect)
  else
    Result := FContentBounds;
  if IsNeedCreateTimeCells then
    Result.Left:= FCaptionLeft;
  if HasMessage then
  begin
    Result.Bottom := Result.Top + CaptionCell.MeasureFontHeight(ACanvas);
    Renderer.CalcTextRect(ACanvas, CaptionCell.Text, Result, True, Font);
    Result.Right := ContentBounds.Right;
    CheckBounds(Result);
  end;
end;

function TcxSchedulerReportEventCell.GetContentBounds(
  ACanvas: TdxPSReportRenderCustomCanvas): TRect;
begin
...
function TcxSchedulerReportEventCell.GetMessageRect(
  ACanvas: TdxPSReportRenderCustomCanvas): TRect;
begin
  if HasImages then
...
function TcxSchedulerReportEventCell.GetTimeLineRect(
  ACanvas: TdxPSReportRenderCustomCanvas): TRect;
begin
...
begin
  ...
  R := AEventViewInfo.EventViewData.Bounds;
  FOrigin := cxPointInvert(R.TopLeft);
...
begin
  FCaptionCell := CreateStringCell(GetCaptionCellText, True,
...
function TcxSchedulerReportGanttMilestoneCell.GetPolygone(ABounds: TRect): TPointArray;
begin
...
function TcxSchedulerReportGanttGroupCell.GetPolygone(ABounds: TRect): TPointArray;
begin
...
begin
  if AEventViewInfo.EventViewData.IsMilestone then
    ...
    if AEventViewInfo.EventViewData.IsGroup then
      ...
      Result := TcxSchedulerReportEventCell;
end;
...
procedure TcxSchedulerReportTimeLineCellsProducer.GetPageResources(
  AResources: TcxObjectList);
var
  I: Integer;
begin

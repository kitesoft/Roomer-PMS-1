...
{$ENDIF}
  ...
  cxDropDownEdit, Menus, cxLookAndFeelPainters, cxButtons, dxPSReportRenderCanvas;
...
function TdxCustomTextReportLink.CreateItem(AParent: TdxReportCell;
  ...
  var
    W: WideString;
  begin
    SetString(W, ATextRow.Text, ATextRow.TextLength);
    Result := dxWideStringToString(W);
  end;
...
begin
  ...
  begin
    ...
    Canvas.Font.Size := 8;
    R := ClientRect;
    cxTextOut(Canvas.Handle, GetText, R, Format, Canvas.Font, 0, 0, 0,
      clDefault, ReportLink.TextRowSpacing);
  end;
end;
...
begin
  ...
  ilAlignments.Draw(ACanvas.Canvas, R.Left + 1, R.Top + 1, AIndex);
  Inc(R.Left, ilAlignments.Width + 4);

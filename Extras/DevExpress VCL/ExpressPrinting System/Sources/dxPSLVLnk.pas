...
uses
  StdCtrls, Classes, Windows, Graphics, Controls, ExtCtrls, ImgList, Dialogs,
  {$IFDEF REGISTERSHELLCTRLS} ShellCtrls, {$ENDIF} dxCore, dxPSGlbl, dxPSCore,
  dxExtCtrls, dxPrnPg, dxPSBaseGridLnk, cxDrawTextUtils, cxControls, cxContainer,
  cxEdit, cxCheckBox, cxLabel, Menus, cxLookAndFeelPainters, cxTextEdit,
  cxButtons, cxPC, cxGraphics, cxMaskEdit, cxDropDownEdit, cxColorComboBox,
  cxLookAndFeels, cxImageComboBox, ComCtrls, CommCtrl;
...
uses
  Messages, Forms, SysUtils, Math,
  cxClasses, dxPSRes, dxPSUtl, dxPrnDev, dxPSImgs;
...
function TCustomdxNativeListViewReportLink.GetDataItemClass(ACol: Integer; 
...
begin
  ...
  FColumnIndexes.Count := ColCount;
  ListView_GetColumnOrderArray(ListViewHandle, FColumnIndexes.Count, PInteger(FColumnIndexes.List));
end;
...
begin
  ...
  chbxShowColumnHeaders.Visible := ReportLink.HasSupportForInvisibledColumnHeaders;
...
begin
  ...
  dxLoadIconFromResourceEx(Image3, IDB_DXPSGROUPICON_SELECTION);
  dxLoadIconFromResourceEx(Image8, IDB_DXPSGROUPICON_LOOKANDFEEL);
  dxLoadIconFromResourceEx(Image5, IDB_DXPSGROUPICON_SIZE);
end;
...
begin
  inherited;
  dxPSDrawGridPreview(ACanvas, R, ReportLink, False, ReportLink.ShowColumnHeaders);
end;
...
procedure TdxfmLVReportLinkDesignWindow.PageControl1Change(Sender: TObject);
begin

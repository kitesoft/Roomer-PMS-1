...
{$ENDIF}
  ...
  dxMessages, cxClasses, dxPSESys, dxPSForm, dxBkgnd, dxPreVw, dxPSGlbl, dxfmClr, dxPrnPg, 
  dxPrnDev, dxExtCtrls, cxLookAndFeelPainters, cxButtons, cxRadioGroup, cxPC, 
  cxControls, cxContainer, cxEdit, cxCheckBox, cxLabel, cxTextEdit, cxMemo,
  cxListBox, cxGraphics, cxMaskEdit, cxDropDownEdit, cxGroupBox, cxSpinEdit,
  cxDrawTextUtils, dxCore, cxGeometry, cxLookAndFeels, IniFiles, dxPSHFToolBarBld;
...
type
  ...
  TdxPrintStyleManager = class(TcxCustomComponent)
  private
  ...
  TdxfmPageSetupDialog = class(TCustomdxPSForm)
    ...
    gbxMargins: TcxGroupBox;
    gbxPaper: TcxGroupBox;
    lblPaperHeight: TcxLabel;
    lblPaperWidth: TcxLabel;
    lblPercentOfNormalSize: TcxLabel;
    lblPreview: TcxLabel;
    lblStyleName: TcxLabel;
    ...
    tshMargins: TcxTabSheet;
    tshPage: TcxTabSheet;
    tshScaling: TcxTabSheet;
    pnlReverse: TcxGroupBox;
    chbxReverseOnEvenPages: TcxCheckBox;
    pnlMargins: TcxGroupBox;
    lblMarginTop: TcxLabel;
    lblMarginBottom: TcxLabel;
    lblMarginLeft: TcxLabel;
    lblMarginRight: TcxLabel;
    seMarginTop: TcxSpinEdit;
    seMarginBottom: TcxSpinEdit;
    seMarginLeft: TcxSpinEdit;
    seMarginRight: TcxSpinEdit;
    pnlHFMargins: TcxGroupBox;
    lblMarginHeader: TcxLabel;
    lblMarginFooter: TcxLabel;
    seMarginHeader: TcxSpinEdit;
    seMarginFooter: TcxSpinEdit;
    Panel3: TcxGroupBox;
    btnFix: TcxButton;
    btnRestoreOriginalMargins: TcxButton;
    pnlHeader: TcxGroupBox;
    pnlHeaderFont: TcxGroupBox;
    btnHeaderFont: TcxButton;
    btnHeaderBackground: TcxButton;
    edHeaderFontInfo: TcxTextEdit;
    pnlHeaderMemos: TcxGroupBox;
    bv3: TBevel;
    bv4: TBevel;
    ...
    procedure lbxPaperTypeDrawItem(AControl: TcxListBox; ACanvas: TcxCanvas;
    ...
    procedure pgctrlMainPageChanging(Sender: TObject; NewPage: TcxTabSheet;
    ...
    procedure cbxPaperSourcePropertiesDrawItem(AControl: TcxCustomComboBox;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        begin
          R := Bounds(ABounds.Left + 1, ABounds.Top + 1, dxStyleGlyphSize.X, dxStyleGlyphSize.Y);
          B := AStyle.StyleGlyph;
        end;
        
      if AreImagesDrawn or not AStyle.StyleGlyph.Empty then
        cxAlphaBlend(Canvas.Handle, B, R, Rect(0, 0, B.Width, B.Height));
    finally
...
begin
  ...
  begin
    LineBounds := Rect(R.Left + I * LineStepX, R.Top, R.Left + I * LineStepX + 1, R.Bottom);
  ...
  begin
    LineBounds := Rect(R.Left, R.Top + I * LineStepY, R.Right + 1, R.Top + I * LineStepY + 1);
    if RectVisible(ACanvas.Handle, LineBounds) then 
      ACanvas.FillRect(LineBounds);
  end;
end;
...
const
  cGlyphWidth = 75; { TextArea width + DownArrow width}
begin
  inherited Create(AOwner);
  FBmpArrow := dxPSUtl.CreateArrowBitmap(udgDown);
  FFooterBkGndGlyph := CreateGlyph(cGlyphWidth, btnFooterBackGround.Height - 6);
  FHeaderBkGndGlyph := CreateGlyph(cGlyphWidth, btnHeaderBackGround.Height - 6);
  ...
  FHFToolBarHelper.ButtonsImageList := FilPredefined;
...
procedure TdxfmPageSetupDialog.SetupDialog(const APageSetupDlgData: TdxPageSetupDlgData);
  ...
  begin
    ...
    begin
      ...
      FHFToolBarHelper.Build(
        FHFFunctionList, AutoHFTextEntries, PrintStyle.PrinterPage.ImageCollection,
  ...
  var
    AIndex: Integer;
  begin
    AIndex := Min(Max(APageSetupDlgData.ActivePageIndex, 0), pgctrlMain.PageCount - 1);
    if not pgctrlMain.Pages[AIndex].TabVisible then
      AIndex := 0;
    pgctrlMain.ActivePage := pgctrlMain.Pages[AIndex];
  end;

  procedure UpdateControlEnableState(AControl: TWinControl; AEnabled: Boolean);
  var
    AChild: TControl;
    I: Integer;
  begin
    ...
    begin
      AChild := AControl.Controls[I];
      if AChild is TcxLabel then
        AChild.Enabled := AEnabled;
      if AChild is TcxSpinEdit then
      begin
        TcxSpinEdit(AChild).TabStop := AEnabled;
        TcxSpinEdit(AChild).Properties.ReadOnly := not AEnabled;
      end;
    end;
  end;
  ...
  begin
    chbxCenterHorz.Visible := psoCenterOnPage in APageSetupDlgData.OptionsVisible;
    chbxCenterVert.Visible := psoCenterOnPage in APageSetupDlgData.OptionsVisible;
    lblCenterOnPage.Visible := psoCenterOnPage in APageSetupDlgData.OptionsVisible;
    tshMargins.TabVisible := gbxMargins.Visible or lblCenterOnPage.Visible;
    ...
    tbPredefined.Visible := (psoHFFunctions in APageSetupDlgData.OptionsVisible);
    gbxFunctions.Visible := tbPredefined.Visible;
    ...
    tshHeaderFooter.TabVisible := pnlHeader.Visible or pnlFooter.Visible or
      pnlBottom.Visible or pnlReverse.Visible;
    ...
    FCanShowStyleOptionsDlg := pnlStyleName.Enabled and
      (psbStyleOptions in APageSetupDlgData.ButtonsEnabled);
  ...
  begin
    ...
    if gbxMargins.Visible then
      UpdateControlEnableState(gbxMargins, PrintStyle.AllowChangeMargins and
        (psoMargins in APageSetupDlgData.OptionsEnabled));
    ...
    begin
      UpdateControlEnableState(pnlHFMargins, gbxMargins.Enabled and
        (psoHFMargins in APageSetupDlgData.OptionsEnabled));
    end;
...
procedure TdxfmPageSetupDialog.DrawBackgroundBitmapContent(
  ACanvas: TcxCanvas; R: TRect; ABackground: TdxBackground);
...
begin
  ...
  try
    ...
    R := cxRectSetWidth(R, cxRectWidth(R) - FBmpArrow.Width);
    InflateRect(R, -4, 0);
    DrawBackground(ACanvas, R);
    ACanvas.Draw(R.Right + 4, (R.Top + R.Bottom - FBmpArrow.Height) div 2, FBmpArrow);
  finally
...
begin
  ...
  try
    DrawBackgroundBitmapContent(ACanvas,
      Rect(0, 0, AGlyph.Width, AGlyph.Height), ABackground);
  finally
    ACanvas.Free;
  end;
end;
...
procedure TdxfmPageSetupDialog.CreateControls;
  ...
  function CreatePreview(AHolder: TControl): TdxPreview;
  begin
    Result := TdxPreview.Create(Self);
    Result.LookAndFeel.MasterLookAndFeel := dxPSEngine.DialogsLookAndFeel;
    Result.OptionsView := Result.OptionsView - [povAutoHideScrollBars];
    Result.Parent := AHolder.Parent;
    Result.BoundsRect := AHolder.BoundsRect;
    Result.Color := clBtnFace;
    Result.ScrollBars := ssNone;
    Result.ZoomMode := pzmPages;
    Result.MinHeaderSize := 0;
    Result.MinFooterSize := 0;
...
begin
  ...
  rbtnFitTo.Caption := cxGetResourceString(@sdxMenuFormatShrinkToPage);
...
begin
  if not FControlsUpdating then
  begin
    Page.RealPageSize := Point(
      Round(1000 * sePaperWidth.Value), Round(1000 * sePaperHeight.Value));
...
begin
  ...
  try
    ...
    edStyleName.Text := Copy(FSavePrintStyle.StyleCaption, 1,
      edStyleName.Properties.MaxLength);
  ...
  FOrientationPreview.FullRefresh;
  FPreview.FullRefresh;
...
begin
  if ActiveControl is TcxCustomInnerMemo then
  begin
    dxPSSplitAutoHFTextEntry(FStyleManager.AutoHFTextEntries[
      TTagToInt(TComponent(Sender).Tag)], Part1, Part2, Part3);
...
begin
  ...
  begin
    ...
    begin
      ...
      TcxButton(Sender).Glyph := FHeaderBkgndGlyph;
    end
    else
    begin
      ChangeBkgndGlyph(FFooterBkgndGlyph, ABackground);
      TcxButton(Sender).Glyph := FFooterBkgndGlyph;
    end;
    CheckModified;
  end;
end;
...
begin
  ...
  begin
    ContentBounds := Rect(ABounds.Left + Margins.Left.VisibleValue,
      ABounds.Top + Margins.Top.VisibleValue,
      ABounds.Right - Margins.Right.VisibleValue,
      ABounds.Bottom - Margins.Bottom.VisibleValue);
    FooterBounds := Rect(ABounds.Left + Margins.Left.VisibleValue,
      ABounds.Bottom - Margins.Footer.VisibleValue,
      ABounds.Right - Margins.Right.VisibleValue,
      ABounds.Bottom - Margins.Bottom.VisibleValue);
    HeaderBounds := Rect(ABounds.Left + Margins.Left.VisibleValue,
...
begin
  ...
  Pt := Point(PaperInfo.Width, PaperInfo.Height);
...
begin
  ...
  ilPapers.Draw(ACanvas.Canvas, ARect.Left, ARect.Top,
    Ord(dxPrintDevice.IsEnvelopePaper(AIndex)), Enabled);
  Inc(ARect.Left, ilPaperTypes.Width + 2);
  if not Enabled then
...
begin
  ...
  InflateRect(R, -2, -1);
  ilBins.Draw(ACanvas.Canvas, R.Left, R.Top, Integer(dxPrintDevice.IsAutoSelectBin(AIndex)));
  Inc(R.Left, ilPaperTypes.Width + 2);
  cxDrawText(ACanvas.Handle, AControl.Properties.Items[AIndex],

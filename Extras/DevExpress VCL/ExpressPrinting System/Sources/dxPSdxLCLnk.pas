...
{$ENDIF}
  ...
  dxPSReportRenderCanvas, cxGeometry, cxGraphics, dxLayoutCommon, dxPSGlbl;
...
uses
  Variants, Messages, SysUtils, StdCtrls, Forms, cxClasses, dxPSRes,
  dxPSUtl, dxLayoutControlAdapters, cxControls, ImgList, Types, dxCore;
...
begin
  ...
  begin
    Assign(ACaptionOptions.GetFont(AContainer));
    ...
    if not dxIsTrueTypeFont(FPreparationFont) then
      Name := TdxLayoutContainerAccess(AContainer).GetDefaultFont.Name;
  end;

...
type
  ...
  private
    ...
    FCoordsConversionFactor: Single;
    ...
    procedure SetCoordsConversionFactor(AValue: Single);
  ...
  public
    ...
    procedure BeginPaint(AClipRegion: HRGN = 0); overload;
    procedure BeginPaint(const AWindowOrg: TPoint; AClipRegion: HRGN); overload;
    procedure EndPaint;

    procedure AdjustFontSize(const AText: WideString; const ATextWidth: Double);
    procedure DrawEllipseFrame(const R: TRect; AColor: TColor; AThickness: Integer);
    procedure DrawFrame(const R: TRect; ABorderWidth: Integer;
      ABorderColor: TColor; ABorders: TcxBorders); overload;
    procedure DrawFrame(const R: TRect; ABorderWidth: Integer;
      ATopLeftBorderColor, ARightBottomBorderColor: TColor; ABorders: TcxBorders); overload;
    ...
    procedure DrawText(ARect: TRect; const AText: WideString;
      ADirection: TcxRotationAngle = ra0; ACharsSpacing: Double = 0; AWordSpacing: Double = 0);
    procedure Fill;
    ...
    procedure Polyline(const APoints: array of TPoint;
      ALineWidth: Integer; AColor: TColor = clDefault);
    ...
    function AbsoluteTextWidth(const AText: WideString): Double; virtual;
    function TextWidth(const AText: WideString): Double; virtual;
    procedure TextOut(const AText: WideString; const X, Y, ACharsSpacing, AWordSpacing: Double);
    //
    procedure WriteCommand(const S: string; ANewLine: Boolean = True);
    procedure WriteCurveTo(X1, Y1, X2, Y2, X3, Y3: Double);
    ...
    //
    property CoordsConversionFactor: Single read FCoordsConversionFactor write SetCoordsConversionFactor;
  ...
  public
    ...
    function CanEncodeText(const S: WideString): Boolean; virtual; abstract;
...
function dxPDFEncodeBounds(const R: TRect; AHeight: Double): string;
function dxPDFEncodeColor(AColor: TColor): string;
function dxPDFEncodeFloat(AFloat: Double): string;

function dxPDFMakeTransform(const M11, M22, DX, DY: Single): TXForm;
implementation
...
{$IFDEF DELPHI18}
  AnsiStrings, 
{$ENDIF}  
  Types, Math, dxPSPDFFonts, cxDrawTextUtils;
...
function TdxPSPDFPageContent.AbsoluteTextWidth(const AText: WideString): Double;
const
  D2P = 72 / 96;
var
  AFont: TdxPSPDFCustomFont;
...
procedure TdxPSPDFPageContent.AdjustFontSize(const AText: WideString; const ATextWidth: Double);
var
  ATempWidth: Double;
begin
...
procedure TdxPSPDFPageContent.BeginPaint(AClipRegion: HRGN = 0);
begin
  BeginPaint(cxNullPoint, AClipRegion);
end;

procedure TdxPSPDFPageContent.BeginPaint(const AWindowOrg: TPoint; AClipRegion: HRGN);
begin
  SaveGraphicState;
  if not SameValue(CoordsConversionFactor, 1) then
    ModifyWorldTransform(dxPDFMakeTransform(CoordsConversionFactor, CoordsConversionFactor, 0, 0));
  SelectClipRegion(AClipRegion);
  if not cxPointIsEqual(AWindowOrg, cxNullPoint) then
    ModifyWorldTransform(dxPDFMakeTransform(1, 1, -AWindowOrg.X, AWindowOrg.Y));
end;

procedure TdxPSPDFPageContent.EndPaint;
begin
  RestoreGraphicState;
end;

procedure TdxPSPDFPageContent.DrawEllipseFrame(
  const R: TRect; AColor: TColor; AThickness: Integer);
begin
  WriteEllipse(R);
  WriteEllipse(cxRectInflate(R, -AThickness, -AThickness));
  Clip(pcmDiff);
  FillRect(R, AColor);
end;
...
begin
  ...
  begin
    ...
    try
      ...
      try
        ModifyWorldTransform(dxPDFMakeTransform(cxRectWidth(R),
          cxRectHeight(R), R.Left, PageHeight - R.Bottom));
...
procedure TdxPSPDFPageContent.DrawText(ARect: TRect; const AText: WideString;
  ADirection: TcxRotationAngle = ra0; ACharsSpacing: Double = 0; AWordSpacing: Double = 0);

  procedure CalculateRectTransformOffset(var AForm: TXForm; const R: TRect);
  begin
    case ADirection of
      raPlus90:
        begin
          AForm.eDx := PageHeight + R.Top;
          AForm.eDy := PageHeight - R.Right;
        end;

      raMinus90:
        begin
          AForm.eDx := R.Bottom - PageHeight;
          AForm.eDy := PageHeight - R.Left;
        end;

      ra180:
        begin
          AForm.eDx := R.Right;
          AForm.eDy := 2 * PageHeight - R.Bottom;
        end;
  ...
  function CalculateTransform(const R: TRect): TXForm;
  const
    DirectionAngle: array[TcxRotationAngle] of Double = (0, 90, 270, 180);
  var
    AAngle: Double;
  begin
    AAngle := Pi * DirectionAngle[ADirection] / 180;
    ZeroMemory(@Result, SizeOf(Result));
    Result.eM11 := Cos(AAngle);
    Result.eM12 := Sin(AAngle);
    Result.eM21 := -Result.eM12;
    Result.eM22 := Result.eM11;
    CalculateRectTransformOffset(Result, R);
  end;

begin
  SaveGraphicState;
  try
    if ADirection in [raPlus90, raMinus90] then
      ARect := cxRectRotate(ARect);
    ModifyWorldTransform(CalculateTransform(ARect));
...
procedure TdxPSPDFPageContent.FillRectByBrush(
  const R: TRect; AColor: TColor; AStyle: TBrushStyle = bsSolid);
begin
  case AStyle of
    bsClear:
      ;
...
procedure TdxPSPDFPageContent.FillRectByGraphic(
  const R: TRect; AWidth, AHeight: Integer; AGraphic: TGraphic);
var
  ABitmap: TBitmap;
  ANeedDestroy: Boolean;
begin
...
procedure TdxPSPDFPageContent.TextOut(
  const AText: WideString; const X, Y, ACharsSpacing, AWordSpacing: Double);

  function CalculateTextWidth(const AText: WideString): Double;
  begin
    Result := ACharsSpacing * (Length(AText) - 1) + TextWidth(AText);
  end;
  ...
  function DoWriteText(const AText: WideString): Double;
  begin
    Result := CalculateTextWidth(AText);
    WriteText(AText);
  end;

  function DisplayTextPartWithCompositeFonts(const S: WideString): Double;
  var
  ...
  begin
    ...
    try
      ...
      begin
        if not Font.CanEncodeText(AScan^) then
        begin
          if APrevFont.CanEncodeText(AScan^) then
            SetFontAndSelect(APrevFont)
          else
            SetFontAndSelect(dxPDFFontsManager.GetFontForSymbol(AScan^, Font));
        end;
        Result := Result + DoWriteText(AScan^) + ACharsSpacing;
        Inc(AScan);
      end;
    finally
      SetFontAndSelect(APrevFont);
  ...
  function DisplayTextPart(const S: WideString): Double;
  begin
    if dxPDFCanCompositeFonts and not Font.CanEncodeText(S) then
      Result := DisplayTextPartWithCompositeFonts(S)
    else
      Result := DoWriteText(S);
    if (Length(S) = 1) and cxGetIsWordDelimeter(Font.CodePage, S[1]) then
      Result := Result + ACharsSpacing + AWordSpacing;
  end;

  function DisplayText(X, Y: Double): Double;
  var
  ...
  begin
    ...
    if not IsZero(ACharsSpacing) then
      WriteCommand(dxPDFEncodeFloat(ACharsSpacing) + sdxPDFSpace + 'Tc');

    WriteTextOffset(X, Y);
    if not IsZero(AWordSpacing) then
    begin
      Result := 0;
      ACursor := PWideChar(AText);
      AEndScan := ACursor + Length(AText);
      while ACursor < AEndScan do
      begin
        ANextWordBreak := cxGetNextWordBreak(Font.CodePage, ACursor, AEndScan);
        AWordWidth := DisplayTextPart(ExtractWord(ACursor, ANextWordBreak));
        WriteTextOffset(AWordWidth, 0);
        Result := Result + AWordWidth;
        ACursor := ANextWordBreak;
      end;
    end
    else
      Result := DisplayTextPart(AText);
...
begin
  ...
  try
    ...
    ATextWidth := DisplayText(X, PageHeight - Y - FontSize);
...
procedure TdxPSPDFPageContent.Polyline(
  const APoints: array of TPoint; ALineWidth: Integer; AColor: TColor);
begin
...
procedure TdxPSPDFPageContent.WriteCurveTo(X1, Y1, X2, Y2, X3, Y3: Double);
begin
  WriteCommand(
...
procedure TdxPSPDFPageContent.WritePie(const X1, Y1, X2, Y2, X3, Y3, X4, Y4: Double);
  ...
  procedure CalcPoint(var X, Y: Double;
...
procedure TdxPSPDFPageContent.WriteRoundRect(
  const R: TRect; const AEllipseWidth, AEllipseHeight: Double);
begin
  // TopLeft
...
procedure TdxPSPDFPageContent.WriteText(const AText: WideString);
begin
  if AText <> '' then
  begin
    WriteStringToStream(ContentStream, Font.EncodeText(AText));
...
begin
  Result := Parent.PageHeight / CoordsConversionFactor;
end;

function TdxPSPDFPageContent.GetPageWidth: Double;
begin
  Result := Parent.PageWidth / CoordsConversionFactor;
end;
...
procedure TdxPSPDFPageContent.SetCoordsConversionFactor(AValue: Single);
begin
  if AValue > 0 then
    FCoordsConversionFactor := AValue;
end;

...
uses
  ...
  cxGroupBox, dxCore, DesignIntf, DesignWindows, cxListBox, cxLookAndFeels;
...
type
  TdxfmPrintStylesDesignWindow = class(TDesignWindow)
    ...
    procedure lbxStylesDragOver(Sender, Source: TObject; X, Y: Integer;
      State: TDragState; var Accept: Boolean);
    procedure lbxStylesDrawItem(AControl: TcxListBox; ACanvas: TcxCanvas;
...
uses
  ...
  dxPrnPg, dxBkgnd, dxPSGlbl, dxPSUtl, dxPSPopupMan, dxPSfmStlAdd, dxPSImgs;
...
procedure TdxfmPrintStylesDesignWindow.lbxStylesDragOver(Sender, Source: TObject;
...
begin
  dxPSDrawStyleItem(Styles[AIndex], AControl.InnerListBox,
    AIndex, AState, ARect, False, True);
end;
...
begin
  ...
  if (Item is TBasedxPrintStyle) and (Controller <> nil) and not
     (csDestroying in Controller.ComponentState)
  then
    DeleteItem(TBasedxPrintStyle(Item));
end;
...
begin
  Result :=
    dxPSDsgProxies.GetBaseRegistryKey + '\' +
      dxPSGlbl.sdxPSRegPathRunTimeFormLayouts + '\' + dxPSUtl.DropT(ClassName);
end;
...
begin
  CheckAddStyle;
  Origin := btnAdd.ClientToScreen(Point(btnAdd.Width, -40));
    ...
    try
      ...
      try
        ...
        begin
          ...
          if not (csAncestor in Item.ComponentState) then
            Item.Free;
        end;
...
begin
  ...
  begin
    ...
    begin
      ...
      AWidth := Max(AWidth, dxStyleGlyphSize.X + 4 + Canvas.TextWidth(Items[I]));
    end;

...
uses
  ...
  dxPScxMCListBoxLnk, cxCheckBox, dxPSRELnk, cxDrawTextUtils;
...
procedure TcxPScxCustomLabelProducer.InitializeItem(AItem: TdxReportVisualItem);
const
...
begin
  ...
  TdxReportCellString(AItem).Text := Control.Caption;
  TdxReportCellString(AItem).Multiline := Properties.WordWrap;
  TdxReportCellString(AItem).HidePrefix := Properties.ShowAccelChar;
  TdxReportCellString(AItem).TextAlignX := TextAlignXMap[Properties.Alignment.Horz];
  TdxReportCellString(AItem).TextAlignY := TextAlignYMap[Properties.Alignment.Vert];
  TdxReportCellString(AItem).PreventAutoIndents :=
    not Control.Style.TransparentBorder or (Control.Style.LookAndFeel.SkinPainter <> nil);
end;

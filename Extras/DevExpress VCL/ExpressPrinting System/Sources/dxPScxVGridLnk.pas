...
{$ENDIF}
  ...
  dxExtCtrls, cxDrawTextUtils, cxPC, cxContainer, cxCheckBox, cxLabel,
  cxMaskEdit, cxDropDownEdit, cxLookAndFeelPainters, cxButtons,
...
type
  ...
  protected
    ...
    procedure UpdateStatePrintModeControl(AControl: TcxComboBox); virtual;
  ...
  protected
    ...
    procedure UpdateStatePrintModeControl(AControl: TcxComboBox); override;
  public
...
uses
  ...
  cxGeometry, cxDataUtils, cxDataStorage, dxPSRes, dxPSImgs, dxPSUtl,dxBkgnd, dxPSPopupMan;
...
begin
  if ReportLink.IsAggregated and
    Supports(TObject(ReportLink.Controller), IdxReportLinkController, ControllerIntf)
  then
    Result := cxRectWidth(ControllerIntf.GetControlSiteBounds(ReportLink.VerticalGrid))
  else
    Result := cxRectWidth(ReportLink.RealPrinterPage.PaintRectPixels);

  Result := MulDiv(Result - 1, 100, ReportLink.GetRealScaleFactor);
end;
...
procedure TcxCustomVerticalGridReportLink.UpdateStatePrintModeControl(AControl: TcxComboBox);
begin
...
procedure TcxVirtualVerticalGridReportLink.UpdateStatePrintModeControl(
  AControl: TcxComboBox);
begin
  AControl.Enabled := AControl.Properties.Items.Count > 1;
...
begin
  ...
  dxSetupPreviewControlLookAndFeel(PreviewVGrid.LookAndFeel,
    ReportLink.OptionsFormatting.LookAndFeelKind, ReportLink.VerticalGrid);

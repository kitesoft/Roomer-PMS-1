...
uses
  Types, Classes, Windows, Messages, Graphics, Controls, Forms, Dialogs, StdCtrls,
  ImgList, CommCtrl, ExtCtrls, ComCtrls, {$IFDEF REGISTERSHELLCTRLS}ShellCtrls, {$ENDIF}
  dxPSCore, dxPSGlbl, dxPSRes, cxDrawTextUtils, cxPC, cxControls,
  cxContainer, cxEdit, cxCheckBox, cxLabel, cxGraphics, cxSpinEdit,
  cxTextEdit, cxMaskEdit, cxDropDownEdit, cxColorComboBox, Menus,
  cxLookAndFeelPainters, cxButtons, dxPSReportRenderCanvas, cxLookAndFeels;
...
type
 ...
 {$ENDIF}
  ...
  TdxfmTVReportLinkDesignWindow = class(TStandarddxReportLinkDesignWindow)
    ...
    tshColors: TcxTabSheet;
    tshFonts: TcxTabSheet;
    pnlPreview: TPanel;
...
uses     
  ClipBrd, SysUtils, Math, cxClasses, dxPrnDev, dxPSUtl, dxExtCtrls, cxGeometry, dxPSImgs, dxCore;
...
begin
  ...
  dxLoadIconFromResourceEx(Image4, IDB_DXPSGROUPICON_EXPANDING);
  dxLoadIconFromResourceEx(Image5, IDB_DXPSGROUPICON_MISCELLANEOUS);
end;
...
const
  ...
  begin
    ...
    if not ReportLink.Transparent then
      cxPaintCanvas.FillRect(R, ReportLink.Color);  
    R := cxRectSetHeight(R, Trunc((cxRectHeight(R) - 2) / ItemsCount) * ItemsCount);
    if ReportLink.IsDrawBorder then
      cxPaintCanvas.FrameRect(R, ReportLink.GridLineColor);    
    InflateRect(R, -1, -1);
  end;

  procedure DrawImage(ACanvas: TcxCanvas; var R: TRect; AIndex: Integer);
  begin
    ilPreview.Draw(ACanvas.Canvas, R.Left, 
      cxRectCenterVertically(R, ilPreview.Height).Top, AIndex);
    Inc(R.Left, ilPreview.Width);  
  end;
  ...
  begin
    ...
    ACanvas.Font := ReportLink.Font;
    ACanvas.Font.Size := 8;
    ACanvas.Brush.Style := bsClear;
    cxDrawText(ACanvas.Handle, GetDesignerString(AIndex), R, DT_SINGLELINE or DT_VCENTER);
  end;

  procedure DrawTreeLine(ACanvas: TcxCanvas; const R: TRect; ARowHeigth: Integer);
  var
  ...
  begin
    ...
    ACanvas.Brush.Style := bsClear;
    ACanvas.MoveTo(R.Left + ARowHeigth div 2, R.Top);
    ACanvas.LineTo(R.Left + ARowHeigth div 2, R.Bottom - ARowHeigth div 2);
    ACanvas.LineTo(R.Right, R.Bottom - ARowHeigth div 2);

    if tvpoButtons in ReportLink.Options then
    begin
      AExpandButtonRect := cxRectSetBottom(R, R.Bottom, ARowHeigth);
      AExpandButtonRect := cxRectCenter(AExpandButtonRect, 11, 11);
      cxLookAndFeelPaintersManager.GetPainter(lfsUltraFlat).DrawExpandButton(
        ACanvas, AExpandButtonRect, True);    
    end;
  end; 

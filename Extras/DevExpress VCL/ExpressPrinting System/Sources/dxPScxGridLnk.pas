...
{$ENDIF}
  ...
  ComCtrls, ExtCtrls, ImgList, Dialogs, ExtDlgs, Menus, cxGraphics, cxClasses,
  cxControls, cxCustomData, cxDataStorage, cxData, cxDBData, cxGridDBDataDefinitions,
  cxGrid, cxGridCustomView, cxGridCustomTableView, cxGridTableView, cxGridDBTableView,
  cxGridCardView, cxGridDBCardView, cxGridBandedTableView, cxGridDBBandedTableView,
  cxGridChartView, cxGridDBChartView, cxGridLevel, cxStyles, cxLookAndFeels,
  cxGridCustomLayoutView, cxLookAndFeelPainters, cxFilter, cxEdit, cxTextEdit,
  cxCheckBox, cxImage, dxBase, dxPSSngltn, dxExtCtrls, dxPSCore, dxPrnPg, dxPScxCommon,
  dxCore, cxContainer, cxLabel, cxPC, cxMaskEdit, cxDropDownEdit, cxSpinEdit,
  cxColorComboBox, cxButtons, dxPSReportRenderCanvas, cxGeometry, cxNavigator,
  dxCoreClasses, dxServerModeData, cxGridServerModeTableView,
...
type
  ...
  protected
    ...
    class function ProducerClass: TdxGridTableViewCustomDataRowProducerClass; virtual;
  ...
  protected
    class function ProducerClass: TdxGridTableViewCustomDataRowProducerClass; override;
    class function RecordClass: TcxCustomGridRecordClass; override;
  public   
  ...
  protected
    SummaryItems: TcxDataSummaryItems;
    SummaryValues: PVariant;
    procedure AddSubItems(AParent: TdxReportCell); override;
    procedure CalculateRowAutoHeight; override;
    procedure FixupRowDataItemHeight(AnItem: TdxReportVisualItem); override;
    procedure InitializeIndent(AIndent: TdxReportCellExpandButton; AIndex: Integer); override;
  ...
  TdxfmGridReportLinkDesignWindow = class(TdxfmCustomcxControlReportLinkDesignWindow)
    ...
    lblPreviewMaxLineCount: TcxLabel;
    lblCardSpaceVert: TcxLabel;
    lblCardSpaceHorz: TcxLabel;
    lblCardSpacing: TcxLabel;
    lblCardSizes: TcxLabel;
    lblCardFraming: TcxLabel;
    lblCardShadowDepth: TcxLabel;
    lblCardShadow: TcxLabel;
    lblCardShadowColor: TcxLabel;
    lblChartsOptions: TcxLabel;
    lblPreviewWindow: TcxLabel;
    bvlRefinementsSeparator: TcxLabel;
...
uses
  ...
  cxDrawTextUtils, dxPSUtl, dxBkgnd, dxPSPopupMan, dxPSRes, dxPSImgs, Math;
...
begin
  Result := GridView.ViewInfo.LookAndFeelPainter.ExpandButtonSize;
end;
...
begin
  ...
  ViewParams := GetItemViewParams(ATableItem, ARecord, AnIsPreview, True);
  ...
  AnItem.Data := TdxNativeInt(ATableItem);
end;
...
function TdxGridTableViewAdapter.GetFooterLineCount(
  AItems: TcxDataSummaryItems; ACanMultiLine: Boolean): Integer;
var
  I, J, ACount: Integer;
begin
  Result := RowLineCount;
  if (AItems = nil) or not ACanMultiLine then Exit;
...
procedure TdxGridTableViewFormatter.PerformColumnCellMerging(AColumn: TcxGridColumn);
  ...
  begin
    ...
    begin
      ...
      if SlaveReportItem is TCustomdxReportCellImageContainer then
      begin
        TCustomdxReportCellImageContainer(SlaveReportItem).Image := nil;
        TCustomdxReportCellImageContainer(SlaveReportItem).ImageIndex := -1;
      end; 
    end;
  end;
...
begin
  ...
  for I := 0 to BottomBandCount - 1 do 
  begin
    V := BottomBands[I].Rows.VisibleCount;
    if Result < V then Result := V;  
  end;  
end;
...
function TdxReportCardRow.MeasureHeight(ACanvas: TdxPSReportRenderCustomCanvas; ACalculateLineHeight: Boolean; 
  ...
  begin
    ...
    Result := Max(Result, DefaultDataRowLineHeight);
  end;
  ...
  var
    MinHeight: Integer;
  begin
    Result := DefaultDataRowLineHeight * GridCardRow.Position.LineCount;
    if HasData then
    begin
...
const
  ...
  begin
    FRowData := ABuilder.Formatter.GetItemClass(GridCardRow, Card.GridCard, False).Create(Self);
...
procedure TdxGridCardViewBuilder.DoResizeRecords;
  ...
  var
    RowCaptionWidth, I, J: Integer;
    ALayerCaptionWidths: array of Integer;
  begin
    if (CardCount = 0) or (Cards[0].LayerCount = 0) then Exit;
    SetLength(ALayerCaptionWidths, Cards[0].LayerCount);
    RowCaptionWidth := 0;
    ...
    begin
      ...
      begin
        for I := 0 to CardCount - 1 do
          if J < Cards[I].LayerCount then
          begin
            ALayerCaptionWidths[J] := Max(ALayerCaptionWidths[J],
              Cards[I].Layers[J].MeasureCaptionWidth(ACanvas));
          end;
      end
      else
        ALayerCaptionWidths[J] := Adapter.CardCaptionWidth;
      RowCaptionWidth := Max(RowCaptionWidth, ALayerCaptionWidths[J]);
    end;
    ...
    for I := 0 to CardCount - 1 do
      for J := 0 to Length(ALayerCaptionWidths) - 1 do
      begin
        Cards[I].SetCaptionWidth(ACanvas, J, Min(ALayerCaptionWidths[J],
          RowCaptionWidth), RowCaptionWidth, Formatter.CaptionAutoWidth);
      end;
  end;
...
begin
  ...
  begin
    ...
    InflateRect(Result, 0, -FooterItemInflateVert);
  end;
end;
...
begin
  ...
  FIndex := AnIndex;
  Adapter.Summary.GetGroupSummaryInfo(GetCorrectRowIndex(AGridRow,
    AGroupLevel), SummaryItems, SummaryValues);
  FInternalLineCount := Adapter.GetFooterLineCount(
...
begin
  inherited;
  AIndex := Adapter.CheckSummaryItemIndex(SummaryItems, AIndex, False);
  Formatter.DoInitializeGroupFooterItem(ASubItem,
    GridRow, GroupLevel, SummaryValues^[AIndex], SummaryItems[AIndex]);
  Formatter.DoReportLinkInitializeGroupFooterItem(ASubItem,
    TcxGridColumn(SummaryItems[AIndex].ItemLink), GridRow, GroupLevel);
end;
...
begin
  Result := Adapter.GetFooterItemCount(
    SummaryItems, Adapter.GetGroupFooterMultiSummaries, False)
end;
...
begin
  ...
  if View <> nil then
    ...
    begin
      ...
      chbxLevelsSkipEmptyViews.Visible := IsDesigning or HasUnwrapableData;
...
begin
  ...
  dxSetupPreviewControlLookAndFeel(PreviewGrid.LookAndFeel,
    ReportLink.OptionsFormatting.LookAndFeelKind, ReportLink.Grid);
  PreviewGrid.Font := ReportLink.Font;
  PreviewGrid.RootLevelOptions.DetailTabsPosition :=
    DetailTabsPositionMap[ReportLink.OptionsView.Caption];
...
begin
  lbxStyles.ReportLinkStyles := ReportLink.ActiveStyles;
  inherited;
  cbxStyleSheets.ItemIndex :=
    cbxStyleSheets.Properties.Items.IndexOfObject(ActiveStyleSheet);
...
begin
  ...
  for I := 0 to RecordCount - 1 do
    AddRecord(I, [dxStringToAnsiString(CarVendorName(I)), dxPScxCommon.dxPSPreviewCarLogosAsString(I),
      dxStringToAnsiString(CarVendorCountry(I)), dxStringToAnsiString(CarModel(I)), dxStringToAnsiString(IsCarSUV[I])]);//, CarSalesCount[I]]);
end;
...
begin
  ...
  begin
    ReportLink.OptionsFormatting.LookAndFeelKind :=
      TcxLookAndFeelKind(TcxComboBox(Sender).ItemObject);
    Modified := True;
    UpdatePreview;
  end;;
end;
...
begin
  ...
  lblPreviewWindow.Parent := TcxPageControl(Sender).ActivePage;
  ...
  begin
    ...
    if PageControl1.ActivePage = tshCharts then
...
procedure TdxfmGridReportLinkDesignWindow.IsSUVModelCustomDrawCell(
  ...
  begin
    ...
    R := Bounds(X, Y, dxPSGlbl.CheckWidth, dxPSGlbl.CheckHeight);

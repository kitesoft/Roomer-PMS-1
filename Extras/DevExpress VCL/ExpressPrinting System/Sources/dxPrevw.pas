...
uses
  Types, Classes, Controls, Messages, Windows, Graphics, Forms, StdCtrls, SysUtils,
  ExtCtrls, FlatSB, IniFiles, dxWrap, dxBkgnd, dxPSUtl, dxPSGlbl,
  cxGraphics, cxGeometry, cxControls, dxPSReportRenderCanvas, cxLookAndFeels,
  cxLookAndFeelPainters, cxClasses, dxTouch;
...
uses
  Themes, UxTheme, Math, TypInfo, Registry, CommCtrl, dxPSRes, dxPSImgs, dxExtCtrls;
...
{$IFDEF DELPHI7}
  ...
  begin
    dxDrawThemeEdge(DC, cxStyleServices.GetElementDetails(twWindowRoot), R,
      BDR_RAISEDOUTER, BF_RECT);
    Exit;
  end;
{$ENDIF}
...
begin
  ...
  FPagesContentCache := TdxPreviewPageContentCachePool.Create(dxPreviewCachedPagesCount);
...
begin
  if not IsDesigning and (pobStoreInRegistry in OptionsBehavior) then
  begin
...
procedure TCustomdxPreview.DblClick;
begin
  if not IsScrollBarsArea(ScreenToClient(GetMouseCursorPos)) then
    inherited;
end;

function TCustomdxPreview.GetDefaultPanOptions: Integer;
begin
  Result := inherited GetDefaultPanOptions and not GC_PAN_WITH_GUTTER;
end;

function TCustomdxPreview.GetMainScrollBarsClass: TcxControlCustomScrollBarsClass;
begin
...
procedure TCustomdxPreview.CheckLeftTopPosition;
begin
  LeftPos := LeftPos;
  TopPos := TopPos;
end;
...
procedure TCustomdxPreview.CheckZoomFactor;
  ...
  function CalcZoomFactor(ADisplaySize, APageSize, APagesCount: Integer): Integer;
  var
    I: Integer;
  begin
    Result := MinZoomFactor;
    for I := APagesCount downto 1 do
    begin
      Result := Trunc(100 * (ADisplaySize - AbsoluteIndentLeft * (1 + I)) /
        (AbsoluteIndentRight + I * (APageSize + AbsoluteIndentRight)));
      if Result >= MinZoomFactor then Break;
    end;
    Result := Max(MinZoomFactor, Result);
  end;

  function CalcZoomFactorX(APagesCount: Integer): Integer;
  begin
    Result := CalcZoomFactor(ClientWidth, PageSize.X, APagesCount);
  end;

  function CalcZoomFactorY(APagesCount: Integer): Integer;
  begin
    Result := CalcZoomFactor(ClientHeight, PageSize.Y, APagesCount);
  end;
...
begin
  ...
  try
    case ZoomMode of
      pzmPageWidth:
        ZoomFactor := CalcZoomFactorX(1);
      pzmPages:
        ZoomFactor := Min(CalcZoomFactorX(PageXCount), CalcZoomFactorY(PageYCount));
    end;
  finally
    FZoomModeFixed := False;
  end;
  UpdateScrollBars;
  CheckLeftTopPosition;
...
procedure TCustomdxPreview.StopMarginDragging;
begin
  CancelDragMargin;
  DestroyPageNumberHint;
end;
...
procedure TCustomdxPreview.DrawPage(ACanvas: TcxCanvas; APage: TdxPreviewPage; ASelected: Boolean);
begin
  if ACanvas.RectVisible(APage.SiteBounds) then
  begin
...
function TCustomdxPreview.IsDefaultGesture(AGestureID: Integer): Boolean;
begin
...
procedure TCustomdxPreview.SetScrollBars(Value: TcxScrollStyle);
begin
...
function TCustomdxPreview.IsDoubleBufferedNeeded: Boolean;
begin
  Result := True;
end;
...
procedure TCustomdxPreview.WMLButtonUp(var Message: TWMLButtonUp);
begin
...
procedure TCustomdxPreview.ContentAreaChanged;
begin
  if HandleAllocated then
  begin
    UpdateScrollBars;
...
function TdxPreviewPageContentCachePool.Find(
  APage: TdxPreviewPage; out AItem: TdxPreviewPageContentCache): Boolean;
var
  I: Integer;
begin
  Result := False;
...
function TdxPreviewPageContentCachePool.FindInvisible(
  APage: TdxPreviewPage; out AItem: TdxPreviewPageContentCache): Boolean;
var
  I: Integer;
begin

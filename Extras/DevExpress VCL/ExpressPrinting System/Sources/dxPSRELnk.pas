...
begin
  ...
  repeat
    ...
    PageRenderInfo.LastChar := Min(LastChar, SendMessage(REHandle, EM_FORMATRANGE, 0, LPARAM(@FFormatRange)));
    PageRenderInfo.ContentBounds := cxRectScale(FFormatRange.rc, PixelsDenominator, 1440);
...
begin
  ...
  RenderInfo.PageHeaderBounds := cxRectScale(RenderInfo.PageHeaderBounds, PixelsPerInch, UnitsPerInch);
  RenderInfo.PageFooterBounds := cxRectScale(RenderInfo.PageFooterBounds, PixelsPerInch, UnitsPerInch);
...
var
  ...
  ALastChar: Integer;
begin
  ACanvas := Canvas.BeginCustomDraw(cxRect(cxNullPoint, RenderInfo.PageSize), nil, clDefault);
  try
    FillChar(AFormatRange, SizeOf(TFormatRange), 0);
    AFormatRange.hdc := ACanvas.Handle;
...
begin
  ...
  RenderInfo.PageHeaderBounds := cxRectScale(RenderInfo.PageHeaderBounds, UnitsPerInch, PixelsPerInch);
  RenderInfo.PageFooterBounds := cxRectScale(RenderInfo.PageFooterBounds, UnitsPerInch, PixelsPerInch);

...
uses
  ...
  dxCore, cxClasses, cxContainer, cxControls, cxGraphics,
...
type
  ...
  public
    ...
    function GetExpandDirection: TcxDirection; virtual;
    ...
    function GetGripSizeCorner: TdxCorner; virtual;
  ...
  private
    ...
    function GetPainter: TdxNavBarNavigationPanePainter;
    procedure SetNewBounds(ALeft, ATop, AWidth, AHeight: Integer);
  ...
  private
    ...
    procedure DrawVerticalText(AFont: TFont; const AText: string; const ARect: TRect; AState: TdxNavBarNavPanePartState);
...
{$ENDIF}
  ...
  cxGeometry, cxLookAndFeels, cxLookAndFeelPainters, dxOffice11,
...
begin
  ...
  AGroupCaptionImageOffsets := GetGroupCaptionImageOffsets;
  AMaxElementSize := Max(AGroupCaptionImageOffsets.Left +
    AGroupCaptionImageOffsets.Right + FMaxImageSize.cx,
    FOverflowPanelViewInfo.GetSignWidth + FOverflowPanelViewInfo.GetClientOffset.Left +
    FOverflowPanelViewInfo.GetClientOffset.Right);
...
function TdxNavBarNavigationPaneViewInfo.GetExpandDirection: TcxDirection;

  function GetMasterControlAlign: TAlign;
  begin
    if IsInternal then
      Result := TdxCustomNavBarAccess(NavBar).GetMasterNavBar.Align
    else
      Result := NavBar.Align;
  end;
...
function TdxNavBarNavigationPaneViewInfo.GetGripSizeCorner: TdxCorner;
begin
...
function TdxNavBarNavigationPaneViewInfo.IsInternal: Boolean;
begin
  Result := TdxCustomNavBarAccess(NavBar).IsInternal;
end;
...
begin
  if IsInternal then
  begin
    FSizeGripRect := cxRectBounds(cxNullPoint, GetGripSize, GetGripSize);
    case GetGripSizeCorner of
      coBottomRight:
        FSizeGripRect := cxRectSetOrigin(FSizeGripRect,
          cxPoint(ClientWidth - GetGripSize, ClientHeight - GetGripSize));
    else // coBottomLeft
      FSizeGripRect := cxRectSetOrigin(FSizeGripRect,
        cxPoint(0, ClientHeight - GetGripSize));
    end;
  end;
end;
...
begin
  ...
  try
    FMaxImageSize := cxSize(FOverflowPanelViewInfo.GetImageWidth, FOverflowPanelViewInfo.GetImageHeight);
...
procedure TdxNavBarNavigationPaneViewInfo.CorrectBounds;
  ...
  begin
    AMinItemPanelHeight := FItemPanelViewInfo.GetMinHeight;
    ...
    if AItemPanelHeight + AHeightDifference < AMinItemPanelHeight then // if AHeightDifference < 0
    begin
...
begin
  ...
  if IsActiveGroupVisible and (ActiveGroupViewInfo.Control <> nil) and
    (ActiveGroupViewInfo.Control.GetMinHeight <> 0) then
...
var
  AYNavBarRelativePosition: Integer;
begin
  AYNavBarRelativePosition := cxRectHeight(ViewInfo.HeaderRect) - 1;
  if Painter.ViewInfo.GetExpandDirection = dirLeft then
    Result := NavBar.ClientToScreen(cxPoint(-cxRectWidth(FRect),
      AYNavBarRelativePosition))
  else
...
type
  ...
  protected
    ...
    procedure DoLinkHotTrack(ANavBar: TdxCustomNavBar; ALink: TdxNavBarItemLink); override;
...
begin
  View := FNavBar.View;
  LookAndFeelSchemes := FNavBar.LookAndFeelSchemes;
  LookAndFeel.MasterLookAndFeel := TdxCustomNavBarAccess(FNavBar).LookAndFeel;
...
begin
  ...
  begin
    ...
    if ADestinationLink = nil then
      ASourceLink.Data := TdxNativeInt(ADestinationGroup.CreateLink(Items[ASourceLink.Item.Index]));
  end;
  for I := 0 to ASourceGroup.LinkCount - 1 do
  begin
    ASourceLink := ASourceGroup.Links[I];
...
begin
  ...
  inherited DoLinkClick(ANavBar, ALink);
end;

procedure TdxNavBarPopupInnerControlController.DoLinkHotTrack(ANavBar: TdxCustomNavBar; ALink: TdxNavBarItemLink);
begin
  if ALink <> nil then
    ALink := TdxNavBarItemLink(ALink.Data);
  inherited DoLinkHotTrack(NavBar.FNavBar, ALink);
end;

procedure TdxNavBarPopupInnerControlController.DoLinkPress(ANavBar: TdxCustomNavBar; ALink: TdxNavBarItemLink);
begin
  if ALink <> nil then
    ALink := TdxNavBarItemLink(ALink.Data);
  inherited DoLinkPress(NavBar.FNavBar, ALink);
end;
...
begin
  ...
  begin
    ...
    SetNewBounds(ANewBounds.Left, ANewBounds.Top,
      cxRectWidth(ANewBounds), cxRectHeight(ANewBounds));
  end;
...
procedure TdxNavBarPopupControl.SetNewBounds(ALeft, ATop, AWidth, AHeight: Integer);
begin
...
begin
  ...
  DrawVerticalText(AItemPanelViewInfo.CollapseBarFont, AItemPanelViewInfo.CollapseBarText, AItemPanelViewInfo.CollapseBarRect, AState);
...
begin
  ...
  DrawVerticalText(AItemViewInfo.Font, AItemViewInfo.Caption, AItemViewInfo.TextRect, AState);
...
procedure TdxNavBarNavigationPanePainter.DoDrawSizeGrip(ACanvas: TcxCanvas;
  const ARect: TRect);
const
  ASpace = 4;
  ALineCount = 3;
...
procedure TdxNavBarNavigationPanePainter.DrawVerticalText(AFont: TFont; const AText: string; const ARect: TRect; AState: TdxNavBarNavPanePartState);
begin
  cxCanvas.Font := AFont;

...
{$IFDEF DELPHI6}
  Types,
{$ENDIF}
  Windows, Graphics, Classes, ImgList, dxNavBar, dxNavBarStyles;
...
type
  ...
  protected
      // Drawing
    procedure DoDrawDropTargetLinkSelection(AItemGroup: TdxNavBarGroupViewInfo; AItem1, AItem2: TdxNavBarLinkViewInfo); virtual;
...
uses
  CommCtrl, cxGraphics, cxGeometry, dxOffice11, dxNavBarGraphics;

{ TdxNavBarCustomImagePainter }

class procedure TdxNavBarCustomImagePainter.DrawImage(ACanvas: TCanvas;
...
class function TdxNavBarCustomImagePainter.GetRGBColor(Value: TColor): DWORD;
begin
...
class procedure TdxNavBarCustomImagePainter.InternalDrawImage(ACanvas: TCanvas;
  AImageList: TCustomImageList; AImageIndex: Integer; ARect: TRect; AEnabled: Boolean);
var
...
begin
  ...
  if IsPressed(AState) then
    AFillColor := GetRealColor(GetLightColor(14, 44, 40))
  else
    AFillColor := GetRealColor(GetLightColor(-2, 30, 72));
end;
...
class procedure TdxNavBarCustomSignPainter.DrawSign(ACanvas: TCanvas; ARect: TRect;
  AForeColor, ABackColor1, ABackColor2: TColor; AState: TdxNavBarObjectStates);
begin
  InternalDrawSign(ACanvas, ARect, AForeColor, ABackColor1, ABackColor2, AState);
end;

class procedure TdxNavBarCustomSignPainter.InternalDrawSign(ACanvas: TCanvas; ARect: TRect;
  AForeColor, ABackColor1, ABackColor2: TColor; AState: TdxNavBarObjectStates);
begin
...
begin
  ...
  begin
    ImagePainterClass.DrawImage(Canvas, ImageList, ImageIndex, ImageRect,
      IsEnabled or UseDisabledImages);
...
procedure TdxNavBarElementPainter.DoDrawDropTargetLinkSelection(AItemGroup: TdxNavBarGroupViewInfo;
  AItem1, AItem2: TdxNavBarLinkViewInfo);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      rapt1.x := lapt1.x + cxRectWidth(AItemGroup.Rect) - 1 - 2 * AViewInfoAccess.GetDragArrowWidth;
      rapt2.x := lapt2.x + cxRectWidth(AItemGroup.Rect) - 1;
      rapt2.y := lapt2.y;
      rapt3.x := lapt3.x + cxRectWidth(AItemGroup.Rect) - 1;
      rapt3.y := lapt3.y;
    end;

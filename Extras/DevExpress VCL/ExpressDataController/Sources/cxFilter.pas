...
uses
  Types, SysUtils, Classes, Variants, Windows, dxCore, dxCoreClasses;
...
type
  ...
  protected
    ...
    procedure Lock; {$IFDEF DELPHI9} inline; {$ENDIF}
    ...
    procedure Unlock; {$IFDEF DELPHI9} inline; {$ENDIF}
  public
  ...
  private
    ...
    FValue: Variant;
    function GetCriteria: TcxFilterCriteria; {$IFDEF DELPHI9} inline; {$ENDIF}
  protected
    procedure Prepare;
...
uses
  ...
  cxVariants, cxLike, cxFilterConsts, cxDateUtils;
...
function TcxFilterNotLikeOperator.IsExpression: Boolean;
begin
...
begin
  Result := ' NOT (' + CriteriaItem.GetFieldName + ' LIKE ' + GetExpressionValue(AValue) + ')';
end;
...
begin
  Result := False;
  AStartIndex := GetStartValueIndex;
  L := AStartIndex;
  H := Count - 1;
  while L <= H do
  begin
  ...
  begin
    try
      ...
      if (fcoIgnoreNull in Options) and not AItem.Operator.IsNullOperator and AItem.ValueIsNull(V) then
        ...
        Result := AItem.Operator.CompareValues(V, AItem.PreparedValue);
    except
      on EVariantError do

...
type
  ...
  private
    ...
    function GetProvider: TcxCustomDataProvider; {$IFDEF DELPHI9} inline; {$ENDIF}
...
//    function GetRecordCount: Integer; virtual;
  ...
  private
    function GetRecordCount: Integer; {$IFDEF DELPHI9} inline; {$ENDIF}
  public
    RowIndex: Integer;
    Level: Integer;
    ...
    Expanded: Boolean;
    constructor Create;
    function Contains(ARecordListIndex: Integer): Boolean; {$IFDEF DELPHI9} inline; {$ENDIF}
  ...
  private
    ...
    function GetFieldCount: Integer; {$IFDEF DELPHI9} inline; {$ENDIF}
    function GetItem(Index: Integer): TcxDataGroupInfo; {$IFDEF DELPHI9} inline; {$ENDIF}
  ...
  protected
    ...
    procedure AddToList(const ADataGroupInfo: TcxDataGroupInfo); {$IFDEF DELPHI9} inline; {$ENDIF}
    procedure Clear;
  ...
  private
    ...
    function GetField(Index: Integer): TcxCustomDataField; {$IFDEF DELPHI9} inline; {$ENDIF}
    function GetFieldCount: Integer; {$IFDEF DELPHI9} inline; {$ENDIF}
    function GetItem(Index: Integer): PcxDataSelectionInfo; {$IFDEF DELPHI9} inline; {$ENDIF}
  protected
    procedure Changed;
  ...
  private
    ...
    function GetFocusedRowIndex: Integer; {$IFDEF DELPHI9} inline; {$ENDIF}
    function GetGroupLevelCount: Integer;
    function GetNewItemRowFocusingChanged: Boolean; {$IFDEF DELPHI9} inline; {$ENDIF}
    function GetPrevFocusedRecordIndex: Integer; {$IFDEF DELPHI9} inline; {$ENDIF}
    function GetPrevFocusedRowIndex: Integer; {$IFDEF DELPHI9} inline; {$ENDIF}
  ...
  public
    procedure Load(AItemIndex: Integer; AInitSortByDisplayText: Boolean = True;
      AUseFilteredValues: Boolean = False; AAddValueItems: Boolean = True); virtual;
    property DataController: TcxCustomDataController read GetDataController;
  end;
  ...
  private
    ...
    function GetActualValue(AValue: TdxDefaultBoolean): Boolean;
  protected
    function IsMultiThreadedFiltering: Boolean; {$IFDEF DELPHI9} inline; {$ENDIF}
    function IsMultiThreadedSorting: Boolean; {$IFDEF DELPHI9} inline; {$ENDIF}
    function IsMultiThreadedSummary: Boolean; {$IFDEF DELPHI9} inline; {$ENDIF}
  ...
  TcxDataControllerChange = (dccLayout, dccData, dccFocus, dccSelection,
    ...
    dccBookmark, dccUpdateRecord, dccGridMode);
  ...
  protected
    procedure AssignTo(AInfo: TcxUpdateControlInfo); override;
    function CreateRowIndexes: TList; {$IFDEF DELPHI9} inline; {$ENDIF}
  public
    constructor Create;
...
//    FDataChangeInfo: TcxDataChangeInfo;
    ...
    function GetCustomDataSource: TcxCustomDataSource; {$IFDEF DELPHI9} inline; {$ENDIF}
    function GetDetailMode: TcxDataControllerDetailMode;
    function GetFilteredIndexByRecordIndex(Index: Integer): Integer; {$IFDEF DELPHI9} inline; {$ENDIF}
    function GetFilteredRecordCount: Integer; {$IFDEF DELPHI9} inline; {$ENDIF}
    function GetFilteredRecordIndex(Index: Integer): Integer;
    function GetIsEditing: Boolean;
    function GetIsPattern: Boolean;
    function GetLockCount: Integer; {$IFDEF DELPHI9} inline; {$ENDIF}
    function GetNewItemRowFocused: Boolean;
    function GetOptions: TcxDataControllerOptions; {$IFDEF DELPHI9} inline; {$ENDIF}
    ...
    procedure SetFocusedRowIndex(Value: Integer); {$IFDEF DELPHI9} inline; {$ENDIF}
  ...
  protected
    ...
    function GetSummaryItemClass: TcxDataSummaryItemClass; virtual;
    function GetSummaryGroupItemLinkClass: TcxDataSummaryGroupItemLinkClass; virtual;
    function HasRecordFilter: Boolean; virtual;
...
//    function GetDisplayText(ARecordIndex, AItemIndex: Integer): string; virtual;
    ...
    function GetGroupRowValue(const ARowInfo: TcxRowInfo; AItemIndex: Integer): Variant; virtual;
    ...
    procedure PopulateFilterValues(AList: TcxDataFilterValueList;
      ...
      var AUseFilteredRecords: Boolean; out ANullExists: Boolean); virtual;
...
uses
  RTLConsts, cxDateUtils,
{$IFDEF DELPHI9}
  Windows,
{$ENDIF}
  cxFormats, cxDataConsts, cxFilterConsts, dxThreading;
...
constructor TcxDataGroupInfo.Create;
begin
  inherited Create;
  EndRecordListIndex := -1; 
end;

function TcxDataGroupInfo.Contains(ARecordListIndex: Integer): Boolean;
begin
  Result := (ARecordListIndex >= BeginRecordListIndex) and
    (ARecordListIndex <= EndRecordListIndex);
end;

function TcxDataGroupInfo.GetRecordCount: Integer;
begin
  Result := EndRecordListIndex - BeginRecordListIndex + 1;
end;
    ...
    try
      ...
      try
        ...
        begin
          ...
          if AExpandingInfo.Level < ALocateObject.Level then
            AValueDefReader.Truncate(AExpandingInfo.Level);
          AValueDefReader.SetValue(FExpandingInfo.Fields[AExpandingInfo.Level].ValueDef,
            AExpandingInfo.Value);
          if GroupNeedsRestoration(AExpandingInfo) then
          begin
            ALocateObject.Level := AExpandingInfo.Level;
            ALocateObject.ReadData(AValueDefReader);
            ARecordIndex := ALocateObject.FindRecordIndex;
            if ARecordIndex <> -1 then
              ExpandGroup(ARecordIndex, ALocateObject.Level, AExpandingInfo.State);
          end;
        end;
      finally
...
procedure TcxCustomDataControllerInfo.DoChangeFocusedRow(AValue: Integer; AIsInternal: Boolean);
  ...
  begin
    ...
    if FFocusingInfo.IsNeedUpdate then
    begin
...
begin
  Result := -1;
  if FocusingInfo.RecordIndex < 0 then
    Exit;
  for I := 0 to RecordList.Count - 1 do
    if GetInternalRecordIndex(I) = FocusingInfo.RecordIndex then
    begin
      Result := I;
      Break;
    end;
...
begin
  Result := FGroupingFieldList.Count;
end;
...
begin
  ...
  with AIterateData^ do
    if (ARecordIndex <> EditingRecordIndex) and
      ( 
       (IsFiltering and not DataController.Filter.DoFilterRecord(ARecordIndex)) or
       (HasFilterRecord and not DataController.DoFilterRecord(ARecordIndex)) or
       (HasIncrementalFilter and not DataController.DoIncrementalFilterRecord(ARecordIndex))
      ) then
...
var
  ...
  AHasIncrementalFilter, AIsFiltering, AHasFilterRecord: Boolean;
  AIterateData: TdxFilterIterateData;
begin
  AHasIncrementalFilter := HasIncrementalFilter;
  AHasFilterRecord := not IsGridMode and HasRecordFilter;
  AIsFiltering := Filter.IsFiltering;
  if not (AHasIncrementalFilter or AIsFiltering or AHasFilterRecord) then
    Exit;
  ...
  try
    ...
    if IsMultiThreadedFiltering and Filter.SupportsMultiThreading then
    begin
      AIterateData.DataController := Self;
      AIterateData.EditingRecordIndex := AEditingRecordIndex;
      AIterateData.HasIncrementalFilter := AHasIncrementalFilter;
      AIterateData.HasFilterRecord := AHasFilterRecord;
      AIterateData.IsFiltering := AIsFiltering;
    ...
    begin
      for I := 0 to ACount - 1 do
        if (I <> AEditingRecordIndex) and
          ( 
           (AIsFiltering and not Filter.DoFilterRecord(I)) or
           (AHasFilterRecord and not DoFilterRecord(I)) or
           (AHasIncrementalFilter and not DoIncrementalFilterRecord(I))
          ) then
...
function TcxCustomDataController.HasRecordFilter: Boolean;
begin
...
function TcxCustomDataController.GetGroupRowValue(
...
procedure TcxCustomDataController.PopulateFilterValues(AList: TcxDataFilterValueList;
  AItemIndex: Integer; ACriteria: TcxFilterCriteria;
...
begin
  ANullExists := False;
  AField := Fields[AItemIndex];
  if AUseFilteredRecords and (FilteredRecordCount <> 0) then
    I := FilteredRecordCount
  else
  begin
  ...
  begin
    ...
    if AUseFilteredRecords then
      ...
      ARecordIndex := I;
    V := GetFilterDataValue(ARecordIndex, AField);
    if ACriteria.ValueIsNull(V) then
      ANullExists := True
    else
      AList.Add(fviValue, V, GetFilterDisplayText(ARecordIndex, AItemIndex), False);
  end;
end;
...
begin
  Result := GetActualValue(FFiltering);
end;

function TcxDataControllerMultiThreadedOptions.IsMultiThreadedSorting: Boolean;
begin
  Result := GetActualValue(FSorting);
end;

function TcxDataControllerMultiThreadedOptions.IsMultiThreadedSummary: Boolean;
begin
  Result := GetActualValue(FSummary);
end;

function TcxDataControllerMultiThreadedOptions.GetActualValue(AValue: TdxDefaultBoolean): Boolean;
begin
  if not dxCanUseMultiThreading then
    Result := False
  else
...
begin
  ...
  if Result and (dcicSorting in DataControllerInfo.FChanges) and
...
begin
  if Assigned(Field) then
    Result := not DataController.IsConversionNeededForCompare(Field)
  else
    Result := True;
end;
...
begin
  ...
  if DataController.IsGridMode then
    ...
    if AAddValueItems then
      DataController.PopulateFilterValues(Self, AItemIndex, Criteria, AUseFilteredValues, ANullExists)
    else
      ANullExists := False;
  if ANullExists then

...
type
  ...
  protected
    function CanCreateParam(const AValue: Variant): Boolean; override;
  end;
...
function TdxServerModeADOSQLQueryBuilder.CanCreateParam(const AValue: Variant): Boolean;
var
  AHour, AMinute, ASecond, AMillisecond: Word;
begin
  Result := inherited CanCreateParam(AValue) and
    (not VarIsStr(AValue) or (Length(AValue) > 0));
  if Result and VarIsDate(AValue) then
  begin
...
begin
  ...
  try
    ...
    try
      ...
      begin
        ATableType := ATypeField.{$IFDEF DELPHI11}AsWideString{$ELSE}AsString{$ENDIF};
        if ATableType = 'VIEW' then
          AList.Add(ANameField.{$IFDEF DELPHI11}AsWideString{$ELSE}AsString{$ENDIF});
        ADataSet.Next;
      end;
    finally
      AList.EndUpdate;

...
uses
  SysUtils, Classes, dxCore, Variants;
...
function GetBcdDigit(const AValue: TBcd; ADigit: Integer): Integer; {$IFDEF DELPHI9} inline; {$ENDIF}
begin
  if Odd(ADigit) then
...
begin
  ...
  {$ENDIF}
    ...
    if (AVarType = VarSQLTimeStamp){$IFDEF DELPHI14} or (AVarType = varSQLTimeStampOffset){$ENDIF} then
    begin

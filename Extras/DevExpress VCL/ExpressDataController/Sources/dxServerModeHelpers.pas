...
var
  ...
  ALlsArgs: TdxLinearLeastSquaresArgs;
begin
  ALlsArgs := TdxLinearLeastSquaresArgs.Create(Length(ACombination), PackSamples(ASamples, ACombination));
  try
    ASolution := LinearLeastSquares(ALlsArgs);
  finally
    ALlsArgs.Free;
  end;
...
var
  ...
  ABaseTime, ATargetFillTime, AEdgeTime, ATargetTime, AClaculatedTake: Double;
  I, ABaseSkip, AFillTake, ASkipFromTop, ASkipFromBottom, AMaxTake, AScanToMiddleFromTop, AScanToMiddleFromBottom,
  AScanToMiddle, ATake, ASkip, AScan, ACalculatedTake, ALastIndex, AFirstIndex: Integer;
begin
{$IFNDEF DELPHI11}
  Result := nil;
{$ENDIF}
  ...
  try
    ...
    if CalculateTakeFromFixedTimeAndSkip(AParam.Model, AEdgeTime, ASkip, ACalculatedTake) then
    begin
      ATake := ACalculatedTake;
    ...
    if CalculateTakeFromFixedTimeAndScan(AParam.Model, AEdgeTime, AScan, ACalculatedTake) then
    begin
      ATake := ACalculatedTake;
      if ATake > AMaxTake then
        ATake := AMaxTake;
  ...
  if not IsZero(AParam.Model.TakeCoeff + AParam.Model.ScanCoeff / 2) then
    AClaculatedTake := (ATargetTime - AParam.Model.ScanCoeff * AScanToMiddle - AParam.Model.ConstantPart) / (AParam.Model.TakeCoeff + AParam.Model.ScanCoeff / 2)
  else
    AClaculatedTake := Infinity;
  if AClaculatedTake >= AMaxTake then
    ATake := AMaxTake
  else
    ATake := Trunc(AClaculatedTake);

...
procedure ImportLayout(ALayoutControl: TdxLayoutControl; ADefaultImport: Boolean = False);
...
begin
  ...
  if IsPageControl or IsGroup or IsSplitter or IsSeparator then
    FreeAndNil(FControl);
end;
...
procedure ImportLayout(ALayoutControl: TdxLayoutControl; ADefaultImport: Boolean = False);
  ...
  function IsParent(AParent, AChild: TWinControl): Boolean;
  begin
    Result := False;
  ...
  function CanExport(AControl: TWinControl): Boolean;
  begin
    Result := (AControl <> ALayoutControl) and
      ((ALayoutControl.Parent = AControl) or not IsParent(AControl, ALayoutControl)) and
...
begin
  ...
  try
    ...
    seDeltaX.Value := 48;
    seDeltaY.Value := 16;
    if ADefaultImport or (ShowModal = mrOk) then
      if cbContainers.ItemIndex >= 0 then
      begin

...
procedure TdxLayoutSplitterDragAndDropObject.DragAndDrop(const P: TPoint; var Accepted: Boolean);
  ...
  begin
    if (FLeftItem <> nil) and (GetNewLeftItemSize(ADelta) < GetItemMinSize(FLeftItem.ViewInfo)) then
      ADelta := GetItemMinSize(FLeftItem.ViewInfo) - FLeftItemSize;
    if (FRightItem <> nil) and (GetNewRightItemSize(ADelta) < GetItemMinSize(FRightItem.ViewInfo)) then
  ...
  begin
    ...
    if FLeftItem <> nil then
      Result := Result and (GetNewLeftItemSize(ADelta) >= GetItemMinSize(FLeftItem.ViewInfo));
    if Result and (FRightItem <> nil) then
      Result := Result and (GetNewRightItemSize(ADelta) >= GetItemMinSize(FRightItem.ViewInfo));
  end;
...
begin
  ...
  try
    ...
    if AGroup.IsRoot then
      R := AGroup.Container.ClientBounds
    else
...
begin
  ...
  begin
    if AGroup.IsRoot then
      AContentBounds := AGroup.Container.ClientBounds
    else

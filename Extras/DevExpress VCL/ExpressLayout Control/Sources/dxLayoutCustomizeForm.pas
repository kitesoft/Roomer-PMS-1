...
begin
  ...
  StoreTreeViewWndProc(tvItems.InnerTreeView, FItemsWindowProcLinkedObject, ItemsWndProc);
...
begin
  if Container.IsDesigning then
    Caption := Format(cxGetResourceString(@sdxLayoutControlDesignerCaptionFormat),
      [cxGetFullComponentName(TdxLayoutContainerAccess(Container).GetItemsParentComponent)])
  else
...
begin
  ...
  begin
    ...
    acTreeViewItemRename.Enabled := ACanModify and (tvItems.Selected <> nil) and (tvItems.SelectionCount = 1) and
      not (TObject(tvItems.Selected.Data) as TdxCustomLayoutItem).IsRoot and
      not TdxCustomLayoutItemAccess(tvItems.Selected.Data).IsParentLocked;
    ...
    acTreeViewItemsDelete.Enabled := CanDeleteItems(tvItems) and ACanAddItem;
    ...
    acAlignBy.Enabled := (tvItems.SelectionCount > 1) and ACanModify and ACanAddItem;
...
begin
  lcMain.LayoutLookAndFeel := TdxLayoutContainerAccess(Container).GetLayoutLookAndFeel;
end;
...
begin
  Result := HasClassInSelection(TdxLayoutCustomLabeledItem);
end;

function TdxLayoutControlCustomizeForm.HasGroupInSelection: Boolean;
begin
  Result := HasClassInSelection(TdxLayoutGroup);
end;
...
begin
  ...
  for I := 0 to tvItems.SelectionCount - 1 do
  begin
    Result := IsHiddenGroup(TdxCustomLayoutItem(tvItems.Selections[I].Data));
    if Result then
      Break;
  end;
end;
...
begin
  ...
  if not Container.IsDesigning then
    for I := 0 to tvItems.SelectionCount - 1 do
      if TObject(tvItems.Selections[I].Data) is TdxLayoutGroup then      
      begin                                                        
        Result := TdxLayoutGroup(tvItems.Selections[I].Data).Locked;
        if Result then
          Break;
      end;
end;
...
begin
  Result := (AItem is TdxLayoutGroup) and TdxLayoutGroup(AItem).Hidden;
end;
...
procedure TdxLayoutControlCustomizeForm.CalculateTreeViewPopupActionEnables;
  ...
  begin
    ...
    if not Result and (tvItems.SelectionCount > 1) then
      ...
      begin
        Result := TdxCustomLayoutItem(tvItems.Selections[I - 1].Data).Parent =
          TdxCustomLayoutItem(tvItems.Selections[I].Data).Parent;
        if not Result then
          Break;
      end;
  end;
...
begin
  ...
  AHasGroupsOnly := AHasGroup and not AHasLabeledItem and not AHasDirectionalItem;
  ...
  acUngroup.Visible := AHasGroupsOnly and not AHasSpecialGroup and 
    (tvItems.SelectionCount = 1) and not TdxCustomLayoutItem(tvItems.Selected.Data).IsRoot and 
...
const
  ...
  function GetIndexByItem(AParent: TdxLayoutGroup; AItem: TdxCustomLayoutItem): Integer;
  begin

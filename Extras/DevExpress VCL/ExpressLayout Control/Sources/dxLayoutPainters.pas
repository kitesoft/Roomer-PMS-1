...
{$IFDEF DELPHI7}
  UxTheme, Themes,
{$ENDIF}
  Math, cxGraphics, cxGeometry, cxControls;
...
{$IFDEF DELPHI7}
  ...
  begin
    cxStyleServices.DrawElement(Canvas.Handle,
      cxStyleServices.GetElementDetails(tbGroupBoxNormal), ViewInfo.FrameBounds);
    Exit;
  end;  
{$ENDIF}
...
procedure TdxLayoutGroupCaptionStandardPainter.DoDrawText;
  ...
  var
    ...
    R: TRect;
  begin
    AFlags := {$IFDEF DELPHI16}TTextFormatFlags{$ENDIF}(cxFlagsToDTFlags(ViewInfo.CalculateTextFlags));
    R := ARect;
...
begin
  ...
  begin
    ...
    if ViewInfo.GetRotationAngle = ra0 then
      DrawThemeText(Canvas.Handle, ATextRect)
    else
    begin
      ...
      try
        ...
        ViewInfo.PrepareCanvas(ABitmap.cxCanvas);
        DrawThemeText(ABitmap.cxCanvas.Handle, ABitmap.ClientRect);
        ABitmap.Rotate(ViewInfo.GetRotationAngle);
        cxDrawImage(Canvas.Handle, ATextRect, ATextRect, ABitmap, nil, -1, idmNormal);
      finally
        ABitmap.Free;
      end;
    end;
...
begin
  ...
  if IsTextVisible and IsImageVisible then
    ...
    begin
      if GetTextWidth < GetImageWidth then
      begin
        Result := cxRectOffset(Result, (GetImageWidth - GetTextWidth) div 2, 0);
        Result := cxRectSetWidth(Result, TextWidth)
      end;
    end
    else
    begin
      if GetTextHeight < GetImageHeight then
      begin
        Result := cxRectOffset(Result, 0, (GetImageHeight - GetTextHeight) div 2);
        Result := cxRectSetHeight(Result, TextHeight);
      end;
    end;
end;
...
function TdxLayoutGroupCaptionCxLookAndFeelViewInfo.GetLookAndFeelPainter: TcxCustomLookAndFeelPainter;
begin
...
function TdxLayoutGroupCxLookAndFeelViewInfo.GetLookAndFeelPainter: TcxCustomLookAndFeelPainter;
begin
...
begin
  ...
  begin
    ...
    LookAndFeelPainter.DrawGroupBoxCaption(Canvas, R,
      ViewInfo.CaptionViewInfo.Bounds, dxCaptionPositionMap[True, ViewInfo.CaptionSide]);
  end;
end;
...
function TdxLayoutItemCaptionCxLookAndFeelViewInfo.GetLookAndFeelPainter: TcxCustomLookAndFeelPainter;
begin
  Result := TdxLayoutCxLookAndFeel(LayoutLookAndFeel).LookAndFeelPainter;
end;

...
uses
  Types, Windows, Classes, Controls, Graphics, cxGraphics, dxLayoutCommon,
...
type
  ...
  public
    ...
    function GetGroupBorderOffset(AContainer: TComponent; ASide: TdxLayoutSide; ACaptionSide: TdxLayoutSide): Integer; virtual;
    function GetGroupBorderWidth(AContainer: TComponent; ASide: TdxLayoutSide; ACaptionSide: TdxLayoutSide;
  ...
  public
    ...
    function GetGroupBorderOffset(AContainer: TComponent; ASide: TdxLayoutSide; ACaptionSide: TdxLayoutSide): Integer; override;
    function GetGroupBorderWidth(AContainer: TComponent; ASide: TdxLayoutSide; ACaptionSide : TdxLayoutSide;
      AHasCaption, AIsExpanded: Boolean): Integer; override;
  published
  ...
  private
    function InternalGetGroupBorderOffset(AContainer: TComponent; ASide: TdxLayoutSide; ACaptionSide: TdxLayoutSide): Integer;
...
const
  CaptionSideToCaptionPosition: array[TdxLayoutSide] of TcxGroupBoxCaptionPosition =
...
begin
  ...
  begin
    with Source as TdxLayoutLookAndFeelOffsets do
    begin
      Self.ControlOffsetHorz := ControlOffsetHorz;
      Self.ControlOffsetVert := ControlOffsetVert;
      Self.ItemOffset := ItemOffset;
      Self.ItemsAreaOffsetHorz := ItemsAreaOffsetHorz;
      Self.ItemsAreaOffsetVert := ItemsAreaOffsetVert;
      Self.RootItemsAreaOffsetHorz := RootItemsAreaOffsetHorz;
      Self.RootItemsAreaOffsetVert := RootItemsAreaOffsetVert;
    end;
  end
  else
    inherited;
...
begin
  Result := DLUToPixels(GetItemCaptionFont(AContainer), Offsets.ControlOffsetHorz);
end;

function TdxCustomLayoutLookAndFeel.GetElementOffsetVert(AContainer: TComponent): Integer;
begin
  Result := DLUToPixels(GetItemCaptionFont(AContainer), Offsets.ControlOffsetVert);
end;
...
begin
  Result := DLUToPixels(GetItemCaptionFont(AContainer), Offsets.ItemOffset);
end;

function TdxCustomLayoutLookAndFeel.GetGroupBorderOffset(AContainer: TComponent; ASide: TdxLayoutSide; ACaptionSide: TdxLayoutSide): Integer;
begin
...
begin
  Result := DLUToPixels(GetGroupCaptionFont(AContainer), Offsets.ItemsAreaOffsetHorz);
end;

function TdxCustomLayoutLookAndFeel.GetItemsAreaOffsetVert(AContainer: TComponent): Integer;
begin
  Result := DLUToPixels(GetGroupCaptionFont(AContainer), Offsets.ItemsAreaOffsetVert);
end;

function TdxCustomLayoutLookAndFeel.GetRootItemsAreaOffsetHorz(AContainer: TComponent): Integer;
begin
  Result := DLUToPixels(GetGroupCaptionFont(AContainer), Offsets.RootItemsAreaOffsetHorz);
end;

function TdxCustomLayoutLookAndFeel.GetRootItemsAreaOffsetVert(AContainer: TComponent): Integer;
begin
  Result := DLUToPixels(GetGroupCaptionFont(AContainer), Offsets.RootItemsAreaOffsetVert);
end;
...
function TdxLayoutWebLookAndFeel.GetGroupBorderOffset(AContainer: TComponent; ASide: TdxLayoutSide; ACaptionSide: TdxLayoutSide): Integer;
begin
...
begin
  with Items[AIndex]^ do
    cxGetTextMetrics(Font, TextMetric);
end;
...
begin
  ...
  begin
    if AIsExpanded or not (ASide in [ACaptionSide, GetOppositeSide(ACaptionSide)]) then
    begin
      ACaptionSize := CaptionSizeMap[LookAndFeelPainter.IsGroupBoxCaptionTextDrawnOverBorder(
        CaptionSideToCaptionPosition[ACaptionSide])];
      Result := InternalGetGroupBorderOffset(AContainer, ASide, ACaptionSide);
    end
    else
    begin
      ACaptionSize := CaptionSizeMap[False];
    ...
    if (ASide = ACaptionSide) and AHasCaption then
      Inc(Result, VDLUToPixels(GetGroupCaptionFont(AContainer), ACaptionSize));
  end
  else
    Result := inherited GetGroupBorderWidth(AContainer, ASide, ACaptionSide, AHasCaption, AIsExpanded);
end;
...
function TdxLayoutCxLookAndFeel.InternalGetGroupBorderOffset(AContainer: TComponent; ASide: TdxLayoutSide; ACaptionSide: TdxLayoutSide): Integer;
begin
  if IsSkinPainterUsed then
  begin
    Result := FrameWidths[ASide, ACaptionSide] + DLUToPixels(GetGroupCaptionFont(AContainer),
      IfThen(ASide in [ACaptionSide, GetOppositeSide(ACaptionSide)], 4, 2));
    Result := Result - inherited GetGroupBorderOffset(AContainer, ASide, ACaptionSide) +
      GetGroupBorderOffset(AContainer, ASide, ACaptionSide);
  end
  else
    Result := inherited GetGroupBorderOffset(AContainer, ASide, ACaptionSide);
end;
...
begin
  if IsSkinPainterUsed then
    with LookAndFeelPainter.GroupBoxBorderSize(True, CaptionSideToCaptionPosition[ACaptionSide]) do
    begin

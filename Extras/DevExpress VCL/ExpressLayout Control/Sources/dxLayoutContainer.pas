...
type
  ...
  TdxLayoutCustomizeFormMenuItem = (cfmiAlignHorz, cfmiAlignVert, cfmiDirection, cfmiCaptionLayout, cfmiCaptionAlignHorz,
    cfmiCaptionAlignVert, cfmiCaption, cfmiBorder, cfmiExpandButton, cfmiGrouping);
  ...
  private
    ...
    FCurrentLayout: TMemoryStream;
    FIndex: Integer;
    FLockCount: Integer;
    FUndoList: TObjectList;
    procedure AddUndo(ALayout: TMemoryStream);
    function GetRedoCount: Integer;
    function GetUndoCount: Integer;
  protected
    procedure CancelLastUndo;
    function IsLocked: Boolean;
    procedure RestoreLayout(AStream: TStream);
...
//    property UserWidth: Integer read FUserWidth write FUserWidth;
  ...
  protected
    ...
    function FindNextFocusControl(ACurrentControl: TWinControl; AGoForward: Boolean; out AIndex: Integer): TWinControl;
  ...
  protected
    ...
    procedure SetItemViewInfo(AItem: TdxCustomLayoutItem; AViewInfo: TdxCustomLayoutItemViewInfo); virtual;
  ...
  {$ENDIF}
    ...
    function CreateCustomizationControlHelper(AItem: TdxLayoutItem): TdxControlsDesignSelectorHelper; virtual;
    ...
    function IsViewInfoValid: Boolean;
    procedure MakeVisible(AItem: TdxCustomLayoutItem); virtual;
  ...
  public
    ...
    procedure RestoreFromStream(AStream: TStream; const ARestoreName: string = ''; ADeleteUnloaded: Boolean = False);
  ...
  private
    ...
    procedure SetItem(Value: TdxCustomLayoutItem);
  ...
  protected
    ...
    procedure ItemChanged(AType: TdxLayoutItemChangeType); virtual;
    ...
    function GetCloneDataSize: Integer; virtual;
    ...
    property Item: TdxCustomLayoutItem read FItem write SetItem;
  ...
  protected
    ...
    procedure DoCreateViewInfos; override;
    procedure DestroyViewInfos; override;
    procedure ItemChanged(AType: TdxLayoutItemChangeType); override;
    procedure PopulateAutoAlignControlList(AList: TList); override;
    procedure PopulateControlViewInfoList(AControls, AWinControls: TList); override;
    ...
    function GetCloneDataSize: Integer; override;
  ...
  protected
    ...
    procedure AssignBounds(AViewInfo: TdxLayoutContainerViewInfo);
    ...
    begin
      ...
      if ASkipEmptyGroups then
        for I := 0 to AGroup.VisibleCount - 1 do
          if (AGroup.VisibleItems[I].IsGroup) and ((AGroup.VisibleItems[I] as TdxLayoutGroup).VisibleCount = 0) then
...
begin
  ...
  try
    ...
    begin
      ACacheItem := TdxLayoutItemChacheState(AList[I]);
...
function TdxLayoutTabbedOptions.DoCanChange(NewTabIndex: Integer): Boolean;
begin
  Result := inherited DoCanChange(NewTabIndex) and Group.DoTabChanging(NewTabIndex);
end;
...
procedure TdxLayoutTabbedOptions.RefreshImages;
  ...
  begin
    ...
    cxPrepareBitmapForDrawing(AGlyph, AImages, AImageIndex, True, clNone, AImageBitmap, AMaskBitmap, AIsAlphaUsed);
    TcxAlphaBitmapAccess(AImageBitmap).Resize(InternalImages.Width, InternalImages.Height, True, False);
    Result := InternalImages.Add(AImageBitmap, nil);
  end;
...
procedure TdxLayoutItem.SetControl(Value: TControl);
  ...
  begin
    if Value <> nil then
    begin
      if Value = Container.GetItemsParent then
  ...
  begin
    ...
    if IsDesigning then
    begin
      FCustomizeControlHelper := Container.CreateCustomizationControlHelper(Self);
...
procedure TdxLayoutItem.ContainerChanged;
  ...
  begin
    if Control.Parent <> Container.GetItemsParent then
    begin
...
begin
  if HasControl and not (csDestroying in Control.ComponentState) and
      (FCustomizeControlHelper <> nil) and (not HasWinControl or Control.Parent.HandleAllocated) then
...
begin
  ...
  if not FControlLocked then
    case Message.Msg of
      ...
      CM_TABSTOPCHANGED:
        if not Container.IsUpdateLocked and Container.ViewInfo.FIsValid then
    ...
    begin
      ...
      try
        ...
        begin
          if IsAvailable then
          begin
            AHieght := FOriginalControlSize.Y;
            AWidth := FOriginalControlSize.X;
          end
          else
          begin
            AHieght := Control.Height;
            AWidth := Control.Width;
          end;
          Control.SetBounds(10000, 10000, AWidth, AHieght);
        end
        else
          Control.BoundsRect := ViewInfo.FControlViewInfo.ControlBounds;
        FControlSizeChanged := True;
      finally
        Container.EndPlacingControls;
      end;
    end;
...
function TdxLayoutBasicItemHitTest.GetGroupForInsert: TdxLayoutGroup;
begin
  Result := Item.Parent;
end;
...
begin
  Result := inherited Item as TdxLayoutBasicItem;
end;
...
begin
  Result := ViewInfo.Specific as TdxLayoutGroupViewInfoTabbedSpecific;
end;
...
begin
  Parent := Container.FindItem(FLoadedParentName) as TdxLayoutGroup;
end;
...
begin
  Result := IsDesigning or (Parent = nil) or not IsParentLocked or Container.IsShowLockedGroupChildren;
end;
...
procedure TdxCustomLayoutItem.SetContainer(Value: TdxLayoutContainer);
  ...
  procedure CheckName;
  begin
    if Container.CanSetItemName(Self) then
    begin
      if not IsRoot then
      begin
        if Self.Name = '' then
...
procedure TdxCustomLayoutItem.SetViewInfo(Value: TdxCustomLayoutItemViewInfo);
begin
  Container.SetItemViewInfo(Self, Value);
end;
...
var
  AIntf: IdxLayoutContainer;
begin
  inherited;
  if Value is TdxLayoutGroup then
    Parent := TdxLayoutGroup(Value)
  else
    if Supports(Value, IdxLayoutContainer, AIntf) then
      AIntf.GetContainer.AddAvailableItem(Self);
end;
...
begin
  Result := Container.GetItemsParentComponent.Name;
end;
...
function TdxCustomLayoutItem.IsNeedDeleteAfterLoading: Boolean;
begin
  Result := Superfluous or
    ((LoadedIndex = dxLayoutInitialLoadedItemIndex) and CanDelete);
end;
...
begin
  Result := ((VisibleIndex < 1) or
    ((Parent.LayoutDirection in [ldVertical, ldTabbed]) or
     Parent.VisibleItems[VisibleIndex-1].IsHorzPositionStable and (Parent.VisibleItems[VisibleIndex-1].GetRealAlignHorz in [ahLeft, ahRight])));
end;

function TdxCustomLayoutItem.IsHorzPositionStable: Boolean;
begin
  Result := ((Parent = nil) or Parent.IsHorzPositionStable or (Parent.ViewInfo.GetRealItemControlAreaAlignment = catOwn)) and
    IsHorzLocalPositionStable;
end;
...
begin
  Result := ((VisibleIndex < 1) or
    ((Parent.LayoutDirection in [ldHorizontal, ldTabbed]) or
     Parent.VisibleItems[VisibleIndex-1].IsVertPositionStable and (Parent.VisibleItems[VisibleIndex-1].GetRealAlignVert in [avTop, avBottom])));
end;

function TdxCustomLayoutItem.IsVertPositionStable: Boolean;
begin
  Result := ((Parent = nil) or Parent.IsVertPositionStable) and IsVertLocalPositionStable;
end;
...
begin
  Result := not (IsLoading or Container.IsRestoring);
end;
...
begin
  ...
  Container.GetItemStoredCustomProperties(Self, AProperties);
end;
...
begin
  ...
  if AName = 'IsUserDefined' then
    AValue := IsUserDefined
  else
    if AName = 'ParentName' then
      AValue := GetParentName
    else
      if AName = 'Index' then
        AValue := Index
      else
        if AName = 'AlignHorz' then
          AValue := Variant(AlignHorz)
        else
          if AName = 'AlignVert' then
            ...
            if AName = 'Width' then
              ...
              if AName = 'Height' then
                ...
                Container.GetItemStoredCustomPropertyValue(Self, AName, AValue);
end;
          ...
          if AName = 'AlignVert' then
            ...
            if AName = 'Width' then
              ...
              if AName = 'Height' then
                ...
                Container.SetItemStoredCustomPropertyValue(Self, AName, AValue);
end;

function TdxCustomLayoutItem.GetParentComponent: TComponent;
begin
  if FParent = nil then
    Result := Container.GetItemsParentComponent
  else
    Result := FParent;
end;
...
begin
  if ViewInfo <> nil then
    ViewInfo.ItemChanged(AType);
  if IsDestroying or (Container = nil) or Container.IsLoading or Container.IsDestroying or Container.IsUpdateLocked or not Container.CanUpdate then
    Exit;
  DoChanged(AType);
end;
...
procedure TdxCustomLayoutItem.DoChanged(AType: TdxLayoutItemChangeType);
  ...
  begin
    APrevBounds := nil;
    if (AItem.Parent <> nil) and AItem.IsViewInfoValid then
    begin
...
procedure TdxCustomLayoutItemStorableCaptionOptions.GetStoredProperties(
...
procedure TdxCustomLayoutItemStorableCaptionOptions.SetStoredPropertyValue(
...
constructor TdxLayoutLabeledItemCustomCaptionOptions.Create(
  AItem: TdxCustomLayoutItem);
begin
...
procedure TdxLayoutLabeledItemCustomCaptionOptions.Assign(Source: TPersistent);
begin
  inherited Assign(Source);
  if Source is TdxCustomLayoutItemStorableCaptionOptions then
  begin
...
function TdxLayoutCustomLabeledItem.GetCaptionOptions: TdxLayoutLabeledItemCustomCaptionOptions;
begin
  Result := inherited CaptionOptions as TdxLayoutLabeledItemCustomCaptionOptions;
end;

procedure TdxLayoutCustomLabeledItem.SetCaptionOptions(Value: TdxLayoutLabeledItemCustomCaptionOptions);
begin
  inherited CaptionOptions := Value;
end;
...
procedure TdxLayoutLabeledItem.SetCaptionOptions(
  Value: TdxLayoutLabeledItemCaptionOptions);
begin
  inherited CaptionOptions := Value;
end;
...
begin
  Result := Container.GetItemsParent <> nil;
end;
...
constructor TdxLayoutContainerFocusController.Create(AContainer: TdxLayoutContainer);
begin
  inherited Create(AContainer);
  FCurrentTabOrderIndex := -1;
end;

function TdxLayoutContainerFocusController.SetNextFocusControl(AFocusedControl: TWinControl): Boolean;

  function GetLastIndex(AControl: TControl; AList: TList): Integer;
  var
    I: Integer;
  begin
    Result := AList.IndexOf(AControl);
    if Result <> -1 then
    begin
      for I := Result to AList.Count - 1 do
        if AList[I] = AControl then
          Result := I;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        AFirstIndex := AList.IndexOf(Container.ItemsParent);
        ALastIndex := GetLastIndex(Container.ItemsParent, AList);
        AControlIndex := AList.IndexOf(AFocusedControl);
        if AGoForward and ((AFirstIndex > AControlIndex) or
            (ALastIndex < AControlIndex)) then
  ...
  begin
    ...
    begin
      ...
      if Result then
        ...
        begin
          if AGoForward then
            ACurrentTabOrderIndex :=CurrentTabOrderIndex + 1
          else
            ACurrentTabOrderIndex := CurrentTabOrderIndex - 1;
          if ACurrentTabOrderIndex < 0 then
            ACurrentTabOrderIndex := GetMaxTabOrderIndex;
          if ACurrentTabOrderIndex > GetMaxTabOrderIndex then
            ACurrentTabOrderIndex := 0;
          CurrentTabOrderIndex := ACurrentTabOrderIndex;
          Result := CurrentTabOrderIndex <> -1;
        end
        else
...
function TdxLayoutContainerFocusController.LayoutIndexOf(AList: TList; ACurrentIndex: Integer): Integer;
var
  I: Integer;
  AStartIndex: Integer;
begin
...
function TdxLayoutContainerFocusController.GetFocusedItem: TdxCustomLayoutItem;
var
  I: Integer;
begin
  Result := nil;
...
function TdxLayoutContainerFocusController.GetMaxTabOrderIndex: Integer;
begin
  Result := Container.GetMaxTabOrderIndex;
end;

function TdxLayoutContainerFocusController.FindNextFocusControl(ACurrentControl: TWinControl;
  ...
  begin
    Result := -1;
    if ACurrentControl <> Container.ItemsParent then
...
begin
  Result := Container.ItemsParent.Focused and not Container.IsCustomization and AItem.CanFocus and
    AItem.IsViewInfoValid and (AItem.ViewInfo.TabOrderIndex = CurrentTabOrderIndex);
end;
...
procedure TdxLayoutContainerFocusController.SetCurrentTabOrderIndex(Value: Integer);
begin
...
begin
  if IsTextVisible then
  begin
    if (Item.CachedTextHeight = 0) or ItemViewInfo.ContainerViewInfo.CanUseCachedInfo then
    begin
      PrepareCanvas(Canvas);
      if IsMultiLine then
      begin
...
function TdxCustomLayoutItemViewInfoCloneData.GetSize: Integer;
begin
  Result := 0;
end;
...
function TdxCustomLayoutItemViewInfo.GetCloneDataSize: Integer;
begin
  if CloneData = nil then
    Result := 0
  else
    Result := CloneData.Size;
end;
...
begin
  Result := (Item <> nil) and ((Item.Parent = nil) and (ParentViewInfo = nil) or (Item.Parent.ViewInfo = ParentViewInfo) and ParentViewInfo.IsValid);
end;
...
begin
  ...
  for I := 0 to ElementCount - 1 do
    Elements[I].AssignBounds(TdxCustomLayoutItemViewInfo(ASource).Elements[I]);
end;
...
procedure TdxCustomLayoutItemViewInfo.SetItem(Value: TdxCustomLayoutItem);
begin
  if Item <> Value then
  begin
    if Item <> nil then
      Item.SetViewInfo(nil);
    FItem := Value;
    if Item <> nil then
      Item.SetViewInfo(Self);
  end;
end;
...
begin
  if IsDragImagePainting then
    Result := dxLayoutDragImageBackgroundColor
  else
    Result := Color;
end;
...
begin
  Result := (IsDragImagePainting and IsDragged) or (not IsTransparent and
...
begin
  ...
  begin
    ...
    case AlignHorz of
      taLeftJustify:
        Dec(Result.Right, ADelta);
      taRightJustify:
        Inc(Result.Left, ADelta);
      taCenter:
        begin
          Inc(Result.Left, ADelta div 2);
          Dec(Result.Right, ADelta - ADelta div 2);
        end;
    end;
  end;
end;
...
begin
  if Item.IsDragged and HasCaption then
    ...
    Result := inherited DoCalculateHeight(AIsMinHeight) + 2; 
end;
...
begin
  if Item.IsDragged and HasCaption then
    ...
    Result := inherited DoCalculateWidth(AIsMinWidth) + 2; 
end;
...
begin
  inherited;
  FCaptionViewInfoBounds := TdxLayoutLabeledItemViewInfo(ASource).FCaptionViewInfoBounds;
end;
...
begin
  inherited;
  FControlViewInfoBounds := TdxLayoutControlItemViewInfo(ASource).FControlViewInfoBounds;
end;
...
procedure TdxLayoutGroupViewInfoSpecific.DoSetOffset(const Value: TPoint);
begin
  FOffset := Value;
end;
  ...
  begin
    ...
    begin
      ...
      if AIsMinWidth then
        ...
        if GetItemAlignHorz(AItemViewInfo) = ahClient then
          Inc(Result, Max(GetItemMinWidth(AItemViewInfo), GetItemUsualWidth(AItemViewInfo)))
        else
...
begin
  ...
  if FTabViewInfo.CanFocusOnClick(P.X, P.Y) then
    ItemViewInfo.ContainerViewInfo.Container.FocusController.CurrentTabOrderIndex := ItemViewInfo.FTabOrderIndex;
...
begin
  ...
  if Result and not GroupViewInfo.Group.DoProcessAccel then
    Container.FocusController.SetItemFocus(GroupViewInfo.GetGroup);
end;
...
begin
  case AAreaPart of
    ...
    apLastChild:
      Result := FTabViewInfo.TabsViewInfo[FTabViewInfo.TabsViewInfo.ViewInfoCount - 1].VisibleRect;
  else
    Result := inherited GetAreaPartBounds(AAreaPart, AItemviewInfo);
  end;
  case GetBorder of
...
begin
  inherited;
  FTabControlBounds := TdxLayoutGroupViewInfoTabbedSpecific(ASource).FTabControlBounds;
  CalculateTabControl;
end;

procedure TdxLayoutGroupViewInfoTabbedSpecific.DoSetOffset(const Value: TPoint);
var
  AOffset: TPoint;
begin
  AOffset := cxPointOffset(Value, Offset, False);
  inherited;
  FTabControlBounds := cxRectOffset(FTabControlBounds, AOffset);
  CalculateTabControl;
end;
...
begin
  ...
  if HasTabControl then
  begin
    Inc(Result, GroupViewInfo.LayoutLookAndFeel.GetGroupBorderOffset(Container, ASide, GroupViewInfo.CaptionSide));
    case ASide of
...
function TdxLayoutGroupViewInfoCloneData.GetSize: Integer;
begin
  Result := SizeOf(FIsExpanded) + SizeOf(FItemIndex);
end;
...
begin
  ...
  if (Result = catAuto) and ((ParentViewInfo = nil) or (ParentViewInfo.GetRealItemControlAreaAlignment = catNone) or not Group.IsHorzLocalPositionStable) then
    Result := catOwn;
end;
...
function TdxLayoutGroupViewInfo.GetCloneDataSize: Integer;
var
  I: Integer;
begin
  if CloneData = nil then
  ...
  begin
    Result := CloneData.Size;
    for I := 0 to ItemViewInfoCount - 1 do
      Inc(Result, ItemViewInfos[I].GetCloneDataSize);
  end;
end;
...
var
  I: Integer;
begin
  inherited SaveCloneData(AStream);
  for I := 0 to ItemViewInfoCount - 1 do
...
begin
  ...
  begin
    if (Item1.CaptionLayout in [clLeft, clRight]) and Item1.Item.IsHorzPositionStable and Item2.Item.IsHorzPositionStable or
       (Item1.CaptionLayout in [clTop, clBottom]) and Item1.Item.IsVertPositionStable and Item2.Item.IsVertPositionStable then
  ...
  procedure AlignControls(AList: TdxList);
    ...
    begin
      ...
      begin
        AItemViewInfo := GetViewInfo(AList, I);
        if IsCaptionLayoutHorizontal then
          AItemViewInfo.CaptionViewInfo.Width := AMaxCaptionSize - AItemViewInfo.ElementOffsetHorz
        else
...
begin
  AViewInfo := GetItemViewInfo(AItem);
...
begin
  Result := Container.HasBackground;
end;
...
procedure TdxLayoutContainerViewInfo.AssignBounds(AViewInfo: TdxLayoutContainerViewInfo);
begin
  if not IsClone or (Container <> AViewInfo.Container) then
    Exit;
  FOffset := AViewInfo.FOffset;
  FContentBounds := AViewInfo.FContentBounds;
  ItemsViewInfo.AssignBounds(AViewInfo.ItemsViewInfo);
end;
...
function TdxLayoutContainerViewInfo.GetCloneDataSize: Integer;
begin
  Result := ItemsViewInfo.GetCloneDataSize;
end;
...
begin
  ...
  FSelectionLayer.ParentControl := Container.GetItemsParent;
  ...
  begin
    ...
    try
      ...
      if FIndex = FUndoList.Count then
        AStream := FCurrentLayout
      else
        AStream := TMemoryStream(FUndoList[FIndex]);
      RestoreLayout(AStream);
    finally
      Dec(FLockCount);
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      FCurrentLayout := TMemoryStream.Create;
      Container.StoreToStream(FCurrentLayout);
    end;
    Inc(FLockCount);
    try
      Dec(FIndex);
...
procedure TdxUndoRedoManager.RestoreLayout(AStream: TStream);
begin
  AStream.Position := 0;
  Container.RestoreFromStream(AStream, '', True);
end;
...
procedure TdxUndoRedoManager.AddUndo(ALayout: TMemoryStream);
begin
...
begin
  AItemsOwner := Container.GetItemsOwner;
...
begin
  Result := FItem.Container.IsRestoring;
end;
...
begin
  if GetCurrentImage(AGlyph, AImages, AImageIndex) then
    Result := dxGetImageSize(AGlyph, AImages, AImageIndex)
  else
    Result := cxNullSize;
end;
...
procedure TdxCustomLayoutItemCaptionOptions.SetStoredPropertyValue(const AName: string;
...
begin
  Canvas.DrawText(ViewInfo.Text, GetTextRect, ViewInfo.CalculateTextFlags, DrawEnabled, ViewInfo.GetRotationAngle);
end;
...
begin
  FrameRectByColor(Canvas.Handle, ViewInfo.SelectionBorderRect, dxLayoutSelectionBorderColor);
end;
...
begin
  ...
  try
    LayoutLookAndFeel.InitializeSubControlCxLookAndFeel(AcxLookAndFeel);
    APainter := GetButtonPainter(AcxLookAndFeel);
...
begin
  if IsDestroying or IsLoading or Container.IsUpdateLocked or (FLockTabbedChangesCount > 0) then
    Exit;
  case AType of
    pctLight:
      Changed(ictLight);
...
procedure TdxLayoutGroup.TabClickHandler(Sender: TObject; ATabVisibleIndex: Integer; AShift: TShiftState);
begin
  ...
  begin
    ...
    begin
      if Value and (Caption = '') then
        Caption := cxGetResourceString(@sdxLayoutControlNewHiddenGroup);
      Changed;
    end;
  end;
end;
...
begin
  ...
  begin
    ANeedFocusItem := (LayoutDirection = ldTabbed) and
      not (Container.IsCustomization or Container.IsLoading or Container.IsRestoring) and
      (FItemIndex >= 0) and (Value >= 0);
    if ANeedFocusItem then
    begin
      AFocusedItem := Container.FindFocusedItem;
      ANeedFocusItem := (AFocusedItem <> nil) and
        ((AFocusedItem = Items[FItemIndex]) or IsChildItem(AFocusedItem));
    end;
    FItemindex := Value;
    Changed(ictMedium);
    if ANeedFocusItem then
      Items[ItemIndex].FocusFirst(True);
    DoTabChanged;
  end;
end;
...
function TdxLayoutGroup.GetIsRoot: Boolean;
begin
...
function TdxLayoutGroup.GetVisible: Boolean;
begin
  Result := inherited GetVisible and (IsRoot or not Hidden or (VisibleCount > 0));
end;
...
procedure TdxLayoutGroup.SetInitialSettings;
var
  I: Integer;
begin
  inherited;
...
var
  AIntf: IdxLayoutContainer;
begin
  if Supports(Value, IdxLayoutContainer, AIntf) and
    not AIntf.GetContainer.Root.IsLoading and
    not (csAncestor in ComponentState) then
    AIntf.GetContainer.SetRootGroup(Self)
  else
    inherited;
end;

function TdxLayoutGroup.GetStoredProperties(AProperties: TStrings): Boolean;
begin
...
procedure TdxLayoutGroup.GetStoredPropertyValue(const AName: string; var AValue: Variant);
begin
...
procedure TdxLayoutGroup.SetStoredPropertyValue(const AName: string; const AValue: Variant);
begin
  if AName = 'Hidden' then
    Hidden := AValue
  else
...
function TdxLayoutGroup.IsNeedDeleteAfterLoading: Boolean;
begin
  Result := (Count = 0) and inherited IsNeedDeleteAfterLoading;
end;
...
function TdxLayoutGroup.IsGroup: Boolean;
begin
  Result := True;
end;
...
function TdxLayoutGroup.DoCollapsing: Boolean;
begin
  Result := True;
...
function TdxLayoutGroup.DoExpanding: Boolean;
begin
  Result := True;
...
function TdxLayoutGroup.DoTabChanging(ANewTabIndex: Integer): Boolean;
begin
  Result := True;
...
function TdxLayoutGroup.FocusFirst(ACheckTabStop: Boolean): Boolean;
var
  I: Integer;
begin
  Result := False;
...
procedure TdxLayoutGroup.LockTabbedChanges;
begin
  Inc(FLockTabbedChangesCount);
end;

procedure TdxLayoutGroup.UnlockTabbedChanges;
begin
  Dec(FLockTabbedChangesCount);
end;
...
function TdxLayoutGroup.IsSuperfluous: Boolean;
  ...
  begin
    Result := (AItem1.GetAlignHorz = AItem2.GetAlignHorz) and (AItem1.GetAlignVert = AItem2.GetAlignVert);
  end;

  function IsSameRealAlign(AItem1, AItem2: TdxCustomLayoutItem): Boolean;
  begin
    Result := (AItem1.GetRealAlignHorz = AItem2.GetRealAlignHorz) and (AItem1.GetRealAlignVert = AItem2.GetRealAlignVert);
  end;

begin
  Result := not IsRoot and not Locked and Hidden and
    ((Count = 0) or (Count = 1) and IsSameRealAlign(Self, Items[0]) or (Parent <> nil) and (Parent.Count = 1) and IsSameAlign(Self, Parent));
end;
...
begin
  ...
  begin
    BeginUpdate;
    try
      with Source as TdxLayoutContainer do
      begin
        Self.HighlightRoot := HighlightRoot;
        Self.ShowDesignSelectors := ShowDesignSelectors;
...
begin
  Result := GetAlignmentConstraintClass.Create(GetItemsOwner);
...
var
  AControl1, AControl2: TWinControl;
begin
  AControl1 := TWinControl(AItem1);
  AControl2 := TWinControl(AItem2);
  if AControl1.Parent = AControl2.Parent then
    Result := AControl1.TabOrder - AControl2.TabOrder
  else
  begin
    if AControl1.Parent <> nil then
      Result := CompareItemsByTabOrder(AControl1.Parent, AControl2)
    else
      Result := 0;
    if (Result = 0) and (AControl2.Parent <> nil) then
      Result := CompareItemsByTabOrder(AControl1, AControl2.Parent);
  end;
end;

procedure TdxLayoutContainer.GetTabOrderList(List: TList);

  procedure SortedByTabOrder(AList: TList);
  var
    AControls: TList;
    I: Integer;
  begin
    ...
    try
      for I := AList.Count - 1 downto 0 do
        if AList[I] <> ItemsParent then
        begin
          AControls.Add(AList[I]);
          AList.Delete(I);
        end;
      AControls.Sort(CompareItemsByTabOrder);
      for I := AControls.Count - 1 downto 0 do
        AList.Insert(0, AControls[I]);
    finally
      AControls.Free;
    end;
  end;
...
begin
  ...
  try
    ...
    if not IsAutoControlTabOrders then
      SortedByTabOrder(AList);
...
begin
  ...
  begin
    RenameComponents(Self, GetItemsParentComponent.Owner, ANewName, AOldName, 1 + AbsoluteItemCount, @GetItem);
    RenameComponents(Self, GetItemsParentComponent.Owner, ANewName, AOldName,  AlignmentConstraintCount, @GetAlignmentConstraints);
...
procedure TdxLayoutContainer.RestoreFromStream(AStream: TStream;
  const ARestoreName: string = ''; ADeleteUnloaded: Boolean = False);
begin
  FDeleteUnloadedItems := ADeleteUnloaded;
  try
    RestoreFrom('', AStream, TcxStreamReader, ARestoreName);
  finally
    FDeleteUnloadedItems := False;
  end;
end;
...
begin
  AControlBounds := dxMapWindowRect(ItemsParent.Parent.Handle, 0, ItemsParent.BoundsRect);
...
var
  AItem: TdxCustomLayoutItem;
begin
  if not IsCustomization and (FocusController.CurrentTabOrderIndex <> - 1) then
  begin
    AItem := FocusController.GetFocusedItem;
    if AItem <> nil then
...
begin
  if not (IsDestroying or IsLoading or IsUpdateLocked) and CanUpdate then
  begin
    BeginPlaceControls;
    try
...
begin
  AControl.Parent := GetItemsParent;
end;
...
procedure TdxLayoutContainer.UnprepareControl(AControl: TControl);
    ...
    begin
      ...
      repeat
        ...
        if AIndex > -1 then
          Result := AWinControl.Controls[AIndex].Top + cxGetValueCurrentDPI(21) + 1;
      until AIndex = -1
    end;
...
var
  AParent: TWinControl;
begin
  if AControl.Parent = GetItemsParent then
  begin
    AParent := GetItemsParent.Parent;
...
begin
  for I := 0 to AbsoluteItemCount - 1 do
    AbsoluteItems[I].AfterRestoring;
  CheckIndexes;
...
begin
  for I := 0 to AbsoluteItemCount - 1 do
    AbsoluteItems[I].BeforeRestoring;
end;
...
var
  ...
  begin
    ...
    begin
      ...
      AItem.Superfluous := not AItem.IsUserDefined or ((AVersion < 3) and (AItemClassKind = 0));;
    end
    else
...
begin
  Result := GetItemsParentComponent <> nil;
end;
...
begin
  if FStoringName <> '' then
    ...
    Result := GetItemsParentComponent.Name;
end;
...
begin
  ...
  for I := 0 to AbsoluteItemCount - 1 do
    AChildren.AddObject(AbsoluteItems[I].Name, AbsoluteItems[I])
end;

procedure TdxLayoutContainer.GetItemStoredCustomProperties(
  AItem: TdxCustomLayoutItem; AProperties: TStrings);
begin
end;

procedure TdxLayoutContainer.GetItemStoredCustomPropertyValue(
  AItem: TdxCustomLayoutItem; const AName: string; var AValue: Variant);
begin
end;

procedure TdxLayoutContainer.SetItemStoredCustomPropertyValue(
...
begin
  ...
  try
    if not IsStoringNameMode then
      AStorage.NamePrefix := GetItemsOwner.Name;
...
function TdxLayoutContainer.FindFocusedItem: TdxLayoutItem;
var
  AWnd: THandle;
begin
  AWnd := GetFocus;
...
begin
  Result := Root.ProcessDialogChar(ACharCode);
end;
...
begin
  ...
  Result := (AFocusedItem <> nil) and Root.ProcessDialogKey(ACharCode, AKeyData, AFocusedItem);
end;

function TdxLayoutContainer.CreateCustomizationControlHelper(AItem: TdxLayoutItem): TdxControlsDesignSelectorHelper;
begin
  Result := nil;
end;
...
function TdxLayoutContainer.HasBackground: Boolean;
begin
  Result := True;
end;
...
begin
  ...
  Result := AItemClass.Create(GetItemsOwner);
...
procedure TdxLayoutContainer.SetItemViewInfo(AItem: TdxCustomLayoutItem; AViewInfo: TdxCustomLayoutItemViewInfo);
begin
  AItem.FViewInfo := AViewInfo;
end;
...
procedure TdxLayoutContainer.DeleteSuperfluousItems;
var
  I: Integer;
begin
  for I := AbsoluteItemCount - 1 downto 0 do
    with AbsoluteItems[I] do
      if IsNeedDeleteAfterLoading then
         Free;
end;

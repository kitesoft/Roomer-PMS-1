...
uses
  ...
  ExtCtrls, dbctrls, db, dbconsts, math, ImgList,
...
uses
  ...
  sMessages, sSkinProps, sDialogs, sGraphUtils, sCommonData, acntUtils, sAlphaGraph;
...
var
  BtnTypeNames: array [TNavigateBtn] of PChar = ('DBN_FIRST', 'DBN_PRIOR', 'DBN_NEXT',
    'DBN_LAST', 'DBN_INSERT', 'DBN_DELETE', 'DBN_EDIT', 'DBN_POST', 'DBN_CANCEL', 'DBN_REFRESH'
    {$IFDEF DELPHI_XE2}, 'DBN_APPLYUPDATES', 'DBN_CANCELUPDATES'{$ENDIF});
...
var
  ResBitmap: TBitmap;
begin
  if TsDBNavigator(Btn.Parent).Images = nil then begin
...
{$ENDIF}
        ResBitmap.LoadFromResourceName(hInstance, 's' + BtnTypeNames[aBtn]);
...
begin
  ...
  for I := Low(Buttons) to High(Buttons) do begin
    ...
    Btn.Images := Images;
    Btn.ImageIndex := FFirstImageIndex + ord(I);
    Btn.Parent := Self;
    LoadGlyph(Btn, I);
    Buttons[I] := Btn;
    X := X + MinBtnSize.X;
  ...
  if FUseColoredGlyphs <> Value then begin
    ...
    if not (csLoading in ComponentState) then begin
      for I := Low(Buttons) to High(Buttons) do
        if Buttons[I] <> nil then begin
          LoadGlyph(Buttons[I], I);
          Buttons[I].SkinData.BGChanged := True;
        end;
...
begin
{$IFDEF DELPHI_XE3}
  if StyleServices.Enabled and not StyleServices.IsSystemStyle then begin
    if not Enabled then
    ...
    Details := StyleServices.GetElementDetails(LButton);
    if SkinData.Skinned then
      StyleServices.DrawElement(SkinData.FCacheBmp.Canvas.Handle, Details, R)
    else
      StyleServices.DrawElement(Canvas.Handle, Details, R);
  end
  else
{$ENDIF}
    if SkinData.Skinned and (Images = nil) {$IFDEF DELPHI_XE2}and not TsDBNavigator(Parent).FUseColoredGlyphs{$ENDIF} then begin
      i := GetFontIndex(Self, SkinData.SkinIndex, SkinData.SkinManager, CurrentState);
      C := SwapRedBlue(SkinData.SkinManager.gd[i].Props[integer(CurrentState > 0)].FontColor.Color);

...
{$IFDEF DELPHI_XE3}[ComponentPlatformsAttribute(pidWin32 or pidWin64)]{$ENDIF}
  ...
  published
    ...
    property OnScrollData: TNotifyEvent read FOnScrollData write FOnScrollData;
...
begin
  ...
  if HighlightCell(Col, Row, Value, State) then
    if SkinData.Skinned then begin
      ...
      Canvas.Font.Color := SkinData.SkinManager.gd[SelNdx].Props[0].FontColor.Color;
    end
    else
{$IFDEF DELPHI_XE2}
      if acThemesEnabled then begin
        ...
        FillRect(Canvas.Handle, Rect, Color);
          ...
          if acThemesEnabled then begin
            ...
            if lvCheckTextWidth and (CaptionWidth > TextRect.Right - TextRect.Left) then begin
              while (CaptionWidth > TextRect.Right - TextRect.Left) and (Length(CurCaption) > 1) do begin
                ...
                CaptionWidth := Canvas.TextWidth(CurCaption) + Canvas.TextWidth(ThreeDot);
              end;
              CurCaption := CurCaption + ThreeDot;
            end;
{$ENDIF}
        end;
        CaptionWidth := Canvas.TextWidth(CurCaption);
...
{$ENDIF}
          if CurCaption = '' then begin
            if not bSkinned then
              WriteText(Canvas, TextRect, FrameOffs, FrameOffs, '', lvCaptionAligment, IsRightToLeft) // mean there is coninue of previous column
          end
          else begin
            if dgColLines in Options then begin
              dec(TextRect.Left);
            ...
            if lvCheckTextWidth and (CaptionWidth > TextRect.Right - TextRect.Left) then begin
              while (CaptionWidth > TextRect.Right - TextRect.Left) and (Length(CurCaption) > 1) do begin
                SetLength(CurCaption, Length(CurCaption) - 1);
                CaptionWidth := Canvas.TextWidth(CurCaption) + Canvas.TextWidth(ThreeDot) - lvCaptionXOffset;
              end;
              CurCaption := CurCaption + ThreeDot;
            end;
...
begin
  if not (dgTitles in Options) then
    ...
    FHeaderHeight := 1;
    for i := 0 to Columns.Count - 1 do begin
      if TacColumnTitle(Columns[i].Title).IsCaptionStored then

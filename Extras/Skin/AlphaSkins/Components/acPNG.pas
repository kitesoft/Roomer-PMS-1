      ...
      if TIHDRChunk(Description).Interlaced = 1 then
        for Pass := 0 to 6 do begin
          ...
          while Row < Height do begin
            ...
            ColorManager.ConvertRow([Pointer(RowBuffer[EvenRow] + 1)], Pointer(LongInt(Row0) + Delta * Row), Width, PassMask[Pass]);
            EvenRow := not EvenRow;
        ...
        for Row := 0 to Height - 1 do begin
          ...
          ColorManager.ConvertRow([Pointer(RowBuffer[EvenRow] + 1)], Pointer(LongInt(Row0) + Delta * Row), Width, $FF);
          EvenRow := not EvenRow;
          OffsetRect(FProgressRect, 0, 1);
        end;

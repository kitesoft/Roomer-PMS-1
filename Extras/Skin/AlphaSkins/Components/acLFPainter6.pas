...
uses sGraphUtils, sSkinProps, acntUtils, sConst, sAlphaGraph, math, sVCLUtils, cxControls, ComCtrls, StdCtrls,
...
begin // +
  if Skinned then begin
    if DefaultManager.ConstData.SmallCheckBox[cbChecked] >= 0
      then Ndx := DefaultManager.ConstData.SmallCheckBox[cbChecked]
      else Ndx := DefaultManager.ConstData.CheckBox[cbChecked];
    if Ndx > -1 then
...
begin
  if Skinned then begin
    ...
    case ACheckState of
      cbsUnchecked:
        if DefaultManager.ConstData.SmallCheckBox[cbUnChecked] >= 0 then
          i := DefaultManager.ConstData.SmallCheckBox[cbUnChecked]
        else
          i := DefaultManager.ConstData.CheckBox[cbUnChecked];

      cbsChecked:
        if DefaultManager.ConstData.SmallCheckBox[cbChecked] >= 0 then
          i := DefaultManager.ConstData.SmallCheckBox[cbChecked]
        else
          i := DefaultManager.ConstData.CheckBox[cbChecked];

      cbsGrayed:
        if DefaultManager.ConstData.SmallCheckBox[cbGrayed] >= 0 then
          i := DefaultManager.ConstData.SmallCheckBox[cbGrayed]
        else
          i := DefaultManager.ConstData.CheckBox[cbGrayed];
    end;
...
begin
  if Skinned then begin
    case AButtonKind of
      cxbkComboBtn: begin
        i := DefaultManager.ConstData.ComboBtn.SkinIndex;
        sSect := s_ComboBtn;
      end
      else begin
    ...
    case AButtonKind of
      ...
      cxbkSpinUpBtn: glIndex := DefaultManager.ConstData.ScrollBtns[asTop].GlyphIndex;
      cxbkSpinDownBtn: glIndex := DefaultManager.ConstData.ScrollBtns[asBottom].GlyphIndex;
      cxbkSpinLeftBtn: glIndex := DefaultManager.ConstData.ScrollBtns[asLeft].GlyphIndex;
      cxbkSpinRightBtn: glIndex := DefaultManager.ConstData.ScrollBtns[asRight].GlyphIndex;
    ...
    if glIndex > -1 then begin
      ...
      case AButtonKind of
        cxbkCloseBtn, {cxbkEditorBtn, }cxbkEllipsisBtn: begin
          TmpBmp.Canvas.Font.Color := DefaultManager.gd[i].Props[integer(State <> 0)].FontColor.Color;
...
begin // +
  if Skinned then begin
    i := DefaultManager.GetMaskIndex(DefaultManager.ConstData.IndexGLobalInfo, s_RadioButtonChecked);
  ...
  begin
    with DefaultManager do
      if IsValidImgIndex(ConstData.ScrollBtns[Side].GlyphIndex) then
        ...
        gR := Rect(0, 0, TmpBmp.Width, TmpBmp.Height);
  ...
  begin
    with DefaultManager do
      if DefaultManager.IsValidImgIndex(ConstData.ScrollBtns[Side].GlyphIndex) then begin
        ...
        C := GetGlobalFontColor;// gd[ConstData.ScrollBtns[Side].SkinIndex].Props[min(State, ac_MaxPropsIndex)].FontColor.Color;
        gR := Rect(0, 0, TmpBmp.Width, TmpBmp.Height);
        if (State > 1) and ButtonsOptions.ShiftContentOnClick then
          OffsetRect(gR, 1, 1);

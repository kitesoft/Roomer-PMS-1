...
uses
  ...
  StdCtrls, ComCtrls, ToolWin, ExtCtrls, ImgList,
  {$IFNDEF DELPHI5} types, {$ENDIF}
  {$IFDEF LOGGED} sDebugMsgs, {$ENDIF}
  sConst, acntUtils, sGraphUtils, sCommonData, sDefaults, sComboBox, acSBUtils, sDialogs;
  ...
  if not ControlIsReady(Self) then
    ...
    if Assigned(FCommonData) then begin
      ...
      if FCommonData.Skinned then
        case Message.Msg of
          WM_NCCALCSIZE, WM_WINDOWPOSCHANGED, CM_VISIBLECHANGED, WM_SIZE, CM_ENABLEDCHANGED, WM_MOUSEWHEEL, WM_MOVE:
    ...
    if FCommonData.Skinned then
      case Message.Msg of
        WM_WINDOWPOSCHANGING, WM_WINDOWPOSCHANGED, CM_VISIBLECHANGED, WM_SIZE, CM_ENABLEDCHANGED, WM_MOUSEWHEEL, WM_MOVE:
...
begin
  if FShowButton then
    w := GetSystemMetrics(SM_CXVSCROLL) + 1
  else
    w := 0;
...
begin
  if Message.Msg = WM_SYSCOMMAND then begin
    ...
    if (Message.Msg >= WM_KEYFIRST) and (Message.Msg <= WM_KEYLAST) then begin
      Form := GetParentForm(Self);
  ...
  with SkinData.SkinManager.ConstData.ComboBtn do begin
    ...
    if FCommonData.SkinManager.IsValidImgIndex(GlyphIndex) then
      DrawSkinGlyph(FCommonData.FCacheBmp,
                    Point(R.Left + (WidthOf(R) - WidthOfImage(FCommonData.SkinManager.ma[GlyphIndex])) div 2,
                    (Height - HeightOfImage(FCommonData.SkinManager.ma[GlyphIndex])) div 2), Mode, 1, FCommonData.SkinManager.ma[GlyphIndex],
...
begin
  ...
  if not Enabled then
    BmpDisabledKind(FCommonData.FCacheBmp, FDisabledKind, Parent, CI, MkPoint(Self));
...
begin
  with Canvas do begin
    ...
    LRect := Rect;
    if FShowColorName or not (odComboBoxEdit in State) then
      LRect.Right := LRect.Bottom - LRect.Top + LRect.Left
    else
      LRect.Right := Rect.Right - WidthOf(ButtonRect);
...
begin
  ...
  if odComboBoxEdit in State then begin // if editor window ...
    ...
    with Bmp do begin
      ...
      if ShowcolorName then begin
        ...
        if Bmp.Canvas.Brush.Color = clDefault then
          ...
          if Bmp.Canvas.Brush.Color = clNone then
        ...
        if Bmp.Canvas.Brush.Color = clDefault then begin
          ...
          if Bmp.Canvas.Brush.Color = clNone then begin
            Bmp.Canvas.Brush.Color := NoneColorColor;
          end;
        Bmp.Canvas.FillRect(R);
        Bmp.Canvas.Brush.Color := ColorToBorderColor(ColorToRGB(Brush.Color));
    ...
    if not Enabled then begin
      ...
      BmpDisabledKind(Bmp, DisabledKind, Parent, CI, aRect.TopLeft);
    end;
    ...
    Canvas.TextRect(Rect, Rect.Left, Rect.Top + (Rect.Bottom - Rect.Top - Canvas.TextHeight(Items[Index])) div 2, Items[Index]);
  end;
end;
...
begin
  ...
  if odComboBoxEdit in State then begin // if editor window ...
    ...
    if (SkinData.Ffocused or Focused or (odFocused in state)) and not DroppedDown then
      Canvas.TextRect(rText, rText.Left, rText.Top + (rText.Bottom - rText.Top - Canvas.TextHeight(Items[Index])) div 2, Items[Index])
    else
    ...
    Canvas.FillRect(Rect);
    Canvas.Font.Color := FCommonData.SkinManager.gd[SkinData.SkinIndex].Props[0].FontColor.Color;
...
begin
  if (Images <> nil) and (Item <> nil) then begin
    ...
    if not (odComboBoxEdit in State) and (Item.Indend > 0) then
...
begin
  ...
  if Operation = opRemove{opInsert} then
...
const
  ImageIndex, SelectedImageIndex, OverlayImageIndex, Indent: Integer;

...
type
  ...
  protected
    ...
    procedure DrawPieSlice(Canvas: TCanvas; Center:  TPoint; Radius, centerh, centerv, indent: integer; StartAngle, SweepAngle: Double; PointIndex: integer; ScaleX, ScaleY: Double); virtual;
...
begin
  ...
  begin
    ...
    begin
      ...
      Yx := YAxis;
      m := (Collection as TChartseries).FOwner.Margin;
...
procedure TChartSerie.DrawPieSlice(Canvas: TCanvas; Center:  TPoint; Radius, centerh, centerv, indent: integer; StartAngle, SweepAngle: Double; PointIndex: integer;
ScaleX, ScaleY: Double);
var
  X1,X2,X3,X4: integer;
  Y1,Y2,Y3,Y4: integer;
...
var
  ...
  oldbrushcolor: TColor;
  oldpenstyle: TPenStyle;
  size, xk1, yk1, xk2, yk2: integer;
  x0, y0, x1, y1, x2, y2, a1, a2: Single;
  innersz: Integer;
begin
  if not SelectedMark then
    Exit;
  ...
  case ChartType of
    ...
    begin
      ...
      innersz := Pie.GetPieInnerSize;

      xk1 := Round(ptSlice.X0 + (innersz / 2) * Cos(a1));
      yk1 := Round(ptSlice.Y0 - (innersz / 2) * Sin(a1));

      xk2 := Round(ptSlice.X0 + (innersz / 2) * Cos(a2));
      yk2 := Round(ptSlice.Y0 - (innersz / 2) * Sin(a2));

      ACanvas.Ellipse(Bounds(ptSlice.X1 - size div 2, ptslice.Y1 - size div 2, size, size));
      ACanvas.Ellipse(Bounds(ptSlice.X2 - size div 2, ptslice.Y2 - size div 2, size, size));
      ACanvas.Ellipse(Bounds(xk1 - size div 2, yk1 - size div 2, size, size));
      ACanvas.Ellipse(Bounds(xk2 - size div 2, yk2 - size div 2, size, size));
    end;
  end;
...
var
  ...
  center: TPoint;
  pbr: TRect;
  pszinner, radius, rad, psz, centerh, centerv: integer;
  totalpie: double;
  startangle, sweepangle: Double;
  ...
  frwtot, frhtot, frwct, frhct: Double;
  fval, fvaln, fvaltot: Double;
  cntf: Integer;
  fnr: TRect;
  fncnt: Integer;
  fc: Double;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if (ChartType <> ctSpider) and (ChartType <> ctHalfSpider) then
                DrawPieSlice(Canvas, center, radius , centerh, centerv, GetPoint(rad).PixelValue1, StartAngle, SweepAngle, rad, Scalex, ScaleY)
              else
              begin
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              fvaln := 0;
              if rad > 0 then
                fvaln := GetPoint(rad - 1).SingleValue;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (dpi + Chart.Range.RangeFrom >= Chart.Range.RangeFrom) and (dpi + Chart.Range.RangeFrom <= Chart.Range.rangeto) and
            (dpi >= 0) and (dpi < Length(FDrawValuesDP)) then
          begin
...
begin
  ...
  begin
    if Chart.XScale > 0 then
      Result := ((value - r.Top) / Chart.XScale) + Chart.Range.RangeFrom
    else
      Result := ((value - r.Top) / 1) +  + Chart.Range.RangeFrom;
  end
  else
  begin
    if Chart.XScale > 0 then
      Result := ((Value - R.Left) / Chart.XScale) + Chart.Range.RangeFrom
    else
      Result := ((Value - r.Left) / 1) +  Chart.Range.RangeFrom;
  end;
end;

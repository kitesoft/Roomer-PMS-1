...
type
  ...
  {$IFDEF FMXLIB}
  TAdvChartBitmap = TBitmap;
  TAdvChartDrawBitmap = TBitmap;
  {$ENDIF}
  {$IFDEF CMNLIB}
  TAdvChartBitmap = TPicture;
  TAdvChartDrawBitmap = TGraphic;
  {$ENDIF}

  TAdvChartBitmapHelper = class helper for TAdvChartBitmap
    {$IFDEF VCLLIB}
    ...
    class function CreateFromResource(AResourceName: String): TAdvChartBitmap;
  end;
...
{$ENDIF}
function BitmapToDrawBitmap(ABitmap: TAdvChartBitmap): TAdvChartDrawBitmap;
function IsBitmapEmpty(ABitmap: TAdvChartBitmap): Boolean;
...
class function TAdvChartBitmapHelper.CreateFromResource(
  AResourceName: String): TAdvChartBitmap;
begin
...
{$IFDEF VCLLIB}
procedure TAdvChartBitmapHelper.SaveToStream(AStream: TCustomMemoryStream);
begin
...
procedure TAdvChartBitmapHelper.LoadFromStream(AStream: TCustomMemoryStream);
var
  pic: TGraphic;
  gcc: TGraphicClass;
begin
  ...
  begin
    pic := nil;
    if TAdvChartUtils.FindGraphicClass(AStream.Memory^, AStream.Size, gcc) then
      pic := gcc.Create;

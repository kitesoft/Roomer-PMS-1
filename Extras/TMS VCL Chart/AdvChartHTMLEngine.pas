...
uses
  Classes, AdvChartGraphics, Types, Graphics, AdvChartTypes, AdvChartBitmapContainer
  {$IFDEF CMNLIB}
  , ImgList
  {$ENDIF}
  {$IFNDEF LCLLIB}
...
function HTMLDrawEx(AGraphics: TAdvChartGraphics; s:string; fr:TRectF;
                    ...
                    {%H-}ResFactor:Double;
                    URLColor,HoverColor,HoverFontColor,ShadowColor: TAdvChartGraphicsColor;
                    ...
                    BitmapContainer: TAdvChartBitmapContainer; {%H-}Opacity: Single; HyperLinkUnderline: Boolean = True{$IFDEF CMNLIB}; ImageList: TCustomImageList = nil{$ENDIF};
                    HighlightColor: TAdvChartGraphicsColor = gcBlue;
                    HighlightTextColor: TAdvChartGraphicsColor = gcWhite;
...
begin
  ...
  begin
    ...
    Prop := Copy(Prop,Pos(AdvHTMLENGINE_ATTR_DELIM,Prop) + 1,Length(Prop));
    Prop := Copy(Prop,1,Pos(AdvHTMLENGINE_ATTR_DELIM,Prop) - 1);
    ControlValue := TAdvChartUtils.UnFixMarkup(Prop);
  end;
  ...
  begin
    ...
    Prop := Copy(Prop,Pos(AdvHTMLENGINE_ATTR_DELIM,Prop) + 1,Length(Prop));
    Prop := Copy(Prop,1,Pos(AdvHTMLENGINE_ATTR_DELIM,Prop) - 1);
    ControlProp := TAdvChartUtils.UnFixMarkup(Prop);
  end;
...
begin
  Result := False;
  Temp := '';
  ControlValue := TAdvChartUtils.FixMarkup(ControlValue);
  lp := 0;
  while VarIPos('<CONTROL ',html,lp) > 0 do
  begin
    ...
    begin
      ...
      if (CP <> '') then
        Temp := Temp + ' PROP='+AdvHTMLENGINE_ATTR_DELIM+TAdvChartUtils.FixMarkup(CP)+AdvHTMLENGINE_ATTR_DELIM;
      if (CL <> '') then
...
var
  ...
  Align: TAlignment;
  PIndent: Integer;
  OldFont: TAdvChartGraphicsFont;
  OldFontColor: TAdvChartGraphicsColor;
  CalcFont: TAdvChartGraphicsFont;
  CalcFontColor: TAdvChartGraphicsColor;
  DrawFont: TAdvChartGraphicsFont;
  DrawFontCOlor: TAdvChartGraphicsColor;
  FontColor: TAdvChartGraphicsColor;
  OldPenColor: TAdvChartGraphicsColor;
  OldCalcFont: TAdvChartGraphicsFont;
  OldCalcFontColor: TAdvChartGraphicsColor;
  BkColor,BGColor: TAdvChartGraphicsColor;
  OldDrawFont: TAdvChartGraphicsFont;
  OldDrawFontColor: TAdvChartGraphicsColor;
  Hotspot, ImageHotspot: Boolean;
  Anchor,OldAnchor,MouseInAnchor,Error: Boolean;
  paracolor,pencolor,blnkcolor,hifcol,hibcol: TAdvChartGraphicsColor;
  ...
  ColL,ColB: TAdvChartGraphicsColor;
  ...
  function HTMLDrawLine(AGraphics: TAdvChartGraphics; var s:string;r: TRectF;Calc:Boolean;
  ...
  var
    ...
    bmp: TAdvChartBitmap;
    bmpcreated: boolean;
    NewColor: TAdvChartGraphicsColor;
...
//    r.Left := r.Left + offsetX;
    ...
    begin
      ...
      while Pos('&nbsp;',su) > 0 do
      begin
        TAdvChartUtils.TagReplacestring('&nbsp;',' ',su);
      end;
      ...
      if WordLen > 0 then
      begin
        th := AGraphics.CalculateText(su).Height;
        ...
        begin
          ...
          begin
            ...
            errs := AGraphics.CalculateText(su).Width;

            if BkColor <> gcNull then
            begin
              AGraphics.Fill.Color := BkColor;
              AGraphics.Stroke.Color := BkColor;
              AGraphics.DrawRectangle(RectF(cr.Left, cr.Top, cr.Left + errs, cr.Top + AGraphics.CalculateText('gh').Height));
            end;
          ...
          begin
            ...
            cr.Right := cr.Left + AGraphics.CalculateText(su).Width;
...
//          end;
          ...
          begin
            ...
            begin
              ...
              if Copy(su, WordLen, 1) = ' ' then
                sw := AGraphics.CalculateText(' ').Width
              else
                sw := 0;
            end
            else
      ...
      begin
        ...
        begin
          ...
          'A':begin
                ...
                begin
                  ...
                  rr.Top := rr.Bottom - AGraphics.CalculateText('gh').Height;
        ...
        begin
                        ...
                        begin
                          ...
                          begin
                            ...
                            begin
                              ...
                              begin
                                AGraphics.DrawBitmap(RectF(fr.left+bmpx,fr.top+bmpy, fr.Left + bmpx + bmp.Width,fr.top+bmpy + bmp.Height), BitmapToDrawBitmap(bmp));
                      ...
                      if (Pos('BGTOPLEFT', TagProp) > 0) then
                        begin
                          ...
                          begin
                            if not IsBitmapEmpty(bmp) and (bmp.Width > 0) and (bmp.Height > 0) then
                            begin
                              AGraphics.DrawBitmap(RectF(fr.Left, fr.Top + bmpy,fr.Left + bmp.Width ,fr.Top + bmp.Height), BitmapToDrawBitmap(bmp));
                            end;
                          end;
                      ...
                      if (Pos('BGTOPRIGHT', TagProp) > 0) then
                        begin
                          ...
                          begin
                            if not IsBitmapEmpty(bmp) and (bmp.Width > 0) and (bmp.Height > 0) then
                            begin
                              AGraphics.DrawBitmap(RectF(fr.Right - bmp.Width, fr.Top + bmpy, fr.Right, fr.Top + bmp.Height ), BitmapToDrawBitmap(bmp));
                            end;
                          end;
                        end; //end of bgtopright

                      if TAdvChartUtils.VarPos('BGCOLOR',TagProp,TagPos) > 0 then
                      begin
                        ...
                        begin
                          ...
                          if Prop[1] = '#' then
                          {$ENDIF}
                            NewColor := TAdvChartGraphics.HTMLToColor(Prop)
                          else
                            NewColor := TAdvChartGraphics.TextToColor(AnsiLowerCase(prop));
                        end;

                        if TAdvChartUtils.VarPos('BGCOLORTO',TagProp,TagPos) > 0 then
                        begin
                          Prop := Copy(TagProp,TagPos + 5,Length(TagProp));
                          Prop := Copy(Prop,Pos('"',Prop) + 1,Length(Prop));
                          Prop := Copy(Prop,1,Pos('"',Prop) - 1);
                          ...
                          Prop := 'H';
                          if TAdvChartUtils.VarPos('DIR',TagProp,TagPos) > 0 then
                          begin
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      ...
                      IMGSize.y := IMGSize.x;
                      Indent := IMGSize.x;
                      ir := RectF(cr.Left + 1, cr.Top + (cr.Height - (ImgSize.Y - 2)) / 2, cr.Left + IMGSize.X - 1, cr.Top + (cr.Height - (ImgSize.Y - 2)) / 2 + (IMGSize.Y - 2));
                    ...
                    begin
                      IMGSize.y := 22;
                      ir := RectF(cr.Left + 2,cr.Top + (cr.Height - IMGSize.Y) / 2,cr.Left + Indent -2, cr.Top + (cr.Height - IMGSize.Y) / 2+ IMGSize.Y);
          ...
          'H':begin
                ...
                begin
                  ...
                  begin
                    ...
                    if TAdvChartUtils.VarPos('COLOR',Uppercase(TagProp),TagPos) > 0 then
                    begin
                      Prop := Copy(TagProp,TagPos+5,Length(TagProp));
                      Prop := Copy(Prop,Pos('"',prop)+1,Length(Prop));
                      Prop := Copy(Prop,1,Pos('"',prop)-1);
          ...
          'I':begin
                ...
                begin
                  ...
                  Prop := Copy(TagProp,TAdvChartUtils.ipos('x',TagProp) + 2,Length(TagProp));
                  Prop := Copy(Prop,Pos('"',Prop) + 1,Length(prop));
                  Prop := Copy(Prop,1,Pos('"',Prop) - 1);
                  ...
                  begin
                    ...
                    begin
                      Tagp := Copy(TagProp,TAdvChartUtils.ipos('HEIGHT',TagProp) + 7,Length(TagProp));
                      Tagp := Copy(Tagp,pos('"',Tagp) + 1,Length(Tagp));
                      Tagp := Copy(Tagp,1,pos('"',Tagp) - 1);
                    ...
                    begin
                      Tagp := Copy(TagProp,TAdvChartUtils.ipos('ALIGN',TagProp) + 6,Length(TagProp));
                      Tagp := Copy(Tagp,pos('"',Tagp) + 1,Length(Tagp));
                      Tagp := Copy(Tagp,1,pos('"',Tagp) - 1);
                    ...
                    begin
                      Tagp := Copy(TagProp,TAdvChartUtils.ipos('FLOAT',TagProp) + 6,Length(TagProp));
                      Tagp := Copy(Tagp,pos('"',Tagp) + 1,Length(Tagp));
                      Tagp := Copy(Tagp,1,pos('"',Tagp) - 1);
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            ...
                            begin
                              AGraphics.DrawBitmap(RectF(ir.Left,ir.Top,ir.Left + imgw,ir.Top + imgh), BitmapToDrawBitmap(bmp));
                            end
                            else
                            if (TagWidth > 0) and (TagHeight = 0) then
                            begin
                              AGraphics.DrawBitmap(RectF(ir.Left,ir.Top,ir.Left + TagWidth,ir.Top + TagHeight),BitmapToDrawBitmap(bmp));
                            end
                            else
                            begin
                              AGraphics.DrawBitmap(RectF(ir.Left,ir.Top,ir.Left + bmp.Width, ir.Top + bmp.Height),BitmapToDrawBitmap(bmp));
                            end;
                          end;
                    ...
                    begin
                      ...
                      begin
                        ImgAli := h - AGraphics.CalculateText('gh').Height;
                      end
                      else
                      begin
                        if Pos('"MIDDLE',TagProp) > 0 then
                          ImgAli := (h - AGraphics.CalculateText('gh').Height) / 2;
                      end;
                    end;

                    if (Pos('WRAP',TagProp) > 0) then
                    begin
                      h := AGraphics.CalculateText('gh').Height;
                      ImgAli := 0;
                    end;
          ...
          'L':begin
                ...
                if (TAdvChartUtils.VarPos('>',s,TagPos)>0) then
                begin
                  TagProp := Uppercase(Copy(s,3,TagPos-1));

                  if TAdvChartUtils.VarPos('TYPE',TagProp,TagPos) > 0 then
                  begin
                  ...
                  if TAdvChartUtils.VarPos('COLOR',TagProp,TagPos) > 0 then
                  begin
                    Prop := Copy(TagProp,TagPos+5,Length(TagProp));
                    Prop := Copy(Prop,Pos('"',prop)+1,Length(Prop));
                    Prop := Copy(Prop,1,Pos('"',prop)-1);
                    ...
                    begin
                      ...
                      {$ENDIF}
                        ...
                        NewColor := TAdvChartGraphics.TextToColor(prop);
                    end;
                  end;
                end;
          ...
          'P':begin
                ...
                begin
                  ...
                  if TAdvChartUtils.VarPos('ALIGN',TagProp,TagPos) > 0 then
                  begin
                    Prop := Copy(TagProp,TagPos+5,Length(TagProp));
                    Prop := Copy(Prop,Pos('"',prop)+1,Length(Prop));
                    Prop := Copy(Prop,1,Pos('"',prop)-1);
                  ...
                  if TAdvChartUtils.VarPos('INDENT',TagProp,TagPos) > 0 then
                  begin
                  ...
                  if TAdvChartUtils.VarPos('FLOAT',TagProp,TagPos) > 0 then
                  begin
                    Prop := Copy(TagProp,TagPos+5,Length(TagProp));
                    Prop := Copy(Prop,Pos('"',prop)+1,Length(Prop));
                    Prop := Copy(Prop,1,Pos('"',prop)-1);
        ...
        'F':begin
              ...
              begin
                ...
                if (TAdvChartUtils.VarPos('FACE',TagProp,TagPos) > 0) then
                begin
                  Prop := Copy(TagProp,TagPos+4,Length(TagProp));
                  Prop := Copy(prop,pos('"',prop)+1,Length(prop));
                  Prop := Copy(prop,1,pos('"',prop)-1);
                ...
                if (TAdvChartUtils.VarPos(' COLOR',TagProp,TagPos) > 0) and not Selected then
                begin
                  Prop := Copy(TagProp,TagPos+6,Length(TagProp));
                  Prop := Copy(Prop,Pos('"',prop)+1,Length(prop));
                  Prop := Copy(Prop,1,Pos('"',prop)-1);
                  ...
                  begin
                    ...
                    if Prop[1] = '#' then
                    {$ENDIF}
                      FontColor := TAdvChartGraphics.HTMLToColor(Prop)
                    else
                      FontColor := TAdvChartGraphics.TextToColor(Prop);
                  end;
                end;

                if (TAdvChartUtils.VarPos('BGCOLOR',TagProp,TagPos)>0) and not Calc and not Selected then
                begin
                  ...
                  begin
                    ...
                    if Prop[1] = '#' then
                    {$ENDIF}
                      BkColor := TAdvChartGraphics.HTMLToColor(Prop)
                    else
                      BkColor := TAdvChartGraphics.TextToColor(prop);
                  end;

                end;

                if (TAdvChartUtils.VarPos('SIZE',TagProp,TagPos)>0) then
                begin
                  Prop := Copy(TagProp,TagPos+4,Length(TagProp));
                  Prop := Copy(Prop,Pos('=',Prop)+1,Length(Prop));
                  Prop := Copy(Prop,Pos('"',Prop)+1,Length(Prop));
                  ...
                  1: TAdvChartUtils.SetFontSize(AGraphics.Font, 8);
                  2: TAdvChartUtils.SetFontSize(AGraphics.Font, 10);
                  3: TAdvChartUtils.SetFontSize(AGraphics.Font, 12);
                  4: TAdvChartUtils.SetFontSize(AGraphics.Font, 14);
                  5: TAdvChartUtils.SetFontSize(AGraphics.Font, 16);
                  else
                    TAdvChartUtils.SetFontSize(AGraphics.Font, IStrToInt(Prop));
                  end;
                end;
              end;
            end;
        'S':begin
              ...
              begin
                ...
                begin
                  if TAdvChartUtils.ipos('<SUB>',s)=1 then
                    isSub := True
                  else
                    if TAdvChartUtils.ipos('<SUP>',s)=1 then
                      isSup := True;
                end;
              end;
            end;
        'R':begin
              TagProp := Copy(s,3,pos('>',s)-1);
              prop := Copy(TagProp,TAdvChartUtils.ipos('a',TagProp)+2,Length(TagProp));
              prop := Copy(prop,pos('"',prop)+1,Length(prop));
              prop := Copy(prop,1,pos('"',prop)-1);
      ...
      if (TAdvChartUtils.VarPos('>',s,TagPos)>0) and not ImgBreak then
      begin
...
begin
  Anchor := False;
  Error := False;
  OldFont := TAdvChartGraphicsFont.Create;
  OldFont.Assign(AGraphics.Font);
  OldFontColor := AGraphics.Font.Color;
  DrawFont := TAdvChartGraphicsFont.Create;
  DrawFont.Assign(AGraphics.Font);
  DrawFontColor := AGraphics.Font.Color;
  CalcFont := TAdvChartGraphicsFont.Create;
  CalcFont.Assign(AGraphics.Font);
  CalcFontColor := AGraphics.Font.Color;
  OldDrawfont := TAdvChartGraphicsFont.Create;
  OldDrawFont.Assign(AGraphics.Font);
  OldDrawFontColor := AGraphics.Font.Color;
  OldCalcFont := TAdvChartGraphicsFont.Create;
  OldCalcFont.Assign(AGraphics.Font);
  ...
  begin
    repeat
      ...
      if TAdvChartUtils.TagReplacestring('&amp;','&&',s) then Foundtag := True;
      if TAdvChartUtils.TagReplacestring('&quot;','"',s) then Foundtag := True;

      if TAdvChartUtils.TagReplacestring('&sect;','§',s) then Foundtag := True;
      if TAdvChartUtils.TagReplacestring('&permil;','®‰',s) then Foundtag := True;
      if TAdvChartUtils.TagReplacestring('&reg;','®',s) then Foundtag := True;

      if TAdvChartUtils.TagReplacestring('&copy;','©',s) then Foundtag := True;
      if TAdvChartUtils.TagReplacestring('&para;','¶',s) then Foundtag := True;

      if TAdvChartUtils.TagReplacestring('&trade;','™',s) then Foundtag := True;
      if TAdvChartUtils.TagReplacestring('&euro;','€',s) then Foundtag := True;
  ...
  begin
    ...
    AGraphics.Font.Assign(CalcFont);
    ...
    ImageHotSpot := False;
    HTMLWidth := 0;
    HtmlHeight := AGraphics.CalculateText('gh').Height;
    ...
    begin
      ...
      AGraphics.Font.Assign(DrawFont);
      ...
      if (HotSpot and
         (YPos > dr.Bottom - ImgAli - AGraphics.CalculateText('gh').Height) and
  ...
  if (ysize = 0) then
    ysize := AGraphics.CalculateText('gh').Height;

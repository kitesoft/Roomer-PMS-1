    ...
    begin
      ...
      begin
        ...
        case itbmp.BitmapAlign of
          ...
          baCenter: bmpr := RectF(r.Left + (r.Width - sz) / 2, r.Top, r.Left + (r.Width - sz) / 2 + sz, r.Bottom);
          baRight: bmpr := RectF(r.Right - sz, r.Top, r.Right, r.Bottom);
        end;

        w := 0;
        h := 0;
        GetAspectSize(w, h, bmp.Width, bmp.Height, bmpr.Width, bmpr.Height, bmpr.Width, bmpr.Height, True, False, False);
        bmpdr := RectF(bmpr.Left + (bmpr.Width - w) / 2, bmpr.Top + (bmpr.Height - h) / 2, bmpr.Left + (bmpr.Width - w) / 2 + w, bmpr.Top + (bmpr.Height - h) / 2 + h);
        bmpdr := RectF(Round(bmpdr.Left), Round(bmpdr.Top), Round(bmpdr.Right), Round(bmpdr.Bottom));
//      if it.Enabled then
          AGraphics.DrawBitmap(bmpdr, BitmapToDrawBitmap(bmp));
//        else
...
begin
  ...
  if Assigned(Bitmap) and not IsBitmapEmpty(Bitmap) then
    Result := Bitmap
  else if Assigned(FOwner.BitmapContainer) and (BitmapName <> '') then
    Result := FOwner.BitmapContainer.FindBitmap(BitmapName);
end;

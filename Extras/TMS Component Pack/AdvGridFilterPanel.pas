...
const
  ...
  // v1.0.2.1 : Fixed : UI sync in dialogs
  // v1.0.2.2 : Fixed : Issue with retrieving column names when FixedCols = 0
  // v1.0.3.0 : New : OnAppliedFilter event added
  //          : Improved : Loading filter from grid
  // v1.0.4.0 : New : OnRemovedFilter, OnRestoredFilter events added
...
begin
  ...
  begin
    ...
    if ed is TComboBox then
      (ed as TComboBox).Text := AText
    else
      (ed as TEdit).Text := AText;
    TabStop := true;
    TabOrder := 2;
  end;
...
procedure TAdvGridFilterPanel.InitFilterFromGrid;
begin
  RestoreFilterClick(Self);
end;
...
begin
  ...
  F := TFilter.Create(grid);
  F.Assign(Grid.Filter);
  FC := F.Count;
  LC := -1;
...
begin
  ...
  ((Form as TAdvGridFilterForm).FilterPanel as TAdvGridFilterPanel).UI.Assign(UI);
  ((Form as TAdvGridFilterForm).FilterPanel as TAdvGridFilterPanel).FilterUIChanged(Self);
end;

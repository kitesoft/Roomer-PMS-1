...
uses
  Classes, Controls, DB, Types, Generics.Collections, RTTI, Graphics, Windows,
  MaskUtils, DBXJSON, DBXJSONReflect
  {$IFDEF DELPHIXE6_LVL}
...
type
  ...
  TDataControl = (dcEdit, dcSpinEdit, dcCheckBox, dcComboBox, dcRadioGroup,
   ...
   dcMaskEdit, dcDate, dcTime, dcEditComboBox);

  TDescriptionType = (dtNone, dtHint, dtLabel);
  ...
  published
    ...
    property DescriptionType: TDescriptionType read FDescriptionType write FDescriptionType;
  ...
  private
    ...
    procedure SetDescriptionType(const Value: TDescriptionType);
  ...
  public
    ...
    [JSONMarshalled(False)]
    property DescriptionCtrl: TControl read FDescriptionCtrl write FDescriptionCtrl;
  published
    ...
    property DescriptionType: TDescriptionType read FDescriptionType write SetDescriptionType default dtHint;
    property EditMask: TEditMask read FEditMask write FEditMask;
  ...
  TDescriptionKind = (dkNone, dkHint, dkLabel);

  TDescriptionSettings = class(TPersistent)
  private
    FDescriptionKind: TDescriptionKind;
    FOnChange: TNotifyEvent;
    FFont: TFont;
    procedure SetDescriptionKind(const Value: TDescriptionKind);
    procedure SetFont(const Value: TFont);
  protected
    procedure Changed; virtual;
  public
  ...
  published
    property Font: TFont read FFont write SetFont;
    property Kind: TDescriptionKind read FDescriptionKind write SetDescriptionKind default dkHint;
    property OnChange: TNotifyEvent read FOnChange write FOnChange;
  end;
  ...
  private
    ...
    FDescriptions: TDescriptionSettings;
    ...
    procedure SetDescriptions(const Value: TDescriptionSettings);
  protected
    function GetOwner: TPersistent; override;
    procedure SubPropsChanged(Sender: TObject);
    function CreateItems(Designing: boolean): TLayoutItems; virtual;
  ...
  published
    ...
    property Descriptions: TDescriptionSettings read FDescriptions write SetDescriptions;
  ...
  TLabelCreatedEvent = procedure(Sender: TObject; ALayoutItem: TLayoutItem; AControl: TControl; AField: TField; var ACaption: string) of object;
  ...
  protected
    procedure DoControlCreated(ALayoutItem: TLayoutItem; var AControl: TControl); virtual;
    procedure DoLabelCreated(ALayoutItem: TLayoutItem; AControl: TControl; var ACaption: string); virtual;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    procedure InitScale(AValue: double; ADialogUnits: integer);
    ...
    property LabelClass: TControlClass read FLabelClass write FLabelClass;
...
begin
  ...
  if (LayoutItem.DataControl in [dcComboBox, dcEditComboBox, dcRadioGroup]) then
  begin
    ...
    begin
      ...
      begin
        ...
        {$ENDIF}
        if LayoutItem.Items.Count = 0 then
          sl.Assign(LayoutItem.Values);
      end;
    end;

    CompProp := CompType.GetProperty('Values');
    if Assigned(CompProp) then
    begin
      PropInfo := TypInfo.GetPropInfo(Result.ClassInfo, 'Values');
      if (PropInfo <> nil) and (PropInfo^.PropType^.Kind = tkClass) then
      begin
        {$IFDEF DELPHIXE2_LVL}
        sl := TStrings(GetOrdProp(Result, PropInfo));
...
procedure TAdvDBFormLayouter.DoControlCreated(ALayoutItem: TLayoutItem; var AControl: TControl);
begin
  if Assigned(OnControlCreated) then
    OnControlCreated(Self, ALayoutItem, AControl, ALayoutItem.Field);
end;

procedure TAdvDBFormLayouter.DoLabelCreated(ALayoutItem: TLayoutItem;
  AControl: TControl; var ACaption: string);
begin
  if Assigned(OnLabelCreated) then
    OnLabelCreated(Self, ALayoutItem, AControl, ALayoutItem.Field, ACaption);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        // create label
        lblctrl := Creator.CreateComponent(FLabelClass, Control);

        lblctrl.Width := Layout.Labels.Size;
        lblctrl.Left := x;
        lblctrl.Top := y;
        lblctrl.Parent := Control;

        c := TRttiContext.Create;

        p := c.GetType(lblctrl.ClassInfo).GetProperty('Font');
        if Assigned(p) then
        begin
          (p.GetValue(lblctrl).AsObject as TFont).Assign(Layout.Labels.Font);
        end;

        p := c.GetType(lblctrl.ClassInfo).GetProperty('Caption');
        if Assigned(p) then
        begin
          lblcaption := '&' + Format(Layout.Labels.Format, [Layout.Items[i].LabelCaption]);

          DoLabelCreated(Layout.Items[i], lblctrl, lblcaption);

          p.SetValue(lblctrl, lblcaption);
        end;

        if Layout.Labels.AutoSize or (Layout.Labels.Size = 0) then
        begin
          p := c.GetType(lblctrl.ClassInfo).GetProperty('AutoSize');
          if Assigned(p) then
          begin
            p.SetValue(lblctrl, true);
          end;
        end;
        ...
        Layout.Items[i].LabelCtrl := lblctrl;
      end;
      ...
      lsTop:
        begin
          cy := y + lblctrl.Height + Layout.Spacing.Vert;
          lw := 0;
        end;
      lsBottom:
        begin
      ...
      begin
        ...
        begin
          sz := Max(3, Layout.Items[i].Field.DisplayWidth);
      ...
      begin
        ...
        begin
          ...
          p := c.GetType(lblctrl.ClassInfo).GetProperty('FocusControl');
          if Assigned(p) then
            p.SetValue(lblctrl,(ctrl as TWinControl));

          c.Free;
        end;

        case Layout.Labels.Position of
        lsLeftCenter: lblctrl.Top := lblctrl.Top + (ctrl.Height - lblctrl.Height) div 2;
        // center vertically
        lsLeftBottom: lblctrl.Top := (ctrl.Top + ctrl.Height - lblctrl.Height);
        // position at bottom of control
        lsBottom: lblctrl.Top := ctrl.Top + ctrl.Height + Layout.Spacing.Vert;
        end;
      end;
      ...
      if Layout.Labels.Position in [lsTop, lsBottom] then
        lh := ctrl.Height + lblctrl.Height + 2 * Layout.Spacing.Vert;

      if lh > mh then
        mh := lh;

      if (Layout.Descriptions.Kind = dkLabel) and
         (Layout.Items[i].DescriptionType = dtLabel) and
         (Layout.Items[i].Description <> '') then
      begin
        // create label
        lblctrl := Creator.CreateComponent(FLabelClass, Control);
        ...
        lblctrl.Top := y + lh;
        lblctrl.Parent := Control;
        lblctrl.Width := (Bounds.Right - Bounds.Left) div Layout.Columns;

        c := TRttiContext.Create;

        p := c.GetType(lblctrl.ClassInfo).GetProperty('WordWrap');
        if Assigned(p) then
        begin
          p.SetValue(lblctrl, true);
        end;
        ...
        if Assigned(p) then
        begin
          (p.GetValue(lblctrl).AsObject as TFont).Assign(Layout.Descriptions.Font);
        end;
        ...
        if Assigned(p) then
        begin
          lblcaption := Layout.Items[i].Description;
          DoLabelCreated(Layout.Items[i], lblctrl, lblcaption);
          p.SetValue(lblctrl, lblcaption);
        end;
        ...
        lh := lh + lblctrl.Height + Layout.Spacing.Vert;

        lbls.Add(lblctrl);

        Layout.Items[i].DescriptionCtrl := lblctrl;
      end;
...
procedure TAdvDBFormLayouter.InitScale(AValue: double; ADialogUnits: integer);
begin
  FScale := AValue;
  FDialogUnits := ADialogUnits;
end;
...
begin
  ...
  FItems.OnChange := ValuesChanged;

  FDescriptionCtrl := nil;
  FLabelCtrl := nil;
  FEditCtrl := nil;
  FReadOnlyCtrl := nil;
...
procedure TLayoutItem.SetDescription(const Value: string);
begin
...
procedure TLayoutItem.SetDescriptionType(const Value: TDescriptionType);
begin
  if (FDescriptionType <> Value) then
  begin
    Collection.BeginUpdate;
    FDescriptionType := Value;
    Collection.EndUpdate;
  end;
end;

procedure TLayoutItem.SetItems(const Value: TStringList);
begin
  FItems.Assign(Value);
end;
...
begin
  ...
  FItems := CreateItems(Designing);
  FItems.OnChange := DoItemsChanged;
  FItems.OnBeforeItemDestroy := DoBeforeItemDestroy;

  FDescriptions := TDescriptionSettings.Create;
  FDescriptions.OnChange := SubPropsChanged;

  FColumns := 1;
  FMode := lmColumns;
end;

function TLayout.CreateItems(Designing: boolean): TLayoutItems;
begin
  Result := TLayoutItems.Create(Self, Designing);
end;
...
procedure TLayout.SetDescriptions(const Value: TDescriptionSettings);
begin
...
{ TDescriptionSettings }

procedure TDescriptionSettings.Assign(Source: TPersistent);
begin
  if (Source is TDescriptionSettings) then
  begin
    FDescriptionKind := (Source as TDescriptionSettings).Kind;
  end;
end;

procedure TDescriptionSettings.Changed;
begin
  if Assigned(OnChange) then
    OnChange(Self);
end;

constructor TDescriptionSettings.Create;
begin
  inherited;
  FDescriptionKind := dkHint;
  FFont := TFont.Create;
end;

destructor TDescriptionSettings.Destroy;
begin
  FFont.Free;
  inherited;
end;

procedure TDescriptionSettings.SetDescriptionKind(
  const Value: TDescriptionKind);
begin
  if (FDescriptionKind <> Value) then
  begin
    FDescriptionKind := Value;
    Changed;
  end;
end;

procedure TDescriptionSettings.SetFont(const Value: TFont);
begin
  FFont.Assign(Value);
  Changed;
end;

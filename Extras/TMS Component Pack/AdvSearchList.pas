...
uses
  ...
  AdvScrollControl, AdvGraphics, AdvTypes, AdvUtils
  {$IFDEF VCLLIB}
...
begin
  ...
  {$IFNDEF FMXLIB}
  sz.cx := GetClientWidth - GetSystemMetrics(SM_CXVSCROLL);
  {$ENDIF}
  {$IFDEF FMXLIB}
  sz.cx := GetClientWidth - VScrollWidth;
  {$ENDIF}
    ...
    begin
      ...
      begin
        ...
        if Items[li].ItemType = itCategory then
          ih := CategoryItemHeight
        else
          ih := ItemHeight;


        if j < Columns.Count - 1 then
          r := Rect(x, h, Min(GetClientWidth, x + Columns[j].Width), h + ih)
        else

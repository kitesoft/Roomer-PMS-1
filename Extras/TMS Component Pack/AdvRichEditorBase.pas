...
uses
  ...
  {$IFNDEF FNCLIB}
  {$IFNDEF FMXLIB}
  , AdvOfficeSelectors, AdvOfficeComboBox, GDIPicture
  {$ENDIF}
  {$IFDEF FMXLIB}
  , FMX.TMSRTFEngine
  {$ENDIF}
  ...
  , FMX.Graphics, FMX.Types, FMX.Controls,  FMX.ActnList
  {$ENDIF}
  {$IFNDEF LCLLIB}
  , StrUtils
  {$ENDIF}
  ...
  {$IFDEF LCLLIB}
  , LCLType
  , Graphics, Controls, ActnList, Dialogs
  {$ENDIF}
  {$IFDEF TMSDEBUG}
  , TMSLogging
  {$ENDIF}
  , AdvScrollControl, AdvGraphics, AdvUtils, AdvRichEditorPic, AdvRichEditorRTF
  ;
...
type
  ...
  TURLOpen = (uoCtrl, uoAlt, uoClick, uoNone);
  ...
  public
    constructor Create({%H-}ARect: TRect);
    destructor Destroy; override;
  ...
  TMergeFieldEvent = procedure(Sender: TObject; MergeName: string; var MergeValue: string; var DoMerge: boolean) of object;
  ...
  TPageSize = (psA4, psA5, psA6, psLetter, psLegal);
  ...
  private
    ...
    FIsMerged: boolean;
    FOnMergeField: TMergeFieldEvent;
    ...
    procedure SetPlainTextContent(const {%H-}Value: string);
    function GetIsMerged: boolean;
  protected
    FCaretUpdate: boolean;
    function GetTextSize({%H-}ACanvas: TCanvas; {%H-}el: TREElement; {%H-}AValue: string): TTextSize; virtual;
    function GetTextDescent({%H-}ACanvas: TCanvas; {%H-}el: TREElement): integer; virtual;
    function GetBulletSize({%H-}el: TREElement; {%H-}AValue: string): TSize; virtual;
    function GetPictureSize({%H-}el: TREElement): TSize; virtual;
    function GetPictureMargin: integer; virtual;
    ...
    procedure CalcLine(ACanvas: TCanvas; Y,FromElementIndex, FromWordIndex, MaxLineWidth: integer; var LineWidth, LineHeight, ToElementIndex, ToWordIndex, Descent, BaseLine, {%H-}Indent: integer; var Alignment: TAlignment);
    procedure CalcCaret({%H-}ACanvas: TCanvas; el: TREElement; x, y, LineHeight, LineDescent, LineBaseLine: integer; AValue: string; CharIndex: integer; DoDraw: boolean = true); virtual;

    // Abstract drawing functions
    procedure DrawElement(ACanvas: TCanvas; el: TREElement; x, y, MaxLineWidth, LineHeight, LineDescent, LineBaseLine: integer; AValue: string; CharIndex, ExtraChar: integer); virtual; abstract;
    ...
    function DrawGraphic({%H-}ACanvas: TCanvas; {%H-}ARect: TRect; {%H-}ID: string): boolean; virtual;
    ...
    // Clipboard functions
    procedure CopyToClipboardStr(s: string); virtual; abstract;
    ...
    procedure SetIsMerged(const Value: boolean);
    ...
    function IsDelimiter(ch: char): boolean; overload; virtual;
    function IsDelimiter(ch: string): boolean; overload; virtual;
    ...
    property ElementCount: integer read GetElementsCount;
    ...
    procedure ChangeBulletType({%H-}AType: TBulletType; {%H-}AFontName: string);
    procedure DeleteBullets({%H-}AType: TBulletType);
    ...
    procedure DoMergeField(MergeName: string; var MergeValue: string; var DoMerge: boolean); virtual;
    function GetBulletChar(const AType: TBulletType): string; virtual; abstract;
    property Producer: TRTEProducer read FProducer write FProducer;
    function InsertLineBreak(IsShift: boolean; AIndent: integer = 0; Prog: boolean = false): TREElement; overload;
    ...
    procedure UpdateTextSelectionType;
    ...
    property OnMergeField: TMergeFieldEvent read FOnMergeField write FOnMergeField;
  ...
  public
    ...
    procedure SelectionToCaret;

    procedure GetWordAndIndexAtCaret(var AValue: string; var AIndex: integer); override;
    procedure UpdateWordAndIndexAtCaret(AValue: string; AIndex: integer); override;
    procedure UpdateSelectionPoint(LeftSel: boolean; var X, Y: integer); override;
    ...
    function InsertLineBreak: TLineBreakElement; overload;
    ...
    function InsertImage(AStream: TStream): TPictureElement; overload;
    ...
    procedure InsertChar(ch: char); overload; override;
    procedure InsertChar(value: string); overload; override;
    procedure InsertTab;
    ...
    procedure SaveToText(AFileName: string); overload;
    procedure SaveToText(AFileName: string; {%H-}Encoding: TEncoding); overload;
    ...
    procedure IgnoreSelectedError({%H-}ErrorWord: string);
    ...
    procedure SetSelectionUppercase(DoUppercase: boolean);
    procedure SetSelectionLowercase(DoLowercase: boolean);
    procedure SetSelectionNoCase(DoNoCase: boolean);
    ...
    procedure RemoveSelectionBullets;
    ...
    function IsSelectionLowercase: boolean;
    function IsSelectionUppercase: boolean;
    function IsSelectionError: boolean;
    ...
    procedure SetCaret(CaretPos: TCaretPos); overload;
    procedure SetCaret(CharPos: integer); overload;
    property Selection: TSelection read FSelection write FSelection;
    ...
    procedure ReadMemoState(AStream: TStream);
    procedure LoadFromTextFile(const FileName: string); overload;
    procedure LoadFromTextFile(const FileName: string; {%H-}Encoding: TEncoding); overload;
    ...
    procedure InsertSelectionFromStream(const AStream: TStream);
    ...
    function Replace(AText, AReplacement: string; {%H-}MatchCase: boolean = false): boolean;
    ...
    {$ENDIF}
    procedure SetPageSize(APageSize: TPageSize);

    property SpaceAsNbsp: boolean read FSpaceAsNbSp write FSpaceAsNbSp;
  ...
  published
    ...
    property URLOpen: TURLOpen read FURLOpen write FURLOpen default uoCtrl;
  end;
  ...
  TAdvRichEditorSmallCaps = class(TAdvRichEditorAction)
  public
    procedure ExecuteTarget(Target: TObject); override;
    procedure UpdateTarget(Target: TObject); override;
  end;

  TAdvRichEditorLargeCaps = class(TAdvRichEditorAction)
  public
    procedure ExecuteTarget(Target: TObject); override;
    procedure UpdateTarget(Target: TObject); override;
  end;
  ...
  TAdvRichEditorFontSizeUp = class(TAdvRichEditorAction)
  public
    procedure ExecuteTarget(Target: TObject); override;
    procedure UpdateTarget(Target: TObject); override;
  end;

  TAdvRichEditorFontSizeDown = class(TAdvRichEditorAction)
  public
    procedure ExecuteTarget(Target: TObject); override;
    procedure UpdateTarget(Target: TObject); override;
  end;
  ...
  function StrInStr(s: string; Index: Integer): String;
  function ColorToHtml(const Value: TColor): string;

  function EqualFont(Font1,Font2: TFont): boolean;
  function DecodePicture(const str: string): TStream;
...
procedure AssignBitmap(FromBitmap: TBitmap; ToPicture: TGDIPPicture);
{$IFDEF FMXLIB}
var
  ms: TMemoryStream;
{$ENDIF}
begin
  {$IFNDEF FMXLIB}
  ToPicture.Assign(FromBitmap);
  {$ENDIF}
  {$IFDEF FMXLIB}
  ms := TMemoryStream.Create;
  try
    FromBitmap.SaveToStream(ms);
    ms.Position := 0;
    ToPicture.LoadFromStream(ms);
  finally
    ms.Free;
...
function Base64Decode(const Input: string): TStream;
var
  I, J, K: Integer;
  Packet: TPacket;
  ms: TMemoryStream;
...
begin
  if MergeRef <> '' then
  {$IFDEF LCLLIB}
    Result := UTF8Encode(ansistring('«')) + Text + UTF8Encode(ansistring('»'))
  {$ENDIF}
  {$IFNDEF LCLLIB}
    Result := '«' + Text + '»'
  {$ENDIF}
...
begin
  ...
  AssignBitmap(Bitmap, el.FPicture);
...
begin
  ...
  AssignPicture(Picture, el.Picture);
...
function TAdvRichEditorBase.InsertLineBreak: TLineBreakElement;
begin
  Result := TLineBreakElement.Create;
  InsertElementAtCaret(Result);
end;
...
begin
  ...
  begin
      ...
      begin
        ...
        begin
          ...
          begin;
            ...
            Caret.Element := el;
            if Assigned(el) then
              Caret.CharIndex := el.IndexAtEnd  // Length(el.Text)
            else
              Caret.CharIndex := 0;
    ...
    begin
      {$IFDEF TMSDEBUG}
      TMSLogger.Info('Delete:'+caret.Element.ClassName);
      {$ENDIF}
...
// TDebug.Write('Calc line: '+ el.ClassName + ':' + el.Text);
  ...
  LineHeight := sz.cy;
  LeftIndent := OffsetX + Round(el.Indent * DPIratio);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        sz := GetTextSize(ACanvas, nel, TextForHeight);
    ...
    begin
      ...
      begin
        if (s = '') and (FromWordIndex = ToWordIndex) and (LeftIndent + el.Indent + sz.cx >= MaxLineWidth) then
        begin
          LineWidth := LineWidth + sz.cx;
          ...
          if (FromWordIndex = ToWordIndex) and not firstwrd then
          begin
            if (s = '') then
            begin
              inc(ToElementIndex);
...
procedure TAdvRichEditorBase.SelectionToCaret;
begin
    ...
    begin
      ...
      begin
        ...
        begin
          SelFromEl := Caret.Element;
          SelFromChar := Caret.CharIndex;
          carX := Round(GetClientWidth - 4 - OffsetX);
          XYToCaret(carX, Caret.XY.Y - SP.Y + 4, False, IsSel);
...
procedure TAdvRichEditorBase.DoMergeField(MergeName: string;
  var MergeValue: string; var DoMerge: boolean);
begin
  if Assigned(OnMergeField) then
    OnMergeField(Self, MergeName, MergeValue, DoMerge);
end;

procedure TAdvRichEditorBase.DoCorrectWord;
var
  s,so,aso,cs,bs: string;
  ci,nci,dc: integer;
  //d: integer;
  Error, Found, PrevError: boolean;
  pel,el: TREElement;
  i: integer;
  ...
  begin
    ...
    begin
      ...
      if Assigned(pel) and (pel is TTextElement) then
        PrevError := (pel as TTextElement).Error;
      ...
      begin
        ...
        if (bs <> s) or Error then
        begin
          //d := Length(s) - Length(bs);
          ...
          if Error or (PrevError and not Error) then
          ...
          begin
            ...
            Caret.CharIndex := {Caret.CharIndex + d; //} Caret.Element.TextLength;
          end;
...
begin
  ...
  begin
    ...
    if (PaintSelection.FromElement = PaintSelection.ToElement) and
       (PaintSelection.FromElement is TCustomGraphicElement) and
       (Selected <> PaintSelection.FromElement) then
       Exit;
...
var
  ...
  del: TREElement;
  ToWordIndex, LeftIndent: integer;
  firstbul,firstwrd: boolean;
begin
  LineWidth := 0;
  CharsSkipped := 0;
  ...
  LeftIndent := OffsetX + Round(nel.Indent * DPIratio);
  ...
  begin
    ...
    begin
      ...
      begin
        DrawElement(ACanvas, el, SP.X, Y, MaxLineWidth, LineHeight, Descent, BaseLine, '', 0, 0);
      end;
    ...
    begin
      ...
      DrawElement(ACanvas, el, x - sz.cx - 2 - SP.X, y, MaxLineWidth, LineHeight, Descent, BaseLine, '', CharsSkipped, 0);
    end;
    ...
    begin
      ...
      if DoDraw then
        DrawElement(ACanvas, el, x - SP.X, y, MaxLineWidth, LineHeight, Descent, BaseLine, '', CharsSkipped, 0);
...
//      TDebug.Write(el.XYE);
    ...
    begin
      ...
      if (x + sz.cx < MaxLineWidth - SP.X) or ((x + sz.cx >= MaxLineWidth - SP.X) and (FromWordIndex = ToWordIndex) and firstwrd and (LeftIndent + el.Indent + x + sz.cx >= MaxLineWidth)) then
      begin
        if DoDraw then
        begin
          if s <> '' then
            DrawElement(ACanvas, el, x - SP.X, y, MaxLineWidth, LineHeight, Descent, BaseLine, su + ' ', CharsSkipped, 1)
          else
            DrawElement(ACanvas, el, x - SP.X, y, MaxLineWidth, LineHeight, Descent, BaseLine, su, CharsSkipped, 0);
        end;
...
begin
  ...
  begin
    ...
    if not (re is TTextElement) and (s <> '') then
    begin
      FWord.ToElement := re;
      FWord.ToChar := 0;
      Break;
...
begin
  ...
  begin
    SetCaret(cpBeginDoc);
    repeat
       fnd := FindAndReplace(Caret.Element, Caret.CharIndex, AText, AReplacement, MatchCase, True);
       if fnd then
         Result := true;
    until not fnd;
  end;
...
function TAdvRichEditorBase.GetPictureMargin: integer;
begin
  Result := PICTURE_MARGIN;
end;
...
begin
  ...
  if (PaintSelection.ToElement <> PaintSelection.FromElement) then
  begin
    idx := FContext.Content.IndexOf(Selection.FromElement);
    inc(idx);
...
function TAdvRichEditorBase.GetSelectionFromXY: TPoint;
begin
  Result := Selection.FromXY;
end;
...
function TAdvRichEditorBase.GetSelectionToXY: TPoint;
begin
  Result := Selection.ToXY;
end;
...
function TAdvRichEditorBase.TextForHeight: string;
begin
  Result := #12354 + 'gh';
end;
...
begin
  ...
  if Width < Result.cx then
  begin
    Result.cx := Trunc(Width) + ((MLW - Trunc(Width)) div 2) + OffsetX;
  end;
...
function TAdvRichEditorBase.HasTextSelection: boolean;
begin
...
begin
  Result := HasTextSelection or
    (Assigned(Selection.FromElement) and (Selection.FromElement is TCustomGraphicElement));
end;
...
procedure TAdvRichEditorBase.InsertChar(value: string);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if IsSpace(CharInStr(su,1)) then
          begin
            ci := Caret.CharIndex - 1;
            nci := 0;
    ...
    if Assigned(Caret.Element) then
      CharSize := GetTextSize(Canvas, Caret.Element, su).cx
    else
      CharSize := 0;

    if (Caret.Element.XYE.X + CharSize) >= GetLineWidth then
    begin
      SetRange(GetSize(false));
    end;

    if (Caret.XY.X + CharSize) > (TopLeft.X + GetClientWidth) then
    begin
      SP.X := Caret.XY.X - GetClientWidth + CharSize;
      TopLeft := SP;
    end
    else
...
function TAdvRichEditorBase.InsertImage(AStream: TStream): TPictureElement;
var
  lb: TPictureElement;
begin
  if HasSelection then
  ...
  lb.Picture.LoadFromStream(AStream);
  lb.FitToPage(GetClientWidth, GetClientHeight);
...
begin
  ...
  AssignPicture(Picture, lb.Picture);
...
function TAdvRichEditorBase.IsDelimiter(ch: string): boolean;
begin
  Result := (ch = ' ') or (ch = ';') or (ch = '.') or (ch = ';') or (ch = ',')  or (ch = ')') or (ch = '(') or (ch = '[') or (ch = ']') or (ch = '}') or (ch = '{');
end;
...
begin
  ...
  begin
    ...
    while j >= 0 do
    begin
      if (FContext.Content.Items[j] is TBulletElement) then
      begin
        sb := j;
        break;
      end;
    ...
    while j < FContext.Content.Count do
    begin
      if (FContext.Content.Items[j] is TBulletEnd) then
      begin
        be := j;
        break;
      end;
    ...
    if (sb <> -1) then
    begin
      // was last bullet element
      if (se = -1) and (be <> -1) and (sb <> -1) then
      begin
        for j := sb + 1 to be - 1 do
        begin
          el := FContext.Content.Items[j];

          if el.Indent >= BULLETINDENT then
            el.Indent := Max(0, el.Indent - BULLETINDENT);
        end;

        el := FContext.Content.Items[sb];

        if Caret.Element = el then
        begin
          Caret.Element := FContext.Content.Items[sb + 1];
          CaretToSelection;
        end;

        el.Free;
        FContext.Content.Items[sb] := FContext.Content.Items[be];
        FContext.Content.Delete(be);

        el := FContext.Content.Items[sb];
        pel := PreviousElement(el);

        if Assigned(pel) and (pel is TBulletStart) then
        begin
          FContext.Content.Delete(sb - 1);
          FContext.Content.Delete(sb - 1);
        end;
    ...
    if (se <> -1) and (sb <> -1) and (be = -1) then
    begin
      for j := sb + 1 to se - 1 do
      begin
        el := FContext.Content.Items[j];
      ...
      bule := TBulletEnd.Create;
      bule.Font.Assign(Caret.Element.Font);
      bule.Alignment := Caret.Element.Alignment;
      bule.Indent := BULLETINDENT;
      bule.TextColor := Caret.Element.TextColor;
      bule.Color := Caret.Element.Color;

      FContext.Content.Items[sb] := bule;

      el := FContext.Content.Items[se];

      buls := TBulletStart.Create;
      buls.Font.Assign(el.Font);
      buls.Alignment := el.Alignment;
      buls.Indent := BULLETINDENT;
      buls.StartIndent := el.Indent;
      buls.TextColor := el.TextColor;
      buls.Color := el.Color;

      FContext.Content.Insert(se,buls);
  ...
  begin
    ...
    begin
      ...
      el := FContext.Content.Items[i];
      bulind := Max(0, el.Indent - el.BulletIndent);
      break;
    end;
    dec(i);
  end;
...
function TAdvRichEditorBase.IsSelectionLowercase: boolean;
var
  el: TREElement;
begin
  Result := false;
  ...
  begin
    Result := (el as TTextElement).StyleTag = 2;
  end;
end;

function TAdvRichEditorBase.IsSelectionUppercase: boolean;
var
  el: TREElement;
begin
  Result := false;
  ...
  if Assigned(el) and (el is TTextElement) then
  begin
    Result := (el as TTextElement).StyleTag = 1;
  end;
end;
...
function TAdvRichEditorBase.IsSelectionError: boolean;
begin
  Result := false;
  if Assigned(Caret.Element) and (Caret.Element is TTextElement) then
  begin
    Result := (Caret.Element as TTextElement).Error;
  end;
end;
...
begin
  ...
  begin
    ...
    Classes.RegisterClass(TTabElement);
    Classes.RegisterClass(TTableElement);
...
begin
  ...
  AState.TextColorC := GetDefaultFontColor;
...
procedure TAdvRichEditorBase.InsertSelectionFromStream(const AStream: TStream);
var
  mss: TStateSaver;
  ms: TMemoState;
  f: single;
...
procedure TAdvRichEditorBase.SaveToText(AFileName: string; Encoding: TEncoding);
var
...
begin
  ...
  try
    ...
    try
      ...
      {$IFNDEF LCLLIB}
      sl.SaveToFile(AFileName, Encoding);
      {$ENDIF}
      {$IFDEF LCLLIB}
...
begin
  ...
  begin
    ...
    begin
      ...
      SetFontColor(AState.TextColorC);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        bulstr := GetBulletChar((fromel as TBulletElement).&Type);
        buls := GetBulletSize(fromel, bulstr);
        sz.cx := buls.cx;
        sz.cy := buls.cy;
      end;
...
function TAdvRichEditorBase.GetCaretLH: integer;
begin
  Result := Caret.LH;
end;
...
function TAdvRichEditorBase.GetCaretXY: TPoint;
begin
  Result := Caret.XY;
end;
...
begin
  Result := GetElementsCount;
end;

function TAdvRichEditorBase.GetElementsCount: integer;
begin
...
function TAdvRichEditorBase.GetIsMerged: boolean;
begin
  Result := FIsMerged;
end;
...
procedure TAdvRichEditorBase.SetIsMerged(const Value: boolean);
begin
  FIsMerged := Value;
end;
...
begin
  ...
  begin
    ...
    begin
      sz := GetTextSize(Canvas, nil, TextForHeight);
...
begin
  ...
  repeat
    ...
    begin
      ...
      if (el <> PaintSelection.ToElement) and (el = PaintSelection.FromElement) then
      begin
        Result := Result + Copy(S, PaintSelection.FromChar + 1, Length(S));
...
procedure TAdvRichEditorBase.SetCaret(CharPos: Integer);
begin
  SelStart := CharPos;
  Caret.Element := Selection.FromElement;
  Caret.CharIndex := Selection.FromChar;
...
var
  ...
  function CheckEOW(ch: string): boolean;
  begin
    if SpaceOnly then
      Result := IsSpace(ch)
    else
...
begin
  ...
  begin
    ...
    begin
      ...
      repeat
        while (idx > 0) and not CheckEOW(StrInStr(s,idx)) do
        begin
          dec(idx);
          inc(CharIndex);
        end;
...
procedure TAdvRichEditorBase.SetPageSize(APageSize: TPageSize);
begin
  case APageSize of
    psA4:
      begin
        Width := Round(FDPIRatio * 96 * 8.27);
        Height := Round(FDPIRatio * 96 * 11.69);
      end;
    psA5:
      begin
        Width := Round(FDPIRatio * 96 * 5.82);
        Height := Round(FDPIRatio * 96 * 8.27);
      end;
    psA6:
      begin
        Width := Round(FDPIRatio * 96 * 4.13);
        Height := Round(FDPIRatio * 96 * 5.82);
      end;
    psLetter:
      begin
        Width := Round(FDPIRatio * 96 * 8.5);
        Height := Round(FDPIRatio * 96 * 11);
      end;
    psLegal:
      begin
        Width := Round(FDPIRatio * 96 * 8.5);
        Height := Round(FDPIRatio * 96 * 14);
      end;
  end;
end;
...
procedure TAdvRichEditorBase.ReadMemoState(AStream: TStream);
var
  mss: TStateSaver;
  ms: TMemoState;
begin
...
procedure TAdvRichEditorBase.UpdateSelectionPoint(LeftSel: boolean; var X, Y: integer);
var
  fprevsel: TSelection;
  b, tc: Boolean;
begin
  fprevsel := nil;
  try
    fprevsel := TSelection.Create;
    fprevsel.Assign(Selection);
    b := isForwardSelection;
    tc := false;
    XYToCaret(X, Y, false, tc);

    if LeftSel then
    begin
      Selection.ToElement := fprevsel.ToElement;
      Selection.ToChar := fprevsel.ToChar;

      if b then
      begin
        X := Selection.FromXY.X;
        Y := Selection.FromXY.Y;
      end
      else
      begin
        X := Selection.ToXY.X;
    ...
    begin
      Selection.FromElement := fprevsel.FromElement;
      Selection.FromChar := fprevsel.FromChar;
      if not b then
      begin
        X := Selection.FromXY.X;
        Y := Selection.FromXY.Y;
...
procedure TAdvRichEditorBase.UpdateTextSelectionType;
begin
...
procedure TAdvRichEditorBase.SetSelectionUppercase(DoUppercase: boolean);
var
  i: integer;
  tel: TTextElement;
begin
  ...
  begin
    if (FSelectionElements[i] is TTextElement) then
    begin
      tel := (FSelectionElements[i] as TTextElement);
...
procedure TAdvRichEditorBase.SetSelectionLowercase(DoLowercase: boolean);
var
  i: integer;
  tel: TTextElement;
begin
...
procedure TAdvRichEditorBase.SetSelectionNoCase(DoNoCase: boolean);
var
  i: integer;
  tel: TTextElement;
begin
...
procedure TAdvRichEditorBase.RemoveSelectionBullets;
begin
  DeleteBullets(GetSelectionBullet);
end;
...
var
  ...
  bul: TBulletElement;
  CurType: TBulletType;
  CurIndex,CurIndent,BulIndent: integer;
...
begin
  ...
  begin
    ...
    if not hassel and not (Caret.Element is TTextElement) and (Caret.CharIndex = 1) then
    begin
      el := NextElement(Caret.Element);
      if Assigned(el) then
      begin
    ...
    begin
      ...
      BulIndent := BULLETINDENT + fel.Indent;

      inds := TBulletStart.Create;
      inds.Font.Assign(fel.Font);
      inds.Alignment := fel.Alignment;
      inds.Indent := BulIndent;
      ...
      begin
        FSelectionElements.Items[j].Indent := BulIndent;
      ...
      begin
        ...
        begin
          ...
          bul.BulletFont := AFontName;
          bul.Indent := BulIndent; //BULLETINDENT + nel.Indent;
          bul.Spacing := BulletSpacing;
...
begin
  ...
  begin
    ...
    TMSLogger.Info(inttostr(i)+'|'+el.ClassName+':'+al+':'+inttostr(el.Indent)+' ['+inttostr(el.XY.X)+','+inttostr(el.XY.Y)+'] '+el.Text);
    inc(i);
  end;
  {$ENDIF}
end;
...
begin
  ...
  begin
    ...
    TMSLogger.Info(inttostr(i)+'|'+el.ClassName+':'+al+':'+inttostr(el.Indent)+' ['+inttostr(el.XY.X)+','+inttostr(el.XY.Y)+'] {'+GetFontName(el.Font)+'-' + inttostr(el.GetFontSize)+'} |' + txt + '| -clr:'+inttohex(el.Color,6) );
    inc(i);
  end;

  if Assigned(Caret.Element) then
  begin
    TMSLogger.Info('******CARET**********');
    TMSLogger.Info(Caret.Element.ClassName+':'+Caret.Element.Text+':'+inttostr(Caret.CharIndex)+' [' +  inttostr(Caret.XY.X)+','+inttostr(Caret.XY.Y)+','+inttostr(Caret.LH)+'] - index:' + inttostr(FContext.Content.IndexOf(Caret.Element)) );
  end;

  if Assigned(Selection.FromElement) then
  begin
    TMSLogger.Info('******SELFROM********');
    TMSLogger.Info(Selection.FromElement.ClassName+' ['+inttostr(Selection.FromChar)+'] '+Selection.FromElement.Text);
    TMSLogger.Info('XY:'+inttostr(Selection.FromXY.X)+':'+inttostr(Selection.FromXY.Y));
  end;

  if Assigned(Selection.ToElement) then
  begin
    TMSLogger.Info('******SELTO**********');
    TMSLogger.Info(Selection.ToElement.ClassName+' [' + inttostr(Selection.ToChar)+'] '+Selection.ToElement.Text+':');
    TMSLogger.Info('XY:'+inttostr(Selection.ToXY.X)+':'+inttostr(Selection.ToXY.Y));
  end;
  {$ENDIF}
end;
...
var
  ...
  isbul,isbold,isitalic,isunderline,isstrike,isaligned,issub,issuper,isfntclr,isbkclr,isfntface,isfntsize,isfntattr,isaurl,isbulel: boolean;
  fntclose,spanclose: boolean;
  fontface,fontattr, elfontface,closetag,opentag: string;
  fontsize,elfontsize: integer;
  clr,bkclr,deftxtclr: TColor;
  s,url,txt: string;
  ...
  pf: TPictureFormat;
  dolb: boolean;
  preamble,picsz: string;
  fstyle,fweight,spanattr: string;
...
//        s := s + '</DIV>';
    ...
    begin
      ...
      if (bkclr <> (el as TTextElement).Color) and (((el as TTextElement).Color <> clNone) or isbkclr) then
      begin
        if isbkclr (*or ((bkclr and $FFFFFF) <> ((el as TTextElement).Color and $FFFFFF))*) then
        begin
          spanclose := true;
          isbkclr := false;
        end;
        ...
        begin
          opentag := opentag + '<SPAN style="background-color:' + ColorToHTML(bkclr) + '">';
          isbkclr := true;
        end;
      end;
      ...
      if (clr <> (el as TTextElement).TextColor) or fntclose then
      begin
        if isfntclr then
        begin
          fntclose := true;
      ...
      if spanattr <> '' then
      begin
        opentag := opentag + spanattr;
      end;

      if fontattr <> '' then
      begin
        opentag := opentag + '<FONT ' + fontattr +'>';
        isfntattr := true;
      end;

      txt := TAdvUtils.FixMarkup(el.Text);
      if not SpaceAsNbsp then
        txt := StringReplace(txt, '&nbsp;', ' ',[rfReplaceAll]);

      s := s + closetag + opentag + txt;
    end;
    ...
    begin
      ...
      begin
        picsz := ' width="'+inttostr(pic.PictureWidth)+'" height="'+inttostr(pic.PictureHeight)+'" ';
        ...
        igFile:
          begin
            ...
            if (imgpath <> '') and (imgpath <> '.\') then
              s := s + '<IMG src="file://' + imgname + '"' + picsz + '>'
            else
        ...
        igInline:
          begin
            ...
            s := s + preamble + EncodePicture(pic.Picture) + '"' + picsz + '>'
          end;
        igID:
          begin
...
function TAdvRichEditorBase.ContentHeight: integer;
var
  sz: TSize;
begin
  sz := GetSize(false);
...
begin
  ...
  begin
    ...
    begin
      fs := Round(2 * GetFontSize(el.Font));
      FRTFEngine.AddFontSize(fs);
    ...
    begin
      ...
      //else
      FRTFEngine.AddPicture((el as TPictureElement).Picture, (el as TPictureElement).PictureWidth, (el as TPictureElement).PictureHeight);
      {$ENDIF}
      {$IFDEF FMXLIB}
    ...
    begin
      ...
      {$ENDIF}
      {$IFNDEF LINUX}
        FRTFEngine.StartBullet(bul.&Type = btNumber, 'Symbol', integer(bul.&Type), bul.TextColor)
      {$ENDIF}
      else
...
{ TAdvRichEditorSmallCaps }

procedure TAdvRichEditorSmallCaps.ExecuteTarget(Target: TObject);
begin
  inherited;
  with GetControl(Target) do
  begin
    SetSelectionLowercase(not IsSelectionLowerCase);
  end;
end;

procedure TAdvRichEditorSmallCaps.UpdateTarget(Target: TObject);
begin
  inherited;
  Enabled := (Target is TAdvRichEditorBase) and TAdvRichEditorBase(Target).Focused;
  Checked := GetControl(Target).IsSelectionLowerCase;
end;

{ TAdvRichEditorLargeCaps }

procedure TAdvRichEditorLargeCaps.ExecuteTarget(Target: TObject);
begin
  inherited;
  with GetControl(Target) do
  begin
    SetSelectionUpperCase(not IsSelectionUpperCase);
  end;
end;

procedure TAdvRichEditorLargeCaps.UpdateTarget(Target: TObject);
begin
  inherited;
  Enabled := (Target is TAdvRichEditorBase) and TAdvRichEditorBase(Target).Focused;
  Checked := GetControl(Target).IsSelectionUpperCase;
end;
...
{ TAdvRichEditorFontSizeUp }

procedure TAdvRichEditorFontSizeUp.ExecuteTarget(Target: TObject);
var
  sz: integer;
begin
  inherited;
  sz := (GetControl(Target).GetSelectionFontSize);

  if sz < 72 then
    GetControl(Target).SetSelectionFontSize(sz + 1);
end;

procedure TAdvRichEditorFontSizeUp.UpdateTarget(Target: TObject);
begin
  inherited;
  Enabled := (Target is TAdvRichEditorBase) and TAdvRichEditorBase(Target).Focused;
end;

{ TAdvRichEditorFontSizeDown }

procedure TAdvRichEditorFontSizeDown.ExecuteTarget(Target: TObject);
var
  sz: integer;
begin
  inherited;
...
procedure TAdvRichEditorFontSizeDown.UpdateTarget(Target: TObject);
begin
  inherited;
  Enabled := (Target is TAdvRichEditorBase) and TAdvRichEditorBase(Target).Focused;
end;

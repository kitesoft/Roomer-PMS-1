...
const
  ...
  // 2.2.0.4 : Fixed : Issue with setting ItemIndex at designtime
  // 2.2.0.5 : Fixed : Issue with setting EditHeight at designtime
...
begin
  inherited CreateWnd;
  SendMessage(Handle, CB_SETCURSEL, FItemIndex, 0);
...
procedure THTMLComboBox.SetImages(const Value: TCustomImageList);
begin
  FImages := Value;
  Repaint;
end;
...
begin
  if FDropped or not HandleAllocated then
    FItemIndex := Value;

  if HandleAllocated and (Items.Count > 0) then
  begin
...
procedure THTMLComboBox.Loaded;
begin
  inherited;
end;

...
const
  ...
  // v1.0.0.1 : Fixed: File filter hides directories as well
  // v1.0.1.0 : New: SortOrder to set sorting order to files first, folders first
  //          : Improved: Filter functionality with pattern matching
...
type
  ...
  private
    ...
    FSortOrder: TAdvDirectoryTreeViewSortOrder;
    {$IFDEF VCLLIB}
    FSystemContextMenu: Boolean;
    {$ENDIF}
    procedure SetFilter(const Value: String);
    procedure SetSortOrder(const Value: TAdvDirectoryTreeViewSortOrder);
  protected
  ...
  published
    ...
    {$IFDEF VCLLIB}
    property SystemContextMenu: Boolean read FSystemContextMenu write FSystemContextMenu default False;
    {$ENDIF}
    property SortOrder: TAdvDirectoryTreeViewSortOrder read FSortOrder write SetSortOrder default tvsoNone;
  end;
...
begin
  {$IFDEF DELPHI_LLVM}
  if (Length(s) >= 1) and (s[Length(s) - 1] <> PthDel) then
  {$ELSE}
  if (Length(s) >= 1) and (s[Length(s)] <> PthDel) then
  {$ENDIF}
    Result := s + PthDel
  else
    Result := s;
...
var
  ...
  procedure AddNodesInternal(AShowDirectories: Boolean; AShowFiles: Boolean);
  begin
    i := FindFirst(APath + '*', 511, sr);
    while i = 0 do
    begin
      ...
      chk := chk and (TAdvUtils.MatchStrEx(Filter, sr.Name, False) or (Filter = '') or (sr.Attr and 16 = 16));

      if (sr.Attr and 16 = 16) and not AShowDirectories then
        chk := False
      else if (sr.Attr and 16 <> 16) and not AShowFiles then
        chk := False;
...
begin
  ...
  case SortOrder of
    tvsoNone: AddNodesInternal(True, True);
    tvsoFilesFirst:
    begin
      AddNodesInternal(False, True);
      AddNodesInternal(True, False);
    end;
    tvsoDirectoriesFirst:
    begin
      AddNodesInternal(True, False);
      AddNodesInternal(False, True);
    end;
...
function TAdvDirectoryTreeView.GetVersion: string;
begin
...
procedure TAdvDirectoryTreeView.SetSortOrder(
  const Value: TAdvDirectoryTreeViewSortOrder);
begin
  FSortOrder := Value;
  case FRootMode of
    dtvmDrives: LoadDrives;
...
{$IFDEF VCLLIB}
procedure TAdvDirectoryTreeView.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  n: TAdvDirectoryTreeViewNode;
begin
  inherited;
  if (Button = TMouseButton.mbRight) and Assigned(FocusedNode) and SystemContextMenu then
  begin
    n := FocusedNode as TAdvDirectoryTreeViewNode;
    if Assigned(n) then
      ShowSysContextMenu(n.FileName, ClientToScreen(Point(X, Y)));
  end;
end;
{$ENDIF}

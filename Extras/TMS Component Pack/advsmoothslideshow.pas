...
const
  ...
  // v1.4.1.0 : New : OnThumbLeftHandleClick and OnThumbRightHandleClick events
  //          : New : RotationMode property to change rotation to clockwise or counterclockwise
  // v1.4.1.1 : Fixed : Issue deleting item during animation
  // v1.4.2.0 : New : AddImageFromStream method to add images from a stream
...
{$ENDIF}
  ...
  TAdvSmoothSlideShowRotationMode = (rmClockWise, rmCounterClockWise);
  ...
  private
    ...
    FOnThumbLeftHandleClick: TNotifyEvent;
    FOnThumbRightHandleClick: TNotifyEvent;
    FRotationMode: TAdvSmoothSlideShowRotationMode;
    ...
    procedure SetRotationMode(const Value: TAdvSmoothSlideShowRotationMode);
  protected
    ...
    procedure DoThumbLeftHandleClick(Sender: TObject);
    procedure DoThumbRightHandleClick(Sender: TObject);
  ...
  public
    ...
    procedure AddItemFromStream(AStream: TStream; AKind: TSlideShowItemKind; ADescription: string = ''; ACaption: String = ''; AThumbHint: String = '');
  ...
  published
    ...
    property RotationMode: TAdvSmoothSlideShowRotationMode read FRotationMode write SetRotationMode default rmClockWise;
    ...
    property OnThumbLeftHandleClick: TNotifyEvent read FOnThumbLeftHandleClick write FOnThumbLeftHandleClick;
    property OnThumbRightHandleClick: TNotifyEvent read FOnThumbRightHandleClick write FOnThumbRightHandleClick;
...
procedure TAdvSmoothSlideShow.AddItemFromStream(AStream: TStream;
  AKind: TSlideShowItemKind; ADescription: string = ''; ACaption: String = ''; AThumbHint: String = '');
begin
...
procedure TAdvSmoothSlideShow.DoThumbLeftHandleClick(Sender: TObject);
begin
  if Assigned(OnThumbLeftHandleClick) then
    OnThumbLeftHandleClick(Self);
end;

procedure TAdvSmoothSlideShow.DoThumbRightHandleClick(Sender: TObject);
begin
  if Assigned(OnThumbRightHandleClick) then
    OnThumbRightHandleClick(Self);
end;
...
begin
  ...
  begin
    ...
    else if (FNextItemIndex >= 0) and (FNextItemIndex <= Items.Count - 1) then
    begin
...
begin
  ...
  begin
    if FHandleHorzLeftDown or FHandleVertTopDown then
      DoThumbLeftHandleClick(Self);

    if FHandleHorzRightDown or FHandleVertBottomDown then
      DoThumbRightHandleClick(Self);
...
procedure TAdvSmoothSlideShow.SetRotationMode(
  const Value: TAdvSmoothSlideShowRotationMode);
begin
  if FRotationMode <> Value then
    FRotationMode := Value;
end;

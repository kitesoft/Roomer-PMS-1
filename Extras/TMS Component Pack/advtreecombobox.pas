...
const
  ...
  // 1.2.1.0 : Improved : Windows 10 appearance
  // 1.2.1.1 : Improved : Keep text after selection also in smDblClick mode
  // 1.2.1.2 : Improved : Painting on Windows 7
...
begin
  ...
  begin
    ...
    if IsWin10 or IsWin7 then
      btn := CP_DROPDOWNBUTTONRIGHT
    else
      btn := CP_DROPDOWNBUTTON;

    if Down then
      DrawThemeBackground(htheme,Canvas.Handle, btn, CBXS_PRESSED, @ARect, nil)
    else
    begin
      if Hover then
        DrawThemeBackground(htheme,Canvas.Handle, btn, CBXS_HOT, @ARect, nil)
      else
        DrawThemeBackground(htheme,Canvas.Handle, btn, CBXS_NORMAL, @ARect, nil);
    end;

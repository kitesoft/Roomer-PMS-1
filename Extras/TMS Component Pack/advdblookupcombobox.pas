...
const
  ...
  // 1.9.0.1 : Fixed : Issue with Lookup when KeyField = DataField and Lookup field contains non-unique values
  // 1.9.0.2 : Fixed : Auto sizing dropdown width when DropWidth = 0
  // 1.9.1.0 : New : OnGridClickFixedCell, OnGridClickCell event added
  // 1.9.1.1 : Fixed : Issue with closing dropdown after selection
  // 1.9.1.2 : Fixed : Issue with sorting in grid dropdown
  // 1.9.2.0 : New : KeyValue property added
  // 1.9.3.0 : Improved : Changed KeyValue type to variant
  //         : Improved : KeyValue can be used also when no DataSource is used
  // 1.9.3.1 : Improved : Dropdown button display in Windows 7 / Windows 10
  // 1.9.4.0 : New : Label shows label caption via hint when label text is longer than label width
...
type
  ...
  TGridClickCellEvent = procedure (Sender: TObject; Col, Row: Integer) of object;
  ...
  private
    ...
    FClickCell: TPoint;
    FOnGridClickFixedCell: TGridClickCellEvent;
    FOnGridClickCell: TGridClickCellEvent;
  ...
  protected
    ...
    procedure DoClickCell(Sender: TObject; ACol,ARow: integer); virtual;
    procedure DoClickFixedCell(Sender: TObject; ACol,ARow: integer); virtual;
  public
   {$IFDEF TMSDEBUG}
    procedure DebugTest;
   {$ENDIF}
  ...
  published
    ...
    property OnGridClickCell: TGridClickCellEvent read FOnGridClickCell write FOnGridClickCell;
    property OnGridClickFixedCell: TGridClickCellEvent read FOnGridClickFixedCell write FOnGridClickFixedCell;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          DrawThemeBackground(htheme,Canvas.Handle,6,CBXS_NORMAL,@ARect,nil);
        end;
      end;
    end;
...
begin
  ...
  tw := FLabel.Canvas.TextWidth(FLabel.Caption);
  ...
  begin
    ...
    FLabel.ShowHint := tw > FLabelWidth;
    tw := FLabelWidth;
  end
  else
...
begin
  ...
  LookupCol := FLookupColumn;

  for i := 0 to FColumns.Count - 1 do
  begin
    if (FColumns.Items[i].ListField = KeyField) and (KeyField = DataField) then
    begin
      LookupCol := i;
      break;
    end;
  ...
  begin
    ...
    begin
       s := AnsiUpperCase(TFindlist(FAllFields.Items[j]).Strings[LookupCol]);   // FSortColumns rep by FLookUpColumn
  ...
  if (FDropWidth = 0) then
  begin
    FChkForm.ClientWidth := GetGridColumnsWidth + FStringGrid.ColCount + GetSystemMetrics(SM_CXBORDER) * 2;

    if (FStringGrid.RowCount > FStringGrid.VisibleRowCount) then
    begin
      sbw := GetSystemMetrics(SM_CXVSCROLL);
      FChkForm.ClientWidth := FChkForm.ClientWidth + sbw;
    end;
    if DropSizeable then
      FChkForm.ClientWidth := FChkForm.ClientWidth + 4;
  end;
...
begin
  ...
  FStringGrid.OnMouseDown := StringGridMouseDown;
...
function TAdvDBLookupComboBox.GetKeyValue: variant;
var
  fld: TField;
begin
  Result := '';

  if Assigned(DataSource) and Assigned(DataSource.DataSet) and (KeyField <> '') and DataSource.DataSet.Active then
  begin
    fld := DataSource.DataSet.FieldByName(KeyField);
    if Assigned(fld) then
      Result := fld.AsVariant;
  end
  else
  begin
    if Assigned(ListSource) and Assigned(ListSource.DataSet) and (KeyField <> '') and ListSource.DataSet.Active then
    begin
      fld := ListSource.DataSet.FieldByName(KeyField);
      if Assigned(fld) then
        Result := fld.AsVariant;
    end;
  end;
...
procedure TAdvDBLookupComboBox.StringGridMouseDown(Sender: TObject;
...
begin
  ...
  FClickCell := Point(c,r);
end;

procedure TAdvDBLookupComboBox.StringGridMouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  r,c: integer;
begin
  ...
  if (FClickCell.X = c) and (FClickCell.Y = r) then
  begin
    if (r < FStringGrid.FixedRows) then
      DoClickFixedCell(FStringGrid, c, r)
    else
      DoClickCell(FStringGrid, c, r);
  end;

  GridMouseDown(Sender, Button, Shift, X, Y);
end;
...
procedure TAdvDBLookupComboBox.DoClickCell(Sender: TObject; ACol,
  ARow: integer);
begin
  if Assigned(OnGridClickCell) then
    OnGridClickCell(Sender, ACol, ARow);
end;

procedure TAdvDBLookupComboBox.DoClickFixedCell(Sender: TObject; ACol,
  ARow: integer);
begin
  if Assigned(OnGridClickFixedCell) then
    OnGridClickFixedCell(Sender, ACol, ARow);
end;

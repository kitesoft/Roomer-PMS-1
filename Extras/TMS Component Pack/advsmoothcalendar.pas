...
uses
  ...
  AdvGDIP, ComCtrls, DateUtils, Types
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
  ;
...
const
  ...
  // v2.3.0.1 : Fixed : Issue with ShowDaysAfter and selecting till end of month
  // v2.3.0.2 : Fixed : Issue with OnSelectMultiDate triggering more than once when selecting multiple dates
  // v2.3.0.3 : Fixed : Issue with OnSelectDate event triggered when programmatically selecting date
...
begin
  ...
  begin
    ...
    begin
      ...
      if (ssShift in Shift) and FMouseDown and MultiSelect and (d <> -1) and not (CompareDate(d, FStartDate) = 0) and (FEndDate <> d) then
      begin
        FEndDate := d;
     ...
     begin
       ...
       begin
         ...
         begin
           ...
           if (wd = 7) or (date = GetDate(start, dim)) or (date = GetDate(stop, dim)) or (Int(date) = Int(startdate)) or (Int(date) = Int(enddate)) then
           begin
...
begin
  AllowChange := true;
  if Assigned(OnBeforeSelectDate) and not FBlockEvents then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (y <> FCurrentYear) and not FBlockEvents then
          DoChangeYear(Self, FCurrentYear);
        ...
        if (c <> FCurrentMonth) and not FBlockEvents then
          DoChangeMonth(Self, FCurrentMonth);
        ...
        if not Animation and not FBlockEvents then
        ...
        if Animation and not FBlockEvents then
          DoSelectDate(Self, FDateMode, FSelectedDate);
        Changed;
      end;
    end;

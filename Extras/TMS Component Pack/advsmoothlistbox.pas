...
const
  ...
  // v2.9.1.0 : New : ClearFilter
  //          : Fixed : Issue with selected item index when clearing items
  // v2.9.1.1 : Improved : Left text positioning
  // v2.9.1.2 : Fixed : Issue with OnScroll not triggered
  // v2.9.1.3 : Fixed : Issue with button caption not drawn on gtButton graphic left or right type
  // v2.9.2.0 : New : Filter.Modes property to filter on caption, notes and info
  // v2.9.2.1 : Improved : ItemAppearance.DeleteButtonFont
  // v2.9.2.2 : Improved : Performance with large amount of items
  // v2.9.2.3 : Fixed : Issue with HTML formatting in Caption, lookupbar functionality with caption containing HTML and anchor click detection
  // v2.9.2.4 : Fixed : Issue with focusing listbox after hiding it in click event
  // v2.9.2.5 : Fixed : Issue focusing listbox after double-click
  // v2.9.2.6 : Fixed : Issue with focusing listbox items with keyboard / mouse
  // v2.9.2.7 : Fixed : Issue with recalculating selected items
  // v2.9.2.8 : Fixed : Issue with notes indenting
  // v2.9.2.9 : Fixed : Issue with caption indenting
  // v2.9.2.10: Fixed : Isue with caption and notes calculation
  // v2.2.2.11: Fixed : Issue with setting database cursor
...
type
  ...
  private
    ...
    procedure SetDeleteButtonCaption(const Value: string);
    procedure SetDeleteButtonFont(const Value: TFont);
  protected
    procedure Changed;
  ...
  published
    ...
    property DeleteButtonCaption: string read FDeleteButtonCaption write SetDeleteButtonCaption;
    property DeleteButtonFont: TFont read FDeleteButtonFont write SetDeleteButtonFont;
  ...
  public
    ...
    function GetCaptionAnchorAt(X, Y: Integer; Focus: Boolean = False): String;
    procedure Collapse;
    procedure Expand;
    ...
    function GetStrippedHTMLCaption: String;
  ...
  TAdvSmoothListBoxFilterMode = (fmCaption, fmNotes, fmInfo);
  TAdvSmoothListBoxFilterModes = set of TAdvSmoothListBoxFilterMode;
  ...
  private
    ...
    FEnabled: Boolean;
    FTextHint: String;
    FModes: TAdvSmoothListBoxFilterModes;
    procedure SetFill(const Value: TGDIPFill);
    procedure SetHeight(const Value: integer);
    procedure SetVisible(const Value: Boolean);
    ...
    procedure SetModes(const Value: TAdvSmoothListBoxFilterModes);
  public
    procedure Changed;
    procedure FillChanged(Sender: TObject);
    constructor Create(AOwner: TAdvSmoothListBox);
  ...
  published
    ...
    property Modes: TAdvSmoothListBoxFilterModes read FModes write SetModes default [fmCaption];
  end;
  ...
  protected
    function GetFilterOwner: TAdvSmoothListBox; virtual;
    procedure SetSelectedItemIndex(const Value: integer); virtual;
    ...
    procedure DoScroll(CurrentPosition, EndPosition: Double); virtual;
...
begin
  ...
  begin
    case CategoryType of
      alphanumeric:
      begin
        if (Length(item1.GetStrippedHTMLCaption) > 0) and (Length(item2.GetStrippedHTMLCaption) > 0) then
        begin
          Result := (Item1.GetStrippedHTMLCaption[1] <> item2.GetStrippedHTMLCaption[1]);
        end
        else
        begin
          if (Length(item1.GetStrippedHTMLCaption) = 0) and (Length(item2.GetStrippedHTMLCaption) = 0) then
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if calc then
            CalculateRect(Items[I]);
        end;
        Items.FSelectedItem := FItems[FSelectedItemIndex];
        Items.FSelectedItem.FSelected := True;
        FFocusedItemIndex := FSelectedItemIndex;
        if Items.FSelectedItem <> nil then
          DoSelectItem(Items.FSelectedItem.Index)
        else
          DoSelectItem(-1);
        CalculateRect(Items.FSelectedItem);
        Changed;
...
begin
  ...
  begin
    ...
    begin
      if DisplayItem <> nil then
      begin
        if pos(Uppercase(FLookupKey), Uppercase(DisplayItem.GetStrippedHTMLCaption)) = 1 then
        begin
...
begin
  ...
  begin
    ...
    begin
      ...
      AnchorC := GetCaptionAnchorAt(X, Y);
  ...
  if (AnchorI <> '') or (AnchorC <> '') or (AnchorF <> '') or (AnchorH <> '') then
  begin
    if AnchorI <> '' then
    begin
      if item <> -1 then
    ...
    begin
      ...
      begin
        with FDisplayList.GetItem(item).DisplayItem do
          DoItemAnchorClick(Self, AnchorC, Index);
      end;
    end;
...
var
  item: integer;
  AnchorF, AnchorH, AnchorI, AnchorC: String;
  rh, rf, dfb, r: TRect;
  dItem: TAdvSmoothListBoxItem;
  di: TImage;
  ...
  begin
    ...
    begin
      ...
      case CategoryType of
        alphanumeric:
        begin
          if Length(Item.GetStrippedHTMLCaption) > 0 then
            SectionCaption := Item.GetStrippedHTMLCaption[1]
          else
            SectionCaption := '';
        end;
        custom:
...
begin
  ...
  begin
    ...
    begin
      ...
      if UseHTML then
      begin
        capr := Bounds(round(ir.x), Round(ir.y), round(ir.Width - CaptionMargin - LookupBar.GetWidth - wl - wr), 10000);

        HTMLDrawGDIP(g, captionf, c , capr, FImages, 0,0,-1,-1,0, False,false,false,false,
          False,False,true,1.0,clBlue,clNone,clNone,clNone,a,s,k,XSize,YSize,l,m,hr,nil, FContainer,2);
...
procedure TAdvSmoothListBox.ClearFilter;
begin
  if Assigned(Filter.Edit) then
    Filter.Edit.Clear;
end;
...
procedure TAdvSmoothListBox.DoScroll(CurrentPosition,
  EndPosition: Double);
begin
  if Assigned(FOnScroll) then
    FOnScroll(Self, CurrentPosition, EndPosition);
end;
      ...
      case CategoryType of
        ...
        begin
          ...
          begin
            ...
            x := 0;
            case Position of
              pLeft: x := InsideRect.Left + (FMargin div 2);
  ...
  begin
   result := FDisplayList.GetItem(i);
    ...
    begin
      if DisplayItem <> nil then
      begin
        if Length(DisplayItem.GetStrippedHTMLCaption) > 0 then
        begin
          if UpCase(NormalizeSpecialChar(DisplayItem.GetStrippedHTMLCaption)[1]) = ch then
          begin
            break;
          end;
        end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        HTMLDrawGDIP(g, captionf, c , fhtmlrvar, FImages, 0,0,-1,-1,0,False,false,false,false,
          False,False,true,1.0,clBlue,clNone,clNone,clNone,a,s,k,XSize,YSize,l,m,hr,nil, FContainer,2);
      end
      else
        ...
        case GraphicLeftType of
          ...
          gtButton:
            begin
              ...
              if FDisplayList.GetItem(DisplayIndex).DisplayItem.ButtonCaption <> ''  then
              begin
                ca.Brush.Style := bsClear;
                DrawText(ca.Handle,
                         PChar(FDisplayList.GetItem(DisplayIndex).DisplayItem.ButtonCaption),
                         Length(FDisplayList.GetItem(DisplayIndex).DisplayItem.ButtonCaption),
                         fgrLeft, DT_SINGLELINE or DT_VCENTER or DT_CENTER);
              end;
            end;
            ...
            begin
              ...
              begin
                ...
                DrawText(ca.Handle,
                         PChar(FDisplayList.GetItem(DisplayIndex).DisplayItem.ButtonCaption),
                         Length(FDisplayList.GetItem(DisplayIndex).DisplayItem.ButtonCaption),
                         fgrRight, DT_SINGLELINE or DT_VCENTER or DT_CENTER);
              end;
    ...
    begin
      ...
      ftsf.SetAlignment(StringAlignmentCenter);
      ftg := g.MakeFont(ItemAppearance.DeleteButtonFont);
      ftb := TGPSolidBrush.Create(MakeColor(255, ItemAppearance.DeleteButtonFont.Color));
...
function TAdvSmoothListBoxItem.GetCaptionAnchorAt(X, Y: Integer;
...
begin
  ...
  begin
    ...
    r := Bounds(Round(Fcaptionr.X), Round(Fcaptionr.Y - GetPosition), Round(Fcaptionr.Width), Round(Fcaptionr.Height));
    ...
    begin
      ...
      HTMLDrawGDIP(g, captionf, self.Caption,r,FImages, X,Y,-1,-1,0,true,false,false,false,
        False,False,true,1.0,clBlue,clNone,clNone,clNone,a,s,k,XSize,YSize,l,m,hr,nil, FContainer,2);
...
function TAdvSmoothListBoxItem.GetStrippedHTMLCaption: String;
begin
  Result := HTMLStrip(Caption);
end;
...
begin
  ...
  begin
    ...
    case FOwner.CategoryType of
      alphanumeric:
      begin
        Result := AnsiCompareStr(item1.GetStrippedHTMLCaption, item2.GetStrippedHTMLCaption);
        {
      ...
      begin
        ...
        begin
          if item1.GetStrippedHTMLCaption < item2.GetStrippedHTMLCaption then
            result :=  -1
          else if item1.GetStrippedHTMLCaption > item2.GetStrippedHTMLCaption then
            result := 1
          else
            result := 0
        end;
...
begin
  for Result := 0 to Count - 1 do
    if AnsiCompareText(Items[Result].GetStrippedHTMLCaption, S) = 0 then Exit;
  Result := -1;
end;
...
begin
  ...
  begin
    ...
    FDeleteButtonFont.Assign((Source as TAdvSmoothListBoxItemAppearance).DeleteButtonFont);
    Changed;
  end;
end;
...
begin
  ...
  FDeleteButtonFont := TFont.Create;
  FDeleteButtonFont.Color := clWhite;
  FDeleteButtonFont.Style := [fsBold];
  FDeleteButtonFont.OnChange := FontChanged;
...
procedure TAdvSmoothListBoxItemAppearance.SetDeleteButtonCaption(
  const Value: string);
begin
  if FDeleteButtonCaption <> Value then
  begin
    FDeleteButtonCaption := Value;
    Changed;
  end;
end;

procedure TAdvSmoothListBoxItemAppearance.SetDeleteButtonFont(
  const Value: TFont);
begin
  if FDeleteButtonFont <> Value then
  begin
    FDeleteButtonFont.Assign(Value);
    Changed;
  end;
end;
    ...
    begin
      ...
      begin
        ...
        begin
          if minl = Level then
          begin
            S := AnsiUpperCase(GetStrippedHTMLCaption);
...
function TAdvSmoothListBox.GetFilterOwner: TAdvSmoothListBox;
begin
  Result := Self;
end;
...
begin
  ...
  begin
    ...
    begin
      b := Filter = '';
      if fmCaption in Modes then
        b := b or AnsiContainsText(FOwner.Items[i].GetStrippedHTMLCaption, Filter);

      if fmNotes in Modes then
        b := b or AnsiContainsText(FOwner.Items[i].Notes, Filter);

      if fmInfo in Modes then
        b := b or AnsiContainsText(FOwner.Items[i].Info, Filter);

      FOwner.Items[i].Visible := b;
    end;
...
procedure TAdvSmoothListBoxFilter.SetModes(
  const Value: TAdvSmoothListBoxFilterModes);
begin
...
begin
  ...
  begin
    p := FOwner.GetFilterOwner;
    if Assigned(p) then
    begin
      rc := p.InsideRect;

      if Visible and not (csDesigning in p.ComponentState) then
        FEdit.Parent := p
      else
        FEdit.Parent := nil;

      FEdit.Left := rc.Left + 5;
      FEdit.Top := rc.Top + p.Header.GetHeight + (p.Filter.GetHeight - Fedit.Height) div 2;
      if (not AutoFilter) then
        FEdit.Width := rc.Right - rc.Left - p.GetShadowOffset - FButton.Width - 15
      else
        FEdit.Width := rc.Right - rc.Left - p.GetShadowOffset - 10;

      FEdit.Visible := Visible and Enabled;
      {$IFDEF DELPHI_UNICODE}
      FEdit.TextHint := TextHint;
      {$ENDIF}
      p.CustomizeFilterEdit(FEdit);

      FButton.Parent := FEdit.Parent;
      FButton.Left := FEdit.Left + FEdit.Width + 5;
      FButton.Top := rc.Top + p.Header.GetHeight + (p.Filter.GetHeight - FButton.Height) div 2;
      FButton.Visible := (not AutoFilter) and Visible and Enabled;

      if not (csDesigning in p.ComponentState)  then
        FShowButton.Parent := p
      else
        FShowButton.Parent := nil;
      FShowButton.Left := rc.Left + 3;
      FShowButton.Top := rc.Top - FShowButton.Height - 3 + p.Header.GetHeight;
      ...
      if not Visible and (p.Header.GetHeight > 0) then

...
const
  ...
  // v1.1.0.0 : New : Clear button capability added, OnClearClick event added
  //          : New : Default button capability added, OnDefaultClick event added
  //          : New : ButtonsAlignment property added
  //          : New : CaptionOK, CaptionCancel, CaptionClear, CaptionDefault properties added

resourcestring
  sOKButton = 'OK';
  sCancelButton = 'Cancel';
  sDefaultButton = 'Default';
  sClearButton = 'Clear';
...
type
  ...
  TButtonsAlignment = (baLeft, baRight);
  ...
  private
    ...
    FCaptionOK: string;
    FCaptionClear: string;
    FShowDefaultButton: boolean;
    FShowClearButton: boolean;
    FButtonsWidth: integer;
    FOnDefaultClick: TNotifyEvent;
    FOnClearClick: TNotifyEvent;
    FButtonsAlignment: TButtonsAlignment;
  ...
  protected
    ...
    procedure ButtonClearClicked(Sender: TObject);
    ...
    property OnDefaultClick: TNotifyEvent read FOnDefaultClick write FOnDefaultClick;
  ...
  public
    ...
    procedure Assign(Source: TPersistent); override;
    property ButtonsAlignment: TButtonsAlignment read FButtonsAlignment write FButtonsAlignment default baRight;
    ...
    property CaptionClear: string read FCaptionClear write FCaptionClear;
    property CaptionDefault: string read FCaptionDefault write FCaptionDefault;
    property Form: TForm read FForm;
    property ResultValues[AName: string]: string read GetResultValue;
    property ShowClearButton: boolean read FShowClearButton write FShowClearButton default false;
    property ShowDefaultButton: boolean read FShowDefaultButton write FShowDefaultButton default false;
  ...
  published
    ...
    property CaptionClear;
    property CaptionDefault;
    property Columns;
    property Version;
...
begin
  ...
  if FButtonsAlignment = baRight then
    ba := TAlign.alRight
  else
    ba := TAlign.alLeft;
  ...
  begin
    ...
    FClearButton.OnClick := ButtonClearClicked;
    FClearButton.Caption := CaptionClear;
    FClearButton.Align := ba;
    FClearButton.AlignWithMargins := True;
    FClearButton.Margins.Left := 5;
    FClearButton.Margins.Top := 5;
    FClearButton.Margins.Right := 5;
    FClearButton.Margins.Bottom := 5;
    FClearButton.Parent := FPanel;
    FButtonsWidth := FClearButton.Width + FClearButton.Margins.Left + FClearButton.Margins.Right;
  end;

  if ShowDefaultButton then
  begin
    FDefaultButton := TButton.Create(FPanel);
    FDefaultButton.OnClick := ButtonDefaultClicked;
    FDefaultButton.Caption := CaptionDefault;
    FDefaultButton.Align := ba;
    FDefaultButton.AlignWithMargins := True;
    FDefaultButton.Margins.Left := 5;
    FDefaultButton.Margins.Top := 5;
    FDefaultButton.Margins.Right := 5;
    FDefaultButton.Margins.Bottom := 5;
    FDefaultButton.Parent := FPanel;
    FButtonsWidth := FButtonsWidth + FDefaultButton.Width + FDefaultButton.Margins.Left + FDefaultButton.Margins.Right;
  end;

  FCancelButton := TButton.Create(FPanel);
  FCancelButton.OnClick := ButtonCancelClicked;
  FCancelButton.Caption := CaptionCancel;
  FCancelButton.Align := ba;
  ...
  FButtonsWidth := FButtonsWidth + FCancelButton.Width + FCancelButton.Margins.Left + FCancelButton.Margins.Right;

  FOkButton := TButton.Create(FPanel);
  FOkButton.OnClick := ButtonOkClicked;
  FOkButton.Caption := CaptionOK;
  FOkButton.Align := ba;
  ...
  FButtonsWidth := FButtonsWidth + FOKButton.Width + FOKButton.Margins.Left + FOKButton.Margins.Right;
...
begin
  ...
  FFormWidth := Max(FButtonsWidth, FFormWidth + maxw);
end;
...
procedure TAdvCustomMultiInputQueryDialog.ButtonClearClicked(Sender: TObject);
var
  i: Integer;
begin
  ...
  begin
    ...
    begin
      if FForm.Controls[i] is TAdvEdit then
        (FForm.Controls[i] as TAdvEdit).Text := '';
    end;
  end;
end;

procedure TAdvCustomMultiInputQueryDialog.ButtonDefaultClicked(Sender: TObject);
var
  i: Integer;
begin
  if Assigned(OnDefaultClick) then
    OnDefaultClick(Self);
  ...
  begin
    ...
    begin
      if FForm.Controls[i] is TAdvEdit then
        (FForm.Controls[i] as TAdvEdit).Text := QueryValues[(FForm.Controls[i] as TAdvEdit).Tag].DefaultValue;
    end;
  end;
end;
...
begin
  ...
  FCaptionDefault := sDefaultButton;
  FCaptionClear := sClearButton;
  FShowClearButton := false;
  FShowDefaultButton := false;
  FButtonsAlignment := baRight;
end;

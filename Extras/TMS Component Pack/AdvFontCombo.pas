...
uses
  Windows, Messages, Types, StdCtrls, Classes, Graphics, Controls, SysUtils, Forms,
  Math, AdvStyleIF
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
...
const
  ...
  // 1.5.4.2 : Fixed : Rare issue with handling ESC key on modal forms
  // 1.5.4.3 : Improved : Hide caret when Style = csDropDownList
  // 1.5.4.4 : Fixed : Issue with keyboard up/down after programmatically setting item index
  // 1.5.4.5 : Fixed : Issue with OnChange event triggering
  // 1.5.4.6 : Improved : ItemIndex handling when multiple identical values are in list
  // 1.5.4.7 : Fixed : Issue with selection of recent items
  // 1.5.4.8 : Fixed : Handling Home/End key and updating ItemIndex
...
begin
  ...
  begin
    ...
    begin
      if (FMouseInControl or (GetFocus = Handle) or (Assigned(FDropDownList) and FDropDownList.Visible)) and (FBorderHotColor <> clNone) then
      begin
  ...
  begin
    ...
    if FDisplayRecentSelection then
      for i := min(FRecentSelection.Count - 1, DropDownCount - 1) downto 0 do
        FDropDownListBox.Items.AddObject(FRecentSelection[i], FRecentSelection.Objects[i]);
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if i < RecentSelection.Count then
        begin
          ItemIndex := integer(FDropDownListBox.Items.Objects[i]);
        end
        else
        begin
          ItemIndex := i - RecentSelection.Count;
          FItemIndex := i - RecentSelection.Count;
        end;
      end
      else
      begin
...
procedure TAdvCustomComboBox.DoChange;
begin
...
begin
  ...
  begin
    if (FItems.Strings[FItemIndex] = Text) then
      Exit;
  end;
...
begin
  ...
  begin
    ...
    if FRecentSelection.IndexOf(Text) < 0 then
      FRecentSelection.AddObject(Text, TObject(FItemIndex));
  end;
end;
...
begin
  if Assigned(FDropDownListBox) and (FItemIndex < FDropDownListBox.Items.Count - 1) then
  begin
    FDropDownListBox.ItemIndex := FItemIndex + 1;
    ItemIndex := FDropDownListBox.ItemIndex;
...
begin
  if Assigned(FDropDownListBox) and (FDropDownListBox.Items.Count > 0) and (FItemIndex > 0) then
  begin
    FDropDownListBox.ItemIndex := FItemIndex - 1;
    ItemIndex := FDropDownListBox.ItemIndex;
    Text := Items[FItemIndex];
...
procedure TDropDownWindow.CMDialogKey(var Message: TCMDialogKey);
begin
  if (Message.CharCode = VK_ESCAPE) then
  begin
    if Visible and FListControl.Focused then
    begin
      Hide;

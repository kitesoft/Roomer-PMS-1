...
const
  ...
  // v1.1.0.1 : Fixed : Issue with changing control visibility
  // v1.1.1.0 : New : Public properties BadgeOffsetX,BadgeOffsetY added in TAdvBadgeGlowButton
  // v1.1.1.1 : Fixed : Issue on high-dpi
...
type
  ...
  private
    ...
    FBadgeTextColor: TColor;
    FOnBadgeClick: TNotifyEvent;
    FBadgeOffsetX: integer;
    FBadgeOffsetY: integer;
    procedure CMVisibleChanged(var Message: TMessage); message CM_VISIBLECHANGED;
    procedure SetBadge(const Value: string);
    procedure SetBadgeColor(const Value: TColor);
    procedure SetBadgeTextColor(const Value: TColor);
    ...
    procedure UpdateBadge;
    procedure BadgeClick(Sender: TObject);
    procedure SetBadgeOffsetX(const Value: integer);
    procedure SetBadgeOffsetY(const Value: integer);
  protected
    procedure DoBadgeClick; virtual;
    procedure SetIsOnRibbon(const Value: boolean); override;
  public
    constructor Create(AOwner: TComponent); override;
    procedure CreateWnd; override;
    procedure SetBounds(ALeft, ATop, AWidth, AHeight: Integer); override;
    property BadgeOffsetX: integer read FBadgeOffsetX write SetBadgeOffsetX;
    property BadgeOffsetY: integer read FBadgeOffsetY write SetBadgeOffsetY;
  published
    property Badge: string read FBadge write SetBadge;
    property BadgeColor: TColor read FBadgeColor write SetBadgeColor default clRed;
    property BadgeTextColor: TColor read FBadgeTextColor write SetBadgeTextColor default clWhite;
  ...
  TAdvBadgeMetroTile = class(TAdvMetroTile)
  private
    FBadgeCtrl: TAdvBadge;
    FBadgeColor: TColor;
    FBadge: string;
...
var
  rgn: THandle;
begin
  rgn := CreateRoundRectRgn(1,1,Width+1,Height+1,Round(Width/1.5),Round(Height/1.5));
...
procedure TAdvBadgeButton.CMVisibleChanged(var Message: TMessage);
begin
  inherited;
  if Assigned(FBadgeCtrl) then
    FBadgeCtrl.Visible := Visible;
end;
...
procedure TAdvBadgeButton.CreateWnd;
begin
  inherited;
  UpdateBadge;
end;
...
begin
  if Assigned(FBadgeCtrl) then
  begin
    FBadgeCtrl.Left := Self.Left + Width - 10;
    FBadgeCtrl.Top := Self.Top - 10;
  end;
end;
...
procedure TAdvBadgeLabel.CMVisibleChanged(var Message: TMessage);
begin
  inherited;
  if Assigned(FBadgeCtrl) then
    FBadgeCtrl.Visible := Visible;
...
procedure TAdvBadgeSpeedButton.CMVisibleChanged(var Message: TMessage);
begin
  inherited;
  if Assigned(FBadgeCtrl) then
    FBadgeCtrl.Visible := Visible;
...
procedure TAdvBadgeSpeedButton.Paint;
begin
  inherited;
  if FUpdateBadge then
  begin
    UpdateBadge;
    FUpdateBadge := false;
  end;
...
procedure TAdvBadgeGlowButton.CMVisibleChanged(var Message: TMessage);
begin
  inherited;
  if Assigned(FBadgeCtrl) then
    FBadgeCtrl.Visible := Visible;
...
procedure TAdvBadgeGlowButton.CreateWnd;
begin
  inherited;
  UpdateBadge;
end;
...
procedure TAdvBadgeGlowButton.SetBadgeOffsetX(const Value: integer);
begin
  FBadgeOffsetX := Value;
  UpdateBadge;
end;

procedure TAdvBadgeGlowButton.SetBadgeOffsetY(const Value: integer);
begin
  FBadgeOffsetY := Value;
  UpdateBadge;
end;
...
procedure TAdvBadgeGlowButton.SetIsOnRibbon(const Value: boolean);
begin
  inherited;
  if IsOnRibbon and (BadgeOffsetY = 10) then
    BadgeOffsetY := 0
//  else
//    BadgeOffsetY := 10;
end;
...
begin
  if Assigned(FBadgeCtrl) then
  begin
    FBadgeCtrl.Left := Left + Width - FBadgeOffsetX;
    FBadgeCtrl.Top := Top - FBadgeOffsetY;
    SetWindowPos(FBadgeCtrl.Handle, HWND_TOPMOST, 0,0,0,0,SWP_NOMOVE or SWP_NOMOVE);
  end;
end;

{ TAdvBadgeMetroTile }

procedure TAdvBadgeMetroTile.BadgeClick(Sender: TObject);
begin
  DoBadgeClick;
end;

procedure TAdvBadgeMetroTile.CMVisibleChanged(var Message: TMessage);
begin
  inherited;
  if Assigned(FBadgeCtrl) then
    FBadgeCtrl.Visible := Visible;
...
constructor TAdvBadgeMetroTile.Create(AOwner: TComponent);
begin
  inherited;
  FBadge := '';
  FBadgeColor := clRed;
...
procedure TAdvBadgeMetroTile.CreateBadge;
begin
  FBadgeCtrl := TAdvBadge.Create(Self);
  FBadgeCtrl.Parent := Self.Parent;
  FBadgeCtrl.OnClick := BadgeClick;
...
procedure TAdvBadgeMetroTile.CreateWnd;
begin
  inherited;
  UpdateBadge;
end;

procedure TAdvBadgeMetroTile.DoBadgeClick;
begin
  if Assigned(OnBadgeClick) then
    OnBadgeClick(Self);
end;

procedure TAdvBadgeMetroTile.SetBadge(const Value: string);
begin
  if (FBadge <> Value) then
  begin
    FBadge := Value;
...
procedure TAdvBadgeMetroTile.SetBadgeColor(const Value: TColor);
begin
  if (FBadgeColor <> Value) then
  begin
    FBadgeColor := Value;
...
procedure TAdvBadgeMetroTile.SetBadgeTextColor(const Value: TColor);
begin
  if (FBadgeTextColor <> Value) then
  begin
    FBadgeTextColor := Value;
...
procedure TAdvBadgeMetroTile.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
begin
  inherited;
  UpdateBadge;
end;

procedure TAdvBadgeMetroTile.UpdateBadge;
begin
  if Assigned(FBadgeCtrl) then
  begin
    FBadgeCtrl.Left := Left + Width - 8;

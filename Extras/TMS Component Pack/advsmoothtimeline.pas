...
const
  ...
  // v2.6.1.0 : New : EndTimeMode property to allow sections to end at Now or at the fixed EndTime property
  //          : Fixed : Issue with loading sections with beginupdate / endupdate
  // v2.6.2.0 : New : AnnotationOrientation property
  // v2.6.2.1 : Fixed : Issue with OnIndicatorPositionChanged event declaration
  // v2.6.3.0 : New : IndicatorHint property databinding for TDBAdvSmoothTimeLine
...
type
  ...
  TAdvSmoothTimeLineBarAnnotationOrientation = (aoAuto, aoLeft, aoRight);
  ...
  private
    ...
    FAnnotationOrientation: TAdvSmoothTimeLineBarAnnotationOrientation;
    ...
    procedure SetAnnotationOrientation(
      const Value: TAdvSmoothTimeLineBarAnnotationOrientation);
  protected
    FDBKeyValue: String;
    procedure Changed;
  ...
  published
    property AnnotationOrientation: TAdvSmoothTimeLineBarAnnotationOrientation read FAnnotationOrientation write SetAnnotationOrientation default aoAuto;
  ...
  TAdvSmoothTimeLineBarSectionEndTimeMode = (etmFixed, etmNow);
  ...
  private
    ...
    FItemObject: TObject;
    FTag: Integer;
    FEndTimeMode: TAdvSmoothTimeLineBarSectionEndTimeMode;
    ...
    procedure SetEndTimeMode(
      const Value: TAdvSmoothTimeLineBarSectionEndTimeMode);
  protected
    FDBKeyValue: String;
    procedure Changed;
  ...
  published
    ...
    property EndTimeMode: TAdvSmoothTimeLineBarSectionEndTimeMode read FEndTimeMode write SetEndTimeMode default etmFixed;
    property Hint: String read FHint write SetHint;
  ...
  TAdvSmoothTimeLineIndicatorPosition = procedure(Sender: TObject;
    indicator: TAdvSmoothTimeLineBarIndicator; var Position: TDateTime) of object;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        xe := min(max(DateTimeToX(GetEndTime), r.X), r.X + r.Width);
      end
...
begin
  ...
  begin
    ...
    FSecStartX := startx;
    endx := DateTimeToX(TimeLineSections[FSelectedSection].GetEndTime);
    FSecEndX := endx;
  end
  else
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      vend := TimeLineSections[FSelectedSection].GetEndTime;
  ...
  begin
    if (FSelectedSection >= 0) and
      (FSelectedSection <= TimeLineSections.Count - 1) then
    begin
      stopx := DateTimeToX(TimeLineSections[FSelectedSection].GetEndTime);
          ...
          begin
            ...
            begin
              if (FSelectedSection >= 0) and
                (FSelectedSection <= TimeLineSections.Count - 1) then
              begin
                delta := TimeLineSections[FSelectedSection].GetEndTime - TimeLineSections[FSelectedSection].StartTime;
                ...
                DoSectionPositionChanged(Self, TimeLineSections[FSelectedSection],
                  ...
                  TimeLineSections[FSelectedSection].GetEndTime);
              end;
            end
          end;
...
begin
  ...
  for I := TimeLineSections.Count - 1 downto 0 do
  begin
    if not TimeLineSections[I].FixedSize and (TimeLineSections[i].GetEndTime <= Range.RangeTo) then
    begin
      r := TimeLineSections[I].GetSectionRect;
      r.X := r.X + r.Width - 3;
      r.Width := 6;
...
begin
  ...
  begin
    FItemObject := (Source as TAdvSmoothTimeLineBarSection).ItemObject;
...
function TAdvSmoothTimeLineBarSection.GetEndTime: TDateTime;
begin
  Result := EndTime;
  if EndTimeMode = etmNow then
    Result := Now;
end;
...
procedure TAdvSmoothTimeLineBarSection.SetEndTimeMode(
  const Value: TAdvSmoothTimeLineBarSectionEndTimeMode);
begin
  if FEndTimeMode <> Value then
  begin
    FEndTimeMode := Value;
    Changed;
  end;
...
begin
  ...
  begin
    FAnnotationOrientation := (Source as TAdvSmoothTimeLineBarIndicator).AnnotationOrientation;
...
begin
  ...
  begin
    ...
    case AnnotationPosition of
      apOnTop:
        begin
          if (((rind.X < rtime.X + rtime.Width / 2) and (AnnotationOrientation = aoAuto)) or (AnnotationOrientation = aoRight)) then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        ca := TCanvas.Create;
        ca.Handle := cahdl;
        if (((r.X < rind.X) and (AnnotationOrientation = aoAuto)) or (AnnotationOrientation = aoLeft)) then
        begin
...
procedure TAdvSmoothTimeLineBarIndicator.SetAnnotationOrientation(
  const Value: TAdvSmoothTimeLineBarAnnotationOrientation);
begin
  if FAnnotationOrientation <> Value then
  begin
    FAnnotationOrientation := Value;
    Changed;
  end;
end;

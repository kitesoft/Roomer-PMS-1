...
const
  ...
  // v2.2.1.0 : New : Method StopEdit added
  // v2.2.2.0 : New : Swipe capability added
...
type
  ...
  public
    ...
    property Swipe: boolean read FSwipe write FSwipe default False;
...
procedure TCustomAdvCardList.StopEdit;
begin
...
//    TWinControl(FEdControl).SetFocus;
  ...
  begin
    ...
    begin  // if no card is selected, make sure to select the first one
      SelectedIndex := FInClientAreaCards[0];
...
begin
  ...
  if Assigned(Card) and (Abs(FDownXY.X - X) < 4) and (Abs(FDownXY.Y - Y) < 4) then
  begin
    CardItemID := Card.ID;
...
begin
  ...
  if (Abs(FDownXY.X - X) <= 4) and (Abs(FDownXY.Y - Y) <= 4)  then
  begin
    Card := CardAtXY(X, Y, True);
...
//      else
  ...
  if (FSwipeXY.X <> -1) and FSwipe then
  begin
    if ScrollBarType = sbtHorizontal then
    begin
      d  := FSwipeXY.X - X;

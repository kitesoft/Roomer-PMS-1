...
const
  ...
  // v1.6.3.0 : New : Designtime support for adding page and linked item in AdvPolyPager
  //          : Improved : Initialization of ImageWidth and ImageHeight on TImageSectionItem and TImageTextItem
  //          : Fixed : Drag & Drop issue in designtime editor
  //          : Fixed : Issue with copy and paste item with modified properties
  // v1.6.4.0 : New: OnAfterItemReorder event triggered after an item is succesfully reordered
  //          : New: StaySelected and Down property for button elements in TButtonBarItem
  // v1.6.4.1 : Fixed: Issue with anti-aliased drawing of images from TImageList
...
{$ENDIF}
    ...
    FHTMLCache: Boolean;
    FDidScale: Boolean;
    FDisableReorderAtDesigntime: Boolean;
    FOnAfterItemReorder: TAfterItemReorderEvent;
  ...
  protected
    ...
    property OnAfterItemReorder: TAfterItemReorderEvent read FOnAfterItemReorder write FOnAfterItemReorder;
{$IFNDEF DELPHI2006_LVL}
...
{$ENDIF}
  ...
  public
    ...
    property DisableReorderAtDesigntime: Boolean read FDisableReorderAtDesigntime write FDisableReorderAtDesigntime default False;
...
begin
  ...
  FReorder := true;
  FDisableReorderAtDesigntime := False;
  FOldScrollHPos := 0;
  FOldScrollVPos := 0;
  AutoScroll := false;
  DoubleBuffered := true;
...
begin
  inherited;
  chk := (Reorder or (csDesigning in ComponentState)) and not DisableReorderAtDesigntime;
  if chk then
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if Assigned(OnAfterItemReorder) then
            OnAfterItemReorder(Self, it, FDropItem);
        end;
        FDropItem := nil;
      end;
    end;

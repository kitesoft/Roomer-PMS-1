...
const
  ...
  // 1.7.0.1 : Fixed : Issue with height calculation
  // 1.7.0.2 : Fixed : When control.ShowHint = false, Office hints are no longer shown
...
begin
  ...
  begin
    ...
    begin
      ...
      if not HintInfo.HintControl.ShowHint  then
        FIsOfficeHint := false;
    end;
    ...
    begin
      ...
      begin
        ...
        if (AHintInfo.Title <> '') or (AHintInfo.WideTitle <> '') then
          TxtH := TxtH + TxTHDiv + (TxtHDiv div 3)
        else
          Rect.Right := hr.Right + 24 + ImgW;
...
//  inherited;
  ...
  begin
    ...
    if Rect.Bottom - Rect.Top < 20 then

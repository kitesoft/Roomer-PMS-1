...
const
  ...
  // 1.0.1.2 : Fixed : Issue with ItemIndex update when Style = csDropDown
  // 1.0.1.3 : Improved : Handling when DB field has non numeric values for images
  // 1.0.1.4 : Fixed : Issue with initialization after desactivating dataset
  // 1.0.1.5 : Fixed : Issue with imagelist image drawing
  // 1.0.2.0 : New : ReturnIsTab exposed
  // 1.0.3.0 : New : Property StyleElements exposed
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        val(s,i,e);
        if e = 0 then
          Images.Draw(Canvas, Rect.Left + 2, Rect.Top + 1, i);
      end
      else

...
type
  ...
  private
    ...
    function SomeURI(Format: TClipFormat): string;
  ...
  TDropFormat = (dfText, dfFile, dfCol, dfRTF, dfURL, dfGridCells, dfURI);
  ...
  public
    constructor Create;
    procedure DropText(pt:TPoint;s:string); virtual; abstract;
    procedure DropStream(pt: TPoint; ms: TMemoryStream); virtual; abstract;
    procedure DropCol(pt:TPoint;col:integer); virtual; abstract;
    procedure DropRTF(pt:TPoint;s:string); virtual; abstract;
    procedure DropFiles(pt:TPoint; files: TStrings); virtual; abstract;
    procedure DropURL(pt:TPoint; s:string); virtual; abstract;
    procedure DragMouseMove(pt:TPoint;var Allow:boolean; DropFormats: TDropFormats); virtual;
    procedure DragMouseEnter; virtual; abstract;
    procedure DragMouseLeave; virtual; abstract;
...
begin
  ...
  if OpenClipBoard(0) then
    begin
     ...
     CF_URI := RegisterClipboardformat('UniformResourceLocator');
     CloseClipBoard;
    end;
end;
...
function TEnumFormats.SomeURI(Format : TClipFormat) : string;
var
  H : hGlobal;
  P: PChar;
  sa: ansistring;
...
function TEnumFormats.URI : string;
begin
  Result := SomeURI(CF_URI)
end;
...
function TEnumFormats.HasURI: boolean;
begin
  Result := HasFormat(CF_URI);
end;
...
begin
  ...
  try
    FOk := (HasText and FAcceptText) or (HasFile and FAcceptFiles) or (HasRTF and RTFAware and FAcceptText)
      or (HasCol and FAcceptCols) or (HasURL and FAcceptURLs) or (HasURI and FAcceptURLs) or (HasGridCells);
    ...
    if HasURI then FDropFormats := FDropFormats + [dfURI];

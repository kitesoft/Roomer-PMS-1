...
const
  ...
  // 1.6.0.1  : Fixed : Rare issue with drag operation during slide-out of panel
...
function TAdvToolPanelTab.GetComponentStyle: TTMSStyle;
begin
  Result := FTMSStyle;
end;
...
begin
  ...
  case Position of
    ...
    ppRight:
      begin
        i := PanelIndex(APanel);
        if (FPanelForm.Width > 0) and (i >= 0) and (i < Panels.Count) and (Panels[i].State = psOpened) then
...
function TAdvToolPanel.GetComponentStyle: TTMSStyle;
begin
  Result := FTMSStyle;
end;

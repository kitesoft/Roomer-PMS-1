...
const
  ...
  // 2.3.8.6  : Fixed : Issue with column widths when moving columns around in the column collection
  // 2.3.8.7  : Fixed : Issue with flickering when search footer is used
  // 2.3.8.8  : Improved : Performance when using dtSequenced for specific datasets
  // 2.3.8.9  : Improved : High DPI handling of column fonts
  // 2.3.9.0  : New : HeaderRow property added
  //          : Improved : Dataset opening handling when search footer is used
  //          : Improved : Ctrl-End key handling
  //          : Fixed : Checkbox handling with readonly fields
  //          : Fixed : Issue with editing and grid form going inactive and EditPostMode = epRow
  // 2.3.9.1  : Improved : Handling of thousand separators with editing numeric values
  // 2.3.9.2  : Fixed : Issue with DB scrolling caused by using grid.SelectRows()
  // 2.3.9.3  : Fixed : Issue with AppendOnArrowDown and DB editing when appending new record
  //          : Fixed : Issue with AdvanceOnEnter, AdvanceInsert and DB editing to append new record
  // 2.3.9.4  : Fixed : Issue with search footer based search for non sequenced datasets
  // 2.3.9.5  : Fixed : Issue with automatic lookup field handling and hidden columns
  // 2.3.9.6  : Fixed : Issue with AdvanceOnEnter and AutoInsert and editing
  //          : Fixed : Issue with AdvanceOnEnter and EditPostMode = epRow
  // 2.3.9.7  : Fixed : Rare issue with scrolling & resizing grid
  //          : Fixed : Issue with AdvanceOnEnter and AutoInsert and editing appended row
  // 2.3.9.8  : Fixed : Issue with toggling FilterDropDownAuto at runtime
  // 2.3.9.9  : Fixed : Issue with cursor up/down handling on vertically merged cells
  // 2.3.9.10 : Fixed : Issue with HTML template processing
  // 2.3.9.11 : Fixed : Issue with datetime inplace editing and MouseActions.DirectEdit = true
  // 2.3.9.12 : Fixed : Issue with row selection on query change
  // 2.4.0.0  : New : DataPictureField type added to show data-linked PictureContainer pictures
  // 2.4.0.1  : Fixed : Issue with EditPostMode = epRow and leaving grid without changing row
  // 2.4.0.2  : Fixed : Issue with scroll when click on partial displayed rows
  // 2.4.0.3  : Fixed : Issue with checkbox handling in grouped grid
  // 2.4.0.4  : Fixed : Issue with setting grid.Row when PageMode = false
  // 2.4.1.0  : Improved : High DPI support
  //          : Improved : Automatic sort style adaption for ftCurrency field types
  // 2.4.1.1  : Fixed : CellTypes[] result for picture field
  // 2.4.1.2  : Fixed : Issue with checkbox handling in epRow mode
  // 2.4.1.3  : Improved : Fields that return type ftBlob but contain picture now also properly export to PDF

resourcestring
  SResExUnidirectional = 'UniDirectional DataSet';
  SResExCheckBoxField = 'Can not assign true when CheckBoxField or ProgressField is also true.';
  SResExPictureField = 'Can not assign true when PictureField or ProgressField is also true.';
  SResExCheckPicField = 'Can not assign true when CheckBoxField or PictureField is also true.';
...
type
  ...
  private
    ...
    procedure SetDataPictureField(const Value: boolean);
  protected
  ...
  published
    ...
    property DataPictureField: boolean read FDataPictureField write SetDataPictureField default false;
  ...
  private
    ...
    function MoveDBCursor(delta: integer): integer;
  protected
  ...
  public
    ...
    procedure ExportNotification(AState: TGridExportState; ARow: Integer); override;
    ...
    property UseRecNo: Boolean read FUseRecNo write FUseRecNo default False;
  published
  ...
  if Active and Assigned(DataSource) then
    if Assigned(DataSource.DataSet) then
      if DataSource.DataSet.IsUnidirectional then
        raise Exception.Create(SResExUnidirectional);
...
begin
  ...
  // store current column width settings
  il := TIntList.Create(-1,-1);

  for i := 0 to (Collection as TDBGridColumnCollection).Count - 1 do
  begin
    il.Add(TDBGridColumnCollection(Collection).Items[i].Width);
  end;
  ...
  // restore current column width settings
  for i := 0 to (Collection as TDBGridColumnCollection).Count - 1 do
  begin
    (Collection as TDBGridColumnCollection).FOwner.ColWidths[i] := il.Items[i];
  end;
...
begin
  if Value and (FCheckBoxField or FProgressField) then
    raise Exception.Create(SResExCheckBoxField);
...
begin
  if Value and (FPictureField or FProgressField) then
    raise Exception.Create(SResExPictureField);
...
begin
  if Value and (FCheckBoxField or FPictureField) then
    raise Exception.Create(SResExCheckPicField);
...
begin
  ...
  if Value then
    FDataPictureField := false;
  TDBGridColumnCollection(Collection).Update(Self);
end;

procedure TDBGridColumnItem.SetDataPictureField(const Value: boolean);
begin
  FDataPictureField := Value;
  if Value then
    FDataImageField := false;
  TDBGridColumnCollection(Collection).Update(Self);
end;
            ...
            begin
              ...
              begin
                ...
                if (aField.DataType = ftCurrency) then
                  FColumnCollection.Items[i].SortStyle := ssFinancial;
              end;
            end;
          end;
        end;
  ...
  {  if not FShowDefinedFields then
   ...
   // FDataLink.BufferCount:= self.VisibleRowCount+1;
      ...
      begin
        ...
        // ensure focused cell is within grid range
...
(*  if FEditPostMode = epRow then
  ...
  begin
    if (FloatingFooter.EnableCalculation <> FOldFoaterEnableCalc) and (FOldEditingState in [dsEdit, dsInsert]) then
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Editor = edComboList) and (idx >= 0) and (Field.DataType in [ftSmallint, ftInteger, ftWord, ftLargeInt {$IFDEF DELPHIXE_LVL}, ftLongWord, ftShortInt{$ENDIF}]) then
      begin
      ...
      begin
        ...
        begin
          ...
          if (Field.DataType in [ftSmallint, ftInteger, ftWord, ftFloat, ftCurrency, ftLargeint {$IFDEF DELPHIXE_LVL}, ftLongWord, ftShortint, ftSingle, ftExtended{$ENDIF}]) then
            Field.Text := RemoveThousandSeparator(FEditText)
          else
      ...
      begin
       ...
       if not FShowDBIndicator or (FixedCols <= 0) then
        ...
        begin
          ...
          if DataSetType = dtSequenced then
          begin
            if (FDataLink.DataSet.State = dsInsert) or (FCacheRecNo {FDataLink.DataSet.RecNo} < 0) then
            begin
              if FDataLink.EOF and (FDataLink.ActiveRecord < r - (Row - FDataLink.ActiveRecord)) then
                FDataLink.ActiveRecord := FDataLink.BufferCount
              else
                FDataLink.ActiveRecord := r - (Row - FDataLink.ActiveRecord)
            end
            else
            begin
              FDataLink.ActiveRecord := r - (Row - FDataLink.ActiveRecord);
          ...
          begin
            ...
            begin
              if FNewAppendRecord then
                FDataLink.ActiveRecord := r - FixedRows - 1
              else
          ...
          try                                                   // do not process HTML template for fixed rows / columns
            if (FColumnCollection.Items[c].HTMLTemplate <> '') and (r >= FixedRows) and (c >= FixedCols) then
            begin
              // query template
          ...
          SB_LINEUP:
            begin
              if (AR > 0) and (AR = FDataLink.BufferCount - 1) then
                AR := AR - 1;
              ds := MoveDBCursor(-AR - 1);
            end;
          SB_LINEDOWN:
            begin
              FDoNotUpdateMe := true;
              a := MoveDBCursor(RC - AR);
        ...
        //FDataLink.DataSet.MoveBy(-1);
            ...
            begin
              ...
              begin
                if FloatingFooter.Visible and (FloatingFooter.FooterStyle = fsFixedLastRow) then
                  ds := MoveDBCursor((MinI(2 - VisibleRowCount, (-AR - 1) + (TopRow - OldTopRow))))
                else
                  ds := MoveDBCursor((-AR - 1) + (TopRow - OldTopRow));
              end
              else
                ds := MoveDBCursor(-VisibleRowCount);
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  ds := MoveDBCursor(MaxI(VisibleRowCount - 2, (RC - AR) + (TopRow - OldTopRow)));
                end
                else
                begin
                  ds := MoveDBCursor((RC - 1 - AR) + (TopRow - OldTopRow));
          ...
          SB_THUMBPOSITION:
            ...
            //FDoNotUpdateMe:= true;
                ...
                begin
                  if (a > 0) then
                  begin
                    ds := MoveDBCursor((RC - AR - 1) + a);
                  end
                  else if (a < 0) then
                  begin
                    ds := MoveDBCursor(-AR + a);
                  end;
                end;
            ...
            //FDoNotUpdateMe:= false;
          {
            ...
            if SI.nTrackPos <= 1 then First
                ...
                case Pos of
                  0: First;
                  1: MoveDBCursor(-VisibleRowCount);
                  2: Exit;
                  3: MoveDBCursor(VisibleRowCount);
                  4: Last;
                end;
            end;
          SB_THUMBTRACK:
  ...
  if FDatalink.Active and HandleAllocated and not FDoNotUpdateMe and PageMode then
    ...
    begin
       ...
       (* if TopRow > FixedRows+1 then
        ...
        begin
          ...
          begin
            ...
            begin
              FBlockCallBack := True;
              Row := MinI(FdataLink.ActiveRecord + FixedRows, RowCount-1);
          ...
          begin
            ...
            begin
              ...
              Row := MinI(FdataLink.ActiveRecord + {1}FixedRows, RowCount - 1);
      ...
      begin
        ...
        if FloatingFooter.Visible and (FloatingFooter.FooterStyle = fsFixedLastRow) then
          nr := MinI(RC, RowCount - 2)
        else
          nr := MinI(RC, RowCount - 1);
...
var
  OldRowCount, i, OldCol, j, d: integer;
...
begin
  ...
  case Msg.CharCode of
    VK_UP:
      begin
        d := 1;
        if (Row - 1 > FixedRows) and IsMergedCell(Col,Row - 1) then
        begin
          d := CellSpan(Col,Row - 1).Y + 1;
        end;
    ...
    VK_DOWN:
      begin
        ...
        try
          ...
          d := 1;
          if (Row < RowCount - 1) and IsMergedCell(Col,Row) then
          begin
            d := CellSpan(Col,Row).Y+1;
          end;
            ...
            begin
              ...
              //FInternalInsert := true;
                ...
                begin
                  ...
                  FInternalCall:= true;
                  FInternalSelection:= true;
                  Row := MinI(Row + 1, RowCount-1);      // FF: Index out of bound
                  FInternalSelection:= false;
    ...
    VK_END:
      begin
        if (Navigation.HomeEndKey in [heFirstLastRow, heFirstLastCell]) or (ssctrl in SS) then
        begin
          BeginUpdate;
          try
            ...
            if (Navigation.HomeEndKey = heFirstLastCell) then
  ...
  if DataSetOK then
    ...
    try
      ...
      begin
        ...
        if (DataSetType = dtSequenced) and (FEditPostMode = epRow)
...
procedure TDBAdvGrid.CreateWnd;
begin
  inherited;
...
begin
  ...
  if Result < ((ClientHeight div DefaultRowHeight) - FixedRows + 1) then
    Result := ((ClientHeight div DefaultRowHeight) - FixedRows + 1)
end;
...
begin
  ...
  begin
    ...
    begin
      if FShowDefinedFields and (Columns[ACol].FieldName = '') then
      begin
      ...
      begin
        ...
        if Assigned(fld) then
        begin
          if fld.DataType in [ftString, ftWideString {$IFDEF DELPHI2007_LVL} ,ftWideMemo {$ENDIF}] then
...
// Show picture when ShowPictureFields = true
  ...
  // DataPictureField
  if (Result = nil) and FColumnCollection.Items[ACol].DataPictureField and (ACol >= IndCol) and (ARow >= FixedRows) then
  begin
    Fld := GetDBFieldAtColumn(ACol);
    if Fld <> nil then
    begin
      if Fld.DataType in [ftString, ftInteger, ftWord, ftMemo {$IFDEF DELPHI_UNICODE}, ftWideString {$ENDIF}] then
      begin
        FCellGraphic.CellType := ctDataPicture;
        FCellGraphic.CellHAlign := haCenter;
        FCellGraphic.CellVAlign := vaCenter;
        FCellGraphic.CellTransparent := True;
...
begin
  ...
  begin
    if ShowBooleanFields and (ACol >= FixedCols) and (ARow >= FixedRows) and
      not ((ARow = RowCount - 1) and (FloatingFooter.Visible)) then
    begin
      Fld := GetDBFieldAtColumn(ACol);
      if Assigned(Fld) then
    ...
    if (Result = ctEmpty) and FColumnCollection.Items[ACol].DataImageField and (ACol >= FixedCols) and (ARow >= FixedRows) and
      not ((ARow = RowCount - 1) and (FloatingFooter.Visible)) then
    begin
    ...
    if (Result = ctEmpty) and ShowPictureFields and (ACol >= FixedCols) and (ARow >= FixedRows) and
      not ((ARow = RowCount - 1) and (FloatingFooter.Visible)) then
    begin
      if not Assigned(Fld) then
        Fld := GetDBFieldAtColumn(ACol);

      if Assigned(Fld) and (Fld.DataType in [ftBlob, ftGraphic]) then
        Result := ctPicture
    end;

    if (Result = ctEmpty) and FColumnCollection.Items[ACol].DataPictureField and (ACol >= FixedCols) and (ARow >= FixedRows) and
      not ((ARow = RowCount - 1) and (FloatingFooter.Visible)) then
    begin
      if not Assigned(Fld) then
        Fld := GetDBFieldAtColumn(ACol);
      ...
      if Assigned(Fld) then
        if Fld.DataType in [ftString, ftMemo {$IFDEF DELPHI_UNICODE}, ftWideString {$ENDIF}] then
        begin
          Result := ctDataPicture;
        end;
    end;
...
begin
  ...
  if (ACol < FColumnCollection.Count) then
  begin
    if (ACol >= FixedCols) and (Arow >= FixedRows) and
      (ACol < ColCount - FixedRightCols + NumHiddenColumns) and
      (ARow < RowCount - FixedFooters) then
...
function TDBAdvGrid.GetFilter(ACol: Integer; Disp: boolean = false; AllRows: boolean = false): Boolean;
var
  i: integer;
begin
  //  FilterList.Clear;
...
begin
  if not CheckDataSet or not Assigned(FColumnCollection) or (ACol >= FColumnCollection.Count) then
    Exit;
...
//FixedRows := FTitleOffset;
  ...
  with FDataLink do
    ...
    begin
      if DataSetType = dtSequenced then
        FCacheRecNo := FDataLink.DataSet.RecNo;

                                         // FF: Field prop changing while inserting
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              Row := MinI(VisibleRowCount+1, RowCount-1);
...
procedure TDBAdvGrid.ChangeScale(M, D: Integer);
var
  j: integer;
begin
  inherited;
...
procedure TDBAdvGrid.ExportNotification(AState: TGridExportState; ARow: Integer);
var
  df: integer;
  OldV: Boolean;
begin
...
//          Exit;
  ...
  //FDataLink.DataSet.First;
    ...
    esExportNewRow, esExportSelRow, esExportFindRow:
      begin
        if (DataSetType = dtSequenced) or (AState = esExportFindRow) then
        begin
          Row := ARow;
        end
        else //DataSetType = dtNonSequenced
...
procedure TDBAdvGrid.InitColumnFonts;
var
  i: integer;
begin
  if (csDesigning in ComponentState) then
  ...
  for i := 0 to Columns.Count - 1 do
  begin
    Columns[i].Font.Height := Round(Columns[i].Font.Height * DPIScale);
    Columns[i].HeaderFont.Height := Round(Columns[i].HeaderFont.Height * DPIScale);
  end;
end;
...
function TDBAdvGrid.MoveDBCursor(delta: integer): integer;
begin
  if UseRecNo then
  begin
    if FDataLink.DataSet.RecNo + delta >= FDataLink.DataSet.RecordCount then
      delta := FDataLink.DataSet.RecordCount - FDataLink.DataSet.RecNo;

    if FDataLink.DataSet.RecNo + delta < 0 then
      delta := -FDataLink.DataSet.RecNo;

    FDataLink.DataSet.RecNo := FDataLink.DataSet.RecNo + delta;
    Result := delta;
  end
  else
    Result := FDataLink.DataSet.MoveBy(delta);
end;

procedure TDBAdvGrid.SetHeaderRow(const Value: Integer);
begin
  if Value >= FixedRows then
    Exit;

  if (FHeaderRow <> Value) then
  begin
    FHeaderRow := Value;
    if not PageMode then
      LoadFromDataSet;
...
procedure TDBAdvGrid.DoExit;
begin
  inherited;

  if CheckDataSet and (FDataLink.DataSet.State = dsEdit) then
  begin
    if FRecordChanged and (FEditPostMode = epRow) then
    begin
      FRecordChanged := false;
      FDataLink.DataSet.Post;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        aField := GetDBFieldAtColumn(c);
        if (FColumnCollection.Items[c].Header <> '') then
          Cells[c, HeaderRow] := FColumnCollection.Items[c].Header
        else if (aField <> nil) and aField.Visible then
          Cells[c, HeaderRow] := aField.DisplayName;
      end;
    end;
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  else if (FColumnCollection.Items[c].DataPictureField) then
                  begin
                    Value := aField.DisplayText;
                    AddDataPicture(c,r,Value,haLeft, vaTop);
                  end
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (AEditor in [edButton, edNormal, edNumeric, edFloat, edValidChars, edPositiveNumeric, edPositiveFloat, edUppercase, edMixedCase, edLowerCase]) then
...
begin
  ...
  if (R >= TopRow + VisibleRowCount - i) and not SearchFooter.Visible then
  begin
...
begin
  ...
  if (DataSetType = dtSequenced) or (PageMode = false) then
    inherited Row := Value
  else
  begin
...
begin
  ...
  if Navigation.AdvanceOnEnter and (FDataLink.DataSet.State in [dsEdit, dsInsert]) and FRecordChanged and (Col = ColCount - 1) and (Navigation.AdvanceInsert) then
  begin
  ...
  if Navigation.AdvanceOnEnter and (FDataLink.DataSet.State in [dsEdit, dsInsert]) and (not Navigation.AdvanceInsert or (Col < ColCount - 1)) and (EditPostMode = epRow) then
  begin
    if Key = VK_RETURN then
    begin
      ch := #13;

...
const
 ...
 //  1.9.0.7 : Fixed : Regression with AutoHide and AlwaysOnTop
 //  1.9.0.8 : Fixed : Issue with AutoSize on newer Delphi versions
...
type
  ...
  private
    ...
    procedure SetFadeStep(const Value: byte);
  protected
  ...
  published
    ...
    property FadeStep: byte read FFadeStep write SetFadeStep;
...
begin
  ...
  begin
    ...
    if Assigned(FOwner) and (not AlwaysOnTop or AutoHide) then
    begin
...
procedure TAdvAlertWindow.SetFadeStep(const Value: byte);
begin
  if Value > 0 then
    FFadeStep := Value;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        SetMessages;
        if AutoSize and not FFading then
        begin
...
procedure TAdvAlertWindow.SetImages(const Value: TCustomImageList);
begin
  FImages := Value;
end;
...
function TAdvAlertWindow.GetComponentStyle: TTMSStyle;
begin
  Result := FTMSStyle;
end;

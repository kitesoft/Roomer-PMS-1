...
const
  ...
  // 1.6.1.0  : New : High-DPI support in THTMLCheckBox, THTMLRadioButton added
  // 1.6.2.0  : New : Added StyleElements property to THTMLButton
  // 1.6.3.0  : New : AllowGrayed property added
  // 1.6.3.1  : Improved : High-DPI handling
...
type
 ...
 protected
  ...
  published
    ...
    property ButtonType: TButtonType read FButtonType write SetButtonType default btTheme;
...
procedure TCustomHTMLCheckBox.CreateWnd;
begin
  inherited;
  FScaleFactor := CalculateDPIScale;
end;
...
begin
  ...
  begin
    ...
    btBorland:
      begin
        bs := Round(12 * FScaleFactor);
        ExtraBW := ExtraBW + (bs - 12);

        case FBtnVAlign of
        tlTop: BT := 4;
        tlCenter: BT := (ClientRect.Bottom - ClientRect.Top) div 2 - (bs div 2);
        tlBottom: BT := ClientRect.Bottom - bs - 4;
        end;
        BB := BT + bs;
        if FAlignment = taRightJustify then
          BL := ClientRect.Right - bs - 1
        else
          BL := 1;
        BR := bs + BL;
    ...
    btWinXP:
      begin
        ...
        begin
          ...
          r := Rect(BL, BT, BL + Round(sz * FScalefactor), BT + Round(sz * FScalefactor));

          ExtraBW := ExtraBW + Round((14 * FScaleFactor) - 14);
    ...
    btClassic, btFlat:
      begin
        bs := Round(13 * FScaleFactor);

        if fAlignment = taRightJustify then
          r.Left := ClientRect.Right - bs
        else
          r.left := 0;

        ExtraBW := ExtraBW + Round((13 * FScaleFactor) - 13);

        r.Right := r.Left + bs;
      	case FBtnVAlign of
        tlTop: r.Top := 4;
        tlCenter: r.Top := (ClientRect.Bottom - ClientRect.Top) div 2 - 6;
        tlBottom: r.Top := ClientRect.Bottom - 4 - bs;
        end;
        r.Bottom := r.Top + bs;
        flg := 0;
...
procedure TCustomHTMLCheckBox.SetImages(const Value: TCustomImageList);
begin
  FImages := Value;
  Invalidate;
end;
...
begin
  ...
  r.left := r.left + Round((BW + 5) * FScaleFactor);
  r.top := r.top + 4;
  ...
  begin
    ...
    btWinXP:
      begin
        ...
        begin
          ...
          bs := Round(sz * FScaleFactor);

          r := Rect(BL, BT, BL + bs, BT + bs);
    ...
    btClassic,btFlat:
      begin
        ...
        tlTop: r.Top := 4;
        tlCenter: r.Top := (ClientRect.Bottom - ClientRect.Top) div 2 - 6;
        tlBottom: r.Top := ClientRect.Bottom - bs - 4;
        end;
        ...
        if FButtonType = btFlat then
          flg := flg or  DFCS_FLAT;
        if not Enabled then
          flg := flg or DFCS_INACTIVE;
      ...
      begin
        r.Left := 0;
        r.Right := r.Right - BR - Round(7 * FScaleFactor);
      end
      else
        r.Left := r.Left + BR + Round(7 * FScaleFactor);
...
procedure THTMLRadioButton.SetImages(const Value: TCustomImageList);
begin
  FImages := Value;
  Invalidate;
end;
...
procedure THTMLButton.SetImages(const Value: TCustomImageList);
begin
   FImages := Value;
   Repaint;
end;
...
procedure TCustomHTMLRadioGroup.SetImages(const Value: TCustomImageList);
begin
  FImages := Value;
  ArrangeButtons;
end;
...
procedure TCustomHTMLCheckGroup.SetImages(const Value: TCustomImageList);
begin
  FImages := Value;
  ArrangeButtons;
end;

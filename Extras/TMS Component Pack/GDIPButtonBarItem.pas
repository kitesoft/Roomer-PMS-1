...
const
  ...
  //v1.1.1.0 : New : StaySelected and Down property for each bar button element
...
type
  ...
  private
    ...
    procedure SetStaySelected(const Value: Boolean);
  ...
  published
    ...
    property StaySelected: Boolean read FStaySelected write SetStaySelected default False;
  end;
    ...
    begin
      ...
      begin
        ...
        begin
          if Enabled then
          begin
            if (FDownElement = Elements[i]) or (Elements[I].Down) then
            begin
...
begin
  ...
  begin
    ...
    FDown := (Source as TButtonBarElement).Down;
    FStaySelected := (Source as TButtonBarElement).StaySelected;
    Changed;
  end;
end;
...
procedure TButtonBarElement.SetDown(const Value: Boolean);
begin
  if FDown <> Value then
  begin
    FDown := Value;
...
procedure TButtonBarElement.SetStaySelected(const Value: Boolean);
begin
  if FStaySelected <> Value then
  begin
    FStaySelected := Value;
    Changed;
  end;

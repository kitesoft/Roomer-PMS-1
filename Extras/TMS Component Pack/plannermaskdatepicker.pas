...
uses
  ...
  AdvMEdBtn, PlannerCal, MaskUtils, Variants;
...
const
  ...
  // 1.5.1.1 : Fixed : Issue with retrieving updated date after focus change
  ...
  begin
    try
      if (Text = '') or (Pos('  ',Text) in [1..5]) then
        ...
        try
          FPlannerCalendar.Date := VarToDateTime(Trim(Text));
        except

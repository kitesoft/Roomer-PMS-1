...
const
  ...
  // v1.1.0.1 : Fixed : Issue setting StartAngle/StopAngle at designtime
  // v1.1.0.2 : Fixed : Issue setting StartAngle/StopAngle twice
  // v1.1.0.3 : Fixed : Issue using dialog version start and stop angle issue
...
type
  ...
  private
    ...
    FStopAngle, FStopAngleTo, FStopAngleTemp, FSaveStopAngleAnim: Double;
...
var
  anim: Boolean;
begin
  if (FUpdateCount > 0) or (csDesigning in ComponentState) or (csLoading in ComponentState) then
  begin
    FStartAngle := Value;
    FStartAngleTo := Value;
...
begin
  ...
  begin
    FStopAngle := Value;
    FStopAngleTo := Value;
    FStopAngleTemp := Value;
    if not FBlockResetStopAngle then
      FSaveStopAngleAnim := -1;
    Changed;
  end
  else
...
begin
  ...
  begin
    if FSaveStopAngleAnim = -1 then
      FSaveStopAngle := StopAngle
    else
      FSaveStopAngle := FSaveStopAngleAnim;
  end;
  ...
  if (amAngle in AnimationMode) and ((Layout = rmlCircularDonut) or (Layout = rmlCircularPie)) then
  begin
    FBlockResetStopAngle := True;
    StopAngle := StartAngle;
    FBlockResetStopAngle := False;
  end;
...
begin
  ...
  if (amAngle in AnimationMode) and ((Layout = rmlCircularDonut) or (Layout = rmlCircularPie)) then
  begin
    FSaveStopAngleAnim := FSaveStopAngle;
    FBlockResetStopAngle := True;
    StopAngle := FSaveStopAngle;
    FBlockResetStopAngle := False;
  end;

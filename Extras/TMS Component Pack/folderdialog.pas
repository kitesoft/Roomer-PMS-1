...
const
  ...
  // v1.1.3.0 : New : Added option fdoShareable to toggle visibility of mapped network drives
  // v1.1.4.0 : New : fdpParentFormCenter position option added in TFolderDialog
...
type
  ...
  TOptions = (fdoBrowseForComputer, fdoBrowseForPrinter, fdoDontgoBelowDomain,
           ...
           fdoNewDialogStyle, fdoNoNewFolderButton, fdoEditBox, fdoShareable);

  TFolderOptions = set of TOptions;

  TDialogPosition = (fdpDefault, fdpScreenCenter, fdpXY, fdpParentFormCenter);
  ...
  published
    ...
    property Options: TFolderOptions read FOptions write SetOptions default [fdoNewDialogStyle, fdoShareable];
      ...
      case DialogPosition of
        ...
        fdpParentFormCenter:
          begin
            ...
            x := r.Left + ((r.Right - r.Left) div 2) - ((rMe.Right - rMe.Left) div 2);
            Y := r.Top +  ((r.Bottom - r.Top) div 2) - ((rMe.Bottom - rMe.Top) div 2);
          end;
      end;
...
begin
  ...
  Options := [fdoNewDialogStyle, fdoShareable];
  FCaption := '';
end;
...
const
  FolderOptions: array [TOptions] of DWORD = (
    ...
    bif_NewDialogStyle, bif_NoNewFolderButton, bif_EditBox, bif_Shareable);
...
begin
  ...
  begin
    if (O in FOptions) and (O <> fdoShareable) then
      bi.ulFlags := bi.ulFlags or FolderOptions[O];
  end;

  if bi.ulFlags and bif_NewDialogStyle = bif_NewDialogStyle then
  begin
    // only add BIF_SHAREABLE when new style dialog is choosen, otherwise, this option isn't available

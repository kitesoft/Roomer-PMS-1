...
begin
  ...
  ButtonWidth := MulDiv(mcButtonWidth, DialogUnits.X, 4) + 1;

  if (verinfo.dwMajorVersion < 6) then
    ButtonHeight := MulDiv(mcButtonHeight, DialogUnits.Y, 8) - 1
  else
    ButtonHeight := MulDiv(mcButtonHeight, DialogUnits.Y, 8) - 3;

  if TaskDialog.DefaultButtonWidth > 0 then
    ButtonWidth := TaskDialog.DefaultButtonWidth;

  if TaskDialog.DefaultButtonHeight > 0 then
    ButtonHeight := TaskDialog.DefaultButtonHeight;
...
//    w := 800;
  ...
  if (TaskDialog.InputType in [itEdit, itPassword]) and Assigned(FInputEdit) then
    FInputEdit.SetBounds(FInputEdit.Left, Y + 20, ClientWidth - FInputEdit.Left - 15, 20);

  if (TaskDialog.InputType in [itMemo]) and Assigned(FInputMemo) then
    FInputMemo.SetBounds(FInputMemo.Left, Y + 20, ClientWidth - FInputMemo.Left - 15, 60);

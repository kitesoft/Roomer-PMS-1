...
type
  ...
  protected
    ...
    function PtOnEdge(pt: TPoint; Items: TCapacityItems): TCapacityItem;
  ...
  public
    ...
    function GetInsideRectF(r: TRect): TGPRectF;
    property BarRect: TGPRectF read FBarRect;
    property OnChange: TNotifyEvent read FOnChange write FOnChange;
  published
...
begin
  ...
  begin
    ...
    p := CreateBubbleRectangleConvex(Rect(hs + x,vs + y,x + Width - 1 - hs, y + (ph div 2) + 1), Rounding, delta);
  ...
  begin
    ...
    begin
      stringFormat.SetHotkeyPrefix(HotkeyPrefixShow);
      if WW then
        g.DrawString(Caption, Length(Caption), f, MakeRect(textr.X - 2, textr.Y, textr.Width + 4, textr.Height), stringformat, bs)
      else
...
begin
  ...
  begin
    AllowChange := (Source as TCapacityItem).AllowChange;
  ...
  begin
    ...
    try
      ...
      try
        ...
        if Reflection then
        begin
          gbmp := GetReflection(bmp, ReflectionOpacityStart, ReflectionOpacityEnd, FMarkSize - 1, ReflectionAxis);
          g.DrawImageRect(gbmp, Round(pt.X), Round(pt.Y) + ReflectionAxis + bmp.Height, bmp.Width, bmp.Height);
          gbmp.Free;
        end;
      finally
        gr.Free;
...
function TGDIPCapacityBar.PtOnEdge(pt: TPoint; Items: TCapacityItems): TCapacityItem;
var
  I, h: Integer;
  R, RE: TRect;
begin
  ...
  begin
    ...
    begin
      R := Rect(Round(Items[I].Left), Round(FBarRect.Y), Round(Items[I].Left) + Round(Items[I].Width), Round(FBarRect.Y) + h);
      RE := Rect(r.Right - 2, r.Top, r.Right + 2, r.Bottom);
      if PtInRect(RE, pt) then
      begin
        Result := Items[I];
        Break;
      end;

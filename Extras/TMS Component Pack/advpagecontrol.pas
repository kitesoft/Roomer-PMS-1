...
uses
  Messages, Windows, Types, SysUtils, Classes, Controls,
...
const
  ...
  // 2.0.0.7 : Improved : Property setter handling
  // 2.0.0.8 : Fixed : Issue with setting tab imageindex to -1
  // 2.0.0.9 : Fixed : Issue when pressing close button on inactive tab
  // 2.0.0.10: Improved : Design time config of tabsheet.TabEnable
...
type
  ...
  protected
    ...
    function CurPageWidth(i : Integer): SmallInt; virtual;
    function MaxPageWidth: Integer; virtual;
    procedure HideScrollButtons; virtual;
...
begin
  ...
  begin
    { For a AdvPageControl, TabIndex refers to visible tabs only. The control doesn't store }
    Visible := 0;
    NotVisible := 0;
    for I := 0 to FPages.Count - 1 do
    begin
...
begin
  ...
  begin
    ...
    begin
      aAdvTabSheet := FindNextPage(Pages[TabIndex], tabIndex <> PageCount - 1, true);
...
function TAdvPageControl.GetPenColor: TColor;
begin
  Result := clNone;
end;

function TAdvPageControl.CurPageWidth(i : Integer): SmallInt;
var
  res: SmallInt;
begin
  res := 0;

  if (Pages [i].TabVisible = True) then
  begin
    res := self.TabWidth;
  end;
...
function TAdvPageControl.MaxPageWidth (): Integer;
var
  i: Integer;
  res: Integer;
begin
...
procedure TAdvPageControl.HideScrollButtons ();
var
  Wnd : HWND;
begin
  Wnd := FindWindowEx(Handle, 0, 'msctls_updown32', nil);
  if ((Wnd <> 0) and (MaxPageWidth () <= ClientWidth)) then
  begin
...
begin
  ...
  while (i >= 0) do
  begin
    if (TAdvTabSheet (Pages [I]).TabVisible = True) then
    begin
      HasTabs := True;
      i := 0;
    end;
...
var
  i: integer;
begin
  if (FDefaultTextColor <> Value) then
  begin
    for i := 0 to PageCount - 1 do
    begin
      if TAdvTabSheet(FPages[i]).TextColor = FDefaultTextColor then 
        TAdvTabSheet(FPages[i]).TextColor := Value;
    end;
...
var
  i: integer;
begin
  if (FDefaultTabColorTo <> Value) then
  begin
    for i := 0 to PageCount - 1 do
    begin
      if TAdvTabSheet(FPages[i]).TabColorTo = FDefaultTabColorTo then 
        TAdvTabSheet(FPages[i]).TabColorTo := Value;
    end;
...
begin
  if (FTabBackGroundActive <> Value) then
  begin

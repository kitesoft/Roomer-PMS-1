...
const
  ...
  // 6.1.0.2 : Fixed : Issue when setting AppColor
  // 6.1.0.3 : Improved : HighDPI support for 4K screens
  // 6.1.0.4 : Improved : More HighDPI improvements
  // 6.1.0.5 : Fixed : Issue with showing always menu shape
  // 6.1.0.6 : Fixed : Issue with SDI handling
  // 6.1.0.7 : New : Property ShowDropPicture: boolean added
...
{$ENDIF}
    ...
    procedure SetShowDropPicture(const Value: Boolean);
  protected
    procedure Changed;
  ...
  public
    ...
    property ShowDropPicture: Boolean read FShowDropPicture write SetShowDropPicture default true;
    procedure ShowFrame;
    procedure HideFrame;
    procedure ShowMenu;
    procedure HideMenu;
...
begin
  ...
  FIsAeroVista := TMS_DWM and IsComCtl6 and IsVista and ThemeServicesThemesEnabled and AeroIsEnabled and not (csDesigning in ComponentState);
...
begin
  ...
  begin
    ...
    SetBounds(0, 3 + h, Round(FTBSize * ADVToolBarDPI_Scale), th - 2);
  end
  else            // Revert Metro
  begin
    if FItones then
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ACanvas.Brush.Color := (Parent as TCustomForm).Color;
        DrawText(ACanvas.Handle, PChar(Text), Length(Text), TextR, DT_CENTER or DT_VCENTER or DT_SINGLELINE);
      end
      else
...
procedure TAdvCustomShapeButton.SetShowDropPicture(const Value: Boolean);
begin
  if (FShowDropPicture <> Value) then
  begin
    FShowDropPicture := Value;
    Changed;
  end;
end;
...
begin
  ...
  begin
    ...
    if Parent is TAdvToolBarPager then
    begin
      ctrl := (Parent as TAdvToolBarPager).Parent;
      if Assigned(ctrl) and (ctrl.HandleAllocated) then
      begin
        sdiex := GetWindowLong(ctrl.Handle, GWL_EXSTYLE) and WS_EX_APPWINDOW = WS_EX_APPWINDOW;
      end;
    end;
...
//        FShowMenuShape := true;
    ...
    SetBounds(0, 3 + t, Round(FTBSize * ADVToolBarDPI_Scale), th - 2);
    Changed;
  end
  else
  begin
    ...
    begin
      ...
      SetBounds(5, 6, Round(45 * ADVToolBarDPI_Scale), Round(45 * ADVToolBarDPI_Scale)); 
...
begin
  ...
  begin
    ...
    if assigned(fForm) then                                               
      fPixelsPerInch := fForm.PixelsPerInch;                              

    TInternalAdvPreviewMenu(AdvPreviewMenu).SetDPIScale(fPixelsPerInch);  // initialize the DPI Scale of AdvPreviewMenu - dont use mon.PixelsPerInch, because we need to use the same DPI as the Application 
    ...
    begin
      if FOffice2013 then
        AdvPreviewMenu.SetMenuButtonAppearance(Appearance.ColorDown, Appearance.ColorDown,

...
uses
  Classes, Windows, Types, Forms, Dialogs, Controls, Graphics, Messages, ExtCtrls,
  SysUtils, Math, ActnList, GDIPicture, AdvGDIP, AdvStyleIF, AdvMetroRes, Menus;
...
const
  ...
  // 1.1.2.0 : New : Property Appearance.PictureColorDisabled added
  // 1.2.0.0 : New : DropDown type added in TAdvMetroButton
...
type
  ...
  published
    ...
    property PictureColorDisabled: TColor read FPictureColorDisabled write FPictureColorDisabled default clGray;
  end;
  ...
  TMetroDropDown = (mdNone, mdRight, mdSplit);
  ...
  private
    ...
    FMouseInArrow: Boolean;
    FMouseDownInArrow: Boolean;
    FHot: Boolean;
    ...
    procedure SetDropDown(const Value: TMetroDropDown);
  protected
    ...
    property DropDown: TMetroDropDown read FDropDown write SetDropDown default mdNone;
...
begin
  ...
  FMouseDownInArrow := (X > ClientRect.Right - 12) and (DropDown = mdSplit);
...
begin
  ...
  ns := X > ClientRect.Right - 12;

  if (ns <> FMouseInArrow) and (DropDown = mdSplit) then
    Invalidate;
...
procedure TAdvCustomMetroButton.Notification(AComponent: TComponent;
...
procedure DrawArrow(Canvas: TCanvas; ArP: TPoint; ArClr: TColor);
var
  i: integer;
  FArrSize: integer;
  scale: single;
begin
  scale :=  CalcDPIScale(Canvas.Handle, False);

  FArrSize := round(scale * 4);
...
begin
  ...
  if not Enabled then
  begin
    PicClr := Appearance.PictureColorDisabled;
    NormClr := PicClr;
  end
  ...
  begin
    ...
    if (DropDown <> mdSplit) or not FMouseInArrow then
      ACanvas.Font.Color := PicClr
    else
      ACanvas.Font.Color := NormClr;
    ...
    if DropDown <> mdNone then
      DR.Right := DR.Right - 12;
  ...
  begin
    ...
    if (DropDown <> mdSplit) or FMouseInArrow then
      AClr := PicClr
    else
      AClr := NormClr;

    DrawArrow(ACanvas, Point(DR.Left + 1, DR.Top + (DR.Bottom - DR.Top) div 2), AClr);

    if DropDown = mdSplit then
    begin
      ACanvas.Pen.Color := clBlack;

      DR.Top := DR.Top + (DR.Bottom - DR.Top - th) div 2;

      ACanvas.MoveTo(DR.Left, DR.Top);
      ACanvas.LineTo(DR.Left, DR.Top + th);
    end;
  ...
  DrawMetroPicture(graphic, Pic, MakeColor(255, AClr), DR);
...
procedure TAdvCustomMetroButton.SetDropDown(const Value: TMetroDropDown);
begin
...
procedure TAdvCustomMetroButton.DoDropDownClick;
begin
...
procedure TAdvCustomMetroButton.DoDropDownMenu;
var
  pt: TPoint;
begin
...
begin
  ...
  if (DropDown = mdSplit) and (Msg.XPos > ClientRect.Right - 12) and (FMouseDownInArrow) then
  begin
    Msg.Result := 1;
    FMouseDownInArrow := false;
    ReleaseCapture;
    DoDropDownClick;
    Exit;
  end;
  ...
  if Assigned(FDropDownMenu) and (DropDown <> mdSplit) then
    DoDropDownMenu;
...
begin
  ...
  begin
    ...
    FPictureColorDisabled := (Source as TMetroButtonAppearance).PictureColorDisabled;
  end
  else
    inherited;
end;

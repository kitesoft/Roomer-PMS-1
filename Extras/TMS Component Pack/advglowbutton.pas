...
const
  ...
  // 2.4.1.2 : Improved : High-DPI handling
  // 2.4.2.0 : Improved : Notes drawing
  // 2.4.2.1 : Fixed : Rare issue with down / hover painting in specific mouse sequence
  // 2.4.2.2 : Improved : Rendering of blGlyphTop, blGlyphTopAdjusted layout when wordwrapped caption & notes are used
  // 2.4.2.3 : Improved : Further rendering improvements for blGlyphTop, blGlyphTopAdjusted when no picture is used
  // 2.4.2.4 : Improved : Improved rendering when MarginHorz > 1
  // 2.4.2.5 : Fixed : Issue with image spacing for layout blGlyphLeft
  // 2.4.2.6 : Fixed : Issue with popup menu overlapping button
  // 2.4.2.7 : Fixed : Issue with vertical alignment of caption + notes when Layout = blGlyphLeft and AntiAlias = aaNone
  // 2.4.3.0 : New : Public property DrawSplitLineAlways: boolean added
  // 2.4.4.0 : New : UseAppearance property to force a glow button to use its own style regardless of toolbar style
  // 2.4.4.1 : Fixed : Picture position with Layout blGlyphLeft when AntiAlias = aaNone
  // 2.4.4.2 : Fixed : Hotkey handling when DropDownButton is assigned and DropDownSplit = true
  // 2.4.5.0 : New : DropDownDirection ddUp capability added
  //         : New : DownPicture property added
  // 2.4.5.1 : Fixed : Issue with showing down picture when GroupIndex > 0
...
type
  ...
  TDropDownDirection = (ddDown, ddRight, ddUp);
  ...
  private
    ...
    FDrawSplitLineAlways: boolean;
    FDesignClicks: boolean;
    FUseAppearance: boolean;
    ...
    procedure SetDrawSplitLineAlways(const Value: Boolean);
  ...
  protected
    ...
    procedure SetIsOnRibbon(const Value: boolean); virtual;
  ...
  public
    ...
    property DesignClicks: boolean read FDesignClicks write FDesignClicks;
    property IsOnRibbon: boolean read FIsOnRibbon write SetIsOnRibbon;
    ...
    property DrawSplitLineAlways: Boolean read FDrawSplitLineAlways write SetDrawSplitLineAlways default false;
  published
    property Align;
    property Action;
    property Anchors;
    ...
    property UseAppearance: boolean read FUseAppearance write FUseAppearance default False;
...
var
  AGBDrawSplitLine : boolean = false;
...
procedure DrawArrow(Canvas: TCanvas; ArP: TPoint; ArClr, ArShad: TColor; Direction: TDropDownDirection);
var
  i: integer;
  FArrSize, s: integer;
begin
  ...
  ddUp:
    begin
      ...
      begin
        s := FArrSize - 1 - I;
        Canvas.MoveTo(ArP.X + s, ArP.Y + I);
        Canvas.LineTo(ArP.X + (FArrSize * 2) - s - 1, ArP.Y + I);
      end;
    end;
  end;
end;
...
function DrawVistaButton(Owner: TControl; Canvas: TCanvas; r: TRect; CFU, CTU, CFB, CTB, PC, PCF: TColor;
   ...
   ButtonPosition: TButtonPosition; DropDownSplit, DrawBorder, OverlapText, WordWrap, AutoSize, Rounded: boolean; DropDir: TDropDownDirection; Spacing: integer;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        notesRect.Right := notesRect.Left + round(rectf.Width) - ImgW - MargH;
      ...
      case Layout of
        blGlyphLeft:
        begin
          sizeRect.X := szRect.Left + Spacing * 2; // (w - (szRect.Right - szRect.Left) - ImgW) div 2;
        ...
        begin
          sizeRect.X := szRect.Left;
          sizeRect.Y := (h - (szRect.Bottom - szRect.Top - Round(noterect.Height)) - ImgH - 2) div 2;
    ...
    begin
      ...
      if (Layout in [blGlyphLeft, blGlyphLeftAdjusted, blGlyphRight, blGlyphRightAdjusted])  then
        mrectf.Width := mrectf.Width - Spacing;

      if ncap <> '' then
        graphics.MeasureString(ncap, Length(ncap), font, mrectf, stringFormat, sizeRect)
      else
        graphics.MeasureString(WideCaption, Length(WideCaption), font, mrectf, stringFormat, sizeRect);
...
//      if (ImgW > 0) then
        ...
        case Layout of
          ...
          begin
            ...
            begin
              ...
              x2 := w - 2 - ImgW;
              if DropDownButton then
                ImgX := -2 + Round(r.Right - 10 - szRect.right - imgw) div 2
              else
                ImgX := -2 + round(r.Right - szRect.right - imgw) div 2
            end
            else
            begin
              x1 := r.Left + 2 + ImgW;
          ...
          begin
            ...
            begin
              ...
              ImgY := round(y2 - sizerect.Height) div 2;
      ...
      begin
        ...
        begin
          ...
          case Layout of
            ...
            begin
              if notes.Text <> '' then
              begin
                uformat := uformat AND NOT DT_VCENTER or DT_TOP or wwformat or DT_CENTER;
              end
              else
            ...
            blGlyphTop: uformat := DT_TOP or wwformat or DT_CENTER;
          ...
          begin
            ...
            blGlyphLeft, blGlyphLeftAdjusted:
              begin
                if notes.Text <> '' then
                begin
                  th := th + DrawText(Canvas.Handle,PChar(Trim(Notes.Text)),Length(Trim(Notes.Text)), szrect, uformat or DT_CALCRECT);
                end;
        ...
        begin
          ...
          begin
            ...
            if (Layout in [blGlyphTop, blGlyphTopAdjusted, blGlyphBottom]) then
              rectf.Y := rectf.Y + imgH;
          end;
  ...
  begin
    ...
    if DropDownButton then
    begin
      if (DrawDwLine or AGBDrawSplitLine) and DropDownSplit then
      begin
...
begin
  ...
  begin
    ...
    DownPicture.Assign((Source as TAdvCustomGlowButton).DownPicture);
    ...
    Down := (Source as TAdvCustomGlowButton).Down;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      ddDown, ddUp: pt := Point(Left, Top + Height);
...
begin
  ...
  if not Assigned(FTimer) and (GlowSpeed > 0) and (GlowState = gsPush) then
  begin
    FTimer := TTimer.Create(self);
    FTimer.OnTimer := TimerProc;
    FTimer.Interval := GlowSpeed;
...
begin
  ...
  //            PenColor := BlendColor(BorderColorDown, BorderColorChecked, FStepPush);
      ...
      begin
        ...
        if (State = absDown) or ((State = absExclusive) and Down and (GroupIndex > 0)) and not DownPicture.Empty then
          Pic := DownPicture
        else
          Pic := Picture;
      end
      ...
      begin
        ...
        begin
          ...
          //if MaxButtonSizeState = bsLarge then
            if (H <> Height) then
              Height := H;
        end;
      ...
      sz := DrawVistaButton(Self, Canvas, drrect, GradColor, GradColorTo, GradColorMirror, GradColorMirrorTo,
        ...
        False, FRounded, FDropDownDirection, FSpacing, FTrimming, FNotes, FNotesFont, FDownChecked, FDroppedDown, TranspHover and Transparent, InternalIsOnGlassQAT, MarginHorz,MarginVert,Appearance.NoBorderDefault, AlwaysShowAccel);
...
procedure TAdvCustomGlowButton.SetDownPicture(const Value: TGDIPPicture);
begin
...
procedure TAdvCustomGlowButton.SetIsOnRibbon(const Value: boolean);
begin
   FIsOnRibbon := Value;
end;
...
procedure TAdvCustomGlowButton.CMDesignHitTest(var Msg: TCMDesignHitTest);
begin
  if FDesignClicks then
    Msg.Result := 1
  else
    inherited;
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if IsMenuButton or (Assigned(DropDownMenu)) and not DropDownSplit then
              DoDropDown
            else
              Click;
            Result := 1;
...
procedure TAdvCustomGlowButton.SetDrawSplitLineAlways(const Value: Boolean);
begin
  FDrawSplitLineAlways := Value;
  AGBDrawSplitLine := Value;
end;
...
begin
  ...
  with Appearance do
    Result := DrawVistaButton(Self, Canvas,ClientRect,FColor, FColorTo, FColorMirror, FColorMirrorTo,
        ...
        FOverlappedText, FWordWrap, True, FRounded, FDropDownDirection, FSpacing, FTrimming, FNotes, FNotesFont, FDownChecked, FDroppedDown, TranspHover and Transparent, InternalIsOnGlassQAT,MarginHorz,MarginVert,NoBorderDefault, AlwaysShowAccel);

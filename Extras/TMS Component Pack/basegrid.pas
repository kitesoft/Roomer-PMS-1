...
uses
  Windows, SysUtils, Types, Classes, Graphics, Controls,
  Grids, AdvObj
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
...
type
  ...
  THoverRowCell = (hcNormal, hcFixed, hcSelected, hcGroupHeader, hcGroupSummary);
  ...
  protected
    ...
    function IsNode(ARow: Integer): Boolean; virtual; abstract;
    function IsSummary(ARow: Integer): Boolean; virtual; abstract;
    ...
    function IsCheckCell(c, r: Integer): boolean; virtual; abstract;
    procedure SetAsCheck(c, r: Integer; const v: string); virtual; abstract;
...
begin
  ...
  if HasNodes then
  begin
    if not (hcGroupHeader in HoverRowCells) then
    begin
      if IsNode(arow) then
        dofix := true;
    end;

    if not (hcGroupSummary in HoverRowCells) then
    begin
      if IsSummary(arow) then
        dofix := true;
    end;
  end;
  ...
  begin
    ACanvas.MoveTo(ARect.Left - GLW ,ARect.Bottom - GLW);
    ACanvas.LineTo(ARect.Right - GLW ,ARect.Bottom - GLW);
  end;
...
function TBaseGrid.GetDefaultAlignment: TAlignment;
begin
...
begin
  ...
  begin
    ...
    if (cp.CellSpanX <> -1) and (cp.CellSpanY <> -1) then
    begin
...
begin
  ...
  cp.FFontHeight := FDefFont.Height;
...
begin
  ...
  begin
    ...
    FFontHeight := TCellProperties(Source).FontSize;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if Grid.IsCheckCell(Col, Row) then
          Grid.SetAsCheck(Col ,Row, OrigValue)
        else
          Grid.Cells[Col, Row] := OrigValue;

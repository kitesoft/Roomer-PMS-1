...
{$IFNDEF FMXLIB}
  ...
  try
    if Pic.Width = 0 then
      Pic.GetImageSizes;
    bmp.Width := PictureWidth;
    bmp.Height := PictureHeight;
...
begin
  ...
  begin
    ...
    if (i > 2) and gotnum and ((ch = 'g') or (ch = 'G')) then
    begin
      len := i;
      Break;
    end;
...
//            else
            ...
            if  (SubStr(su,1) = 'u') and IsUnicodeChar(su,unilen) then
            begin
              numuni := FDefUC;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (FRTFObjects[i + 1].RTFType <> 'pict') then
        begin
          FPushAttr := FAttr;
...
begin
  ...
  begin
    ...
    if ((state = psAttr) or (state = psType) or (state = psData)) and (ch = '}') and (chp <> '\') then
    begin
      dec(FParseDepth);
      Result := psNone;
      Exit;

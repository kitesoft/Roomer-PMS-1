...
const
  ...
  // 1.6.1.3 : Fixed : Issue with HighDPI height handling
  // 1.6.1.4 : Fixed : Issue with alignment on psImage panel that also has text
  // 1.6.1.5 : Improved : Vertically centered text for HTML formatted text panels
  // 1.6.1.6 : Improved : High DPI handling
  // 1.6.1.7 : Improved : Multimonitor High DPI handling
  // 1.6.2.0 : New : TAdvOfficeStatusPanel VAlignment property added
  // 1.6.2.1 : Fixed : Issue with OfficeHint handling
...
{ TAdvOfficeStatusBar }
  ...
  TVerticalPanelAlignment = (vpaTop,vpaCenter,vpaBottom);
  ...
  private
    ...
    FAutoSize: Boolean;
    FMinWidth: Integer;
    FVAlignment: TVerticalPanelAlignment;
    ...
    procedure SetVAlignment(const Value: TVerticalPanelAlignment);
  protected
  ...
  published
    ...
    property VAlignment: TVerticalPanelAlignment read FVAlignment write SetVAlignment default vpaCenter;
...
begin
  ...
  begin
    ...
    VAlignment := TAdvOfficeStatusPanel(Source).VAlignment;
  end
  else
...
procedure TAdvOfficeStatusPanel.SetVAlignment(
  const Value: TVerticalPanelAlignment);
begin
...
var
  ...
  r, dr: TRect;
  DTSTYLE: DWORD;
  HyperLinks,MouseLink, tw: Integer;
  Focusanchor: string;
  re: TRect;
  PanelAppearance: TVistaBackGround;
  TxtClr: TColor;
  ...
  case Panel.Style of
    psHTML:
      begin
        ...
        // make sure to vertically center HTML formatted statusbar text
        HTMLDrawEx(Canvas, panel.Text, dr, fImages, 0, 0, -1, -1, 1, true, false, false, false, (fTimerCount > 5), false,
          true, 1.0, URLColor, clNone, clNone, clGray, anchor, stripped, focusanchor, xsize, ysize,
          hyperlinks, mouselink, re, nil , FContainer, 0);

        case panel.VAlignment of
          vpaCenter:
            begin
              if Ysize < (dr.Bottom - dr.Top) then
                dr.top := dr.Top + (dr.Bottom - dr.Top - Ysize) div 2;
            end;
          vpaBottom:
            begin
              if Ysize < (dr.Bottom - dr.Top) then
                dr.top := dr.Bottom - Ysize;
            end;
        end;
    ...
    psEllipsText:
      begin
        DTSTYLE := DT_SINGLELINE or DT_END_ELLIPSIS or DT_NOPREFIX;

        case panel.VAlignment of
        vpaTop: DTSTYLE := DTSTYLE or DT_TOP;
        vpaCenter: DTSTYLE := DTSTYLE or DT_VCENTER;
        vpaBottom: DTSTYLE := DTSTYLE or DT_BOTTOM;
        end;
    ...
    psFileEllipsText:
      begin
        DTSTYLE := DT_SINGLELINE or DT_PATH_ELLIPSIS or DT_NOPREFIX;
    ...
    psImage:
      begin
        ...
        begin
          tw := 0;
          if Panel.Text <> '' then
            tw := Canvas.TextWidth(Panel.Text) + 4;

          case al of
          taLeftJustify: r.Left := r.Left + 2;
          taRightJustify: r.Left := r.Right - Images.Width - tw - 2;
          taCenter: r.Left := r.Left + (r.Right - r.Left - Images.Width -  tw) div 2;
          end;
...
begin
  ...
  if Assigned(frm) and (frm is TForm) and (frm as TForm).Scaled and not (csLoading in ComponentState) then
  begin
    //Height := Round(FDTHeight * CalculateDPIScale(True, Canvas.Handle));
    for I := 0 to Panels.Count - 1 do                                       
    begin                                                                  
      Panels[I].Width:= round(Panels[I].Width * M / D);                    
    end;                                                                   
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          FDummyHintControl.OfficeHint.Assign(Panels[idx].OfficeHint);
          FDummyHintControl.ShowHint := true;
          hi^.HintControl := FDummyHintControl;
        end;
      end
      else
...
begin
  ...
  begin                                                                      
{$IFNDEF DELPHI10_LVL}                                                  
    fScale := GetDeviceCaps(Canvas.Handle, LOGPIXELSX)/96;                   
{$ENDIF}    
{$IFDEF DELPHI10_LVL}                                                          
    fScale := TForm(frm).Monitor.PixelsPerInch/96;                           
{$ENDIF}
    Height := Round(FDTHeight * fScale);                                     
    for I := 0 to Panels.Count - 1 do                                        
    begin                                                                    
      Panels[I].Width:= round(Panels[I].Width * fScale);                     
    end;                                                                     
  end;  
                                                                     
  if not odd(Height) then
    Height := Height + 1;

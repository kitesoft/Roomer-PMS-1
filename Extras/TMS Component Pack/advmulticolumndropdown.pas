...
begin
  ...
  begin
    ...
    if (ItemIndex + 1 >= FAdvColGrid.FixedRows) and (FAdvColGrid.RowCount > ItemIndex + 1) then
...
procedure TAdvMultiColumnDropDown.DoSelect;
begin
...
begin
  ...
  begin
    if (ItemIndex + 1 > 0) and (FAdvColGrid.RowCount > ItemIndex + 1) then
      FAdvColGrid.Row := ItemIndex + 1
    else
      FAdvColGrid.Row := 1;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FAdvColGrid.Row := i + FAdvColGrid.FixedRows;
...
begin
  ...
  if ((Key in [VK_UP, VK_DOWN]) and (GetKeyState(VK_MENU) and $8000 = $8000)) then
  begin
...
begin
  ...
  begin
    ...
    s := columns[aCol].Header;

    //measure header when it exists
    if (s <> '') then
    begin
...
begin
  ...
  begin
    ...
    RowHeights[0] := MaxI(1,AdvDropDown.HeaderHeight);
  end;
end;

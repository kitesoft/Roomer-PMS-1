...
uses
  Classes, Windows, Types, Graphics, Controls, Messages, ExtCtrls, SysUtils, ImgList,
...
const
  ...
  // v1.4.1.2 : Fixed : Issue with TAdvColorPickerDropdown & use of HTML tags with colors
  // v1.4.1.3 : Fixed : Issue with OnChange when deleting all text
  //          : Fixed : Issue with Ctrl-A handling
  // v1.4.2.0 : Improved : High DPI support
  // v1.4.2.1 : Improved : Made mousewheel handling when dropdown is closed consistent with TComboBox
  // v1.4.2.2 : Improved : Adapt dropdown position when at right side of active monitor
  // v1.4.2.3 : Fixed : Issue with VCL styles and dropdown
  // v1.4.2.4 : Fixed : Issue with empty multicolumn dropdown
  // v1.4.2.5 : Fixed : Issue with retrieving changed dropdown size
  // v1.4.2.6 : Fixed : Issue with repeatedly changing the DropDown.Control
  // v1.4.2.7 : Fixed : Issue with Cursor when EditorEnabled = false
  // v1.4.2.8 : Improved : Handling OnSelect() event in TAdvMultiColumnDropDown
  // v1.4.2.9 : Fixed : Issue with updating footer & header while dropdown is being displayed
...
type
  ...
  published
    property Color: TColor read FColor write FColor default clSilver;
    property ColorTo: TColor read FColorTo write FColorTo default clWhite;
    property Caption: string read FCaption write SetCaption; //(supporting HTML formatted text)
    property Font: TFont read FFont write SetFont;
    property GradientDirection: TGradientDirection read FGradientDirection write FGradientDirection default gdVertical;
    property Visible: Boolean read FVisible write SetVisible;
  ...
  protected
    ...
    property EmptyTextFocused: boolean read FEmptyTextFocused write FEmptyTextFocused;
...
function TAdvCustomDropDown.GetMinValue: Integer;
begin
  Result := 0;
end;
  ...
  *)
  ...
  case FDropPosition of
    ...
    begin
      ...
      begin //Up
        FdropDownForm.Left := P.x - delta;
        FdropDownForm.Top := p.y - FDropDownForm.Height + delta;
      end
      else
      begin //Down
        FDropDownForm.Left := P.x - delta;
        FDropDownForm.Top := p.y + Height - 1 + delta;
      end;
    end;
    dpDown:
    begin
  ...
  if (R.Left >= 0) and (R.Right < FDropDownForm.Left + FDropDownForm.Width) and (R.Right > FDropDownForm.Left) then
    FDropDownForm.Left := R.Right - FDropDownForm.Width;
  ...
  begin
    ...
    if Assigned(FControl) and (FControl is TWinControl) then
       Windows.SetFocus((FControl as TWinControl).Handle);
...
begin
  ...
  if IsCtrl and (msg.CharCode = 65) then
  begin
    SelectAll;
  end;
...
begin
  ...
  if Assigned(FDropDownForm) and DropDownSizeable then
  begin
...
begin
  ...
  if not IsCtrl and (SelLength = length(Text)) and (Text <> '') then
    FBlockChange := true;
...
procedure TAdvCustomDropDown.WMSetCursor(var Msg: TWMSetCursor);
begin
  if not EditorEnabled then
...
begin
  if (FControl <> Value) and (Value <> Self) then
  begin
    // when there was a control previously assigned, make sure it isn't visible
    if Assigned(FControl) then
    begin
...
begin
  ...
  begin
    ...
    case FSelectionColorStyle of
      ...
      scWindows7:
      begin
        FSelectionColor := $FCEBDC;
        FSelectionColorTo := $FCDBC1;
      end;
      scOffice2007:
      begin
        FSelectionColor := $D7FFFD;
        FSelectionColorTo := $58D4FC;
      end;
    end;
  end;
end;
...
procedure TAdvCustomDropDown.SetEmptyText(const Value: string);
begin
  FEmptyText := Value;
  Invalidate;
end;
...
procedure TAdvCustomDropDown.SetEmptyTextStyle(const Value: TFontStyles);
begin
...
begin
  ...
  if Assigned(FOnClipboardCopy) and not FBlockCopy then
    FOnClipboardCopy(self, Copy(Text, SelStart + 1 - Length(fPrefix), SelLength), Allow);
  FBlockCopy := False;
...
procedure TAdvCustomDropDown.PaintEdit;
var
  DC: HDC;
  Canvas: TCanvas;
  {$IFDEF DELPHIXE2_LVL}
...
begin
  ...
  begin
    FDropDownForm.InitializeAndUpdate;
    FDropDownForm.Footer.Invalidate;
  end;
end;
...
begin
  ...
  begin
    FDropDownForm.InitializeAndUpdate;
    FDropDownForm.Header.Invalidate;
  end;
end;
...
procedure TFooterAppearance.SetCaption(const Value: string);
begin
  FCaption := Value;
  Changed;
end;

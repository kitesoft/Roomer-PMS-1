...
const
  ...
  // v1.5.0.2 : Fixed: Issue with double-click showing TAdvSmoothTileList from another form
  // v1.5.1.0 : New: Header and Footer HTML support
...
begin
  ...
  begin
    ...
    if (FTilePos = 0) and (FMode = tmView) and FMouseDown then
    begin
...
begin
  ...
  begin
    ...
    begin
      if (AnsiPos('</', Caption) > 0) or (AnsiPos('/>', Caption)  > 0) or (AnsiPos('<BR>', UpperCase(Caption)) > 0) then
      begin
        htmlr := Bounds(Round(r.X), Round(r.Y), Round(r.Width), Round(r.Height));

        HTMLDrawGDIP(g, FFont, Caption,htmlr,FOwner.ImageList, 0,0,-1,-1,0,False,true,false,false,
          False,False,true,1.0,clBlue,clNone,clNone,clGray,a,ms,k,XSize,YSize,l,m,hr,nil,FOwner.PictureContainer,2);

        xsize := xsize + 2;
        ysize := ysize + 2;

        if CaptionPosition <> tpCustom then
          GetTextPosition(x, y, r, XSize, YSize, CaptionPosition)
        else
        begin
          x := CaptionLeft;
          y := CaptionTop;
        ...
        HTMLDrawGDIP(g, FFont, Caption,htmlr,FOwner.ImageList, 0,0,-1,-1,0,False,false,false,false,
          False,False,true,1.0,clBlue,clNone,clNone,clGray,a,ms,k,XSize,YSize,l,m,hr,nil,FOwner.PictureContainer,2);
      end
      else
      begin

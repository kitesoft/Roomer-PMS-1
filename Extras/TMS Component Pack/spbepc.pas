...
const
  ...
  // v1.1.0.0  : New : ChartType ctEPC2010EEI added
  //           : New : property RangeVisible added


type
  TSPBEPCChartType = (ctEPC, ctEIC, ctEPC2010EEI);
  ...
  private
    ...
    procedure SetRangeVisible(const Value: Boolean);
    property OnResize;
  ...
  published
    ...
    property RangeVisible: Boolean read FRangeVisible write SetRangeVisible default true;
...
begin
  case ChartType of
    ctEPC, ctEPC2010EEI: Result := EPC_A_Color;
...
begin
  case ChartType of
    ctEPC, ctEPC2010EEI: Result := EPC_B_Color;
...
begin
  case ChartType of
    ctEPC, ctEPC2010EEI: Result := EPC_C_Color;
...
begin
  case ChartType of
    ctEPC, ctEPC2010EEI: Result := EPC_D_Color;
...
begin
  case ChartType of
    ctEPC, ctEPC2010EEI: Result := EPC_E_Color;
...
begin
  case ChartType of
    ctEPC, ctEPC2010EEI: Result := EPC_F_Color;
...
begin
  case ChartType of
    ctEPC, ctEPC2010EEI: Result := EPC_G_Color;
...
var
  ...
  function GetNextRange1(Value: Integer; GoForward: Boolean): Integer;
  begin
    Result := Value;
    case Value of
      0..20  : begin
  ...
  function GetNextRange2(Value: Integer; GoForward: Boolean): Integer;
  begin
    Result := Value;
    case Value of
      0..9  : begin
...
begin
  ...
  if ChartType = ctEPC2010EEI then
    delta := -1
  else
    delta := 1;
  ...
  case key of
    ...
    begin
      ...
      begin
        Potential := Min(100, Potential + delta);
      end;
    end;
    VK_DOWN:
    begin
      ...
      if (SelectedCol = 3) and PotentialVisible and not ReadOnly then
      begin
        Potential := Max(0, Potential - delta);
      end;
    end;
    VK_PRIOR:
    begin
      ...
      begin
        if ChartType = ctEPC2010EEI then
          Current := Min(100, GetNextRange2(Current, True))
        else
          Current := Min(100, GetNextRange1(Current, True));
      end;

      if (SelectedCol = 3) and PotentialVisible and not ReadOnly then
      begin
        if ChartType = ctEPC2010EEI then
          Potential := Min(100, GetNextRange2(Potential, True))
        else
          Potential := Min(100, GetNextRange1(Potential, True));
      end;
    end;
    VK_NEXT:
    begin
      ...
      begin
        if ChartType = ctEPC2010EEI then
          Current := Max(0, GetNextRange2(Current, False))
        else
          Current := Max(0, GetNextRange1(Current, False));
      end;

      if (SelectedCol = 3) and PotentialVisible and not ReadOnly then
      begin
        if ChartType = ctEPC2010EEI then
          Potential := Max(0, GetNextRange2(Potential, False))
        else
          Potential := Max(0, GetNextRange1(Potential, False));
      end;
    end;
    VK_HOME:
    begin
...
begin
  if FBottomCaption = '' then
    case ChartType of
      ctEPC, ctEPC2010EEI: Result := 'Not energy efficient - higher running costs';
...
begin
  ...
  begin
    case Value of
      60..1000 : Result := G_Color;
      42..59 : Result := F_Color;
      30..41 : Result := E_Color;
      23..29 : Result := D_Color;
      16..22 : Result := C_Color;
      10..15 : Result := B_Color;
      0..9: Result := A_Color;
      else Result := clBlack;
    end;
  end
...
begin
  if FTitle = '' then
    case ChartType of
      ctEPC, ctEPC2010EEI: Result := 'Energy Efficiency Rating';
...
begin
  if FTopCaption = '' then
    case ChartType of
      ctEPC, ctEPC2010EEI: Result := 'Very energy efficient - lower running costs';
...
begin
  ...
  if ChartType = ctEPC2010EEI then
    Ratio := Trunc((BaseLocationDetails.StairSize - PenWidth) / (Higher - Lower) * (Value - Lower))
  else
...
{$ENDIF}
  ...
  if ChartType = ctEPC2010EEI then
    RangeRect.Right := Trunc(Rect.Right * 0.33)
  else
  ...
  for I := 1 to 7 do begin
    ...
    if ChartType = ctEPC2010EEI then
      BaseImage.Canvas.Font.Height := BaseImage.Canvas.Font.Height - 2;
...
procedure TSPBEPC.SetRangeVisible(const Value: Boolean);
begin
  if (FRangeVisible <> Value) then
  begin
    FRangeVisible := Value;
    RefreshBaseImage;
    Refresh;

...
const
  ...
  // 2.6.1.2 : Fixed : Drawing of notes in sharper text
  // 2.6.1.3 : Fixed : Issue with menu drawing in some specific RDP clients
  // 2.6.1.4 : Fixed : Regression with TAdvPopupMenu
  // 2.6.2.0 : New : ActionImages property added to select type of images for TActionManager associated menu items
  // 2.6.2.1 : Fixed : Rare issue with drawing in popup menu with scrolling items
  // 2.6.3.0 : Improved : High DPI support
  // 2.6.4.0 : Improved : High DPI support when not using system fonts
...
type
  ...
  IAdvMenu = interface
    ...
    procedure SetActionImage(const Value: TActionImage);
    function GetActionImage: TActionImage;
    { Properties }
    { Protected }
    property Self: TMenu read GetSelf;
    property CurrentMenuStyler: TCustomAdvMenuStyler read GetCurrentMenuStyler;
    { Public }
    property ActionImages: TActionImage read GetActionImage write SetActionImage;
  ...
  public
    ...
    procedure Refresh;
  published
    property ActionImages: TActionImage read GetActionImage write SetActionImage default aiNormal;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          ACanvas.Font.Height := Round(ACanvas.Font.Height * GetDPIScale);
        end;
      end
      else
      begin
...
begin
  ...
  if Assigned(MenuItem.Owner) and (MenuItem.Owner is TPopupMenu) then
  begin
    i :=  (MenuItem.Owner as TPopupMenu).Items.IndexOf(MenuItem);
    if (i = (MenuItem.Owner as TPopupMenu).Items.Count - 1) then
    begin
       delta := (ARect.Bottom - ARect.Top);
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      // patch for last item painting on scrolling menu
      SmallItemRect.Bottom := SmallItemRect.Bottom + delta;
      SmallMenuRect.Bottom := SmallMenuRect.Bottom + delta;
  ...
  ARect.Bottom := ARect.Bottom - delta;
...
begin
  ...
  begin
    ...
    if SystemParametersInfo(SPI_GETKEYBOARDCUES, 0, @ShowAccel, 0) then
    begin
      if not ShowAccel and (GetKeyState(VK_MENU) and $8000 = 0) then
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Font.Height := Round(Font.Height * GetDPIScale);
        end;
      ...
      *)
      ...
      begin
        ...
        begin
          ...
          s := copy(Text, 1, pos('\n', Text) - 1);
          DrawText(ACanvas.Handle, PChar(s), Length(s), Rect, Flags);
          s := Text;
          dr := Rect;
...
var
  ...
  begin
    ...
    begin
      ...
      if TopLevel then
        ...
        begin
          ...
          begin
            ...
            if IsInTopMainMenu(MenuItem) and IsRightmost(MenuItem) and Assigned(AdvMenu) then
            begin
              Rl := FilledRect;
              Rs := SaveRect;
              OwnerForm := nil;
      ...
      HaveActionImage := false;

      if AdvMenu.ActionImages = aiNormal then
      begin
        HaveActionImage := Assigned(MenuItem.Action) and
          ...
          (ImageIndex <> -1) and (ImageIndex < (MenuItem.Action as TCustomAction).ActionList.Images.Count);
      end
      else
      begin
{$IFDEF DELPHIXE_LVL}
        if ((MenuItem.Action as TCustomAction).ActionList is TActionManager) then
        HaveActionImage := Assigned(MenuItem.Action) and
          (MenuItem.Action is TCustomAction) and Assigned
          ((MenuItem.Action as TCustomAction).ActionList) and Assigned
          (((MenuItem.Action as TCustomAction).ActionList as TActionManager).LargeImages) and
          (ImageIndex <> -1) and (ImageIndex < ((MenuItem.Action as TCustomAction).ActionList as TActionManager).LargeImages.Count);
{$ENDIF}
      end;
...
{$IFDEF DELPHI_UNICODE}
          ...
          begin
            if AdvMenu.ActionImages = aiNormal then
            begin
              if not Enabled and Assigned(((MenuItem.Action as TCustomAction).ActionList as TActionManager).DisabledImages) then
              begin
                drwenabled := True;
            ...
            begin
              {$IFDEF DELPHIXE_LVL}
              if not Enabled and Assigned(((MenuItem.Action as TCustomAction).ActionList as TActionManager).LargeDisabledImages) then
              begin
                drwenabled := True;
                ImgList := ((MenuItem.Action as TCustomAction).ActionList as TActionManager).LargeDisabledImages;
              end
              else
              begin
                if not Enabled and Assigned(DisabledImageList) then
                  ...
                  ImgList := ((MenuItem.Action as TCustomAction).ActionList as TActionManager).LargeImages;
              end;
              {$ENDIF}
            end;
          end
...
{$ENDIF}
      ...
      SaveRect := ARect;
      if (pos('\n',Caption) = 0) then // multiline is always top aligned
...
begin
  ...
  if IsInTopMenu(MenuItem) then
  begin
    // Don't use this function with top level items
    Result := True
  end
  else
  begin
...
procedure TurnOffMenuAnimation;
begin
  SystemParametersInfo(SPI_SETMENUANIMATION, 0, TObject(0), SPIF_SENDCHANGE);
end;
...
procedure TAdvPopupMenu.SetActionImage(const Value: TActionImage);
begin
  FActionImage := Value;
end;
...
begin
  ...
  // Disable menu animation in case it was reset
  TurnOffMenuAnimation;
  inherited Popup(X, Y);
...
procedure TAdvMainMenu.SetActionImage(const Value: TActionImage);
begin
  FActionImage := Value;
end;
...
begin
  ...
  level := GetMenuItemLevel(MI);

  // forced turn off menu animations again in case it was reset by an RDP session
  if (level = 0) and (self.items.IndexOf(mi) = 0) then
    TurnOffMenuAnimation;
...
function TAdvMainMenu.GetActionImage: TActionImage;
begin
  Result := FActionImage;
end;
...
function TAdvPopupMenu.GetActionImage: TActionImage;
begin
  Result := FActionImage;
end;

...
const
  ...
  BLD_VER = 0; // Build nr.
  DATE_VER = 'Aug, 2016'; // Month version
  ...
  // v3.4.3.1 : Fixed : Header item DB key initialization
  // v3.4.3.2 : Improved : Behavior with time handling when mode is day period
  // v3.4.4.0 : New : RAD Studio 10.1 Berlin Support
  // v3.4.4.1 : Fixed : Issue with DBDisjunctDaySource & multiresource handling
  // v3.4.4.2 : Fixed : Issue in TDBDaySource.ReadDBItem when time range < 24hrs is shown in Planner for specific events.
  // v3.4.5.0 : New : Upgraded ResourceIndex in ResourceMap to Int64 for allowing a larger range of resource indexes
...
type
  ...
  published
    property ResourceIndex: Int64 read FResourceIndex write FResourceIndex;
...
begin
  ...
  begin
    ...
    FRecurrencyHandler.StartTime := dts;
    FRecurrencyHandler.EndTime := dte;
    FRecurrencyHandler.TimeSpan := FDay + Span + ZoneStart + 1;
    FRecurrencyHandler.Generate;
    ...
    if (ResourceField <> '') and
       (Mode in [dmMultiResource,dmMultiDayRes,dmMultiResDay]) and
       (dte >= FDay + ZoneStart) and (dts < FDay + Span + ZoneEnd) then
    begin
      FieldsToRes(D.Fields,ResourcePos, Accept);
    end;
        ...
        begin
          ...
          begin
            ...
            begin
              DatePos := DatePos + 1;
              ItemPos := CalcItemPos(DatePos,ResourcePos);
              DBKey := FPlanner.Items.DBItem.DBKey;
            end;
          end;
        end;
      end;
...
begin
  ...
  begin
    ...
    FPlanner.Items.DBItem.RealTime := true;
    DTS := Int(DTS) + Frac(FPlanner.Items.DBItem.ItemRealStartTime);
    DTE := Int(DTE) + Frac(FPlanner.Items.DBItem.ItemRealEndTime);
    FPlanner.Items.DBItem.RealTime := False;
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      {$IFDEF DELPHIXE_LVL}
        id := D.FieldByName(FDBItemSource.ResourceDataSource.ResourceIDField).AsLargeInt;
      {$ENDIF}
      {$IFNDEF DELPHIXE_LVL}

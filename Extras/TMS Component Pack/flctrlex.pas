...
uses
  Windows, Types, Messages, SysUtils, Classes, Controls, Graphics, Forms,
  Menus, StdCtrls, FileCtrl, ShellApi, CommCtrl
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes, AdvShell
  {$ENDIF}
  ;
...
const
  ...
  // v1.3.0.0 : New : Optional System popup menu builtin for FileListBoxEx, CheckFileListBoxEx
...
begin
  ...
  if (X < 16) and (Button = mbLeft) then
  begin
  ...
  begin
    idx := SendMessage(Handle, LB_ITEMFROMPOINT,0, MakeLong(X,Y));

    ItemIndex := idx;

    if idx >= 0 then
    begin
      fn := Directory +'\'+Items[idx];
      ShowSysContextMenu(fn, ClientToScreen(Point(X,Y)));
    end;
  end;
  {$ENDIF}

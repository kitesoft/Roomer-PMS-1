...
const
  ...
  // v2.4.2.0 : Improved : Property setter handling
  // v2.5.0.0 : New : Support for VCL styles added
  // v2.5.1.0 : New : Support for control of VCL styles via StyleElements added
  // v2.5.2.0 : New : TAdvPanelGroup.PanelClass public property added to control types of panels created
  // v2.5.2.1 : Fixed : Caption background when seClient not in StyleElements
  // v2.5.2.2 : Fixed : Statusbar caption topindent wrt statusbar border issue
  // v2.5.3.0 : New : Property BottomIndent added
  // v2.5.4.0 : Improved : High DPI support
  // v2.5.4.1 : Improved : Panel caption gradient background drawing
  // v2.5.4.2 : Fixed : Issue with caption height on High DPI
  // v2.5.5.0 : Improved : Panel caption button display in High DPI mode
  // v2.5.5.1 : Fixed : Issue with windowed child controls repaint when using BeginUpdate/EndUpdate
...
type
  ...
  published
    ...
    property Indent: Integer read FIndent write SetIndent stored true;
  ...
  protected
    ...
    property UsedVCLStyles: boolean read FUsedVCLStyles write FUsedVCLStyles;
  public
    { Public declarations }
  ...
  // properties to make published
    ...
    property BottomIndent: Integer read FBottomIndent write SetBottomIndent default 0;
...
//    property OnKeyUp;
  ...
  TPanelClass = class of TCustomAdvPanel;
  ...
  public
    ...
    function AddPanel: TCustomAdvPanel; virtual;
    function AppendPanel: TCustomAdvPanel; virtual;
    function InsertPanel(Index: Integer): TCustomAdvPanel; virtual;
    procedure RemovePanel(Index: Integer);
    procedure MovePanel(FromIndex,ToIndex: Integer);
    property PanelClass: TPanelClass read FPanelClass write FPanelClass;
...
begin
  ...
  begin
    ...
    GW := Round(12 * CalcDPIScale(Canvas.Handle, True));
...
var
  a,s,fa: string;
  xsize,ysize: Integer;
  r,hr,cr,dr,capr: TRect;
  xo,yo,hl,ml, GW: Integer;
  hrgn: THandle;
...
begin
  ...
  if se and LStyle.Enabled and (LStyle.Name <> 'Windows') then
  begin
  ...
  begin
    ...
    begin
      if ColorMirror <> clNone then
        ...
        DrawGradient(ACanvas, Color, ColorTo, 128, r, GradientDirection = gdHorizontal)
    end
    else
    begin
  ...
  begin
    if FCaption.Position = cpTop then
      R.Top := R.Top + FCapHeight - 2 * BevelWidth
    else if FCaption.Position = cpLeft then
      R.Left := R.Left + 2 + FCapHeight - 2 * BevelWidth
    else
      R.Right := R.Right - 2 - FCapHeight + 2 * BevelWidth;
  end;
...
//  InflateRect(r,-1,-1);
  ...
  r.Bottom := r.Bottom - BottomIndent;
  ...
  begin
    ...
    if FCaption.Position = cpTop then
      capr := Rect(r.Left, r.Top, r.Right, r.Top + FCapHeight - 2 * BevelWidth);

    if FCaption.Position = cpLeft then
      capr := Rect(r.Left, r.Top, r.Left + FCapHeight - 2 * BevelWidth, r.Bottom - shadebdr);

    if FCaption.Position = cpRight then
      capr := Rect(r.Right - FCapHeight, r.Top, r.Right, r.Bottom - shadebdr);

    {$IFDEF DELPHIXE2_LVL}
    if LStyle.Enabled and (LStyle.Name <> 'Windows') and se then
    begin
      if not FCollaps then
        LDetails := LStyle.GetElementDetails(tgFixedCellNormal)
      else
        LDetails := LStyle.GetElementDetails(tgFixedCellHot);

      LStyle.DrawElement(ACanvas.Handle, LDetails, capr);
    ...
    begin
      ...
      begin
        ...
        begin
          if FCaption.Position = cpTop then
          begin
            DrawGradient(ACanvas,FCaption.Color,FCaption.ColorTo, 128, capr, Caption.GradientDirection = gdHorizontal)
          end
          else if FCaption.Position = cpLeft then
          begin
            DrawGradient(ACanvas,FCaption.Color,FCaption.ColorTo, 128, capr, Caption.GradientDirection = gdVertical)
          end
          else // FCaption.Position = cpRight
          begin
            DrawGradient(ACanvas,FCaption.ColorTo,FCaption.Color, 128, capr, Caption.GradientDirection = gdVertical)
          end;
        end
        else
        begin
        ...
        DrawCaptionBkg(ACanvas, capr);
    end;

    GW := Round(14 * CalcDPIScale(ACanvas.Handle, True));
      ...
      begin
        ...
        begin
          ...
          begin
            if FCaption.Position = cpTop then
            begin
              ACanvas.MoveTo(r.Right - GW + 4,r.Top + 4);
              ACanvas.LineTo(r.Right - 4,r.Top + GW - 4);
              ACanvas.MoveTo(r.Right - 4,r.Top + 4);
              ACanvas.LineTo(r.Right - GW + 4,r.Top + GW - 4);
              r.Right := r.Right - GW;
            end
            else if FCaption.Position = cpLeft then
            begin
              ACanvas.MoveTo(r.Left + 4,r.Top + 4);
              ACanvas.LineTo(r.Left + GW - 4,r.Top + GW -4);
              ACanvas.MoveTo(r.Left + GW - 4,r.Top + 4);
              ACanvas.LineTo(r.Left + 4,r.Top + GW - 4);
              r.Top := r.Top + GW - 2;
            end
            else // FCaption.Position = cpRight
            begin
              ACanvas.MoveTo(r.Right - 4,r.Bottom - GW + 2);
              ACanvas.LineTo(r.Right - GW + 4,r.Bottom - (GW - 2) div 2);
              ACanvas.MoveTo(r.Right - GW + 4,r.Bottom - GW + 2);
              ACanvas.LineTo(r.Right - 4,r.Bottom - (GW - 2) div 2);
              r.Bottom := r.Bottom - GW;
            end;
          end
          else
          begin
            ...
            begin
              ...
              ACanvas.MoveTo(r.Left + 4,r.Bottom - 4);
              ACanvas.LineTo(r.Left + GW - 4,r.Bottom - GW + 4);
              ACanvas.MoveTo(r.Left + GW - 4,r.Bottom - 4);
              ACanvas.LineTo(r.Left + 4,r.Bottom - GW + 4);
              r.Bottom := r.Bottom - GW + 2;
            end
            else // FCaption.Position = cpRight
            begin
              ACanvas.MoveTo(r.Right - GW + 4,r.Top + 4);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if FCaption.ButtonPosition = cbpRight then
              ACanvas.Polygon([Point(r.Right- GW + 4,r.Top + (GW - 4) div 2), Point(r.Right-4,r.Top+(GW -4) div 2),point(r.Right - (GW div 2),r.Top + 3*(GW div 4))])
            else
              ACanvas.Polygon([Point(r.Left + 4,r.Top + (GW - 4) div 2), Point(r.Left + GW - 4,r.Top + (GW - 4) div 2),point(r.Left + (GW div 2),r.Top + 3*(GW div 4))]);
          end
          else if FCaption.Position = cpLeft then
          begin
            if FCaption.ButtonPosition = cbpRight then
              ACanvas.Polygon([Point(r.Left + (GW -4) div 2, r.Top + GW - 4),point(r.Left + (GW -4) div 2, r.Top + 4),Point(r.Left + 3 * (GW div 4), r.Top + GW div 2)])
            else
              ACanvas.Polygon([Point(r.Left + (GW -4) div 2, r.Bottom - 4),point(r.Left + (GW -4) div 2, r.Bottom - GW + 4),Point(r.Left + 3 * (GW div 4), r.Bottom - GW div 2)]);
          end
          else // FCaption.Position = cpRight
          begin
            if FCaption.ButtonPosition = cbpRight then
              ACanvas.Polygon([Point(r.Right - GW + 3 * (GW div 4), r.Bottom - 4),point(r.Right - GW + 3 * (GW div 4), r.Bottom - GW + 4),Point(r.Right - GW + (GW - 4) div 2, r.Bottom - GW div 2)])
            else
              ACanvas.Polygon([Point(R.Right - GW + 3 * (GW div 4), r.Top + GW - 4),point(r.Right - GW + 3 * (GW div 4), r.Top + 4),Point(r.Right - GW + (GW - 4) div 2, r.Top + GW div 2)])
          end;
        end
        else
        begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if FCaption.ButtonPosition = cbpRight then
              ACanvas.Polygon([Point(r.Right - GW + 4,r.Top + 3 * (GW div 4)),point(r.Right-4, r.Top + 3 * (GW div 4)),Point(r.Right- (GW div 2),r.Top + (GW - 4) div 2)])
            else
              ACanvas.Polygon([Point(r.Left + 4,r.Top + 3 * (GW div 4)),point(r.Left + GW - 4 ,r.Top + 3 * (GW div 4)),Point(r.Left+ (GW div 2),r.Top + (GW - 4) div 2)])
          end
          else if FCaption.Position = cpLeft then
          begin
            if FCaption.ButtonPosition = cbpRight then
              ACanvas.Polygon([Point(r.Left + 3 * (GW div 4), r.Top + GW - 4),point(r.Left + 3 * (GW div 4), r.Top + 4),Point(r.Left + (GW - 4) div 2, r.Top + (GW div 2))])
            else
              ACanvas.Polygon([Point(R.Left + 3 * (GW div 4), r.Bottom - 4),point(r.Left + 3 * (GW div 4), r.Bottom - GW + 4),Point(r.Left + (GW - 4) div 2, r.Bottom - (GW div 2))])
          end
          else // FCaption.Position = cpRight
          begin
            if FCaption.ButtonPosition = cbpRight then
              ACanvas.Polygon([point(r.Right - GW + (GW - 4) div 2, r.Bottom - 4),point(r.Right - GW + (GW - 4) div 2, r.Bottom - GW + 4),point(r.Right - GW + 3 * (GW div 4), r.Bottom - GW + 7)])
            else
              ACanvas.Polygon([point(r.Right - GW + (GW - 4) div 2, r.Top + GW - 4),point(r.Right - GW + (GW - 4) div 2, r.Top + 4),point(r.Right - GW + 3 * (GW div 4), r.Top + GW div 2)]);
          end;
        end
        else
        begin
          ...
          begin
            if FCaption.ButtonPosition = cbpRight then
              ACanvas.Draw(r.Right - GW + 2, r.Bottom - FCaption.MinGlyph.Height,FCaption.MinGlyph)
            else
              ACanvas.Draw(r.Right - GW + 2, r.Top,FCaption.MinGlyph)
          end;
        end;
      end;
    end;
    ...
    if not ParentFont then
      ACanvas.Font.Height := Round(FCaption.Font.Height * FDPIScale);
    ...
    begin
      ...
      begin
        if FCaption.CloseButton then
          r.Bottom := r.Bottom - GW - 6;
        if FCaption.MinMaxButton then
          r.Bottom := r.Bottom - GW - 6;
      end;

      r.Right := r.Left + FCapHeight;
  ...
  begin
    ...
    begin
      LDetails := LStyle.GetElementDetails(tgFixedCellHot);
      LStyle.DrawElement(ACanvas.Handle, LDetails, R2);
    ...
    begin
      ACanvas.Font.Assign(StatusBar.Font);
      ACanvas.Font.Color := txtcol;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if FStatusBar.Visible then
              delta := (Height - FCapHeight - FStatusBar.Height - 2 * BorderWidth) div CollapsSteps
            else
              delta := (Height - FCapHeight - 2 * BorderWidth) div CollapsSteps;
          ...
          if FStatusBar.Visible then
            Height := FCapHeight + FStatusBar.Height - 2 + bdelta * 2
          else
            Height := FCapHeight - 2 + bdelta * 2;
        end
        else if FCaption.Position = cpLeft then
        begin
          FFullHeight := Width;
          ...
          begin
            ...
            if FStatusBar.Visible then
              delta := (Width - FCapHeight - FStatusBar.Height) div CollapsSteps
            else
              delta := (Width - FCapHeight) div CollapsSteps;
          ...
          if FStatusBar.Visible then
            Width := FCapHeight + FStatusBar.Height + bdelta
          else
            Width := FCapHeight + bdelta;
        end
        else // FCaption.Position = cpRight
        begin
          FFullHeight := Width;
      ...
      begin
        ...
        begin
          ...
          if FStatusBar.Visible then
            delta := (FullHeight - FCapHeight - FStatusBar.Height) div CollapsSteps
          else
            delta := (FullHeight - FCapHeight) div CollapsSteps;
...
begin
  //
  if Caption.Visible and (pt.Y < FCapHeight) and not (Caption.OfficeHint.IsEmpty) then
  begin
...
begin
  ...
  if FCaption.Visible and (((y < FCapHeight) and (FCaption.Position = cpTop)) or ((X < FCapHeight) and (FCaption.Position = cpLeft)) or ((X > Width - FCapHeight) and (FCaption.Position = cpRight)) ) then
  begin
    if FCaption.Position = cpTop then
    begin
      r.Bottom := r.Top + FCapHeight;
  ...
  begin
    ...
    begin
      if FCaption.Position = cpTop then
      begin
        r.Top := r.Top + FCapHeight;
      end
      else if FCaption.Position = cpLeft then
        r.Left := r.Left + FCapHeight
      else // FCaption.Position = cpRight
        r.Right := r.Right - FCapHeight;
    end;
...
begin
  ...
  begin
    if ShowHint and not Caption.OfficeHint.IsEmpty and (FPrevY > FCapHeight) and (y < FCapHeight) then
  ...
  if ( ((y < FCapHeight) and (FCaption.Position = cpTop)) or ((X < FCapHeight) and (FCaption.Position = cpLeft)) or ((X > Width - FCapHeight) and (FCaption.Position = cpRight)) ) and FCaption.Visible then
  begin
...
begin
  ...
  if (FCaption.Visible) and (((pt.y < FCapHeight) and (pt.x < r.right) and (pt.X > r.Left) and (FCaption.Position = cpTop))
      or ((pt.x < FCapHeight) and (pt.Y < r.Bottom) and (pt.Y > r.Top) and (FCaption.Position = cpLeft))
      or ((pt.x > Width - FCapHeight) and (pt.Y < r.Bottom) and (pt.Y > r.Top) and (FCaption.Position = cpRight)) )
...
procedure TCustomAdvPanel.SetBottomIndent(const Value: Integer);
begin
  if (FBottomIndent <> Value) then
  begin
    FBottomIndent := Value;
    if AutoSize.Enabled then
      DoAutoSize
    else
      Invalidate;
  end;
...
begin
  ...
  begin
    ...
    if flg then
    begin // force child win controls repaint
      Width := Width + 1;
      Width := Width - 1;
    end;
  end;
...
begin
  ...
  FCapHeight := Round(FDPIScale * Caption.Height);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      cpTop:
        begin
          incaption := Caption.Visible and (Controls[i - 1].Top < FCapHeight);
        end;
      cpLeft:
        begin
          incaption := Caption.Visible and (Controls[i - 1].Left < FCapHeight);
        end;
      cpRight:
        begin
          incaption := Caption.Visible and (Controls[i - 1].Left > Width - FCapHeight);
        end;
      end;
...
begin
  ...
  begin
    r := ClientRect;
    if ((PtInRect(rect(r.Left,r.Top,r.Right-2,r.Top + FCapHeight),point(Message.XPos,Message.YPos)) and (FCaption.Position = cpTop))
      or ((PtInRect(rect(r.Left,r.Top,r.Left + FCapHeight,r.Bottom),point(Message.XPos,Message.YPos))) and (FCaption.Position = cpLeft))
      or ((PtInRect(rect(r.Right - FCapHeight,r.Top,r.Right,r.Bottom),point(Message.XPos,Message.YPos))) and (FCaption.Position = cpRight)) ) then
...
begin
  ...
  begin
    if FCaption.Position = cpTop then
    begin
      if Rect.Top < FCapHeight then
        Rect.Top := FCapHeight - 2;
    end
    else if FCaption.Position = cpLeft then
    begin
      if Rect.Left < FCapHeight then
        Rect.Left := FCapHeight - 2;
    end
    else // FCaption.Position = cpRight
    begin
      if Rect.Right > Width - FCapHeight then
        Rect.Right := Width - FCapHeight;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if FStatusBar.Visible then
          Width := XSize + FCapHeight + FStatusBar.Height + 2 * BevelWidth + Indent
        else
          Width := XSize + FCapHeight + 2 * BevelWidth + Indent;
      end
      else
      begin
        if FStatusBar.Visible then
  ...
  begin
    ...
    begin
      ...
      begin
        if FStatusBar.Visible then
          Height := YSize + FCapHeight + FStatusBar.Height + 2 * BevelWidth + TopIndent + BottomIndent + 2 * BorderWidth
        else
          Height := YSize + FCapHeight + 2 * BevelWidth + TopIndent + BottomIndent;
      end
      else
      begin
        if FStatusBar.Visible then
          Height := YSize + FStatusBar.Height + 2 * BevelWidth + TopIndent + BottomIndent
        else
          Height := YSize + 2 * BevelWidth + TopIndent + BottomIndent;
      end;
    end
    else
      Height := Ysize + 2 * BevelWidth + TopIndent + BottomIndent;
...
// CloseButton and MinMaxButton are independent on style
  ...
  BottomIndent := Settings.BottomIndent;
...
begin
  ...
  begin
    ...
    FBottomIndent := (Source as TAdvPanelSettings).BottomIndent;
  end;
end;
...
begin
  if (FAutoHideChildren <> Value) then
  begin
...
procedure TAdvPanelSettings.SetBottomIndent(const Value: Integer);
begin
  if (FBottomIndent <> Value) then
  begin
    FBottomIndent := Value;

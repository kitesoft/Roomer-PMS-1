...
const
  ...
  // 2.2.12.1: Improved : Error reporting when making an FTP connection with no host specified
  // 2.2.12.2: Fixed : Issue with default logging path for Windows 10
  // 2.2.12.3: Fixed : Issue with downloading update control .INF file when FTP folder does not exist
  // 2.3.0.0 : New: OnBeforeUrlToFile event added for custom downloading of files
  // 2.3.0.1 : Fixed : Issue with dealing with FTP downloads & caching
  // 2.4.0.0 : New : Whats New dialog can display mini HTML formatted text including having hyperlinks (* TMS Component Pack only)
...
type
  ...
  TWebUpdateBeforeUrlToFile = procedure(hfile:hinternet;url,tgt:string; var uncompress,handled,success:Boolean) of object;
  ...
  private
    ...
    FBeforeUrlToFile: TWebUpdateBeforeUrlToFile;
  ...
  protected
    ...
    function IsHTML(ms: TMemoryStream): Boolean;
  ...
  public
   ...
   published
    ...
    property OnBeforeUrlToFile:TWebUpdateBeforeUrlToFile read FBeforeUrlToFile
      write FBeforeUrlToFile;
...
const
  ...
  DXE9 = '\Software\Embarcadero\BDS\17.0';
  DXE10= '\Software\Embarcadero\BDS\18.0';
...
begin
  ...
  if IsPlatformInstalled(DXE7) then
    DevPlatforms := DevPlatforms + 'DXE7';
  if IsPlatformInstalled(DXE7) then
    DevPlatforms := DevPlatforms + 'DXE7';

  if IsPlatformInstalled(DXE8) then
    DevPlatforms := DevPlatforms + 'DXE8';
  if IsPlatformInstalled(DXE9) then
    DevPlatforms := DevPlatforms + 'D10';
  if IsPlatformInstalled(DXE10) then
    DevPlatforms := DevPlatforms + 'D10.1';
...
begin
  ...
  fn := StringReplace(fn,'{APP}',ExtractFilePath(Application.EXEName),[rfReplaceAll,
    rfIgnoreCase]);

  if pos('\\', fn) = 1 then
  begin
    fn := StringReplace(copy(fn,3,high(integer)),'\\','\',[rfReplaceAll, rfIgnoreCase]);
    fn := '\\' + fn;
  end
  else
...
var
  ...
  s: string;
  DoCancel, dwnloaderror, handled: Boolean;
...
begin
  ...
  if Assigned(FBeforeUrlToFile) then
  begin
     handled := false;
     fn := tgt + fn;
     FBeforeUrlToFile(hfile,url,tgt, uncompress, handled, Result);
     if handled and FileExists(fn) then
     begin
        CustomProcess(fn);
  ...
  begin
   ...
   if findfile = nil then
     DoStatus(925,fn,11,GetLastError);
   ...
   hintfile := FtpOpenFile(hfile,pchar(fn),GENERIC_READ,FTP_TRANSFER_TYPE_BINARY or INTERNET_FLAG_RELOAD or INTERNET_FLAG_NO_CACHE_WRITE,0);

   if hintfile = nil then
     DoStatus(946,fn,13,GetLastError);
  end
  else
  begin
...
function TWebUpdate.IsHTML(ms: TMemoryStream): boolean;
var
  s: ansistring;
begin
  SetLength(s, ms.Size);
  ms.Position := 0;
  ms.Read(s[1], ms.Size);
  Result := pos(ansistring('</'),Uppercase(string(s))) > 0;
  ms.Position := 0;
end;
...
begin
  ...
  begin
    ...
    if HTML then
    begin
      scrl := TScrollBox.Create(form);
      scrl.Parent := form;
      scrl.Top := 10;
      scrl.Left := 10;
      scrl.Width := form.ClientRect.Right - 20;
      scrl.Height := form.ClientRect.Bottom - 60;
      scrl.Ctl3D := false;
      scrl.BorderStyle := bsNone;

      hlbl := THTMLabel.Create(scrl);
      hlbl.Parent := scrl;
      hlbl.HTMLText.LoadFromStream(ms);
      hlbl.AutoSizing := true;

      ctrlwidth := scrl.Width;
      ctrlheight := scrl.Height;
      ctrltop := scrl.Top;
      ctrlleft := scrl.Left;
    end
    else

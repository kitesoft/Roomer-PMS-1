...
const
  ...
  // 2.2.1.1 : Fixed : Issue with calendar header hover painting
  // 2.2.2.0 : Improved : High-DPI support
  // 2.2.2.1 : Fixed : Day names painting when large fonts are used
  // 2.3.0.0 : New : Support for VCL styles added
  // 2.3.0.1 : Fixed : Week number column header text painting
  // 2.3.1.0 : New : StyleElements exposed
  // 2.3.1.1 : Fixed : Painting issue in non-themed case
  // 2.3.1.2 : Fixed : Issue with OnRangeSelect time of triggering
...
type
  ...
  private
    ...
    FUsedVCLStylesTextColor: boolean;
    FTxtColor: TColor;
  ...
  protected
    ...
    property UsedVCLStylesTextColor: boolean read FUsedVCLStylesTextColor write FUsedVCLStylesTextColor;
...
begin
  ...
  FIsGrouped := false;
  FBorderFix := false;
  FUsedVCLStyles := false;
  FUsedVCLStylesTextColor := false;
...
function TPlannerCalendar.GetBevelOuter: TBevelCut;
begin
...
procedure TPlannerCalendar.ChangeScale(M, D: Integer);
begin
  inherited;

  if not (csDesigning in ComponentState) and (M <> D) then
  begin
    FDayFont.Height := MulDiv(FDayFont.Height, M, D);
    FWeekFont.Height := MulDiv(FWeekFont.Height, M, D);
    FEventFont.Height := MulDiv(FEventFont.Height, M, D);
  end;
end;
...
begin
  ...
  l := FCanvas.TextWidth(labels);
  yw := FCanvas.TextWidth(' 9999');

  if flgla then
    FCanvas.Font.Color := FSelectColor
  else
    FCanvas.Font.Color := FTxtColor;

  SetBKMode(FCanvas.Handle, TRANSPARENT);

  FCanvas.TextOut(xoffset + ((self.Width - loword(l) - xoffset) shr 1), 2, labels);
...
begin
  ...
  begin
    ...
    if FUsedVCLStylesTextColor then
      FCanvas.Font.Color := FTxtColor;

    SetBKMode(Handle, TRANSPARENT);
    r := rect(0,0,100,100);
    dstr := 'gh';
    wfh := DrawText(FCanvas.Handle, PChar(dstr), Length(dstr), r, DT_TOP or DT_SINGLELINE or DT_CALCRECT);
    ...
    if UsedVCLStyles then
      pen.Color := FTxtColor
    else
      pen.Color := FLineColor;

    MoveTo(BorderWidth,r.Top + wfh + 2);
    LineTo(Width - BorderWidth,r.Top + wfh + 2);

    if FLine3D and not UsedVCLStyles then
    begin
      Pen.Color := clWhite;
      MoveTo(BorderWidth, r.Top + wfh + 3);
      LineTo(Width - BorderWidth, r.Top + wfh + 3);
    end;
...
var
  ...
  bevtop,bevbot: TColor;
  {$IFDEF DELPHIXE2_LVL}
  LStyle: TCustomStyleServices;
  LDetails: TThemedElementDetails;
  SaveIndex: integer;
  LColor, bkcol, brdrcol: TColor;
  {$ENDIF}
...
begin
  ...
  begin
    ...
    try
      ...
      FUsedVCLStyles := true;
      FUsedVCLStylesTextColor := true;

      {$IFDEF DELPHIXE6_LVL}
      FUsedVCLStylesTextColor := seFont in StyleElements;
      {$ENDIF}
      ...
      begin
        DrawBkg := false;
        FCanvas.Brush.Color := bkcol;
        FCanvas.Pen.Color := bkcol;
      ...
      begin
        ...
        begin
          ...
          Frame3D(FCanvas, R, TopColor, BottomColor, BevelWidth);
        end;

        if BorderStyle = bsSingle then
          Frame3D(FCanvas, R, brdrcol, brdrcol, 1);
  ...
  if DrawBkg or DrawBrdr then
  begin
    if FUseTheme and FBorderXP and DrawBrdr then
    begin
...
procedure TPlannerCalendar.SetBevelOuter(const Value: TBevelCut);
begin
...
procedure TPlannerCalendarGroup.ChangeScale(M, D: Integer);
begin
  inherited;

  if not (csDesigning in ComponentState) and (M <> D) then
  begin
    FCalendarWidth := MulDiv(FCalendarWidth, M, D);
    FCalendarHeight := MulDiv(FCalendarHeight, M, D);

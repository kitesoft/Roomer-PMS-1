...
const
  ...
  // v1.1.2.0 : New : Exposed CurvyMemo.WordWrap property
  // v1.1.3.0 : New : DropDownWidth property added in TCurvyCombo
  // v1.2.0.0 : New : VCL Style support added to TCurvyPanel
  // v1.2.1.0 : New : StyleElements property exposed
...
begin
  ...
  try
    fill.BorderColor := ColorToRGB(brdrcol);
    fill.Rounding := FRounding;
    fill.RoundingType := rtBoth;

    fill.Color := ColorToRGB(col);
    fill.ColorTo := ColorToRGB(col);
    fill.ColorMirror := ColorToRGB(col);
    fill.ColorMirrorTo := ColorToRGB(col);
...
function TCurvyMemo.GetWordWrap: boolean;
begin
...
procedure TCurvyMemo.SetWordWrap(const Value: boolean);
begin
  FEdit.WordWrap := Value;
end;
...
procedure TBorderlessCustomComboBox.CreateWnd;
begin
  inherited;
...
procedure TBorderlessCustomComboBox.SetDropDownWidth(const Value: integer);
begin
  FDropDownWidth := Value;
  if (Value > 0) and HandleAllocated then
...
begin
  ...
  begin
    ...
    if DropDownWidth > 0 then
      FCombo.DropDownWidth := DropDownWidth;
  end;
end;
...
procedure TCurvyCombo.SetDropDownWidth(const Value: integer);
begin
  FDropDownWidth := Value;
  if Assigned(FCombo) then
    FCombo.DropDownWidth := Value;
end;

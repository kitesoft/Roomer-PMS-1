...
type
  TContainerEditor = class(TForm)
    ...
    procedure LBWMDROPFILES(var Msg: TMessage);
...
procedure TContainerEditor.LBWMDROPFILES(var Msg: TMessage);
var
  pcFileName: PChar;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (pcFileExt = '.GIF') or (pcFileExt = '.JPG') or (pcFileExt = '.JPEG') or (pcFileExt = '.PNG') or
      ...
      begin
        ...
        Name := Uppercase(Copy(Name, 1, Length(Name) - Length(pcFileExt)));
...
begin
  if Message.Msg = WM_DROPFILES then
    LBWMDROPFILES(Message); // handle WM_DROPFILES message
...
begin
  ...
  begin
    ...
    begin
      ...
      s := OpenDialog.Files[i];
      APictureItem.Picture.LoadFromFile(s);
      s := ExtractFileName(s);
      ext := ExtractFileExt(s);
      s := Copy(s,1, Length(s) - Length(ext));
      APictureItem.Name := Uppercase(s);

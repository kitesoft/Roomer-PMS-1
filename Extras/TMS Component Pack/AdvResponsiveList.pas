...
uses
  ...
  AdvGraphics, AdvTypes, AdvScrollControl, AdvResponsiveListTypes, AdvUtils,
  PictureContainer
  {$IFNDEF FMXLIB}
  , Controls, Forms, Graphics
  {$ENDIF}
  {$IFDEF FMXLIB}
  , FMX.Controls, FMX.Forms, FMX.Graphics, System.UIConsts, System.UITypes
  {$ENDIF}
  {$IFDEF VCLLIB}
  , Messages, ImgList
  {$IFNDEF FNCLIB}
...
type
  ...
  protected
    procedure DrawItem(AGraphics: TAdvGraphics; ATemplate, AHeaderTemplate, AFooterTemplate: string; ARect: TRect; Focus: boolean); virtual;
  ...
  protected
    ...
    procedure GetWordAndIndexAtCaret(var {%H-}AValue: string; var {%H-}AIndex: integer); override;
    procedure UpdateWordAndIndexAtCaret({%H-}AValue: string; {%H-}AIndex: integer); override;
...
uses
  AdvGraphicsHTMLEngine, SysUtils
  {$IFDEF VCLLIB}
  , ShellApi
  {$ENDIF}
  {$IFDEF USEVCLSTYLES}
  ...
  begin
    ...
    begin
      if Items[i].Control.Align = alClient then
        Items[i].Control.SetBounds(Items[i].Rect.Left,Items[i].Rect.Top,
...
begin
  ...
  begin
    if MatchFromTo(Round(Width), Conditions[i].WidthFrom, Conditions[i].WidthTo) and
       MatchFromTo(Round(Height), Conditions[i].HeightFrom, Conditions[i].HeightTo) then
    begin
...
begin
  ...
  begin
    ...
    begin
      Appearance.SelectedItemColor := clr;
    end
    else
...
procedure TAdvResponsiveList.KeyPressN(var {%H-}Key: Char);
begin
  inherited;
end;

procedure TAdvResponsiveList.KeyUpN(var {%H-}Key: Word; {%H-}Shift: TShiftState);
begin
  inherited;
end;
...
begin
  ...
  begin
    ...
    ith := cond.ItemHeight;
    itw := cond.ItemWidth;
    ml := cond.Margins.Left;
    mr := cond.Margins.Right;
    mt := cond.Margins.Top;
    mb := cond.Margins.Bottom;
  end;
...
begin
  ...
  begin
    ...
    if BorderStyle = bsSingle then
      AGraphics.DrawLine(PointF(hr.Left,hr.Bottom - FooterHeight), PointF(hr.Right - 1,hr.Bottom - FooterHeight));

    AGraphics.Fill.Color := FooterColor;
    AGraphics.Stroke.Color := FooterColor;

    hr.Top := hr.Bottom - FooterHeight;
    ...
    AGraphics.DrawText(RectF(ARect.Left + 2, ARect.Bottom - FooterHeight, ARect.Right - 2, ARect.Bottom), s);
    cr.Bottom := cr.Bottom - FooterHeight;
  end;
  ...
  begin
    ...
    if BorderStyle = bsSingle then
    begin
      AGraphics.DrawLine(PointF(hr.Left,hr.Top + HeaderHeight),PointF(hr.Right - 1,hr.Top + HeaderHeight));
    end;

    AGraphics.Fill.Color := HeaderColor;
    AGraphics.Stroke.Color := HeaderColor;

    hr.Bottom := hr.Top + HeaderHeight;
    ...
    AGraphics.DrawText(RectF(hr.Left + 2, hr.Top, hr.Right - 2, hr.Top + HeaderHeight), s);
    cr.Top := cr.Top + HeaderHeight;
  end;
...
//function TResponsiveConditions.GetItemClass: TCollectionItemClass;
//begin
//  Result := TResponsiveCondition;
//end;

...
const
  ...
  // 1.8.1.2 : Fixed : Issue with OnEditStart triggered twice
  // 1.8.1.3 : Fixed : Issue with hidden items and Expand/Collaps
  // 1.8.2.0 : New : InspectorItem.FloatValue property added
  // 1.8.3.0 : New : TFormControlInspectorEditLink added
...
begin
  ...
  begin
    ...
    begin
      ...
      TInspectorItems(Collection).Items[i].Collapsed := True;
    end
    else
      Break;
    Inc(i);
...
begin
  ...
  begin
    ...
    begin
      TInspectorItems(Collection).Items[i].Collapsed := False;
    end;
...
function TInspectorItem.GetVisible: Boolean;
begin
  Result := FVisible and not Collapsed;
end;
...
procedure TInspectorItem.SetFloatValue(const Value: double);
begin
  FFloatValue := Value;

  FTextValue := FloatToStr(Value);
  Changed;
end;
...
begin
  ...
  case FPropertyType of
    ...
    ptFloat:
      TextValue := '0' + DecimalSeparator + '0';
    ptInteger, ptIntSpin:
      TextValue := '0';
  else
    TextValue := '';
...
procedure TInspectorBar.CreateWnd;
begin
  inherited;
  if FDesignTime then
  begin
...
//-------Button for scroll 20x20---------------
  ...
  begin
    ...
    repeat
      if not FPanels[realtoppanel].FItems[k].Visible then
        Dec(k);
      if k <= 0 then
        Break;
    until (FPanels[realtoppanel].FItems[k].Visible);
...
//draw the items
  ...
  for i := TopPanel.FTopItem + 1 to TopPanel.Items.Count do
    if TopPanel.Items.Items[i - 1].Visible then
    begin
  ...
  begin
    ...
    if (TopPanel.Items[i - 1].Visible) and (q <= VisibleItems) then
    begin
      Inc(q);
...
// j holds the current drawing position
  ...
  //draw the items
      ...
      begin
        ...
        if TopPanel.Items[q - 1].Visible then
        begin
      ...
      begin
        ...
        begin
          ...
          for m := 1 to Item.Index do
            if FPanels.Items[i - 1].Items[m - 1].Visible then
              if FPanels.Items[i - 1].VariableItemHeight then
...
//-------- Items--------
    ...
    begin
      ...
      if FPanels.Items[i - 1].FItems.Items[j].Visible then
        if (AnsiPos('&' + ch, ss) <> 0) then
        begin
...
begin
  ...
  //Items
      ...
      begin
        if not FPanels.Items[i].Items[m - 1].Visible then
          Continue;
        if FPanels.Items[i].VariableItemHeight then
...
// compute absolutete coordinates from local coordinates
  ...
  //Items
      ...
      begin
        if not FPanels.Items[i].Items[k - 1].Visible then
          Continue;
        if FPanels.Items[i].VariableItemHeight then
          tempHeight := FPanels.Items[i].Items[k - 1].Height
        else

...
uses
  Classes, DateUtils, Types, SysUtils, AdvPDFGraphicsLib, AdvGraphics
...
const
  ...
  PDFPageAcroFormFieldsRef = '{PDFPAGEACROFORMFIELDSREF}';
  PDFPageAcroFormFieldContentRef = '{PDFPAGEACROFORMFIELDCONTENTREF}';
...
type
  ...
  TAdvGeneralPDFLibMetaData = class(TAdvGeneralPDFLibObject);
...
//  TAdvGeneralPDFLibTextField = class(TAdvGeneralPDFLibAcroFormField);
  ...
  private
    ...
    FMetaData: TAdvGeneralPDFLibMetaData;
...
begin
  Result := FormatFloat('0000', YearOf(ADateTime)) + FormatFloat('00', MonthOf(ADateTime)) + FormatFloat('00', DayOf(ADateTime)) +
    FormatFloat('00', HourOf(ADateTime)) + FormatFloat('00', MinuteOf(ADateTime)) + FormatFloat('00', SecondOf(ADateTime));
end;
...
begin
  ...
  FOutput.WriteString(PDFStart + PDFLB);
...
//  FOutput.WriteString('/AcroForm ' + PDFPageAcroFormRef + ' 0 R');
  ...
  FMetaData := TAdvGeneralPDFLibMetaData.Create(Self);
  r := FXRefObjects.AddObject(FMetaData);
  FOutput.StartNewStream(r);
  FOutput.WriteString(IntToStr(FXRefObjects.IndexOf(r)) +' 0 obj' + PDFLB);
  FOutput.WriteString('<<');
...
begin
  ...
  begin
    ...
    if Assigned(FActivePage) then
      FActivePage.FFontList.Remove(AFont);
...
begin
  ...
  begin
    ...
    FOutput.WriteString('/Info 3 0 R');
...
begin
  ...
  begin
    if (ft.Name = ABaseFontName) and (ft.Unicode = AUnicodeFont) then
    begin
      Result := ft;
      Break;
    end;
...
begin
  ...
  begin
    ...
    if (FOutput.FontBase <> fontn) or (FOutput.FontStyle <> g.Font.Style) or ASearchForUnicodeFont then
    begin
      ...
      ft := SearchForFont(FOutput.FontBase, ASearchForUnicodeFont);
...
begin
  r := RefName;
  if r <> '' then
    Result := r + ' ' + IntToStr(PDFLib.FXRefObjects.IndexOfObject(Self)) + ' 0 R'
  else
    Result := IntToStr(PDFLib.FXRefObjects.IndexOfObject(Self)) + ' 0 R';
end;
...
begin
  ...
  r := RefNameBase;
  if r <> '' then
    Result := r + IntToStr(PDFLib.FXRefObjects.IndexOfObject(Self, True));
end;

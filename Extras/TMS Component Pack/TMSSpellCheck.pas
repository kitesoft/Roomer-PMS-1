...
const
  ...
  // v1.0.3.0 : New : RAD Studio 10.1 Berlin support
  // v1.0.3.1 : Fixed : Issue with interfacing to TAdvRichEditor
  // v1.0.3.2 : Fixed : Issue with German affix file
  // v1.0.3.3 : Fixed : Issue with reading encoded chars from .aff files
  // v1.0.4.0 : New : OnUpdateProgress event added to replace built-in progress form
...
type
  ...
  TSpellCheckUpdateProgressEvent = procedure(Sender: TObject; Position: integer; Max: integer)
    of object;
  ...
  private
    ...
    FActive: boolean;
    FTempActive: boolean;
    FOnUpdateProgress: TSpellCheckUpdateProgressEvent;
  ...
  public
    ...
    property OnUpdateProgress: TSpellCheckUpdateProgressEvent
      read FOnUpdateProgress write FOnUpdateProgress;
...
{$ENDIF}
          OnUpdateProgress(Self, I, st.Count);
      end;
...
{$IFNDEF FMXLIB}
  FProgressform := nil;
  if not Assigned(OnUpdateProgress) then
...
{$ENDIF}
      ...
      begin
{$IFNDEF FMXLIB}
        if Assigned(FProgressForm) and not FProgressform.Visible then
          FProgressform.Show;
{$ENDIF}

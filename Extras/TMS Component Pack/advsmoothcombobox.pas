...
const
  ...
  // v2.2.1.0 : New : ClearFilter
  //          : Fixed : Issue with selected item index when clearing items
  // v2.2.1.1 : Fixed : Issue with setting database cursor
...
type
  ...
  protected
    ...
    function GetFilterOwner: TAdvSmoothListBox; override;
    procedure SetSelectedItemIndex(const Value: integer); override;
  ...
  published
    ...
    property SelectedItemIndex: integer read FSelectedItemIndex write SetSelectedIdx default -1;
...
procedure TAdvSmoothComboBox.ClearFilter;
begin
  inherited;
  if Assigned(FListBox.Filter.Edit) and FListBox.Filter.Edit.HandleAllocated then
    FListBox.Filter.Edit.Clear;
end;
...
function TAdvSmoothComboBox.GetFilterOwner: TAdvSmoothListBox;
begin
  Result := nil;
end;
...
begin
  inherited;
  FSelectedItemIndex := inherited SelectedItemIndex;
end;

procedure TAdvSmoothComboBox.SetSelectedIdx(const Value: integer);
begin

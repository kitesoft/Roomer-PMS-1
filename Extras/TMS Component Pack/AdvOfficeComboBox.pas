...
const
  ...
  // v1.3.0.1 : Fixed : Issue with font size selector dropdown width calculation
...
function TAdvCustomOfficeComboBox.GetComponentStyle: TTMSStyle;
begin
  Result := FTMSStyle;
end;
...
begin
  ...
  sm := GetSystemMetrics(SM_CXVSCROLL);

  FDropDownList.Canvas.Font.Assign(Font);

  for i := 0 to FDropDownListBox.Items.Count - 1 do
  begin
    if FIsFontList then
    begin
      FDropDownList.Canvas.Font.Name := FDropDownListBox.Items[i];
      FDropDownList.Canvas.Font.Size := FontSize;
    end;

    cw := FDropDownList.Canvas.TextWidth(FDropDownListBox.Items[i]);

    MinTW := Max(MinTW, 16 + Round(1.0 * cw) + sm);
  end;

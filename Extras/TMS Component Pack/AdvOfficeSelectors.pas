...
function TAdvCustomOfficeSelector.GetComponentStyle: TTMSStyle;
begin
  Result := FTMSStyle;
end;
...
procedure TAdvCustomOfficeColorSelector.SetCustomColors(const Value: TStrings);
begin
  ...
  begin
    ...
    if (Tools.Items[SelectedIndex].BackGroundColor = clMoreColors) and
      ...
      begin
        ...
        dlg.CustomColors.Assign(FCustomColors);
        if dlg.Execute then
        begin
          FCustomColors.Assign(dlg.CustomColors);
...
function TAdvCustomOfficeScrollSelector.GetComponentStyle: TTMSStyle;
begin
  Result := FStyle;
end;
...
begin
  if TAdvOfficeColorSelector(FClient).SelectedColor <> AColor then
...
begin
  if TAdvOfficeToolSelector(FClient).SelectedIndex <> AIndex then

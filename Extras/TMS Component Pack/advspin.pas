...
const
  ...
  // 1.6.2.4 : Fixed : Issue with checking min/max for hex values
  // 1.6.3.0 : Improved : HighDPI support
  // 1.6.3.1 : Fixed : Issue with changing LabelFont at design-time
  // 1.7.0.0 : New : SpinButtonWidth property added
  // 1.7.1.0 : New : ImeMode & ImeName exposed
  // 1.7.2.0 : New : OnClipboardCut,OnClipboardCopy,OnClipboardPaste events added
  //         : New : OnAfterClipboardCut,OnAfterClipboardCopy,OnAfterClipboardPaste events added
  // 1.7.3.0 : New : Property StyleElements exposed
...
{ TAdvSpinEdit }
  ...
  private
    ...
    procedure SetSpinButtonWidth(const Value: Integer);
  ...
  published
    ...
    property SpinButtonWidth: Integer read FSpinButtonWidth write SetSpinButtonWidth default 0;
    property ShowHint;
...
begin
  ...
  {$IFDEF DELPHIXE4_LVL}
  Result := ch.IsNumber and (ch <> chr($BD)) and (ch <> chr($BC)) and (ch <> chr($BE));
  {$ENDIF}
end;
...
procedure TAdvSpinButton.SetButtonWidth(const Value: Integer);
var
  W, H: Integer;
begin
...
begin
  ...
  FButton.ButtonWidth := SpinButtonWidth;
  FButton.Width := 15;
  FButton.Height := 17;
...
begin
  ...
  FButton.ButtonWidth := FSpinButtonWidth;
...
procedure TAdvSpinEdit.SetSpinButtonWidth(const Value: Integer);
begin
  if (FSpinButtonWidth <> Value) then
  begin
    FSpinButtonWidth := Value;
    FButton.ButtonWidth := Value;
...
begin
  ...
  if (pos('-',oldval) > 0) and (pos('-',Text) = 0) then
  begin
    dec(ss);
  end;
  if (pos('-',oldval) = 0) and (pos('-',Text) > 0) then
...
var
  ...
  newval: integer;
begin
  newval := CheckValue(Value);

  newtxt := '0x'+IntToHex(newval,0);
...
procedure TAdvSpinEdit.WMCopy(var Message: TWMCopy);
var
  Allow: Boolean;
begin
  Allow := True;
  if Assigned(FOnClipboardCopy) then
    FOnClipboardCopy(self, copy(self.Text, SelStart + 1, SelLength), allow);

...
const
  ...
  BLD_VER = 0;// Build nr.
  DATE_VER = 'Nov, 2016'; // Month version
  ...
  // 3.4.4.3  : Fixed : Issue with handling double ESC chars
  // 3.4.5.0  : New : Property LinesModified: boolean added
  //          : Fixed : Issue with painting modified line marker
  // 3.4.6.0  : New : Events OnFindText, OnReplaceText, OnReplaceAllText added in TAdvMemoFindReplaceDialog
  // 3.4.6.1  : Fixed : Rare issue with scrollbar updating
  // 3.4.6.2  : Fixed : Rare issue with EscapeChar handling
  // 3.4.7.0  : New : High-DPI support added
  // 3.4.7.1  : Fixed : Issue with SQL styler
  // 3.4.7.2  : Fixed : Issue with clipboard handling in 64bit
  // 3.4.7.3  : Fixed : Issue with highlighting + & - operators
  // 3.4.7.4  : Improved : Horiz. scrollbar large change setting
  // 3.4.7.5  : Fixed : Issue with Find & replace and replace all function with WrapAtEnd
  //          : Fixed : Issue with case sensitive search
  // 3.4.8.0  : New : Method CursorToSelection added
  //          : Fixed : Issue with Redo and ShowModified = true
  // 3.4.9.0  : New : Event OnFindDone added to TAdvMemoFindReplaceDialog
  ...
  // 3.4.9.1  : Fixed : Issue with vertical scrolling & resizing memo
  // 3.4.9.2  : Improved : Modal behavior for find & replace dialogs
  //          : Improved : Sequence in Find and Find/Replace dialog when searching after end of document
  // 3.4.9.3  : Improved : HTML syntax handling
  // 3.5.0.0  : New : Support for RAD Studio 10.1 Berlin
  // 3.5.0.1  : Fixed : Issue with timer-triggered auto complete
  // 3.5.0.2  : Fixed : Rare issue with resizing editor & vertical scrollbar
  // 3.5.0.3  : Fixed : Issue with Find & replace all in specific circumstances
  // 3.5.0.4  : Fixed : Issue with Up search in replace
  //          : Fixed : Issue with case-sensitive search in replace
  // 3.5.1.0  : New : GotoBegin / GotoEnd methods to move caret to begin or end of document
  // 3.5.1.1  : Fixed : Issue with TDBAdvMemo updating DB field
  //          : Fixed : Issue with search and wrap at end
  // 3.5.1.2  : Fixed : Issue with find & replace in up direction
  // 3.5.1.3  : Improved : High DPI scale calculation
  // 3.5.1.4  : Fixed : Issue with background color rendering for RTF output
  // 3.5.2.0  : Improved : TrailingLinebreak default false for saving to file
  // 3.5.3.0  : New : Method UnSelect added
  // 3.5.4.0  : New : Events OnMouseLeave, OnMouseEnter exposed
...
//------------------------------------------------------------------------------
  ...
  TFindOptionEx = (freDown, freFindNext, freHideMatchCase, freHideWholeWord,
    ...
    freDisableWholeWord, freReplace, freReplaceAll, freWholeWord, freShowHelp, freSelection, freExpression, freWrapAtEnd);
...
//------------------------------------------------------------------------------
  ...
  private
    ...
    function GetLinesModified: boolean;
  protected
    FLetRefresh: Boolean;
  ...
  public
    ...
    property LinesModified: boolean read GetLinesModified;
...
//------------------------------------------------------------------------------
  ...
  private
    ...
    FOnClose: TNotifyEvent;
    FCount: integer;
    FOnReplaceText: TNotifyEvent;
    FOnReplaceAllText: TNotifyEvent;
    FOnFindText: TNotifyEvent;
    FOnFindDone: TNotifyEvent;
  ...
  published
    ...
    property OnFindText: TNotifyEvent read FOnFindText write FOnFindText;
    property OnFindDone: TNotifyEvent read FOnFindDone write FOnFindDone;
    property OnReplaceText: TNotifyEvent read FOnReplaceText write FOnReplaceText;
    property OnReplaceAllText: TNotifyEvent read FOnReplaceAllText write FOnReplaceAllText;
...
procedure TAdvCustomMemo.GotoBegin;
begin
  CurX := 0;
  CurY := 0;
end;
...
//        UNSELECT
//--------------------------------------------------------------
procedure TAdvCustomMemo.UnSelect;
begin
  FSelStartX := 0;
  FSelStartY := 0;
  FSelEndX := 0;
...
begin
  ...
  Canvas.Font.Size := Round(FFont.Size * FDPIScale);
  Canvas.Font.Style := [];
...
procedure TAdvCustomMemo.CursorToSelection;
begin
  ClearSelection;
end;
...
procedure TAdvCustomMemo.UpdateData;
begin
  //
end;
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  validesc := (lc = InternalStyles.EscapeChar) and (plc <> InternalStyles.EscapeChar);

                  if (length(s) > 0) and not validesc then
                  begin
                    if (chBE = #0) then
                    begin
...
var
  ...
  var
    i, len, toStart, toEnd, Innr, lc, rc, allb, arrb, actr, actl, keyidx: integer;
    done, WasPoint: boolean;
    validno, checkkey: boolean;
    part, partc: string;
    numsallowed: string;
    mcrl, mcll: Integer;
  begin
...
//      lastch := #0;
    ...
    if (InternalStyles.KeyWordPrefix <> '') and (s <> '') then
    begin
      if not (s[1] = InternalStyles.KeyWordPrefix[1]) then
        checkkey := false;
    end;
    ...
    begin
      ...
      if not checkkey and (InternalStyles.KeyWordPrefix <> '') and (s <> '') then
      begin
        if (s[tostart] = InternalStyles.KeyWordPrefix[1]) then
          checkkey := true;
      end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            validesc := (lastch = InternalStyles.EscapeChar) and (plastch <> InternalStyles.EscapeChar);

            // end of bracket string detected here
            if ((s[tostart] = SegmentStyle.EndBracket) or ((SegmentStyle.EndBracket = #0) and IsWordBoundary(s[tostart + 1]))) and not validesc then
            begin
              BufferingDraw(s[tostart], SegmentStyle);
              Delete(s, tostart, 1);
            ...
            begin
              ...
              if (plastch = InternalStyles.EscapeChar) and (lastch = InternalStyles.EscapeChar) then
                plastch := #0
              else
                plastch := lastch;
          ...
          begin
            ...
            begin
              ...
              if (s[toStart] = SegmentStyle.StartBracket) and (lastch <> InternalStyles.EscapeChar) and not validesc then
              begin
      ...
      if validno then
        ...
        begin
          ...
          begin
            ...
            nxtnum := #0;
            isnumstart := false;
            if (toEnd < Length(s)) then
            begin
              nxtnum := s[toEnd + 1];
              isnumstart := AnsiPos(nxtnum,'0123456789') > 0;
            end;

            if (toEnd > toStart) and (((S[toEnd] = '+') or (s[toEnd] = '-')) and (isnumstart)) then
            begin
            ...
            numsallowed := StringReplace(numsallowed, '+', '',[rfReplaceAll]);
            numsallowed := StringReplace(numsallowed, '-', '',[rfReplaceAll]);
          ...
          begin
            ...
            SegmentStyle.isNumber := (part <> '+') and (part <> '-');
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              keyidx := InternalStyles.FAllStyles.Items[i].FKeyWords.IndexOf(partc);
              if (keyidx <> -1) then
              begin
...
{$ENDIF}
  ...
  begin
    ...
    Canvas.Font.Size := Round(self.Font.Size * FDPIScale);
...
begin
  ...
  begin
    if (otl > InternalLines.Count - VisibleLineCount) and (InternalLines.Count > VisibleLineCount) then
      otl := InternalLines.Count - VisibleLineCount;

    FTopLine := otl;
  end;

  if sbVert.Visible = false then
    FTopLine := 0;
...
{$ENDIF}
      ...
      begin
        ...
        if oldmax <> InternalLines.Count then
          sbVert.Max := Math.Max(sbVert.Min, LineIndexToVisIndex(InternalLines.Count - 1) + (VisibleLineCount - 1));
...
function TAdvCustomMemo.GetLinesModified: boolean;
var
  i: integer;
begin
  Result := false;
  ...
  begin
    if LineModifiedInt[i] = lmModified then
    begin
      Result := true;
      break;
    end;
...
begin
  ...
  begin
    ...
    //localhtmlfont := '<FONT style="background-color: ' + HTMLClr(bc) + ';color:' + HTMLClr(C) + ';' + tagstrl(sl) + '">';
    if bc = clNone then
      FRTFEngine.AddHighLightColor(self.Color)
    else
...
begin
  ...
  begin
    ...
    wasmodified := (Items[FPos] is TModifiedUndo);
    ...
    if (FPos > 0) and wasmodified then
    begin
      Dec(FPos);
      Items[FPos].Redo;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        ACanvas.Font.Size := Round(6 * FDPIScale);
    ...
    begin
      ...
      begin
        ...
        ACanvas.Font.Size := Round(FGutter.Font.Size * FDPIScale);
    ...
    begin
      ...
      begin
        ...
        if Brush.Color <> clNone then
          ACanvas.FillRect(Rect(Right - 5, Top, Right - 1, Bottom));
      end;
...
begin
  ...
  begin
    ...
    begin
      if (i = fl) and (FSelStartX > 0) and (freDown in Options) then
      begin
...
var
  rz,oldrz,count: integer;
  sex,sey,ssx,ssy,dx,oldsey: integer;
begin
  oldrz := -1;
  count := 0;
  ...
  if (rz = -1) and (freReplaceAll in Options) and (freWrapAtEnd in Options) and (freDown in Options) and (sey > 0) then
  begin
    CurX := 0;
    CurY := 0;
...
begin
  if AutoHighLight and (Length(AText) > 0) then
...
begin
  ...
  begin
    ...
    begin
      FAdvMemo.CurY := FAdvMemo.Lines.Count - 1;
      FAdvMemo.CurX := Length(FAdvMemo.Lines[FAdvMemo.CurY]);
    end;
  end;
  ...
  begin
    ...
    // reverse when previous button is clicked
    if prev then
    begin
      if not (AdvFindDialogForm.fdoDown in FFindDialogEx.Options) then
        Options := Options + [freDown];
    end;
...
procedure TAdvMemoFindReplaceDialog.DoFindDone;
begin
  if Assigned(OnFindDone) then
    OnFindDone(Self);
end;
...
begin
  ...
  if Assigned(OnReplaceText) then
    OnReplaceText(Self);
    ...
    begin
      ...
      begin
        for vp := CurX + 1 to length(s) do
          if (s[vp] = #32) or (s[vp] = '(') or (s[vp] = ')') or (s[vp] = '[') or (s[vp] = ']') or (s[vp] = '.') or (s[vp] = ',') then
          begin
            ae := vp - 1;
            Break;
          end;
...
begin
  ...
  begin
    sWord := FullWordAtXY(CurX - 1, CurY);
...
begin
  ...
  begin
    if (str[i] = ' ') or (str[i] = '+') or (str[i] = '-') or (str[i] = '*') or (str[i] = '/') then
    begin
      vp := i;
      break;
    end;

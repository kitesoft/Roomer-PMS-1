...
const
  ...
  // v1.1.1.4 : Fixed : Issue with updating Time & OnExit event handler
  // v1.2.0.0 : New : Style wsCustom added
  // v1.2.1.0 : New : Resolution property added in TAdvWatch, TAdvTimePickerDropDown
...
type
  ...
  TWatchStyle = (wsClassic, wsTower, wsFluorescent, wsEmerald, wsBlueStar, wsFuchsia, wsBlack, wsBlackWhite, wsGray, wsSuperNova, wsSports, wsSmooth, wsCustom);
  ...
  private
    ...
    procedure SetResolutions(const Value: integer);
  protected
  ...
  published
    ...
    property Resolution: integer read FResolution write SetResolutions default 1;
...
begin
  ...
  FAdvWatch.Resolution := FResolution;
...
procedure TAdvTimePickerDropDown.SetResolutions(const Value: integer);
begin
  if Value <= 30 then
    FResolution := Value;
end;
...
begin
  ...
  begin
    Result := Round((Result / FResolution)) * FResolution;
  end;
end;
...
procedure TAdvWatch.SetResolution(const Value: integer);
begin
  if Value <= 30 then
    FResolution := Value;
end;

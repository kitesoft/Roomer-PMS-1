...
interface
  ...
  // 1.7.1.0 : New : Unicode parameters added for SaveToHTML / SaveToXML functions
  // 1.7.1.1 : Fixed : Column aligment issue in combination with sorting
  // 1.7.1.2 : Fixed : Memory leak in SaveToCSV() function
  // 1.7.1.3 : Fixed : Issue with CheckBoxes and AllowSelection = false
  // 1.7.2.0 : Improved : High DPI support
  // 1.7.3.0 : New : ssAnsiAlphaCase , ssAnsiAlphaNoCase sort modes added
  // 1.7.3.1 : Fixed : Issue with highlighting and alignment in columns
  // 1.7.4.0 : New : OnSaveValue event added
  // 1.7.4.1 : Fixed : Issue with column alignment & multi select/checkbox select
...
const
  ...
  BLD_VER = 1; // Build nr.
  DATE_VER = 'Oct, 2016'; // Month version
...
type
  ...
  TSortStyle = (ssAutomatic, ssAlphabetic, ssNumeric, ssDate, ssAlphaNoCase, ssAlphaCase,
    ssShortDateEU, ssShortDateUS, ssCustom, ssFinancial, ssRaw, ssAnsiAlphaCase, ssAnsiAlphaNoCase);
  ...
  TSaveValueEvent = procedure(Sender: TObject; iItem: integer; iSubItem: integer; var AValue: string) of object;
...
{$ENDIF}
    ...
    procedure SetSubItemEdit(const Value: Boolean);
  protected
    ...
    procedure DoSaveValue(iItem, iSubItem: Integer; var AValue: string); virtual;
  public
    ...
    procedure SaveToHTML(filename: string; SaveWithHTML: boolean = true; Unicode: boolean = false);
    procedure SaveToXML(FileName: string; ListDescr, RecordDescr: string; FieldDescr: TStringList; SaveWithHTML: boolean = true; Unicode: boolean = false);
...
{$ENDIF}
    ...
    property SubItemEdit: Boolean read FSubItemEdit write SetSubItemEdit default False;
    ...
    property OnSaveValue: TSaveValueEvent read FOnSaveValue write FOnSaveValue;
      ...
      if Pos('{\', s) = 1 then
        ...
        begin
          ...
          begin
            ...
            if (Columns[i + 1].Alignment in [tacenter, taRightJustify]) then
            begin
              HTMLDrawEx(FCanvas, s, DrwRect, TImageList(SmallImages), 0, 0, -1, -1, 1, False, True, False, False, True, False, WordWrap, 0.0,
                FURLColor, clNone, clNone, clGray, Anchor, Stripped, FA, XSize, YSize, ml, hl, hr, FImageCache, FContainer,0);

              case Columns[i + 1].Alignment of
              taCenter: DrwRect.Left := DrwRect.Left + (DrwRect.Right - DrwRect.Left - XSize) div 2;
              taRightJustify: DrwRect.Left := DrwRect.Right - XSize;
              end;
            end;
...
var
  chk: Boolean;
  itmidx,colidx: integer;
begin
  GetIndexAtPoint(msg.xpos, msg.ypos, itmidx, colidx);

  if CheckBoxes and not AllowSelection and (colidx = 0) then
  begin
    itmidx := ItemIndex;
    inherited;
    if ItemIndex <> itmidx then
      ItemIndex := itmidx;
    Exit;
  end;
...
procedure TAdvListView.DoSaveValue(iItem, iSubItem: Integer; var AValue: string);
begin
  if Assigned(OnSaveValue) then
    OnSaveValue(Self, iItem, iSubItem, AValue);
end;


{ All IO methods }

procedure TAdvListView.SaveToXML(FileName: string; ListDescr, RecordDescr: string; FieldDescr: TStringList; SaveWithHTML: boolean = true; Unicode: boolean = false);
var
  i, j: Integer;
  s: string;
  flist: TStringList;
...
begin
  ...
  flist.Add('<?xml version="1.0"?>');
  flist.Add('<' + ListDescr + '>');

  for i := 1 to Items.Count do
  begin
    flist.Add('<' + RecordDescr + '>');

    for j := 1 to Columns.Count do
    begin
      if j <= FieldDescr.Count then
        flist.Add('<' + fielddescr.strings[j - 1] + '>')
      else
        flist.Add('<FIELD' + inttostr(j - 1) + '>');
      ...
      begin
        if SaveWithHTML then
          ...
          s := HTMLStrip(Items[i - 1].Caption);

        DoSaveValue(i - 1, -1, s);
      end
      else
      begin
        if (Items[i - 1].SubItems.Count > j - 2) then
        begin
          if SaveWithHTML then
            s := Items[i - 1].SubItems[j - 2]
          else
            s := HTMLStrip(Items[i - 1].SubItems[j - 2]);

          DoSaveValue(i - 1, j - 2, s);
        end;
      end;
  ...
  try
    {$IFDEF DELPHI_UNICODE}
    if Unicode then
      flist.SaveToFile(FileName, TEncoding.Unicode)
    else
...
procedure TAdvListView.SaveToHTML(filename: string; SaveWithHTML: boolean = true; Unicode: boolean = false);
var
  i, j: Integer;
  s, al, ac, afs, afe, ass, ase: string;
  flist,slist: TStringList;
  AlignValue: TAlignment;
  parr: array[0..LV_MAX_COLS] of Integer;
  iCount: integer;
...
begin
  ...
  begin
    try
      flist.LoadFromFile(FHTMLSettings.HeaderFile);
    except
  ...
  begin
    ...
    try
      slist.LoadFromFile(fHTMLSettings.FooterFile);
    except
  ...
  begin
    ...
    for i := 0 to RowCount - 1 do
      ...
      begin
        ...
        DoSaveValue(i, j - 1, ss);

        if j = 0 then
          imidx := Items[i].imageindex
        else
          imidx := SubitemImages[i, j - 1];
  ...
  begin
    ...
    begin
      ...
      if FLoadHeader and (y = 0) then
        Columns[x].Caption := FileToLF(ss, false)
      else
      begin
        if (x = 0) then
          Items[y - k].Caption := filetolf(ss, false)
        else
          Items[y - k].Subitems[x - 1] := filetolf(ss, false);

        if (x = 0) then
          Items[y - k].ImageIndex := imidx
        else
          SubItemImages[y - k, x - 1] := imidx;
      end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        s := s + FUsedDelimiter + QuoteForCSV(v,FUsedDelimiter);
      end;
      sl.writeln(s);
    end;
...
begin
  ...
  begin
    ...
    begin
      if (i = 0) then
        ss := Columns[j].caption
      else
      begin
        if (j = 0) then
          ss := Items[i - 1].Caption
        else
        begin
          if j - 1 < Items[i - 1].SubItems.Count then
            ss := Items[i - 1].Subitems[j - 1]
          else
            ss := '';  
        end;
      end;
...
procedure TAdvListView.SetSubItemEdit(const Value: Boolean);
begin
  FSubItemEdit := Value;
  if FSubItemEdit then
    OwnerDraw := true;
end;
...
//  ShowFilter(FFilterBar);
  ...
  begin
    HeaderFont.Size := Round(HeaderFont.Size * FDPIScale);
  end;
end;

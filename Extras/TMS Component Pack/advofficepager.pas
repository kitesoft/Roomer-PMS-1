...
const
  ...
  // 3.5.0.1 : Improved : Performance in HighDPI mode
  // 3.5.1.0 : New : TabEdit property added to allow editing of tab caption
  //         : New : OnTabEditDone event added
  //         : Fixed : Issue with dropping non tab objects on pager
  // 3.5.1.1 : Fixed : Issue with FreeOnClose in 64bit apps
  // 3.5.1.2 : Improved : High DPI handling when form scaling is false
  // 3.5.1.3 : Fixed : Issue with prev/next page buttons and down state
  //         : Fixed : Design time handling for multiline tabs
  //         : Improved : Control button border
  //         : Improved : Buttons work at design-time
  // 3.5.2.0 : New : AdvOfficePager.Text, AdvOfficePage.Text property added to show text on empty pager/page
...
type
  ...
  TTabEditDoneEvent = procedure(Sender: TObject; PageIndex: integer; var Value: string) of object;
  ...
  private
    ...
    FEditPageIndex: Integer;
    FEditPageCaption: string;
    ...
    FPagerEdit: TPagerEdit;
    //--- MulitiLine
    FMultiLine: Boolean;
    FTabRows: array of TPoint;  // 1: start index;  2: end index
    FMultiLineRowCount: Integer;
    FTabRowSize: Integer;
    FText: string;  // Height in case of tpTop, tpBottom and width otherwise
    //---
  ...
  published
    ...
    property TabEdit: Boolean read FTabEdit write FTabEdit default False;
...
function TPagerTabSettings.GetAlignment: TAlignment;
begin
  Result := FAlignment;
  if Width = 0 then
...
procedure TAdvOfficePage.SetText(const Value: string);
begin
  if (FText <> Value) then
  begin
    FText := Value;
...
{$ENDIF}
  ...
  FPagerEdit := TPagerEdit.Create(Self);
...
begin
  ...
  if not FFormScaled then
    FCurrentOfficePagerStyler.TabAppearance.Font.Size := Font.Size;
end;
...
begin
  ...
  begin
    ...
    *)
    ...
    begin
      {$IFDEF DELPHIXE_LVL}
      PostMessage(Self.Handle, WM_OPDESTROYCLOSEBTN, nativeuint(Pointer(AdvPage.FCloseButton)), 0);
      {$ENDIF}
      {$IFNDEF DELPHIXE_LVL}
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Button <> mbMiddle) and not IsActive and AdvPages[Tab].TabEnabled then
      begin
        // Select Tab
      ...
      begin
        r := GetTabRect(ActivePageIndex);

        if TabEdit then
        begin
          FEditPageIndex := ActivePageIndex;
          FEditPageCaption := AdvPages[ActivePageIndex].Caption;
          FPagerEdit.Parent := self;
          FPagerEdit.Text := FEditPageCaption;
          FPagerEdit.BorderStyle := bsNone;
          InflateRect(r,-2,-2);
          FPagerEdit.Left := r.Left;
          FPagerEdit.Width := r.Right - r.Left;
          FPagerEdit.Top := r.Top;
          FPagerEdit.Height := r.Bottom - r.Top;
          FPagerEdit.Visible := true;
          FPagerEdit.SetFocus;
          FPagerEdit.OnExit := OnExitEdit;
          FPagerEdit.OnKeyDown := OnKeyDownEdit;
          FPagerEdit.OnChange := OnChangeEdit;
        end;
        ...
        else if not FDragging and AllowTabUndock and not AdvPages[Tab].Locked then
        begin
          Allow := true;
...
var
  ...
  R: TRect;
begin
  if FDragging and Assigned(FDraggingPage) then
  begin
    FDraggingPage.FDraggingAccept := False;
    ExceedBounds := false;
    P := Point(X, Y);
    if FDraggingPage.FFloating then
    begin
      ...
      begin
        ...
        begin
          if FDraggingPage.AdvOfficePager <> PDockInfo(FDockList[i]).Dock then
          begin
            PDockInfo(FDockList[i]).Dock.AddAdvPage(FDraggingPage);
            PDockInfo(FDockList[i]).Dock.ActivePage := FDraggingPage;

            FDraggingPage.FFloating := False;
    ...
    else if Assigned(FDraggingPage.AdvOfficePager) then
    begin
      R := FDraggingPage.AdvOfficePager.GetTabsRect;
      CP := FDraggingPage.AdvOfficePager.ScreenToClient(P);
      ay := CP.Y;
      ax := CP.X;
      ...
      begin
        ExceedBounds := True;
        if Assigned(FDraggingPage.AdvOfficePager.FArrow) then
          FDraggingPage.AdvOfficePager.DragCanceled;
      end
      else
      begin
        if FDraggingPage.AdvOfficePager.TabReorder and PtInRect(R, CP) then
        begin
          ...
          begin
            CreateDropArrow;
            FDraggingPage.FDraggingAccept := True;
            FDraggingPage.AdvOfficePager.DragOver(Self, CP.X, CP.Y, dsDragMove, FDraggingPage.FDraggingAccept);
          end;
        end;
      end;

      if ExceedBounds and Assigned(FDraggingPage.AdvOfficePager) then
      begin
        FDraggingPage.AdvOfficePager.SetFloatingPage(FDraggingPage, P.X, P.Y);
      end;
    end;
  end;
end;
...
begin
  ...
  if Assigned(FDraggingPage) then
  begin
    if Assigned(FDraggingPage.AdvOfficePager) and (FDraggingPage.AdvOfficePager.TabReorder) and FDraggingPage.FDraggingAccept then
    begin
      GetCursorPos(P);
      P := FDraggingPage.AdvOfficePager.ScreenToClient(P);
      FDraggingPage.AdvOfficePager.DragDrop(FDraggingPage.AdvOfficePager, P.X, P.Y);
    end;
...
procedure TAdvOfficePager.SetText(const Value: string);
begin
  if (FText <> Value) then
  begin
    FText := Value;
...
begin
  ...
  begin
    ...
    if PtInRect(GetTabsArea, p) and (GetAsyncKeyState(VK_LBUTTON) <> 0) then
    begin
      Tab := PTOnTab(P.X, P.Y);
      if (Tab >= 0) then
      begin
...
procedure TAdvOfficePager.OnExitEdit(Sender: TObject);
var
  s:string;
begin
  s := FPagerEdit.Text;
  if Assigned(OnTabEditDone) then
    OnTabEditDone(Self, FEditPageIndex, s);

  AdvPages[FEditPageIndex].Caption := s;
  FPagerEdit.Visible := false;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FScrollFirstButton.DesignClicks := true;
    ...
    begin
      ...
      FScrollLastButton.DesignClicks := true;
    ...
    begin
      ...
      FScrollPrevButton.DesignClicks := true;
    ...
    begin
      ...
      FScrollNextButton.DesignClicks := true;
...
procedure TAdvOfficePager.OnChangeEdit(Sender: TObject);
var
  r: TRect;
begin
  //
  if Length(FPagerEdit.Text) > Length(FEditPageCaption) then
  begin
    AdvPages[FEditPageIndex].Caption := FPagerEdit.Text;
    r := GetTabRect(FEditPageIndex);
    FPagerEdit.Width := r.Right- r.Left - 4;
  end;
end;
...
begin
  inherited;
  if (Source = Self) and TabReorder then
  begin
...
procedure TAdvOfficePager.OnKeyDownEdit(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin

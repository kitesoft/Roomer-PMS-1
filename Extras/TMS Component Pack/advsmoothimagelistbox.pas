...
const
  ...
  // v2.5.0.2 : Fixed : Issue with ampersand conversion in HTML engine
  // v2.5.0.3 : Fixed : Issue with END key handling when last item is splitter item
  // v2.5.0.4 : New : ZoomOutOnDblClick property
...
type
  ...
  private
    ...
    procedure SetZoomOutOnDblClick(const Value: Boolean);
  protected
  ...
  published
    ...
    property ZoomOutOnDblClick: Boolean read FZoomOutOnDblClick write SetZoomOutOnDblClick default False;
...
begin
  ...
  begin
    ...
    FZoomOutOnDblClick := (Source as TAdvSmoothImageListBox).ZoomOutOnDblClick;
...
procedure TAdvSmoothImageListBox.BeginUpdate;
begin
  Items.BeginUpdate;
end;
...
begin
  ...
  begin
    if not FAnimateDoubleClick and FIsZoomed and ZoomOutOnDblClick then
      DoZoom;
  end;
...
procedure TAdvSmoothImageListBox.EndUpdate;
begin
  Items.EndUpdate;
end;
...
begin
  ...
  begin
    if not FAnimateDoubleClick and FIsZoomed and not ZoomOutOnDblClick then
      DoZoom;
  end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          while Items[FFocusedItemIndex].Splitter and (FFocusedItemIndex > 0) and (FFocusedItemIndex < Items.Count - 1) or (Items[FFocusedItemIndex].Enabled = false) or (Items[FFocusedItemIndex].Visible = false) do
          begin
            case Key of
            VK_NEXT: FFocusedItemIndex := FFocusedItemIndex + 1;
            VK_PRIOR: FFocusedItemIndex := FFocusedItemIndex - 1;
      ...
      begin
        ...
        begin
          ...
          i := FSelectedItemIndex;
          while Items[FSelectedItemIndex].Splitter and (FSelectedItemIndex > 0) and (FSelectedItemIndex < Items.Count - 1) or (Items[FSelectedItemIndex].Enabled = false) or (Items[FSelectedItemIndex].Visible = false) do
          begin
            case Key of
            VK_NEXT: i := FSelectedItemIndex + 1;
            VK_PRIOR: i := FSelectedItemIndex - 1;
...
procedure TAdvSmoothImageListBox.SetZoomOutOnDblClick(const Value: Boolean);
begin
  if FZoomOutOnDblClick <> value then
  begin
    FZoomOutOnDblClick := Value;
    Changed;
  end;
end;

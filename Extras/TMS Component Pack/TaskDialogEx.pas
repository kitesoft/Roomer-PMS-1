...
begin
  ...
  if pos('|', Value) > 0 then
  begin
    TAdvGlowButton(aButton).Caption := Copy(Value, 1, Pos('|', Value) - 1);
    TAdvGlowButton(aButton).Hint := Copy(Value, Pos('|', Value) + 1, Length(Value));
    TAdvGlowButton(aButton).ShowHint := true;
  end
  else

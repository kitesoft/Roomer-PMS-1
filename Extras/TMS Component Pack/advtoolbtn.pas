...
const
  ...
  // 1.6.1.2 : Fixed : Issue with rounded corner drawing for grouped buttons
  // 1.6.1.3 : Fixed : Issue with centering glyph for button with ShowCaption = false
  // 1.6.2.0 : Improved : Disabled action glyph painting on button
...
uses
  ActnList, SysUtils, ComObj, CommCtrl, Math;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Direction then
        Rectangle(R.Left + stepw,R.Top,Min(R.Right, R.Left + stepw + Trunc(rstepw)+1),R.Bottom)
      else
        Rectangle(R.Left,R.Top + stepw,R.Right,Min(R.Bottom, R.Top + stepw + Trunc(rstepw)+1));
    end;
...
//  Shaded := true;
  ...
  begin
    ...
    begin
      if not Enabled and (IsVista and TMSDISABLEWITHCOLORSATURATION) then
        DrawDisabledImage(Canvas.Handle, ImgList, ImageIndex, GlyphPos.X, GlyphPos.Y)
      else
        imgList.Draw(Canvas,GlyphPos.X,GlyphPos.Y, ImageIndex, Enabled)
    end;
  end;
end;
...
begin
  ...
  if (Length(Caption) > 0) and ShowCaption then
  begin

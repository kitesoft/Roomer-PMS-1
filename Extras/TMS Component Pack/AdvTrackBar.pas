...
const
  ...
  // v1.6.8.3 : Improved : Track hint behavior
  // v1.6.8.4 : Fixed : Issue with track hint position for vertical trackbar
  // v1.6.9.0 : New : Support for high-dpi
  // v1.6.9.1 : Fixed : Issue with TAdvRangeSlider thumb drawing for non default ThumbPointer settings
  // v1.6.9.2 : Fixed : Return of minimum value when control is left with empty value
...
type
  ...
  protected
    ...
    function GetThumbTotalWidth: integer; virtual;
  ...
  protected
    ...
    function GetThumbTotalWidth: integer; override;
    function GetThumbRightWidth: Integer;
    function GetThumbRightSize: Integer;
...
begin
  ...
  begin
    ...
    begin
      R2 := SliderRect;
      Result.Top := (R2.Top + (R2.Bottom - R2.Top) div 2) - (GetThumbSize div 2);
      Result.Bottom := Result.Top + GetThumbSize;
    end
    ...
    begin
      ...
      begin
        R2 := TickMarkTopRect;
        Result.Top := R2.Top + TickMark.Spacing - (GetThumbSize div 2);
        Result.Bottom := Result.Top + GetThumbSize;
      end
      else
      begin
    ...
    Result.Right := Result.Left + GetThumbWidth;
  ...
  begin
    ...
    begin
      R2 := SliderRect;
      Result.Left := (R2.Left + (R2.Right - R2.Left) div 2) - (GetThumbSize div 2);
      Result.Right := Result.Left + GetThumbSize;
    end
    else if (FTickMark.Style <> tsNone) then
    begin
      if (FTickMark.Position in [tmTopLeft, tmBoth]) then
      begin
        R2 := TickMarkTopRect;
        Result.Left := R2.Left + TickMark.Spacing - (GetThumbSize div 2);
        Result.Right := Result.Left + GetThumbSize;
      end
      else
      begin
    ...
    Result.Top := Result.Bottom - GetThumbWidth;
...
begin
  ...
  begin
    if (Orientation = trHorizontal) then
    begin
      i := ThumbRect.Left + (GetThumbWidth div 2);
      R1 := Rect(R.Left, R.Top, i, R.Bottom);
      R2 := Rect(R1.Right-1, R.Top, R.Right, R.Bottom);
    end
    else
    begin
      i := ThumbRect.Top + (GetThumbWidth div 2);
...
begin
  Result := 0;
  if (GetThumbWidth > 0) then
  begin
    case Thumb.ThumbPointer of
      tpLeft: Result := 0;
      tpCenter: Result := (GetThumbWidth div 2);
      tpRight: Result := GetThumbWidth;
    end;
  end;
end;
...
begin
  Result := Round(FThumb.Width * FDPIScale);
end;
...
function TCustomTrackBar.GetThumbSize: integer;
begin
  Result := Round(Thumb.Size * FDPIScale);
end;

//------------------------------------------------------------------------------

function TCustomTrackBar.GetThumbTotalWidth: integer;
begin
...
begin
  ...
  begin
    ...
    if (Mx <> FMin) then
      d := ((R.Right - R.Left) - GetThumbTotalWidth) / (Mx - FMin)
    else
      d := 0;
...
var
  ...
  drwShape: TShapeDir;
begin
  if IsRating or (GetThumbSize <= 0) then
    Exit;
...
procedure TCustomTrackBar.DoChanged;
begin
...
begin
  ...
  begin
    ...
    if (Orientation = trHorizontal) then
    begin
      W := (R.Right - R.Left) - GetThumbWidth; // div 2);
      P := Round((((X - R.Left) - (GetThumbPointerPos)) / W) * (FMax - FMin) + FMin);
    end
    else // Orientation = trVertical
    begin
      H := (R.Bottom - R.Top) - GetThumbWidth; // div 2);
...
begin
  ...
  if HandleAllocated and IsWindowVisible(Handle) and (Windows.GetFocus <> Handle) then
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        TW := Round ((R.Bottom - R.Top) / DT);

        H := (R.Bottom - R.Top) - GetThumbWidth;
      ...
      if sh and TrackHint and (P <> FHintTickIndex) then
      begin
        FHintTickIndex := P;
...
begin
  ...
  if TrackHint and not FMouseDown then
  begin
    R := TrackClientRect;

    if (Orientation = trHorizontal) then
    begin
      P := FMin + Round( (FMax - FMin) * PHI.CursorPos.X / (R.Right - R.Left));
    end
    else
    begin
      P := FMin + Round( (FMax - FMin) * PHI.CursorPos.X / (R.Right - R.Left));
...
function TCustomTrackBar.GetComponentStyle: TTMSStyle;
begin
  Result := FStyle;
end;
...
function TAdvRangeSlider.GetThumbRightSize: Integer;
begin
  Result := Round(ThumbRight.Size * DPIScale);
end;
...
begin
  ...
  begin
    ...
    begin
      R2 := SliderRect;
      Result.Top := (R2.Top + (R2.Bottom - R2.Top) div 2) - (GetThumbRightSize div 2);
      Result.Bottom := Result.Top + GetThumbRightSize;
    end
    else if (FTickMark.Style <> tsNone) then
    begin
      if (FTickMark.Position in [tmTopLeft, tmBoth]) then
      begin
        R2 := TickMarkTopRect;
        Result.Top := R2.Top + TickMark.Spacing - (GetThumbRightSize div 2);
        Result.Bottom := Result.Top + GetThumbRightSize;
      end
      else
      begin
    ...
    if (FMax <> FMin) then
    begin
      i := ((R.Right - R.Left) - GetThumbTotalWidth) * (FPositionRight - FMin) div (Fmax - FMin);

      if (GetThumbRightWidth > 0) then
      begin
        case ThumbLeft.ThumbPointer of
          ...
          begin
            case ThumbRight.ThumbPointer of
              tpLeft: i := i + GetThumbRightWidth div 2;
          ...
          begin
            case ThumbRight.ThumbPointer of
              tpLeft: i := i + GetThumbRightWidth;
              tpCenter: i := i + GetThumbRightWidth div 2;
              tpRight:;  // not allowed
            end;
          end;
        end;
    ...
    Result.Left := R.Left + i;
    Result.Right := Result.Left + GetThumbRightWidth;
  ...
  begin
    ...
    begin
      R2 := SliderRect;
      Result.Left := (R2.Left + (R2.Right - R2.Left) div 2) - (GetThumbRightSize div 2);
      Result.Right := Result.Left + GetThumbRightSize;
    end
    else if (FTickMark.Style <> tsNone) then
    begin
      if (FTickMark.Position in [tmTopLeft, tmBoth]) then
      begin
        R2 := TickMarkTopRect;
        Result.Left := R2.Left + TickMark.Spacing - (GetThumbRightSize div 2);
        Result.Right := Result.Left + GetThumbRightSize;
      end
      else
      begin
    ...
    if (FMax <> FMin) then
      i := ((R.Bottom - R.Top) - GetThumbRightWidth) * (FPositionRight - FMin) div (Fmax - FMin)
    else
      i := 0;
    Result.Bottom := R.Bottom - i;
    Result.Top := Result.Bottom - GetThumbRightWidth;
...
begin
  ...
  begin
    case ThumbRight.ThumbPointer of
      tpLeft: Result := 0;
      tpCenter: Result := (GetThumbRightWidth div 2);
      tpRight: Result := GetThumbRightWidth;
    end;
  end;
end;
...
(*
  ...
  begin
    case ThumbLeft.ThumbPointer of
      ...
      begin
        case ThumbRight.ThumbPointer of
          tpLeft: Result := Result + GetThumbRightWidth;
          tpCenter: Result := Result + GetThumbRightWidth div 2;
        end;
      end;
    end;
  end;
*)
  Result := Round(DPIScale * Result);
end;
...
function TAdvRangeSlider.GetThumbTotalWidth: integer;
begin
  Result := GetThumbWidth;

  if (ThumbLeft.ThumbPointer = tpRight) and (ThumbRight.ThumbPointer = tpLeft) then
    Result := Result + GetThumbWidth;
end;
...
begin
  ...
  begin
    ...
    begin
      i := ThumbRect.Left + (GetThumbWidth div 2);
      j := ThumbRightRect.Left + (GetThumbRightWidth div 2);
    ...
    begin
      i := ThumbRect.Top + (GetThumbWidth div 2);
      j := ThumbRightRect.Top + (GetThumbRightWidth div 2);

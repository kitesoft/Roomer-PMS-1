...
const
  ...
  // v1.8.1.2 : Fixed : Issue with sending Ctrl-key combinations
  // v1.8.1.3 : Improved : Handling of special key combinations Shift/Ctrl & Alt
  // v1.8.2.0 : New : ShowBorder, BorderColor and BorderColorDown properties
  // v1.8.2.1 : Fixed : Issue with events not called on TAdvSmoothPopupTouchKeyBoard
  // v1.8.3.0 : New : OnFocusControlChange event
  //          : Improved : Keyboard resize calculations
  // v1.8.3.1 : Fixed : Issue with designtime initialization when switching layouts in RAD Studio
  // v1.8.4.0 : New : Methods to turn capslock on and off
  // v1.8.4.1 : Fixed : Memory leak with cached keys
  // v1.8.4.2 : Fixed : Issue showing / hiding close button in combination with VCL styles
...
{TAdvSmoothTouchKeyItem}
  ...
  protected
    ...
    procedure DrawBG(g: TGPGraphics; r: TGPRectF; c, bc: TColor);
...
{TAdvSmoothTouchKeyCollection}
  ...
  public
    ...
    procedure TurnOnCapslock;
    procedure TurnOffCapslock;
  ...
  TAdvSmoothPopupTouchKeyboardFocusChangeEvent = procedure(Sender: TObject; NewFocusControl: TWinControl; var ShowKeyboard: boolean) of object;
  ...
  private
    ...
    FOnKeyDown: TKeyEvent;
    FOnKeyClick: TAdvSmoothTouchKeyEvent;
    FOnFocusControlChange: TAdvSmoothPopupTouchKeyboardFocusChangeEvent;
    procedure SetAutoCapsDisplay(const Value: Boolean);
    procedure SetAutoPostKey(const Value: Boolean);
    procedure SetHighlightAltGr(const Value: TColor);
    procedure SetHighlightCaps(const Value: TColor);
  ...
  protected
    ...
    procedure DoTextChanged(Sender: TObject; var Text: String);
    procedure DoBeforeKeyClick(Sender: TObject);
    procedure DoKeyClick(Sender: TObject; Index: integer);
    procedure DoKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
    procedure DoQueryWindowHandle(Sender: TObject; var hw: HWND);
    procedure DoKeyboardSizeChanged(Sender: TObject);
  ...
  published
    ...
    property OnFocusControlChange: TAdvSmoothPopupTouchKeyboardFocusChangeEvent read FOnFocusControlChange write FOnFocusControlChange;
...
procedure TAdvSmoothTouchKeyBoard.TurnOffCapslock;
begin
  if (GetKeyState(VK_CAPITAL) = 1) then
  begin
...
procedure TAdvSmoothTouchKeyBoard.TurnOnCapslock;
begin
...
function TAdvSmoothTouchKeyBoard.IsAltDown: boolean;
begin
  Result := FAltDown;
end;

function TAdvSmoothTouchKeyBoard.IsCtrlDown: boolean;
begin
  Result := FCtrlDown;
end;
...
procedure TAdvSmoothTouchKeyBoard.PostSpecialKeys(Key: Word; const pShift: TShiftState; SpecialKey: Boolean);
type
...
begin
  ...
  begin
    ...
    FBorderColor := (Source as TAdvSmoothTouchKeyItem).BorderColor;
    FBorderColorDown := (Source as TAdvSmoothTouchKeyItem).BorderColorDown;
    FShowBorder := (Source as TAdvSmoothTouchKeyItem).ShowBorder;
    Changed;
  end;
end;
...
begin
  ...
  begin
      if (c = FOwner.FCachedBitmaps[I].ID)
      and (bc = FOwner.FCachedBitmaps[I].IDBorder)
      and (ShowBackGround = FOwner.FCachedBitmaps[I].ShowBackground)
      and (ShowBorder = FOwner.FCachedBitmaps[I].ShowBorder)
  ...
  begin
    ...
    FOwner.FCachedBitmaps[Length(FOwner.FCachedBitmaps) - 1].IDBorder := integer(bc);
    ...
    FOwner.FCachedBitmaps[Length(FOwner.FCachedBitmaps) - 1].ShowBorder := ShowBorder;
    FOwner.FCachedBitmaps[Length(FOwner.FCachedBitmaps) - 1].ShowBackground := ShowBackGround;
    ...
    gbmp.Clear(0);
    DrawBG(gbmp, MakeRect(curDistance / 2, curDistance / 2, r.Width - curDistance, r.Height - curDistance), c, bc);
    gbmp.Free;
    chbmp := FOwner.FCachedBitmaps[Length(FOwner.FCachedBitmaps) - 1];
  end;


  if Assigned(chbmp.CachedBitmap) and (ShowBackGround or ShowBorder) then
...
procedure TAdvSmoothTouchKeyItem.DrawBG(g: TGPGraphics; r: TGPRectF; c, bc: TColor);
var
...
begin
  ...
  begin
    ...
    p := TGPPen.Create(MakeColor(200, bc));
  end
  else
  begin
    ...
    p := TGPPen.Create(MakeColor(150, bc));
  end;

  if FOwner.Shading and ShowBackground then
    g.FillPath(b, path);
  ...
  if not FOwner.FMetroStyle and FOwner.Shading and ShowBackGround then
  begin
...
procedure TAdvSmoothTouchKeyItem.SetBorderColor(const Value: TColor);
begin
...
procedure TAdvSmoothTouchKeyItem.SetBorderColorDown(const Value: TColor);
begin
...
procedure TAdvSmoothTouchKeyItem.SetShowBorder(const Value: Boolean);
begin
...
begin
  ...
  FKbd.OnTextChanged := DoTextChanged;
  FKbd.OnResize := DoKeyboardSizeChanged;
  FKbd.OnKeyClick := DoKeyClick;
  FKbd.OnQueryWindowHandle := DoQueryWindowHandle;
  FKbd.OnKeyDown := DoKeyDown;
  FKbd.OnBeforeKeyClick := DoBeforeKeyClick;
  FKbd.Align := alClient;
...
procedure TAdvSmoothPopupTouchKeyBoard.DoFocusControlChange(AControl: TWinControl; var ShowKeyboard: boolean);
begin
  if Assigned(OnFocusControlChange) then
    OnFocusControlChange(Self, AControl, ShowKeyboard);
end;

procedure TAdvSmoothPopupTouchKeyBoard.DoBeforeKeyClick(Sender: TObject);
begin
  if Assigned(OnBeforeKeyClick) then
    OnBeforeKeyClick(Sender);
end;

procedure TAdvSmoothPopupTouchKeyBoard.DoKeyClick(Sender: TObject;
  Index: integer);
begin
...
procedure TAdvSmoothPopupTouchKeyBoard.DoKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TAdvSmoothPopupTouchKeyBoard.DoKeyboardSizeChanged(Sender: TObject);
begin
...
procedure TAdvSmoothPopupTouchKeyBoard.DoQueryWindowHandle(Sender: TObject;
  var hw: HWND);
begin
  if Assigned(OnQueryWindowHandle) then
    OnQueryWindowHandle(Self, hw);
end;
...
begin
  ...
  if FFrm.ShowClose then
    FFrm.BorderIcons := [biSystemMenu]
  else
    FFrm.BorderIcons := [];
...
procedure TAdvSmoothPopupTouchKeyBoard.DoTextChanged(Sender: TObject;
  var Text: String);
begin
  if Assigned(OnTextChanged) then

...
uses
  ...
  {$ENDIF}
  SysUtils, Classes, tmsXLSAdapter, tmsUExcelAdapter, tmsUFlxUtils, StrUtils,
...
type
  ...
  (
    ...
    ///	<summary>
  ///	  Determines how (and if) hyperlinks in the grid will be exported to
  ///	  Excel.
  ///	</summary>
  THyperlinkExportMode = (
    ///	<summary>
    ...
    ///	<summary>
    ///	  The first link in the cell will be
    ///	  exported. Note that Excel doesn't allow more than 1 link per cell.
    ///	</summary>
    hleFirstLink
  );
  ...
  private
    ...
    FExportHyperlinks: THyperlinkExportMode;
  ...
  published
    ...
    ///	<summary>
    ///	  Determines how Hyperlinks in the grid will be exported to Excel.
    ///   Excel is more limited than AdvStringGrid in that an hyperlink in Excel
    ///   can only cover a full cell or not.
    ///	</summary>
    property ExportHyperlinks: THyperlinkExportMode read FExportHyperlinks write FExportHyperlinks default hleFirstLink;
  ...
  private
    ...
    procedure ExportLinks(const Workbook: TExcelFile; var w: string; const rx, cx, rreal, creal: integer);
    ...
    begin
      ...
      try
        ...
        begin
          ...
          if Anchor.Row2 + GridStartRow - XlsStartRow + 1 > CurrentGrid.RowCount then
            if Anchor.Row2 - XlsStartRow + GridStartRow + 1 > CurrentGrid.FixedRows then
              CurrentGrid.RowCount := Anchor.Row2 - XlsStartRow + GridStartRow + 1;
        end;
         ...
         if Options.ExportHiddenColumns then
          ...
          try
            ...
            try
              ...
              LinkedCell := true;
              if Options.ExportHyperlinks = hleNone then LinkedCell := false;
              if not (CurrentGrid.HasHyperlink(creal, rreal)) then LinkedCell := false;

              if LinkedCell then
              begin
                Fm.Font.Underline := fu_Single;
                Fm.Font.ColorIndex := NearestColorIndex(Workbook, CurrentGrid.URLColor, UsedColors);
              end;

              {$IFDEF DELPHI2008UP}
              if LinkedCell then ExportLinks(Workbook, w, rx, cx, rreal, creal);
              {$ENDIF}
...
procedure CheckURLShow(const w: String; out url, description: String);
begin
  url := '';
  description := '';
  if (
    AnsiStartsText('HTTP://', w) or
    AnsiStartsText('WWW.', w) or
    AnsiStartsText('MAILTO:', w) or
    AnsiStartsText('HTTPS://', w) or
    AnsiStartsText('NNTP://', w) or
    AnsiStartsText('FILE://', w) or
    AnsiStartsText('FTP://', w)
  ) then
    begin
      url := w;
      Description := w;
...
procedure TAdvGridExcelIO.ExportLinks(const Workbook: TExcelFile; var w: string; const rx, cx, rreal, creal: integer);
var
  hlinktype: THyperLinkType;
  url, description: String;
  CellRange: TXlsCellRange;
  HyperLink: THyperLink;
//  SchemaPos: integer;
begin
  CurrentGrid.GetHyperlinkDetails(creal, rreal,url,description);

  if CurrentGrid.URLShow and (url = '') then
  begin
    CheckURLShow(w, url, description);
  end;

  if CurrentGrid.URLShowInText and (url = '') then
  begin
//    SchemaPos := Pos('://', w);
//    if (SchemaPos <= 0) then SchemaPos := Pos('mailto:', w);
//    if (SchemaPos > 0) then CheckURLShowInText(w, url, description);
  end;

  if (url = '') then exit;
  if (description <> '') then w := description;
  if AnsiStartsText('file://', url) then hlinktype := hl_LocalFile else hlinktype := hl_URL;

  CellRange.Top := rx;
  CellRange.Left := cx;
  CellRange.Bottom := rx;
  CellRange.Right := cx;
  HyperLink.LinkType := hlinktype;
  HyperLink.Description := description;
  HyperLink.TargetFrame :='';
  HyperLink.TextMark := '';
  HyperLink.Text := url;
  HyperLink.Hint := '';
  Workbook.AddHyperLink(CellRange, HyperLink);
end;
...
begin
  ...
  begin
    ...
    FExportHyperlinks := (Source as TASGIOOptions).ExportHyperlinks;
  end;
end;
...
begin
  ...
  FExportHyperlinks := hleFirstLink;
end;

...
uses
  Classes, Graphics, Types, Controls, Windows, Messages
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
...
const
  ...
  // v1.0.0.2 : Fixed : Issue with URL handling on non-left aligned data values
...
begin
  ...
  taCenter:
    begin
      w := (ClientRect.Right - DataIndent) - (Result.Right - Result.Left);
      Result.Left := Result.Left + w div 2;
      Result.Right := Result.Right + w div 2;
    end;
  end;
end;
...
begin
  ...
  case Data[Index].DataAlignment of
  taRightJustify:
    begin
      w := Result.Right - Result.Left;
      Result.Right := cr.Right;
      Result.Left := cr.Right - w;
    end;
  taCenter:
    begin
      w := (cr.Right - DataIndent) - (Result.Right - Result.Left);
      Result.Left := Result.Left + w div 2;
      Result.Right := Result.Right + w div 2;
    end;
  end;

...
uses
  ...
  DB, RTTI, AdvFilterPanelButton, GDIPicture, Variants;
...
type
  ...
  private
    ...
    FOnRemovedFilter: TNotifyEvent;
    FOnRestoredFilter: TNotifyEvent;
  ...
  protected
    ...
    procedure FilterApplied; virtual;
    procedure FilterRestored; virtual;
    procedure FilterRemoved; virtual;
  ...
  public
   ...
   published
    ...
    property OnRestoredFilter: TNotifyEvent read FOnRestoredFilter write FOnRestoredFilter;
    property OnApplyFilter: TNotifyEvent read FOnApplyFilter write FOnApplyFilter;
    property OnAppliedFilter: TNotifyEvent read FOnAppliedFilter write FOnAppliedFilter;
    property OnRemoveFilter: TNotifyEvent read FOnRemoveFilter write FOnRemoveFilter;
    property OnRemovedFilter: TNotifyEvent read FOnRemovedFilter write FOnRemovedFilter;
  end;
...
procedure TAdvCustomFilterPanel.FilterApplied;
begin
  if Assigned(OnAppliedFilter) then
    OnAppliedFilter(Self);
end;

procedure TAdvCustomFilterPanel.FilterRemoved;
begin
  if Assigned(OnRemovedFilter) then
    OnRemovedFilter(Self);
end;

procedure TAdvCustomFilterPanel.FilterRestored;
begin
  if Assigned(OnRestoredFilter) then
    OnRestoredFilter(Self);
end;
...
begin
  ...
  begin
    ...
    // covers more date format conversions
    Date := VarToDateTime(AText);
  end;
end;
...
begin
  ...
  begin
    FApplyButton := (Source as TFilterUI).ApplyButton;
    FApplyDialogText := (Source as TFilterUI).ApplyDialogText;
    FCaseHint := (Source as TFilterUI).CaseHint;
    FCaseLabel := (Source as TFilterUI).CaseLabel;
    FClearButton := (Source as TFilterUI).ClearButton;
    FClearDialogText := (Source as TFilterUI).ClearDialogText;
    FColumnHint := (Source as TFilterUI).ColumnHint;
    FColumnLabel := (Source as TFilterUI).ColumnLabel;
    FHintApplyFilter := (Source as TFilterUI).HintApplyFilter;
    FHintClearFilter := (Source as TFilterUI).HintClearFilter;
    FHintFilterAdd := (Source as TFilterUI).HintFilterAdd;
    FHintRestoreFilter := (Source as TFilterUI).HintRestoreFilter;
    FOperationBeginsWith := (Source as TFilterUI).OperationBeginsWith;
    FOperationContains := (Source as TFilterUI).OperationContains;
    FOperationEndsWith := (Source as TFilterUI).OperationEndsWith;
    FOperationEqual := (Source as TFilterUI).OperationEqual;
    FOperationHint := (Source as TFilterUI).OperationHint;
    FOperationLabel := (Source as TFilterUI).OperationLabel;
    FOperationLargerOrEqual := (Source as TFilterUI).OperationLargerOrEqual;
    FOperationLargerThen := (Source as TFilterUI).OperationLargerThen;
    FOperationNotEqual := (Source as TFilterUI).OperationNotEqual;
    FOperationSmallerOrEqual := (Source as TFilterUI).OperationSmallerOrEqual;
    FOperationSmallerThen := (Source as TFilterUI).OperationSmallerThen;
    FOperationTrueFalse := (Source as TFilterUI).OperationTrueFalse;
    FOperatorAnd := (Source as TFilterUI).OperatorAnd;
    FOperatorOr := (Source as TFilterUI).OperatorOr;
    FRestoreButton := (Source as TFilterUI).RestoreButton;
    FValueLabel := (Source as TFilterUI).ValueLabel;
    FValueTextHint := (Source as TFilterUI).ValueTextHint;
  end;
end;
...
procedure TAdvCustomGridFilterDialog.ExecuteModal;
begin
  FForm := CreateFilterForm;
  FForm.FilterPanel.UI.Assign(UI);
  FForm.OnApplyFilter := DoApplyFilter;
...
procedure TAdvCustomGridFilterDialog.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

...
uses
  ...
  AdvTypes, Types, BaseGrid, PictureContainer, AdvObj;
...
begin
  ...
  begin
    ...
    begin
      ...
      pgb[Length(pgb) - 1] := CellRange(0, pi, g.ColCount - 1, I - 1);
      pi := I;
    end;
  end;

  if pi < g.RowCount - 1 then
  begin
    SetLength(pgb, Length(pgb) + 1);
    pgb[Length(pgb) - 1] := CellRange(0, pi, g.ColCount - 1, g.RowCount - 1);
  end;
...
var
  ...
  begin
    ...
    for I := 0 to CurrentGrid.FixedCols - 1 do
      Result := Result + CurrentGrid.ColWidths[I] * sc;
  end;
  ...
  begin
    ...
    begin
      w := 0;
      for K := 0 to CurrentGrid.ColCount - 1 do
        w := w + Round(CurrentGrid.ColWidths[K]);
...
begin
  ...
  begin
    if (CompareValue(Floor(FSaveXBounds + CurrentGrid.ColWidths[I] * sc), w - Options.Margins.Right) = GreaterThanValue) then
      Break;

    FSaveXBounds := FSaveXBounds + CurrentGrid.ColWidths[I] * sc;
  end;
  ...
  begin
    ...
    begin
      ...
      chk := (c = bc) and (r = br);
      if chk then
        AMergeCellPrintPageNr[bc, br] := APDFLib.GetPageIndex
      else if (bc <> c) and (br <> r) then
        chk := AMergeCellPrintPageNr[bc, br] <> APDFLib.GetPageIndex;
      ...
      begin
        ...
        for I := bc to bc + cs - 1 do
          cwrt := cwrt + CurrentGrid.ColWidths[I] * sc;
        ...
        xr := 0;
        for I := bc to c - 1 do
          xr := xr + CurrentGrid.ColWidths[I] * sc;
    ...
    if (CompareValue(Floor(x + CurrentGrid.ColWidths[c] * sc) , w - Options.Margins.Right) = GreaterThanValue) or (c = ASelection.EndCol + 1) then
    begin
      FSaveXBounds := x;

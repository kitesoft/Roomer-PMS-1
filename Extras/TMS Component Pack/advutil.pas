...
type
  ...
  procedure DrawComboButton(Canvas: TCanvas;Handle: THandle; ARect: TRect; AControlStyle: TControlDrawStyle; AState: TControlState; ATones: TColorTones; DPIScale: single);
  ...
  function ToFixedDecSep(s: string): string;
  function FromFixedDecSep(s: string): string;
  function StripThousandSeparator(s: string): string;
...
begin
  ...
  r := MaxI(0,round(r + r * Perc /100));
  g := MaxI(0,round(g + g * Perc /100));
  b := MaxI(0,round(b + b * Perc /100));
...
function StripThousandSeparator(s: string): string;
begin
  Result := StripThousandSep(PChar(s));
end;

function StrUnquotedPos(s: pchar; sp: char): pchar;
var
  q: integer;
begin
  result := nil;
...
begin
  ...
  begin
    ...
    while ((s2^ <> ' ') or odd(q)) and (s2^ <> NULL) and (odd(q) or ((s2^ <> '&') and (s2^ <> '|')))  do
    begin
      if (s2^= '"') then
        inc(q);
      ...
      if (s2^= ' ') and not odd(q) then // skip space if between quotes
      begin
        lastchar := s2^;
        inc(s2);
      end;
  ...
  s2 := strunquotedpos(s0a,negation);
  ...
  if (flag3) then
  begin
    if strunquotedpos(s0a,larger) = nil then
      flag1 := flag3;
    if strunquotedpos(s0a,smaller) = nil then
      flag2 := flag3;
  end;

  if (strunquotedpos(s0a,logor) <> nil) then
  ...
  if (strunquotedpos(s0a,logand)<>nil) then
  ...
  if ((strunquotedpos(s0a,larger) <> nil) and (oklarger)) or
     ((strunquotedpos(s0a,smaller) <> nil) and (oksmaller)) then
  begin
    matches := True;
    Exit;
  end;
...
begin
  ...
  for i := 1 to Length(s) do
  begin
    if not quot and (s[i] = p) then
      Result := Result + 1;

    if quot and (s[i] = p) then
      q := q + 1;

    if not quot and (s[i] = '"') then
    begin
      quot := true;
...
begin
  ...
  begin
    ...
    if Direction then
      Canvas.Rectangle(R.Left + stepw,R.Top,MinI(R.Right, R.Left + stepw + Trunc(1 + rstepw)),R.Bottom)
    else
      Canvas.Rectangle(R.Left,R.Top + stepw,R.Right,MinI(R.Bottom, R.Top + stepw + Trunc(1 + rstepw)));
  end;
end;
...
var
  ...
  pt: Array of TPoint;
begin
  ARect.Left := ARect.Right - Round(17 * DPIScale);

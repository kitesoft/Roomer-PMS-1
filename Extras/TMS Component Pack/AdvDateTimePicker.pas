...
const
  ...
  // v1.2.2.1 : Improved : Handling of custom input when ParseInput = true
  // v1.2.2.2 : Fixed : Regression with ParseInput handling improvement
  // v1.2.3.0 : Improved : High DPI support
  // v1.2.3.1 : Fixed : Issue with setting font size
  // v1.2.3.2 : Improved : Handling of null dates
  // v1.2.4.0 : New : NullDateDate & NullDateFormat properties added
  // v1.2.4.1 : Fixed : Issue with programmatically setting DateTime from the OnEnter event
  // v1.2.4.2 : Fixed : Issue in combination with TAdvFocusHelper
  // v1.2.5.0 : New : MinTime, MaxTime properties added
  // v1.3.0.0 : New : OnCheckClick event added
...
type
  ...
  private
    ...
    FNullDateFormat: String;
    FNullDateDate: TDateTime;
    FOriginalFormat: string;
    FInternalChangedFlag: boolean;
    FOldCheck: boolean;
    FMaxTime: TTime;
    FMinTime: TTime;
    ...
    procedure CheckAndActivateFormat;
    procedure ResetToOriginalDateFormat;
  protected
    ...
    procedure DoCheckClicked; virtual;
    ...
    procedure SetDateTimeInt(const Value: TDateTime);
    ...
    procedure TimePickerExit(Sender: TObject);
  ...
  published
    ...
    property MinTime: TTime read FMinTime write FMinTime;
    property MaxTime: TTime read FMaxTime write FMaxTime;
    ...
    property NullDateDate: TDateTime read FNullDateDate write FNullDateDate;
    property NullDateFormat: String read FNullDateFormat write FNullDateFormat;
...
function CalculateDPIScale: single;
var
  FHDC: HDC;
  FDPI: integer;
...
begin
  ...
  begin
    ...
    FTimePicker.OnExit := TimePickerExit;
...
{$IFDEF DELPHI_UNICODE}
procedure TAdvDateTimePicker.ChangeScale(M, D: Integer);
begin
  inherited;
...
begin
  ...
  begin
    FLblUpdate := true;
    LabelFont.Height := Round(FLblFntHeight * CalculateDPIScale);
    FLblUpdate := false;
  end;
...
begin
  ...
  begin
    ...
    FTimePicker.DateTime := Int(Now) + Frac(Self.DateTime);
...
procedure TAdvDateTimePicker.DoCheckClicked;
begin
  if Assigned(OnCheckClick) then
    OnCheckClick(Self);
end;

procedure TAdvDateTimePicker.DoEnter;
begin
  FInternalChangedFlag := false;
  if DateTime = FNullDateDate then
  begin
    SetDateTimeInt(SysUtils.Date);
    FInternalChangedFlag := true;
  end;
  ResetToOriginalDateFormat;
  inherited DoEnter;
end;

procedure TAdvDateTimePicker.DoExit;
begin
  if FInternalChangedFlag then
  begin
    FInternalChangedFlag := false;
    SetDateTimeInt(FNullDateDate);
  end;
  CheckAndActivateFormat;
  inherited DoExit;
end;
...
procedure TAdvDateTimePicker.WMKeyUp(var Message: TWMKeyDown);
begin
  inherited;
  if (Message.CharCode = VK_DELETE) and (GetKeyState(VK_CONTROL) and $8000 = $8000) and (NullDateFormat <> '') then
    DateTime := 0;
end;
...
begin
  ...
  begin
    ...
    begin
      if frac(DateTime) < frac(MinTime) then
        DateTime := frac(MinTime);

      if frac(DateTime) > frac(MaxTime) then
        DateTime := frac(MaxTime);
    end;
  end;
end;
...
procedure TAdvDateTimePicker.CheckAndActivateFormat;
begin
  if (FNullDateDate = DateTime) and (FNullDateFormat <> '') then
    Format := FNullDateFormat
  else
    ResetToOriginalDateFormat;
end;
...
procedure TAdvDateTimePicker.SetDateTimeInt(const Value: TDateTime);
var
  ho,mi,se,se100:word;
  v: TDateTime;
begin
  ...
  if (MinDate <> 0) and (v < MinDate) then
    v := MinDate;

  if (MaxDate <> 0) and (v > MaxDate) then
    v := MaxDate;

  if (int(v) = 0) and FNullDateIsToday then
    inherited DateTime := int(Now) + + EncodeTime(ho,mi,se,0)
  else
    inherited DateTime := int(v) + EncodeTime(ho,mi,se,0);

  if Assigned(FTimePicker) then
    FTimePicker.DateTime := int(Now) + EncodeTime(ho,mi,se,0);
...
begin
  if Value <> FNullDateFormat then
    FOriginalFormat := Value;
...
begin
  ...
  if (MinTime <> MaxTime) then
  begin
    if frac(FTimePicker.Time) < frac(MinTime) then
      FTimePicker.Time := frac(MinTime);

    if frac(FTimePicker.Time) > frac(MaxTime) then
      FTimePicker.Time := frac(MaxTime);
  end;
...
procedure TAdvDateTimePicker.ResetToOriginalDateFormat;
begin
  Format := FOriginalFormat;
end;
...
procedure TAdvDateTimePicker.TimePickerExit(Sender: TObject);
begin
  Invalidate;
end;

...
///  s := s + '     AND (roomreservations.Departure =  ' + _db(adate,true) + ') '+#10;
  ...
  ' SELECT RR_Arrival(roomreservation, true) as Arrival'+
  ' FROM roomreservations  '+
...
//  s := s+'  order by room, adate '#10;
  ...
  s := s+'       (  ({roomreservation} = 0 and rr.Reservation={reservation})   -- for groupinvoice: all records from reservation ' +#10;
  s := s+'          -- or for roominvoice: gather roomreservations from this roomreservation and from roomsdate records that will be paidBy this roomreservation ' +#10;
  s := s+'          or ( {roomreservation} <> 0 and rd.RoomReservation IN (SELECT DISTINCT rd2.RoomReservation ' +#10;
  s := s+'                                                                  FROM roomsdate rd2 ' +#10;
  s := s+'                                                                  JOIN roomreservations rr ON (rr.Reservation={reservation} and rr.RoomReservation=rd2.RoomReservation) or rd2.paidBy=rr.roomreservation ' +#10;
  s := s+'                                                                  WHERE rr.GroupAccount = 0 ' +#10;
  s := s+'                                                                  AND (rd2.PaidBy={roomreservation} OR (rd2.RoomReservation={roomreservation} AND rd2.PaidBy=0)) ' +#10;
  s := s+'                                                                 ) ' +#10;
  s := s+'             ) ' +#10;
  s := s+'        ) ' +#10;
  s := s+'          AND ({invoiceindex}=-1 or coalesce(rd.invoiceindex, rr.invoiceindex)= {invoiceindex}) ' +#10;
  s := s+'          AND rd.ResFlag NOT IN (''X'', ''C'') ' +#10;
  s := s+'          AND not rd.Paid ' +#10;
  s := s+'          AND ({roomreservation} <> 0 or rr.GroupAccount=1) ' +#10;
  s := s+'  order by room, adate '#10;

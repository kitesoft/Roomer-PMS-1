...
begin
  ...
  try
    ...
    b.Append('  , ' + _db(ord(theData.Breakfast)) + #10);
...
begin
  ...
  s := s + '  , ' + _db(ord(theData.Breakfast)) + #10;
...
begin
  ...
  try
    ...
    sql := ' SELECT '#10 + '     Room '#10 + '   , RoomReservation '#10 + '   , Resflag '#10 + '   , ADate '#10 + ' FROM '#10 + '   roomsdate '#10 +
      ' WHERE '#10 + '   (Room = %s)  and (ADate = %s) ' + '   AND (ResFlag <> ' + _db(STATUS_DELETED) + ' ) ' + // **zxhj b�tti vi�
      ' LIMIT 1 ';
...
//  debugmessage(s);
  ...
  try
    ...
    begin
      ...
      result.PROBE_DATE         := RSet.FieldByName('PROBE_DATE').AsString;
...
begin
  ...
  s := s + '   , PROBE_DATE =' + _db(theData.Probe_date);
...
begin
  ...
  try
    ...
    sql := ' SELECT RoomType FROM roomsdate ' + ' WHERE (RoomType = %s ) ' + '   AND (ResFlag <> ' + _db(STATUS_DELETED) + ' ) '; // **zxhj b�tti vi�
